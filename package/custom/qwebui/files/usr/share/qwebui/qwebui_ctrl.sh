...
get_ip(){
    ...
    if [ -z "$ip_addr_v4" ] && [ -z "$ip_addr_v6" ]; then
        ip_addr_v4=$(ip addr show ${modem_network}.1 | grep 'inet ' | grep -v '127.0.0.1' | awk '{print $2}' | cut -d'/' -f1 | head -1)
        ip_addr_v6=$(ip addr show ${modem_network}.1 | grep 'inet6 ' | grep 'scope global' | awk '{print $2}' | cut -d'/' -f1 | head -1)
    fi
