...
#define TESTMODE_GET_PARAM(_ad, _band, _member) CONFIG_GET_PARAM(net_ad_wrap_service(_ad), _member)
#define TESTMODE_GET_PADDR(_ad, _band, _member) CONFIG_GET_PADDR(net_ad_wrap_service(_ad), _member)
#define TESTMODE_SET_PARAM(_ad, _band, _member, _val) CONFIG_SET_PARAM(net_ad_wrap_service(_ad), _member, _val)
#else
...
#define TESTMODE_GET_PARAM(_ad, _band, _member) (_ad->ATECtrl._member)
#define TESTMODE_GET_PADDR(_ad, _band, _member) (&_ad->ATECtrl._member)
...
#if !defined(COMPOS_TESTMODE_WIN)/* for MT_testmode.c */
INT MT_ATERxDoneHandle(struct _RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);
#endif
...
INT MtATE_Group_Pre_Cal_Store_Proc_Bellwether(
	RTMP_ADAPTER *pAd, UINT8 op);
INT MtATE_DPD_Cal_Store_Proc_Bellwehter(
	RTMP_ADAPTER *pAd, UINT8 op);
INT MtATE_DNL_Cal_Store_Proc_Bellwether(
	RTMP_ADAPTER *pAd,
	RTMP_STRING * arg);
...
INT ate_tx_pkt_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk);
