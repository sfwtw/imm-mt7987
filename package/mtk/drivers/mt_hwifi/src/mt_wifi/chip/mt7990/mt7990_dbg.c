...
static struct bmac_empty_queue_info ple_queue_empty_info[] = {
	...
	{"ALTX Q2", ENUM_UMAC_LMAC_PORT_2,    0x1C, 2},
	{"BMC Q2",  ENUM_UMAC_LMAC_PORT_2,    0x1D, 2},
	{"BCN Q2",  ENUM_UMAC_LMAC_PORT_2,    0x1E, 2},
	{"PSMP Q2", ENUM_UMAC_LMAC_PORT_2,    0x1F, 2},
...
static EMPTY_QUEUE_INFO_T ple_txcmd_queue_empty_info[3][32] = {
...
static INT32 chip_show_agg_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	band_idx = hc_get_hw_band_idx(pAd);
		...
		for (idx = 0; idx < 0xF ; idx++) {
			UINT32 value0 = 0, value1 = 0;

			RTMP_IO_WRITE32(pAd->hdev_ctrl, WF_WTBLON_TOP_ITCR_ADDR, (0x81000000 + idx));
			RTMP_IO_READ32(pAd->hdev_ctrl, WF_WTBLON_TOP_ITDR0_ADDR, &value0);
			RTMP_IO_READ32(pAd->hdev_ctrl, WF_WTBLON_TOP_ITDR1_ADDR, &value1);
			MTWF_PRINT("Winsize%X limit = %d, %d\n", idx, value0, value1);
		}
	...
	struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	UINT32 rvsr0 = 0, rscr26 = 0, rscr35 = 0, mctr5 = 0, mctr6 = 0, msr0 = 0,
		   msr1 = 0, msr2 = 0;
	UINT32 tbcr0 = 0, tbcr1 = 0, tbcr2 = 0, tbcr3 = 0, tbcr4 = 0;
	UINT32 btscr[7] = {};
	UINT32 tdrcr[5] = {};
	...
	UINT32 mu_cnt[5] = {};
	UINT32 ampdu_cnt[3] = {};
	UINT32 mpdu_retry_drop_cnt = 0;
	UINT32 rts_drop_cnt = 0;
	ULONG per = 0;
	...
	RTMP_IO_READ32(pAd->hdev_ctrl,
		BN0_WF_MIB_TOP_TSCR15_ADDR + band_offset, &mpdu_retry_drop_cnt);
	RTMP_IO_READ32(pAd->hdev_ctrl,
		BN0_WF_MIB_TOP_TSCR16_ADDR + band_offset, &rts_drop_cnt);
	...
	RTMP_IO_READ32(pAd->hdev_ctrl,
		BN0_WF_MIB_TOP_TBCR4_ADDR + band_offset, &tbcr4);
	MTWF_PRINT("\tTx 320MHz Cnt=0x%x\n",
	...
	MTWF_PRINT("\tMPDU Retry Drop Cnt=0x%x\n", mpdu_retry_drop_cnt);
	MTWF_PRINT("\tRTS Drop Cnt=0x%x\n", rts_drop_cnt);
...
static INT32 chip_show_pse_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, WF_PSE_TOP_PG_MDP3_GROUP_ADDR, &pg_flow_ctrl[24]);
	HW_IO_READ32(pAd->hdev_ctrl, WF_PSE_TOP_MDP3_PG_INFO_ADDR, &pg_flow_ctrl[25]);
	...
	MTWF_PRINT("\tReserved page counter of MDP3 group: 0x%08x\n", pg_flow_ctrl[24]);
	MTWF_PRINT("\tMDP3 group page status: 0x%08x\n", pg_flow_ctrl[25]);
	...
	MTWF_PRINT("\t\tThe max/min quota pages of MDP3 group=0x%03x/0x%03x\n", max_q, min_q);
	...
	MTWF_PRINT("\t\tThe used/reserved pages of MDP3 group=0x%03x/0x%03x\n", used_pg, rsv_pg);
	/* Queue Empty Status */
								  ...
								  UINT32 dumptxd)
{
	...
	for (band_idx = 0; band_idx < MAX_BAND_NUM ; band_idx++) {
		...
		case 1:
			...
			break;
		case 2:
			TXCMD_STAT_ADDR = WF_PLE_TOP_BN2_TXCMD_QUEUE_EMPTY_ADDR;
			NATIVE_TXCMD_STAT_ADDR = WF_PLE_TOP_BN2_NATIVE_TXCMD_QUEUE_EMPTY_ADDR;
			break;
		default:
			return;
		}
...
static INT32 chip_show_ple_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	if (DRR_QUERY_PAUSE_BY_CMD(pAd) && (!QueryCR))
		...
		if (ppause_bitmap && ppause_bitmap->pause && ppause_bitmap->twt_pause) {
			...
			chip_get_sta_pause_by_band(pAd, &(ppause_bitmap->pause[2][0]), &(ppause_bitmap->twt_pause[2][0]), 2);
		}
	}
...
{
	/* SN is not be use in FW side for BA_SESSION_ORI case (BA TX), so set it to 0 */
	AsicUpdateBASession(pAd, wtbl_idx, tid, 0, (UINT16)win_size, TRUE, BA_SESSION_ORI, 1);
...
static VOID chip_set_hw_amsdu(RTMP_ADAPTER *pAd, UINT32 wcid, uint8_t num, UINT32 len)
{
	...
	UniCmdMecParam.mec_amsdu_para_sta_t.u2WlanIdx = (UINT16)wcid;
	UniCmdMecParam.mec_amsdu_para_sta_t.u1AmsduEn = 1;
	UniCmdMecParam.mec_amsdu_para_sta_t.u1AmsduNum = (UINT8)num;
	UniCmdMecParam.mec_amsdu_para_sta_t.u2AmsduLen = (UINT16)len;
	UniCmdMecParam.MecTagValid[UNI_CMD_MEC_AMSDU_PARA_STA] = TRUE;

	UniCmdMecCtrl(pAd, &UniCmdMecParam);
}

static VOID chip_set_header_translation(RTMP_ADAPTER *pAd, UINT32 wcid, uint8_t RHTR)
{
	if (RHTR)
		UniCmdStaRecWTBLHeaderTrans(pAd, wcid, 0);	/* enable header translation */
	else
		UniCmdStaRecWTBLHeaderTrans(pAd, wcid, 1);	/* disable header translation */
}
...
static VOID chip_show_dma_info(struct hdev_ctrl *ctrl)
{
	...
	if (IS_RBUS_INF(pAd) || IS_PCI_INF(pAd) || IS_HWIFI_INF(pAd)) {
		...
		dump_dma_rx_ring_info(ctrl, "R6:BUF1(MAC2H)", "Both",
		...
		dump_dma_rx_ring_info(ctrl, "R9:TxDone0(MAC2H)", "Both",
			WF_WFDMA_HOST_DMA0_WPDMA_RX_RING9_CTRL0_ADDR);
		dump_dma_rx_ring_info(ctrl, "R10:MSDU_PG0(MAC2H)", "Both",
			WF_WFDMA_HOST_DMA0_WPDMA_RX_RING10_CTRL0_ADDR);
		dump_dma_rx_ring_info(ctrl, "R11:MSDU_PG1(MAC2H)", "Both",
			WF_WFDMA_HOST_DMA0_WPDMA_RX_RING11_CTRL0_ADDR);
		dump_dma_rx_ring_info(ctrl, "R12:MSDU_PG2(MAC2H)", "Both",
			WF_WFDMA_HOST_DMA0_WPDMA_RX_RING12_CTRL0_ADDR);
		dump_dma_rx_ring_info(ctrl, "IND:IND_CMD(MAC2H)", "Both",
											   ...
											   uint8_t spe_idx)
{
	...
	if (tx_mode == MODE_HE_TRIG) {
		...
		MTWF_PRINT("mt7990: %s, check correct cr settings\n",
			__func__);
	}
...
VOID ShowCpuUtilSum(RTMP_ADAPTER *pAd)
{
	...
	/* MT7990 TODO: check correct cr setting */
...
{
	UINT32 i, offset = 0;
	UINT32 macVal = 0, macVal_1 = 0;
	ULONG  now = 0;

	if (mcu_type == WA_LOG_OUTPUT) {
		offset = CONN_MCU_WA_EXCP_CON_BASE - CONN_MCU_EXCP_CON_BASE;
	...
	HW_IO_WRITE32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBGOUT_SEL_WF_MCU_DBGOUT_SEL_ADDR,
		macVal);

	for (i = 0; i < count; i++) {
		macVal = 0x3f;
		HW_IO_WRITE32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBG_PC_LOG_SEL_WF_MCU_DBG_PC_LOG_SEL_ADDR,
			macVal);
		HW_IO_READ32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBG_PC_LOG_WF_MCU_DBG_PC_LOG_ADDR,
			&macVal);
		MTWF_PRINT("Current PC=%x\n", macVal);
	}

	if (mcu_type == WM_LOG_OUTPUT) {
		macVal = 0x20;
		HW_IO_WRITE32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBG_PC_LOG_SEL_WF_MCU_DBG_PC_LOG_SEL_ADDR,
			macVal);
		HW_IO_READ32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBG_PC_LOG_WF_MCU_DBG_PC_LOG_ADDR,
			&macVal);
		MTWF_PRINT("PC_LOG_SEL=>6'h20, %x\n", macVal);
		...
		HW_IO_READ32(pAd->hdev_ctrl, CONN_DBG_CTL_WF_MCU_DBG_PC_LOG_WF_MCU_DBG_PC_LOG_ADDR,
			&macVal);
		MTWF_PRINT("PC_LOG_SEL=>6'h21, sw_dbg_ctrl=%x\n", macVal);
	...
	macVal = 0;
	macVal_1 = 0;
	HW_IO_READ32(pAd->hdev_ctrl, CONN_MCU_EXCP_CON_DBG_PC_LOG_CTL_ADDR + offset, &macVal);
	HW_IO_READ32(pAd->hdev_ctrl,
		CONN_MCU_EXCP_CON_DBG_PC_LOG_CTL_RO_DBG_PC_LOG_IDX_STATUS_ADDR + offset, &macVal_1);

	macVal_1 = (macVal_1 & CONN_MCU_EXCP_CON_DBG_PC_LOG_CTL_RO_DBG_PC_LOG_IDX_STATUS_MASK)
		>> CONN_MCU_EXCP_CON_DBG_PC_LOG_CTL_RO_DBG_PC_LOG_IDX_STATUS_SHFT;

	MTWF_PRINT("PC log contorl=%x(T=%lu)(latest PC index = 0x%x)\n",
		macVal, now, macVal_1);

	for (i = 0; i < 32; i++) {
		HW_IO_READ32(pAd->hdev_ctrl, CONN_MCU_EXCP_CON_DBG_PC_LOG_0_ADDR + (i * 4) +
			offset, &macVal);
	...
	/* LR log */
	NdisGetSystemUpTime(&now);
	HW_IO_READ32(pAd->hdev_ctrl, CONN_MCU_EXCP_CON_DBG_GPR_LOG_CTL_ADDR + offset, &macVal);
	HW_IO_READ32(pAd->hdev_ctrl, CONN_MCU_EXCP_CON_DBG_GPR_LOG_CTL_RO_DBG_GPR_LOG_IDX_STATUS_ADDR
			+ offset, &macVal_1);

	macVal_1 = (macVal_1 & CONN_MCU_EXCP_CON_DBG_GPR_LOG_CTL_RO_DBG_GPR_LOG_IDX_STATUS_MASK)
		>> CONN_MCU_EXCP_CON_DBG_GPR_LOG_CTL_RO_DBG_GPR_LOG_IDX_STATUS_SHFT;

	MTWF_PRINT("\nLR log contorl=%x(T=%lu)(latest LR index = 0x%x)\n",
		macVal, now, macVal_1);

	for (i = 0; i < 32; i++) {
		HW_IO_READ32(pAd->hdev_ctrl, CONN_MCU_EXCP_CON_DBG_GPR_LOG_0_ADDR + (i * 4)
			+ offset, &macVal);
...
{
	UINT32 coredump_occur = 0,  macVal = 0, addr = 0x00400000, end_addr  = 0, dump_size = 0;
	struct _RTMP_CHIP_DBG *chip_dbg = hc_get_chip_dbg(pAd->hdev_ctrl);

	RTMP_IO_READ32(pAd->hdev_ctrl, 0x02209e90, &coredump_occur);
	if (coredump_occur == EXP_DUMP_PUTTING_DONE && pAd->bIsBeenDumped == FALSE) {
		...
		RTMP_IO_READ32(pAd->hdev_ctrl, 0x02209e9C, &dump_size);
...
{"ULM0_DUMP.bin", 0x00800000, 0x00050000},
{"ULM1_DUMP.bin", 0x00900000, 0x00038000},
{"ULM2_DUMP.bin", 0x02200000, 0x00040000},
{"fw_dump_wmcpu.cmm", 0x00400000, 0x00068000},
{"CRAM_DUMP.bin", 0xE0000000, 0x0027c000},
{NULL,}
};

static struct _COREDUMP_INFO WA_COREDUMP_INFO[] = {
{"WACPU_CRAM.bin", 0xE0000000, 0x00010000},
{"WACPU_CRAM2.bin", 0xE0010000, 0x00011800},
...
static INT32 chip_show_coredump_proc(struct _RTMP_ADAPTER *pAd, UINT mcu_type)
{
	...
	if (mcu_type == WA_LOG_OUTPUT) {
		RTMP_IO_READ32(pAd->hdev_ctrl, 0x7C05B080, &ctrl_info_ram_addr);
...
VOID mt7990_chip_dbg_init(struct _RTMP_CHIP_DBG *dbg_ops)
{
	...
	dbg_ops->show_mac_cap = hwifi_show_mac_cap;
}
