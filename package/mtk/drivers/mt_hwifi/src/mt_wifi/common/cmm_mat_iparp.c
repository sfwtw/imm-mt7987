	...
	IN int			index)
{
	...
	for (; startIdx < endIdx; startIdx++) {
		...
		RtmpHashListForEachSafe(pEntry, pTmp, &pIPMacTable->hash[startIdx], list) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_INFO, "IPMac[%d]:\n", startIdx);
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_INFO, "\t:IP=0x%x,Mac="MACSTR", lastTime=0x%lx\n",
					 pEntry->ipAddr, MAC2STR(pEntry->macAddr), pEntry->lastTime);
		}
	}
	...
	IN UINT				ipAddr)
{
	UINT				hashIdx;
	IPMacMappingTable *pIPMacTable;
	IPMacMappingEntry *pEntry, *pNewEntry;
	RTMP_HLIST_NODE *pTmp;
	ULONG			now;
	...
	RtmpHashListForEachSafe(pEntry, pTmp, &pIPMacTable->hash[hashIdx], list) {
	...
	if (pNewEntry != NULL) {
		...
		RtmpHashListAddHead(&(pNewEntry->list), &pIPMacTable->hash[hashIdx]);
	...
	IN MAT_STRUCT *pMatCfg)
{
	...
	if (pIPMacTable->valid) {
		...
		for (i = 0; i < MAT_MAX_HASH_ENTRY_SUPPORT; i++) {
			...
			RtmpHashListForEachSafe(pEntry, pTmp, &pIPMacTable->hash[i], list) {
	...
	IN MAT_STRUCT *pMatCfg)
{
	...
	if (pMatCfg->MatTableSet.IPMacTable != NULL)
		...
		if (pMatCfg->MatTableSet.IPMacTable) {
			...
			for (i = 0; i < MAT_MAX_HASH_ENTRY_SUPPORT; i++)
				RtmpHashHeadInit(&pIPMacTable->hash[i]);
	...
	IN ULONG BufLen)
{
	...
	for (; startIdx <  endIdx; startIdx++) {
		...
		RtmpHashListForEachSafe(pEntry, pTmp, &pIPMacTable->hash[startIdx], list) {
			...
			IPintToIPstr(pEntry->ipAddr, Ipstr, sizeof(Ipstr));
			ret = snprintf(pOutBuf + strlen(pOutBuf),
					BufLen, "%-18s%02x:%02x:%02x:%02x:%02x:%02x\n",
					Ipstr, pEntry->macAddr[0], pEntry->macAddr[1], pEntry->macAddr[2],
