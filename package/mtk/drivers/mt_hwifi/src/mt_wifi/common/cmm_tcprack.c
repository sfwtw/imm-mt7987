...
#define CNX_DPRINT(pAd, cnx, string)                           \
	do {                                                    \
		...
		if ((cnx) == NULL) {                               \
			MTWF_PRINT("[rack] cnx is NULL (%s)\n", string);   \
			break;                                         \
		}                                                  \
		MTWF_PRINT("[cnx] %u.%u.%u.%u:%u --> %u.%u.%u.%u:%u, seq=%u, rel_seq=%u, ack=%u, rel_ack=%u, win=%u, bif=%u (%s)\n",\
...
#define PKT_DPRINT(pAd, pkt, string)                           \
	do {                                                    \
		...
		if ((pkt) == NULL) {                               \
			MTWF_PRINT("[pkt] pkt is NULL (%s)\n", string);   \
		}                                                  \
		MTWF_PRINT("[pkt] %u.%u.%u.%u:%u --> %u.%u.%u.%u:%u, len=%u, opt_len=%u, flags=%x (%s)\n",\
			   ((pkt)->sip >> 24) & 0xFF,                 \
			   ((pkt)->sip >> 16) & 0xFF,                 \
			   ((pkt)->sip >> 8) & 0xFF,                  \
			   ((pkt)->sip >> 0) & 0xFF,                  \
...
#define MY_DPRINT(format, ...)                               \
	do {                                                 \
		MTWF_PRINT("[rack] "format, ##__VA_ARGS__);          \
	} while (0)
#else
...
#define MY_LOCK(pAd) do {OS_SEM_LOCK(&pAd->ReduceAckLock); } while (0)
#define MY_UNLOCK(pAd) do {OS_SEM_UNLOCK(&pAd->ReduceAckLock); } while (0)
...
static VOID rack_show_ex(PRTMP_ADAPTER   pAdapter)
{
...
#endif /* REDUCE_ACK_PKT_CACHE */
	{
		...
		list_for_each_entry(cnx, &(pAdapter->ackCnxList), list) {
			...
			MTWF_PRINT("(conn#%02d) %u.%u.%u.%u:%u --> %u.%u.%u.%u:%u (%u)\n",
				   ...
				   cnx->tuple.dport,
				   cnx->state);
			MTWF_PRINT("    dropped/total/sent ack count = %u/%u/%u\n", cnx->stats.dropped, cnx->stats.total, cnx->stats.total - cnx->stats.dropped);
			MTWF_PRINT("    total data count = %u\n", cnx->stats.total_data);
			MTWF_PRINT("    window size = %u\n", cnx->wsize);
			MTWF_PRINT("    BIF = %u (max: %u, avg: %u)\n", cnx->bif, cnx->max_bif, cnx->avg_bif);
			MTWF_PRINT("    MSS = %u\n", cnx->mss);
			MTWF_PRINT("    last received ack timestamp = %lu\n", cnx->last_tstamp);
			MTWF_PRINT("    last received FIN timestamp = %lu\n", cnx->fin_tstamp);
			MTWF_PRINT("    ACK ratio = ~%u:1\n", cnx->ack_ratio);
			MTWF_PRINT("    retransmission count = %u\n", cnx->stats.retrans);
			MTWF_PRINT("    duplicate ack count = %u\n", cnx->stats.dupack);
			MTWF_PRINT("    ack flush timeout count = %u\n", cnx->stats.timeout);
#if REDUCE_ACK_PKT_CACHE

			for (k = 0; k < 10 && cnx->stats.timeout_acks[k] != 0; k++)
				MTWF_PRINT("    timeout_acks[%u] = %u\n", k, cnx->stats.timeout_acks[k]);

#endif /* REDUCE_ACK_PKT_CACHE */
			MTWF_PRINT("    RWND full count = %u\n", cnx->stats.rwnd_full);
			MTWF_PRINT("    max consecutive data count = %u\n", cnx->stats.max_consecutive_data);
			MTWF_PRINT("    max consecutive ack count = %u\n", cnx->stats.max_consecutive_ack);
			MTWF_PRINT("    old ack count = %u\n", cnx->stats.oldack);
			MTWF_PRINT("    data jump count = %u\n", cnx->stats.data_jump);
			MTWF_PRINT("    passed by state (not in reduction state) = %u\n", cnx->stats.pass_state);
			MTWF_PRINT("    passed by consecutive data = %u\n", cnx->stats.pass_con_data);
			MTWF_PRINT("    passed by consecutive dropped ack = %u\n", cnx->stats.pass_con_drop);
			MTWF_PRINT("    passed by pushed data = %u\n", cnx->stats.pass_push_data);
			MTWF_PRINT("    passed by pushed data3 = %u\n", cnx->stats.pass_push_data3);
			MTWF_PRINT("    passed by selective ack = %u\n", cnx->stats.pass_sack);
			MTWF_PRINT("    passed by BIF warning = %u\n", cnx->stats.pass_bif_warning);
			MTWF_PRINT("    passed by RWND warning = %u\n", cnx->stats.pass_win_warning);
			MTWF_PRINT("    passed by wsize changed = %u\n", cnx->stats.pass_wsize_change);
			MTWF_PRINT("    passed by ack ratio = %u\n", cnx->stats.pass_ack_ratio);
			j++;
		}
		MTWF_PRINT("Total DATA received = %u\n", total_data_received);
		MTWF_PRINT("Total ACK dropped = %u\n", total_ack_dropped);
		MTWF_PRINT("Total ACK received = %u\n", total_ack_received);
		MTWF_PRINT("Total ACK sent = %u\n", total_ack_received - total_ack_dropped);
		MTWF_PRINT("Total timeout count = %u\n", total_ack_timeout);
	}
	return;
}
...
VOID ReduceAckInit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(NULL, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_DEBUG,
		"inf=%s\n", pAd->net_dev->name);
	MTWF_PRINT("INIT REDUCE TCP ACK, %s\n", pAd->net_dev->name);
}
...
VOID ReduceAckExit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(NULL, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_DEBUG,
		"inf=%s)\n", pAd->net_dev->name);
	MTWF_PRINT("EXIT REDUCE TCP ACK, %s\n", pAd->net_dev->name);
}
...
VOID ReduceAckSetEnable(PRTMP_ADAPTER pAdapter, UINT32 enable)
{
	...
	if (enable > REDUCE_ACK_ENABLE_LAST) {
		MTWF_PRINT("ERROR!! - Valid range is 0 ~ %d", REDUCE_ACK_ENABLE_LAST);
		return;
	}
...
VOID ReduceAckSetProbability(PRTMP_ADAPTER pAdapter, UINT32 percentage)
{
	...
	if (percentage != 0) {
		MTWF_PRINT("You could only set 0%% to disable reduction\n");
		return;
	}
	...
	if (percentage < 0 || percentage > 100) {
		MTWF_PRINT("Out of range - please try 0~100");
		return;
	}
