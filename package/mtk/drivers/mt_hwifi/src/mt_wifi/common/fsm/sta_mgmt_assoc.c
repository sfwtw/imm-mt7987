			...
			while (len > 0) {
				...
				if ((pEid->Eid == IE_WPA)
					...
					MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
						 "=> Store RSN_IE for WPA SM negotiation\n");
				}
				/* For WPA2/WPA2PSK/WPA3/WPA3PSK */
					...
					pEntry->RSNIE_Len = tmp_len;
					MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
						 "=> Store RSN_IE for WPA2 SM negotiation\n");
				}
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
		"ASSOC - enqueue ASSOC_FSM_ASSOC_TIMEOUT\n");
	...
	MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_ASSOC_TIMEOUT, 0,
		NULL, 0, pContext->wdev, FALSE, NULL);
	...
	IN PVOID SystemSpecific3)
{
	...
	MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_REASSOC_TIMEOUT, 0, NULL,
		0, pContext->wdev, FALSE, NULL);
	RTMP_MLME_HANDLER(pAd);
}
	...
	IN PVOID SystemSpecific3)
{
	...
	MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_DISASSOC_TIMEOUT, 0, NULL,
		0, pContext->wdev, FALSE, NULL);
	RTMP_MLME_HANDLER(pAd);
}
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	if (HAS_HT_CAPS_EXIST(cmm_ies->ie_exists)) {
		...
		if (pEdcaParm->bValid == FALSE) {
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
				"peer doesn't have WmmIE, add it due to we are running at HT+");
		...
		if (wdev->bWmmCapable == FALSE) {
			...
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
				"update WmmCapable flag due to we are running at HT+");
		}
	}
	...
	if (Idx == BSS_NOT_FOUND)
		...
		if (IS_AKM_WPA2(wdev->SecConfig.AKMMap) ||
			...
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
				"=> Store Assoc Resp RSN_IE for WPA2 SM negotiation\n");
		}
		...
		if (pEntry->RSNIE_Len == 0)
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO, "=> no RSN_IE\n");
		else
			hex_dump("RSN_IE", pEntry->RSN_IE, pEntry->RSNIE_Len);
	}
}
...
static void sta_cert_security_handle(
	...
	ULONG *FrameLen)
{
	PSTA_ADMIN_CONFIG pStaCfg = GetStaCfgByWdev(mac_ad, wdev);
	MAC_TABLE_ENTRY *pAPEntry = GetAssociatedAPByWdev(mac_ad, wdev);

	/* RSN start */
	if (mac_ad->CommonCfg.wifi_cert) {
		MTWF_PRINT("======================================> check WPA2PSK :%d\n",
		...
		if ((IS_AKM_OWE(wdev->SecConfig.AKMMap))
			&& (IS_AKM_OPEN_ONLY(pAPEntry->SecConfig.AKMMap))) {
	...
	(defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
			UCHAR *wpa_ie = NULL;
			UINT wpa_ie_len = 0;
			UCHAR *assoc_ie = pStaCfg->wpa_supplicant_info.pWpaAssocIe;
			UINT assoc_ie_len = pStaCfg->wpa_supplicant_info.WpaAssocIeLen;
#endif
		struct EXCLUDED_IES elem = {0};
		UCHAR *temp_assoc_ie = NULL;
		UCHAR temp_assoc_ie_len = 0;
		UINT alloc_flag = 0;
		UCHAR desired_temp_assoc_ie_len = 0;
		int pos1 = 0, pos2 = 0;
		int i = 0;

		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
			"APCLI WPA_ASSOC_IE FROM SUPPLICANT CERT ENABLE\n");
		hex_dump_with_lvl("assoc req before  modification",
			assoc_ie, assoc_ie_len, DBG_LVL_DEBUG);
		if (!WPA_Extract_extended_regclass_IE(assoc_ie, assoc_ie_len, &elem)
			|| (!elem.extended_capa && !elem.operating_class)) {
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
			"assoc ies error or ext capa and operating class IE not present\n");
			temp_assoc_ie = assoc_ie;
			temp_assoc_ie_len = assoc_ie_len;
			alloc_flag = 1;
		} else {
			desired_temp_assoc_ie_len = assoc_ie_len;
			desired_temp_assoc_ie_len -=
				(elem.extended_capa) ? (elem.extended_capa_len + 2) : 0;
			desired_temp_assoc_ie_len -=
				(elem.operating_class) ? (elem.operating_class_len + 2) : 0;
			os_alloc_mem(mac_ad, &temp_assoc_ie, desired_temp_assoc_ie_len);
			if (!temp_assoc_ie) {
				MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
			...
			if (!alloc_flag) {
				pos1 = (elem.extended_capa) ? (elem.extended_capa - assoc_ie):0;
				pos2 = (elem.operating_class) ? (elem.operating_class - assoc_ie):0;
				MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
					"pos1 = %d pos2 = %d\n", pos1, pos2);
				for (i = 0; i < assoc_ie_len; i++) {
					if (elem.extended_capa
						&&  i >= pos1
						&&  i < (pos1 + elem.extended_capa_len + 2))
						continue;
					if (elem.operating_class
						&&  i >= pos2
						&&  i < (pos2 + elem.operating_class_len + 2))
						continue;
					temp_assoc_ie[temp_assoc_ie_len] = assoc_ie[i];
					temp_assoc_ie_len += 1;
				}
			}
			hex_dump_with_lvl("assoc req after modification",
				temp_assoc_ie, temp_assoc_ie_len, DBG_LVL_DEBUG);
		}

#if defined(SUPP_SAE_SUPPORT) || defined(SUPP_OWE_SUPPORT) ||\
	(defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
			if (IS_AKM_WPA3PSK(pAPEntry->SecConfig.AKMMap)
				|| IS_AKM_OWE(pAPEntry->SecConfig.AKMMap)
#if defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3)
				|| (IS_MAP_ENABLE(mac_ad)
					&& IS_MAP_R3_ENABLE(mac_ad)
					&& IS_AKM_DPP(pAPEntry->SecConfig.AKMMap))
#endif /* CONFIG_MAP_SUPPORT && MAP_R3 */
			) {
				UCHAR *pBuf;
				UINT8 count;
				...
				if (IS_AKM_SAE_SHA256(pStaCfg->wdev.SecConfig.AKMMap))
					...
					SET_AKM_OWE(sec_akm);
				else if (IS_AKM_SAE_EXT(pStaCfg->wdev.SecConfig.AKMMap))
					SET_AKM_SAE_EXT((sec_akm));

				idx = sta_search_pmkid_cache(mac_ad, ApAddr, ifIndex, wdev,
						sec_akm, pStaCfg->MlmeAux.Ssid,
						pStaCfg->MlmeAux.SsidLen);
				if ((idx != INVALID_PMKID_IDX) && (*temp_assoc_ie == 0x30)) {
					wpa_ie_len = *(temp_assoc_ie + 1);
					pBuf = WPA_ExtractSuiteFromRSNIE(
						temp_assoc_ie,
						wpa_ie_len + 2,
						RSN_CAP_INFO,
						&count);
					os_alloc_mem(mac_ad, &wpa_ie, (wpa_ie_len + 2 + 22));
					if (wpa_ie && pBuf) {
						UINT16	pmk_count = 1;
						UCHAR *pos;

						wpa_ie_len = (pBuf - temp_assoc_ie);
						os_move_mem(wpa_ie, temp_assoc_ie,
						...
						/* Update the pentry->pmkcache
						   from the Saved PMK cache
						*/
						...
						NdisMoveMemory(pos,
							pentry->SecConfig.pmkid, LEN_PMKID);
						pos += LEN_PMKID;
			...
			if (wpa_ie) {
				UINT8 ie_len = *(temp_assoc_ie + 1) + 2;
				...
				MakeOutgoingFrame(pOutBuffer + (*FrameLen), &TmpWpaAssocIeLen,
					(temp_assoc_ie_len - ie_len), (temp_assoc_ie + ie_len),
					END_OF_ARGS);
				(*FrameLen) += TmpWpaAssocIeLen;
				/*VarIesOffset = 0;*/
				...
				/*VarIesOffset = 0;*/
				NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
					(temp_assoc_ie + ie_len), (temp_assoc_ie_len - ie_len));
				(*VarIesOffset) += (temp_assoc_ie_len - ie_len);
...
#endif
				MakeOutgoingFrame(pOutBuffer + (*FrameLen), &TmpWpaAssocIeLen,
								temp_assoc_ie_len, temp_assoc_ie,
								END_OF_ARGS);
				(*FrameLen) += TmpWpaAssocIeLen;
				/*VarIesOffset = 0;*/
				NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
								temp_assoc_ie, temp_assoc_ie_len);
				(*VarIesOffset) += temp_assoc_ie_len;
				/* Set Variable IEs Length */
				pStaCfg->ReqVarIELen = (*VarIesOffset);
#if defined(SUPP_SAE_SUPPORT) || defined(SUPP_OWE_SUPPORT)
			}
		...
		if ((IS_AKM_PSK(pAPEntry->SecConfig.AKMMap)
			...
			if (IS_AKM_WPA3PSK(pStaCfg->wdev.SecConfig.AKMMap) ||
				...
				pentry = entry_get(mac_ad, pStaCfg->MacTabWCID);
				...
				if (idx != INVALID_PMKID_IDX) {
					...
					MTWF_PRINT("PMKID not found: Normal Assoc\n");
					pentry->SecConfig.pmkid = NULL;
					pentry->SecConfig.pmk_cache = NULL;
				}
			}
...

			{
				...
				{ /* Todo by Eddy: It's not good code */
					...
					/*
					   Need to fill the pSecConfig->PmfCfg to let WPAMakeRSNIE()
					   generate correct RSNIE
					*/
					{
						RSN_CAPABILITIES RsnCap;

						NdisMoveMemory(&RsnCap,
							&pStaCfg->MlmeAux.RsnCap,
						...
						/*mismatch case*/
						if (((pSecConfig->PmfCfg.MFPR)
							&& (RsnCap.field.MFPC == FALSE))
							|| ((pSecConfig->PmfCfg.MFPC == FALSE)
								&& (RsnCap.field.MFPR))) {
							pSecConfig->PmfCfg.UsePMFConnect = FALSE;
							pSecConfig->key_deri_alg =
								SEC_KEY_DERI_SHA256;
						}

						if ((pSecConfig->PmfCfg.MFPC)
							&& (RsnCap.field.MFPC)) {
							struct _MLME_AUX *mlme_aux;

							mlme_aux = &pStaCfg->MlmeAux;
							pSecConfig->PmfCfg.UsePMFConnect = TRUE;
							if ((mlme_aux->IsSupportSHA256KeyDerivation)
								|| (RsnCap.field.MFPR))
								pSecConfig->key_deri_alg =
									SEC_KEY_DERI_SHA256;
						}
					...
					* bit can be set in assoc req. It will be finally
					* set when AP also supports OCV */
				...
				for (rsne_idx = 0; rsne_idx < SEC_RSNIE_NUM; rsne_idx++) {
					...
					MakeOutgoingFrame(pOutBuffer + (*FrameLen), &TempLen,
						  ...
						  pAPEntry->SecConfig.RSNE_Len[rsne_idx],
						  &pAPEntry->SecConfig.RSNE_Content[rsne_idx][0],
						  END_OF_ARGS);
					(*FrameLen) += TempLen;
				}
			}

			{
				...
				for (rsne_idx = 0; rsne_idx < SEC_RSNIE_NUM; rsne_idx++) {
					...
					/* Append Variable IE */
					NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
						&pStaCfg->wdev.SecConfig.RSNE_EID[rsne_idx][0], 1);
					(*VarIesOffset) += 1;
					NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
						&pStaCfg->wdev.SecConfig.RSNE_Len[rsne_idx], 1);
					(*VarIesOffset) += 1;
					NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
						&pStaCfg->wdev.SecConfig.RSNE_Content[rsne_idx][0],
...
static void sta_security_handle(
	struct _RTMP_ADAPTER *mac_ad,
	struct wifi_dev *wdev,
	UCHAR *ApAddr,
	ULONG *Timeout,
	...
	ULONG *FrameLen)
{
	...
	(defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
		...
		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			"APCLI WPA_ASSOC_IE FROM SUPPLICANT\n");
		hex_dump_with_lvl("assoc req before  modification",
			assoc_ie, assoc_ie_len, DBG_LVL_DEBUG);
		if (!WPA_Extract_extended_regclass_IE(assoc_ie, assoc_ie_len, &elem)
			|| (!elem.extended_capa && !elem.operating_class)) {
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
				"assoc ies from wpa parsing fail");
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
				" or extended capa and operating class IE not present\n");
			temp_assoc_ie = assoc_ie;
			temp_assoc_ie_len = assoc_ie_len;
			alloc_flag = 1;
		} else {
	...
	(defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
		if (IS_AKM_WPA3PSK(pAPEntry->SecConfig.AKMMap)
			|| IS_AKM_OWE(pAPEntry->SecConfig.AKMMap)
#if defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3)
			|| (IS_MAP_ENABLE(mac_ad) && IS_MAP_R3_ENABLE(mac_ad)
				&& IS_AKM_DPP(pAPEntry->SecConfig.AKMMap))
#endif /* CONFIG_MAP_SUPPORT && MAP_R3 */
		) {
			UCHAR *pBuf;
			UINT8 count;
			INT idx;
			MAC_TABLE_ENTRY *pentry = entry_get(mac_ad, pStaCfg->MacTabWCID);
			PBSSID_INFO psaved_pmk = &pStaCfg->SavedPMK[0];
			VOID *psaved_pmk_lock = (VOID *)&pStaCfg->SavedPMK_lock;
			UINT32 sec_akm = 0;
			...
			*Timeout = 5000;
			...
			if ((idx != INVALID_PMKID_IDX) && (*temp_assoc_ie == 0x30)) {
				wpa_ie_len = *(temp_assoc_ie + 1);
				pBuf = WPA_ExtractSuiteFromRSNIE(
					temp_assoc_ie, wpa_ie_len + 2, RSN_CAP_INFO, &count);
				os_alloc_mem(mac_ad, &wpa_ie, (wpa_ie_len + 2 + 22));
				if (wpa_ie && pBuf) {
					UINT16	pmk_count = 1;
					UCHAR *pos;
					...
					pos += LEN_PMKID;
					wpa_ie_len += (2 + LEN_PMKID);
					pBuf = WPA_ExtractSuiteFromRSNIE(temp_assoc_ie,
						(*(temp_assoc_ie + 1) + 2), G_MGMT_SUITE, &count);
					if (pBuf) {
						NdisMoveMemory(pos, pBuf, 4);
						wpa_ie_len += 4;
					}
		...
		} else if (IS_AKM_DPP(pAPEntry->SecConfig.AKMMap))
			*Timeout = 5000;
		...
		if (wpa_ie) {
			...
			MakeOutgoingFrame(pOutBuffer + (*FrameLen), &TmpWpaAssocIeLen,
						(temp_assoc_ie_len - ie_len),
						(temp_assoc_ie + ie_len),
						END_OF_ARGS);
			(*FrameLen) += TmpWpaAssocIeLen;
			/*VarIesOffset = 0;*/
			NdisMoveMemory(pStaCfg->ReqVarIEs + (*VarIesOffset),
		...
		if (IS_AKM_WPA3PSK(pStaCfg->wdev.SecConfig.AKMMap) ||
			...
			if (idx != INVALID_PMKID_IDX) {
				...
				MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_DEBUG,
					"PMKID not found in cache: Normal Assoc\n");
				pentry->SecConfig.pmkid = NULL;
				pentry->SecConfig.pmk_cache = NULL;
			}
		}
#endif
		pSecConfig = &pAPEntry->SecConfig;
		AKMMap = pSecConfig->AKMMap;
		PairwiseCipher = pSecConfig->PairwiseCipher;
		GroupCipher = pSecConfig->GroupCipher;
#ifdef DOT11W_PMF_SUPPORT
		/*
		   Need to fill the pSecConfig->PmfCfg
		   to let WPAMakeRSNIE() generate correct RSNIE
		 */
		{
			RSN_CAPABILITIES RsnCap;
			...
			if ((pSecConfig->PmfCfg.MFPC) && (RsnCap.field.MFPC)) {
				...
				if ((pStaCfg->MlmeAux.IsSupportSHA256KeyDerivation)
					|| (RsnCap.field.MFPR))
					pSecConfig->key_deri_alg = SEC_KEY_DERI_SHA256;
			}
		}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
...
#endif /* MAC_REPEATER_SUPPORT */
	{
		...
		*/
		...
		/* RSN for cert */
		sta_cert_security_handle(
			pAd, wdev, ApAddr, wdev->func_idx, &VarIesOffset, pOutBuffer, &FrameLen);
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_EHT, DBG_LVL_INFO,
			"WMODE_CAP_BE: %d, HAS_EHT_CAPS_EXIST: %d, HAS_EHT_MLD_EXIST:%d\n",
			WMODE_CAP_BE(wdev->PhyMode),
			HAS_EHT_CAPS_EXIST(pStaCfg->MlmeAux.ie_exists),
			HAS_EHT_MLD_EXIST(pStaCfg->MlmeAux.ie_exists));
		if (WMODE_CAP_BE(wdev->PhyMode) &&
			(HAS_VENDOR_EHT_CAPS_EXIST(pStaCfg->MlmeAux.vendor_ie.ie_exists) || HAS_EHT_CAPS_EXIST(pStaCfg->MlmeAux.ie_exists)) &&
			chip_determine_mlo_allow(pAd, &pStaCfg->MlmeAux.vendor_ie)) {
			ULONG eht_ies_start = FrameLen;

			if (HAS_EHT_MLD_EXIST(pStaCfg->MlmeAux.ie_exists)
#ifdef APCLI_CFG80211_SUPPORT
				&& !(pStaCfg->wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE_WPS)
#endif
				)
				FrameLen += eht_add_assoc_req_ml_ies(wdev, pOutBuffer, FrameLen);

			FrameLen += eht_add_assoc_req_cap_ies(wdev, pOutBuffer, FrameLen);

			NdisMoveMemory(pStaCfg->ReqVarIEs + VarIesOffset,
				pOutBuffer + eht_ies_start, FrameLen - eht_ies_start);
			hex_dump("ETH IES: ", pStaCfg->ReqVarIEs + VarIesOffset, FrameLen - eht_ies_start);
			VarIesOffset += FrameLen - eht_ies_start;

			/* add MTK MLO V1 IE */
			FrameLen += build_mtk_tlv1_vendor_ie(pAd, wdev, (pOutBuffer + FrameLen), VIE_ASSOC_REQ, BIT(MEDIATEK_TLV1_TYPE4));
		}
#endif
#endif /* DOT11_N_SUPPORT */
...
#ifdef MBO_SUPPORT
		if (IS_MBO_ENABLE(wdev) || (WMODE_CAP_6G(wdev->PhyMode)))
...
#ifdef WSC_INCLUDED
				&& pWscControl && !pWscControl->bWscTrigger
#endif /* WSC_INCLUDED */
				)
		...
		if (IS_RRM_ENABLE(wdev)) {
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO,
				"APCLI INSERT RM CAPA FrameLen = %lu\n", FrameLen);
		} else
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO,
				"APCLI NOT INSERT RM CAPA\n");
#endif
		...
		/* RSN */
		sta_security_handle(
			pAd, wdev, ApAddr, &Timeout, wdev->func_idx,
			&VarIesOffset, pOutBuffer, &FrameLen);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
...

	{
		...
		ie_info.frame_subtype = SUBTYPE_REASSOC_REQ;
		ie_info.channel = pStaCfg->MlmeAux.Channel;
		ie_info.phy_mode = wdev->PhyMode;
		ie_info.wdev = wdev;
		...
		if (pStaCfg->Dot11RCommInfo.bFtSupport &&
			...
			if (pStaCfg->Dot11RCommInfo.bInMobilityDomain &&
				...
				pEntry = entry_get(pAd, MCAST_WCID);
		...
		if ((pStaCfg->wpa_supplicant_info.WpaSupplicantUP & 0x7F) ==  WPA_SUPPLICANT_ENABLE) {
			ULONG TmpWpaAssocIeLen = 0;
#if defined(SUPP_SAE_SUPPORT) || defined(SUPP_OWE_SUPPORT)  || (defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
			UCHAR *wpa_ie = NULL;
			UINT wpa_ie_len = 0;
			UCHAR *assoc_ie = pStaCfg->wpa_supplicant_info.pWpaAssocIe;
			UINT assoc_ie_len = pStaCfg->wpa_supplicant_info.WpaAssocIeLen;
			...
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
					"APCLI WPA_ASSOC_IE FROM SUPPLICANT\n");
			hex_dump_with_lvl("assoc req before  modification", assoc_ie, assoc_ie_len, DBG_LVL_DEBUG);
			if (!WPA_Extract_extended_regclass_IE(assoc_ie, assoc_ie_len, &elem) ||
					(!elem.extended_capa && !elem.operating_class)) {
				MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
					"Fail Extended capa and operating class IE not present\n");
				temp_assoc_ie = assoc_ie;
				temp_assoc_ie_len = assoc_ie_len;
				alloc_flag = 1;
			} else {
				desired_temp_assoc_ie_len = assoc_ie_len;
				desired_temp_assoc_ie_len -= (elem.extended_capa) ? (elem.extended_capa_len + 2) : 0;
				desired_temp_assoc_ie_len -= (elem.operating_class) ? (elem.operating_class_len + 2) : 0;
				os_alloc_mem(pAd, &temp_assoc_ie, desired_temp_assoc_ie_len);
				if (!temp_assoc_ie) {
					MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR,
						"memory allocation fail\n");
					temp_assoc_ie = assoc_ie;
				...
				os_zero_mem(temp_assoc_ie, desired_temp_assoc_ie_len);
				if (!alloc_flag) {
					pos1 = (elem.extended_capa) ? (elem.extended_capa - assoc_ie):0;
					pos2 = (elem.operating_class) ? (elem.operating_class - assoc_ie):0;
					MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
							"pos1 = %d pos2 = %d\n", pos1, pos2);
					for (i = 0; i < assoc_ie_len; i++) {
						if ((elem.extended_capa && i >= pos1 && i < (pos1 + elem.extended_capa_len + 2)) ||
								(elem.operating_class && i >= pos2 && i < (pos2 + elem.operating_class_len + 2)))
							continue;
						temp_assoc_ie[temp_assoc_ie_len] = assoc_ie[i];
						temp_assoc_ie_len += 1;
					}
				}
				hex_dump_with_lvl("assoc req after modification", temp_assoc_ie, temp_assoc_ie_len, DBG_LVL_DEBUG);
			}

#if defined(SUPP_SAE_SUPPORT) || defined(SUPP_OWE_SUPPORT) || (defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3))
			if (IS_AKM_WPA3PSK(pAPEntry->SecConfig.AKMMap) || IS_AKM_OWE(pAPEntry->SecConfig.AKMMap)
#if defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3)
					|| (IS_MAP_ENABLE(pAd) && IS_MAP_R3_ENABLE(pAd) && IS_AKM_DPP(pAPEntry->SecConfig.AKMMap))
#endif /* CONFIG_MAP_SUPPORT && MAP_R3 */
			   ) {
				UCHAR *pBuf;
				UINT8 count;
				INT idx;
				MAC_TABLE_ENTRY *pentry = entry_get(pAd, pStaCfg->MacTabWCID);
				PBSSID_INFO psaved_pmk = &pStaCfg->SavedPMK[0];
				VOID *psaved_pmk_lock = (VOID *)&pStaCfg->SavedPMK_lock;
				UINT32 sec_akm = 0;
				...
				if ((idx != INVALID_PMKID_IDX) && (*temp_assoc_ie == 0x30)) {
					wpa_ie_len = *(temp_assoc_ie + 1);
					pBuf = WPA_ExtractSuiteFromRSNIE(temp_assoc_ie, wpa_ie_len + 2, RSN_CAP_INFO, &count);
					os_alloc_mem(pAd, &wpa_ie, (wpa_ie_len + 2 + 22));
					if (wpa_ie && pBuf && psaved_pmk_lock) {
						UINT16  pmk_count = 1;
						UCHAR *pos;

						wpa_ie_len = (pBuf - temp_assoc_ie);
						os_move_mem(wpa_ie, temp_assoc_ie, wpa_ie_len + sizeof(RSN_CAPABILITIES));
						wpa_ie_len += sizeof(RSN_CAPABILITIES);
						pos = wpa_ie + wpa_ie_len;
						NdisAcquireSpinLock(psaved_pmk_lock);
						/* Update the pentry->pmkcache from the Saved PMK
						 * cache */
						pentry->SecConfig.pmkid = psaved_pmk[idx].PMKID;
						pentry->SecConfig.pmk_cache = psaved_pmk[idx].PMK;
						NdisReleaseSpinLock(psaved_pmk_lock);
						...
						pos += LEN_PMKID;
						wpa_ie_len += (2 + LEN_PMKID);
						pBuf = WPA_ExtractSuiteFromRSNIE(temp_assoc_ie, (*(temp_assoc_ie + 1) + 2),
								G_MGMT_SUITE, &count);
						if (pBuf)
							NdisMoveMemory(pos, pBuf, 4);
						else
							MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC,
							DBG_LVL_WARN,
							"Extract Suite From RSNIE FAIL!\n");
						wpa_ie_len += 4;
						wpa_ie[1] = (wpa_ie_len - 2);
					}
				} else {
			...
			if ((IS_AKM_PSK(pAPEntry->SecConfig.AKMMap)
				...
				if (IS_AKM_WPA3PSK(pStaCfg->wdev.SecConfig.AKMMap) ||
					...
					idx = sta_search_pmkid_cache(pAd, ApAddr, ifIndex, wdev,
							sec_akm, pStaCfg->MlmeAux.Ssid, pStaCfg->MlmeAux.SsidLen);
					if (idx != INVALID_PMKID_IDX && psaved_pmk_lock) {
						NdisAcquireSpinLock(psaved_pmk_lock);
						/* Update the pentry->pmkcache from the Saved PMK
						 * cache */
						pentry->SecConfig.pmkid = psaved_pmk[idx].PMKID;
...
static VOID sta_mlme_disassoc_req_action(
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	MLME_DISCONNECT_STRUCT *pDisassocReq;
	...
	/* skip sanity check */
	pDisassocReq = (MLME_DISCONNECT_STRUCT *) (Elem->Msg);
	NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);	/*Get an unused nonpaged memory */
	...
	MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO,
			 "ASSOC - Send DISASSOC request[BSSID::"MACSTR", reason=%d\n",
			  MAC2STR(pDisassocReq->addr), pDisassocReq->reason);
	MgtMacHeaderInitExt(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pDisassocReq->addr,
			wdev->if_addr,
	...
	COPY_MAC_ADDR(pStaCfg->DisassocSta, pDisassocReq->addr);
#ifdef MAC_REPEATER_SUPPORT
	if (wdev->wdev_type != WDEV_TYPE_REPEATER)
#endif /* MAC_REPEATER_SUPPORT */
	{
...
#ifdef APCLI_SUPPORT
/* Caller should confirm the validity of pointers before calling this function */
static VOID sta_peer_ocv_validate_in_assoc_resp(
	...
	IE_LISTS *ie_list)
{
	...
	if (ie_list) {
		rsnie_ptr = ie_list->RSN_IE;
		rsnie_len = ie_list->RSNIE_Len;
	} else {
		rsnie_ptr = pEntry->RSN_IE;
		rsnie_len = pEntry->RSNIE_Len;
	}

	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_INFO,
			"ApCli OCV: rsnie_len:%d\n", rsnie_len);
	if (rsnie_len) {
		pStaCfg->wdev.SecConfig.ocv_support = TRUE;
		if (wpa_check_rsn_cap(&pStaCfg->wdev,
					&pEntry->SecConfig,
					rsnie_ptr,
					rsnie_len)) {
			MTWF_DBG(NULL, DBG_CAT_SEC,
				CATSEC_OCV, DBG_LVL_INFO,
				"ApCli OCV: wpa_check_rsn_cap return TRUE\n");
		} else {
			MTWF_DBG(NULL, DBG_CAT_SEC,
				CATSEC_OCV, DBG_LVL_WARN,
				"ApCli OCV: wpa_check_rsn_cap return FALSE\n");
		}
		if (pEntry->SecConfig.ocv_support == FALSE)
			pStaCfg->wdev.SecConfig.ocv_support = FALSE;

		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_INFO,
			"ApCli OCV: Self.ocv_support:%d pEntry.ocv_support:%d\n",
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (ie_list == NULL) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_ERROR, "mem alloc failed!\n");
		return;
	}
		...
		if (MAC_ADDR_EQUAL(Addr2, pStaCfg->MlmeAux.Bssid)) {
#if defined(CONFIG_MAP_SUPPORT) && defined(MTK_HOSTAPD_SUPPORT)
			if (IS_MAP_ENABLE(pAd)) {
				map_ext = ie_list->MAP_AttriValue;
				MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
					"wpa_supplicant WPS %x, map_ext %x\n",
					pStaCfg->wpa_supplicant_info.WpaSupplicantUP, map_ext);
				/*We are allowing 3rd party(non-map) connection due IOT QCA.*/
				if ((!(pStaCfg->wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE_WPS)
				    && map_ext) && !(map_ext & 0x40))
					goto err;
			}
#endif
#if defined(APCLI_CFG80211_SUPPORT)
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
				"received assoc resp from "MACSTR" (status=%d)...\n",
				MAC2STR(Addr2), Status);
			...
			if (!IS_APCLI_DISABLE_MLO(wdev) && HAS_EHT_MLD_EXIST(ie_list->cmm_ies.ie_exists)
				...
				if ((mac_bss->if_cfg.mld_group_idx != 0) &&
					...
					entrytb_mlo_sync(pEntry);
					sta_sync_mlo_comm_cap(pAd, pEntry, &ie_list->cmm_ies, &EdcaParm);
					os_alloc_mem(NULL, (UCHAR **)&ml_info, sizeof(struct ml_ie_info));
					if (!ml_info) {
						MTWF_DBG(NULL,
							DBG_CAT_MLO, CATMLME_ASSOC, DBG_LVL_ERROR,
							"failed to allocate memory for parsing ML IE\n");
						goto err;
					}
					NdisZeroMemory(ml_info, sizeof(struct ml_ie_info));

					if (parse_multi_link_ie(ie_list->cmm_ies.ml_ie,
						&ie_list->cmm_ies.ml_frag_info, ml_info) < 0) {
						MTWF_DBG(NULL,
							DBG_CAT_MLO, CATMLME_ASSOC, DBG_LVL_ERROR,
							"failed to parse ML IE\n");
						goto err;
					}
					sta_sync_mld_per_link_cap(pEntry, ml_info, &ie_list->cmm_ies);
				}
			}
#endif /*DOT11_EHT_BE*/
...
#ifdef APCLI_SUPPORT
				if (pStaCfg->wdev.SecConfig.apcli_ocv_support)
					sta_peer_ocv_validate_in_assoc_resp(pStaCfg, pEntry, NULL);
#endif
				if (IF_COMBO_HAVE_AP_STA(pAd) && wdev->wdev_type == WDEV_TYPE_STA) {
					...
					if (IS_MAP_ENABLE(pAd)) {
						...
						MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAP,
...
/* XXX: TRACE_NM: Should we process this during EAPOL-1_of_4 handling */
	...
	{
		PMAC_TABLE_ENTRY pEntry = entry_get(pAd, pStaCfg->MacTabWCID);
		...
		if (!(IS_AKM_WPA_CAPABILITY_Entry(pEntry)
			...
			if (IS_WPA_SUPPLICANT_UP(pStaCfg))
				CFG80211_CONN_RESULT_INFORM(pAd, pStaCfg->MlmeAux.Bssid, ifIndex,
					pStaCfg->ReqVarIEs, pStaCfg->ReqVarIELen,
					pStaCfg->ResVarIEs, pStaCfg->ResVarIELen, Status);
#endif
		...
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			"received assoc resp, sanity check fail.\n");
		...
		if (MAC_ADDR_EQUAL(Addr2, pStaCfg->MlmeAux.Bssid)) {	/* The frame is for me ? */
...
#endif
#ifdef APCLI_CFG80211_SUPPORT
			/* Store the ReAssocRsp Frame to wpa_supplicant via CFG80211 */
			NdisZeroMemory(pAd->StaCfg[ifIndex].ResVarIEs, MAX_VIE_LEN);
			pAd->StaCfg[ifIndex].ResVarIELen = 0;
			pAd->StaCfg[ifIndex].ResVarIELen = Elem->MsgLen - 6 - sizeof(HEADER_802_11);
			NdisCopyMemory(pAd->StaCfg[ifIndex].ResVarIEs, &pFrame->Octet[6], pAd->StaCfg[ifIndex].ResVarIELen);
			...
			if (Status == MLME_SUCCESS) {
...
#endif
				{
					...
					pEntry = MacTableInsertEntry(pAd, Addr2, wdev, ENTRY_INFRA, OPMODE_STA, TRUE, MLD_STA_NONE, NULL);
					ASSERT(pEntry);
					if (!pEntry) {
#ifdef CUSTOMER_VENDOR_IE_SUPPORT
						/* fix memory leak when trigger scan continuously*/
...
#ifdef APCLI_SUPPORT
				if (pStaCfg->wdev.SecConfig.apcli_ocv_support)
					sta_peer_ocv_validate_in_assoc_resp(pStaCfg, pEntry, ie_list);
#endif
				/* go to procedure listed on page 376 */
				sta_assoc_post_proc(pAd, Addr2, CapabilityInfo, Aid,
									&EdcaParm, ie_list, pEntry);
				...
				if (HAS_EHT_MLD_EXIST(ie_list->cmm_ies.ie_exists) && pEntry->mlo.mlo_en) {
					...
					if ((mac_bss->if_cfg.mld_group_idx != 0) &&
						...
						os_alloc_mem(NULL, (UCHAR **)&ml_info, sizeof(struct ml_ie_info));
						if (!ml_info) {
							MTWF_DBG(NULL, DBG_CAT_MLO, CATMLME_ASSOC,
							DBG_LVL_ERROR,
							"failed to alloc memory for parsing MLIE\n");
							goto err;
						}
						NdisZeroMemory(ml_info, sizeof(struct ml_ie_info));
...
static VOID sta_peer_disassoc_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#if defined(FAST_EAPOL_WAR) || defined(APCLI_CFG80211_SUPPORT) || defined(DOT11_EHT_BE)
	USHORT ifIndex = wdev->func_idx;
#endif
#ifdef DOT11_EHT_BE
	...
	MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO, "ASSOC - PeerDisassocAction()\n");

	if (PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason)) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			"received dis-assoc from "MACSTR", reason=%d\n",
		...
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_NOTICE,
			"received dis-assoc, sanity check fail\n");
	}
}
...
static VOID sta_mlme_reassoc_req_timeout_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_ASSOC, DBG_LVL_INFO, "ASSOC - ReassocTimeoutAction\n");
	assoc_fsm_state_transition(Elem->wdev, ASSOC_IDLE);
	Status = MLME_REJ_TIMEOUT;
	cntl_auth_assoc_conf(Elem->wdev, CNTL_MLME_REASSOC_CONF, Status);
}
