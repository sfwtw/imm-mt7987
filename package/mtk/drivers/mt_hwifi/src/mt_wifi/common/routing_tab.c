	...
	IN UINT32 Flag)
{
	...
	if (pMbss->pRoutingEntryPool) {
		...
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_4ADDR, DBG_LVL_ERROR,
	...
	IN UINT32 Flag)
{
	...
	pMbss = &pAd->ApCfg.MBSSID[ifIndex];
	OS_SEM_LOCK(&pMbss->RoutingTabLock);
	for (i = 0; i < ROUTING_HASH_TAB_SIZE; i++) {
	...
	OS_SEM_UNLOCK(&pMbss->RoutingTabLock);
}
	...
	IN UINT32 Flag)
{
	...
	if (HashId < 0) {
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_4ADDR, DBG_LVL_ERROR, "Hash Id isn't correct!\n");
		return;
	}
	...
	UINT32 SrcIP)
{
	...
	pEntry = entry_get(pAd, pRoutingEntry->Wcid);
	wdev = pEntry->wdev;
	if (!wdev)
		return FALSE;
