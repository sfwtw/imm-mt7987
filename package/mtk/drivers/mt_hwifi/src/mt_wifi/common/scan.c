...
void update_scan_channel_stats(RTMP_ADAPTER *pAd, UINT8 BandIdx, SCAN_CTRL *ScanCtrl)
{
	...
	pAd->ScanChnlStats.Obss_Time = OBSSAirtime;
	...
	pAd->ScanChnlStats.Tx_Time = MyTxAirtime;
	...
	pAd->ScanChnlStats.Rx_Time = MyRxAirtime;
	...
	pAd->ScanChnlStats.PCCA_Time = PCCA_Time;
	...
	pAd->ScanChnlStats.Tx2_Time = MyTx2Airtime;
	...
	*/
	...
	BATime = BACount * 32;
	pAd->ScanChnlStats.BA_Time = BATime;

	//Obss time, Max( PCCA-MyRx-BA,(OBSS > 20% ?  OBSS + newSCCA : OBSS)  )
	if ((OBSSAirtime/(ScanCtrl->ScanTime[ScanCtrl->CurrentGivenChan_Index] * 10)) > 20) {
		// OBSS+newSCCA
		if (MyRxAirtime + BATime > PCCA_Time)
			pAd->ScanChnlStats.Obss_Time = OBSSAirtime + pAd->Ch_Stats.SCCA_Time;
		else if (PCCA_Time - MyRxAirtime - BATime > OBSSAirtime + pAd->Ch_Stats.SCCA_Time)
			pAd->ScanChnlStats.Obss_Time = PCCA_Time - MyRxAirtime - BATime;
		else
			pAd->ScanChnlStats.Obss_Time = OBSSAirtime + pAd->Ch_Stats.SCCA_Time;
	} else {// OBSS
		pAd->ScanChnlStats.Obss_Time = OBSSAirtime;
	...
	if ((OBSSAirtime/(ScanCtrl->ScanTime[ScanCtrl->CurrentGivenChan_Index] * 10)) > 20) {
		// OBSS+IPI
		if (PCCA_Time > BATime + OBSSAirtime + pAd->Ch_Stats.SCCA_Time)
			ChBusyTime = MyTx2Airtime + PCCA_Time;
		else
			ChBusyTime = MyTx2Airtime + BATime + OBSSAirtime + pAd->Ch_Stats.SCCA_Time;
	} else {// OBSS
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
		"ChannelBusyTime: %u Rx-Time: %u PCCA Time: %u Obss-Time: %u\n",
		pAd->ChannelInfo.chanbusytime[pAd->ApCfg.current_channel_index],
		pAd->ScanChnlStats.Rx_Time, pAd->ScanChnlStats.PCCA_Time,
		pAd->ScanChnlStats.Obss_Time);
}
...
INT scan_ch_restore(RTMP_ADAPTER *pAd, UCHAR OpMode, struct wifi_dev *pwdev)
{
	...
	ch = wlan_operate_get_cen_ch_1(wdev);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN, "End of SCAN(TYPE: %d, BandIdx: %d), restore to BW(%d) channel %d, Total BSS[%02d]\n",
	...
	if (OpMode == OPMODE_STA) {
		...
		greenap_resume(pAd, GREENAP_REASON_STA_ACTIVE_SCAN);
		...
		*/
		...
		if ((wdev->func_idx < MAX_APCLI_NUM) && (wdev->func_idx >= 0) &&
#ifdef CON_WPS
		...
		if ((ScanCtrl->Channel != 0) &&
			...
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN, "bFastRoamingScan ~~~ Get back to send data ~~~\n");
		}
	}
...
INT ch_switch_monitor_scan_ch_restore(RTMP_ADAPTER *pAd, UCHAR OpMode, struct wifi_dev *pwdev)
{
	...
	ch = wlan_operate_get_cen_ch_1(wdev);
	bw = wlan_operate_get_bw(wdev);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO, "Restore to BW(%d) channel %d\n", bw, ch);
...
static INT scan_active(RTMP_ADAPTER *pAd, UCHAR OpMode, UCHAR ScanType, struct wifi_dev *wdev)
{
	...
	if (WMODE_CAP_6G(wdev->PhyMode) && (wdev->wdev_type == WDEV_TYPE_STA) &&
		(ScanCtrl->SsidLen == 0)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"Disable active scan for 6G ApCli\n");
		return TRUE;
	}
	...
	MakeOutgoingFrame(frm_buf,               &FrameLen,
				  ...
				  END_OF_ARGS);
	FrameLen += build_support_rate_ie(wdev, rate->sup_rate,
		rate->sup_rate_len, frm_buf + FrameLen);
	...
	if (WMODE_CAP_N(wdev->PhyMode)) {
		...
		if (WMODE_CAP_AX(wdev->PhyMode)) {
			ie_info.frame_buf = (UCHAR *)(frm_buf + FrameLen);
			FrameLen += add_probe_req_he_ies(wdev, ie_info.frame_buf);
		}
#endif /*DOT11_HE_AX*/
	...
	if (IS_MBO_ENABLE(wdev)) {
		...
		MakeMboOceIE(pAd, wdev, NULL, frm_buf + FrameLen, &FrameLen, MBO_FRAME_TYPE_PROBE_REQ);
	}
#endif /* MBO_SUPPORT */
#ifdef DOT11_EHT_BE
	ie_info.frame_buf = (UCHAR *)(frm_buf + FrameLen);
	...
	FrameLen += build_mtk_tlv1_vendor_ie(pAd, wdev, (frm_buf + FrameLen), VIE_PROBE_REQ, BIT(MEDIATEK_TLV1_TYPE4));
#endif /*DOT11_EHT_BE*/
	...
	MiniportMMRequest(pAd, 0, frm_buf, FrameLen, NULL);
#ifdef MT_MAC_BTCOEX
	...
	OUT UCHAR *ScanType)
{
	...
	if (ScanCtrl->state == OFFCHANNEL_SCAN_START) {
		...
		if (cap->uni_cmd_support) {
			Reg[0].Counter = UNI_CMD_MIB_CNT_P_CCA_TIME;
		...
		ScanCtrl->ScanTimeActualStart = mt_ktime_get();
#ifdef ZERO_PKT_LOSS_SUPPORT
		/*read channel stats initial reading*/
		pAd->ScanCtrl.ScanTimeStartMibEvent = ktime_get();
...
#endif
		phy_update_channel_info(pAd, Channel2Index(pAd, ScanCtrl->Channel));
...
set_stay_time:
	if (ScanCtrl->ScanTime[0])
		stay_time = ScanCtrl->ScanTime[0];

	RTMPSetTimer(sc_timer, stay_time);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO, "SYNC - stay_time:%d\n", stay_time);
	return TRUE;
...
/* TODO: Star, fix me when Scan is prepare to modify */
...
#endif
		if (ScanCtrl->opAbortScan || wdev->MarkToClose) {
			ScanCtrl->Channel = 0;
			scan_ch_restore(pAd, OpMode, wdev);
			ScanCtrl->PartialScan.bScanning = FALSE;
			if (ScanCtrl->opAbortScan == TRUE)
				ScanCtrl->opAbortScan = FALSE;
	...
	if (CFG80211DRV_OpsScanRunning(pAd) && !ScanPending) {
		...
		ScanCtrl->Channel = CFG80211DRV_OpsScanGetNextChannel(pAd);
			if (ScanCtrl->Channel == 0)
				break;
			if (!IsValidChannel(pAd, ScanCtrl->Channel, wdev))
				continue;
			else
				ChannelFound = 1;
		}
	...
	if ((ScanCtrl->Channel == 0) || ScanPending) {
		...
		if (scan_ch_restore(pAd, OpMode, wdev) == FALSE)
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
				"scan_ch_restore FAIL!!\n");

#ifdef OFFCHANNEL_SCAN_FEATURE
#ifdef CONFIG_MAP_SUPPORT
		if (IS_MAP_TURNKEY_ENABLE(pAd) && ScanType == SCAN_ACTIVE
			&& ScanCtrl->state != OFFCHANNEL_SCAN_COMPLETE) {
#ifdef WIFI_UNIFIED_COMMAND
			struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
#endif
			/*resume fw log and host log when APCLI scan ends*/
			if (ScanCtrl->dbg_lvl_ori != 0xff)
				set_dbg_lvl_all(ScanCtrl->dbg_lvl_ori);
#ifdef WIFI_UNIFIED_COMMAND
			if (cap->uni_cmd_support) {
				MtUniCmdFwLog2Host(pAd, HOST2N9, 1);
			} else
#endif
				MtCmdFwLog2Host(pAd, 0, 1);
			ScanCtrl->dbg_lvl_ori = 0xff;
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_NOTICE,
				"Resume fw and host log after APCLI scan done!!\n");
		}
#endif
		...
		if (ScanCtrl->state == OFFCHANNEL_SCAN_COMPLETE) {
			...
			/*resume fw log and host log when off-ch scan ends*/
			if (ScanCtrl->dbg_lvl_ori != 0xff)
				set_dbg_lvl_all(ScanCtrl->dbg_lvl_ori);
#ifdef WIFI_UNIFIED_COMMAND
			if (cap->uni_cmd_support) {
			...
			ScanCtrl->dbg_lvl_ori = 0xff;
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
				"Resume fw and host log after off-ch scan done!!\n");
#ifdef CONFIG_MAP_SUPPORT
		...
		if (ScanCtrl->PartialScan.bScanning == TRUE) {
			...
			*  We should wait (BeaconPeriod + 100ms), then trigger next scan.*/
			if (ScanCtrl->PartialScan.BreakTime >
				(pAd->CommonCfg.BeaconPeriod / MLME_TASK_EXEC_INTV) + 1) {
				MLME_SCAN_REQ_STRUCT ScanReq;
				...
				if (IF_COMBO_HAVE_AP_STA(pAd) && (wdev->wdev_type == WDEV_TYPE_STA)) {
					...
					pWpsCtrl = &pAd->StaCfg[wdev->func_idx].wdev.WscControl;
...
INT scan_partial_init(RTMP_ADAPTER *pAd)
{
	...
	PartialScanCtrl = &pAd->ScanCtrl.PartialScan;
...
{
	ad->ScanCtrl.SkipCh_Num = 0;
	if (ad->ScanCtrl.SkipList) {
		os_free_mem(ad->ScanCtrl.SkipList);
		ad->ScanCtrl.SkipList = NULL;
	}
...
void wait_scan_stop(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	if (wdev->ScanInfo.scan_running) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_NOTICE,
					"Stop scan for wdev %d\n", wdev->BssIdx);
		RTMP_OS_REINIT_COMPLETION(&wdev->ScanInfo.scan_complete);
		if (!RTMP_OS_WAIT_FOR_COMPLETION_TIMEOUT(&wdev->ScanInfo.scan_complete, WAIT_SCAN_STOP_TIME))
			MTWF_DBG(NULL, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_NOTICE,
				"wait scan timeout for wdev %d!!!\n", wdev->BssIdx);
	}
}

void mark_scan_start_on_wdev(struct wifi_dev *wdev)
{
	SCAN_INFO *ScanInfo = &wdev->ScanInfo;

	ScanInfo->scan_running = TRUE;
	ScanInfo->ScanChannelCnt = 0;
	NdisGetSystemUpTime(&ScanInfo->LastScanTime);
}

void mark_scan_end_on_wdev(struct _RTMP_ADAPTER *pAd)
{
	UCHAR i, band_idx;
	...
	/*loop all wdev to mark scan end*/
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		...
		if (tdev
			...
			SCAN_INFO *ScanInfo = &tdev->ScanInfo;

			MTWF_DBG(NULL, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_NOTICE,
				"wdev[%d] scan completed\n", tdev->BssIdx);

			ScanInfo->scan_running = FALSE;
			RTMP_OS_COMPLETE(&ScanInfo->scan_complete);
		}
	}

}

void force_scan_stop(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR reason)
{
	UCHAR buf[2];

	buf[0] = wdev->func_idx;
	buf[1] = reason;
	if (RTEnqueueInternalCmd(pAd, CMDTHREAD_FORCE_SCAN_STOP, buf, sizeof(buf)
		!= NDIS_STATUS_SUCCESS))
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
			"Enq CMDTHREAD_FORCE_SCAN_STOP failed!\n");
}

NTSTATUS scan_stop_handle(PRTMP_ADAPTER pAd, PCmdQElmt CMDQelmt)
{
	UCHAR apIdx, stop_reason, *ptr = (UCHAR *)CMDQelmt->buffer;
	...
	cntl_scan_conf(wdev, stop_reason);
	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	int alloc_size = 0;
	int ret;
	struct event_bss_entry *EventBssEntry = NULL;
	...
	if (EventBssEntry)
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
				"fail to allocate memory for EventBssEntry\n");
		return -ENOMEM;
	}
...
#endif /* CUSTOMER_VENDOR_IE_SUPPORT */
#ifdef RT_CFG80211_SUPPORT
static void update_survey_active_time(PRTMP_ADAPTER pAd, ktime_t time, UCHAR ch_idx)
{
	if (ch_idx >= MAX_NUM_OF_CHANNELS + 1)
		return;
	pAd->ScanCtrl.active_time[ch_idx] += ktime_to_us(ktime_sub(time,
						  pAd->ScanCtrl.survey_time));
	pAd->ScanCtrl.survey_time = time;
}
#endif
VOID phy_update_channel_info(
	PRTMP_ADAPTER pAd,
	UCHAR ch_index)
{
	struct wifi_dev *wdev = pAd->wdev_list[0];
	...
	if (ch_index >= MAX_NUM_OF_CHANNELS + 1)
		return;
	NdisZeroMemory(Reg, sizeof(Reg));
	...
	pAd->ScanCtrl.tx_time[ch_index] += Reg[1].Value - pAd->ScanCtrl.pre_tx_time;
	pAd->ScanCtrl.bss_rx_time[ch_index] += Reg[2].Value - pAd->ScanCtrl.pre_bss_rx_time;
	pAd->ScanCtrl.rx_time[ch_index] += Reg[0].Value + Reg[2].Value + Reg[3].Value - pAd->ScanCtrl.pre_rx_time;
	pAd->ScanCtrl.busytime[ch_index] += Reg[0].Value + Reg[1].Value + Reg[2].Value + Reg[3].Value - pAd->ScanCtrl.pre_busytime;


	pAd->ScanCtrl.pre_tx_time = Reg[1].Value;
	pAd->ScanCtrl.pre_bss_rx_time = Reg[2].Value;
	pAd->ScanCtrl.pre_rx_time = Reg[0].Value + Reg[2].Value + Reg[3].Value;
	pAd->ScanCtrl.pre_busytime = Reg[0].Value + Reg[1].Value + Reg[2].Value + Reg[3].Value;
	pAd->ScanCtrl.ch_AvgNF[ch_index] = pAd->noise_floor;
#ifdef RT_CFG80211_SUPPORT
	cur_time = mt_ktime_get();
	update_survey_active_time(pAd, cur_time, ch_index);
#endif
}
#endif /* SCAN_SUPPORT */
