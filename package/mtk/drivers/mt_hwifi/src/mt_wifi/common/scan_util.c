	...
	IN  struct wifi_dev *wdev)
{
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) {
		 ...
		 */
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO, "StaSiteSurvey:: Scanning now\n");
	...
	struct wifi_dev		*wdev)
{
	...
	UCHAR ifIdx;
#endif /*ifIdx*/
	struct DOT11_H *pDot11hTest = &pAd->Dot11_H;
...
#endif
		(pDot11hTest->ChannelMode == CHAN_SILENCE_MODE)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN, "INFO::AP is in Silent Mode.DFS CAC is under process!\n");
		return;
	}
#ifdef SCAN_SUPPORT
	...
	if (!TakeChannelOpCharge(pAd, wdev, CH_OP_OWNER_SCAN, FALSE)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN, "TakeChannelOpCharge fail for SCAN!!\n");
		return;
	}
	...
	if ((ifIdx < MAX_APCLI_NUM)
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO, "\033[1;32m ApSiteSurvey_by_wdev don't need Init BSS table\033[0m\n");
	else
#endif /*CON_WPS*/
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO, "INFO::reset ChannelInfo!\n");
