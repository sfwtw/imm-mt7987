	...
	INOUT UINT *CipherBlockSize)
{
	...
	*/
	...
	if (PlainBlockSize != AES_BLOCK_SIZES) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"plain block size is %d bytes, it must be %d bytes(128 bits).\n",
	...
	if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
	...
	if (*CipherBlockSize < AES_BLOCK_SIZES) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"cipher block size is %d bytes, it must be %d bytes(128 bits).\n",
	...
	if (paes_ctx == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "Allocate memory fail!!!\n");
		return;
	}
	...
	INOUT UINT *CipherTextLength)
{
	...
	if ((NonceLength < 7) || (NonceLength > 13)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"A valid nonce length is 7-13 bytes\n");
		return -2;
	}
	...
	if ((MACLength != 4) && (MACLength != 6) && (MACLength != 8) && (MACLength != 10)
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"The MAC length  must be 4, 6, 8, 10, 12, 14, or 16 bytes\n");
		return -3;
	}

	if (*CipherTextLength < (PlainTextLength + MACLength)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"The CipherTextLength is not enough.\n");
		return -4;
	}
	...
	INOUT UINT *PlainTextLength)
{
	...
	if (*PlainTextLength < (CipherTextLength - MACLength)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"The PlainTextLength is not enough.\n");
		return -4;
	}
	...
	*PlainTextLength = CipherTextLength - MACLength;
	...
	if (NdisCmpMemory(Block_MAC_From_Cipher, Block_MAC, MACLength) != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "The MIC does not match.\n");
		return -5;
	}
...
static void xor_block(UINT8 *dst, const UINT8 *src)
{
	UINT32 *d = (UINT32 *) dst;
	UINT32 *s = (UINT32 *) src;
	*d++ ^= *s++;
...
static inline UINT32 len32_str_to_val(const UINT8 *a)
{
	return ((UINT32) a[0] << 24) | (a[1] << 16) | (a[2] << 8) | a[3];
}

static inline void val_to_len32_str(UINT8 *a, UINT32 val)
{
	a[0] = (val >> 24) & 0xff;
	a[1] = (val >> 16) & 0xff;
	a[2] = (val >> 8) & 0xff;
...
static void shift_right_block(UINT8 *v)
{
	UINT32 val;

	val = len32_str_to_val(v + 12);
	val >>= 1;
	if (v[11] & 0x01)
		val |= 0x80000000;
	val_to_len32_str(v + 12, val);

	val = len32_str_to_val(v + 8);
	val >>= 1;
	if (v[7] & 0x01)
		val |= 0x80000000;
	val_to_len32_str(v + 8, val);

	val = len32_str_to_val(v + 4);
	val >>= 1;
	if (v[3] & 0x01)
		val |= 0x80000000;
	val_to_len32_str(v + 4, val);

	val = len32_str_to_val(v);
	val >>= 1;
	val_to_len32_str(v, val);
}

/* Multiplication in GF(2^128) */
static void gf_mult(const UINT8 *x, const UINT8 *y, UINT8 *z)
{
	UINT8 v[16];
	int i, j;

	NdisZeroMemory(z, 16); /* Z_0 = 0^128 */
	NdisMoveMemory(v, y, 16); /* V_0 = Y */
...
static void ghash(const UINT8 *h, const UINT8 *x, UINT32 xlen, UINT8 *y)
{
	UINT32 m, i;
	const UINT8 *xpos = x;
	UINT8 tmp[16];
	...
	for (i = 0; i < m; i++) {
		 ...
		 * 2^128 elements */
		gf_mult(y, h, tmp);
		NdisMoveMemory(y, tmp, 16);
	}

	if (x + xlen > xpos) {
		/* Add zero padded last block */
		UINT32 last = x + xlen - xpos;

		NdisMoveMemory(tmp, xpos, last);
		NdisZeroMemory(tmp + last, sizeof(tmp) - last);
...
static inline void val_to_len64_str(UINT8 *a, UINT64 val)
{
	a[0] = val >> 56;
	a[1] = val >> 48;
	a[2] = val >> 40;
...
static inline UINT32 WPA_GET_BE32(const UINT_8 *a)
{
	return ((UINT_32) a[0] << 24) | (a[1] << 16) | (a[2] << 8) | a[3];
}

static inline void WPA_PUT_BE32(UINT_8 *a, UINT_32 val)
{
	a[0] = (val >> 24) & 0xff;
	a[1] = (val >> 16) & 0xff;
	a[2] = (val >> 8) & 0xff;
...
static void inc32(UINT8 *block)
{
	UINT32 val;

	val = WPA_GET_BE32(block + AES_BLOCK_SIZES - 4);
	val++;
	WPA_PUT_BE32(block + AES_BLOCK_SIZES - 4, val);
}
...
{
	UINT32 i, n, last;
	UINT8 cb[AES_BLOCK_SIZES], tmp[AES_BLOCK_SIZES];
	const UINT8 *xpos = x;
	UINT8 *ypos = y;
	UINT y_size = AES_BLOCK_SIZES;
	UINT tmp_size = AES_BLOCK_SIZES;
	...
	NdisMoveMemory(cb, icb, AES_BLOCK_SIZES);
	/* Full blocks */
	for (i = 0; i < n; i++) {
		RT_AES_Encrypt(cb, AES_BLOCK_SIZES, key, key_len, ypos, &y_size);
		xor_block(ypos, xpos);
		xpos += AES_BLOCK_SIZES;
		ypos += AES_BLOCK_SIZES;
		inc32(cb);
	}

	NdisZeroMemory(tmp, AES_BLOCK_SIZES);
	last = x + xlen - xpos;
	if (last) {
		/* Last, partial block */
		RT_AES_Encrypt(cb, AES_BLOCK_SIZES, key, key_len, tmp, &tmp_size);
		for (i = 0; i < last; i++)
			*ypos++ = *xpos++ ^ tmp[i];
	}
}
...
VOID AES_GMAC(
	...
	OUT UINT8 *tag)
{
	UINT8 PH[AES_BLOCK_SIZES];
	UINT8 H[AES_BLOCK_SIZES];
	UINT8 J0[AES_BLOCK_SIZES];
	UINT8 S[16];
	UINT8 len_buf[16];
	UINT h_size = AES_BLOCK_SIZES;

	/* Generate hash subkey H = AES_K(0^128) */
	NdisZeroMemory(PH, AES_BLOCK_SIZES);
	NdisZeroMemory(H, AES_BLOCK_SIZES);
	RT_AES_Encrypt(PH, AES_BLOCK_SIZES, key, key_len, H, &h_size);

	/*if (NonceLength == 12)
		return;*/

	/* Prepare block J_0 = IV || 0^31 || 1 [len(IV) = 96] */
	NdisMoveMemory(J0, nonce, nonce_len);
	NdisZeroMemory(J0 + nonce_len, AES_BLOCK_SIZES - nonce_len);
	J0[AES_BLOCK_SIZES - 1] = 0x01;

	/* C = GCTR_K(inc_32(J_0), P) */
	NdisZeroMemory(S, 16);
	ghash(H, aad, aad_len, S);
	val_to_len64_str(len_buf, aad_len * 8);
	val_to_len64_str(len_buf + 8, 0);
	...
	INOUT UINT *CipherTextLength)
{
	...
	if (*CipherTextLength < (PlainTextLength + PaddingSize)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"cipher text length is %d bytes < (plain text length %d bytes + padding size %d bytes).\n",
	...
	if (IVLength != AES_CBC_IV_LENGTH) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"IV length is %d bytes, it must be %d bytes(128bits).\n",
	...
	INOUT UINT *PlainTextLength)
{
	...
	if ((CipherTextLength % AES_BLOCK_SIZES) != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"cipher text length is %d bytes, it can't be divided with no remainder by block size(%d).\n",
	...
	OUT UINT *CipherTextLength)
{
	...
	if (pResult == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR,
			"allocate %zu bytes memory failure.\n", sizeof(UINT8)*PlainTextLength);
		return -2;
	}
