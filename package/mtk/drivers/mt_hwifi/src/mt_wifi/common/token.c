	...
	struct token_tx_pkt_queue *que)
{
	...
	if (que->token_inited == TRUE) {
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
			"%p,%p\n", que, &que->token_inited);
		OS_SEM_LOCK(&que->enq_lock);
		que->token_inited = FALSE;
		OS_SEM_UNLOCK(&que->enq_lock);
	...
	struct token_tx_pkt_queue *que)
{
	...
	if (que->token_inited == FALSE) {
		...
		if (qidx == 0) {
			...
			MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
				"ct sw token(%d) number = %d\n",
				 qidx, que->pkt_tkid_cnt);
		} else if (qidx == 1) {
		...
		if (!que->pkt_token) {
			que->token_inited = TRUE;
			MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
				"token que pkt_token inited fail!\n");
			return NDIS_STATUS_RESOURCES;
		}
		...
		que->token_inited = TRUE;
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
			"token que(%d) inited done!id_head/tail=%d/%d\n",
			qidx, que->id_head, que->id_tail);
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
			"%p,%p\n",
			que, &que->token_inited);
	} else {
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
			"token que(%d) already inited!shall not happened!\n",
			qidx);

		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
			"\ttoken que(%d) id_head=%d, list.id_tail=%d\n",
			qidx, que->id_head, que->id_tail);
	}
	...
	struct token_tx_pkt_queue *que)
{
	...
	if (que->token_inited == FALSE) {
...
#ifdef WHNAT_SUPPORT
		if (IS_ASIC_CAP(pAd, fASIC_CAP_WHNAT) && PD_GET_WHNAT_ENABLE(pAd->physical_dev)) {
		...
		que->token_inited = TRUE;
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_INFO,
			"token que inited done!id_head/tail=%d/%d\n",
		...
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
			"token que already inited!shall not happened!\n");
		...
		if (que) {
			if (token <= que->pkt_tkid_max) {
				...
				*type = entry->Type;
				...
				if (pkt_buf == NULL) {
					MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
						"buggy here? token ID(%d) without pkt!\n", token);
					OS_SEM_UNLOCK(&que->deq_lock);
					if (!is_dump_wa) {
				...
				if (ATE_ON(pAd)) {
					...
					if (que->total_enq_cnt_per_band[band_idx] > 0)
						que->total_enq_cnt_per_band[band_idx]--;
				}
#endif
			} else {
				MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
...
#ifdef CONFIG_WLAN_SERVICE
			if (ATE_ON(pAd)) {
				UCHAR band_idx = RTMP_GET_BAND_IDX(pkt);
				que->total_enq_cnt_per_band[band_idx]++;
			}
#endif
		} else {
...
static INT token_rx_init(PKT_TOKEN_CB *cb, RTMP_ADAPTER *ad)
{
	...
	if (!que->pkt_token) {
		MTWF_DBG(ad, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR,
			"token que pkt_token inited fail!\n");
		return NDIS_STATUS_RESOURCES;
	}
		...
		if (alloc_pa == que->pkt_token[token_loop].dma_buf.AllocPa) {
			...
			*pkt = que->pkt_token[token_loop].pkt_buf;
			...
			MTWF_DBG(NULL, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_DEBUG,
				"tkn_rx_id: %d according to %04x\n",
				*token_id, (UINT32)alloc_pa);
