...
static INT vie_sanity_check(UINT32 oper, UINT32 input_argument, UINT32 frm_map, UINT32 length, UCHAR *oui, UCHAR *ctnt)
{
	...
	if (oper >= VIE_OPER_MAX) {
		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_WARN,
			"oper error:%d\n", oper);
...
INT vie_oper_proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_NOTICE,
		...
		if ((oper == VIE_ADD) || (oper == VIE_UPDATE) || (oper == VIE_REMOVE)) {
			...
			if (oper == VIE_REMOVE) {
				if (remove_vie(pAd,
					...
					ret = FALSE;
					MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_WARN,
						"remove failed.\n");
				}
			} else {
				if (add_vie(pAd,
...
VOID build_mtk_cap_vendor_ie(struct _RTMP_ADAPTER *pAd,
		      struct wifi_dev *wdev,
		      struct _mediatek_ie *mtk_ie)
{
	...
	if (mtk_ie) {
		...
		if (pAd->CommonCfg.g_band_256_qam && MCS_NSS_CAP(pAd)->g_band_256_qam
...
ULONG build_mtk_tlv1_vendor_ie(struct _RTMP_ADAPTER *pAd,
		      struct wifi_dev *wdev,
		      UCHAR *frame_buffer,
		      VIE_FRM_TYPE vie_frm_type,
		      UCHAR mtk_vendor_ie_tlv1_type)
{
	struct _mediatek_ie mtk_ie;
	struct _mtk_tlv1_type4 mtk_tlv1_type4;
	ULONG mtk_ie_len = 0, vendor_ie_len = 0;
#ifdef DOT11_EHT_BE
	struct _mtk_tlv1_type2 *mtk_tlv1_type2 = NULL;
	UCHAR mtk_tlv1_typ2_buf[255];
#endif
#ifdef MLR_SUPPORT
	struct _mtk_tlv1_type1 mtk_tlv1_type1;
#endif

	/* Build MTK TLV1 IE */
	NdisZeroMemory(&mtk_ie, sizeof(struct _mediatek_ie));
	/* Generate mtk cap vendor IE */
	build_mtk_cap_vendor_ie(pAd, wdev, &mtk_ie);
	/* Generate mtk TLV1 cap */
	mtk_ie.cap0 |= MEDIATEK_TLV1_CAP;

	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE4)) {
		/* Build TLV1 TYPE4 IE */
		NdisZeroMemory(&mtk_tlv1_type4, sizeof(struct _mtk_tlv1_type4));
		mtk_tlv1_type4.type = MEDIATEK_TLV1_TYPE4;
		mtk_tlv1_type4.len = 8;
		mtk_tlv1_type4.val[0] |= chip_determine_mlo_type(pAd, wdev);
		mtk_tlv1_type4.val[1] |= chip_determine_hmlo_support(pAd, wdev);

		mtk_ie.ie_hdr.len += sizeof(struct _mtk_tlv1_type4);
	}
#ifdef DOT11_EHT_BE
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE2)) {
		/* build TLV1 TYPE2 IE */
		NdisZeroMemory(&mtk_tlv1_typ2_buf[0], sizeof(mtk_tlv1_typ2_buf));
		mtk_tlv1_type2 = (struct _mtk_tlv1_type2 *)&mtk_tlv1_typ2_buf[0];
		mtk_tlv1_type2->type = MEDIATEK_TLV1_TYPE2;
		mtk_tlv1_type2->ver0 = 0x0;
		mtk_tlv1_type2->ver1 = 0x2;
		/* shall be more than 2, due to ver0 ver1 */
		mtk_tlv1_type2->len = 2;

		if (vie_frm_type == VIE_BEACON && wdev->bcn_buf.csa.ml_ie_offset != 0)
			wdev->bcn_buf.csa.ml_ie_offset += mtk_tlv1_type2->len;

		mtk_tlv1_type2->len += eht_add_beacon_ies(wdev, &mtk_tlv1_type2->ie[0], 0);

		mtk_ie.ie_hdr.len += mtk_tlv1_type2->len + 2;
	}
#endif
#ifdef MLR_SUPPORT
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE1)) {
		/* Build TLV1 TYPE1 IE */
		NdisZeroMemory(&mtk_tlv1_type1, sizeof(struct _mtk_tlv1_type1));
		mtk_tlv1_type1.type = MEDIATEK_TLV1_TYPE1;
		mtk_tlv1_type1.len = 1;
		if (pAd->CommonCfg.MLRVersion == MLR_MODE_MLR_V2)
			mtk_tlv1_type1.val[0] |= 0x3;
		else
			mtk_tlv1_type1.val[0] |= 0x1;

		mtk_ie.ie_hdr.len += sizeof(struct _mtk_tlv1_type1);
	}
#endif
	/* make MTK TLV1 IE */
	...
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE4)) {
		/* make TLV1 TYPE4 IE */
		MakeOutgoingFrame((frame_buffer + vendor_ie_len),
						  &mtk_ie_len, sizeof(struct _mtk_tlv1_type4), &mtk_tlv1_type4,
						  END_OF_ARGS);
		vendor_ie_len += mtk_ie_len;
	}
	...
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE2)) {
		/* make TLV1 TYPE2 IE */
		MakeOutgoingFrame((frame_buffer + vendor_ie_len),
						  &mtk_ie_len, mtk_tlv1_type2->len + 2, mtk_tlv1_type2,
						  END_OF_ARGS);
		vendor_ie_len += mtk_ie_len;
	}
#endif
	...
	if (mtk_vendor_ie_tlv1_type & BIT(MEDIATEK_TLV1_TYPE1)) {
		/* make TLV1 TYPE4 IE */
		MakeOutgoingFrame((frame_buffer + vendor_ie_len),
						  &mtk_ie_len, sizeof(struct _mtk_tlv1_type1), &mtk_tlv1_type1,
						  END_OF_ARGS);
		vendor_ie_len += mtk_ie_len;
	}
#endif
		      ...
		      VIE_FRM_TYPE vie_frm_type)
{
	...
	if (WMODE_CAP_2G(wdev->PhyMode) && !pAd->CommonCfg.g_band_256_qam)
		return vendor_ie_len;
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		mtk_ie.ie_hdr.len += mtk_vht_ie_len;
...
UINT parse_caps_in_vendor_ie(struct _mtk_tlv1_type2 *mtk_tlv1_type2_ptr, struct _vendor_ie_cap *vendor_ie)
{
	struct _mtk_tlv1_type2 *mtk_tlv1_type2 = (struct _mtk_tlv1_type2 *)mtk_tlv1_type2_ptr;
	UCHAR		eid = 0, eid_len = 0, eid_tag;
	UINT		total_ie_len = 0, mkt_ie_len;

	if (mtk_tlv1_type2->len > sizeof(struct _mtk_tlv1_type2))
		mkt_ie_len = mtk_tlv1_type2->len - 2;
	else {
		MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_DEBUG,
			"Malformed IE, break here.\n");
		return total_ie_len;
	}
	/* calculate v0 & v1 first*/
	while ((total_ie_len + 2) < mkt_ie_len) {
		eid = mtk_tlv1_type2->ie[total_ie_len];
		eid_len = mtk_tlv1_type2->ie[total_ie_len + 1];

		if (eid == IE_WLAN_EXTENSION
			&& ((total_ie_len + 3) < mkt_ie_len)) {
			eid_tag = mtk_tlv1_type2->ie[total_ie_len + 2];
#ifdef DOT11_EHT_BE
			if (eid_tag == EID_EXT_EHT_MULTI_LINK)
				SET_VENDOR_EHT_MLD_EXIST(vendor_ie->ie_exists);
			else if (eid_tag == EID_EXT_EHT_CAPS) {
				SET_VENDOR_EHT_CAPS_EXIST(vendor_ie->ie_exists);
#ifdef CONFIG_MAP_SUPPORT
				if (total_ie_len + 14 >= mkt_ie_len) {
					MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_DEBUG,
						"Malformed IE, break here.\n");
					break;
				}
				vendor_ie->txrx_stream = mtk_tlv1_type2->ie[total_ie_len + 14];
#endif
			} else if (eid_tag == EID_EXT_EHT_OP) {
				SET_VENDOR_EHT_OP_EXIST(vendor_ie->ie_exists);
#ifdef CONFIG_MAP_SUPPORT
				/*eht op ie exist*/
				if ((mtk_tlv1_type2->ie[total_ie_len + 3]&0x1) != 0) {
					if (total_ie_len + 10 >= mkt_ie_len) {
						MTWF_DBG(NULL, DBG_CAT_MLME,
							CATMLME_IE_INFO, DBG_LVL_DEBUG,
							"Malformed IE, break here.\n");
						break;
					}
					vendor_ie->eht_op.op_info.control = mtk_tlv1_type2->ie[total_ie_len + 8];
					vendor_ie->eht_op.op_info.ccfs0 = mtk_tlv1_type2->ie[total_ie_len + 9];
					vendor_ie->eht_op.op_info.ccfs1 = mtk_tlv1_type2->ie[total_ie_len + 10];
				}
#endif
			}
#endif /* DOT11_EHT_BE */
...
VOID parse_mtk_tlv1_vendor_ie(struct _RTMP_ADAPTER *pAd, UCHAR *ie_buffer, struct _vendor_ie_cap *vendor_ie)
{
	PEID_STRUCT info_elem = (PEID_STRUCT)ie_buffer;
	UCHAR mtk_tlv1_type = 0;
	struct _mtk_tlv1 *mtk_tlv1 = NULL;
	UINT i = 0;

	if (info_elem->Octet[3] & MEDIATEK_TLV1_CAP) {
		i = 7;
		while ((i + 2) < info_elem->Len) {
			mtk_tlv1 = (struct _mtk_tlv1 *)&info_elem->Octet[i];
			mtk_tlv1_type = mtk_tlv1->type;

			if (mtk_tlv1->len == 0) {
				MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_DEBUG,
					"No more data or malformed IE, break here.\n");
				break;
			}

			if ((i + 2 + mtk_tlv1->len) > info_elem->Len) {
				MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_DEBUG,
					"No more data or malformed IE, break here.\n");
				break;
			}

			if (mtk_tlv1_type == MEDIATEK_TLV1_TYPE4) {
				struct _mtk_tlv1_type4 *mtk_tlv1_type4 = (struct _mtk_tlv1_type4 *)mtk_tlv1;

				if (mtk_tlv1_type4->len != 8) {
					MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_DEBUG,
						"Malformed IE, break here.\n");
					break;
				}
				/* Type 4 */
				if (mtk_tlv1_type4->val[0] & MEDIATEK_TLV1_TYPE4_MLO_V1)
					vendor_ie->mtk_mlo_enable = MEDIATEK_TLV1_TYPE4_MLO_V1;
				else if (mtk_tlv1_type4->val[0] & MEDIATEK_TLV1_TYPE4_MLO_V2)
					vendor_ie->mtk_mlo_enable = MEDIATEK_TLV1_TYPE4_MLO_V2;
				else if (mtk_tlv1_type4->val[0] & MEDIATEK_TLV1_TYPE4_MLO_V1_1)
					vendor_ie->mtk_mlo_enable = MEDIATEK_TLV1_TYPE4_MLO_V1_1;

				if (mtk_tlv1_type4->val[0] & MEDIATEK_TLV1_TYPE4_MLO_FORCE)
					vendor_ie->mtk_mlo_enable |= MEDIATEK_TLV1_TYPE4_MLO_FORCE;
			} else if (mtk_tlv1_type == MEDIATEK_TLV1_TYPE2) {
				parse_caps_in_vendor_ie((struct _mtk_tlv1_type2 *)mtk_tlv1, vendor_ie);
#ifdef MLR_SUPPORT
			} else if (mtk_tlv1_type == MEDIATEK_TLV1_TYPE1) {
				struct _mtk_tlv1_type1 *mtk_tlv1_type1 = (struct _mtk_tlv1_type1 *)mtk_tlv1;

				if (mtk_tlv1_type1->len == 1)
					vendor_ie->mtk_mlr_cap = mtk_tlv1_type1->val[0];
#endif /* MLR_SUPPORT */
			}
		     ...
		     struct _vendor_ie_cap *vendor_ie)
{
	...
	UCHAR mediatek_oui[] = VENDOR_OUI_MTK;
...
#define MAP_TURNKEY_IE(B1)            ((B1)&0x01)
		...
		if (info_elem->Len > 7) {
			...
			/* Parse TLV1 if TLV1 CAP existed */
			parse_mtk_tlv1_vendor_ie(pAd, ie_buffer, vendor_ie);
				    ...
				    BCN_IE_LIST *ie_list)
{
	...
	if (ret == NDIS_STATUS_FAILURE) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_IE_INFO, DBG_LVL_ERROR,
			"\n\n\nALLOC Memory fail\n\n\n");
		return;
	}
