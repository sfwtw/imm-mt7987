...
static INT dvt_ut_seudo_sta_he_template(struct _RTMP_ADAPTER *ad, struct dvt_seudo_sta *sta)
{
	...
	peer->cap.ch_bw.he_ch_width = SUPP_40M_CW_IN_24G_BAND | SUPP_40M_80M_CW_IN_5G_6G_BAND;
...
INT dvt_ut_seudo_sta_connect(struct _RTMP_ADAPTER *ad, struct dvt_seudo_sta *sta)
{
	...
	union _HTTRANSMIT_SETTING ht_phy_mode;
	...
	entry = MacTableInsertEntry(ad, sta->addr, wdev,
	...
	if (entry) {
		...
		os_zero_mem(&ht_phy_mode, sizeof(union _HTTRANSMIT_SETTING));
		...
		if (WMODE_CAP_AC(phy_mode) && WMODE_CAP_5G(wdev->PhyMode)) {
			...
			vht_mode_adjust(ad, entry, &vht_cap, NULL, NULL, NULL);
			...
			DVT_LOG("PhyCap&Mode,%s&BW,%s&MC,0x%x&Word,0x%x",
					 ...
					 get_bw_str(entry->MaxHTPhyMode.field.BW, BW_FROM_OID),
...
{
	if (sta->mac_entry)
		mac_entry_delete(ad, sta->mac_entry, TRUE);
}
