...
#define FT_RIC_AP_LIST_INSERT_TAIL(__pAd, __pList, __pEntry)		\
	{																\
		OS_SEM_LOCK(&(__pAd)->FT_RicLock);						\
		insertTailList((__pList), (RT_LIST_ENTRY *)(__pEntry));		\
		OS_SEM_UNLOCK(&(__pAd)->FT_RicLock);						\
	}
...
{
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ap_ftrc> Initialize FT RIC Module...\n");
#ifdef CONFIG_STA_SUPPORT
	...
	if (pAd->pFT_RIC_Ctrl_BK == NULL) {
		...
		if (pAd->pFT_RIC_Ctrl_BK == NULL) {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, "ap_ftrc> Allocate control block fail!\n");
			return;
		} /* End of if */
...
VOID TYPE_FUNC FT_RIC_Release(
	IN	PRTMP_ADAPTER		pAd)
{
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ap_ftrc> Release FT RIC Module...\n");
#ifdef CONFIG_STA_SUPPORT
	...
	if (pAd->pFT_RIC_Ctrl_BK != NULL) {
		...
		os_free_mem(pAd->pFT_RIC_Ctrl_BK);
		...
		*/
				...
				*/
				...
				case IE_FT_RIC_DESCRIPTOR:
					...
					if (BA_ResourceAllocate(pAd,
											...
											pBufRspElm,
											&RspLen) == 0) {
						MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ft_ric> Accept BA.\n");
		...
		if (FlgIsTspecAccepted == TRUE) {
			...
			*/
			...
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ft_ric> Resource request fails!\n");
		} /* End of if */
	...
	OUT ULONG *pBufRspLen)
{
	...
	if (!ba_decline && IS_HT_STA(pMacEntry)) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "Rcv Wcid(%d) AddBAReq\n", pMacEntry->wcid);

		if (ba_resrc_rec_add(pAd, pMacEntry->wcid, AddreqFrame.BaParm.TID,
			AddreqFrame.TimeOutValue, AddreqFrame.BaStartSeq.field.StartSeq, 64))
			Status = 0;
		else
	...
	ba_rx_wsize = wlan_config_get_ba_rx_wsize(pMacEntry->wdev);
	BaParm.BufSize = min(((UINT16)AddreqFrame.BaParm.BufSize), (UINT16)ba_rx_wsize);
