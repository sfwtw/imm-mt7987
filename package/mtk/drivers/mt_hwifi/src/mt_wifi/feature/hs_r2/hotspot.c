...
void wext_hotspot_onoff_event(PNET_DEV net_dev, int onoff)
{
	...
	if (buf == NULL) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_ERROR,
	...
	if ((pMbss->HotSpotCtrl.HotSpotEnable)
		...
		if (srcPort  == 0x43 && dstPort == 0x44) {
			...
			if (*pWcid == wdev->bss_info_argument.bmc_wlan_idx && pMbss->HotSpotCtrl.DGAFDisable) {
				...
				MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO,
					"\033[1;32m wcid before convert %d \033[0m\n",
					*pWcid);
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_DEBUG,
				"Convert broadcast dhcp to unicat frame when dgaf disable\n");

				if (wdev_idx < MAX_BEACON_NUM) {
					if (ops->tx_pkt_allowed(pAd, &pAd->ApCfg.MBSSID[wdev_idx].wdev, pPacket) != NDIS_STATUS_SUCCESS)
						return FALSE;
				}

				MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO,
					"\033[1;32m wcid after convert %d \033[0m\n",
					*pWcid);
			}
		}
	}
...
BOOLEAN L2FilterInspection(
	...
	IN USHORT DataSize)
{
	if (IsICMPv4EchoPacket(pAd, pData, DataSize)) {
...
VOID hotspot_update_bssflag(RTMP_ADAPTER *pAd, UINT8 flag, UINT8 value, PHOTSPOT_CTRL pHSCtrl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO,
		"===> flag 0x%x value %d pHSCtrl->HotspotBSSFlags 0x%x\n"
...
#ifdef CONFIG_AP_SUPPORT
	UpdateBeaconHandler(pAd, &pAd->ApCfg.MBSSID[Event->ControlIndex].wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
#endif /* CONFIG_AP_SUPPORT */
...
#if defined(CONFIG_HOTSPOT) || defined(CONFIG_PROXY_ARP)
BOOLEAN hotspot_rx_snoop(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, struct _RX_BLK *pRxBlk)
{
	BOOLEAN drop = FALSE;
	...
	if (IsIpv6DuplicateAddrDetect(pAd, pData, pDataEnd, &Offset)) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO, "Drop IPv6 DAD\n");
		drop = TRUE;
		goto done;
	}
...
#ifdef CONFIG_HOTSPOT
BOOLEAN hotspot_rx_l2_filter(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, struct _RX_BLK *pRxBlk)
{
	...
	PUCHAR pData = pRxBlk->pData + 12;
	USHORT DataSize = pRxBlk->DataSize - 12;
	pRxEntry = MacTableLookup(pAd, pRxBlk->Addr3);

	if (pMbss->HotSpotCtrl.L2Filter == L2FilterBuiltIn) {
		BOOLEAN NeedDrop = FALSE;

		if (pRxEntry)
			NeedDrop = L2FilterInspection(pAd, &pMbss->HotSpotCtrl, pData, DataSize);
		...
		if (NdisEqualMemory(SNAP_802_1H, pRxBlk->pData, 6)) {
			pData = pRxBlk->pData + 6;
			DataSize = pRxBlk->DataSize - 6;
		} else {
			pData = pRxBlk->pData;
			DataSize = pRxBlk->DataSize;
		}

		NeedSendToExternal = L2FilterInspection(pAd, &pMbss->HotSpotCtrl, pData, DataSize);
	...
	if (IsTDLSPacket(pAd, pData_tdls)) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO, "Drop TDLS Packet\n");
		drop = TRUE;
	}
...
BOOLEAN hotspot_osu_data_handler(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, struct _RX_BLK *pRxBlk)
{
	BOOLEAN drop = FALSE;
	MAC_TABLE_ENTRY *pA3Entry;
	BSS_STRUCT *pMbss = NULL, *pA3Mbss;

	pA3Entry = MacTableLookup(pAd, pRxBlk->Addr3);

	pMbss = MBSS_GET(pEntry->pMbss);
	if (IS_AKM_OSEN(pEntry->SecConfig.AKMMap))
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HS_R2, DBG_LVL_INFO,
				"pEntry has OSEN AKM\n");
	else {
		if (pA3Entry && pMbss
	...
	if ((pMbss) && ((pA3Entry) && (pA3Entry->pMbss))) {
		pA3Mbss = MBSS_GET(pA3Entry->pMbss);
...
#if defined(CONFIG_HOTSPOT) || defined(CONFIG_PROXY_ARP)
BOOLEAN hotspot_rx_handler(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, struct _RX_BLK *pRxBlk)
{
	BOOLEAN drop = FALSE;

	if (MBSS_GET(pEntry->pMbss)->WNMCtrl.ProxyARPEnable)
		drop = hotspot_rx_snoop(pAd, pEntry, pRxBlk);

#ifdef CONFIG_HOTSPOT
	if ((drop == FALSE) && (MBSS_GET(pEntry->pMbss)->HotSpotCtrl.L2Filter != L2FilterDisable))
