	...
	UCHAR *buf)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_DEBUG,
		"[PMF] RSNIE Capability MFPC=%d, MFPR=%d\n",
	...
	struct wifi_dev *wdev)
{
	...
	if (IS_FD_FRAME_FW_MODE(cap)) {
		HW_SET_FRAME_OFFLOAD(pAd, wdev->wdev_idx, FrameLen, TRUE,
			0 /* wcid */, 1 /* BSSINFO_UNSOLICIT_TX_FILS_DISC */, 0, 200, pOutBuffer);
	...
	PRT_802_11_STA_MLME_EVENT mlmeEvent)
{
	...
	if (filsInfo->pending_decrypt) {
		...
		if (offset <= ARRAY_SIZE(pElem->Msg) && offset <= filsInfo->pending_ie_len)
		...
		if (offset < ARRAY_SIZE(pElem->Msg) && ARRAY_SIZE(pElem->Msg) - offset >= mlmeEvent->len)
			NdisMoveMemory(&pElem->Msg[offset], mlmeEvent->ie, mlmeEvent->len);
		else {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR,
			"mlmeEvent->len is invalid!\n");
			os_free_mem(pElem);
			return;
		}
	...
	PRT_802_11_STA_MLME_EVENT mlmeEvent)
{
	...
	if (mlmeEvent->len > MAX_OPT_IE)
		mlmeEvent->len = MAX_OPT_IE;
	...
	wrq->u.data.length = sizeof(RT_802_11_STA_MLME_EVENT);
	if (copy_to_user(wrq->u.data.pointer, mlmeEvent, wrq->u.data.length)) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR, "copy_to_user() fail\n");
		return;
	}
	...
	PRT_802_11_STA_MLME_EVENT mlmeEvent)
{
	...
	MiniportMMRequest(pAd, 0, mlmeEvent->ie, mlmeEvent->len, NULL);
	...
	IN      RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (pMlmeEvent->len > sizeof(pMlmeEvent->ie)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_MISC, DBG_LVL_ERROR, "invalid ie len\n");
		os_free_mem(pMlmeEvent);
		return;
	}
	...
	if (pEntry != NULL) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_INFO,
			MACSTR", action: %d, algo: %d)\n", MAC2STR(pMlmeEvent->addr),
			pMlmeEvent->mgmt_subtype, pMlmeEvent->auth_algo);

		if (pMlmeEvent->mgmt_subtype == SUBTYPE_AUTH) {
			if (pMlmeEvent->fils_kek_len > WPA_KEK_MAX_LEN)
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_WARN,
					"tainted data.. check\n");
			else
	...
	IN      RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (pSyncEvent->GTK_len <= ARRAY_SIZE(pSyncEvent->GTK) &&
			pSyncEvent->GTK_len <= ARRAY_SIZE(pSecConfig->GTK))
	...
	if ((pSyncEvent->IGN - 4 + pSyncEvent->IGTK_len) <= ARRAY_SIZE(pSecConfig->PmfCfg.IGTK))
		NdisMoveMemory(pSyncEvent->IGTK, pSecConfig->PmfCfg.IGTK[pSyncEvent->IGN-4], pSyncEvent->IGTK_len);
	else {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR,
			"Length of IGN is invalid.\n");
		os_free_mem(pSyncEvent);
		return;
	}
	...
	if (pSyncEvent->rsne_len > MAX_OPT_IE) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR,
			"syncEvent.rsne_len > MAX_OPT_IE) fail\n");
		os_free_mem(pSyncEvent);
		return;
	}
	...
	IN      RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (KeyEvent == NULL) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR, "os_alloc_mem fail.\n");
		return;
		}
	else
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_INFO,
		"wrq->u.data.length = %d, sizeof(RT_802_11_KEY_EVENT) = %d\n",
		wrq->u.data.length, (UINT)(sizeof(RT_802_11_KEY_EVENT)));
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_INFO,
		MACSTR" action: %d)\n", MAC2STR(keyInfo->addr), KeyEvent->action);

	pEntry = MacTableLookup(pAd, keyInfo->addr);

	if (!pEntry) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR, "Entry not found\n");
		os_free_mem(KeyEvent);
		return;
	}
	...
	IN      RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (PmkCacheEvent.res == PMK_CACHE_QUERY) {
		INT		CacheIdx;
		/* Search PMK Cache */
		CacheIdx = RTMPSearchPMKIDCacheByPmkId(PD_GET_PMKID_PTR(pAd->physical_dev),
			pEntry->wdev->bssid, pEntry->Addr, PmkCacheEvent.pmkid, FALSE);
