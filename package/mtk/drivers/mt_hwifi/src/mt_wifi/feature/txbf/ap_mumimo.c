...
error:
	MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, Ret ? DBG_LVL_DEBUG : DBG_LVL_ERROR,
			 "(Ret = %d\n", Ret);
	return Ret;
}
...
INT SetClusterTblEntryProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	AndesAppendCmdMsg(msg, (char *)&param, sizeof(param));
	AndesSendCmdMsg(pAd, msg);
	MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, DBG_LVL_DEBUG, "msg is sent\n");
error:
	MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, Ret ? DBG_LVL_DEBUG : DBG_LVL_ERROR,
			 "(Ret = %d\n", Ret);
	return Ret;
...
INT SetMuNdpDeltaTxPwr(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (arg == NULL) {
		MTWF_DBG(pAd, DBG_CAT_FW, CATBF_CFG, DBG_LVL_ERROR,
			"arg is NULL\n");
		Ret = FALSE;
		goto error;
	}
...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, DBG_LVL_INFO,
	...
	AndesSendCmdMsg(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, (Ret == 0) ? DBG_LVL_DEBUG : DBG_LVL_ERROR,
		"Ret=%d\n", Ret);
	return Ret;
}
...
)
{
	...
	if (ptr->type == SU) {
		...
		MTWF_DBG(pAd, DBG_CAT_BF, CATBF_CFG, DBG_LVL_ERROR, "Error type!\n");
		Ret = -1;
		goto error;
	}
