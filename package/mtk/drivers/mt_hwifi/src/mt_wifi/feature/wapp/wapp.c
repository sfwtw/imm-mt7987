...
#ifdef CONFIG_MAP_SUPPORT
	ObssAirTime = Get_OBSS_AirTime_MAP(pAd, i);
	MyTxAirTime = Get_My_Tx_AirTime_MAP(pAd, i);
	MyRxAirTime = Get_My_Rx_AirTime_MAP(pAd, i);
#else
	ObssAirTime = Get_OBSS_AirTime(pAd, i);
	MyTxAirTime = Get_My_Tx_AirTime(pAd, i);
	MyRxAirTime = Get_My_Rx_AirTime(pAd, i);
#endif /* CONFIG_MAP_SUPPORT */

#ifdef EM_PLUS_SUPPORT
	EDCCA = Get_EDCCA_Time(pAd, i);
#endif
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
	"One sec values Channel %d,Band %d, ObssAirTime %d, MyTxAirTime %d, MyRxAirTime %d\n",
	Channel, i, ObssAirTime, MyTxAirTime, MyRxAirTime);
	if (ObssAirTime != 0)
		ObssAirOccupyPercentage = (ObssAirTime*100)/ONE_SEC_2_US;
#ifdef EM_PLUS_SUPPORT
	if (EDCCA != 0)
		EDCCAOccupyPercentage = (EDCCA*100)/ONE_SEC_2_US;
#endif

	if (MyTxAirTime != 0 || MyRxAirTime != 0)
		MyAirOccupyPercentage = ((MyTxAirTime + MyRxAirTime) * 100)/ONE_SEC_2_US;

	if (MyAirOccupyPercentage >= 100) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_CHN, DBG_LVL_DEBUG,
			"MyAirOccupyPercentage exceeds and return 255!!\n");
		return 255;
	}
#ifndef EM_PLUS_SUPPORT
	res = (MyAirOccupyPercentage + ObssAirOccupyPercentage);
#else
	res = (MyAirOccupyPercentage + ObssAirOccupyPercentage + EDCCAOccupyPercentage);
#endif
...
INT wapp_send_wdev_info_report(PRTMP_ADAPTER pAd, struct wifi_dev *wdev)
{
	struct wapp_event event;
	wapp_dev_info *dev_info;
	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		if (pAd->wdev_list[i] != NULL) {
			...
			if (wdev->if_dev && (RtmpOsGetNetIfIndex(wdev->if_dev) == event.ifindex)) {
				...
				case 4:
					drv_vht_op.basic_mcs_set.mcs_ss4 = mcs_nss->max_vht_mcs;
					fallthrough;
				case 3:
					drv_vht_op.basic_mcs_set.mcs_ss3 = mcs_nss->max_vht_mcs;
					fallthrough;
				case 2:
					drv_vht_op.basic_mcs_set.mcs_ss2 = mcs_nss->max_vht_mcs;
					fallthrough;
				case 1:
					drv_vht_op.basic_mcs_set.mcs_ss1 = mcs_nss->max_vht_mcs;
					break;
				}
...
u16 get_nss_from_he_chwidth(MAC_TABLE_ENTRY *mac_entry, u8 bw)
{
	...
	case HE_BW_80: /*For Bw less than equal to 80*/
		if (mac_entry->cap.ch_bw.he_ch_width & (SUPP_40M_CW_IN_24G_BAND | SUPP_40M_80M_CW_IN_5G_6G_BAND))
	...
	case HE_BW_160: /*For Bw 160 support*/
		if (mac_entry->cap.ch_bw.he_ch_width & SUPP_160M_CW_IN_5G_6G_BAND)
			for (i = 0; i < DOT11AX_MAX_STREAM; i++) {
	...
	case HE_BW_8080: /*For Bw 8080 support*/
		if (mac_entry->cap.ch_bw.he_ch_width & SUPP_160M_8080M_CW_IN_5G_6G_BAND)
			for (i = 0; i < DOT11AX_MAX_STREAM; i++) {
		...
		break;
	default:
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
			"bw is inwalid!!\n");
		break;
	}
	return i;
...
void wapp_fill_client_mlo_info(PRTMP_ADAPTER pAd, wapp_client_info *cli_info, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pEntry && pEntry->mlo.mlo_en) {
		cli_info->mlo_join = pEntry->mlo_join;
		cli_info->mlo.mlo_en = pEntry->mlo.mlo_en;
		cli_info->mlo.is_setup_link_entry = pEntry->mlo.is_setup_link_entry;
		cli_info->mlo.link_info.link_id = pEntry->mlo.link_info.link_id;
		cli_info->mlo.link_info.tid_bitmap_dl = pEntry->mlo.link_info.tid_map_dl;
		cli_info->mlo.link_info.tid_bitmap_ul = pEntry->mlo.link_info.tid_map_ul;
		cli_info->EmlsrBitmap = pEntry->EmlsrBitmap;
#ifdef MAP_R6
		memcpy(cli_info->mlo.sta_mld_addr, pEntry->mlo.mld_addr, MAC_ADDR_LEN);
		if (pEntry->wdev) {
			if (pEntry->wdev->wdev_type == WDEV_TYPE_AP) {
				mbss = &pAd->ApCfg.MBSSID[pEntry->wdev->func_idx];
				index = mbss->mld_grp_idx;
			}
			if (BMGR_VALID_MLD_GRP_IDX(index))
				mld = GET_MLD_BY_GRP_IDX(index);
			if (BMGR_VALID_MLO_DEV(mld))
				memcpy(cli_info->mlo.ap_mld_addr, mld->mld_addr, MAC_ADDR_LEN);
		}
#endif
		mt_rcu_read_lock();
		...
		if (mld_entry) {
			memcpy(cli_info->mlo.mld_info.addr, mld_entry->addr, MAC_ADDR_LEN);
			cli_info->mlo.mld_info.mld_sta_idx = mld_entry->mld_sta_idx;
			cli_info->mlo.mld_info.nsep = mld_entry->nsep;
			cli_info->mlo.mld_info.emlmr = mld_entry->emlmr;
			cli_info->mlo.mld_info.emlsr = mld_entry->emlsr;
			cli_info->mlo.mld_info.eml_cap = mld_entry->eml_cap;
			cli_info->mlo.mld_info.setup_link_wcid = mld_entry->setup_link_wcid;
			cli_info->mlo.mld_info.setup_link_id = mld_entry->setup_link_id;
			cli_info->mlo.mld_info.link_num = mld_entry->link_num;
			cli_info->mlo.mld_info.valid = mld_entry->valid;
#ifdef MAP_R6
			for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++)
				memset(cli_info->mlo.affiliated_info[i].affiliated_ap_bssid, 0, MAC_ADDR_LEN);
			for (i = 0; i < BSS_MNGR_MAX_BAND_NUM; i++) {
				link_entry = (struct _MAC_TABLE_ENTRY *)mld_entry->link_entry[i];
				if (!link_entry)
					continue;

				if (!link_entry->mlo.is_setup_link_entry)
					j++;
				cli_info->mlo.affiliated_ap_num = j;
				if (link_entry->wdev) {
					if (!link_entry->mlo.is_setup_link_entry)
						COPY_MAC_ADDR(cli_info->mlo.affiliated_info[j-1].affiliated_ap_bssid, link_entry->wdev->bssid);
				}
			}
#endif
		} else
...
#ifdef MAP_R6
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"affiliated ap num sent from driver is %d\n", cli_info->mlo.affiliated_ap_num);
#endif
		mt_rcu_read_unlock();
	}
}
	...
	PSTA_ADMIN_CONFIG pApCliEntry)
{
	...
	pEntry = MacTableLookup(pAd, pApCliEntry->Bssid);
	if (pEntry == NULL)
		return 0;

	memcpy(cli_info->mac_addr, wdev->if_addr, MAC_ADDR_LEN);
	memcpy(cli_info->bssid, pEntry->Addr, MAC_ADDR_LEN);
	cli_info->sta_status = \
		(pEntry->wdev->PortSecured == WPA_802_1X_PORT_SECURED) ?
		WAPP_STA_CONNECTED : WAPP_STA_DISCONNECTED;
#ifdef CONFIG_MAP_SUPPORT
#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
	wapp_fill_client_mlo_info(pAd, cli_info, pEntry);
#endif
	...
	MAC_TABLE_ENTRY *mac_entry)
{
	...
	union _HETRANSMIT_SETTING HEPhyMode;

#ifdef CONFIG_MAP_SUPPORT
	union _EHTTRANSMIT_SETTING EHTPhyMode;
#endif
	PRTMP_ADAPTER pAd2;
#ifdef DOT11_HE_AX
	UINT8 he_dcm = 0, he_mcs = 0, he_nss = 0, he_sgi = 0, he_stbc = 0;
#endif
#ifdef CONFIG_MAP_SUPPORT
	UINT8 eht_dcm = 0, eht_mcs = 0, eht_nss = 0, eht_sgi = 0, eht_stbc = 0;
#endif

#ifdef EM_PLUS_SUPPORT
	EXT_EVENT_TX_STATISTIC_RESULT_T rTxStatResult_2;
#endif
	...
	memset(&HTPhyMode, 0, sizeof(union _HTTRANSMIT_SETTING));
	memset(&HEPhyMode, 0, sizeof(union _HETRANSMIT_SETTING));
#ifdef CONFIG_MAP_SUPPORT
	memset(&EHTPhyMode, 0, sizeof(union _EHTTRANSMIT_SETTING));
#endif

	/* before tx_rx_ctl move to phy_device , we map mac_entry to correct pAd*/
	if (mac_entry->pAd != pAd) {
		pAd2 = mac_entry->pAd;
	} else
		pAd2 = pAd;

	tr_entry = tr_entry_get(pAd2, mac_entry->wcid);
	...
	if (mac_entry->wdev) {
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"Port secure for tr:%u wdev:%u\n",
		tr_entry->PortSecured, mac_entry->wdev->PortSecured);
#endif
	} else
		NdisZeroMemory(cli_info->bssid, MAC_ADDR_LEN);

	if (mac_entry->wdev && mac_entry->wdev->wdev_type == WDEV_TYPE_AP) {
#if defined(EM_PLUS_SUPPORT)
		cli_info->sta_status = WAPP_STA_CONNECTED;
#else
	if (IS_MAP_API_ENABLE(pAd)) {
		cli_info->sta_status = (mac_entry->wdev->PortSecured == WPA_802_1X_PORT_SECURED) ?
			WAPP_STA_CONNECTED : WAPP_STA_DISCONNECTED;
	} else {
		cli_info->sta_status = WAPP_STA_CONNECTED;
	}
		...
		cli_info->sta_status = (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED) ?
			WAPP_STA_CONNECTED : WAPP_STA_DISCONNECTED;

#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
	if (mac_entry->wdev && mac_entry->wdev->PortSecured == WPA_802_1X_PORT_SECURED)
		wapp_fill_client_mlo_info(pAd, cli_info, mac_entry);
#endif

	cli_info->assoc_time = mac_entry->StaConnectTime;
	cli_info->assoc_req_len = mac_entry->assoc_req_len;
	HEPhyMode.word = (UINT32)mac_entry->map_LastTxRate;
#ifdef CONFIG_MAP_SUPPORT
	EHTPhyMode.word = (UINT32)mac_entry->map_LastTxRate;
#endif
#ifdef MAP_R2
...
#ifdef EM_PLUS_SUPPORT
	os_zero_mem(&rTxStatResult_2, sizeof(EXT_EVENT_TX_STATISTIC_RESULT_T));

	HTPhyMode.word = GetTxRatePerSta_stats_info(pAd, mac_entry->wcid, &rTxStatResult_2);

	HTPhyMode.field.MODE = rTxStatResult_2.rEntryTxRate.MODE;
	if (rTxStatResult_2.rEntryTxRate.MODE == MODE_VHT)
		HTPhyMode.field.MCS = (((rTxStatResult_2.rEntryTxRate.VhtNss - 1) & 0x3) << 4)
								+ rTxStatResult_2.rEntryTxRate.MCS;
	else
		HTPhyMode.field.MCS =  rTxStatResult_2.rEntryTxRate.MCS;
	HTPhyMode.field.BW = rTxStatResult_2.rEntryTxRate.BW;
	HTPhyMode.field.ShortGI = rTxStatResult_2.rEntryTxRate.ShortGI;
	HTPhyMode.field.STBC = rTxStatResult_2.rEntryTxRate.STBC;
	HTPhyMode.field.ldpc = rTxStatResult_2.rEntryTxRate.ldpc;
#endif
#ifdef CONFIG_MAP_SUPPORT
	if (EHTPhyMode.field.MODE == MODE_EHT_MU) {
#ifdef EM_PLUS_SUPPORT
		eht_mcs = rTxStatResult_2.rEntryTxRate.MCS & 0xf;
		eht_dcm = rTxStatResult_2.rEntryTxRate.MCS & 0x10 ? 1 : 0;
		eht_nss = rTxStatResult_2.rEntryTxRate.VhtNss;
		eht_sgi =	rTxStatResult_2.rEntryTxRate.ShortGI ? 1 : 0;
		eht_stbc = rTxStatResult_2.rEntryTxRate.STBC;
		get_rate_eht(eht_mcs, rTxStatResult_2.rEntryTxRate.BW, eht_nss, eht_dcm, &DataRate);
#else
		eht_mcs = EHTPhyMode.field.MCS & 0xf;
		eht_dcm = EHTPhyMode.field.MCS	& 0x10 ? 1 : 0;
		eht_nss = ((EHTPhyMode.field.MCS & (0x3 << 4)) >> 4) + 1;
		eht_sgi = EHTPhyMode.field.ShortGI;
		get_rate_eht(eht_mcs, EHTPhyMode.field.BW, eht_nss, eht_dcm, &DataRate);
#endif
		if (eht_sgi == 1)
...
#endif
#ifdef DOT11_HE_AX
	if (IS_HE_MODE(HEPhyMode.field.MODE)) {
#ifdef EM_PLUS_SUPPORT
		he_mcs = rTxStatResult_2.rEntryTxRate.MCS & 0xf;
		he_dcm = rTxStatResult_2.rEntryTxRate.MCS & 0x10 ? 1 : 0;
		he_nss = rTxStatResult_2.rEntryTxRate.VhtNss;
		he_sgi = rTxStatResult_2.rEntryTxRate.ShortGI ? 1 : 0;
		get_rate_he(he_mcs, rTxStatResult_2.rEntryTxRate.BW, he_nss, he_dcm, &DataRate);
#else
		he_mcs = HEPhyMode.field.MCS & 0xf;
		he_dcm = HEPhyMode.field.MCS  & 0x10 ? 1 : 0;
		he_nss = HEPhyMode.field.Nss;
		he_sgi = HEPhyMode.field.ShortGI;
	...
	{
#ifndef EM_PLUS_SUPPORT
		HTPhyMode.word = (UINT32)mac_entry->map_LastTxRate;
#endif
	...
	HEPhyMode.word = (UINT32) mac_entry->map_LastRxRate;
#ifdef CONFIG_MAP_SUPPORT
	EHTPhyMode.word = mac_entry->map_LastRxRate;
	if (IS_ENTRY_PEER_AP(mac_entry)) {
		...
		EHTPhyMode.word = GetTxRatePerSta_stats_info(pAd, mac_entry->wcid, &rTxStatResult);
		eht_mcs = rTxStatResult.rEntryTxRate.MCS & 0xf;
		eht_dcm = rTxStatResult.rEntryTxRate.MCS & 0x10 ? 1 : 0;
		eht_nss = rTxStatResult.rEntryTxRate.VhtNss - 1;
		eht_sgi =   rTxStatResult.rEntryTxRate.ShortGI ? 1 : 0;
		eht_stbc = rTxStatResult.rEntryTxRate.STBC;
		eht_nss = (eht_nss + 1)/(eht_stbc + 1);
		if (rTxStatResult.rEntryTxRate.MODE == MODE_EHT_MU)
			get_rate_eht(eht_mcs, rTxStatResult.rEntryTxRate.BW, eht_nss, eht_dcm, &DataRate_r);
		else if (IS_HE_MODE(rTxStatResult.rEntryTxRate.MODE))
			get_rate_he(eht_mcs, rTxStatResult.rEntryTxRate.BW, eht_nss, eht_dcm, &DataRate_r);
		else {
			no_apcli_mode = 1;
			HTPhyMode.word = (UINT32) EHTPhyMode.word;
			HTPhyMode.field.MODE = rTxStatResult.rEntryTxRate.MODE;
			HTPhyMode.field.MCS =  rTxStatResult.rEntryTxRate.MCS;
			HTPhyMode.field.BW = rTxStatResult.rEntryTxRate.BW;
			HTPhyMode.field.ShortGI = rTxStatResult.rEntryTxRate.ShortGI;
			HTPhyMode.field.STBC = rTxStatResult.rEntryTxRate.STBC;
			HTPhyMode.field.ldpc = rTxStatResult.rEntryTxRate.ldpc;
	...
	} else if (EHTPhyMode.field.MODE == MODE_EHT_MU) {
		eht_mcs = EHTPhyMode.field.MCS & 0xf;
		eht_dcm = EHTPhyMode.field.MCS	& 0x10 ? 1 : 0;
#ifndef EM_PLUS_SUPPORT
		eht_nss = ((EHTPhyMode.field.MCS & (0x3 << 4)) >> 4);
#else
		eht_nss = EHTPhyMode.field.Nss;
#endif
		eht_sgi =  EHTPhyMode.field.ShortGI;
		eht_stbc = EHTPhyMode.field.STBC;
		eht_nss = (eht_nss + 1)/(eht_stbc + 1);
		get_rate_eht(eht_mcs, EHTPhyMode.field.BW, eht_nss, eht_dcm, &DataRate_r);
	...
	if (IS_HE_MODE(HEPhyMode.field.MODE)) {
		...
		he_nss = HEPhyMode.field.Nss;
		he_sgi = HEPhyMode.field.ShortGI;
		he_stbc = HEPhyMode.field.STBC;
		he_nss = (he_nss + 1)/(he_stbc + 1);
...
#endif
	{
		HTPhyMode.word = (UINT32) mac_entry->map_LastRxRate;
		getRate(HTPhyMode, &DataRate_r);
		cli_info->uplink = (u16) DataRate_r;
	...
	cli_info->uplink_rssi = RTMPAvgRssi(pAd2, &mac_entry->RssiSample);
	...
	/*11 AX Support*/
#ifdef CONFIG_MAP_SUPPORT
	if (cli_info->cli_caps.phy_mode == MODE_EHT) {
		cli_info->cli_caps.bw = EHTPhyMode.field.BW;
		cli_info->cli_caps.nss = eht_nss;
	} else
#endif
#ifdef DOT11_HE_AX
...
#ifdef EM_PLUS_SUPPORT
	if (((IS_ENTRY_CLIENT(mac_entry) || IS_ENTRY_PEER_AP(mac_entry) || IS_ENTRY_REPEATER(mac_entry))
					&& (mac_entry->Sst == SST_ASSOC)) || IS_ENTRY_WDS(mac_entry)) {
		Get_Pkt_Cnt_Per_Sta(pAd, mac_entry->wcid, &mac_entry->txMsduRetryCnt, &mac_entry->txMsduDropCnt);
	}
#endif
	/*traffic stats*/
...
#ifdef EM_PLUS_SUPPORT
	cli_info->tx_packets_errors = mac_entry->txMsduDropCnt + mac_entry->TxFailCount;
#else
	cli_info->tx_packets_errors = mac_entry->TxFailCount; /* to do */
#endif
	cli_info->rx_packets_errors = 0; /* not support */
#ifdef EM_PLUS_SUPPORT
	cli_info->retransmission_count = mac_entry->TxRetriedPktCount;
#else
	cli_info->retransmission_count = mac_entry->txMsduRetryCnt;
#endif
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"WF6:Driver:Setting TID value for %02x:%02x:%02x:%02x:%02x:%02x\n",
		PRINT_MAC(cli_info->mac_addr));
	if (ppdu) {
		for (i = 0; i < MAX_TID; i++) {
			cli_info->status_tlv[i].tid = i;
			if (WMODE_CAP_AX(PhyMode))
				cli_info->status_tlv[i].tid_q_size = (u16)(BA_WIN_SZ_256);
			else
				cli_info->status_tlv[i].tid_q_size = (u16)(BA_WIN_SZ_64);
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
				"WF6:Driver:tid_cnt %x, tid %x, tid_q_size %x\n",
	...
	MAC_TABLE_ENTRY *mac_entry)
{
	...
	memset(&HTPhyMode, 0, sizeof(union _HTTRANSMIT_SETTING));
	memset(&LastTxRate, 0, sizeof(union _HTTRANSMIT_SETTING));
	...
	tr_entry = tr_entry_get(pAd, mac_entry->wcid);
	...
	for (i = 0; i < MAX_TID; i++) {
		cli_info->status_tlv[i].tid = i;
		if (mac_entry->wdev && WMODE_CAP_AX(mac_entry->wdev->PhyMode))
			cli_info->status_tlv[i].tid_q_size = (u16)(BA_WIN_SZ_256);
		else
			cli_info->status_tlv[i].tid_q_size = (u16)(BA_WIN_SZ_64);
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
	...
	struct wapp_req *req)
{
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (IS_ENTRY_CLIENT(mac_entry)) {/* report all entry no matter which wdev it is belonged */
			wdev = mac_entry->wdev;
			if (wdev->if_dev == NULL) {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
					"!!!! wdev->ifdev = NULL\n");
				continue;
			}
#ifdef CONFIG_MAP_SUPPORT
	...
	PSTA_ADMIN_CONFIG pApCliEntry)
{
	...
	if (pApCliEntry) {
		...
		if (wdev->if_dev) {
			event.event_id = WAPP_AP_JOIN_EVENT;
			event.ifindex = RtmpOsGetNetIfIndex(wdev->if_dev);
			cli_info = &event.data.cli_info;
			wapp_fill_ap_info(pAd, wdev, cli_info, pApCliEntry);
			wapp_send_wapp_qry_rsp(pAd, &event);
		}
	}
...
INT wapp_send_cli_all_leave_event(
	PRTMP_ADAPTER pAd,
	UINT32 ifindex,
	UCHAR *mac_addr,
	MAC_TABLE_ENTRY *mac_entry)
{
	...
	event.event_id = WAPP_CLI_ALL_LEAVE_EVENT;
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
		"in driver WAPP_CLI_ALL_LEAVE_EVENT reason %d\n", mac_entry->DisconnectReason);
	event.ifindex = ifindex;
	cli_info = &event.data.cli_info;
	cli_info->is_APCLI = 0;
	COPY_MAC_ADDR(cli_info->mac_addr, mac_addr);
#if (defined(MAP_R6) && defined(DOT11_EHT_BE))
	if (mac_entry->mlo.mlo_en) {
		cli_info->mlo.mlo_en = 1;
		COPY_MAC_ADDR(cli_info->mlo.sta_mld_addr, mac_entry->mlo.mld_addr);
		if (mac_entry->wdev)
			COPY_MAC_ADDR(cli_info->mlo.ap_mld_addr, mac_entry->wdev->bssid);
	}
#endif
	...
	struct wapp_req *req)
{
	...
	for (i = 0; i < MAX_MULTI_STA; i++) {
		...
		if (!apcli_entry || !wdev) {
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
				"entry or wdev null continue\n");
			continue;
		}

#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
		pEntry = MacTableLookup(pAd, apcli_entry->Bssid);

		if (pEntry == NULL) {
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR, "pEntry NULL\n");
			continue;
		}
#endif
		if (((apcli_entry->ApcliInfStat.Valid == TRUE)
			...
			mac_entry = entry_get(pAd, apcli_entry->MacTabWCID);
			if (IS_ENTRY_PEER_AP(mac_entry)) {
				wapp_fill_client_info_new(pAd, cli_info, mac_entry);
				wapp_send_wapp_qry_rsp(pAd, &event);
				break;
...
VOID RTMPIoctlGetCCEResults(
	...
	for (i = 0; i < ScanTab->BssNr; i++) {
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_DEBUG,
			"(DPP) ABOVE CCE res_channl[%d] = %d, cce_flag:%d, cce_channel:%d\n",
			i, result.cce_ch[i], bss->cce_vendor_ie_found, bss->Channel);
		...
		if (bss->cce_vendor_ie_found == TRUE) {
...
#ifdef MAP_R3_6E_SUPPORT
					|| (band_6g && (j < MAX_RNR_CHANNEL) && (result.rnr_6e_ch[j] == bss->Channel))
#endif
				) {
					dup = 1;
					break;
			...
			if (!band_6g)
				...
				if (k < MAX_RNR_CHANNEL) {
					result.rnr_6e_ch[k] = bss->Channel;
					k = k + 1;
					result.rnr_6e_num = k;
				}

				if (index < MAX_RNR_CHANNEL)
					MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_DEBUG,
						"(DPP) PSC SCAN channel%d,\n", result.rnr_6e_ch[index]);
			}
#endif
#ifdef MAP_R3_6E_SUPPORT
			if (bss->rnr_info.channel != 0 && (!band_6g)) {
				...
				dup = 0;
				for (j = 0; (j < k) && (j < MAX_RNR_CHANNEL); j++) {
			...
			if (!band_6g) {
				MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_DEBUG,
					"(DPP) bss->rnr_info.channel%d,\n", bss->rnr_info.channel);
#endif
				result.num++;
#ifdef MAP_R3_6E_SUPPORT
			}
#endif
			if (index < MAX_CCE_CHANNEL - 1)
				index++;
		}
	}

	for (i = 0; (i < result.num) && (i < MAX_CCE_CHANNEL); i++)
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"result.num = %d, channl[%d] = %d\n", result.num, i, result.cce_ch[i]);
...
#endif
	for (i = 0; (i < pAd->ChannelListNum) && (i < (sizeof(chn_list->ch_list)/sizeof(struct chnList))) && (i < MAX_NUM_OF_CHANNELS);
		i++) {
		...
		if (pAd->ChannelList[i].DfsReq) {
			...
			if ((pAd->CommonCfg.RDDurRegion == CE) &&
				DfsCacRestrictBand(pAd, pDfsParam->band_bw, pAd->ChannelList[i].Channel, 0))
				...
				BOOLEAN tx_error, UINT16 seq_no)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"sending tx status to wapp status=%d\n", tx_error);
	event.ifindex = RtmpOsGetNetIfIndex(wdev->if_dev);
					...
					const char *frm, UINT16 frm_len, BOOL is_gas, UINT32 frm_count)
{
	...
	if (buf == NULL) {
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"source mac address for dpp frame :%02x:%02x:%02x:%02x:%02x:%02x\n",
			PRINT_MAC(req_data->src));
...
void wext_send_dpp_frame_rx_event(IN PRTMP_ADAPTER pAdapter, struct wifi_dev *wdev, UINT32 frm_count)
{
	struct wapp_event *event;
	UINT16 buflen = 0;
	char *buf = NULL;
	...
	os_alloc_mem(NULL, (UCHAR **)&buf, buflen);
	if (buf == NULL) {
		MTWF_DBG(pAdapter, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
				"Fail to allocate memory!\n");
	} else {
		NdisZeroMemory(buf, buflen);
		event = (struct wapp_event *)buf;
		...
		MTWF_DBG(pAdapter, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"sending event to wapp with frame id %d\n", event->data.wapp_dpp_frame_id_no);
							  ...
							  const char *frm, UINT16 frm_len, BOOL is_gas)
{
	...
	wext_send_dpp_frame_rx_event(pAd, wdev, pAd->dpp_rx_frm_counter);
				...
				MAC_TABLE_ENTRY *pEntry)
{
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		...
		if (pEntry->mlo.mlo_en)
			NdisCopyMemory(req_data->src, pEntry->mlo.mld_addr, MAC_ADDR_LEN);
		else
#endif
		...
		if (pEntry->func_tb_idx >= MAX_MULTI_STA) {
			MTWF_DBG(pAd, DBG_CAT_CLIENT, CATAP_WAPP, DBG_LVL_ERROR,
						"invalid idx %u\n", pEntry->func_tb_idx);
			return;
		}
		...
		NdisCopyMemory(req_data->ssid,
			pAd->StaCfg[pEntry->func_tb_idx].Ssid, (MAX_LEN_OF_SSID+1));
	}
				...
				PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_DEBUG,
	...
	struct wapp_req *req)
{
	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		if (pAd->wdev_list[i] != NULL) {
			wdev = pAd->wdev_list[i];
			if (wdev->if_dev && (RtmpOsGetNetIfIndex(wdev->if_dev) == event.ifindex)) {
				if (i >= MAX_MBSSID_NUM(pAd) || wdev->wdev_type != WDEV_TYPE_AP) {
					MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
						"wdev %d is not an AP\n", i);
					break;
				}
...
#ifdef MAP_R6
// saurabh : tx rx values need to be checked
				ap_metric->mlo_ext_ap_metric.mlo_pkts_tx = (mbss->ucPktsTx + mbss->bcPktsTx + mbss->mcPktsTx);
				ap_metric->mlo_ext_ap_metric.mlo_pkts_rx = (mbss->ucPktsRx + mbss->bcPktsRx + mbss->mcPktsRx);
				ap_metric->mlo_ext_ap_metric.mlo_tx_error_count = mbss->TxErrorCount;

				ap_metric->mlo_ext_ap_metric.mlo_bc_rx =
					mbss->mlo_bcBytesRx == 0?0:((mbss->mlo_bcBytesRx/1024) == 0?1:mbss->mlo_bcBytesRx/1024);
				ap_metric->mlo_ext_ap_metric.mlo_bc_tx =
					mbss->mlo_bcBytesTx == 0?0:((mbss->mlo_bcBytesTx/1024) == 0?1:mbss->mlo_bcBytesTx/1024);
				ap_metric->mlo_ext_ap_metric.mlo_mc_rx =
					mbss->mlo_mcBytesRx == 0?0:((mbss->mlo_mcBytesRx/1024) == 0?1:mbss->mlo_mcBytesRx/1024);
				ap_metric->mlo_ext_ap_metric.mlo_mc_tx =
					mbss->mlo_mcBytesTx == 0?0:((mbss->mlo_mcBytesTx/1024) == 0?1:mbss->mlo_mcBytesTx/1024);
				ap_metric->mlo_ext_ap_metric.mlo_uc_rx =
					mbss->mlo_ucBytesRx == 0?0:((mbss->mlo_ucBytesRx/1024) == 0?1:mbss->mlo_ucBytesRx/1024);
				ap_metric->mlo_ext_ap_metric.mlo_uc_tx =
					mbss->mlo_ucBytesTx == 0?0:((mbss->mlo_ucBytesTx/1024) == 0?1:mbss->mlo_ucBytesTx/1024);
#endif
#ifdef MAP_R2
...
#ifdef MAP_R2
static UINT8 wapp_rssi_to_rcpi(CHAR rssi)
{
	if (!rssi)
	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		if (pAd->wdev_list[i] != NULL) {
			...
			if (wdev->if_dev && (RtmpOsGetNetIfIndex(wdev->if_dev) == event.ifindex)) {
				...
				radio_metric->cu_noise = wapp_rssi_to_rcpi(pAd->noise_floor);
				radio_metric->cu_tx = (Get_My_Tx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
				radio_metric->cu_rx = (Get_My_Rx_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
				radio_metric->cu_other = (Get_OBSS_AirTime_MAP(pAd, band_idx)*255)/ONE_SEC_2_US;
	...
	IN  PWSC_CREDENTIAL pCredential)
{
	...
	if (wdev == NULL) {
		MTWF_DBG(ad, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
		"Error :: Wapp_Send_wps_config --> wdev NULL");
		return 0;
	}
	if (wdev->if_dev) {
		...
		if (pCredential->SSID.SsidLength <= ARRAY_SIZE(event.data.wps_conf_info.SSID))
			NdisMoveMemory(event.data.wps_conf_info.SSID, pCredential->SSID.Ssid, pCredential->SSID.SsidLength);
		else
			MTWF_DBG(ad, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
				"Not have enough buffer size for event.data.wps_conf_info.SSID!\n");
		...
		MTWF_DBG(ad, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"SSID = %s, psk = %s, len = %d",
			event.data.wps_conf_info.SSID, event.data.wps_conf_info.Key,
			event.data.wps_conf_info.KeyLength);
		MTWF_DBG(ad, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
			"Auth = %d, EncyType = %d, Bss_role = %d",
	...
	UINT8 ControlChannel)
{
	...
	if (wdev->if_dev) {
		...
		event.data.eht_ch_change = wdev->eht_ch_change;
		wdev->eht_ch_change = 0;
		wapp_send_wapp_qry_rsp(pAd, &event);
	}
	return 0;
}
	...
	PSTA_ADMIN_CONFIG pApCliEntry)
{
	...
	if (pApCliEntry->wdev.if_dev && (!pApCliEntry->wdev.bh_ready_sent
	    || apcli_assoc_state == WAPP_APCLI_DISASSOCIATED)) {
		...
		if (pAd->wdev_list[i] != NULL) {
			...
			if (wdev->if_dev && (RtmpOsGetNetIfIndex(wdev->if_dev) == req->data.ifindex)) {
				...
				if (wdev->func_idx < 0 || wdev->func_idx >= MAX_BEACON_NUM) {
					MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR, "%s: failed\n", __func__);
					return 0;
				}
...
INT wapp_send_cosr_sta_rssi_query_rsp(
	PRTMP_ADAPTER pAd,
	struct wapp_req *req)
{
	struct wapp_event event;
	...
	event.event_id = WAPP_COSR_STA_RSSI_RSP;
	event.ifindex = req->data.ifindex;
	cli_info = &event.data.cli_info;
	...
	struct wapp_req *req)
{
	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		if (pAd->wdev_list[i] != NULL) {
			...
			if (wdev->if_dev && (RtmpOsGetNetIfIndex(wdev->if_dev) == req->data.ifindex)) {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
					"ifindex = %u Band:%u\n", req->data.ifindex, req->data.band_index);
...
/* get SR Support Mode Rep  */
INT wapp_send_sr_support_mode_query_rsp(
	PRTMP_ADAPTER pAd, UINT32 value)
{
#ifdef CFG_SUPPORT_FALCON_SR
	struct wapp_event event;

	event.event_id = WAPP_SR_SUPPORT_MODE_RSP;
	event.ifindex = pAd->CommonCfg.SRmode_req_ifindex;
	event.data.sr_support_mode = value;

	MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_SR, DBG_LVL_ERROR,
		"[%s] support mode = %d, req_ifindex = %d\n",
		__func__, event.data.sr_support_mode, event.ifindex);
...
INT wapp_handle_sr_support_mode_query_req(
	PRTMP_ADAPTER pAd,
	struct wapp_req *req)
{
#ifdef CFG_SUPPORT_FALCON_SR
	...
	UINT_8 isSet = 0;
	UINT_8 u1CmdSubId = UNI_CMD_SR_CFG_SR_SUPPORT_MODE;
	UINT_32 u4Value = 0;
	...
	pAd->CommonCfg.SRmode_req_ifindex = req->data.ifindex;
	MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_SR, DBG_LVL_ERROR,
		"[%s] req ifindex = %d\n", __func__, pAd->CommonCfg.SRmode_req_ifindex);
...
#else
	MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_SR, DBG_LVL_ERROR,
		"not support ShowSrCfgSrSupportMode cmd\n");
	return NDIS_STATUS_FAILURE;
#endif
#else
	return 0;
...
INT wapp_send_lte_safe_chn_event(
	PRTMP_ADAPTER pAd, UINT64 *safe_chn_bitmask)
{
	struct wapp_event event;

	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"safe_chn_bitmask:%llu-%llu-%llu\n", safe_chn_bitmask[0],
	...
	PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UINT8 u1UlStatus)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"uplink traffic event, ifindex:%u status:%u\n", event.ifindex, u1UlStatus);
	...
	IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev, IN PUINT8 prSrgBitmap)
{
	...
	event.ifindex = RtmpOsGetNetIfIndex(wdev->if_dev);
	event.data.mesh_sr_info.bss_color = bss_color->color;
	NdisCopyMemory(&(event.data.mesh_sr_info.bm_info), prSrgBitmap,
		sizeof(struct wapp_srg_bitmap));

	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"ifindex:%u bss color %d Color:[63_32][%u]-[31_0][%u] PBssid::[63_32][%u]-[31_0][%u]\n",
		event.ifindex, event.data.mesh_sr_info.bss_color,
	...
	PRTMP_ADAPTER pAd, struct wifi_dev *wdev, BOOLEAN IsStaAllHe)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
		"STA Mode Rpt event, ifindex:%u IsStaAllHe:%u\n", event.ifindex, IsStaAllHe);
...
#ifdef MTK_MLO_MAP_SUPPORT
#ifdef MAP_R6
#define IWEVCUSTOM_PAYLOD_MAX_LEN_MAP 415
#else
#define IWEVCUSTOM_PAYLOD_MAX_LEN_MAP 400
#endif
#endif

#ifdef MTK_MLO_MAP_SUPPORT
	custom_event_length = IWEVCUSTOM_PAYLOD_MAX_LEN_MAP;
#else
	custom_event_length = IWEVCUSTOM_PAYLOD_MAX_LEN;
#endif

	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_DEBUG, "RTMPIoctlGetSiteSurvey - enter\n");
	/*
	...
	TotalLen = (sizeof(CHAR) * 5) + ((sizeof(struct scan_bss_info)) * max_bss) + (sizeof(UINT32) * 2);
	...
	for (i = bss_start_idx; i < ScanTab->BssNr && count < max_bss; i++) {
		...
		if (bss->Channel == 0) {
			i++;
			MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
					"break from here bss Channel is 0, i %d", i);
			break;
		}
		...
		ht_cap->ht_40 = bss->AddHtInfo.AddHtInfo.RecomWidth;/*current operating bw*/
		...
		/* ch width can be 1 for 80, 160, 80/80 */
		if (bss->vht_op_ie.vht_op_info.ch_width == 1) {
		...
		if (he_ch_width == HE_BW_160) {
			...
			he_cap->he_160 = 0;
		he_op_bw = (bss->he6g_opinfo.ctrl & HE_6G_OP_CONTROL_CH_WIDTH_MASK);
		if (he_op_bw == 1)
			ht_cap->ht_40 = 1;
		else if (he_op_bw == 2)
			vht_cap->vht_160 = 1;
...
#endif
#ifdef CONFIG_MAP_SUPPORT
		pBss->eht_cap.eht_ch_width = bss->eht_cap.eht_ch_width;
		pBss->eht_cap.ccfs0 = bss->eht_cap.ccfs0;
		pBss->eht_cap.ccfs1 = bss->eht_cap.ccfs1;
		if (PD_CEHCK_MLO_V1_DISABLE(pAdapter->physical_dev)
		    || PD_CEHCK_MLO_V1_1_ENABLE(pAdapter->physical_dev)) {
			pBss->eht_cap.rx_stream = bss->eht_cap.rx_stream;
			pBss->eht_cap.tx_stream = bss->eht_cap.tx_stream;
			/*rx bit 0-3,tx bit 4-7*/
			pBss->eht_cap.rx_stream = pBss->eht_cap.rx_stream & 0x0f;
			pBss->eht_cap.tx_stream = (pBss->eht_cap.tx_stream & 0xf0) >> 4;
		} else {
			if (pBss->eht_cap.eht_ch_width == EHT_BW_20)
				txrx_stream = bss->eht_support_mcs_nss.eht_txrx_mcs_nss_20_only.max_tx_rx_mcs0_7_nss;
			else
				txrx_stream = bss->eht_support_mcs_nss.eht_txrx_mcs_nss[0].max_tx_rx_mcs0_9_nss;
			/*rx bit 0-3,tx bit 4-7*/
			pBss->eht_cap.rx_stream = txrx_stream & 0x0f;
			pBss->eht_cap.tx_stream = (txrx_stream & 0xf0) >> 4;
		}

#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
			pBss->ml_capable = bss->ml_capable;
		if (pBss->ml_capable == TRUE) {
			os_zero_mem(&pBss->ml_info, sizeof(struct multi_link_bss_info));

			NdisCopyMemory(&pBss->ml_info.mld_addr, &bss->ml_info.mld_addr, MAC_ADDR_LEN);
			pBss->ml_info.link_num = bss->ml_info.link_num;
			for (j = 0 ; j <  BSS_MNGR_MAX_BAND_NUM ; j++) {
				os_zero_mem(&pBss->ml_info.sta_profiles[j], sizeof(struct sta_profile_info));
				pBss->ml_info.sta_profiles[j].channel = bss->ml_info.sta_profiles[j].channel;

				if (wdev) {
					if (WMODE_CAP_6G(wdev->PhyMode)) {
						pBss->ml_info.sta_profiles[0].rfband = 0;
						pBss->ml_info.sta_profiles[1].rfband = 1;
						pBss->ml_info.sta_profiles[2].rfband = 255;
					} else if (WMODE_CAP_5G(wdev->PhyMode)) {
						pBss->ml_info.sta_profiles[0].rfband = 0;
						pBss->ml_info.sta_profiles[1].rfband = 255;
						pBss->ml_info.sta_profiles[2].rfband = 2;
					} else if (WMODE_CAP_2G(wdev->PhyMode)) {
						pBss->ml_info.sta_profiles[0].rfband = 255;
						pBss->ml_info.sta_profiles[1].rfband = 1;
						pBss->ml_info.sta_profiles[2].rfband = 2;
					}
				}
			//if HT exists
			pBss->ml_info.sta_profiles[j].ht_cap.sgi_20 = bss->ml_info.sta_profiles[j].HtCapability.HtCapInfo.ShortGIfor20;
			pBss->ml_info.sta_profiles[j].ht_cap.sgi_40 = bss->ml_info.sta_profiles[j].HtCapability.HtCapInfo.ShortGIfor40;
			pBss->ml_info.sta_profiles[j].ht_cap.ht_40 = bss->ml_info.sta_profiles[j].HtCapability.HtCapInfo.ChannelWidth;

			//if VHT exists
			NdisMoveMemory(&pBss->ml_info.sta_profiles[j].vht_cap.sup_tx_mcs,
						&bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.tx_mcs_map,
						sizeof(vht_cap->sup_tx_mcs));
			NdisMoveMemory(&pBss->ml_info.sta_profiles[j].vht_cap.sup_rx_mcs,
						&bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.rx_mcs_map,
						sizeof(vht_cap->sup_rx_mcs));

			if (bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.rx_stream = 4;
			else if (bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.rx_stream = 3;
			else if (bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.rx_stream = 2;
			else
				pBss->ml_info.sta_profiles[j].vht_cap.rx_stream = 1;

			if (bss->ml_info.sta_profiles[j].vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.tx_stream = 4;
			else if (bss->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.tx_stream = 3;
			else if (bss->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA)
				pBss->ml_info.sta_profiles[j].vht_cap.tx_stream = 2;
			else
				pBss->ml_info.sta_profiles[j].vht_cap.tx_stream = 1;

			pBss->ml_info.sta_profiles[j].vht_cap.sgi_80 = bss->ml_info.sta_profiles[j].vht_cap_ie.vht_cap.sgi_80M;
			pBss->ml_info.sta_profiles[j].vht_cap.sgi_160 = bss->ml_info.sta_profiles[j].vht_cap_ie.vht_cap.sgi_160M;
			pBss->ml_info.sta_profiles[j].vht_cap.vht_160 = bss->ml_info.sta_profiles[j].vht_op_ie.vht_op_info.ch_width;
			pBss->ml_info.sta_profiles[j].vht_cap.su_bf = bss->ml_info.sta_profiles[j].vht_cap_ie.vht_cap.bfee_cap_su;
			pBss->ml_info.sta_profiles[j].vht_cap.mu_bf = bss->ml_info.sta_profiles[j].vht_cap_ie.vht_cap.bfee_cap_mu;
#ifdef DOT11_HE_AX
		pBss->ml_info.sta_profiles[j].he_ch_width = peer_max_bw_cap(GET_DOT11AX_CH_WIDTH(bss->ml_info.sta_profiles[j].he_caps.phy_cap.phy_capinfo_1));
		mcsptr_sta = pBss->ml_info.sta_profiles[j].he_cap.he_mcs;
		NdisCopyMemory(mcsptr_sta, &bss->ml_info.sta_profiles[j].he_caps.txrx_mcs_nss, sizeof(struct he_txrx_mcs_nss));
		if (pBss->ml_info.sta_profiles[j].he_ch_width == HE_BW_160) {
			pBss->ml_info.sta_profiles[j].he_cap.he_160 = 1;
			mcsptr_sta += sizeof(struct he_txrx_mcs_nss);
			NdisCopyMemory(mcsptr_sta, &bss->ml_info.sta_profiles[j].he_mcs_nss_160, sizeof(struct he_txrx_mcs_nss));
		} else
			pBss->ml_info.sta_profiles[j].he_cap.he_160 = 0;
#endif
				//if EHT exists
#ifdef CONFIG_MAP_SUPPORT
			pBss->ml_info.sta_profiles[j].eht_cap.eht_ch_width = bss->ml_info.sta_profiles[j].eht_cap.eht_ch_width;
			pBss->ml_info.sta_profiles[j].eht_cap.ccfs0 = bss->ml_info.sta_profiles[j].eht_cap.ccfs0;
			pBss->ml_info.sta_profiles[j].eht_cap.ccfs1 = bss->ml_info.sta_profiles[j].eht_cap.ccfs1;

			txrx_stream = bss->ml_info.sta_profiles[j].eht_support_mcs_nss.eht_txrx_mcs_nss[0].max_tx_rx_mcs0_9_nss;
			pBss->ml_info.sta_profiles[j].eht_cap.rx_stream = txrx_stream & 0x0f;
			pBss->ml_info.sta_profiles[j].eht_cap.tx_stream = (txrx_stream & 0xf0) >> 4;

			if (wdev) {
				if (WMODE_CAP_6G(wdev->PhyMode)) {
					pBss->ml_info.sta_profiles[0].eht_cap.eht_ch_width = pBss->ml_info.sta_profiles[0].ht_cap.ht_40;
					if (pBss->ml_info.sta_profiles[1].vht_cap.vht_160 == 0)
						pBss->ml_info.sta_profiles[1].eht_cap.eht_ch_width = 2;
					else
						pBss->ml_info.sta_profiles[1].eht_cap.eht_ch_width = 3;
				} else if (WMODE_CAP_5G(wdev->PhyMode)) {
					pBss->ml_info.sta_profiles[0].eht_cap.eht_ch_width = pBss->ml_info.sta_profiles[0].ht_cap.ht_40;
				} else  if (WMODE_CAP_2G(wdev->PhyMode)) {
					if (pBss->ml_info.sta_profiles[1].vht_cap.vht_160 == 0)
	...
	IN	PRTMP_ADAPTER	pAdapter, struct wapp_req *req)
{
	...
	for (count = 0; count < pkt_count && count < 256; count++) {
		/* TODO status */
	...
	struct wireless_dev *nl_wdev)
{
	...
	case WAPP_STA_RSSI_QUERY_REQ:
		...
		break;
	case WAPP_COSR_STA_RSSI_QUERY_REQ:
		wapp_send_cosr_sta_rssi_query_rsp(pAd, req);
		break;
...
#ifdef RT_CFG80211_SUPPORT
			struct net_device *pNetDev = nl_wdev->netdev;
#else
			POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
#endif
...
#ifdef RT_CFG80211_SUPPORT
			if (nl_wdev->iftype == NL80211_IFTYPE_AP)
				ifIndex = CFG80211_FindMbssApIdxByNetDevice(pAd, pNetDev);
#ifdef APCLI_CFG80211_SUPPORT
			else if (nl_wdev->iftype == NL80211_IFTYPE_STATION)
				ifIndex = CFG80211_FindStaIdxByNetDevice(pAd, pNetDev);
#endif
			else {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
					"failed - [ERROR]No iftype matched.\n");
				return -EINVAL;
			}
...
#ifdef APCLI_SUPPORT
#ifdef RT_CFG80211_SUPPORT
			if (nl_wdev->iftype == NL80211_IFTYPE_STATION)
#else
	...
	case WAPP_SR_SUPPORT_MODE_QUERY_REQ:
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_SR, DBG_LVL_ERROR,
			"[SUPPORT_MODE_QUERY_REQ] wapp req id=%d\n", req->req_id);
		wapp_handle_sr_support_mode_query_req(pAd, req);
		break;

	default:
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO, "invalid wapp req id=%d\n", req->req_id);
		break;
	}
	return 0;
}
...
#endif /* CONFIG_HOTSPOT_R2 */
	default:
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR, "Unknown Parameter:%d\n", Param);
		break;
	}
#ifdef CONFIG_HOTSPOT_R2
	...
	IN UCHAR apidx)
{
...
#endif /* CONFIG_HOTSPOT_R2 */
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO, "Set Interworking IE\n");
		break;
	...
	case IE_ROAMING_CONSORTIUM:
		...
		pGasCtrl->RoamingConsortiumIELen = IELen;
		OS_SEM_UNLOCK(&pGasCtrl->IeLock);
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO,
	...
	case IE_QOS_MAP_SET: {
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_WARN,
			"=========================================== Set Qos MAP Set IE\n");
		break;
	}
#endif /* CONFIG_HOTSPOT_R2 */
	...
	case IE_QOS_R1_MAP_SET:
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_WARN,
	...
	default:
	    os_free_mem(tmp_buf_ptr);
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR, "Unknown IE(EID = %d)\n", EID);
		break;
	}
	/* Update Beacon Template */
	if (pAd->ApCfg.MBSSID[apidx].wdev.bAllowBeaconing) {
		UpdateBeaconHandler(pAd, &pAd->ApCfg.MBSSID[apidx].wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
	}
	...
	UCHAR *pFrame)
{
	...
	if (IS_MAP_ENABLE(pAd) && wdev->DPPCfg.cce_ie_len) {
		ULONG DPPIeTmpLen = 0;

		MakeOutgoingFrame(pFrame + FrameLen, &DPPIeTmpLen,
	...
	IN UCHAR ApIdx)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO, "Set MTK IE\n");
	os_zero_mem(wdev->MAPCfg.vendor_ie_buf, VENDOR_SPECIFIC_LEN);
	wdev->MAPCfg.vendor_ie_len = IELen;
	if (wdev->MAPCfg.vendor_ie_len <= ARRAY_SIZE(wdev->MAPCfg.vendor_ie_buf))
		NdisMoveMemory(wdev->MAPCfg.vendor_ie_buf, IE, wdev->MAPCfg.vendor_ie_len);
	else
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
				 "Not have enough buffer size for wdev->MAPCfg.vendor_ie_buf!\n");
	if (wdev->bAllowBeaconing)
		UpdateBeaconHandler(pAd, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
#endif
}
	...
	IN UCHAR apidx)
{
	...
	case OUI_P2P:
		...
		OS_SEM_UNLOCK(&pHSCtrl->IeLock);
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO, "Set P2P IE\n");
		break;
	...
	case OUI_HS2_INDICATION:
		...
		OS_SEM_UNLOCK(&pHSCtrl->IeLock);
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_INFO, "Set HS2.0 Indication IE\n");
		break;
#endif

	default:
		os_free_mem(tmp_buf_ptr);
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR, "Unknown OUIType = %d\n", OUIType);
		break;
	}
	...
	IN UCHAR ApIdx)
{
	...
	case IE_QOS_R1_MAP_SET:
#endif
		set_wapp_cmm_ie(pAd, EID, IE, IELen, ApIdx);
		break;
	...
	case IE_VENDOR_SPECIFIC:
		...
		if (NdisEqualMemory(&IE[2], &MTK_OUI[0], 3))
			...
			Set_AP_VENDOR_SPECIFIC_IE(pAd, OUIType, IE, IELen, ApIdx);
		break;
...
int wapp_send_wapp_qry_rsp(IN PRTMP_ADAPTER pAd, IN struct wapp_event *event)
{
#ifdef RT_CFG80211_SUPPORT
	int ret = 0;

	ret = nl80211_send_wapp_qry_rsp(pAd->pCfg80211_CB->pCfg80211_Wdev->wiphy,
				pAd->pCfg80211_CB->pCfg80211_Wdev, event);
	if (ret != 0) {
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
			"Err: event send fail!\n");
		return ret;
...
int wapp_send_event_offchannel_info(IN PRTMP_ADAPTER pAd, IN void *data, IN int len)
{
#ifdef RT_CFG80211_SUPPORT
	int ret;

	ret = nl80211_send_event_offchannel_info(pAd->pCfg80211_CB->pCfg80211_Wdev->wiphy,
						 pAd->pCfg80211_CB->pCfg80211_Wdev, data, len);
	if (ret != 0) {
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WAPP, DBG_LVL_ERROR,
			"Err: event send fail!\n");
		return ret;
