...
INT rtmp_read_country_region_from_eeporm(RTMP_ADAPTER *pAd)
{
	...
	value = ph_dev->EEPROMDefaultValue[EEPROM_COUNTRY_REG_OFFSET] >> 8;	/* 2.4G band*/
	value2 = ph_dev->EEPROMDefaultValue[EEPROM_COUNTRY_REG_OFFSET] & 0x00FF;	/* 5G band*/
...
/*
	Read BAND_SELECT_BAND from EEPROM
*/

INT rtmp_read_bandinfo_from_eeprom(RTMP_ADAPTER *pAd)
{
	UCHAR band_idx = hc_get_hw_band_idx(pAd);
	USHORT value = 0, bandinfo = 0;

	RT28xx_EEPROM_READ16(pAd, EEPROM_BAND_SELECT_BAND, value);

	switch (band_idx) {
	case BAND0:
		bandinfo = (value & BAND_SELECT_BAND0_MASK) >> BAND_SELECT_BAND0_OFFSET;
		break;
	case BAND1:
		bandinfo = (value & BAND_SELECT_BAND1_MASK) >> BAND_SELECT_BAND1_OFFSET;
		break;
	case BAND2:
		bandinfo = (value & BAND_SELECT_BAND2_MASK) >> BAND_SELECT_BAND2_OFFSET;
		break;
	default:
		break;
	}

	pAd->CommonCfg.BandSelBand = (UCHAR) bandinfo;
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "BAND_SELECT_BAND = %x\n", bandinfo);

	return TRUE;
}

INT show_bandinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	rtmp_read_bandinfo_from_eeprom(pAd);
	return TRUE;
}
...
INT NICReadEEPROMParameters(
	RTMP_ADAPTER *pAd, RTMP_STRING *mac_addr, BOOLEAN is_cal)
{
	EEPROM_VERSION_STRUC Version = {0};
	EEPROM_ANTENNA_STRUC Antenna;
	UINT16 BandSel = 0;
#ifdef MAC_ADDR_ADJACENT_CHK
	u8 MaxBands = PD_GET_BAND_NUM(pAd->physical_dev);
...
#if defined(PRE_CAL_TRX_SET2_SUPPORT) || defined(PRE_CAL_MT7915_SUPPORT) || \
	defined(PRE_CAL_MT7986_SUPPORT) || defined(PRE_CAL_MT7981_SUPPORT) || \
	defined(PRE_CAL_MT7916_SUPPORT) || defined(PRE_CAL_MT7990_SUPPORT) || \
	defined(PRE_CAL_MT7992_SUPPORT) || defined(PRE_CAL_MT7993_SUPPORT)
	UINT16 DoPreCal = 0;
#endif /* PRE_CAL_TRX_SET2_SUPPORT */
	struct _RTMP_CHIP_OP *ops = hc_get_chip_ops(pAd->hdev_ctrl);

#if defined(PRE_CAL_TRX_SET2_SUPPORT) || \
	defined(PRE_CAL_MT7915_SUPPORT) || defined(PRE_CAL_MT7986_SUPPORT) || \
	defined(PRE_CAL_MT7981_SUPPORT) || defined(PRE_CAL_MT7916_SUPPORT) || \
	...
	if (ops->eeinit) {
		...
		ops->eeinit(pAd);
		RTMP_GET_BAND_SELECT(pAd, &BandSel);
#ifdef CAL_FREE_IC_SUPPORT
		RTMP_CAL_FREE_IC_CHECK(pAd, bCalFree);
		if (bCalFree) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "Cal Free IC!!\n");
		...
		if (IS_MT7990(pAd)) {
			...
			struct physical_device *ph_dev = pAd->physical_dev;
			UINT32 reg_val = 0;
			UINT32 reg_val_is_2adie = 0;
			UINT32 reg_val_2adie_triband = 0;
			/* Check DoPreCal bits */
			RTMP_IO_READ32(pAd->hdev_ctrl, MT7990_CBTOP_GPIO_ADIE_COMBATION_STRAP_ADDR, &reg_val);

			reg_val_is_2adie = (reg_val & MT7990_CBTOP_GPIO_ADIE_COMBATION_STRAP_MASK) >>
			MT7990_CBTOP_GPIO_ADIE_COMBATION_STRAP_SHIF;

			reg_val_2adie_triband = (reg_val & MT7990_CBTOP_GPIO_2ADIE_TRIBAND_STRAP_MASK) >>
			MT7990_CBTOP_GPIO_2ADIE_TRIBAND_STRAP_SHIF;

			if ((reg_val_is_2adie == MT7990_2ADIE_75_77) || (reg_val_is_2adie == MT7990_2ADIE_76_77)) {
				if (reg_val_2adie_triband == MT7990_2ADIE_3BAND) {
					cap->prek_ee_info.pre_cal_total_size = cap->prek_ee_info.pre_cal_total_size_2adie;
					MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
						"2Die modify group cal size\n");
				}
			}
			RT28xx_EEPROM_READ16(pAd, PRECAL_INDICATION_BYTE, DoPreCal);
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
				"\x1b[34m EEPROM 0x%2x %x\x1b[m\n", PRECAL_INDICATION_BYTE, DoPreCal);
			if (ph_dev->E2pAccessMode == E2P_FLASH_MODE ||
				ph_dev->E2pAccessMode == E2P_BIN_MODE) {
				if (DoPreCal & (1 << GROUP_PRECAL_INDN_BIT)) {
					...
					while (setPreCalTotal > 0) {
						...
						MtCmdSetGroupPreCal_7990(pAd, setPreCalIdx, sendLength);
						MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
				...
				if (DoPreCal & (1 << DPD6G_PRECAL_INDN_BIT)) {
					MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
						"DPD 6G Pre-Cal finished!\n");
				}
			...
			if (ph_dev->E2pAccessMode == E2P_FLASH_MODE ||
				...
				if (DoPreCal & (1 << GROUP_PRECAL_INDN_BIT)) {
					...
					while (setPreCalTotal > 0) {
						sendLength = (setPreCalTotal >= PRE_CAL_SET_MAX_LENGTH) ?
									PRE_CAL_SET_MAX_LENGTH : setPreCalTotal;
						MtCmdSetGroupPreCal_7992(pAd, setPreCalIdx, sendLength);
						MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
							"idx=%d, sendLength=%d, remainedLength=%d\n",
							setPreCalIdx, sendLength, setPreCalTotal);
						setPreCalIdx++;
			...
			if (ph_dev->E2pAccessMode == E2P_FLASH_MODE ||
				...
				if (DoPreCal & (1 << GROUP_PRECAL_INDN_BIT)) {
					...
					while (setPreCalTotal > 0) {
						sendLength = (setPreCalTotal >= PRE_CAL_SET_MAX_LENGTH) ?
									PRE_CAL_SET_MAX_LENGTH : setPreCalTotal;
						MtCmdSetGroupPreCal_7993(pAd, setPreCalIdx, sendLength);
						MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
							"idx=%d, sendLength=%d, remainedLength=%d\n",
							setPreCalIdx, sendLength, setPreCalTotal);
						setPreCalIdx++;
			...
			if (ret != NDIS_STATUS_SUCCESS) {
				...
				if (PD_GET_E2P_ACCESS_MODE(pAd->physical_dev) == E2P_BIN_MODE) {
					...
					if (ret_cal_data != NDIS_STATUS_SUCCESS) {
						...
						MTWF_DBG(pAd,
						DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
...
#endif
	{
		...
		chip_pa_lna_set(pAd, ph_dev->EEPROMImage);
		/*configure TSSI sestting by efuse*/
		chip_tssi_set(pAd, ph_dev->EEPROMImage);
		if (is_cal)
			MtCmdEfusBufferModeSet(pAd, PD_GET_EEPROM_TYPE(pAd->physical_dev));
		else
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_NOTICE,
				"mac efuse ready done, ignore efuse set to fw.\n");
	}
	...
	if (hc_get_hw_band_idx(pAd) == BAND0) {
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND0_MAC0, Addr01);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND0_MAC1, Addr23);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND0_MAC2, Addr45);
	...
	} else if (hc_get_hw_band_idx(pAd) == BAND1) {
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND1_MAC0, Addr01);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND1_MAC1, Addr23);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND1_MAC2, Addr45);
		pAd->PermanentAddress[0] = (UCHAR)(Addr01 & 0xff);
		pAd->PermanentAddress[1] = (UCHAR)(Addr01 >> 8);
		pAd->PermanentAddress[2] = (UCHAR)(Addr23 & 0xff);
		pAd->PermanentAddress[3] = (UCHAR)(Addr23 >> 8);
		pAd->PermanentAddress[4] = (UCHAR)(Addr45 & 0xff);
		pAd->PermanentAddress[5] = (UCHAR)(Addr45 >> 8);
	} else if (hc_get_hw_band_idx(pAd) == BAND2) {
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND2_MAC0, Addr01);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND2_MAC1, Addr23);
		RT28xx_EEPROM_READ16(pAd, EEPROM_BAND2_MAC2, Addr45);
		pAd->PermanentAddress[0] = (UCHAR)(Addr01 & 0xff);
		pAd->PermanentAddress[1] = (UCHAR)(Addr01 >> 8);
		pAd->PermanentAddress[2] = (UCHAR)(Addr23 & 0xff);
		pAd->PermanentAddress[3] = (UCHAR)(Addr23 >> 8);
	...
	if (IS_BELLWETHER(pAd)) {
		if (hc_get_hw_band_idx(pAd) == BAND1)
			pAd->PermanentAddress[0] |= (0x8);

		if (hc_get_hw_band_idx(pAd) == BAND2)
			pAd->PermanentAddress[0] |= (0xc);

	}

	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "E2PROM MAC: ="MACSTR"\n",
	...
	/*Insert MAC address to ra mac table*/
	if (hc_get_hw_band_idx(pAd) == BAND0)
		COPY_MAC_ADDR(ph_dev->ra_mac_table[BAND0], pAd->CurrentAddress);
	else if (hc_get_hw_band_idx(pAd) == BAND1)
		COPY_MAC_ADDR(ph_dev->ra_mac_table[BAND1], pAd->CurrentAddress);
	else if (hc_get_hw_band_idx(pAd) == BAND2)
		COPY_MAC_ADDR(ph_dev->ra_mac_table[BAND2], pAd->CurrentAddress);

#ifdef MAC_ADDR_ADJACENT_CHK
	/* per band MAC in sequence condition check  */
	if ((MaxBands >= 2) && (hc_get_hw_band_idx(pAd) == (MaxBands-1))) {
		INT i  = 0;
		UCHAR InvalidMac[MAC_ADDR_LEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
		UCHAR zeroMac[MAC_ADDR_LEN]  = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

		for (i = 0 ; i < (MaxBands-1); i++) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_NOTICE, "BN(%d) MAC: ="MACSTR"\n",
					  i, MAC2STR(ph_dev->ra_mac_table[i]));
			if (!NdisEqualMemory(InvalidMac, ph_dev->ra_mac_table[i], MAC_ADDR_LEN)
				&& !NdisEqualMemory(zeroMac, ph_dev->ra_mac_table[i], MAC_ADDR_LEN)
				&& !NdisEqualMemory(InvalidMac, ph_dev->ra_mac_table[i+1], MAC_ADDR_LEN)
				&& !NdisEqualMemory(zeroMac, ph_dev->ra_mac_table[i+1], MAC_ADDR_LEN)) {
				if ((ph_dev->ra_mac_table[i][5] & 0xF0) == (ph_dev->ra_mac_table[i+1][5] & 0xF0))
					MTWF_PRINT("\x1b[41m !!! BN(%d) and BN(%d): MAC ADDR ADJACENT !!! \x1b[m\n", i, (i+1));
			}
		}
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_NOTICE, "BN(%d) MAC: ="MACSTR"\n",
				  i, MAC2STR(ph_dev->ra_mac_table[i]));
	...
	ph_dev->EepromVersion = Version.field.Version + Version.field.FaeReleaseNumber * 256;
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
...
void rtmp_eeprom_info_extract(RTMP_ADAPTER *pAd)
{
	...
	if (chip_ops->eeprom_extract) {
		chip_ops->eeprom_extract(pAd, (VOID *)ph_dev->EEPROMImage);
	}
}
...
)
{
	...
	pu1Param = (PUINT8)(ph_dev->EfuseInfoAll.prSys);
...
)
{
	...
	struct physical_device *ph_dev = pAd->physical_dev;
								/* BAND_5G						BAND_2G4 */
	UINT_8 u1StartAdd[EFUSE_COMMON_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{EFUSE_INFO_5G_COMMON_TEMP_TSSI_OFFSET, EFUSE_INFO_2G4_COMMON_TEMP_TSSI_OFFSET},
								...
								/* BAND_5G		BAND_2G4 */
	UINT_8 u1GroupNum[EFUSE_COMMON_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{8, 1},
								 ...
								 /* BAND_5G						BAND_2G4 */
	UINT_8 u1ColumnNum[EFUSE_COMMON_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{EFUSE_COMMON_TEMP_TSSI_CATEGORY_ITEM_NUM, EFUSE_COMMON_TEMP_TSSI_CATEGORY_ITEM_NUM},
	...
	pu1Param = (eBand == BAND_24G) ?
		((PUINT8)ph_dev->EfuseInfoAll.pr2G4Cmm) : ((PUINT8)ph_dev->EfuseInfoAll.pr5GCmm);
...
)
{
	...
	UINT_8 u1StartAdd[EFUSE_WF_PATH_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{EFUSE_INFO_5G_WF_PATH_FE_LOSS_OFFSET, EFUSE_INFO_2G4_WF_PATH_FE_LOSS_OFFSET},
								 ...
								 /* BAND_5G		 BAND_2G4 */
	UINT_8 u1GroupNum[EFUSE_WF_PATH_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{3, 1},
								   {8, 1},
								   {SINGLE_GROUP, SINGLE_GROUP},
								   {8, 1},
								   {4, 1},
								  ...
								  /* BAND_5G						 BAND_2G4 */
	UINT_8 u1ColumnNum[EFUSE_WF_PATH_CATOGORY_NUM][CFG_WIFI_RAM_BAND_NUM] = {{EFUSE_5G_WF_PATH_FE_LOSS_CATEGORY_ITEM_NUM, EFUSE_2G4_WF_PATH_FE_LOSS_CATEGORY_ITEM_NUM},
	...
	pu1Param =
		(eBand == BAND_24G) ?
			((PUINT8)ph_dev->EfuseInfoAll.pr2G4WFPath[AntIdx])
			: ((PUINT8)ph_dev->EfuseInfoAll.pr5GWFPath[AntIdx]);
...
)
{
	...
	case CATEGORY_NTX_POWER_OFFSET:
		pu1Param = (PUINT8)(ph_dev->EfuseInfoAll.prNTxPowerOffset);
	...
	case CATEGORY_TX_POWER_RATE:
		...
		pu1Param =
			(eBand == BAND_24G) ?
				((PUINT8)(ph_dev->EfuseInfoAll.pr2G4TxPower))
				: ((PUINT8)(ph_dev->EfuseInfoAll.pr5GTxPower));
		if (fgGBand == TRUE)
...
)
{
	...
	pu1Param =
		(eBand == BAND_24G) ?
		((PUINT8)(ph_dev->EfuseInfoAll.pr2G4IBfParam))
		: ((PUINT8)(ph_dev->EfuseInfoAll.pr5GIBfParam));
	fgGBand = (eBand == BAND_24G) ? (TRUE) : (FALSE);
	if (fgGBand == TRUE)
...
)
{
	...
	case CATEGORY_DEALY_COMP_CTRL:
		pu1Param = (PUINT8)(ph_dev->EfuseInfoAll.prDelayCompCtrl);
		break;
	case CATEGORY_RX_LATENCY:
		pu1Param =
			(eBand == BAND_24G) ?
			((PUINT8)(ph_dev->EfuseInfoAll.pr2G4RxLatency))
			: ((PUINT8)(ph_dev->EfuseInfoAll.pr5GRxLatency));
		break;
	default:
		break;
	}
...
static NDIS_STATUS rtmp_ee_bin_init(PRTMP_ADAPTER pAd)
{
	...
	if (ph_dev->EEPROMImage == NULL) {
		...
		if (ph_dev->EEPROMImage == NULL) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_ERROR,
				"Allocate memory for ph_dev->EEPROMImage failed!\n");
			return NDIS_STATUS_FAILURE;
		}
	}
#endif
...
#ifdef PRE_CAL_TRX_SET1_SUPPORT
	{
		...
		if (pAd->CalDCOCImage == NULL) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_ERROR,
				"Allocate memory for pAd->CalDCOCImage failed!\n");
			return NDIS_STATUS_FAILURE;
		}
		os_alloc_mem(pAd, &pAd->CalDPDAPart1Image, TXDPD_IMAGE1_SIZE);
		if (pAd->CalDPDAPart1Image == NULL) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_ERROR,
				"Allocate memory for pAd->CalDPDAPart1Image failed!\n");
			if (pAd->CalDCOCImage != NULL)
		...
		if (pAd->CalDPDAPart2Image == NULL) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_ERROR,
				"Allocate memory for pAd->CalDPDAPart2Image failed!\n");
			if (pAd->CalDCOCImage != NULL)
				os_free_mem(pAd->CalDCOCImage);
		...
		if (NDIS_STATUS_SUCCESS == rtmp_cal_load_from_bin(pAd, pAd->CalDCOCImage, 0, DCOC_IMAGE_SIZE))
			...
			MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_INFO,
					"bDCOCReloaded = false.\n");
	...
	if (IS_MT7990(pAd) ||
		...
		pAd->PreCalImageInfo = ph_dev->EEPROMImage + cap->EEPROM_DEFAULT_BIN_SIZE;
		pAd->PreCalImage     = ph_dev->EEPROMImage + prek_ee_offset;
		pAd->TxDPDImage      = ph_dev->EEPROMImage + dpdk_ee_offset;
		MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_INFO,
			"\x1b[42m[EEPROMImage - PreCalImageInfo - PreCalImage - TxDPDImage]\x1b[m\n"
			"\x1b[42m[0x%p - 0x%p - 0x%p - 0x%p]\x1b[m\n",
			ph_dev->EEPROMImage, pAd->PreCalImageInfo,
			pAd->PreCalImage, pAd->TxDPDImage);
		}
	}
#endif

	{
		...
		pAd->RXGainCal = ph_dev->EEPROMImage + rxgaincal_offset;
		MTWF_DBG(pAd, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_INFO,
				"\x1b[42m[EEPROMImage - RXGainCal]\x1b[m\n"
				 "\x1b[42m[0x%p - 0x%p]\x1b[m\n",
				  ph_dev->EEPROMImage, pAd->RXGainCal);
	}
	...
	UINT8 cmd_total)
{
	...
	if (ops->bufferModeCmdFill) {
		...
		MTWF_DBG(ad, DBG_CAT_FW, CATHW_EEPROM, DBG_LVL_ERROR,
...
INT RtmpChipOpsEepromHook(RTMP_ADAPTER *pAd, INT forceMode)
{
	...
	if (forceMode != E2P_NONE && forceMode < NUM_OF_E2P_MODE) {
		...
		e2p_type = PD_GET_E2P_ACCESS_MODE(ph_dev);
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
			"e2p_type=%d\n", e2p_type);

		/* If e2p_type is out of range, get the default mode */
		e2p_type =
			((e2p_type != 0) && (e2p_type < NUM_OF_E2P_MODE)) ? e2p_type : e2p_default;

#ifdef RTMP_EFUSE_SUPPORT
		if (e2p_type != E2P_EFUSE_MODE)
			PD_SET_USE_EFUSE(ph_dev, FALSE);
#endif /* RTMP_EFUSE_SUPPORT */
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
		...
		e2p_type, PD_GET_E2P_ACCESS_MODE(ph_dev), e2p_default);
	PD_SET_EEPROM_TYPE(ph_dev, (e2p_type == E2P_EFUSE_MODE ||
		e2p_type == E2P_EEPROM_MODE) ? EEPROM_EFUSE : EEPROM_FLASH);
	pE2pCtrl->e2pCurMode = e2p_type;

	if (ph_dev->EEPROMImage == NULL) {
		os_alloc_mem(pAd, &ph_dev->EEPROMImage,
				(get_dev_eeprom_size(pAd) > cap->EFUSE_BUFFER_CONTENT_SIZE) ?
				 get_dev_eeprom_size(pAd) : cap->EFUSE_BUFFER_CONTENT_SIZE);
		if (ph_dev->EEPROMImage == NULL) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
				"Allocate memory for ph_dev->EEPROMImage failed, please check!\n");
		}
	}

	switch (e2p_type) {
	case E2P_EEPROM_MODE: {
		pChipOps->eeinit = rtmp_ee_i2c_eeprom_init;
		pChipOps->eeread = rtmp_ee_i2c_eeprom_read16;
		pChipOps->eewrite = rtmp_ee_i2c_eeprom_write16;
		pChipOps->ee_gen_cmd = rtmp_ee_fr_mcu;

		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "NVM is I2C_EEPROM mode\n");
		break;
	}
	...
	case E2P_BIN_MODE: {
		...
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "NVM is BIN mode\n");
		break;
	}
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
			"Do not support E2P type(%d), change to BIN mode\n",
			e2p_type);
		pChipOps->eeinit = rtmp_ee_bin_init;
		pChipOps->eeread = rtmp_ee_bin_read16;
		pChipOps->eewrite = rtmp_ee_bin_write16;
...
BOOLEAN rtmp_ee_bin_read16(RTMP_ADAPTER *pAd, UINT32 Offset, UINT16 *pValue)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_DEBUG, "Read from EEPROM buffer\n");
	NdisMoveMemory(pValue, &(ph_dev->EEPROMImage[Offset]), 2);
	...
	IN USHORT			data)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_DEBUG, "Write to EEPROM buffer\n");
	data = le2cpu16(data);
	NdisMoveMemory(&(ph_dev->EEPROMImage[Offset]), &data, 2);
	return 0;
}
...
INT rtmp_ee_bin_write_with_range(PRTMP_ADAPTER pAd, UINT32 start, UINT32 Length, UCHAR *pbuf)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_DEBUG,
		"Write data range to EEPROM buffer\n");
	NdisMoveMemory(&(ph_dev->EEPROMImage[start]), pbuf, Length);
...
lib_firmware:
	...
	if (ops && ops->get_bin_image_file != NULL) {
		ops->get_bin_image_file(pAd, src, fgBinMode);
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
			"bin FileName=%s\n", src);
	} else {
	...
	if (strlen(src)) {
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			if (fgBinMode) {
				...
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
				"\n\n\n\n\n");
				/*In the CPE project, if loading bin fail in nvcfg,*/
				/*then loading from /lib/firmware/MT7990_EEPROM.bin*/
				goto lib_firmware;
			}
#endif
			goto error;
		} else {
			NdisZeroMemory(ph_dev->EEPROMImage,
					((get_dev_eeprom_size(pAd) > chip_cap->EEPROM_DEFAULT_BIN_SIZE) ?
					   get_dev_eeprom_size(pAd) : chip_cap->EEPROM_DEFAULT_BIN_SIZE));
			RtmpOSFileSeek(srcf, get_dev_eeprom_offset(pAd));
			ret_val = RtmpOSFileRead(srcf, (RTMP_STRING *)ph_dev->EEPROMImage,
						 ((get_dev_eeprom_size(pAd) > chip_cap->EEPROM_DEFAULT_BIN_SIZE) ?
						   get_dev_eeprom_size(pAd) : chip_cap->EEPROM_DEFAULT_BIN_SIZE));
	...
	ret = snprintf(pE2pCtrl->BinSource, sizeof(pE2pCtrl->BinSource), "%s", src);
	if (os_snprintf_error(sizeof(pE2pCtrl->BinSource), ret))
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
				"snprintf error!\n");
	RtmpOSFSInfoChange(&osFSInfo, FALSE);
	return TRUE;
error:
	if (cap->EEPROM_DEFAULT_BIN != NULL) {
		NdisMoveMemory(ph_dev->EEPROMImage, cap->EEPROM_DEFAULT_BIN,
		...
		pE2pCtrl->e2pSource = E2P_SRC_FROM_BIN;
		ret = snprintf(pE2pCtrl->BinSource, sizeof(pE2pCtrl->BinSource), "Default BIN");
		if (os_snprintf_error(sizeof(pE2pCtrl->BinSource), ret))
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
					"snprintf error!\n");
	}
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	if (rtmp_get_default_bin_file_by_chip(pAd, chip_ver, &bin_file_name) == TRUE) {
		if (pAd->MC_RowID > 0) {
			ret = snprintf(bin_file_path, sizeof(bin_file_path), "%s%s", EEPROM_2ND_FILE_DIR, bin_file_name);
			if (os_snprintf_error(sizeof(bin_file_path), ret)) {
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
					"snprintf error!\n");
				return FALSE;
			}
		} else {
			ret = snprintf(bin_file_path, sizeof(bin_file_path), "%s%s", EEPROM_1ST_FILE_DIR, bin_file_name);
			if (os_snprintf_error(sizeof(bin_file_path), ret)) {
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
					"snprintf error!\n");
				return FALSE;
		...
		strlcpy(src, bin_file_path, sizeof(src));
	} else
	...
	if (strlen(src)) {
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			RtmpOSFileSeek(srcf, get_dev_eeprom_offset(pAd));
			RtmpOSFileWrite(srcf, (RTMP_STRING *)ph_dev->EEPROMImage,
					(get_dev_eeprom_size(pAd) > chip_cap->EEPROM_DEFAULT_BIN_SIZE) ?
						get_dev_eeprom_size(pAd) : chip_cap->EEPROM_DEFAULT_BIN_SIZE);
		}
	} else {
...
INT Set_LoadEepromBufferFromBin_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (bEnable < 0)
		...
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO,
			"Load EEPROM buffer from BIN, and change to BIN buffer mode\n");
...
INT Set_EepromBufferWriteBack_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	case E2P_EEPROM_MODE:
		...
		rtmp_ee_write_to_i2c_eeprom(pAd);
		break;
...
INT show_e2pinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_PRINT("Default eeprom mode from profile: %s\n",
		e2p_mode[PD_GET_E2P_ACCESS_MODE(pAd->physical_dev)]);
	...
	if (pE2pCtrl->e2pSource & E2P_SRC_FROM_BIN)
