...
INT	Set_RateAlg_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if ((ra_alg < RATE_ALG_MAX_NUM) && (ra_alg != pAd->rateAlg)) {
		...
		for (IdEntry = 0; VALID_UCAST_ENTRY_WCID(pAd, IdEntry); IdEntry++) {
			entry = entry_get(pAd, IdEntry);
			entry->rateAlg = ra_alg;
		}
	}
	...
	if (arg) {
		do {
			...
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_NOTICE,
				"WCID = %d, Mode = %d, BW = %d, MCS = %d, VhtNss = %d\n"
	...
	IN	RTMP_STRING *arg)
{
	...
	UINT16 i;
	INT32 i4Recv = 0;
	UINT32 u4Mode = 0, u4Bw = 0, u4Mcs = 0, u4VhtNss = 0, u4SGI = 0;
	UINT32 u4Preamble = 0, u4STBC = 0, u4LDPC = 0, u4SpeEn = 0, u4HeLtf = 0;
	UCHAR ucNsts;
	MAC_TABLE_ENTRY *pEntry = NULL;
	struct _RTMP_CHIP_CAP *cap;
	...
	i4Recv = sscanf(arg, "%d-%d-%d-%d-%d-%d-%d-%d-%d-%d",
		&(u4Mode), &(u4Bw), &(u4Mcs), &(u4VhtNss), &(u4SGI),
		&(u4Preamble), &(u4STBC), &(u4LDPC), &(u4SpeEn), &(u4HeLtf));
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_NOTICE,
		"Mode = %d, BW = %d, MCS = %d, VhtNss = %d, SGI = %d,\n"
		"Preamble = %d, STBC = %d, LDPC = %d, SpeEn = %d, u4HeLtf = %d\n",
		u4Mode, u4Bw, u4Mcs, u4VhtNss, u4SGI, u4Preamble, u4STBC, u4LDPC, u4SpeEn, u4HeLtf);
	...
	if ((u4Mode == MODE_CCK && u4Mcs > 3) ||
		...
		(u4Mode == MODE_HTMIX && u4Mcs > 32) ||
		(u4Mode == MODE_VHT && u4Mcs > 9) ||
		(u4Mode == HW_HE_SU_MODE && u4Mcs > 11) ||
		(u4Mode == HW_EHT_MU && u4Mcs > 13)) {
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_WARN,
			"Unknown MCS!\n");
		goto Error;
	}
	...
	for (i = 1; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (IS_ENTRY_NONE(pEntry) || (pEntry->Sst != SST_ASSOC)
			|| (pEntry->pAd != pAd) || (pEntry->func_tb_idx != pObj->ioctl_if))
			continue;
		...
		if (cap->fgRateAdaptFWOffload == TRUE) {
			...
			rRaParam.FixedRateCfg.STBC = u4STBC;
			rRaParam.FixedRateCfg.ShortGI = u4SGI;
			rRaParam.FixedRateCfg.he_ltf = u4HeLtf;
			rRaParam.FixedRateCfg.BW = u4Bw;
			rRaParam.FixedRateCfg.ldpc = TxPhyCfg.ldpc;
			rRaParam.FixedRateCfg.MCS = u4Mcs;
			rRaParam.FixedRateCfg.VhtNss = u4VhtNss;
...
Error:
	MTWF_PRINT("iwpriv ra0 set FixedRatePerBss=");
	MTWF_PRINT("[Mode]-[BW]-[MCS]-[VhtNss]-[SGI]-[Preamble]-[STBC]-[LDPC]-[SPE_EN]-[HE_LTF]\n");
	MTWF_PRINT("[Mode]CCK=0, OFDM=1, HT=2, GF=3, VHT=4, HE_SU=8, HE_EXT_SU=9, EHT_SU=15\n");
	MTWF_PRINT("[BW]BW20=0, BW40=1, BW80=2, BW160=3, BW320=4\n");
	MTWF_PRINT("[MCS]CCK=0~4, OFDM=0~7, HT=0~32, VHT=0~9, HE=0~11, EHT=0~13\n");
	MTWF_PRINT("[VhtNss]VHT/HE=1~4, Other=ignore\n");
	MTWF_PRINT("[SGI]HT/VHT:Long=0, Short=7; HE:SGI/MGI/LGI=0/1/2\n");
	MTWF_PRINT("[Preamble]Long=0, Short=1\n");
	MTWF_PRINT("[STBC]Disable=0, Enable=1\n");
	MTWF_PRINT("[LDPC]BCC=0, LDPC=1\n");
	MTWF_PRINT("[SPE_EN]Disable=0, Enable=1\n");
	MTWF_PRINT("[HE_LTF]1X=0, 2X=1, 4X=2\n");
	...
	IN RTMP_STRING * arg)
{
	...
	if (!VALID_UCAST_ENTRY_WCID(pAd, u4WlanIndex)) {
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_ERROR,
			"u4WlanIndex exceed pAd->MaxUcastEntryNum!\n");
		return FALSE;
	}
	pEntry = entry_get(pAd, u4WlanIndex);
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			...
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
				"WCID = %d, HE_LTF = %d\n", u4WCID, u4HeLtf);
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			i4Recv = sscanf(arg, "%d-%d", &(u4WCID), &(u4Stbc));
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO, "WCID = %d, STBC = %d\n",
				u4WCID, u4Stbc);
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			...
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_NOTICE, "WCID = %d\n",
				u4WCID);
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			i4Recv = sscanf(arg, "%d", &(u4WCID));
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO, "WCID = %d\n",
				u4WCID);
	...
	IN	RTMP_STRING * arg)
{
	...
	i4Recv = sscanf(arg, "%d-%d", &(AutoRateEn), &(u4Mode));
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_NOTICE, "AutoRateEn = %d, Mode = %d\n", AutoRateEn, u4Mode);
...
Error:
	MTWF_PRINT("iwpriv ra0 set AutoRatePerBss=[AutoRateEn]-[Mode]\n");
	MTWF_PRINT("[AutoRateEn]Disable=0, Enable=1\n");
	MTWF_PRINT("[Mode]CCK=0, OFDM=1, HT=2, GF=3, VHT=4, HE_SU=8, HE_EXT_SU=9, EHT_SU=15\n");
	...
	IN	RTMP_STRING * arg)
{
	...
	if (arg) {
		do {
			...
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO, "WCID = %d, SPE = %d\n",
				u4WCID, u4Spe);
