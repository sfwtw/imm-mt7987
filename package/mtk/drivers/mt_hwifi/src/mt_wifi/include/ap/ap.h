...
/* mcli iwpriv cmd */
#define MCLI_DEBUG_ON			1
#define MCLI_CLI_NUMS_EAP_TH	2
...
enum mgmt_rx_action {
	MGMT_RX_ACCEPT_ALL = 0,
	MGMT_RX_ACCEPT_BEACON_PROBERSP = 1,
	MGMT_RX_REJECT_ALL = 2,
	MGMT_RX_ACTION_MAX
};
...
extern struct tx_stats_monitor_threshold txm_thres;
#endif
...
BOOLEAN ap_rx_mgmt_drop(struct _RTMP_ADAPTER *pAd, FRAME_CONTROL *FC);
...
UINT32 starec_ap_feature_decision(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry, UINT64 *feature);
...
INT ap_tx_pkt_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk);
INT ap_legacy_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk);
INT ap_amsdu_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk);
INT ap_frag_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk);
NDIS_STATUS ap_group_frame_sn(
	...
	OUT UINT8 *mld,
	OUT UINT8 *agent_bss,
	OUT struct wifi_dev **wdev_list,
	OUT PNDIS_PACKET *pkt_list);
...
VOID ap_rx_error_handle(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);
...
VOID  APCls3errAction(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);
...
VOID APCls2errAction(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);

/* ap_connect.c */

/* ap_sync.c */
#ifdef DOT11_EHT_BE
/* 802.11be, for multi-link probe response */
...
VOID MakeQBSSLoadIe(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	BSS_STRUCT *pMbss, UCHAR apidx, ULONG *pFrameLen, UCHAR *pFrame);
...
VOID ap_handle_mic_error_event(RTMP_ADAPTER *ad, MAC_TABLE_ENTRY *entry, struct _RX_BLK *rx_blk);
...
INT set_ap_reconfig_rm_link_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
#define SLAVE_MODE_EN(_pAd) ((_pAd)->slave_ctrl.enable)
#define SLAVE_BEACON_STOPPED(_pAd) ((_pAd)->slave_ctrl.disable_beacon)
