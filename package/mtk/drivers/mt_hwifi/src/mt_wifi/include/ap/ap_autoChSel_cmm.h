...
#define AUTO_CH_SEL_SCAN_RESTORE			        2
#define AUTO_CH_SEL_SCAN_MAX_MSG			        3

#define AUTO_CH_SEL_SCAN_FUNC_SIZE                  (AUTO_CH_SEL_SCAN_MAX_STATE * AUTO_CH_SEL_SCAN_MAX_MSG)


#if defined(TXRX_STAT_SUPPORT) || defined(OFFCHANNEL_SCAN_FEATURE) || defined(ZERO_PKT_LOSS_SUPPORT)
...
typedef struct {
	...
	UINT32 full_cca_time[MAX_NUM_OF_CHANNELS + 1]; /*used for uper daemon Alg*/
...
struct EVENTACSINFO {
	UCHAR channel;
	ULONG ApCnt;
	UINT32 chanbusytime;  /*used for MTK Alg*/
	UINT32 pcca_time; /*used for uper daemon Alg*/
	UINT32 full_cca_time; /*used for uper daemon Alg*/
	UINT32 obss_time; /*used for uper daemon Alg*/
	UINT32 tx_air_time; /*used for uper daemon Alg*/
	UINT32 rx_air_time; /*used for uper daemon Alg*/
	UINT32 non_wifi_time; /*used for uper daemon Alg*/
	UINT32 cca_nav_tx_time; /*used for uper daemon Alg*/
};
...
typedef struct _AUTOCH_SEL_CTRL {
	...
	UINT32 pre_full_pcca_time;
	RALINK_TIMER_STRUCT AutoChScanTimer;
	RALINK_TIMER_STRUCT AutoChResoreTimer;
	...
	BOOLEAN PSC_ACS;
#endif
	BOOLEAN PartialScanACS;
	USHORT ScanningDwell;
	USHORT RestoreDwell;
	UCHAR ScanningChNum;
	UCHAR CountofCh;
	BOOLEAN RestoreFlag;
	/*the max channel priority that ACS should run*/
	UCHAR MaxChPriority;
	UINT32 ACSCheckCount; /* if  ACSCheckCount > ACSCheckTime, then do ACS check */
	UINT32 AutoTriggerCount;
