...
struct phy_cfg {
	...
	UCHAR pp_dl_mu_mimo;
	UCHAR pp_ul_mu_mimo;
	UCHAR pp_mu_ctrl_valid[PP_MU_OP_MAX_NUM];
#endif
};
...
UCHAR wlan_config_get_he_pp_bw(struct wifi_dev *wdev);
...
VOID wlan_config_set_rnr_in_probe_rsp(struct wifi_dev *wdev, UINT8 rnr_6g_in_probe);
#endif /* DOT11_HE_AX */
...
VOID wlan_config_set_ba_txrx_wsize(struct wifi_dev *wdev, UINT16 tx_wsize, UINT16 rx_wsize, UINT16 max_ba_wsize);
VOID wlan_config_set_ht_edca(struct wifi_dev *wdev, UINT8 param, UINT8 aci, UINT16 value);
VOID wlan_config_set_ba_tx_wsize(
