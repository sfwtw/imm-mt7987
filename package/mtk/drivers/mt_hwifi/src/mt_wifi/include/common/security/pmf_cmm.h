...
typedef struct GNU_PACKED _PMF_CFG {
	...
	UCHAR SAQueryConfirmTimes;
	UCHAR SAQueryStatus;
	USHORT TransactionID;
} PMF_CFG, *PPMF_CFG;

struct deauth_pmf_arg_t {
	struct _RTMP_ADAPTER *pAd;
	struct _MAC_TABLE_ENTRY *pEntry;
	struct _MLME_DISCONNECT_STRUCT *pInfo;
};
...
struct bcn_protection_cfg {
	...
	UCHAR bcn_prot_en;
	UINT8 show_bcn_prot_info; /* used to show dbg info */
