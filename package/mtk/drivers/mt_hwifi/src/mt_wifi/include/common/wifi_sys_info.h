...
typedef struct _BSS_INFO_ARGUMENT_T {
	...
	UINT64 u8BssInfoFeature;
	CMD_BSSINFO_PM_T rBssInfoPm;
	union _HTTRANSMIT_SETTING BcTransmit;
	union _HTTRANSMIT_SETTING McTransmit;
	UINT8 FixedRateIdx;
	UINT8 mloFixedRateIdx;
	UINT8 FixedRateBW;
	UINT16	bcn_period;
	UINT8	dtim_period;
	...
	UINT8   tx_omac_idx; /* transmitted mac index */
#endif
...
INT wifi_sys_update_starec(struct _RTMP_ADAPTER *ad, struct _STA_REC_CTRL_T *_new);
INT wifi_sys_update_starec_info(struct _RTMP_ADAPTER *ad, struct _STA_REC_CTRL_T *_new);
...
UINT32 wifi_sys_header_trans_update(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *pEntry);
