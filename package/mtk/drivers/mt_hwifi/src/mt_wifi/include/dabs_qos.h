...
struct qos_param_rec {
	...
	bool valid;
	unsigned long long tot_mac_dly;	/* 0.125 TU */
	unsigned int tot_msdu_cnt;
	...
	unsigned int max_mac_dly;
	unsigned short delay_lvl[16];
	unsigned int delay_lvl_cnt[16];
	unsigned int qdelay_lvl_cnt[16];
...
void dabs_init(PRTMP_ADAPTER pAd);
void dabs_latency_update(RTMP_ADAPTER *pAd, UINT_8 qid, UINT_16 wcid, UINT_16 mac_latency, UINT_16 air_latency, UINT_32 cnt);
