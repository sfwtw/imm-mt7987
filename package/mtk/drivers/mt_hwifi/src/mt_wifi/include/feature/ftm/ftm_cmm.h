...
#define CNT_DOWN_DECREASE(var)	{ \
		if (var) \
			...
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_WARN,  \
				"CAUSION!! Before CntDown the value is zero!!\n"); \
	}


#define FTM_DEFAULT_ROLE				FTMROLE_INIT	/* FTM Initiator */
#define FTM_DEFAULT_ASAP				TRUE
#define FTM_DEFAULT_MIN_DELTA_FTM		40
#define FTM_DEFAULT_FTMS_PER_BURST		6
...
struct _FTM_PEER_INFO {
	/* FTM Parameters */
	UINT8 num_burst_exponent:4;
	UINT8 burst_duration:4;
	UINT8 min_delta_ftm;
	UINT16 partial_tsf;
	BOOLEAN ptsf_no_perference;
	BOOLEAN asap;
	UINT8 ftms_per_burst;
	UINT8 fmt_and_bw; /* format_and_bandwidth */
	UINT8 preamble;
	UINT8 bandwidth;
	UINT16 burst_period;
	...
	BOOLEAN create_STAREC;
	UINT8 trigger_mc_req;
};
...
typedef struct _FTM_PEER_ENTRY {
	...
	struct _FTM_PEER_INFO VerdictPeerInfo;
	UINT8 DialogToken;
	UINT8 LciToken;
	UINT8 CivicToken;
	UINT8 BurstCntDown;
...
struct ftm_toae_cfg {
	...
	UINT8  u1AntIdx;       /* mac antenna index */
	UINT8  u1SpeIdxFtm;    /* 0: wf0 / 1: wf1 */
	UINT8  u1SpeIdxFtmAck; /* 0: wf0 / 1: wf1 */
	UINT16 u2ChainSel;     /* 0x0000 = T0R0 / 0x1111 = T0R1 */
	UINT8  aucpadding[2];
};
...
typedef struct _FTM_CTRL {
	...
	struct _CMD_GET_LOC_CAP_T loc_cap;

	UINT8 ista_work_cand;
	...
	UINT8 LastAssignedRSTA;
	UINT8 LastAssignedISTA;
	struct _FTM_PEER_INFO rSTA_pinfo[MAX_FTM_TBL_SIZE];
	struct _FTM_PEER_INFO iSTA_pinfo[MAX_FTM_TBL_SIZE];
...
enum FTM_ISTA_STATUS {
	FTMISTA_UNUSED,
	FTMISTA_FIRST_BURST,
	FTMISTA_IN_BURST,
	FTMISTA_WAIT_SWITCH_BACK,
};
