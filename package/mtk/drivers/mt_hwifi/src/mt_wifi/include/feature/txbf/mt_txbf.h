...
enum txbf_manual_conf {
	MANUAL_EHT_HE_SU_MU = 0,
	MANUAL_EHT_HE_RU_RANGE,
	MANUAL_EHT_HE_TRIGGER,
	MANUAL_EHT_HE_NG16,
	MANUAL_EHT_HE_CODEBOOK,
	MANUAL_EHT_HE_LTF,
	MANUAL_EHT_HE_IBF,
	MANUAL_EHT_HE_PP
};
...
struct txbf_pfmu_tags_info {
	...
	UINT16 ibf_desired_bw_bitmap;	/* for Kite requirement, need 16-bits BW bitmap */
#else
...
typedef struct _BF_SND_CNT_CFG_T
{
	...
	UINT_16 u2SndCnt[CFG_WIFI_RAM_BAND_NUM];
...
typedef struct _BF_SND_CTRL_EVT_T {
	...
	UINT_16 u2WlanIdxForMcSnd[CFG_WIFI_RAM_BAND_NUM];
	UINT_16 u2WlanIdxForTbSnd[CFG_WIFI_RAM_BAND_NUM];
...
#endif
#ifdef DOT11_EHT_BE
	struct eht_bf_info *eht_bf_info;
#endif
...
BOOLEAN TxBfProfileDataFormatTranslate_5x5(
	IN struct _RTMP_ADAPTER *pAd,
	IN PUCHAR               pucDataIn,
	IN struct _PFMU_HALF_DATA_5X5 *pPfmuHalfData);
...
UINT32 starec_txbf_feature_decision(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry, UINT64 *feature);
...
struct wifi_dev *txbf_get_last_wdev(IN VOID);
