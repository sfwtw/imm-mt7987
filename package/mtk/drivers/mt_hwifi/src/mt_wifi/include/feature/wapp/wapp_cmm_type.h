...
#define WMODE_CAP_AX(_x)               (((_x) & (WMODE_AX_24G | WMODE_AX_5G | WMODE_AX_6G)) != 0)
...
struct GNU_PACKED eml_cap_info {
	u8 emlsr_supp;
	u8 emlsr_padding;
...
struct GNU_PACKED mld_cap_info {
	u8 max_simul_link;
	u8 srs_supp;
	u8 t2l_nego_supp;
	u8 freq_sep_str;
...
struct GNU_PACKED mlo_cap_per_role {
	...
	struct eml_cap_info EmlCap;
	struct mld_cap_info MldCap;
};
...
struct GNU_PACKED mld_data {
	...
	struct mlo_non_setup_info affiliated_info[STA_MLD_LINK_MAX];
#endif
};
...
typedef struct GNU_PACKED _wdev_op_class_info_ext {
	u8		num_of_op_class;
#ifdef MAP_VENDOR_SUPPORT
	u8		is_40M_bw_disable;
#endif /* MAP_VENDOR_SUPPORT */
	struct opClassInfoExt opClassInfoExt[MAX_OP_CLASS];
} wdev_op_class_info_ext;
...
#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
struct GNU_PACKED _wdev_mlo_mld_info {
...
#if (defined(MTK_MLO_MAP_SUPPORT) && defined(DOT11_EHT_BE))
	u8 setup_link_num;
	struct mlo_non_setup_linkinfo linkinfo[3];
#ifdef MAP_R6
...
struct GNU_PACKED multi_link_bss_info {
	...
	uint8_t link_num;
	struct sta_profile_info sta_profiles[STA_MLD_LINK_MAX];
};
#endif /* DOT11_EHT_BE */
...
{
	u64 ch_bitmap[LTE_CHN_MASK_IDX_NUM];
};
...
#ifdef MAP_VENDOR_SUPPORT
#define MAX_MAP_VENDOR_LIMIT 16
#define MAX_MAP_VENDOR_OP_CLASS 64

struct GNU_PACKED channel_pref {
	u8      op_class;
	u8      num_of_ch;
	u8      ch_list[MAX_MAP_VENDOR_LIMIT];
	u8      ch_pref[MAX_MAP_VENDOR_LIMIT];
};

struct GNU_PACKED cont_ch_info {
	u8      num_of_op_class;
	u8  is_40M_bw_disable;
	struct channel_pref channel_pref[MAX_MAP_VENDOR_OP_CLASS];
};
struct GNU_PACKED channel_nop {
	u8      op_class;
	u8      num_of_ch;
	u8      ch_list[MAX_NUM_OF_CHANNELS];
	u8      nop_state[MAX_NUM_OF_CHANNELS];   /* 0 - Radar, 1 - Clear */
};

struct GNU_PACKED cont_nop_info {
	u8      num_of_op_class;
	struct channel_nop channel_nop[MAX_MAP_VENDOR_LIMIT];
};
#endif /* MAP_VENDOR_SUPPORT */
...
union GNU_PACKED wapp_event_data {
...
typedef union GNU_PACKED _RRM_BSSID_INFO
{
	struct GNU_PACKED {
		...
		u32 Reserved:10;
		u32 EHT:1;
		u32 _6G_CO_locatedAP:1;
		u32 OCT_Support:1;
		u32 _2G5G_CO_locatedAP:1;
		u32 _20_TU_ProbeRspActive:1;
		u32 CO_locatedAP:1;
		u32 HE_ER_BSS:1;
		u32 HE:1;
		u32 FTM:1;
