...
#define WSC_HDR_BTN_MR_PRESS_FLG_GET(__pAd, __FlgIsPressed)				\
	{																	\
		UINT32 __gpio_value = 0, mask;											\
		if (IS_HIF_TYPE(__pAd, HIF_MT)) {\
			MTWF_DBG(__pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, "Not support for HIF_MT yet!\n");\
		} else {\
...
#ifdef MAP_R3
#define WSC_DPP_URI_MAX_LEN		120
#endif /* MAP_R3 */
...
#ifdef MAP_R3
	char                            rcvd_dpp_uri[WSC_DPP_URI_MAX_LEN];
	USHORT				rcvd_uri_len;
#endif /* MAP_R3 */
#ifdef WPS_UNCONFIG_FEATURE_SUPPORT
	UCHAR				wps_unconfig_trigger;
#endif
}	WSC_CTRL, *PWSC_CTRL;
