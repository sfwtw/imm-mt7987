...
#define CCK_RATE_MAX  (4)  /* 1, 2, 5.5, 11 */
#define OFDM_RATE_MAX (8)  /* 6, 9, 12, 18, 24, 36, 48, 54 */
#define HT_RATE_MAX   (8)  /* mcs0 - 7 */
#define VHT_RATE_MAX  (10) /* mcs0 - 9 */
#define HE_RATE_MAX   (12) /* mcs0 - 11 */
#define EHT_RATE_MAX  (14) /* mcs0 - 13 */
#if defined(MT7992_SKU_BE3600SDB)
#define RA_NSS_MAX    (2)  /* 2 NSTS */
#else
#define RA_NSS_MAX    (4)  /* 4 NSTS */
#endif
...
#define GET_TX_STAT_ENTRY_TX_RETRY_CNT    0x20
...
struct TXRX_RATE_REPORT_2G_T {
	struct RATE_CNT_T rCckCnt[CCK_RATE_MAX];
	struct RATE_CNT_T rOfdmCnt[OFDM_RATE_MAX];
	struct RATE_CNT_T rHtCnt[RA_NSS_MAX][HT_RATE_MAX];
	struct RATE_CNT_T rHeCnt[RA_NSS_MAX][HE_RATE_MAX];
	struct RATE_CNT_T rEhtCnt[RA_NSS_MAX][EHT_RATE_MAX];
};

struct TXRX_RATE_REPORT_5G_T {
	struct RATE_CNT_T rOfdmCnt[OFDM_RATE_MAX];
	struct RATE_CNT_T rHtCnt[RA_NSS_MAX][HT_RATE_MAX];
	struct RATE_CNT_T rVhtCnt[RA_NSS_MAX][VHT_RATE_MAX];
	struct RATE_CNT_T rHeCnt[RA_NSS_MAX][HE_RATE_MAX];
	struct RATE_CNT_T rEhtCnt[RA_NSS_MAX][EHT_RATE_MAX];
};

struct TXRX_RATE_REPORT_6G_T {
	struct RATE_CNT_T rHeCnt[RA_NSS_MAX][HE_RATE_MAX];
	struct RATE_CNT_T rEhtCnt[RA_NSS_MAX][EHT_RATE_MAX];
};

struct COUNTER_TXRX_RATE_REPORT {
	UINT32 STBCCnt[CFG_WIFI_RAM_BAND_NUM][ENUM_DIR_MAX];
	UINT32 HeEhtGICnt[CFG_WIFI_RAM_BAND_NUM][ENUM_DIR_MAX][ENUM_GI_MAX];
	UINT32 HtVhtGICnt[CFG_WIFI_RAM_BAND_NUM][ENUM_DIR_MAX][ENUM_GI_MAX];
	struct TXRX_RATE_REPORT_2G_T Report2G[ENUM_DIR_MAX];
	struct TXRX_RATE_REPORT_5G_T Report5G[ENUM_DIR_MAX];
	struct TXRX_RATE_REPORT_6G_T Report6G[ENUM_DIR_MAX];
};

struct TXRX_RATE_UNI_CMD_PARAM_T {
	UINT8 ucBandIdx;
	enum ENUM_DIRECTION_T eDir;
	USHORT PhyMode;
	struct COUNTER_TXRX_RATE_REPORT *prTxRxRateReport;
};
...
typedef struct _RA_ENTRY_INFO_T {
	...
	INT_8   AvgRssiSample[MAX_RSSI_LEN];
	UINT_8  ucCERMSD;
...
#define MAX_FW_AGG_MSG 20 /*for host to agg fw msg and fw to agg fw event*/
...
typedef enum _RA_CTRL_OPTION {
	...
	RA_CTRL_OPTION_LEGACY_FREQ_DUP = 1,
	RA_CTRL_OPTION_FAST_RATE_DOWN = 2,
	RA_CTRL_OPTION_UBA_CTRL = 3,
	RA_CTRL_OPTION_BA_BOUND_EN = 4,
	RA_CTRL_OPTION_HRC_EN = 5,
	RA_CTRL_OPTION_AGG_LIMIT_EN = 6,
	RA_CTRL_OPTION_DYNAMIC_VHT_1024QAM_EN = 7,
	RA_CTRL_OPTION_TRY_RATE_SANDBOX_EN = 8,
	RA_CTRL_OPTION_LPI_EN = 9,
	RA_CTRL_OPTION_NUM
} RA_CTRL_OPTION;
...
struct _EXT_EVENT_TX_STATISTIC_RESULT_HEADER_T {
	UINT_8 ucField;
	UINT_8 aucReserve;
	UINT_16 u2Num;
...
struct _EXT_EVENT_TX_STATISTIC_BAND_RESULT_T {
	UINT_8 ucBandIdx;
	RA_PHY_CFG_T rLastTxRate;
	UINT_8 aucResv2[2];
	UINT_32 u4TotalTxCount;
...
struct _EXT_EVENT_TX_STATISTIC_WLAN_RATE_RESULT_T {
	UINT_16 u2WlanIdx;
	RA_PHY_CFG_T rEntryTxRate;
	UINT_8 ucReserve;
};

struct _EXT_EVENT_TX_STATISTIC_WLAN_CNT_RESULT_T {
	UINT_16 u2WlanIdx;
	UINT_8 aucReserve[2];
	UINT_32 u4EntryTxCount;
	UINT_32 u4EntryTxFailCount;
...
struct _EXT_EVENT_TX_STATISTIC_WLAN_PER_RESULT_T {
	UINT_16 u2WlanIdx;
	UINT_8 ucEntryTxPer;
	UINT_8 ucReserve;
};
