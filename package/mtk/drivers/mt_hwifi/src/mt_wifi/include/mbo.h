...
#define MBO_IE_MAX_LEN					12

struct GNU_PACKED neighbor_report_subelement {
	UINT8 subelement_id;
	UINT8 length;
	union {
		struct {
			UINT8 preference;
		} bss_transition_candi_preference;
		...
		struct {
			UINT16 bearing;
			UINT32 distance;
			UINT16 relative_height;
		} bearing;
	} u;
};

struct GNU_PACKED neighbor_report_element {
	UINT8 eid;
	UINT8 length;
	UINT8 bssid[6];
	UINT32 bss_info;
