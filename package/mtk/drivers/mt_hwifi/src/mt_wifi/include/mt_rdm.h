...
typedef enum _ENUM_RDD_RADAR_D
{
	...
	ENUM_RDM_ETSI_7_HOPPING, /* New ETSI hopping radar */
	ENUM_RDM_ETSI_8_HOPPING, /* New ETSI hopping radar */
	ENUM_RDM_JP_2 = 15,
	ENUM_RDM_JP_3,      /* New Japan radar */
	ENUM_RDM_JP_4,      /* New Japan radar */
	ENUM_RDM_KR_1,      /* Korea radar */
	ENUM_RDM_KR_2,      /* Korea radar */
	ENUM_RDM_KR_3 = 20, /* Korea radar */
	ENUM_RDM_BAND4_1,
	ENUM_RDM_BAND4_2,
...
enum {
	HW_RDD0 = 0,	/* Band 2, RDD0, PHY2 */
	HW_RDD1,	/* Band 1, RDD1, PHY1 */
	HW_RDD2,	/* Band 1, RDD2, PHY0*/
	HW_RDD_NUM,
};

enum {
	RDD_INBAND_IDX_2 = 0,		/* Band 2, RDD0, PHY2 */
	RDD_INBAND_IDX_1,		/* Band 1, RDD1, PHY1 */
	RDD_DEDICATED_IDX,		/* Band 1, RDD2, PHY0*/
	HW_RDD_IDX_NUM,
};
...
#define CAC_NON_WEATHER_BAND 65
#define CAC_WEATHER_BAND 605
#define CAC_OCACENABLE_NON_WEATHER_BAND 360
#define CAC_OCACENABLE_WEATHER_BAND 3600
...
#define RadarDetectSelectWidestBWDFS	3
#define RadarDetectSelectNum	4

#define DFS_NOP_EXPIRE_SET_CH_POLICY_NOT_RESTORE	0
#define DFS_NOP_EXPIRE_SET_CH_POLICY_RESTORE		1
#define DFS_NOP_EXPIRE_SET_CH_POLICY_SET_BEST		2
#define DFS_NOP_EXPIRE_SET_CH_POLICY_NUM			3

#define OUTB_SET_CH_DEFAULT		0
#define OUTB_SET_CH_CAC			1
#define OUTB_SET_CH_CAC_DONE	2

#define DFS_PRE_SELECT_CH_DISABLE	0
#define DFS_PRE_SELECT_CH_ENABLE	1
#define DFS_PRE_SELECT_CH_NUM		2
...
#define IS_ADJ_BW_ZERO_WAIT_TX0RX0(_state)	\
		((_state == DFS_BW160_TX0RX0) ||      \
		 (_state == DFS_BW80_TX0RX0))

#define IS_ADJ_BW_ZERO_WAIT_RDD_DETECT(_state)	\
		((_state == DFS_BW160_TX160RX160) ||	\
		 (_state == DFS_BW160_TX80RX160) ||		\
		 (_state == DFS_BW80_TX80RX80) ||      \
		 (_state == DFS_BW80_TX80RX160) ||      \
...
#define IS_SUPPORT_MULTIPLE_RDD_TEST(_pAd) \
	(IS_MT7915(_pAd) || IS_MT7986(_pAd) || IS_MT7916(_pAd) || IS_MT7981(_pAd) || \
	 IS_BELLWETHER(_pAd) || IS_MT7990(_pAd) || IS_MT7992(_pAd) || IS_MT7993(_pAd))
#define IS_SUPPORT_SINGLE_PHY_DBDC_DUAL_RDD(_pAd) (IS_MT7915(_pAd))
...
enum DFS_RANGE_CHANGE_CTRL {
	DFS_RANGE_NARROW = 0X0,
	DFS_RANGE_EXPAND = 0X1,
	DFS_RANGE_NO_CHANGE = 0X2,
	DFS_RANGE_NULL = 0X3,
};
...
typedef enum _ENUM_DFS_INB_CH_SWITCH_STAT_T {
	...
	DFS_INB_DFS_RADAR_OUTB_CAC_DONE_QUICK
...
typedef struct _DFS_PARAM {
	...
	BOOLEAN bNoSwitchCh;
	BOOLEAN bDfsEnable;
	BOOLEAN bDfsWaitCfgChNop;
	BOOLEAN bDfsRestoreCfgChAfterNopEnd;
	...
	UCHAR	ZeroWaitBandidx;	/* select the band for Zero-Wait DFS support*/
	BOOLEAN bZeroWaitCacSecondHandle;
	BOOLEAN bDedicatedZeroWaitSupport;
	BOOLEAN DfsNopExpireSetChPolicy;
	BOOLEAN bDfsDedicatedRxPreselectCh;
#ifdef DFS_VENDOR10_CUSTOM_FEATURE
	...
	BOOLEAN bBootUpInBandUnavailableCh;
	...
	UCHAR DFSChHitBand;
#endif
	UINT16 BW160ZeroWaitState;	/* SW-based Zero-Wait DFS*/
	BOOLEAN BW160ZeroWaitSupport;	/* SW-based Zero-Wait DFS*/
	UCHAR band_orich;		/* SW-based Zero-Wait DFS*/
	BOOLEAN BW80DedicatedZWSupport;
	UCHAR	OutBandOriCh;
	UINT32	TriggerEventIntvl;
	BOOLEAN ucDisTm;
	BOOLEAN ByPassCac; /* non-dfs ch switch to non-dfs ch used for BW160 case*/
	BOOLEAN MbssInitCac; /* non-dfs ch switch to non-dfs ch used for BW160 case*/
	UCHAR ZwAdjBw;
	BOOLEAN ZwAdjBwFlag;
	BOOLEAN ZwChBootUp;
	UCHAR	OrigCh;
	...
	BOOLEAN SetChByCmd; /* True when set channel by command  */
	UCHAR SetOutBandChStat;
#ifdef DFS_SDB_SUPPORT
	BOOLEAN bDfsSdbEnable; /* use dedicateRX to monitor inband radar */
...
INT Set_DfsZeroWaitCert_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
BOOLEAN DfsZwBypassCac(
	PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR OriChannel, UCHAR NewChannel);

VOID Set_Zwdfs_Proc(RTMP_ADAPTER *ad, struct wifi_dev *wdev);

BOOLEAN DfsSetZeroWaitInitMbssCh(
	PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR Ch);
...
VOID DfsSyncCh1stBss(IN struct _RTMP_ADAPTER *ad);
...
VOID DfsSelectChannel(/*Select new channel, finish*/
	IN PRTMP_ADAPTER pAd, PDFS_PARAM pDfsParam, UCHAR ucRddIdx);

UCHAR WrapDfsRandomSelectChannel(/*Select new channel using random selection, finish*/
	IN PRTMP_ADAPTER pAd, UCHAR avoidCh, UCHAR ucRddIdx);

UCHAR DfsRandomSelectChannel(/*Select new channel using random selection, finish*/
	IN PRTMP_ADAPTER pAd, PDFS_PARAM pDfsParam, UCHAR avoidCh, UCHAR ucRddIdx);

USHORT DfsBwChQueryByDefault(/*Query current available BW & Channel list or select default*/
	IN PRTMP_ADAPTER pAd, UCHAR Bw, PDFS_PARAM pDfsParam, UCHAR level, BOOLEAN bDefaultSelect, BOOLEAN SkipNonDfsCh, UCHAR BandIdx);
...
BOOLEAN DfsDedicatedInBandSetChannel(IN PRTMP_ADAPTER pAd, UCHAR Channel, UCHAR Bw, BOOLEAN doCAC, UCHAR band_idx);
...
VOID uni_dfs_dump_radar_hw_pls_info(IN PRTMP_ADAPTER pAd, IN P_UNI_EVENT_RDD_SEND_PULSE_T TlvData);
VOID uni_dfs_dump_radar_sw_pls_info(IN PRTMP_ADAPTER pAd, IN P_UNI_EVENT_RDD_SEND_PULSE_T TlvData);

VOID uni_dfs_update_radar_info(IN P_UNI_EVENT_RDD_SEND_PULSE_T TlvData);
