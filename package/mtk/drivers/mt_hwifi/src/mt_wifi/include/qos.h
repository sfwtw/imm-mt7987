...
#define IS_QOS_ENABLE(pAd) (pAd->bQoSEnable == 1)
...
#ifdef QOS_R3
#define MAX_QOS_CHARA_IE_BUF_LEN 44 /*Max IE len is 41 and 4 alignment is 44*/
#endif
/*status code*/
#define  SUCCESS 0
...
struct GNU_PACKED qos_characteristics {
	...
	u8 request_type;
	u8 dir;
	u8 QosCharIE[MAX_QOS_CHARA_IE_BUF_LEN];
};
#endif

#ifdef EPCS_SUPPORT
#define EPCS_ENTRY_SIZE	16
...
struct epcs_sta {
	...
	u8 mac[ETH_ALEN];
	u8 state;
	struct wmm_parameter wmmpe[4];	/*0:AC_BE, 1:AC_BK, 2:AC_VI, 3:AC_VO*/
};

struct EPCS_ENTRY {
	UINT16 u2WlanIdx;
	UINT16 u2Idx;
	UINT16 flag;	/*0: invalid, 1: valid*/
	u8 mac[ETH_ALEN];
};
...
UCHAR QoS_parse_mscs_descriptor_ies(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *srcAddr, UCHAR *buf, UCHAR buflen);
...
void MakeWfaCapaIe(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR apidx, ULONG *pFrameLen, UCHAR *pFrame);
