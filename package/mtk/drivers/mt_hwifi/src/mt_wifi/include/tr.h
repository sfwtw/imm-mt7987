...
struct fq_stainfo_type {
	struct _QUEUE_ENTRY Entry[WMM_NUM_OF_AC];
...
typedef struct _STA_TR_ENTRY {
	...
	*/
	...
	/*
		Tx Info
	*/
	struct _QUEUE_HEADER tx_queue[WMM_QUE_NUM];
	struct _QUEUE_HEADER ps_queue;
#define SQ_ENQ_DELAYQ_MAX 10
	struct _QUEUE_HEADER delay_queue;
	NDIS_SPIN_LOCK delay_queue_lock;
	UINT enqCount;
...
#define TIME_SLOT_NUMS 10
	...
	/*
	*	original tr_entry->wcid entry usages are most for S/W concept
	*	only minor parts are for H/W concept.
	*	so add extra tr_entry->hw_wcid for hw backup
	*	when fill TxD real H/W wcid
	*/
	UINT16 HwWcid;
#ifdef SW_CONNECT_SUPPORT
	/* mark this entry is pure S/W or not */
	BOOLEAN bSw; /* based on sw main sta to use occupied virtual wcid */
	BOOLEAN bSwMainSta; /* Real the peer that occupied one hw resource */
...
struct tr_flow_control {
	unsigned long TxFlowBlockState;
...
#define queue_deep_cnt_steps 8
	UINT32 queue_deep_cnt[queue_deep_cnt_steps];
#if defined(CTXD_SCATTER_AND_GATHER) || defined(CTXD_MEM_CPY)
	UINT32 ctxd_num[4];
#endif
	UINT32 tx_enq_cpu_stat[4];
	UINT32 tx_deq_cpu_stat[4];

	/* RX */
	UINT32 rx_icv_err_cnt;
	UINT32 rx_sw_q_drop;
	UINT32 rx_invalid_pkt_drop; /* pkt format err */
	UINT32 rx_invalid_wdev; /* wdev is null */
	UINT32 rx_invalid_rxblk_drop; /* invalid rx block format */
	UINT32 rx_invalid_wcid_drop; /* pEntry is null | wcid invalid */
	UINT32 rx_invalid_large_pkt_drop;
	UINT32 rx_invalid_small_pkt_drop;
	UINT32 rx_a4_conn_drop_a3_bmc; /* drop a3 bmc pkt in a4 connection */
	UINT32 rx_crc_err_drop; /* rx crc or phy error */
	UINT32 rx_not_allowed_drop;
	UINT32 rx_invalid_amsdu_drop;
	UINT32 rx_duplicate_drop;
	UINT32 rx_invalid_frag_drop; /* rx de-frag error */
	UINT32 rx_cipher_mismatch_drop; /* rx blk flag (CM | CLM) */
	...
	UINT32 ba_rx_bar_cnt;
	UINT32 ba_tx_bar_cnt;
	UINT32 bar_large_win_start;
	UINT32 ba_flush_all;
	UINT32 ba_amsdu_miss;
	struct _rx_profiling rx_rate_rc;
	UINT32 rx_to_os_cnt;
#ifdef CONFIG_CSO_SUPPORT
	UINT32 rx_cso_err_drop;
#endif /*  CONFIG_CSO_SUPPORT */
	UINT32 rx_nondata_error;
	UINT32 rfdisable_cnt;
	UINT32 rx_cl2cl3_err_cnt;
};
...
enum {
	NO_ENOUGH_SWQ_SPACE = 0,
};

#ifdef HWIFI_SUPPORT
int fill_rx_pkt_info(struct _RTMP_ADAPTER *pAd, struct _RX_BLK *rx_blk);
#endif /* HWIFI_SUPPORT */
