...
struct vht_ch_layout *get_ch_array(UINT8 bw, UCHAR ch_band, INT *ch_layout_size);
...
VOID update_peer_vht_caps(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies);
VOID sta_vht_mode_adjust(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies);
...
INT vht_mode_adjust_sta_max_capab(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *pEntry,
	VHT_CAP_IE *cap, VHT_OP_IE *op, OPERATING_MODE *op_mode, UCHAR *bw_from_opclass);
...
VOID peer_assoc_vht_info_debugshow(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies);
...
UINT32 starec_vht_feature_decision(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry, UINT64 *feature);
UCHAR rf_bw_2_vht_bw(UCHAR rf_bw);
UCHAR vht_bw_2_rf_bw(UCHAR vht_bw);
VOID update_peer_support_mcs(struct _RTMP_ADAPTER *pAd, UCHAR op_mode, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies);
