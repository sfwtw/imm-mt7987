...
BOOLEAN bss_color_acquire(struct wifi_dev *wdev, UINT8 *color)
{
	*color = bcolor_acquire_entry((struct _RTMP_ADAPTER *)wdev->sys_handle);
		...
		struct bss_color_ctrl *bss_color)
{
	...
	bss.u8BssInfoFeature = BSS_INFO_BSS_COLOR_FEATURE;
...
static void bss_color_ageout(struct wifi_dev *wdev, UINT8 sec)
{
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		...
		if (RTMP_TIME_AFTER(current_time, bss_color->collision_time + (sec * OS_HZ))) {
			...
			ad->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt = 0;
		}
	}
#endif

	/* age out the hw resource table entry */
	bcolor_entry_ageout(ad, sec);
	...
	for (idx = 0; idx < WDEV_NUM_MAX; idx++) {
		...
		if (tmp_wdev &&
			...
			if (bss_color->color)
				bcolor_occupy_entry(ad, bss_color->color);
		}
	}
}
	...
	if (OP_MODE == BSS_COLOR_OPMODE_BAND) {
		...
		for (i = 0; i < WDEV_NUM_MAX; i++) {
			...
			if (curr_wdev && (band_idx == HcGetBandByWdev(curr_wdev))
				...
				if (bcn_bpcc_op_lock(pAd, curr_wdev, TRUE, BCN_BPCC_HEOP) == FALSE)
				...
				UpdateBeaconHandler_BPCC(pAd, curr_wdev, BCN_REASON(BCN_UPDATE_IE_CHG),
										BCN_BPCC_HEOP, TRUE);
#ifdef DOT11_EHT_BE
				if (bss_mngr_is_wdev_in_mlo_group(curr_wdev))
					bss_mngr_mld_sync_ml_probe_rsp(curr_wdev);
#endif/*DOT11_EHT_BE*/
			}
		}
	} else if (OP_MODE == BSS_COLOR_OPMODE_SINGLE) {
		if (wdev->wdev_type == WDEV_TYPE_AP) {
			if (bcn_bpcc_op_lock(pAd, wdev, TRUE, BCN_BPCC_HEOP) == FALSE)
				MTWF_PRINT("%s(%d): bcn_bpcc_op_lock fail!\n", __func__, __LINE__);
...
void get_bss_color_bitmap(struct wifi_dev *wdev, UINT8 *bitmap)
{
	...
	bcolor_get_bitmap((struct _RTMP_ADAPTER *)wdev->sys_handle, bitmap);
}
...
static void bss_color_trigger_collision(struct wifi_dev *wdev, UINT8 OP_MODE)
{
	...
	if (pAd->ApCfg.bss_color_cfg.bss_color_collision_ignore)
		return;
		...
		for (i = 0; i < WDEV_NUM_MAX; i++) {
			...
			if (curr_wdev && (band_idx == HcGetBandByWdev(curr_wdev))
				...
				if (bss_color->collision_detected == FALSE) {
					pAd->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt++;
					MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_DEBUG,
						"rem_ap_bss_color_change_cnt for band %d after ++: %d\n",
						band_idx, pAd->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt);
				}
	...
	if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx)) {
		...
		for (i = 0; i < WDEV_NUM_MAX; i++) {
			...
			if (wdev_temp && (band_idx == HcGetBandByWdev(wdev_temp))
				...
				bcolor_update_by_bitmap(ad, bitmap);
...
{
	bcolor_occupy_entry((struct _RTMP_ADAPTER *)wdev->sys_handle, *color);
}
...
void bss_color_event_handler(struct wifi_dev *wdev)
{
	...
	if (bcn_bpcc_op_lock(ad, wdev, TRUE, BCN_BPCC_HEOP) == FALSE)
		MTWF_PRINT("%s(%d): bcn_bpcc_op_lock fail!\n", __func__, __LINE__);

#ifdef CONFIG_AP_SUPPORT
	band_idx = HcGetBandByWdev(wdev);
	if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx)) {
		if (ad->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt > 0)
			ad->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt--;
		MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_DEBUG,
					"rem_ap_bss_color_change_cnt for band %d after --: %d\n",
					band_idx, ad->ApCfg.bss_color_cfg.rem_ap_bss_color_change_cnt);
	} else
#endif
	...
	if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx) &&
		...
		ad->ApCfg.bss_color_cfg.bss_color_next = 0;
			MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_INFO,
					"BSS color change completed for all MBSS\n");
			return;
	}
#endif
	/* Updated ml prob rsp here since set_bss_color_disabled also calls to it */
#ifdef DOT11_EHT_BE
	if (bss_mngr_is_wdev_in_mlo_group(wdev))
		bss_mngr_mld_sync_ml_probe_rsp(wdev);
#endif/*DOT11_EHT_BE*/
...
void bss_color_init(struct wifi_dev *wdev, struct _BSS_INFO_ARGUMENT_T *bssinfo)
{
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		...
		if (pAd->ApCfg.bss_color_cfg.bss_color_enable == BSS_COLOR_DISABLE) {
			bss_color->disabled = TRUE;
			wlan_operate_set_he_bss_color(wdev, 0, TRUE);
			return;
		} else if (pAd->ApCfg.bss_color_cfg.bss_color_enable == BSS_COLOR_VALUE_AUTO_INIT) {
			if (bss_color_acquire(wdev, &bss_color->color) == FALSE) {
				wlan_operate_set_he_bss_color(wdev, 0, TRUE);
				return;
			}
		} else if (pAd->ApCfg.bss_color_cfg.bss_color_enable == BSS_COLOR_VALUE_AUTO_SEL_ALL_MBSS) {
			if (bss_color_acquire(wdev, &bss_color->color) == FALSE) {
				wlan_operate_set_he_bss_color(wdev, 0, TRUE);
				return;
			}
			pAd->ApCfg.bss_color_cfg.bss_color_enable = bss_color->color;

		} else {
			bss_color->color = pAd->ApCfg.bss_color_cfg.bss_color_enable;
			bss_color->disabled = FALSE;
			bcolor_occupy_entry(pAd, bss_color->color);
		}
		...
		IN PVOID SystemSpecific3)
{
	...
	/*acquire BPCC lock*/
	if (bcn_bpcc_op_lock(ad, wdev, TRUE, BCN_BPCC_BSSCCA) == FALSE) {
		if (in_interrupt()) {
			struct _BCN_BSSCCA_UPDATE_PARAM update_param = {0};

			update_param.BSSCAUpdateReason = RSN_COLLISION_DETECT;
			update_param.count = bss_color->u.ap_ctrl.bcc_count;
			/* defer it to workqueue */
			bcn_bpcc_ct_switch(ad, wdev, BCN_BPCC_BSSCCA, BCN_REASON(BCN_UPDATE_IE_CHG), update_param);
			bss_color->u.ap_ctrl.trigger_timer_running = FALSE;
			return;
		} else
	...
	if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx)) {
		...
		if (!ad->ApCfg.bss_color_cfg.bss_color_next) {
			...
			ad->ApCfg.bss_color_cfg.bss_color_next = next_color;
			ad->ApCfg.bss_color_cfg.bss_color_enable = next_color;
			bss_color->next_color = next_color;
		} else {
			MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_INFO,
				"Update color to MBSS(%d):%d\n",
					wdev->wdev_idx, ad->ApCfg.bss_color_cfg.bss_color_next);
			bss_color->next_color = ad->ApCfg.bss_color_cfg.bss_color_next;
		}
	} else {
...
static void send_event_report(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev)
{
	...
	MlmeEnqueueWithWdev(ad, ACTION_STATE_MACHINE, MT2_MLME_WNM_EVT_REPORT,
			...
			(PVOID)&event_report, 0, wdev, FALSE, NULL);
...
void show_bss_color_info(struct _RTMP_ADAPTER *ad)
{
	...
	for (idx = 0; idx < WDEV_NUM_MAX; idx++) {
		...
		if (wdev->if_up_down_state &&
			...
			MTWF_PRINT("%8d %4d %3d 0x%02x %4d %9d %7d 0x%02x%02x%02x%02x%02x%02x%02x%02x\n",
					 idx, wdev->wdev_type,
...
void set_bss_color_dbg(struct _RTMP_ADAPTER *ad, UINT8 wdev_idx, UINT8 action, UINT8 value)
{
	...
	if (wdev_idx >= ARRAY_SIZE(ad->wdev_list))
		return;
	...
	if (wdev->if_up_down_state &&
		...
		case BSS_COLOR_DBG_ASSIGN_MANUAL:
			...
			bcolor_release_entry(ad, bss_color->color);
			bcolor_occupy_entry(ad, value);
		...
		case BSS_COLOR_DBG_DISABLE:
#ifdef CONFIG_AP_SUPPORT
			if (wdev->wdev_type != WDEV_TYPE_AP)
				break;

			/* ignore it if desired disable value is same as current setting */
			if (bss_color->disabled == value)
				break;

			if (value == 0 || value == 1)
				set_bss_color_disabled(wdev, BSS_COLOR_OPMODE_BAND, value);
#endif
			break;

		case BSS_COLOR_DBG_IGNORE_COLLISION:
#ifdef CONFIG_AP_SUPPORT
			if (wdev->wdev_type != WDEV_TYPE_AP)
				break;

			if (value == 0 || value == 1)
				ad->ApCfg.bss_color_cfg.bss_color_collision_ignore = value;
#endif
			break;
		...
		MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_ERROR,
			"BSS color is not applied for this wdev\n");
	}
}
#ifdef CONFIG_AP_SUPPORT
NDIS_STATUS bss_color_profile_enable(
	IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING * buffer)
{
	UINT8 value = 0;

	value = (UINT8)simple_strtol(buffer, 0, 10);

	MTWF_PRINT("%s: value = %d\n", __func__, value);
	if (value == BSS_COLOR_DISABLE)
		pAd->ApCfg.bss_color_cfg.bss_color_enable = BSS_COLOR_DISABLE;
	else if (value == BSS_COLOR_VALUE_AUTO_INIT)
		pAd->ApCfg.bss_color_cfg.bss_color_enable = BSS_COLOR_VALUE_AUTO_INIT;
	else if (value == BSS_COLOR_VALUE_AUTO_SEL_ALL_MBSS)
		pAd->ApCfg.bss_color_cfg.bss_color_enable = BSS_COLOR_VALUE_AUTO_SEL_ALL_MBSS;
	else {
		if (value >= BSS_COLOR_VALUE_MIN && value <= BSS_COLOR_VALUE_MAX)
			pAd->ApCfg.bss_color_cfg.bss_color_enable = value;
		else {
			pAd->ApCfg.bss_color_cfg.bss_color_enable = BSS_COLOR_VALUE_AUTO_INIT;
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_ERROR,
				"BSS color value %d is not in range 1 >= bss color <=63\n", value);
		}
	}
