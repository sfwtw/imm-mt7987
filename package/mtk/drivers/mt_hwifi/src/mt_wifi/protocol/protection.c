...
{
	struct _BSS_STRUCT *mbss = MBSS_GET(peer->pMbss);

	if ((peer->MaxHTPhyMode.field.MODE == MODE_CCK) && (peer->Sst == SST_ASSOC)) {
		if (peer_state == PEER_JOIN)
			mbss->conn_sta.nonerp_sta_cnt++;

		if ((peer_state == PEER_LEAVE) && (mbss->conn_sta.nonerp_sta_cnt > 0))
			mbss->conn_sta.nonerp_sta_cnt--;
	}

	return mbss->conn_sta.nonerp_sta_cnt;
}
