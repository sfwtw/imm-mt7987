...
wed_dma_ctrl(struct wed_entry *wed, u8 txrx)
{
	...
	warp_io_read32(wed, WED_COPY_DMA_CFG_ADDR, &wed_copy_dma_cfg);
	...
	case WARP_DMA_RX:
		...
		wed_copy_dma_cfg |= WED_COPY_DMA_CFG_enable_MASK;
		...
		warp_io_write32(wed, WED_COPY_DMA_CFG_ADDR, wed_copy_dma_cfg);
		warp_io_write32(wed, WED_GLO_CFG_ADDR, wed_cfg);
		warp_io_write32(wed, WED_WPDMA_GLO_CFG_ADDR, wed_wpdma_cfg);
#ifdef WED_HW_TX_SUPPORT
		warp_io_write32(wed, WED_WDMA_GLO_CFG_ADDR, wed_wdma_cfg);
...
#ifndef WED_ROLLBACK_SUPPORT
	value &= ~WED_CTRL_WDMA_INT_AGT1_EN_MASK;
#endif
...
#ifdef WED_RX_HW_RRO_2_0
static int reset_wo_state(struct wed_entry *wed)
{
	struct warp_entry *warp = wed->warp;
	u32 wo_heartbeat = 0;
	u8 check_wo_heart_cnt = 5;
	u8 i = 0;
	...
	/* check heartbeat, if heartbeat is not 0 polling to 0.*/
	while (i < check_wo_heart_cnt) {
		mdelay(CHK_DELAY_MS);
...
reset_rx_rro_traffic(struct wed_entry *wed, u32 reset_type)
{
	...
	if (reset_type == WARP_RESET_INTERFACE && reset_wo_state(wed))
		ret = -EIO;
#endif
...
wed_wpdma_inter_rx_init(struct wed_entry *wed, struct wifi_hw *hw)
{
	...
	case 2:
		/*RX1*/
		value |= WED_WPDMA_INT_CTRL_RX_RX_DONE_EN1_MASK;
		value |= WED_WPDMA_INT_CTRL_RX_RX_DONE_CLR1_MASK;
		value |= SET_FIELD(WED_WPDMA_INT_CTRL_RX_RX_DONE_TRIG1,
				hw->wfdma_rx_done_trig1_bit);
		fallthrough;
	case 1:
		/*RX0*/
		value |= WED_WPDMA_INT_CTRL_RX_RX_DONE_EN0_MASK;
		value |= WED_WPDMA_INT_CTRL_RX_RX_DONE_CLR0_MASK;
		value |= SET_FIELD(WED_WPDMA_INT_CTRL_RX_RX_DONE_TRIG0,
...
static void
wed_wdma_inter_init(struct wed_entry *wed,
	struct wdma_entry *wdma, u8 enable, int idx)
{
#ifdef WED_ROLLBACK_SUPPORT
	u32 value = 0;
	...
	if (enable) {
		tx_mask |= tx_ring_ctrl->int_sts_mask;
		rx_mask |= rx_ring_ctrl->int_sts_mask;
	}

	warp_io_write32(wed, WED_WDMA_INT_TRIG_ADDR, tx_mask);
	warp_io_write32(wed, WED_WDMA_INT1_TRIG_ADDR, rx_mask);

	warp_io_read32(wed, WED_WDMA_INT_CLR_ADDR, &tx_mask);
	warp_io_read32(wed, WED_WDMA_INT1_CLR_ADDR, &rx_mask);
	...
	warp_io_write32(wed, WED_WDMA_INT_CLR_ADDR, tx_mask);
	warp_io_write32(wed, WED_WDMA_INT1_CLR_ADDR, rx_mask);
#endif
	...
	u8 enable, u32 pcie_ints_offset, int idx)
{
	...
	/* WED_WDMA */
	wed_wdma_inter_init(wed, wdma, enable, idx);
	/* WDMA */
	wdma_int_ctrl(wdma, enable);
...
#ifdef WED_RX_HW_RRO_2_0
	//Used by WOCPU0
	wed->ext_int_mask1 |= WED_EX_INT_STA_MID_RDY_MASK;
#endif
}
...
warp_wdma_init_hw(struct wed_entry *wed, struct wdma_entry *wdma, int idx)
{
	...
	struct wdma_rx_ctrl *rx_ctrl = &res->rx_ctrl;
	struct warp_ring *tx_ring = tx_ctrl->tx_ring_ctrl.ring;
	struct warp_ring *rx_ring = rx_ctrl->rx_ring_ctrl.ring;
	u32 value;
	u32 wed_wdma_cfg, wed_wdma_pref;

	/* WDMA base physical address */
	warp_io_write32(wed, WED_WDMA_CFG_BASE_ADDR, tx_ctrl->base_phy_addr);
#ifndef WED_ROLLBACK_SUPPORT
	warp_io_write32(wed, WED_WDMA_CFG_BASE1_ADDR, rx_ctrl->base_phy_addr);
#endif

	/* WDMA GLO_CFG and INT_STA offset */
	value = SET_FIELD(WED_WDMA_OFST0_GLO_CFG, tx_ctrl->glo_cfg_offset);
	value |= SET_FIELD(WED_WDMA_OFST0_INTS, tx_ctrl->int_sts_offset);
	warp_io_write32(wed, WED_WDMA_OFST0_ADDR, value);

	/* WDMA RX0 and TX0 offset */
	value = SET_FIELD(WED_WDMA_OFST1_RX0_CTRL, rx_ring->hw_desc_base);
	value |= SET_FIELD(WED_WDMA_OFST1_TX0_CTRL, tx_ring->hw_desc_base);
	warp_io_write32(wed, WED_WDMA_OFST1_ADDR, value);
...
#ifndef WED_ROLLBACK_SUPPORT
	/* Separate Tx/Rx */
	wed_wdma_cfg |= WED_WDMA_GLO_CFG_TX_RX_SEP_MASK;
#endif
	warp_io_write32(wed, WED_WDMA_GLO_CFG_ADDR, wed_wdma_cfg);
...
#ifdef WED_HW_TX_SUPPORT
static void
warp_wdma_rx_ring_hw(struct wed_entry *wed, struct wdma_entry *wdma)
{
	struct wdma_rx_ring_ctrl *rx_ring_ctrl = &wdma->res_ctrl.rx_ctrl.rx_ring_ctrl;
	...
	for (i = 0; i < rx_ring_ctrl->ring_num; i++) {
		...
		/* WDMA */
		wdma_rx_ring_init_hw(wdma, rx_ring_ctrl, i);
...
#ifdef WED_HW_RX_SUPPORT
static void
warp_wdma_tx_ring_hw(struct wed_entry *wed, struct wdma_entry *wdma)
{
	struct wdma_tx_ring_ctrl *tx_ring_ctrl = &wdma->res_ctrl.tx_ctrl.tx_ring_ctrl;
	...
	/*WDMA*/
	wdma_tx_ring_init_hw(wdma, tx_ring_ctrl, 0);
...
{
#ifdef WED_HW_TX_SUPPORT
	warp_wdma_rx_ring_hw(wed, wdma);
#endif
#ifdef WED_HW_RX_SUPPORT
	warp_wdma_tx_ring_hw(wed, wdma);
#endif
}
...
#ifdef WED_BMU_SUPPORT
int
warp_rx_tkid_init_hw(struct wed_entry *wed)
{
	...
	value = WED_MOD_RST_RX_TKID_MASK;
	WHNAT_RESET(wed, WED_MOD_RST_ADDR, value);

	value = SET_FIELD(WED_RX_TKID_BASE_ADDRESS, desc->alloc_pa);
	warp_io_write32(wed, WED_RX_TKID_BASE_ADDR, value);

#ifdef CONFIG_WARP_64BIT_SUPPORT
	value = SET_FIELD(WED_RX_TKID_BASE_H_ADDRESS, desc->alloc_pa >> 32);
	warp_io_write32(wed, WED_RX_TKID_BASE_H_ADDR, value);
#endif

	value = SET_FIELD(WED_RX_TKID_CTRL_VLD_GRP_NUM, rx_tkid->tkn_vld_grp);
	warp_io_write32(wed, WED_RX_TKID_CTRL_ADDR, value);

	warp_io_read32(wed, WED_CTRL_ADDR, &value);
	value |= WED_CTRL_WED_RX_TKID_EN_MASK;
	warp_io_write32(wed, WED_CTRL_ADDR, value);
...
warp_rx_data_ring_init_hw(struct wed_entry *wed, struct wifi_entry *wifi)
{
	...
	for (i = 0; i < ring_ctrl->ring_num; i++) {
		...
		/* WED_WFDMA */
		warp_io_write32(wed, WED_WPDMA_RX_D_RX0_BASE_ADDR + i * ring_offset,
			ring->cell[0].alloc_pa);
		warp_io_write32(wed, WED_WPDMA_RX_D_RX0_CNT_ADDR + i * ring_offset,
			ring->ring_lens);
	}

	/* Reset Index of Ring */
	value = WED_WPDMA_RX_D_RST_IDX_CRX_IDX0_MASK;
	value |= WED_WPDMA_RX_D_RST_IDX_CRX_IDX1_MASK;
	value |= WED_WPDMA_RX_D_RST_IDX_DRV_IDX0_MASK;
	value |= WED_WPDMA_RX_D_RST_IDX_DRV_IDX1_MASK;
	warp_io_write32(wed, WED_WPDMA_RX_D_RST_IDX_ADDR, value);
	warp_io_write32(wed, WED_WPDMA_RX_D_RST_IDX_ADDR, 0);

	/* Reset Prefetch index of Ring */
	warp_io_read32(wed, WED_WPDMA_RX_D_PREF_RX0_SIDX_ADDR, &value);
	value |= WED_WPDMA_RX_D_PREF_RX0_SIDX_IDX_CLR_MASK;
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_RX0_SIDX_ADDR, value);
	value &= ~(WED_WPDMA_RX_D_PREF_RX0_SIDX_IDX_CLR_MASK);
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_RX0_SIDX_ADDR, value);

	warp_io_read32(wed, WED_WPDMA_RX_D_PREF_RX1_SIDX_ADDR, &value);
	value |= WED_WPDMA_RX_D_PREF_RX1_SIDX_IDX_CLR_MASK;
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_RX1_SIDX_ADDR, value);
	value &= ~(WED_WPDMA_RX_D_PREF_RX1_SIDX_IDX_CLR_MASK);
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_RX1_SIDX_ADDR, value);

	/* Reset Prefetch FIFO of Ring */
	value = WED_WPDMA_RX_D_PREF_FIFO_CFG_RING0_CLEAR_MASK;
	value |= WED_WPDMA_RX_D_PREF_FIFO_CFG_RING1_CLEAR_MASK;
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_FIFO_CFG_ADDR, value);
	warp_io_write32(wed, WED_WPDMA_RX_D_PREF_FIFO_CFG_ADDR, 0);
...
warp_rx_bm_init_hw(struct wed_entry *wed, struct wifi_entry *wifi)
{
...
#else
	warp_rx_tkid_init_hw(wed);

	value = SET_FIELD(WED_RX_BM_RX_DMAD_SDL0, hw->rx_pkt_size);
	value &= ~(WED_RX_BM_RX_DMAD_BUF_TYPE_MASK |
		   WED_RX_BM_RX_DMAD_IS_TX_DONE_MASK);
	warp_io_write32(wed, WED_RX_BM_RX_DMAD_ADDR, value);
#endif
	...
	value |= WED_CTRL_WED_RX_BM_EN_MASK;
#ifndef WED_BMU_SUPPORT
	/* Set RXBM get buffer from internal pool */
	value |= WED_CTRL_RX_BM_MODULE_SEL_MASK;
#endif
	warp_io_write32(wed, WED_CTRL_ADDR, value);
...
warp_rx_rro_init_hw(struct wed_entry *wed, struct wifi_entry *wifi)
{
	...
	value = 0;
	//unit: DW
	value |= SET_FIELD(WED_RROQM_MIOD_CFG_MID_DW, rro_ctrl->mid_size >> 2);
	value |= SET_FIELD(WED_RROQM_MIOD_CFG_MOD_DW, rro_ctrl->mod_size >> 2);
	value |= SET_FIELD(WED_RROQM_MIOD_CFG_MIOD_ENTRY_DW, rro_ctrl->miod_entry_size >> 2);
	warp_io_write32(wed, WED_RROQM_MIOD_CFG_ADDR, value);

	/* Specify the MID/MOD Ring */
	warp_io_read32(wed, WED_RROQM_MIOD_CTRL0_ADDR, &value);
	value |= SET_FIELD(WED_RROQM_MIOD_CTRL0_BASE_PTR, rro_ctrl->miod_desc_base_pa);
	warp_io_write32(wed, WED_RROQM_MIOD_CTRL0_ADDR, value);

	warp_io_read32(wed, WED_RROQM_MIOD_CTRL1_ADDR, &value);
	value |= SET_FIELD(WED_RROQM_MIOD_CTRL1_MAX_CNT, rro_ctrl->miod_cnt);
	warp_io_write32(wed, WED_RROQM_MIOD_CTRL1_ADDR, value);

	/* Specify the Feedback Command Ring */
	warp_io_read32(wed, WED_RROQM_FDBK_CTRL0_ADDR, &value);
	value |= SET_FIELD(WED_RROQM_FDBK_CTRL0_BASE_PTR, rro_ctrl->fdbk_desc_base_pa);
	warp_io_write32(wed, WED_RROQM_FDBK_CTRL0_ADDR, value);

	warp_io_read32(wed, WED_RROQM_FDBK_CTRL1_ADDR, &value);
	value |= SET_FIELD(WED_RROQM_FDBK_CTRL1_MAX_CNT, rro_ctrl->fdbk_cnt);
	warp_io_write32(wed, WED_RROQM_FDBK_CTRL1_ADDR, value);

	warp_io_write32(wed, WED_RROQM_FDBK_CTRL2_ADDR, 0);

	/* Specify the RRO Queue */
	warp_io_read32(wed, WED_RROQ_BASE_L_ADDR, &value);
	value |= SET_FIELD(WED_RROQ_BASE_L_PTR, rro_ctrl->rro_que_base_pa);
	warp_io_write32(wed, WED_RROQ_BASE_L_ADDR, value);

	/* Reset the Index of Rings */
	warp_io_read32(wed, WED_RROQM_RST_IDX_ADDR, &value);
	value |= WED_RROQM_RST_IDX_FDBK_MASK;
	value |= WED_RROQM_RST_IDX_MIOD_MASK;
	warp_io_write32(wed, WED_RROQM_RST_IDX_ADDR, value);

	warp_io_write32(wed, WED_RROQM_RST_IDX_ADDR, 0);

	warp_io_write32(wed, WED_RROQM_MIOD_CTRL2_ADDR, rro_ctrl->miod_cnt - 1);

	warp_io_read32(wed, WED_CTRL_ADDR, &value);
	value |= WED_CTRL_WED_RX_RRO_QM_EN_MASK;
	warp_io_write32(wed, WED_CTRL_ADDR, value);
	return 0;
}
...
warp_reset_hw(struct wed_entry *wed, u32 reset_type)
{
	...
	case WARP_RESET_INTERFACE:
		...
		/* Reset write-once protection, need reset whole WED */
		WHNAT_RESET(wed, WED_MOD_RST_ADDR, WED_MOD_RST_WED_MASK);
		break;
	default:
...
warp_wifi_set_hw(struct wed_entry *wed, struct wifi_entry *wifi)
{
	...
	if (wifi_cap_rro_3_0 || wifi_cap_rro_3_1) {
		/* GLO_CFG */
		warp_io_write32(wed, WED_WPDMA_RRO3_1_RX_D_GLO_CFG_BASE_ADDR,
...
warp_procinfo_dump_txinfo_hw(struct warp_entry *warp, struct seq_file *output)
{
	...
	/* WDMA */
	dump_wdma_rx_ring_hw(wdma, output);
...
warp_procinfo_dump_rxinfo_hw(struct warp_entry *warp, struct seq_file *output)
{
	...
	/*WDMA*/
	dump_wdma_tx_ring_hw(wdma, output);
			 ...
			 u8 idx)
{
	...
	ring->hw_desc_base = wifi_cr_get(warp, WED_RX_BASE_PTR_0_ADDR + offset);
	ring->hw_cnt_addr = wifi_cr_get(warp, WED_RX_MAX_CNT_0_ADDR + offset);
	ring->hw_cidx_addr = wifi_cr_get(warp, WED_RX_CRX_IDX_0_ADDR + offset);
	ring->hw_didx_addr = wifi_cr_get(warp, WED_RX_DRX_IDX_0_ADDR + offset);
}
...
void warp_woif_bus_init_hw(struct woif_bus *bus)
{
	...
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_desc_base, ring->cell[0].alloc_pa);
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_cnt_addr, bus_to_tx_ring_len(bus));
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_cidx_addr, 0);
	...
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_desc_base, ring->cell[0].alloc_pa);
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_cnt_addr, bus_to_rx_ring_len(bus));
	warp_io_write32((struct warp_io *) bus_hw, ring->hw_cidx_addr, 0);
}
...
u32 warp_woif_bus_get_tx_res(struct woif_bus *bus)
{
	...
	warp_io_read32((struct warp_io *) bus_hw, tx_ring->hw_cidx_addr, &cidx);
	warp_io_read32((struct warp_io *) bus_hw, tx_ring->hw_didx_addr, &didx);
...
void warp_woif_bus_get_rx_res(struct woif_bus *bus, u32 *didx, u32 *cidx)
{
	...
	warp_io_read32((struct warp_io *) bus_hw, rx_ring->hw_didx_addr, didx);
	warp_io_read32((struct warp_io *) bus_hw, rx_ring->hw_cidx_addr, cidx);
}
...
void warp_woif_bus_set_rx_res(struct woif_bus *bus, u32 cidx)
{
	...
	warp_io_write32((struct warp_io *) bus_hw, rx_ring->hw_cidx_addr, cidx);
}
...
void warp_woif_bus_kickout(struct woif_bus *bus, int cpu_idx)
{
	...
	warp_io_write32((struct warp_io *) bus_hw, tx_ring->hw_cidx_addr, cpu_idx);
