...
struct _MURU_MDVT_PARAM_INFO_T {
	enum _ENUM_MURU_SET_MDVT_INFO_T eItemId;
...
const struct _MURU_MDVT_PARAM_INFO_T arMuruMdvtParamUpdateTbl[] = {
	{MDVT_RLM_SET_USR_MCS,		"mcs"},
	{MDVT_RLM_SET_USR_NSTS,		"nsts"},
	{MDVT_RLM_SET_USR_RU_ALLOC,	"rualloc"},
	{MDVT_RLM_SET_UL_LEN,		"ullen"},
	{MDVT_RLM_SET_START_SS,		"startss"},
	{MDVT_RLM_SET_MU_MIMO_SPATIAL,	"mumimospatial"},
	{MDVT_RLM_SET_FIX_BAND,         "fixband"},
	{MDVT_RLM_SET_MLO_EN_CTRL,      "mloen"},
	{MDVT_RLM_SET_ICS_TO_HOST,      "icstohost"},
	{MDVT_RLM_SET_ADD_DEV_FRM_ATE,  "adddev"},
	{MDVT_RLM_SET_REAL_STA_CNT,     "realstacnt"},
	{MDVT_RLM_SET_SUBCASE_RANGE,    "subcase"},
	{MDVT_RLM_SET_STA_CNT,          "stacnt"},
	{MDVT_RLM_CLR_FIX_CMM,          "clrfixcmm"},
	{MDVT_RLM_SET_STEP_RUN,         "steprun"},
	{MDVT_RLM_SET_RECAL_BW,         "recalbw"},
	{MDVT_RLM_SET_BF_TXOP_SND,      "bf_txop_snd"},
	{MDVT_RLM_SET_BF_PROTECT_EN,    "bf_prot_en"},
	{MDVT_RLM_SET_BF_DIRECT_FBK_EN, "bf_dfbk"},
	{MDVT_RLM_SET_BF_SEG_EN,        "bf_seg_en"},
	{MDVT_RLM_SET_BF_DATA_TYPE,     "bf_data_type"},

};
size_t u4MuruMdvtParamUpdateTblSize = sizeof(arMuruMdvtParamUpdateTbl) / sizeof(struct _MURU_MDVT_PARAM_INFO_T);
...
error:
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, Ret ? DBG_LVL_INFO : DBG_LVL_ERROR, ":Ret = %d\n", Ret);
	return Ret;
}
...
INT SetMuruAlgoDbgCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, arg, UNI_CMD_MURU_SET_ALGO_DBG_CTRL);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 MtCmdCr4QoSSet(RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 u4ExtSize, PUINT8 pExtData)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_INFO, ":ret = %d\n", Ret);
	return Ret;
}
...
BOOLEAN SendQoSCmd(RTMP_ADAPTER *pAd, UINT32 op_flag, MURU_QOS_SETTING *pqos_setting)
{
	...
	rQosCfg.u4OpFlag = op_flag;
	if (pqos_setting)
		memcpy(&rQosCfg.QoSSetting, pqos_setting, sizeof(MURU_QOS_SETTING));
	if (cap->uni_cmd_support)
		ret = UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&rQosCfg, UNI_CMD_MURU_SET_QOS_CFG);
	else
		ret = FALSE;
#else
	...
	if (pqos_setting) {
		if (op_flag == QOS_CMD_PARAM_SETTING)
			MTWF_PRINT("Set STA%d AC%d,DelayBound=%u,DelayReq=%u Weight=%u,Rate=%u,BWReq=%u,Dir=%u,Dropth=%u\n",
				...
				pqos_setting->u4DataRate, pqos_setting->u2BWReq,
				pqos_setting->u1Dir,
...
INT SetMuruQoSCfg(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	switch (op) {
		...
		case 1:
			...
			qos_setting.u4DataRate = value[5];
...
#ifdef WIFI_UNIFIED_COMMAND
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&u1TxcTxEnable, UNI_CMD_MURU_SET_TXC_TX_STATS_EN);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT SetMuruStatisticConfig(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	CMD_MURU_STAT_RECORD_CTRL ctrl = {0};
		...
		if (cap->uni_cmd_support)
			return UniCmdMuruParameterSet(pAd, arg, UNI_CMD_MURU_SET_STATISTIC_CONFIG);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 set_muru_cert_bsrp_per_txop(RTMP_ADAPTER *ad, UINT8 flag)
{
	INT32 ret = TRUE;
	UINT32 cmd = MURU_SET_BSRP_PER_TXOP;
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
#ifdef WIFI_UNIFIED_COMMAND
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&flag, UNI_CMD_MURU_SET_BSRP_PER_TXOP);
#endif /* WIFI_UNIFIED_COMMAND */

	/* Allocate memory for msg */
	msg = AndesAllocCmdMsg(ad, sizeof(cmd) + sizeof(flag));
	if (!msg) {
		ret = 0;
		goto error;
	}
...
#endif
	AndesAppendCmdMsg(msg, (char *)&cmd, sizeof(cmd));
	AndesAppendCmdMsg(msg, (char *)&flag, sizeof(flag));
	AndesSendCmdMsg(ad, msg);
error:
	MTWF_DBG(ad, DBG_CAT_HW, CATHW_PHY, ret ? DBG_LVL_INFO : DBG_LVL_ERROR, ":ret = %d\n", ret);
...
INT32 set_muru_ignore_nav(RTMP_ADAPTER *ad, UINT8 ignore)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&ignore, UNI_CMD_MURU_SET_IGNORE_NAV);
#endif /* WIFI_UNIFIED_COMMAND */
...
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP * cap = hc_get_chip_cap(ad->hdev_ctrl);
	struct UNI_CMD_MURU_CERT_FRAME_CTRL rMuruCertFrameCtrl;

	rMuruCertFrameCtrl.u4PpduDur = ppdu_dur;
	rMuruCertFrameCtrl.u2TargetWcid = target_wcid;
	rMuruCertFrameCtrl.u1Interval = interval;
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&rMuruCertFrameCtrl, UNI_CMD_MURU_CERT_SEND_FRAME_CTRL);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT ShowMuruTxcTxStats(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterGet(pAd, arg, UNI_CMD_MURU_GET_TXC_TX_STATS);
#endif /* WIFI_UNIFIED_COMMAND */
...
#ifdef WIFI_UNIFIED_COMMAND
	if (cap->uni_cmd_support) {
		if (cmd == MU_MIMO_GET_GROUP_TBL_ENTRY)
			return UniCmdMuruParameterGet(pAd, (RTMP_STRING *)&index, UNI_CMD_MURU_GET_GROUP_TBL_ENTRY);
	}
#endif /* WIFI_UNIFIED_COMMAND */
...
{
	INT32 Ret = TRUE;
	P_EVENT_MURU_GET_SPL_CNT *presult = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	UINT32 cmd = MURU_GET_SPL_CNT;
	struct cmd_msg *msg = NULL;
	os_alloc_mem(pAd, (UCHAR **)&presult, sizeof(EVENT_MURU_GET_SPL_CNT));
	if (!presult) {
		Ret = 0;
		goto error;
	}
	...
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_QUERY_AND_WAIT_RSP);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, sizeof(EVENT_MURU_GET_SPL_CNT));
...
INT ShowMuruUlRuStatus(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, arg, UNI_CMD_MURU_GET_UL_RU_STATUS);
#endif /* WIFI_UNIFIED_COMMAND */
	os_alloc_mem(pAd, (UCHAR **)&presult, sizeof(EVENT_MURU_GET_SPL_CNT));
	...
	char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	if (rsp_payload == NULL) {
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, ": rsp_payload is null!!\n");
		return;
	}
	...
	char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	if (msg == NULL) {
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, ": msg is null!!\n");
		return;
	}
...
VOID muru_get_ul_tx_cnt_handler(struct cmd_msg *msg,
	char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	for (idx = 0; idx <= pEntry->u1StaCnt &&
		idx < ARRAY_SIZE(pEntry->u4TotSentPktCnt) &&
		idx < ARRAY_SIZE(pEntry->u4TotOkCnt); idx++) {
	...
	char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	PRTMP_ADAPTER mac_ad = NULL;

	pEvent = (P_EVENT_MURU_TUNE_AP_MUEDCA)rsp_payload;

	mac_ad = physical_device_get_mac_adapter_by_band(pAd->physical_dev, pEvent->u1BandIdx);

	if (mac_ad == NULL) {
		MTWF_PRINT("%s band %d pAd NULL ....error\n",
				__func__, pEvent->u1BandIdx);
		return;
	}

	timer_value = pEvent->u2MuEdcaSetting;

	if (mac_ad->CommonCfg.bShowMuEdcaParam == TRUE) {
		...
		MTWF_DBG(mac_ad, DBG_CAT_HW, CATHW_PHY, DBG_LVL_INFO,
				"EVENT_MURU_TUNE_AP_MUEDCA setting %d tune Band %d BCN MU_EDCA timer to %d\n",
				pEvent->u2MuEdcaSetting, pEvent->u1BandIdx, timer_value);
	}

	for (BssIdx = 0; BssIdx < mac_ad->ApCfg.BssidNum; BssIdx++) {
		wdev = &mac_ad->ApCfg.MBSSID[BssIdx].wdev;
		if (wdev->bAllowBeaconing && (HcGetBandByWdev(wdev) == pEvent->u1BandIdx)) {
			...
			BOOLEAN update_bcn = FALSE;

			if (mu_edca == NULL) {
				MTWF_DBG(mac_ad, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR,
					":get mu_edca fail\n");
				return;
			}

			if (bcn_bpcc_op_lock(mac_ad, wdev, TRUE, BCN_BPCC_MUEDCA) == FALSE) {
				if (in_interrupt()) {
					/* defer it to workqueue */
					struct _BCN_MUEDCA_UPDATE_PARAM update_param = {0};

					update_param.MUEDCAUpdateReason = RSN_AP_MUEDCA;
					update_param.value = timer_value;
					bcn_bpcc_ct_switch(mac_ad, wdev, BCN_BPCC_MUEDCA, BCN_REASON(BCN_UPDATE_IE_CHG), update_param);
					return;
				} else
					MTWF_DBG(mac_ad, DBG_CAT_AP, CATAP_BPCC, DBG_LVL_INFO,
						"%s: MUEDCA bcn_bpcc_op_lock fail!\n", __func__);
			}

			if ((mu_edca->mu_ac_rec[ACI_AC_BE].mu_edca_timer != timer_value) ||
			...
			if (update_bcn == TRUE) {
				...
				mu_edca->mu_ac_rec[ACI_AC_VI].mu_edca_timer = timer_value;
				mu_edca->mu_ac_rec[ACI_AC_VO].mu_edca_timer = timer_value;
				UpdateBeaconHandler_BPCC(mac_ad, wdev, BCN_REASON(BCN_UPDATE_IE_CHG),
					BCN_BPCC_MUEDCA, TRUE);
			} else
				bcn_bpcc_op_unlock(mac_ad, wdev, TRUE);
		}
	}
}
	...
	char *rsp_payload, UINT16 rsp_payload_len)
{
    ...
    for (idx = 0; idx < MURU_MAX_STA_CNT_PER_EVENT; idx++) {
		...
		if (prRecord->u1TrigRxMode >= MODE_HE) {
			...
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, "Trigger:: No data\n");
		}
...
VOID muru_show_group_tbl_entry(struct cmd_msg *msg,
	char *rsp_payload, UINT16 rsp_payload_len)
{
	P_MUCOP_GROUP_TBL_ENTRY_T pEvent = NULL;

	pEvent = (P_MUCOP_GROUP_TBL_ENTRY_T)rsp_payload;

	MTWF_PRINT("%s\n", __func__);

	MTWF_PRINT("EntryIdx:%u Num:%u GI:%u:%u Ax:%u DlVld:%u UlVld:%u RuAlloc:%u\n",
			pEvent->u4EntryIdx, pEvent->u1NumUser, pEvent->u1DlGi,
			pEvent->u1UlGi, pEvent->u1Ax, pEvent->u1DlVld,
			pEvent->u1UlVld, pEvent->u1RuAlloc);

	MTWF_PRINT("User 0:: DW:%u PFID:%u Nss:%u DL-MCS:%u DL-WF:%u UL-MCS:%u UL-WF:%u\n",
			pEvent->DW0, pEvent->u1PFIDUser0, pEvent->u1NssUser0,
			pEvent->u1DlMcsUser0, pEvent->u1DlWfUser0,
			pEvent->u1UlMcsUser0, pEvent->u1UlWfUser0);

	if (pEvent->u1NumUser)
		MTWF_PRINT("User 1:: DW:%u PFID:%u Nss:%u DL-MCS:%u DL-WF:%u UL-MCS:%u UL-WF:%u\n",
				pEvent->DW1, pEvent->u1PFIDUser1, pEvent->u1NssUser1,
				pEvent->u1DlMcsUser1, pEvent->u1DlWfUser1, pEvent->u1UlMcsUser1,
				pEvent->u1UlWfUser1);

	if (pEvent->u1NumUser > 1)
		MTWF_PRINT("User 2:: DW:%u PFID:%u Nss:%u DL-MCS:%u DL-WF:%u UL-MCS:%u UL-WF:%u\n",
				pEvent->DW2, pEvent->u1PFIDUser2, pEvent->u1NssUser2,
				pEvent->u1DlMcsUser2, pEvent->u1DlWfUser2, pEvent->u1UlMcsUser2,
				pEvent->u1UlWfUser2);

	if (pEvent->u1NumUser > 2)
		MTWF_PRINT("User 3:: DW:%u PFID:%u Nss:%u DL-MCS:%u DL-WF:%u UL-MCS:%u UL-WF:%u\n",
				pEvent->DW3, pEvent->u1PFIDUser3, pEvent->u1NssUser3,
				pEvent->u1DlMcsUser3, pEvent->u1DlWfUser3, pEvent->u1UlMcsUser3,
				pEvent->u1UlWfUser3);
}
...
#ifdef WIFI_UNIFIED_COMMAND
static struct UNI_CMD_TBRX_CFG_INTERFACER grUniCmdTBRXCfgInf;
#endif
...
)
{
	...
	if (strcmp("comm_toneplan", type) == 0) {
		...
		for (loop_idx = 0 ; loop_idx < loop_cnt ; loop_idx++) {
			...
			pch = strsep(&val, ":");
			if (pch != NULL) {
				if ((loop_idx % 5) == 4 && c26_idx < ARRAY_SIZE(pCfgDl->au1C26)) {
					pCfgDl->au1C26[c26_idx] = (UINT8)os_str_tol(pch, 0, 10);
					c26_idx++;
				} else if (ru_idx < ARRAY_SIZE(pCfgDl->au1RU)) {
...
error:
#endif
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, status ? DBG_LVL_INFO : DBG_LVL_ERROR, ":status = %d\n", status);
...
)
{
	...
	if (strcmp("dl_user_wlan_idx", type) == 0) {
		...
		if (MURU_MANUAL_CFG_CHK(*pCfgBmpDl, MURU_FIXED_TOTAL_USER_CNT)) {
			...
			for (loop_idx = 0; loop_idx < loop_cnt; loop_idx++) {
				pch = strsep(&val, ":");
				if (pch != NULL && loop_idx < ARRAY_SIZE(pCfgDl->arUserInfoDl)) {
	...
	if (strcmp("ul_comm_ta", type) == 0) {
		...
		for (loop_idx = 0 ; loop_idx < MAC_ADDR_LEN ; loop_idx++) {
			...
			if (pch != NULL && loop_idx < ARRAY_SIZE(pCfgUl->u1TrigTa)) {
				pCfgUl->u1TrigTa[loop_idx] = (UINT8)os_str_tol(pch, 0, 16);
			} else {
				status = FALSE;
				goto error;
	...
	if (strcmp("ul_user_wlan_idx", type) == 0) {
		if (MURU_MANUAL_CFG_CHK(*pCfgBmpUl, MURU_FIXED_UL_TOTAL_USER_CNT)) {
			...
			for (loop_idx = 0; loop_idx < loop_cnt; loop_idx++) {
				pch = strsep(&val, ":");
				if (pch != NULL && loop_idx < ARRAY_SIZE(pCfgUl->arUserInfoUl)) {
	...
	if (strcmp("ul_comm_toneplan", type) == 0) {
		...
		if (MURU_MANUAL_CFG_CHK(*pCfgBmpUl, MURU_FIXED_UL_BW)) {
			...
			for (loop_idx = 0 ; loop_idx < loop_cnt ; loop_idx++) {
				...
				pch = strsep(&val, ":");
				if (pch != NULL) {
					if ((loop_idx % 5) == 4 && c26_idx < ARRAY_SIZE(pCfgUl->au1UlC26)) {
						pCfgUl->au1UlC26[c26_idx] = (UINT8)os_str_tol(pch, 0, 10);
						c26_idx++;
					} else if (ru_idx < ARRAY_SIZE(pCfgUl->au1UlRU)) {
...
error:
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, Ret ? DBG_LVL_INFO : DBG_LVL_ERROR,
...
#ifdef WIFI_UNIFIED_COMMAND
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&fgRst, UNI_CMD_MURU_SET_ULTX_CNT_RESET);
#endif /* WIFI_UNIFIED_COMMAND */
...
error:
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, Ret ? DBG_LVL_INFO : DBG_LVL_ERROR, ": CMD %s\n", Ret ? "Success":"Fail");
	return Ret;
}
...
INT32 hqa_muru_get_ul_tx_cnt(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterGet(pAd, arg, UNI_CMD_MURU_GET_ULTX_CNT);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 hqa_muru_set_mu_tx_pkt_en(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret;
#ifdef WIFI_UNIFIED_COMMAND
	Ret = uni_hqa_muru_set_mu_tx_pkt_en(pAd, arg);
#else
	INT8 *value = NULL;
...
INT32 hqa_muru_set_mu_tx_pkt_cnt(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret;
#ifdef WIFI_UNIFIED_COMMAND
	Ret = uni_hqa_muru_set_mu_tx_pkt_cnt(pAd, arg);
#else
	INT8 *value = NULL;
...
INT32 set_muru_trig_variant(RTMP_ADAPTER *ad, UINT8 variant)
{
	INT32 ret = TRUE;
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&variant, UNI_CMD_MURU_SET_TRIG_VARIANT);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 set_muru_cert_muedca_override(RTMP_ADAPTER *ad, UINT8 capi_override)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&override, UNI_CMD_MURU_SET_CERT_MU_EDCA_OVERRIDE);
#endif /* WIFI_UNIFIED_COMMAND */

	MTWF_DBG(ad, DBG_CAT_HW, CATHW_PHY, DBG_LVL_INFO, "override = %d\n", override);
...
INT32 set_pqs_manual_scs(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP * cap = hc_get_chip_cap(pAd->hdev_ctrl);
	if (cap->uni_cmd_support)
		return uni_set_pqs_manual_scs(pAd, arg);
#endif /* WIFI_UNIFIED_COMMAND */

	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR,
				": %s only support UNI_CMD\n", __func__);
	return FALSE;
}

INT32 set_muru_bf_in_acq(RTMP_ADAPTER *ad, BOOL bfinacq_en)
{
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP * cap = hc_get_chip_cap(ad->hdev_ctrl);

	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(ad, (RTMP_STRING *)&bfinacq_en, UNI_CMD_MURU_SET_BF_IN_AC01);
#endif /* WIFI_UNIFIED_COMMAND */
	return FALSE;
}
...
INT32 set_muru_debug_info(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	for (loop_idx = 0; loop_idx < param->rCfgDl.u1UserCnt &&
		loop_idx < ARRAY_SIZE(param->rCfgDl.arUserInfoDl); loop_idx++) {
	...
	for (loop_idx = 0; loop_idx < param->rCfgUl.u1UserCnt &&
		loop_idx < ARRAY_SIZE(param->rCfgUl.arUserInfoUl); loop_idx++) {
...
#ifdef WIFI_UNIFIED_COMMAND
INT32 set_test_mode_tbrx_config(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	INT32 Ret = TRUE;
	UINT_32 user_index;
	char sep_val = ':';
	...
	struct UNI_CMD_TBRX_CFG_INTERFACER *pTBRXCfg = &grUniCmdTBRXCfgInf;

	param_type = arg;
	if (strlen(param_type)) {
		if (strcmp("init", param_type) == 0) {
			os_zero_mem(pTBRXCfg, sizeof(struct UNI_CMD_TBRX_CFG_INTERFACER));
		} else if (strcmp("update", param_type) == 0) {
			MTWF_PRINT("%s: update!!!\n", __func__);
			Ret = UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&grUniCmdTBRXCfgInf, UNI_CMD_TEST_MODE_TBRX_CONFIG);
		} else {
			...
			if (strlen(param_type) && param_val && strlen(param_val)) {
				if (strcmp("user_cnt", param_type) == 0) {
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						pTBRXCfg->u1UserCnt = (UINT_8)os_str_tol(pch, 0, 10);
					} else {
						Ret = FALSE;
						goto error;
					}

					MTWF_PRINT("cmd=user_cnt: %u\n", pTBRXCfg->u1UserCnt);
				} else if (strcmp("csd_cfg", param_type) == 0) {
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						pTBRXCfg->u8CsdCfg = (UINT_64)os_str_tol(pch, 0, 16);
					} else {
						Ret = FALSE;
						goto error;
					}

					MTWF_PRINT("cmd=csd_cfg: HI:0x%x, LO:0x%x\n",
							(UINT_32)(pTBRXCfg->u8CsdCfg >> 32),
							(UINT_32)(pTBRXCfg->u8CsdCfg & 0xFFFFFFFF));
				} else if (strcmp("common_info", param_type) == 0) {
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						pTBRXCfg->u8CommonInfo = (UINT_64)os_str_tol(pch, 0, 16);
					} else {
						Ret = FALSE;
						goto error;
					}

					MTWF_PRINT("cmd=common_info: HI:0x%x, LO:0x%x\n",
							(UINT_32)(pTBRXCfg->u8CommonInfo >> 32),
							(UINT_32)(pTBRXCfg->u8CommonInfo & 0xFFFFFFFF));
				} else if (strcmp("special_user_info", param_type) == 0) {
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						pTBRXCfg->u8SpecialUserInfo = (UINT_64)os_str_tol(pch, 0, 16);
					} else {
						Ret = FALSE;
						goto error;
					}

					MTWF_PRINT("cmd=special_user_info: HI:0x%x, LO:0x%x\n",
							(UINT_32)(pTBRXCfg->u8SpecialUserInfo >> 32),
							(UINT_32)(pTBRXCfg->u8SpecialUserInfo & 0xFFFFFFFF));
				} else if (strcmp("basic_user_info", param_type) == 0) {
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						user_index = (UINT_32)os_str_tol(pch, 0, 10);
					} else {
						Ret = FALSE;
						goto error;
					}

					if (user_index >= UNI_MURU_MAX_NUM_TXCMD_TX_USER) {
						Ret = FALSE;
						goto error;
					}
					...
					pch = strsep(&param_val, ":");
					if (pch != NULL) {
						pTBRXCfg->au8BasicUserInfo[user_index] = (UINT_64)os_str_tol(pch, 0, 16);
					} else {
						Ret = FALSE;
						goto error;
					}

					MTWF_PRINT("cmd=basic_user_info[%u]: HI:0x%x, LO:0x%x\n",
							user_index,
							(UINT_32)(pTBRXCfg->au8BasicUserInfo[user_index] >> 32),
							(UINT_32)(pTBRXCfg->au8BasicUserInfo[user_index] & 0xFFFFFFFF));
				}
...
)
{
	...
	for (loop_idx = 0; loop_idx < param.rCfgUl.u1UserCnt &&
		loop_idx < ARRAY_SIZE(param.rCfgUl.arUserInfoUl) &&
		loop_idx < ARRAY_SIZE(pMuruManCfg->rCfgUl.arUserInfoUl); loop_idx++) {
...
)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&IsUlTxTrigger, UNI_CMD_MURU_SET_ULTX_TRIGGER);
#endif /* WIFI_UNIFIED_COMMAND */
	...
	if (!msg) {
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, ": msg is NULL\n");
		Ret = FALSE;
		goto error;
	}
...
VOID muru_update_he_cfg(PRTMP_ADAPTER pAd)
{
	...
	MTWF_PRINT("set muru_update_he_cfg()!!!!\n");
...
)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&u1BandIdx, UNI_CMD_MURU_CFG_DLUL_LIMIT);
#endif /* WIFI_UNIFIED_COMMAND */

	param.u1BandIdx     = u1BandIdx;
	param.u1Dis160RuMu  = pAd->CommonCfg.Dis160RuMu;
	param.u1MaxRuOfdma  = pAd->CommonCfg.MaxRuOfdma;
	param.u1MaxDLMuMimo = pAd->CommonCfg.MaxDLMuMimo;
	param.u1MaxULMuMimo = pAd->CommonCfg.MaxULMuMimo;
...
#ifdef WIFI_UNIFIED_COMMAND
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	struct UNI_CMD_MURU_SET_DLUL_VAL_T uni_param;

	os_zero_mem(&uni_param, sizeof(uni_param));
	uni_param.u1BandBssSelect = BandBssSelect;
	uni_param.u1Index = Index;
	uni_param.u1DlUlUpdList = DlUlUpdList;
	uni_param.u1DlUlVal = DlUlVal;

	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&uni_param, UNI_CMD_MURU_SET_DLUL_EN);
#endif /* WIFI_UNIFIED_COMMAND */

	os_zero_mem(&param, sizeof(param));
	param.u1BandBssSelect = BandBssSelect;
	param.u1Index = Index;
...
)
{
	PCHAR pch = NULL;
	INT32 Ret;
	UINT_8 BandBssSelect, Index, DlUlUpdList, DlUlVal;

	pch = strsep(&arg, ":");
	if (pch != NULL) {
		BandBssSelect = os_str_toul(pch, 0, 10);
		/* sanity check for BandBss Select */
		if (BandBssSelect >= MURU_SET_DLUL_MAX) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, ": Invalid BandBss Select :%d !!\n", BandBssSelect);
			Ret = 0;
			goto error;
		}
	} else {
	...
	pch = strsep(&arg, ":");
	if (pch != NULL) {
		Index = os_str_toul(pch, 0, 10);
		if (BandBssSelect == MURU_SET_DLUL_BY_BAND) {
			/* sanity check for Band index */
			if (Index >= CFG_WIFI_RAM_BAND_NUM) {
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR,
	...
	Ret = MtCmdSetMuruCfgDlUlVal(pAd, BandBssSelect, Index, DlUlUpdList, DlUlVal);
...
INT mtk_cfg80211_Set_MuOfdma_Dl_Enable(
	RTMP_ADAPTER * pAd,
	UINT_8 enable
)
{
	...
	if ((pObj->ioctl_if_type == INT_MBSSID) || (pObj->ioctl_if_type == INT_MAIN)) {
		...
		IfIdx = hc_get_bssidx(wdev);
	} else
		return -EFAULT;

	BandBssSelect = MURU_SET_DLUL_BY_BSS;
	Index = IfIdx;
...
INT mtk_cfg80211_Set_MuOfdma_Ul_Enable(
	RTMP_ADAPTER * pAd,
	UINT_8 enable
)
{
...
INT mtk_cfg80211_Set_MuMimo_Dl_Enable(
	RTMP_ADAPTER * pAd,
	UINT_8 enable
)
{
...
INT mtk_cfg80211_Set_MuMimo_Ul_Enable(
	RTMP_ADAPTER * pAd,
	UINT_8 enable
)
{
...
)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, arg, UNI_CMD_MURU_MDVT_INFO);
#endif
	return Ret;
}

INT set_muru_mdvt_param_info(
	PCHAR pch,
	RTMP_STRING *arg,
	UINT_16 *pu2Item,
	UINT_32 u4Val[],
...
)
{
	...
	CHAR *set_val = NULL;
	UINT_8 i, j;
	RTMP_STRING *param_field = NULL;

	if ((pch == NULL) || (arg == NULL)) {
		MTWF_PRINT("%s: Input String NULL\n", __func__);
		return FALSE;
	}

	for (i = 0; i < u4MuruMdvtParamUpdateTblSize; i++) {
		param_field = arMuruMdvtParamUpdateTbl[i].pucParserStr;

		if (strcmp(pch, param_field) == 0) {
			*pu2Item = arMuruMdvtParamUpdateTbl[i].eItemId;
			for (j = 0; j < u1ValSize; j++) {
				set_val = strsep(&arg, "-");
				if (set_val)
					u4Val[j] = os_str_tol(set_val, 0, 10);
				else
					break;
			}
...
#ifdef WIFI_UNIFIED_COMMAND
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	struct UNI_CMD_MURU_SET_AGG_POLICY_T uni_param;

	os_zero_mem(&uni_param, sizeof(uni_param));
	uni_param.u1AggPolicy = agg_policy;
	uni_param.u1DurComp = dur_comp;

	if (cap->uni_cmd_support)
		return UniCmdMuruParameterSet(pAd, (RTMP_STRING *)&uni_param, UNI_CMD_MURU_SET_AGGPOLICY);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 hqa_muru_set_agg_policy(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (arg != NULL) {
		...
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_PHY, DBG_LVL_ERROR, ": Argument is NULL\n");
		Ret = FALSE;
		goto error;
	}
...
)
{
	...
	if (cap->uni_cmd_support)
		return UniCmdMuruParameterGet(pAd, (RTMP_STRING *)&u2Wcid, UNI_CMD_MURU_GET_FW_BLACKLIST_CTRL);
#endif /* WIFI_UNIFIED_COMMAND */
...
INT set_muru_drv_black_list_ctrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret;
#ifdef WIFI_UNIFIED_COMMAND
	Ret = uni_muru_set_drv_black_list_ctrl(pAd, arg);
#else
	PCHAR pch = NULL;
	/* prepare command message */
