	...
	IN		RTMP_ADAPTER * pAd)
{
	...
	for (i = 0; ((i < pAd->ApCfg.BssidNum) && (i < MAX_BEACON_NUM)); i++) {
		...
		if ((op_ext_cha != 0) &&
			...
			MTWF_DBG(NULL, DBG_CAT_AP, CATAP_QBSS, DBG_LVL_INFO,
				"qbss> Alarm! Change to 20 bw...\n");
			/* disassociate stations without D3 2040Coexistence function */
			pMacTable = pAd->MacTab;
			for (StaId = 0; VALID_UCAST_ENTRY_WCID(pAd, StaId); StaId++) {
				MAC_TABLE_ENTRY *pEntry = entry_get(pAd, StaId);
...
#ifdef SW_CONNECT_SUPPORT
				if (hc_is_sw_wcid(pAd, StaId))
					continue;
#endif /* SW_CONNECT_SUPPORT */

				if (pEntry->BSS2040CoexistenceMgmtSupport && (skip_kick == FALSE))
					bDisconnectSta = TRUE;
				...
				if (bDisconnectSta) {
					...
					RTMPSendWirelessEvent(pAd, IW_AGEOUT_EVENT_FLAG, pEntry->Addr, 0, 0);
					{
						...
						if (NStatus != NDIS_STATUS_SUCCESS) {
							MTWF_DBG(NULL, DBG_CAT_AP, CATAP_QBSS,
								DBG_LVL_INFO,
					...
					MTWF_DBG(NULL, DBG_CAT_AP, CATAP_QBSS, DBG_LVL_INFO,
						"qbss> Alarm! Deauth the station "MACSTR"\n",
							 MAC2STR(pEntry->Addr));
#ifdef MAP_R2
...
VOID QBSS_LoadInit(RTMP_ADAPTER *pAd)
{
	...
	for (IdBss = 0; ((IdBss < pAd->ApCfg.BssidNum) && (IdBss < MAX_BEACON_NUM)); IdBss++) {
		if ((pAd->ApCfg.MBSSID[IdBss].wdev.bWmmCapable)
#ifdef DOT11K_RRM_SUPPORT
			|| (IS_RRM_ENABLE(&pAd->ApCfg.MBSSID[IdBss].wdev))
#endif /* DOT11K_RRM_SUPPORT */
			) {
			pQloadCtrl = (struct _QLOAD_CTRL *) HcGetQloadCtrlByRf(pAd, wmode_2_rfic(pAd->ApCfg.MBSSID[IdBss].wdev.PhyMode));
			if (!pQloadCtrl)
				continue;
	...
	UCHAR *pFrame)
{
	...
	if (pQloadCtrl && pQloadCtrl->FlgQloadEnable != 0) {
		...
		if (pMbss->HotSpotCtrl.QLoadTestEnable == 1)
			FrameLen += QBSS_LoadElementAppend_HSTEST(pAd, pFrame+FrameLen, apidx);
		else if (pMbss->HotSpotCtrl.QLoadTestEnable == 0)
#endif
			FrameLen += QBSS_LoadElementAppend(pAd, pFrame+FrameLen, pQloadCtrl, apidx);
	}
...
VOID update_ap_qload_to_bcn(RTMP_ADAPTER *pAd)
{
	...
	if ((pAd->Mlme.OneSecPeriodicRound % pQloadCtrl->QloadChanUtilBeaconInt) == 0) {
		for (BssIdx = 0; ((BssIdx < pAd->ApCfg.BssidNum) && (BssIdx < MAX_BEACON_NUM)); BssIdx++) {
	...
	IN		ULONG			UpTime)
{
	...
	if ((wdev == NULL) || (wdev->pHObj == NULL)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_MISC, DBG_LVL_ERROR, "wdev is NULL\n");
		return;
	}
...
#ifdef DOT11_N_SUPPORT
	{
		...
		if ((oper.ext_cha != 0) &&
			...
			&& (!scan_in_run_state(pAd, NULL))
			&& (pAd->ScanCtrl.state == OFFCHANNEL_SCAN_INVALID)
#endif
			) {
			/* in 20MHz, no need to check busy time of secondary channel */
#ifdef ACS_CTCC_SUPPORT
			if (auto_ch_ctrl->AutoChSelCtrl.AutoChScanStatMachine.CurrState
				== AUTO_CH_SEL_SCAN_IDLE)
#endif
				/*
				 * BusyTimeCcaNavTx will be covered
				 * by OneSecMibBucket.ChannelBusyTimeCcaNavTx[i] below.
				 * BusyTimeCcaNavTx = AsicGetChBusyCnt(pAd, wdev->DevInfo.BandIdx);
				 */
				BusyTimeCcaNavTx = pAd->OneSecMibBucket.ChannelBusyTimeCcaNavTx;
			pQloadCtrl->QloadLatestChannelBusyTimeSec = BusyTimeCcaNavTx;
#ifdef QLOAD_FUNC_BUSY_TIME_STATS
			...
			if (BusyTimeId < ARRAY_SIZE(pQloadCtrl->QloadBusyCountSec))
			...
			if ((pQloadCtrl->FlgQloadAlarmIsSuspended == FALSE) &&
				...
				if ((pQloadCtrl->QloadBusyTimeThreshold != 0) &&
					(BusyTimeCcaNavTx >= pQloadCtrl->QloadBusyTimeThreshold))
	...
	BusyTime = pAd->OneSecMibBucket.ChannelBusyTime;
	BusyTimeCcaNavTx = pAd->OneSecMibBucket.ChannelBusyTimeCcaNavTx;

	pQloadCtrl->QloadLatestChannelBusyTimePri = BusyTimeCcaNavTx;
#ifdef QLOAD_FUNC_BUSY_TIME_STATS
	BusyTimeId = BusyTimeCcaNavTx >> 10; /* translate us to ms */
	/* ex:95ms, 95*20/100 = 19 */
	BusyTimeId = (BusyTimeId*QLOAD_BUSY_INTERVALS)/TimePeriod;
	...
	if (BusyTimeId < ARRAY_SIZE(pQloadCtrl->QloadBusyCountPri))
	...
	if (++pQloadCtrl->QloadChanUtilBeaconCnt >= pQloadCtrl->QloadChanUtilBeaconInt) {
		...
		ChanUtilNu *= 255;
		ChanUtilNuCcaNavTx = pQloadCtrl->QloadChanUtilTotalCcaNavTx;
		...
		pQloadCtrl->QloadChanUtilCcaNavTx = (UINT8)(ChanUtilNuCcaNavTx/ChanUtilDe);
		...
		pQloadCtrl->QloadChanUtilTotalCcaNavTx = 0;
#ifdef OCE_SUPPORT
		if (IS_OCE_ENABLE(wdev) && (!SameChanUtil))
			UpdateBeaconHandler(pAd, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
#endif /* OCE_SUPPORT */
...
{
	QLOAD_CTRL *pQloadCtrl = (struct _QLOAD_CTRL *) hc_get_qload_by_wdev(wdev);
...
INT Show_QoSLoad_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (BusyTimeId = 0; ((BusyTimeId < QLOAD_BUSY_INTERVALS) && (BusyTimeId < ARRAY_SIZE(pQloadCtrl->QloadBusyCountPri)));
		BusyTimeId++) {
		MTWF_PRINT("\t%dms ~ %dms\t\t%d\n",
				 BusyTimeId*Time,
				 (BusyTimeId+1)*Time,
				 pQloadCtrl->QloadBusyCountPri[BusyTimeId]);
	}
	...
	for (BusyTimeId = 0; ((BusyTimeId < QLOAD_BUSY_INTERVALS) && (BusyTimeId < ARRAY_SIZE(pQloadCtrl->QloadBusyCountSec)));
		BusyTimeId++) {
		MTWF_PRINT("\t%dms ~ %dms\t\t%d\n",
				 BusyTimeId*Time,
				 (BusyTimeId+1)*Time,
...
#else
	MTWF_PRINT("\tBusy time statistics is not included into the driver!\n");
#endif /* QLOAD_FUNC_BUSY_TIME_STATS */
	MTWF_PRINT("\n");
	return TRUE;
}
