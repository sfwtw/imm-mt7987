	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "Repeater Cli ASSOC - enqueue APCLI_MT2_ASSOC_TIMEOUT\n");
	pAd = pRepeaterCliEntry->pAd;
	MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
			 "ifIndex = %d, CliIdx = %d !!!\n",
			  pRepeaterCliEntry->wdev.func_idx,
			  pRepeaterCliEntry->MatchLinkIdx);
	MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_ASSOC_TIMEOUT, 0, NULL, 0, wdev, FALSE, NULL);
	RTMP_MLME_HANDLER(pAd);
}
...
VOID RepeaterCtrlInit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "() is done\n");
	NdisReleaseSpinLock(&pAd->ApCfg.ReptCliEntryLock);
}
...
VOID RepeaterCliReset(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "%s() is done\n", __func__);
	...
	OUT UINT16 *pWcid)
{
	...
	if (RTMPRepeaterVaildMacEntry(pAd, pSrcBufVA + MAC_ADDR_LEN, HcGetBandByWdev(cli_link_wdev))
			&& (pApCliEntry->MacTabWCID >= 0) &&  IS_WCID_VALID(pAd, pApCliEntry->MacTabWCID)) {
		...
		if ((tr_entry) &&
			...
			if (pMacEntry && IS_ENTRY_CLIENT(pMacEntry)) {
				STA_TR_ENTRY *sta_tr_entry = NULL;

				if ((pMacEntry->wcid >= 0) && IS_WCID_VALID(pAd, pMacEntry->wcid))
					sta_tr_entry = tr_entry_get(pAd, pMacEntry->wcid);
				...
				if ((mbss_wdev->func_idx >= 0) && (mbss_wdev->func_idx < MAX_BEACON_NUM))
					pMbssToCliLinkMap = &pAd->ApCfg.MbssToCliLinkMap[mbss_wdev->func_idx];

				if (pMbssToCliLinkMap && (pMbssToCliLinkMap->cli_link_wdev->PortSecured != WPA_802_1X_PORT_SECURED
					|| pMbssToCliLinkMap->cli_link_wdev == cli_link_wdev)) {
#ifdef WSC_AP_SUPPORT
					if (!((wsc_control->WscConfMode != WSC_DISABLE) &&
						...
						MTWF_DBG(NULL, DBG_CAT_CLIENT,
							CATCLIENT_APCLI, DBG_LVL_WARN,
	...
	IN UCHAR band_idx)
{
	...
	if (pAd->ApCfg.RepeaterCliSize >= GET_PER_BAND_MAX_REPEATER_ENTRY_NUM(cap))
		return FALSE;
	...
	IN PUCHAR pAddr)
{
	...
	NdisAcquireSpinLock(&pAd->ApCfg.ReptCliEntryLock);
	HashIdx = MAC_ADDR_HASH_INDEX(pAddr);
	if ((idx >= 0) && (idx < MAX_IGNORE_AS_REPEATER_ENTRY_NUM))
		pEntry = &pAd->ApCfg.ReptControl.IgnoreAsRepeaterEntry[idx];
	...
	if ((pAd->ApCfg.bMACRepeaterEn) && (ReptPool != NULL)) {
		for (CliIdx = 0; CliIdx < GET_MAX_REPEATER_ENTRY_NUM(cap); CliIdx++) {
			...
			if (pReptCliEntry->CliEnable && pReptCliEntry->pMacEntry) {
				...
				MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"%s:repeater_entry(%d),p_sec(%d),wcid(%d),En(%d),Vld(%d)\n",
					__func__, pReptCliEntry->CliIdx, tr_entry->PortSecured, Wcid, pReptCliEntry->CliEnable, pReptCliEntry->CliValid);
						...
						if (IS_AKM_SAE_SHA256(pApCliEntry->MlmeAux.AKMMap) || IS_AKM_OWE(pApCliEntry->MlmeAux.AKMMap)) {
							...
							if (
								...
								(IS_AKM_SAE_SHA256(pApCliEntry->MlmeAux.AKMMap) &&
									sae_get_pmk_cache(sae_cfg, pReptCliEntry->CurrentAddress, pApCliEntry->MlmeAux.Bssid, FALSE, pmkid, pmk))
#endif
							) {
...
#ifdef DOT11_SAE_SUPPORT
									SAE_INSTANCE *pSaeIns = search_sae_instance(sae_cfg, pReptCliEntry->CurrentAddress, pApCliEntry->MlmeAux.Bssid);
...
INT Show_Repeater_Cli_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (pEntry &&
			...
			MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"%-6s", get_bw_str(pEntry->HTPhyMode.field.BW, BW_FROM_OID));
			MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
	...
	struct wifi_dev *mbss_link_wdev)
{
	...
	NdisAcquireSpinLock(&pAd->ApCfg.CliLinkMapLock);
	if ((MbssIdx >= 0) && (MbssIdx < MAX_BEACON_NUM)) {
		pMbssToCliLinkMap = &pAd->ApCfg.MbssToCliLinkMap[MbssIdx];
		pMbssToCliLinkMap->mbss_wdev = mbss_link_wdev;
		pMbssToCliLinkMap->cli_link_wdev = cli_link_wdev;
	}
...
BOOLEAN MtRepeaterGetTxRxInfo(RTMP_ADAPTER *pAd, UINT16 wcid, UCHAR CliIfIndex, RETRTXRXINFO *pReptStatis)
{
	...
	if ((wcid != WCID_INVALID) && (!IS_WCID_VALID(pAd, wcid))) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "Invalid wcid!\n");
		return FALSE;
	}
	if ((CliIfIndex > MAX_APCLI_NUM) && (CliIfIndex != 0xf)) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "Invalid Client Index!\n");
		return FALSE;
	}
	if (pReptStatis == NULL) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "NULL Pointer!\n");
		return FALSE;
	}
	NdisZeroMemory(RptrTRInfo, sizeof(RptrTRInfo));
	for (Index = 1; IS_WCID_VALID(pAd, Index); Index++) {
		if ((wcid != WCID_INVALID) && (wcid != Index))
			continue;
		pEntry = entry_get(pAd, Index);
		...
		if (IS_ENTRY_APCLI(pEntry) || IS_ENTRY_REPEATER(pEntry)) {
			...
			if (cap->fgRateAdaptFWOffload == TRUE) {
				...
				"(%d):wcid(%d)failfw(%u),totalfw(%u)-If(%d)\"%s\".\n", __LINE__,
			...
			MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_WARN,
				"(%d):IfIdx(%d),wcid(%d)TXInfo:RxBytes:%llu,RxPackets:%llu,TxBytes:%llu,TxDropPackets:%llu,TxFailPackets:%llu,TxPackets:%llu\n",
				__LINE__, CliIfIndex, pEntry->wcid, RptrTRInfo[CliIfIndex].RxBytes,
				...
				MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_INFO, "%s(%d):It's not Repeater Entry!\n", __func__, __LINE__);
		}
	}
	if (wcid != WCID_INVALID) {
		main_sta = entry_get(pAd, wcid);
...
{
	return ad->ApCfg.mac_repeater_en;
}
