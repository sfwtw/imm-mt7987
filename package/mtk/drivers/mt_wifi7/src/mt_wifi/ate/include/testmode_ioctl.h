...
#define PKTLA_DUMP(_lvl, _len, _byte_array)({			\
		...
		UINT32 *_arr = (UINT32 *)_byte_array;				\
		for (_cnt = 0; _cnt < _len; _cnt++)						\
			MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_ATE, _lvl, \
				"DWORD%d:%08x\n", _cnt, _arr[_cnt]); \
		_len;												\
	});
#define PKTUC_DUMP(_lvl, _len, _byte_array)({			\
		...
		MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_ATE, _lvl, "PKTUC_DUMP(%x): ", _len);	\
		for (_cnt = 0; _cnt < _len; _cnt++)						\
			MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_ATE, _lvl, "%02x", _arr[_cnt]);	\
		MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_ATE, _lvl, "\n");	\
		_len;												\
	});
#endif
