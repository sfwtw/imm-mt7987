...
static s_int32 mt_serv_init_op(struct test_operation *ops)
{
	...
	ops->op_set_ch_pwr_offset = mt_op_set_ch_pwr_offset;
	ops->op_set_cvt_pwr_offset = mt_op_set_cvt_pwr_offset;
	...
	ops->op_get_hetb_info = mt_op_get_hetb_info;
	ops->op_set_ru_aid = mt_op_set_ru_aid;
	ops->op_set_ru_info = mt_op_set_ru_info;
	...
	/* Set testmode Fixed rate table*/
	ops->op_set_frtbl_info = mt_op_set_frtbl_info;
	ops->op_set_preamble_punc_cfg = mt_op_set_preamble_punc_cfg;

	ops->op_set_tmr = mt_op_set_tmr;
	ops->op_set_txpwr_power_drop = mt_op_set_txpwr_power_drop;
	ops->op_set_txpwr_power_percentage = mt_op_set_txpwr_power_percentage;
...
#endif
	ops->op_get_chip_info = mt_op_get_chip_info;
	ops->op_get_rx_sw_count = mt_op_get_rx_sw_count;
	...
	struct test_configuration *configs, u_char band_idx)
{
	...
	if (band_idx == TEST_BAND0) {
		...
		configs->mcs = 7;
		configs->sgi = 0;
		configs->preampunc = ~(configs->preampunc & 0);
...
s_int32 mt_serv_init_test(struct service_test *serv_test)
{
	...
	if (!serv_test->engine_offload) {
		...
		sys_ad_zero_mem(serv_test->test_rx_statistic,
			sizeof(struct test_rx_stat) * band_num);
...
s_int32 mt_serv_set_channel(struct service_test *serv_test)
{
	...
	max_stream_num = max(GET_MAX_PATH(mcs_nss, ctrl_band_idx, 0),
							GET_MAX_PATH(mcs_nss, ctrl_band_idx, 1));
...
s_int32 mt_serv_start_tx(struct service_test *serv_test)
{
	...
	net_ad_set(winfo, RF_AT_FUNCID_SET_DBDC_BAND_IDX, ctrl_band_idx);
	net_ad_set(winfo, RF_AT_FUNCID_SET_MAC_HEADER, (fc | (dur << 16)));
	net_ad_set(winfo, RF_AT_FUNCID_SET_SEQ_CTRL, seq);
...
s_int32 mt_serv_stop_tx(struct service_test *serv_test)
{
	...
	serv_test->test_config.op_mode &= OP_MODE_TXSTOP;
...
s_int32 mt_serv_start_rx(struct service_test *serv_test)
{
	...
	ret = serv_test->test_op->op_set_rx_user_idx(winfos, ctrl_band_idx, user_idx);
	if (ret)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: err=0x%08x\n", __func__, ret));

	mt_serv_reset_txrx_counter(serv_test);
...
s_int32 mt_serv_stop_rx(struct service_test *serv_test)
{
	...
	serv_test->test_config.op_mode &= OP_MODE_RXSTOP;
...
s_int32 mt_serv_group_prek_dump_5g(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
...
s_int32 mt_serv_group_prek_dump_6g(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
...
s_int32 mt_serv_dpd_prek_dump_2g(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
...
s_int32 mt_serv_dpd_prek_dump_5g(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
...
s_int32 mt_serv_dpd_prek_dump_6g(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
...
s_int32 mt_serv_dpd_prek(struct service_test *serv_test)
{
	...
	if (ret)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: PREK_DPD_2G_PROC err=0x%08x\n", __func__, ret));

	return ret;
}

s_int32 mt_serv_set_freq_offset(
	struct service_test *serv_test, u_int32 type, u_int32 band_idx)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	...
	struct service_test *serv_test, u_int32 item)
{
	...
	case SERV_TEST_TXPWR_SET_PWR:
		if (pwr_param->ant_idx >= TEST_ANT_NUM) {
			SERV_LOG(SERV_DBG_CAT_ENGN, SERV_DBG_LVL_ERROR,
				("%s: pwr_param->ant_idx is invalid.\n", __func__));
			break;
		}
	...
	case SERV_TEST_TXPWR_SET_CH_PWR_OFFSET:
		if (pwr_param->ant_idx >= TEST_ANT_NUM) {
			SERV_LOG(SERV_DBG_CAT_ENGN, SERV_DBG_LVL_ERROR,
				("%s: pwr_param->ant_idx is invalid.\n", __func__));
			break;
		}
		configs->ch_pwr_offset[pwr_param->ant_idx] = pwr_param->ch_pwr_offset;
		ret = ops->op_set_ch_pwr_offset(
			winfos, configs, ctrl_band_idx, pwr_param);
		break;

	case SERV_TEST_TXPWR_SET_CVT_PWR_OFFSET:
		if (pwr_param->ant_idx >= TEST_ANT_NUM) {
			SERV_LOG(SERV_DBG_CAT_ENGN, SERV_DBG_LVL_ERROR,
				("%s: pwr_param->ant_idx is invalid.\n", __func__));
			break;
		}
		configs->cvt_pwr_offset[pwr_param->ant_idx] = pwr_param->cvt_pwr_offset;
		ret = ops->op_set_cvt_pwr_offset(
			winfos, configs, ctrl_band_idx, pwr_param);
		break;
...
s_int32 mt_serv_get_chip_capability(
	struct service_test *serv_test,
	struct test_capability *chip_cap)
{
	...
	ret = net_ad_get_chip_capability(winfos, chip_cap);
...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct test_configuration *config_band;
	struct test_rx_stat *test_rx_st;
	u_int8 ant_idx = 0;
	u_int8 band_idx = serv_test->ctrl_band_idx;

	config_band = &serv_test->test_config;
	...
	config_band->tx_stat.tx_done_cnt = 0;

	ret = serv_test->test_op->op_reset_txrx_counter(
			serv_test->test_winfo, band_idx);
	...
	struct service_test *serv_test)
{
	...
	ret = ops->op_set_fagc_rssi_path(
		serv_test->test_winfo,
		serv_test->ctrl_band_idx,
		serv_test->test_config.fagc_path);
	...
	struct test_rx_stat_u *st)
{
	...
	ops = serv_test->test_op;
	/* winfos = serv_test->test_winfo; */
	if (band_idx < TEST_MAX_BAND_NUM)
		stat = &serv_test->test_rx_statistic[band_idx];
	else {
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: band_idx is invalid.\n",
			__func__));
		return SERV_STATUS_SERV_TEST_INVALID_BANDIDX;
	}
	...
	case TEST_RX_STAT_BAND:
		...
		if ((blk_idx + sizeof(struct test_rx_stat_band_info)) <=
			sizeof(stat->rx_st_band))
	...
	u_int32 log_ctrl, u_int32 log_size)
{
	...
	ops = serv_test->test_op;
	if (log_type - 1 < TEST_LOG_TYPE_NUM)
		log_cb = &serv_test->test_log_dump[log_type-1];
	else {
		ret = SERV_STATUS_SERV_TEST_INVALID_LEN;
		SERV_LOG(SERV_DBG_CAT_ENGN, SERV_DBG_LVL_ERROR,
			("%s: log_type is invalid.\n", __func__));
		goto err0;
	}
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
		("%s: log_type=0x%08x, overwrite:%d\n", __func__, log_type, overwrite));
...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char tx_mode = serv_test->test_config.tx_mode;
...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char mcs = serv_test->test_config.mcs;
...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char sys_bw = serv_test->test_config.bw;
...
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char per_pkt_bw = serv_test->test_config.per_pkt_bw;
...
s_int32 mt_serv_continuous_tx(struct service_test *serv_test)
{
	...
	u_int32 tx_mode = CONFIG_GET_PARAM(serv_test, tx_mode);
	u_int32 bw = CONFIG_GET_PARAM(serv_test, bw);
	u_int32 pri_ch = CONFIG_GET_PARAM(serv_test, ctrl_ch);
	u_int32 central_ch = CONFIG_GET_PARAM(serv_test, channel);
	u_int32 rate = CONFIG_GET_PARAM(serv_test, mcs);
	u_int32 ant_sel = CONFIG_GET_PARAM(serv_test, tx_ant);
	u_int32 tx_fd_mode = CONFIG_GET_PARAM(serv_test, tx_fd_mode);
...
s_int32 mt_serv_get_tx_info(struct service_test *serv_test)
{
	...
	for (band_idx = 0; band_idx < TEST_BAND_NUM; band_idx++) {
		serv = NULL;
		ad = net_ad_multi_inf_get_pAd_by_Band(serv_test->test_winfo, band_idx);
		if (ad != NULL)
			serv = net_ad_wrap_service(ad);

		if (serv == NULL)
			continue;

		test_configs_band = &serv->test_config;
		ret = serv->test_op->op_get_tx_info(
			serv->test_winfo,
			band_idx,
			test_configs_band,
			NULL);
		...
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
			("%s: band[%d] => txed_cnt:%u, tx_done_cnt:%u\n", __func__,
				band_idx, test_configs_band->tx_stat.txed_cnt,
				test_configs_band->tx_stat.tx_done_cnt));
...
s_int32 mt_serv_set_txpwr_drop(
	struct service_test *serv_test,
...
)
{
	...
	ret = serv_test->test_op->op_set_txpwr_power_drop(
			serv_test->test_winfo,
			cPowerDropLevel,
			band_idx);
...
s_int32 mt_serv_set_txpwr_percentage(
	struct service_test *serv_test,
	boolean enable,
	u_char band_idx
)
{
	...
	ret = serv_test->test_op->op_set_txpwr_power_percentage(
			serv_test->test_winfo,
			enable,
			band_idx);
	...
	boolean fg_tx_param_update)
{
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
		...
		CONFIG_GET_PARAM(serv_test, tx_ant)));

	tx_ant_cfg = CONFIG_GET_PARAM(serv_test, tx_ant);

	/* Assign Wlan ID for fixed rate TxD */
	CONFIG_SET_PARAM(serv_test, wcid_ref, (u_int8)wlan_idx);
	...
	if (fg_bf) {
		...
		/* Set Tx Nss = 1 */
		CONFIG_SET_PARAM(serv_test, nss, 1);
		...
		if (CONFIG_GET_PARAM(serv_test,
				fgEBfEverEnabled) == FALSE) {
			/* Stop Rx before ready to Tx */
			mt_serv_stop_rx(serv_test);

			/* ATECtrl->eTxBf = FALSE; */
			/* ATECtrl->iTxBf = FALSE; */
			CONFIG_SET_PARAM(serv_test, ebf, FALSE);
			CONFIG_SET_PARAM(serv_test, ibf, FALSE);
			...
			/* Set Tx Nss = 1 */
			if ((tx_ant_cfg == 3) || (tx_ant_cfg == 7) || (tx_ant_cfg == 15) || (tx_ant_cfg == 31))
				CONFIG_SET_PARAM(serv_test, nss, 1);
	...
	struct service_test *serv_test, u_char pfmu_idx, u_char nc)
{
	...
	/* Wcid, Bssidx mapping */
	BssIdx = 0;
	switch (serv_test->ctrl_band_idx) {
	case 0:
		wcid = 1;
		break;
	case 1:
	...
	sRet = snprintf(cmd_str, sizeof(cmd_str),
		"%.2x:%.2x:%.2x:00:00:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:",
					wcid, BssIdx, pfmu_idx, ndp_nss, nc, nr);
	if (os_snprintf_error(sizeof(cmd_str), sRet))
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
			("%s: err=0x%08x\n", __func__, sRet));

	sRet = snprintf(&cmd_str[45], sizeof(cmd_str) - 45,
	...
	sRet = snprintf(cmd_str, sizeof(cmd_str), "%d", wcid);
	if (os_snprintf_error(sizeof(cmd_str), sRet))
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
			("%s: err=0x%08x\n", __func__, sRet));
	...
	/* Configure WTBL */
	addr1 = (u_char *)CONFIG_GET_PADDR(serv_test, addr1[wcid-1]);
	...
	sRet = snprintf(&cmd_str[52], sizeof(cmd_str) - 52,
	...
	struct service_test *serv_test, u_char pfmu_idx, u_char nc)
{
	...
	sRet = snprintf(cmd_str, sizeof(cmd_str),
		"%.2x:%.2x:%.2x:00:01:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:",
					wcid, BssIdx, pfmu_idx, ndp_nss, nc, nr);
	if (os_snprintf_error(sizeof(cmd_str), sRet))
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
			("%s: err=0x%08x\n", __func__, sRet));
	...
	struct service_test *serv_test, u_int16 *arg)
{
	...
	s_int16 phi11,    phi21,     phi31,    phi41;
	s_int16 h11,      angle_h11, h21, angle_h21, h31, angle_h31, h41, angle_h41, h51, angle_h51;
	s_int32 ret = SERV_STATUS_SUCCESS;
	u_char  ucTxPath;
	...
	angle_h51 = 0;
	phi41     = 0;
	if (serv_test->test_config.tx_ant == SERV_TEST_TX_PATH_5) {
		h51       = (s_int16)(arg[11] << 3) >> 3;
		angle_h51 = (s_int16)(arg[12] << 3) >> 3;
		phi41     = 0;
	}
	phi11     = 0;
	phi21     = 0;
	...
	ucTxPath = CONFIG_GET_PARAM(serv_test, tx_ant);
	...
	default:
		...
		break;
	case SERV_TX_PATH_5:
		phi11 = angle_h51 - angle_h11;
		phi21 = angle_h51 - angle_h21;
		phi31 = angle_h51 - angle_h31;
		phi41 = angle_h51 - angle_h41;
	}
	...
	case SERV_TX_PATH_5:
		...
		buf[4] = (u_int16)((u_int16)phi21 & 0xFFF);
		buf[6] = (u_int16)((u_int16)phi31 & 0xFFF);
		buf[8] = (u_int16)((u_int16)phi41 & 0xFFF);
		break;
	}
...
s_int32 mt_serv_set_txbf_profile_data_Write_20m_all_5x5(
	struct service_test *serv_test,
	u_char profile_idx,
	u_char *data)
{
	return mt_ad_set_txbf_profile_data_write_20m_all_5x5(
					serv_test->test_winfo,
					profile_idx,
					data);
}
	...
	u_int8 *data)
{
	...
	case TEST_CALI_SRC_FILE:
		net_ad_w_cali_2_bin(serv_test->test_winfo, data);
		break;
	...
	case TEST_CALI_SRC_ROM:
		net_ad_w_cali_2_i2c_eeprom(serv_test->test_winfo, data);
		break;
	default:
		ret = SERV_STATUS_SERV_TEST_INVALID_PARAM;
	}
...
s_int32 mt_serv_tx_carr(struct service_test *serv_test)
{
	...
	configs->rf_pwr = 10;
	configs->digi_pwr = serv_test->test_config.tx_pwr[0];
...
s_int32 mt_serv_tx_carr_stop(struct service_test *serv_test)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct test_configuration *configs;
	...
	configs->tone_type = 0;
	configs->tone_freq = 0;
	configs->dc_offset_I = 0;
	configs->dc_offset_Q = 0;
	configs->rf_pwr = 0;
	configs->digi_pwr = 0;
...
s_int32 mt_serv_get_chip_info(struct service_test *serv_test, struct _net_testmode_chip_info *ptestmode_chip_info)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct test_operation *ops;
	...
	ret = net_ad_get_chip_info(serv_test->test_winfo, ptestmode_chip_info);
	if (ret)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: err=0x%08x\n", __func__, ret));
...
s_int32 mt_serv_get_rx_sw_count(
	struct service_test *serv_test, u_int32 band_idx, u_int32 *rxed_slt_pkt_cnt)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	struct test_operation *ops;
...
s_int32 mt_serv_get_serv_test_by_band(
	struct service_test **serv_test,
	u_int32 band_idx)
{
	s_int32 ret = SERV_STATUS_SUCCESS;
	...
	if (ad != NULL)
		*serv_test = net_ad_wrap_service(ad);
	else {
		*serv_test = NULL;
	...
	if (ret)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: err=0x%08x, band_idx=%d\n", __func__,
			SERV_STATUS_OSAL_SYS_INVALID_NULL_POINTER, band_idx));
