...
extern UINT8 g_u1MinPwrlimitperRate[MAX_BAND_NUM][DATA_TXPOWER_MAX_BW_NUM][DATA_TXPOWER_MAX_MCS_NUM];
#endif
...
#define MSDU_CNT_CR_NUMBER 8
	...
	for (idx = 0; VALID_UCAST_ENTRY_WCID(pAd, idx); idx++) {
		...
		if ((pEntry->Sst == SST_ASSOC) &&
			...
			(pEntry->ba_info.TxAmsduBitmap != 0)) {
			MTWF_PRINT("Wcid%03d: %02x", idx, pEntry->ba_info.TxAmsduBitmap);
		}
	}
	return TRUE;
}
...
VOID mtf_update_raw_counters(RTMP_ADAPTER *pAd)
{
	...
	chip_update_ampdu_raw_counters(pAd, hc_get_hw_band_idx(pAd));
	...
	union _HTTRANSMIT_SETTING *transmit)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, info->WCID))
		mac_entry = entry_get(pAd, info->WCID);
...
UINT16 mtf_tx_rate_to_tmi_rate(UINT8 mode, UINT8 mcs, UINT8 nss, BOOLEAN stbc, UINT8 preamble)
{
	...
	default:
		MTWF_DBG(NULL, DBG_CAT_TX, CATTX_TMAC, DBG_LVL_ERROR, "Invalid mode(mode=%d)\n",
				 mode);
		break;
	}
...
VOID mtf_dump_wtbl_base_info(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "Not supported\n");
}
 ...
 * since WTBL_BASE_ADDR is changed from 0x820e0000 to 820e8000
 * and pcie remap address is map 0x820e0000 of MAC to 0x3xxxx of PCIE
 */
#define WTBL_BASE_ADDR_OFFSET 0x8000

INT mtf_init_wtbl(RTMP_ADAPTER *pAd, BOOLEAN bHardReset)
{
	pAd->mac_ctrl.wtbl_base_addr[0] = (UINT32)WTBL_BASE_ADDR + WTBL_BASE_ADDR_OFFSET;
	pAd->mac_ctrl.wtbl_entry_size[0] = (UINT16)WTBL_PER_ENTRY_SIZE;
	pAd->mac_ctrl.wtbl_entry_cnt[0] = hc_get_chip_wtbl_max_num(pAd);
	return TRUE;
}
...
INT32 mtf_trans_rxd_into_rxblk(RTMP_ADAPTER *pAd, struct _RX_BLK *rx_blk, PNDIS_PACKET rx_pkt)
{
	...
	if (((rxd_grp0->rxd_1 & (RXD_GROUP3_VLD | RXD_GROUP5_VLD)) == (RXD_GROUP3_VLD | RXD_GROUP5_VLD))
		...
		if (rx_blk->band > DBDC_BAND1) {
			MTWF_DBG(pAd, DBG_CAT_RX, CATRX_RXINFO, DBG_LVL_ERROR, "Invalid band(%d)!\n", rx_blk->band);
			goto err_out;
		}

		rx_rate_rc = &pAd->tr_ctl.tr_cnt.rx_rate_rc;
		if (dbw >= ARRAY_SIZE(rx_rate_rc->mpdu_cnt)) {
			MTWF_DBG(NULL, DBG_CAT_RX, CATRX_RXINFO, DBG_LVL_WARN, "Invalid BW(%d)!\n", dbw);
			goto err_out;
		}
	...
	for (loop = 0; loop < token_cnt; loop++) {
		...
		if (pkt != NULL) {
			...
			if (ATE_ON(pAd)) {
				...
				mt_serv_get_serv_test_by_band(&serv_test, band_idx);
			...
			if (pkt != NULL) {
...
#endif
				{
					...
					if (VALID_UCAST_ENTRY_WCID(pAd, wcid)) {
						...
						pEntry = entry_get(pAd, wcid);
						CalStaTxLatency(pEntry, pkt_time, 0, 0);
						if (pkt_status == 1)
										...
										UINT8 *data, UINT32 token_cnt)
{
	...
	struct token_tx_pkt_queue *que = NULL;
#ifdef EAP_STATS_SUPPORT
	UINT32 pkt_time = 0, pkt_airtime = 0, pkt_queuetime = 0;
	UINT8 pkt_status, pkt_tx_cnt = 0;
	PMAC_TABLE_ENTRY pEntry = NULL;
	...
	while (loop < token_cnt) {
		...
		if ((*token_ptr) & TXDONE_MT7986_H) {
			...
			if (VALID_UCAST_ENTRY_WCID(pAd, wlan_id)) {
				pkt_time = (((*token_ptr) & TXDONE_MT7986_TX_DELAY_MASK) >> TXDONE_MT7986_TX_DELAY_SHIFT);
				pkt_status = (((*token_ptr) & TXDONE_MT7986_STAT_MASK) >> TXDONE_MT7986_STAT_SHIFT);
#ifdef EAP_ENHANCE_STATS_SUPPORT
				pkt_airtime = (((*token_ptr) & TXDONE_MT7986_AIR_DELAY_MASK) >> TXDONE_MT7986_AIR_DELAY_SHIFT);
				pkt_tx_cnt = (((*token_ptr) & TXDONE_MT7986_TX_CNT_MASK) >> TXDONE_MT7986_TX_CNT_SHIFT);
				pkt_queuetime = 0;
				...
				/* Get Mac table entry */
				pEntry = entry_get(pAd, wlan_id);
				CalStaTxLatency(pEntry, pkt_time, pkt_airtime, pkt_tx_cnt);
				if (pkt_status == 1)
					pEntry->mpdu_xretries.QuadPart++;
			}
#endif
								...
								UINT8 *dataPtr, UINT32 TxDCnt, UINT32 Len, UINT8 Ver)
{
	...
	for (loop = 0; loop < TxDCnt; loop++) {
		...
		if (pkt != NULL) {
			RTMP_SET_DEBUGGER_SKIP_DUMP(pkt, 1);
#ifdef CONFIG_ATE
			if (ATE_ON(pAd)) {
#ifdef CONFIG_WLAN_SERVICE
				struct service_test *serv_test;
...
static UINT32 mtf_v1_txdone_handle(RTMP_ADAPTER *pAd, PKT_TOKEN_CB *cb, VOID *ptr)
{
	...
	MTWF_DBG(NULL, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_DEBUG,
			 "%s: rx_byte_cnt = %d,token_cnt = %d, token_cnt = %d\n",
			  __func__, rx_byte_cnt, token_cnt, token_cnt);

	if (token_cnt > 0x7f) {
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "Invalid token_cnt(%d)!\n", token_cnt);
		return FALSE;
	}
	...
	case RMAC_RX_PKT_TYPE_TXRX_NOTIFY:
		if (((token_cnt << 2) + 8) != rx_byte_cnt) {
			MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "token_cnt(%d) and rx_byte_cnt(%d) mismatch!\n", token_cnt, rx_byte_cnt);
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "Invalid type(%d)!\n", pkt_type);
		break;
	}
									...
									VOID *ptr)
{
	...
	MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_DEBUG,
			 "DW0=0x%08x, rxByteCnt=%d,tokenCnt= %d, pkt_type=%d\n",
			  dw0, rxByteCnt, tokenCnt, pkt_type);

	if (tokenCnt > 0x7f) {
		MTWF_DBG(pAd, DBG_CAT_TX, TOKEN_INFO, DBG_LVL_ERROR, "Invalid tokenCnt(%d)!\n", tokenCnt);
		return FALSE;
	}
	...
	case RMAC_RX_PKT_TYPE_TXRX_NOTIFY:
		...
		if ((tokenCnt * len_per_token + 8) != rxByteCnt) {
			MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "tokenCnt(%d) and rxByteCnt(%d) mismatch!\n", tokenCnt, rxByteCnt);
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "Invalid PTK_TYPE(%d) in TX_FREE_DONE_EVENT!\n", pkt_type);
		break;
	}
...
inline UINT32 mtf_txdone_handle(RTMP_ADAPTER *pAd, VOID *ptr, UINT8 resource_idx)
{
	...
	if (ver == MT7615_TXDONE || ver == MT7622_TXDONE)
		...
		MTWF_DBG(pAd, DBG_CAT_TOKEN, TOKEN_INFO, DBG_LVL_ERROR, "invalid txdone version(%d)!\n", ver);
...
VOID mtf_wa_cpu_update(RTMP_ADAPTER *ad)
{
	...
	if (PD_GET_WHNAT_ENABLE(ad->physical_dev) &&
...
INT32 mtf_txs_handler(RTMP_ADAPTER *pAd, VOID *rx_packet)
{
	...
	if (rx_byte_cnt != (txs_cnt * txs_entry_len + 8)) {
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_TMAC, DBG_LVL_ERROR,
		...
		for (idx = 0; idx < txs_cnt; idx++) {
			...
			MTWF_DBG(NULL, DBG_CAT_TX, CATTX_TMAC, dbg_prn,
...
VOID mtf_rx_event_handler(RTMP_ADAPTER *pAd, UCHAR *data)
{
	...
	event_rxd = (EVENT_RXD *)(data + chip_cap->sizeof_rxd_grp0);
	...
	AndesAppendCmdMsg(msg, (char *)(data + chip_cap->sizeof_rxd_grp0),
...
VOID mtf_fill_uni_cmd_header(struct _RTMP_ADAPTER *pAd, struct cmd_msg *msg, VOID *pkt)
{
	...
	cmd_header->header_2.field.s2d_index = msg->attr.mcu_dest;
	...
	MTWF_DBG(NULL, DBG_CAT_TX, CATTX_TMAC, DBG_LVL_INFO,
		 "%s: cmd_header: 0x%x 0x%x 0x%x, Length=%d\n", __func__,
...
VOID mtf_fill_cmd_header(struct _RTMP_ADAPTER *pAd, struct cmd_msg *msg, VOID *pkt)
{
	...
	fw_txd->fw_txd_2.field.ucS2DIndex = msg->attr.mcu_dest;
...
UINT32 mtf_rxv_handler(RTMP_ADAPTER *pAd, struct _RX_BLK *rx_blk, VOID *rx_packet)
{
...
VOID mtf_calculate_ecc(struct _RTMP_ADAPTER *ad, UINT32 oper, UINT32 group, UINT8 *scalar, UINT8 *point_x, UINT8 *point_y)
{
	...
	MTWF_DBG(ad, DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_INFO,
		"oper:%d, group:%d\n", oper, group);
#ifdef WIFI_UNIFIED_COMMAND
...
VOID mtf_txpower_boost(struct _RTMP_ADAPTER *pAd, UCHAR ucBandIdx)
{
	...
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_CCK,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpCck[0]),
				SINGLE_SKU_FILL_TABLE_CCK_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_OFDM,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpOfdm[0]),
				SINGLE_SKU_FILL_TABLE_OFDM_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_HT20,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpHt20[0]),
				SINGLE_SKU_FILL_TABLE_HT20_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_HT40,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpHt40[0]),
				SINGLE_SKU_FILL_TABLE_HT40_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_VHT20,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht20[0]),
				SINGLE_SKU_FILL_TABLE_VHT20_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_VHT40,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht40[0]),
				SINGLE_SKU_FILL_TABLE_VHT40_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_VHT80,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht80[0]),
				SINGLE_SKU_FILL_TABLE_VHT80_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_VHT160,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht160[0]),
				SINGLE_SKU_FILL_TABLE_VHT160_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU26,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU26[0]),
				SINGLE_SKU_FILL_TABLE_RU26_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU52,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU52[0]),
				SINGLE_SKU_FILL_TABLE_RU52_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU106,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU106[0]),
				SINGLE_SKU_FILL_TABLE_RU106_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU242,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU242[0]),
				SINGLE_SKU_FILL_TABLE_RU242_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU484,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU484[0]),
				SINGLE_SKU_FILL_TABLE_RU484_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU996,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996[0]),
				SINGLE_SKU_FILL_TABLE_RU996_LENGTH_V1);
	TxPwrUpCtrl(pAd, ucBandIdx, POWER_UP_CATE_V1_RU996X2,
				&(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996X2[0]),
				SINGLE_SKU_FILL_TABLE_RU996X2_LENGTH_V1);
}

VOID mtf_txpower_boost_ctrl(
	struct _RTMP_ADAPTER *pAd, CHAR cPwrUpCat, PUCHAR pcPwrUpValue)
{
	switch (cPwrUpCat) {
	case POWER_UP_CATE_V1_CCK:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpCck,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_CCK_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_OFDM:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpOfdm,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_OFDM_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_HT20:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpHt20,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_HT20_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_HT40:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpHt40,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_HT40_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_VHT20:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht20,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_VHT20_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_VHT40:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht40,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_VHT40_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_VHT80:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht80,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_VHT80_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_VHT160:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpVht160,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_VHT160_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU26:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU26,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU26_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU52:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU52,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU52_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU106:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU106,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU106_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU242:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU242,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU242_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU484:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU484,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU484_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU996:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU996_LENGTH_V1);
		break;

	case POWER_UP_CATE_V1_RU996X2:
		os_move_mem(pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996X2,
			pcPwrUpValue, sizeof(CHAR) * SINGLE_SKU_FILL_TABLE_RU996X2_LENGTH_V1);
		break;
...
BOOLEAN mtf_txpower_boost_power_cat_type(struct _RTMP_ADAPTER *pAd, UINT8 u1PhyMode, UINT8 u1Bw, PUINT8 pu1PowerBoostRateType)
{
	...
	if (u1PhyMode > 4) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "Invalid PhyMode!! Please Enter PhyMode as CCK:0, OFDM:1, HT:2, VHT:3, HE:4\n");
		return FALSE;
	}

	if ((u1PhyMode == 0 || u1PhyMode == 1) && (u1Bw != 0)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "Invalid BW!! For CCK/OFDM mode valid BW option is 0\n");
		return FALSE;
	} else if ((u1PhyMode == 2) && (u1Bw > 1)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "Invalid BW!! For HT mode valid BW option is 0/1\n");
		return FALSE;
	} else if ((u1PhyMode == 3) && (u1Bw > 3)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "Invalid BW!! For VHT mode valid BW option is 0/1/2/3\n");
		return FALSE;
	} else if ((u1PhyMode == 4) && (u1Bw > 6)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "Invalid BW!! For HE mode valid BW is 0/1/2/3/4/5/6\n");
		return FALSE;
	}
...
BOOLEAN mtf_txpower_boost_rate_type(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx, UINT8 u1PowerBoostRateType)
{
	...
	pcTxPwrBoost = &(pAd->CommonCfg.PowerBoostParamV1.cPowerUpCck[0]);
	...
	if (RTMPGetKeyParameter("PowerUpRU26", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU26_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU26_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU26] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU26[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU52", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU52_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU52_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU52] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU52[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU106", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU106_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU106_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU106] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU106[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU242", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU242_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU242_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU242] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU242[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU484", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU484_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU484_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU484] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU484[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU996", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU996_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU996_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_TX, DBG_LVL_ERROR, "[PowerUpRU996] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
	...
	if (RTMPGetKeyParameter("PowerUpRU996X2", tmpbuf, 32,
		...
		for (i = 0, value = rstrtok(tmpbuf, "-"); (value) && (i < SINGLE_SKU_FILL_TABLE_RU996X2_LENGTH_V1); value = rstrtok(NULL, "-"), i++) {
			...
			if (ucPwrBoostReg[0] >= SINGLE_SKU_FILL_TABLE_RU996X2_LENGTH_V1)
				MTWF_DBG(pAd, DBG_CAT_CFG, CATHW_TX, DBG_LVL_ERROR, "[PowerUpRU996X2] Input parameter incorrect!!\n");
			else
				pAd->CommonCfg.PowerBoostParamV1.cPowerUpRU996X2[ucPwrBoostReg[0]] = ucPwrBoostReg[1];
		}
	}
}
#endif
...
VOID mtf_txpower_show_info(struct _RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
	...
	MTWF_PRINT("=============================================================================\n");
	MTWF_PRINT("  Band Index: %d,  Channel Band: %d:%s\n", prEventTxPowerInfo->u1BandIdx, prEventTxPowerInfo->u1ChBand,
			((prEventTxPowerInfo->u1ChBand) ? ((prEventTxPowerInfo->u1ChBand == 1) ? "5G" : "6G") : "2G"));
...
{
	P_EXT_EVENT_TXPOWER_ALL_RATE_POWER_INFO_T pAllPow;
	UINT16 u1TxPwrIdx, u1RateIdx, nextRateOffset;
	...
	P_FRAME_POWER_CONFIG_INFO_T pFPow;
	...
	/* get event info buffer contents */
	pAllPow = (P_EXT_EVENT_TXPOWER_ALL_RATE_POWER_INFO_T)Data;
	pFPow = &pAllPow->rRatePowerInfo;

#ifdef MGMT_TXPWR_CTRL
	pAd->ApCfg.MaxMgmtTxpwr = 0;
	for (u1TxPwrIdx = TXPOWER_RATE_CCK_OFFSET; u1TxPwrIdx < TXPOWER_RATE_OFDM_OFFSET; u1TxPwrIdx++) {
		pAd->ApCfg.MgmtTxPwrCck[u1TxPwrIdx] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (pAd->ApCfg.MaxMgmtTxpwr < pAd->ApCfg.MgmtTxPwrCck[u1TxPwrIdx])
			pAd->ApCfg.MaxMgmtTxpwr = pAd->ApCfg.MgmtTxPwrCck[u1TxPwrIdx];
	}
	for (u1TxPwrIdx = TXPOWER_RATE_OFDM_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HT20_OFFSET; u1TxPwrIdx++) {
		pAd->ApCfg.MgmtTxPwrOfdm[u1TxPwrIdx - TXPOWER_RATE_OFDM_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (pAd->ApCfg.MaxMgmtTxpwr < pAd->ApCfg.MgmtTxPwrOfdm[u1TxPwrIdx - TXPOWER_RATE_OFDM_OFFSET])
			pAd->ApCfg.MaxMgmtTxpwr = pAd->ApCfg.MgmtTxPwrOfdm[u1TxPwrIdx - TXPOWER_RATE_OFDM_OFFSET];
	}
	...
	g_fgCommand = FALSE;
		...
		for (u1BwIdx = 0; (u1BwIdx < DATA_TXPOWER_MAX_BW_NUM); u1BwIdx++)
			for (u1McsIdx = 0; (u1McsIdx < DATA_TXPOWER_MAX_MCS_NUM); u1McsIdx++)
				g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][u1BwIdx][u1McsIdx] = 100;
#endif
	...
	MTWF_PRINT("=============================================================================\n");
	MTWF_PRINT("  Band Index: %d,  Channel Band: %d:%s ePAGain:%d\n",
		pAllPow->u1BandIdx, pAllPow->u1ChBand, pAllPow->u1ChBand ?
		((pAllPow->u1ChBand == 1) ? "5G" : "6G") : ("2G"), pAllPow->u1EpaFeGain);
	MTWF_PRINT("-----------------------------------------------------------------------------\n");

	/* CCK */
	for (u1TxPwrIdx = TXPOWER_RATE_CCK_OFFSET; u1TxPwrIdx < TXPOWER_RATE_OFDM_OFFSET; u1TxPwrIdx++) {
		MTWF_PRINT("  [CCK_%02dM]: 0x%02x (%d)\n",
		u1TxPwrCckRate[u1TxPwrIdx - TXPOWER_RATE_CCK_OFFSET],
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	/* OFDM */
	for (u1TxPwrIdx = TXPOWER_RATE_OFDM_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HT20_OFFSET; u1TxPwrIdx++) {
		MTWF_PRINT("  [OFDM_%02dM]: 0x%02x (%d)\n",
	...
	for (u1TxPwrIdx = TXPOWER_RATE_HT20_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HT40_OFFSET; u1TxPwrIdx++) {
		MTWF_PRINT("  [HT20_M%02d]: 0x%02x (%d)\n",
		...
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HT20_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HT20_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	}

	/* HT40 */
	for (u1TxPwrIdx = TXPOWER_RATE_HT40_OFFSET; u1TxPwrIdx < TXPOWER_RATE_VHT20_OFFSET; u1TxPwrIdx++) {
		MTWF_PRINT("  [HT40_M%02d]: 0x%02x (%d)\n",
		...
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HT40_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HT40_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_VHT20_OFFSET; u1TxPwrIdx < TXPOWER_RATE_VHT40_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [VHT20_M%02d]: 0x%02x (%d)   [VHT40_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrIdx < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_VHT20_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_VHT20_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_VHT20_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_VHT20_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT20_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_VHT80_OFFSET; u1TxPwrIdx < TXPOWER_RATE_VHT160_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [VHT80_M%02d]: 0x%02x (%d)   [VHT160_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_VHT80_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_VHT80_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_VHT80_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_VHT80_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_VHT80_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_HE26_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HE52_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [HE26_M%02d]:  0x%02x (%d)   [HE52_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_HE26_NUM + MODULATION_SYSTEM_HE52_NUM;
		MTWF_PRINT("   [HE106_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_HE242_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HE484_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [HE242_M%02d]: 0x%02x (%d)   [HE484_M%02d]: 0x%02x (%d)",
			u1RateIdx,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
			u1RateIdx,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
			nextRateOffset = MODULATION_SYSTEM_HE242_NUM + MODULATION_SYSTEM_HE484_NUM;
		MTWF_PRINT("   [HE996_M%02d]: 0x%02x (%d)\n",
			u1RateIdx,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
			pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_HE242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_HE242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	}
	...
	/* HE996X2 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_HE996X2_OFFSET; u1TxPwrIdx < TXPOWER_RATE_HE_NUM; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [HE996X2_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_HE996X2_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_HE996X2_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	/* EHT26 EHT52 EHT106 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT26_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT52_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT26_M%02d]: 0x%02x (%d)   [EHT52_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_EHT26_NUM + MODULATION_SYSTEM_EHT52_NUM;
		MTWF_PRINT("   [EHT106_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	/* EHT242 EHT484 EHT996 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT242_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT484_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT242_M%02d]: 0x%02x (%d)   [EHT484_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_EHT242_NUM + MODULATION_SYSTEM_EHT484_NUM;
		MTWF_PRINT("   [EHT996_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][0][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][1][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
			g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][2][u1TxPwrIdx - TXPOWER_RATE_EHT242_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	}

	MTWF_PRINT("\n");

	/* EHT996X2 EHT996X4 EHT26_52 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT996X2_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT996X4_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT996X2_M%02d]: 0x%02x (%d)   [EHT996X4_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_EHT996X2_NUM + MODULATION_SYSTEM_EHT996X4_NUM;
		MTWF_PRINT("   [EHT26_52_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_EHT996X2_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][3][u1TxPwrIdx - TXPOWER_RATE_EHT996X2_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][4][u1TxPwrIdx - TXPOWER_RATE_EHT996X2_OFFSET] > pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
		g_u1MinPwrlimitperRate[pAllPow->u1BandIdx][4][u1TxPwrIdx - TXPOWER_RATE_EHT996X2_OFFSET] = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996X2_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	}

	MTWF_PRINT("\n");

	/* EHT26_106 EHT484_242 EHT996_484 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT26_106_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT484_242_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT26_106_M%02d]: 0x%02x (%d)   [EHT484_242_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_106_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_106_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_EHT26_106_NUM + MODULATION_SYSTEM_EHT484_242_NUM;
		MTWF_PRINT("   [EHT996_484_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_106_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT26_106_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
	if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
		u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	}
	...
	/* EHT996_484_242 EHT996X2_484 EHT996X3 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT996_484_242_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT996X2_484_OFFSET; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT996_484_242_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm);
		MTWF_PRINT("  [EHT996X2_484_M%02d]: 0x%02x (%d)",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996_484_242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996_484_242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm);
		nextRateOffset = MODULATION_SYSTEM_EHT996_484_242_NUM + MODULATION_SYSTEM_EHT996X2_484_NUM;
		MTWF_PRINT("   [EHT996X3_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996_484_242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + MODULATION_SYSTEM_EHT996_484_242_NUM][pAllPow->u1BandIdx].i1FramePowerDbm;
		if (u1TxPwrMax < pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm)
			u1TxPwrMax = pFPow->ai1FramePowerConfig[u1TxPwrIdx + nextRateOffset][pAllPow->u1BandIdx].i1FramePowerDbm;
#endif
	...
	/* EHT996X3_484 */
	for (u1RateIdx = 0, u1TxPwrIdx = TXPOWER_RATE_EHT996X3_484_OFFSET; u1TxPwrIdx < TXPOWER_RATE_EHT_NUM; u1TxPwrIdx++, u1RateIdx++) {
		MTWF_PRINT("  [EHT996X3_484_M%02d]: 0x%02x (%d)\n",
		u1RateIdx,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm,
		pFPow->ai1FramePowerConfig[u1TxPwrIdx][pAllPow->u1BandIdx].i1FramePowerDbm);
#ifdef DATA_TXPWR_CTRL
		...
		pAd->ApCfg.MaxBaseTxPwr = u1TxPwrMax;
