...
static void andes_starv_block_init(struct starv_log *ctrl, struct MCU_CTRL *mcu_ctrl)
{
	...
	strlcpy(block->name, "andes", sizeof(block->name));
	block->priv = mcu_ctrl;
	block->ctrl = ctrl;
	block->timeout = 1000;
...
struct cmd_msg *AndesAllocCmdMsgGe(RTMP_ADAPTER *ad, unsigned int length, BOOLEAN bOldCmdFmt)
{
	...
	struct MCU_CTRL *ctl = PD_GET_MCU_CTRL_PTR(ad->physical_dev);
...
VOID AndesInitCmdMsg(struct cmd_msg *msg, CMD_ATTRIBUTE attr)
{
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "mcu_dest(%d), cmd_type(0x%x), ExtCmdType(0x%x)\n",
			  attr.mcu_dest, attr.type, attr.ext_type);
...
VOID AndesFreeCmdMsg(struct cmd_msg *msg)
{
	...
	ctl = PD_GET_MCU_CTRL_PTR(ad->physical_dev);
...
VOID AndesCtrlExit(RTMP_ADAPTER *ad)
{
	...
	if (OS_TEST_BIT(MCU_INIT, &ctl->flags)) {
		...
		MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "tx_kickout_fail_count = %ld\n", ctl->tx_kickout_fail_count);
		MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "tx_timeout_fail_count = %ld\n", ctl->tx_timeout_fail_count);
		MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "rx_receive_fail_count = %ld\n", ctl->rx_receive_fail_count);
		MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "alloc_cmd_msg = %ld\n", ctl->alloc_cmd_msg);
		MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "free_cmd_msg = %ld\n", ctl->free_cmd_msg);
	}
...
#ifdef WF_RESET_SUPPORT
VOID AndesStatusCheck(RTMP_ADAPTER *ad, int ret)
{
	...
	if (ret == -ETIMEDOUT) {
		...
		if ((exp_type != 0) && (PD_GET_WF_RESET_IN_PROGRESS(ad->physical_dev) == FALSE)) {
			RTCMDUp(&ad->physical_dev->wf_reset_thread);
			return;
		}

		PD_SET_WF_CMD_TIMEOUT_COUNT(ad->physical_dev, PD_GET_WF_CMD_TIMEOUT_COUNT(ad->physical_dev) + 1);

		if (PD_GET_WF_CMD_TIMEOUT_COUNT(ad->physical_dev) > (FW_CMD_TO_RECORD_CNT - 1))
			PD_SET_WF_CMD_TIMEOUT_COUNT(ad->physical_dev, 0);

		if (arch_ops->heart_beat_check)
			arch_ops->heart_beat_check(ad);
	}
}
#endif
...
INT32 _AndesSendCmdMsg(PRTMP_ADAPTER ad, struct cmd_msg *msg)
{
	struct MCU_CTRL *ctl = PD_GET_MCU_CTRL_PTR(ad->physical_dev);
	...
	if (is_cmd_need_wait) {
		...
		if (!IsComplete) {
			...
			/* check timeout (possibly) caused by SER */
			{
				...
				if ((Lowpart - ad->HwCtrl.ser_times[SER_TIME_ID_T0]) < (CMD_MSG_TIMEOUT * 1000)) {
					MTWF_DBG(ad, DBG_CAT_HW, CATHW_SER, DBG_LVL_WARN,
			...
			if (bDump) {
				MTWF_DBG(ad, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			...
			if (bDump) {
				...
				if (ad->FwCmdTimeoutCnt < FW_CMD_TO_DBG_INFO_PRINT_CNT) {
					if (exp_type == 0) {
						...
						mdelay(15);
#endif /* CONFIG_CONNINFRA_SUPPORT */
						PD_SET_WF_RESET_WM_COUNT(ad->physical_dev, PD_GET_WF_RESET_WM_COUNT(ad->physical_dev) + 1);
...
INT register_fw_cmd_notifier(struct _RTMP_ADAPTER *ad, struct notify_entry *ne)
{
	...
	ret = mt_notify_chain_register(&ctl->fw_cmd_notify_head, ne);
...
INT unregister_fw_cmd_notifier(struct _RTMP_ADAPTER *ad, struct notify_entry *ne)
{
	...
	ret = mt_notify_chain_unregister(&ctl->fw_cmd_notify_head, ne);
	return ret;
}
...
INT call_fw_cmd_notifieriers(INT val, struct _RTMP_ADAPTER *ad, void *msg)
{
	...
	RTMP_QueryPacketInfo(msg, msg_buf, msg_len);
	...
	ret = mt_notify_call_chain(&ctl->fw_cmd_notify_head, val, &info);
	return ret;
}
