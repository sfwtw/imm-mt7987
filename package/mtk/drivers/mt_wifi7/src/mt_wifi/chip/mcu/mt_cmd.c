...
#include "rt_config.h"
	...
	defined(PRE_CAL_BELLWETHER_SUPPORT) || defined(PRE_CAL_MT7990_SUPPORT) || \
...
INT32 MtCmdEepromAccessRead(RTMP_ADAPTER *pAd, UINT32 offset,
						PUCHAR pData, PUINT pIsValid, UINT32 DataLen)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEepromAccessRead(pAd, offset, pData, pIsValid, DataLen);
#endif /* WIFI_UNIFIED_COMMAND */
...
VOID MtCmdEepromAccessWrite(RTMP_ADAPTER *pAd, UINT32 offset,
						PUCHAR pData, UINT32 DataLen)
{
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdEepromAccessWrite(pAd, offset, pData, DataLen);
#endif
}
						   ...
						   PUCHAR pData, PUINT pIsValid)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEfuseAccessRead(pAd, offset, pData, pIsValid);
#else
	struct cmd_msg *msg;
...
{
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdEfuseAccessWrite(pAd, offset, pData);
#else
	struct cmd_msg *msg;
	CMD_ACCESS_EFUSE_T CmdAccessEfuse;
	int ret = 0;
...
INT32 MtCmdEfuseFreeBlockCount(struct _RTMP_ADAPTER *pAd, PVOID GetFreeBlock, PVOID Result)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEfuseFreeBlockCount(pAd, GetFreeBlock, Result);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	UINT8 Version = 0;
...
INT32 MtCmdSmeshConfigSet(RTMP_ADAPTER *pAd, UCHAR *pdata, P_EXT_EVENT_SMESH_T prSmeshResult)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "ucBand = %d, ucAccessMode = %d, ucSmeshEn = %d, fgSmeshRxA2 = %d, fgSmeshRxA1 = %d, fgSmeshRxData = %d, fgSmeshRxMgnt = %d, fgSmeshRxCtrl = %d\n",
			  pconfig_smesh->ucBand,
...
static INT32 MtCmdRfTest(RTMP_ADAPTER *pAd, struct _UNI_CMD_TEST_CTRL_T TestCtrl,
	char *rsp_payload, uint16_t rsp_len)
{
	INT32 ret = 0;
	uint64_t u8TestCtrlFeature = UNI_CMD_RF_TEST_CTRL_FEATURE;
	struct _UNI_RSP_T rRsp = {0};

	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"Action = %d\n", TestCtrl.ucAction);

	rRsp.u2RspSize = rsp_len;
	rRsp.pcRspData = rsp_payload;
	ret = UniCmdTestmodeCtrl(pAd, (struct _UNI_CMD_TEST_CTRL_T *)&TestCtrl, u8TestCtrlFeature, &rRsp);
...
#else
	struct _UNI_CMD_TEST_CTRL_T TestCtrl;
#endif
...
INT32 MtCmdGetTestEngine(RTMP_ADAPTER *pAd,
	UINT32 id, UINT32 data, UINT32 *ret_data)
{
	INT32 ret = 0;
#ifndef WIFI_UNIFIED_COMMAND
	CMD_TEST_CTRL_T TestCtrl;
	...
	os_zero_mem(&TestCtrl, sizeof(TestCtrl));
	TestCtrl.ucAction = ACTION_GET_ENGINE;
	TestCtrl.u.rRfATInfo.u4FuncIndex = id;
	TestCtrl.u.rRfATInfo.Data.u4FuncData = data;

	ret = MtCmdRfTest(pAd, TestCtrl, payload, 4);

	if (ret_data) {
		*ret_data = le2cpu32(*(UINT32 *) payload);
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
				"ret Data: 0x%x\n", *ret_data);
	} else {
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"error ret_data is NULL\n");
	}
...
INT32 MtCmdSetTestEngine(RTMP_ADAPTER *pAd,
...
#endif
	os_zero_mem(&TestCtrl, sizeof(TestCtrl));
	TestCtrl.ucAction = ACTION_SET_ENGINE;
	TestCtrl.u.rRfATInfo.u4FuncIndex = id;
	TestCtrl.u.rRfATInfo.Data.u4FuncData = data;

	ret = MtCmdRfTest(pAd, TestCtrl, NULL, 4);
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"ret: 0x%x, id: %u, Data: 0x%x\n",
		ret, id, TestCtrl.u.rRfATInfo.Data.u4FuncData);
...
#ifdef WIFI_UNIFIED_COMMAND
	Status = UniCmdPhyIcsStart(pAd, pData);
#else
	...
	IN UINT16 rsp_len)
{
	...
	msg = MtAndesAllocCmdMsg(pAd, sizeof(EXT_CMD_SPECTRUM_CTRL_T));
	...
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, rsp_payload);
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)SpectrumCtrl, sizeof(EXT_CMD_SPECTRUM_CTRL_T));
	Status = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, (Status != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_INFO,
			"(Status = %d)\n", Status);
	...
	IN UINT8 *pData)
{
	INT32 Status;
#ifdef WIFI_UNIFIED_COMMAND
	Status = UniCmdWifiSpectrumStart(pAd, pData);
#else
...
error:
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, (Status != CAP_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_INFO,
			"(Status = %d)\n", Status);
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	UINT32 CapDone = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdWifiSpectrumCapStatus(pAd, (INT8 *)&CapDone, SPECTRUM_WAIT_RESP_TIME);
#else
	EXT_CMD_SPECTRUM_CTRL_T SpectrumCtrl;
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	/* Initialization of Spectrum status */
	Status = pAd->SpectrumStatus = CAP_BUSY;
	...
	IN UINT8 *pData)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "\n"
			...
			prICapInfo->u4BW, prICapInfo->u4MACTriggerEvent, prICapInfo->u4SourceAddressLSB,
			prICapInfo->u4SourceAddressMSB, prICapInfo->u4BandIdx, prICapInfo->u4PhyIdx,
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	TestCtrl.ucAction = cpu2le32(ACTION_IN_RFTEST);
	TestCtrl.u.rRfATInfo.u4FuncIndex = cpu2le32(GET_ICAP_CAPTURE_STATUS);
	TestCtrl.ucIcapAttr = cpu2le32(ATTRIBUTE_ICAP_GET);
	...
	IN	UINT8	FType)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"<SetADC> Structure parser Checking Log\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "--------------------------------------------------------------\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
	...
	IN	UINT8	AntIndex)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"<SetRxGain> Structure parser Checking Log\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "--------------------------------------------------------------\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"LPFG = %d, LNA = %d, DbdcIdx = %d, AntIndex= %d\n",
	...
	IN	UINT8	DbdcIdx)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"<SetTTG> Structure parser Checking Log\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "--------------------------------------------------------------\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"ChannelFreq = %d, ToneFreq = %d, TTGPwrIdx = %d, DbdcIdx= %d\n",
	...
	IN	UINT8	AntIndex)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"<SetTTGOnOff> Structure parser Checking Log\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "--------------------------------------------------------------\n");
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"TTGEnable = %d, DbdcIdx = %d, AntIndex = %d\n",
...
INT32 MtCmdChannelSwitch(RTMP_ADAPTER *pAd, MT_SWITCH_CHANNEL_CFG SwChCfg)
{
	...
	if (SwChCfg.Bw == BW_320)
		CmdChanSwitch.ucCentralCh = SwChCfg.eht_cen_ch;
	else
#endif /* DOT11_EHT_BE */
	...
	if (ATE_ON(pAd)) {
		...
		for (ant_seq = 0 ; ant_seq < GET_MAX_PATH(MCS_NSS_CAP(pAd), SwChCfg.BandIdx, 1) ; ant_seq++)
	...
	if ((pAd->CommonCfg.ucTxPowerPercentage > 90) &&
		...
		TxPowerDrop = 0;
			 ...
			 (pAd->CommonCfg.ucTxPowerPercentage <= 90))  /* reduce Pwr for 1 dB. */
		TxPowerDrop = 1;
	else if ((pAd->CommonCfg.ucTxPowerPercentage > 30) &&
			 (pAd->CommonCfg.ucTxPowerPercentage <= 60))  /* reduce Pwr for 3 dB. */
		TxPowerDrop = 3;
	else if ((pAd->CommonCfg.ucTxPowerPercentage > 15) &&
			 (pAd->CommonCfg.ucTxPowerPercentage <= 30))  /* reduce Pwr for 6 dB. */
		TxPowerDrop = 6;
	else if ((pAd->CommonCfg.ucTxPowerPercentage > 9) &&
			 (pAd->CommonCfg.ucTxPowerPercentage <= 15))   /* reduce Pwr for 9 dB. */
		TxPowerDrop = 9;
	else if ((pAd->CommonCfg.ucTxPowerPercentage > 0) &&
			 (pAd->CommonCfg.ucTxPowerPercentage <= 9))   /* reduce Pwr for 12 dB. */
		TxPowerDrop = 12;

	CmdChanSwitch.cTxPowerDrop = TxPowerDrop;
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, " TxPowerDrop = 0x%x, ucTxPath:%u\n", CmdChanSwitch.cTxPowerDrop, ucTxPath);
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, " fg5Gband = 0x%x\n", fg5Gband);

	for (SKUIdx = 0; SKUIdx < SKU_TOTAL_SIZE; SKUIdx++)
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "CmdChanSwitch.aucTxPowerSKU[%d]: 0x%x\n", SKUIdx,
				 CmdChanSwitch.acTxPowerSKU[SKUIdx]);

	os_move_mem(pAd->TxPowerSKU, CmdChanSwitch.acTxPowerSKU, SKU_SIZE);

	for (SKUIdx = 0; SKUIdx < SKU_SIZE; SKUIdx++)
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "pAd->TxPowerSKU[%d]: 0x%x\n", SKUIdx,
	...
	{
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_WARN,
		...
		if (SwChCfg.ap_bw) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "AP BW = %d, AP central_chn = %d\n", SwChCfg.ap_bw, SwChCfg.ap_central_channel);
		}
	}
...
INT MtCmdSetTxRxPath(struct _RTMP_ADAPTER *pAd, MT_SWITCH_CHANNEL_CFG SwChCfg)
{
	...
	CmdChanSwitch.ucAPBW = SwChCfg.ap_bw;
		...
		char *data, UINT16 len)
{
	...
	RegPair = Start;
	for (Index = 0; Index < Num; Index++) {
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "0x%08x=0x%08x\n", RegPair->Register, RegPair->Value);
		RegPair++;
	}
#endif /* defined(INTERNAL_CAPTURE_SUPPORT) || defined(WIFI_SPECTRUM_SUPPORT) */
}
...
static VOID MtCmdGetTxPowerRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "Power: 0x%x Band: %d\n",
...
INT32 MtCmdSetTxPowerCtrl(RTMP_ADAPTER *pAd, ATE_TXPOWER TxPower)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetTxPowerCtrl(pAd, (UINT8)TxPower.Dbdc_idx, (INT8)TxPower.Power, (UINT8)TxPower.Ant_idx, (UINT8)TxPower.Channel);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdGetTxPower(RTMP_ADAPTER *pAd, UINT8 u1DbDcIdx, UINT8 u1CenterCh, UINT8 u1AntIdx, P_EXT_EVENT_ID_GET_TX_POWER_T prTxPwrResult)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdGetTxPower(pAd, u1DbDcIdx, u1CenterCh, u1AntIdx, prTxPwrResult);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdSetForceTxPowerCtrl(RTMP_ADAPTER *pAd, UINT8 ucBandIdx, INT8 cTxPower, UINT8 ucPhyMode, UINT8 ucTxRate, UINT8 ucBW)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetForceTxPowerCtrl(pAd, ucBandIdx, cTxPower, ucPhyMode, ucTxRate, ucBW);
#else
	struct cmd_msg *msg;
	...
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"Band(%d), TxMode(%d), MCS(%d), BW(%d), TxPower(%d)\n",
					 ...
					 UINT8 RFDiffTemp, UINT8 HiBBPHT, UINT8 HiBBPNT, INT8 LoBBPLT, INT8 LoBBPNT)
{
	...
	Thermo.ucThermoSetting[0].ucValue =
		ph_dev->EEPROMImage[Thermo.ucThermoSetting[0].u2Addr];
	Thermo.ucThermoSetting[1].u2Addr = 0x54;
	Thermo.ucThermoSetting[1].ucValue =
		ph_dev->EEPROMImage[Thermo.ucThermoSetting[1].u2Addr];
	Thermo.ucThermoSetting[2].u2Addr  = 0x55;
	Thermo.ucThermoSetting[2].ucValue =
		ph_dev->EEPROMImage[Thermo.ucThermoSetting[2].u2Addr];
#ifdef CFG_BIG_ENDIAN
...
INT32 MtCmdFwLog2Host(RTMP_ADAPTER *pAd, UINT8 McuDest, UINT8 FWLog2HostCtrl)
{
	...
	if (McuDest < 2) {
		if (McuDest == 0)
			...
			host_2_mcu = HOST2CR4;
		SET_CMD_ATTR_MCU_DEST(attr, host_2_mcu);
		SET_CMD_ATTR_TYPE(attr, EXT_CID);
	...
	UINT_8        u1OwnMacIdx)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfAidSetting(pAd, u2Aid, u1BandIdx, u1OwnMacIdx);
#else
	struct cmd_msg *msg;
	...
	UINT_8       ucCmmWlanId)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfApClientCluster(pAd, u2WlanIdx, (UINT16)ucCmmWlanId);
#else
	struct cmd_msg *msg;
	...
	UINT8        u1PhaseOutRptVersion)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdITxBfPhaseCal(pAd, ucGroup, ucGroup_L_M_H, ucDbdcBandIdx, ucPhaseCalType,
							  ucPhaseVerifyLnaGainLevel, u1BandIdx, u1PhaseOutRptVersion);
#else
	struct cmd_msg *msg;
	EXT_CMD_ITXBf_PHASE_CAL_CTRL_T aucIBfPhaseCal;
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "Enable iBF phase calibration : ucGroup = %d, ucGroup_L_M_H = %d, ucDbdcBandIdx = %d\n",
			  ucGroup, ucGroup_L_M_H, ucDbdcBandIdx);
	...
	UINT8		u1BandIdx)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfLnaGain(pAd, ucLnaGain, u1BandIdx);
#else
	struct cmd_msg *msg;
	UCHAR aucCmdBuf[4];
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"LNA gain setting for iBF phase calibration : %d\n",
			 ucLnaGain);
	...
	BOOLEAN      fgDisComp)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdITxBfPhaseComp(pAd, ucBW, ucBand, ucDbdcBandIdx, ucGroup, fgRdFromE2p, fgDisComp);
#else
	struct cmd_msg *msg;
	...
	UINT8 config_para[])
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmd_txbf_config(pAd, config_type, &config_para[0]);
#else
	struct cmd_msg *msg;
	...
	BOOLEAN       fgPhaseCali)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfTxApplyCtrl(pAd, u2WlanId, fgETxBf, fgITxBf, fgMuTxBf, fgPhaseCali);
#else
	struct cmd_msg *msg;
	...
	PUCHAR        pwlanidx)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfSoundingPeriodicTriggerCtrl(pAd, SndgEn, u4SNDPeriod, ucSu_Mu, ucMuNum, pwlanidx);
#else
	struct cmd_msg *msg;
	...
	UINT16        u2WlanId)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdPfmuMemAlloc(pAd, ucSu_Mu, u2WlanId);
#else
	struct cmd_msg *msg;
	UCHAR aucCmdBuf[4];
	struct _CMD_ATTRIBUTE attr = {0};
	...
	UINT16       u2WlanId)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdPfmuMemRelease(pAd, u2WlanId);
#else
	struct cmd_msg *msg;
	UCHAR aucCmdBuf[4];
	struct _CMD_ATTRIBUTE attr = {0};
	...
	RTMP_ADAPTER *pAd)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdPfmuMemAllocMapRead(pAd);
#else
	struct cmd_msg *msg;
	UCHAR aucCmdBuf[4];
	struct _CMD_ATTRIBUTE attr = {0};
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileTagRead(pAd, PfmuIdx, fgBFer, BandIdx);
#else
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileTagWrite(pAd, prPfmuTag1, prPfmuTag2, tag1_len, tag2_len, PfmuIdx, BandIdx);
#else
	msg = MtAndesAllocCmdMsg(pAd, sizeof(ETxBfPfmuProfileTag));
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileDataRead(pAd, PfmuIdx, fgBFer, subCarrIdx, BandIdx);
#else
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileDataWrite(pAd, PfmuIdx, SubCarrIdx, pProfileData, BandIdx);
#else
	if (!pProfileData) {
		ret = NDIS_STATUS_INVALID_DATA;
		goto error;
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuFullDimDataWrite(pAd, PfmuIdx, SubCarrIdx, bfer, pProfileData, DataLength, BandIdx);
#else
	if (!pProfileData) {
		ret = NDIS_STATUS_INVALID_DATA;
		goto error;
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileDataWrite20MAll(pAd, PfmuIdx, BandIdx, pProfileData);
#else
...
INT32 CmdETxBfPfmuProfileDataWrite20MAll_5x5(
	RTMP_ADAPTER     *pAd,
	UCHAR            PfmuIdx,
	PUCHAR           pProfileData)
{
...
#ifndef WIFI_UNIFIED_COMMAND
	struct cmd_msg *msg;
	EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_5X5_ALL_T ETxBfPfmuProfileData;
	struct _CMD_ATTRIBUTE attr = {0};
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPfmuProfileDataWrite20MAll_5x5(pAd, PfmuIdx, BandIdx, pProfileData);
#else
	msg = MtAndesAllocCmdMsg(pAd, sizeof(EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_5X5_ALL_T));
	...
	os_zero_mem(&ETxBfPfmuProfileData, sizeof(EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_5X5_ALL_T));
	ETxBfPfmuProfileData.ucPfmuProfileFormatId = BF_PROFILE_WRITE_20M_5X5_ALL;
	ETxBfPfmuProfileData.ucPfmuIdx			   = PfmuIdx;
	os_move_mem(ETxBfPfmuProfileData.ucBuf, pProfileData, 640);
	ETxBfPfmuProfileData.ucBandIdx = BandIdx;
	...
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)&ETxBfPfmuProfileData,
						sizeof(EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_5X5_ALL_T));
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "(ret = %d)\n", ret);
#endif
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfFbRptDbgInfo(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfTxSndInfo(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"ucInf=0x%02X, fgMan=0x%02X, ucAcQ=0x%02X, fgSxnProtect:0x%02X\n",
			prTxSndCmd->ucInf,
			prTxSndCmd->fgMan,
			prTxSndCmd->ucAcQ,
			prTxSndCmd->fgSxnProtect);

	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"fgDirectFBK=0x%02X\n", prTxSndCmd->fgDirectFBK);
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfPlyInfo(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfTxCmd(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfSndCnt(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
...
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfCfgBfPhy(pAd, prTxBfCfgBfPhy->ucAction, prTxBfCfgBfPhy->ucBandIdx, prTxBfCfgBfPhy->ucSmthIntlBypass);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PUINT8 pucData)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdHeRaMuMetricInfo(pAd, pucData);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	UINT16        u2WlanId)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdETxBfStaRecRead(pAd, u2WlanId);
#else
	struct cmd_msg *msg;
	CHAR ucCmd[8];
	struct _CMD_ATTRIBUTE attr = {0};
	...
	BOOLEAN       fgIBf)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfHwEnableStatusUpdate(pAd, fgEBf, fgIBf);
#else
	struct cmd_msg *msg;
	...
	UINT8 u1BfSelBand[])
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfModuleEnCtrl(pAd, u1BfNum, u1BfBitmap, u1BfSelBand);
#else
	struct cmd_msg *msg;
	...
	BOOLEAN       fgBfeeHwEn)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfeeHwCtrl(pAd, fgBfeeHwEn);
#else
	struct cmd_msg *msg;
	...
	EXT_CMD_ETXBF_PFMU_SW_TAG_T rEBfPfmuSwTag)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_INFO, "(ret = %d)\n", ret);
	return ret;
}
	...
	PUINT8 pucData)
{
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "u2Action=0x%02X\n",
			prMecCtrlCmd->u2Action);
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u2WlanIdx=0x%02X, u1AmsduAlgoEn=0x%02X\n",
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u1BaNum=0x%02X, u1AmsduNum=0x%02X, u2AmsduRateThr=0x%02X\n",
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "u2WlanIdx=0x%02X\n",
		prMecCtrlCmd->mecCmdPara.mec_set_amsdu_max_size_t.u2WlanIdx);

	for (u1Idx = 0 ; u1Idx < sizeof(CMD_MEC_CTRL_CMD_T) ; u1Idx++) {
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "[%u]: %u\n", u1Idx, pu1Ptr[u1Idx]);
	}
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_INFO,
			 "(ret = %d)\n", ret);
	return ret;
}
...
static VOID CmdEfuseBufferRDRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG,
			 "Len = 0x%x\n",
			  Len);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG,
			 "EventExtCmdResult->ucSourceMode = 0x%x, 0x%x\n",
			  EventExtCmdResult->ucSourceMode, &EventExtCmdResult->ucSourceMode);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG,
			 "EventExtCmdResult->ucContentFormat = 0x%x, 0x%x\n",
			  EventExtCmdResult->ucContentFormat, &EventExtCmdResult->ucContentFormat);
	EventExtCmdResult->u2Offset = le2cpu16(EventExtCmdResult->u2Offset);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG,
			 "EventExtCmdResult->u2Offset = 0x%x, 0x%x\n",
			  EventExtCmdResult->u2Offset, &EventExtCmdResult->u2Offset);
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG,
			 "EventExtCmdResult->ucCount = 0x%x, 0x%x\n",
			  EventExtCmdResult->u2Count, &EventExtCmdResult->u2Count);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_EFUSE, DBG_LVL_DEBUG, "EEProm Content: 0x%x\n\r",
...
{
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdEfuseBufferModeSet(pAd, EepromType);
#else
	struct cmd_msg *msg = NULL;
...
{
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdEfusBufferModeGet(pAd, EepromType, dump_offset, dump_size, epprom_content);
#else
	struct cmd_msg *msg = NULL;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtect(pAd, ucBand, HighEn, HighTempTh, LowEn, LowTempTh, RechkTimer, RFOffEn, RFOffTh, ucType);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectAdmitDutyInfo(pAd);
#else
	struct cmd_msg *msg;
	...
	UINT16 recheck_time)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectEnable(pAd, band_idx, protection_type,
	...
	UINT8 trigger_type)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectDisable(pAd, band_idx,
	...
	UINT8 duty)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectDutyCfg(pAd, band_idx,
			level_idx, duty);
#else
	struct cmd_msg *msg;
	...
	struct THERMAL_PROTECT_MECH_INFO *info_buf)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectInfo(pAd, band_idx);
#else
	struct cmd_msg *msg;
	...
	struct THERMAL_PROTECT_DUTY_INFO *info_buf)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectDutyInfo(pAd, band_idx);
#else
	struct cmd_msg *msg;
	...
	UINT8 state)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalProtectStateAct(pAd, band_idx,
...
INT32 MtCmdGetThermalSensorResult(RTMP_ADAPTER *pAd, UINT8 ActionIdx, UINT8 ucDbdcIdx, UINT32 *SensorResult)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalGetTemp(pAd, ActionIdx, ucDbdcIdx, SensorResult);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdGetThermalTempAdcResult(
	struct _RTMP_ADAPTER *pAd,
	UINT8  sensorType,
	UINT8  u1BandIdx,
	UINT8 *pu4SensorResult)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalGetAdcTemp(pAd, sensorType, u1BandIdx, pu4SensorResult);
#endif
	return ret;
}
...
static VOID MtCmdGetTxStatisticRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	if (prTxStat->ucField & GET_TX_STAT_TOTAL_TX_CNT) {
		...
		struct _EXT_EVENT_TX_STATISTIC_BAND_RESULT_T *p_event_data =
		(struct _EXT_EVENT_TX_STATISTIC_BAND_RESULT_T *)(p_event_hdr->aucTxStatisticResult);
	...
	if (prTxStat->ucField & GET_TX_STAT_ENTRY_TX_RATE) {
		...
		struct _EXT_EVENT_TX_STATISTIC_WLAN_RATE_RESULT_T *p_event_data =
		(struct _EXT_EVENT_TX_STATISTIC_WLAN_RATE_RESULT_T *)(p_event_hdr->aucTxStatisticResult);
	...
	if (prTxStat->ucField & GET_TX_STAT_ENTRY_TX_PER) {
		struct _EXT_EVENT_TX_STATISTIC_WLAN_PER_RESULT_T *p_event_data =
		(struct _EXT_EVENT_TX_STATISTIC_WLAN_PER_RESULT_T *)(p_event_hdr->aucTxStatisticResult);
	...
	UINT16 u2Wcid, P_EXT_EVENT_TX_STATISTIC_RESULT_T prTxStatResult)
{
	...
	while (ucField) {
		if (ucField & GET_TX_STAT_TOTAL_TX_CNT) {
			UniCmdStateGetTotalTxCnt(pAd, ucBand, (UINT32 *)prTxStatResult);
			ucField = ucField & (~GET_TX_STAT_TOTAL_TX_CNT);
		} else if (ucField & GET_TX_STAT_LAST_TX_RATE) {
			UniCmdStateGetLastTxRate(pAd, ucBand, (UINT32 *)prTxStatResult);
			ucField = ucField & (~GET_TX_STAT_LAST_TX_RATE);
		} else if (ucField & GET_TX_STAT_ENTRY_TX_RATE) {
			UniCmdPerStaGetTxRate(pAd, &u2Wcid, 1, 0, (UINT32 *)prTxStatResult);
			ucField = ucField & (~GET_TX_STAT_ENTRY_TX_RATE);
		} else if (ucField & GET_TX_STAT_ENTRY_TX_CNT) {
			UniCmdPerStaGetTxCnt(pAd, &u2Wcid, 1, 0, (UINT32 *)prTxStatResult);
			ucField = ucField & (~GET_TX_STAT_ENTRY_TX_CNT);
		} else
			ucField = 0;
	}
#else
	...
	struct _RTMP_ADAPTER *ad, TX_STAT_STRUC *p_buff, UCHAR num)
{
	...
	os_alloc_mem_suspend(ad, (UCHAR **)&pu2Wcid, sizeof(*pu2Wcid) * num);
	...
	/* If Field is different for any Wcid, then there will be issue */
	for (i = 0; i < num; i++) {
		p_temp = p_buff + i;
		ucField = p_temp->Field;
		pu2Wcid[i] = p_temp->Wcid;
	}

	if (ucField & GET_TX_STAT_ENTRY_TX_RATE)
		UniCmdPerStaGetTxRate(ad, pu2Wcid, num, 0, NULL);

	if (ucField & GET_TX_STAT_ENTRY_TX_CNT)
		UniCmdPerStaGetTxCnt(ad, pu2Wcid, num, 0, NULL);

	if (ucField & GET_TX_STAT_ENTRY_TX_RETRY_CNT)
		UniCmdAllStaInfoGetInfo(ad, UNI_CMD_ALL_STA_DATA_TX_RETRY_COUNT);
...
static VOID MtCmdMutiGetTxStatisticRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	if (field & GET_TX_STAT_ENTRY_TX_CNT) {
		...
		num = (Len - sizeof(struct _EXT_EVENT_TX_STATISTIC_RESULT_HEADER_T)) / sizeof(struct _EXT_EVENT_TX_STATISTIC_WLAN_CNT_RESULT_T);
		...
		num = (Len - sizeof(struct _EXT_EVENT_TX_STATISTIC_RESULT_HEADER_T)) / sizeof(struct _EXT_EVENT_TX_STATISTIC_WLAN_RATE_RESULT_T);
		...
		num = (Len - sizeof(struct _EXT_EVENT_TX_STATISTIC_RESULT_HEADER_T)) / sizeof(struct _EXT_EVENT_TX_STATISTIC_WLAN_PER_RESULT_T);
...
INT32 MtCmdGetMutiTxStatistic(struct _RTMP_ADAPTER *pAd, TX_STATISTIC_RESULT_PAIR *TxStatResultPair, UINT32 Num)
{
	...
	/* If Field is different for any Wcid, then there will be issue */
	for (index = 0; index < Num; index++) {
		ucField = TxStatResultPair[index].ucField;
		pu2Wcid[index] = TxStatResultPair[index].u2WlanIdx;
	}

	if (ucField & GET_TX_STAT_ENTRY_TX_RATE)
		UniCmdPerStaGetTxRate(pAd, pu2Wcid, Num, 1, (UINT32 *)TxStatResultPair);

	if (ucField & GET_TX_STAT_ENTRY_TX_CNT)
		UniCmdPerStaGetTxCnt(pAd, pu2Wcid, Num, 1, (UINT32 *)TxStatResultPair);
...
INT32 MtCmdMuarConfigSet(RTMP_ADAPTER *pAd, UCHAR *pdata)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "ucMuarModeSel = %d, ucForceClear = %d, ucEntryCnt = %d, ucAccessMode = %d\n",
			  pconfig_muar->ucMuarModeSel,
...
INT32 MtCmdFdFrameOffloadSet(RTMP_ADAPTER *pAd, P_CMD_FRAME_OFFLOAD_T fdFrame_offload)
{
	struct cmd_msg *msg;
	INT32 ret = 0, size = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	size = sizeof(CMD_FRAME_OFFLOAD_T);
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
...
#if (NEW_MCU_INIT_CMD_API)
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u4CtrlFieldID = 0x%x, ExtCmd (0x%02x)\n",
			 EventExtCmdResult->u4CtrlFieldID, msg->attr.ext_type);
#else
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u4CtrlFieldID = 0x%x, ExtCmd (0x%02x)\n",
			 EventExtCmdResult->u4CtrlFieldID, msg->ext_cmd_type);
#endif /* NEW_MCU_INIT_CMD_API */
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "ucCtrlStatus = 0x%x\n",
			 EventExtCmdResult->ucCtrlStatus);
}
...
static VOID MtCmdSetVoWGroupCtrlRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u4CtrlFieldID = 0x%x, ExtCmd (0x%02x)\n",
			 le2cpu32(EventExtCmdResult->u4CtrlFieldID), msg->attr.ext_type);
#else
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u4CtrlFieldID = 0x%x, ExtCmd (0x%02x)\n",
			 le2cpu32(EventExtCmdResult->u4CtrlFieldID), msg->ext_cmd_type);
#endif /* NEW_MCU_INIT_CMD_API */
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "ucCtrlStatus = 0x%x\n",
			 EventExtCmdResult->ucCtrlStatus);
}
...
#endif /* NEW_MCU_INIT_CMD_API */
#if (NEW_MCU_INIT_CMD_API)
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "ExtCmd = 0x%x\n",
			 msg->attr.ext_type);
#else
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "ExtCmd = 0x%x\n",
			 msg->ext_cmd_type);
...
#endif /* NEW_MCU_INIT_CMD_API */
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "Get fail!\n");
		else
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "Set fail!\n");

		if (EventExtCmdResult->u2IfApplyBss_0_to_16_CtrlFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2Bss_0_to_16_CtrlValue = 0x%0x\n",
					 EventExtCmdResult->u2Bss_0_to_16_CtrlValue);
		}

		if (EventExtCmdResult->u2IfApplyRefillPerildFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2RefillPerildValue = 0x%0x\n",
					 EventExtCmdResult->u2RefillPerildValue);
		}

		if (EventExtCmdResult->u2IfApplyDbdc1SearchRuleFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2Dbdc1SearchRuleValue = 0x%0x\n",
					 EventExtCmdResult->u2Dbdc1SearchRuleValue);
		}

		if (EventExtCmdResult->u2IfApplyDbdc0SearchRuleFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2Dbdc0SearchRuleValue = 0x%0x\n",
					 EventExtCmdResult->u2Dbdc0SearchRuleValue);
		}

		if (EventExtCmdResult->u2IfApplyEnTxopNoChangeBssFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2EnTxopNoChangeBssValue = 0x%0x\n",
					 EventExtCmdResult->u2EnTxopNoChangeBssValue);
		}

		if (EventExtCmdResult->u2IfApplyAirTimeFairnessFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2AirTimeFairnessValue = 0x%0x\n",
					 EventExtCmdResult->u2AirTimeFairnessValue);
		}

		if (EventExtCmdResult->u2IfApplyEnbwrefillFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2EnbwrefillValue = 0x%0x\n",
					 EventExtCmdResult->u2EnbwrefillValue);
		}

		if (EventExtCmdResult->u2IfApplyEnbwCtrlFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2EnbwCtrlValue = 0x%0x\n",
					 EventExtCmdResult->u2EnbwCtrlValue);
		}

		if (EventExtCmdResult->u2IfApplyBssCheckTimeToken_0_to_16_CtrlFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2BssCheckTimeToken_0_to_16_CtrlValue = 0x%0x\n",
					 EventExtCmdResult->u2BssCheckTimeToken_0_to_16_CtrlValue);
		}

		if (EventExtCmdResult->u2IfApplyBssCheckLengthToken_0_to_16_CtrlFlag) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"u2BssCheckLengthToken_0_to_16_CtrlValue = 0x%0x\n",
					 EventExtCmdResult->u2BssCheckLengthToken_0_to_16_CtrlValue);
		}
	}

	/* need to ask FW to add ExtendCID and CtrlStatus */
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "i am here~\n");
}
...
static VOID MtCmdSetVoWRxAirtimeCtrlRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			"u4CtrlFieldID = 0x%x, ExtCmd (0x%02x)\n",
			 EventExtCmdResult->u4CtrlFieldID, msg->ext_cmd_type);
#endif /* NEW_MCU_INIT_CMD_API */
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "u4CtrlGetStatus = 0x%x\n",
			 EventExtCmdResult->u4CtrlGetStatus);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG, "u4CtrlSetStatus = 0x%x\n",
	...
	if (EventExtCmdResult->u4CtrlFieldID == EMUM_RX_AT_REPORT_CTRL) {
		if (EventExtCmdResult->u4CtrlSubFieldID == ENUM_RX_AT_REPORT_SUB_TYPE_RX_NONWIFI_TIME) {
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"Non Wi-Fi for band%d = 0x%x\n",
					 EventExtCmdResult->rRxAtGeneralCtrl.rRxAtReportSubCtrl.ucRxNonWiFiBandIdx,
			...
			MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
					"OBSS for band%d = 0x%x\n",
					 EventExtCmdResult->rRxAtGeneralCtrl.rRxAtReportSubCtrl.ucRxObssBandIdx,
...
INT32 MtCmdSetVoWGroupCtrl(struct _RTMP_ADAPTER *pAd, struct _EXT_CMD_BSS_CTRL_T *param)
{
	...
	error:
		...
		if (ret == NDIS_STATUS_FAILURE) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_ERROR, "(rty = %u)\n", retry_times);
			retry_times++;
			if (retry_times <= 2)
				goto retry;
		}
...
INT32 MtCmdSetVoWCounterCtrl(struct _RTMP_ADAPTER *pAd, UCHAR cmd, UCHAR val)
{
	...
	param[0] = cmd;
	param[1] = val;
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "cmd = %d, val = %d)\n",
			 cmd, val);
...
INT32 MtCmdSetDscpPri(struct _RTMP_ADAPTER *pAd, UINT8 inf_idx, INT if_type)
{
#ifdef MT7993_FPGA
	/* TODO */
	...
	if (if_type == INT_APCLI) {
		dscp.bss_id = pAd->StaCfg[inf_idx].wdev.bss_info_argument.ucBssIndex;
#ifdef MAP_R5
		dscp.dscp_pri_enable = pAd->StaCfg[inf_idx].dscp_pri_map_enable;
		NdisCopyMemory(dscp.dscpPriMap, pAd->StaCfg[inf_idx].dscp_pri_map, 64);
#endif
	} else {
		dscp.bss_id = pAd->ApCfg.MBSSID[inf_idx].wdev.bss_info_argument.ucBssIndex;
		dscp.dscp_pri_enable = pAd->ApCfg.MBSSID[inf_idx].dscp_pri_map_enable;
		NdisCopyMemory(dscp.dscpPriMap, pAd->ApCfg.MBSSID[inf_idx].dscp_pri_map, 64);
	}
	...
	if (pAd->cp_support_off_by_qos_map)
						  ...
						  UINT8 SifsTime, UINT8 RifsTime, UINT16 EifsTime, UCHAR BandIdx)
{
	...
	struct _EXT_CMD_ATE_TEST_MODE_T ATE_param;
#else
	struct _UNI_CMD_TEST_TR_PARAM_CTRL_T ATE_param;
...
#else
	ATE_param.ucAction = ENUM_TR_PARAM_SET;
	u8TestTrParamFeature = UNI_ATE_SLOT_TIME_FEATURE;
#endif /*WIFI_UNIFIED_COMMAND*/
	...
	ret = MtCmdATETest(pAd, &ATE_param);
#else
	ret = UniCmdATETest(pAd, &ATE_param, u8TestTrParamFeature, ucParamNum, NULL);
#endif /*WIFI_UNIFIED_COMMAND*/
	return ret;
}
...
#else
	ATE_param.ucAction = ENUM_TR_PARAM_SET;
	u8TestTrParamFeature = UNI_ATE_POWER_PERCENTAGE_ON_OFF_FEATURE;
#endif
...
#else
	struct _UNI_CMD_TEST_TR_PARAM_CTRL_T ATE_param;
	uint64_t u8TestTrParamFeature = 0;
	uint8_t ucParamNum = 1;
...
#else
	ATE_param.ucAction = ENUM_TR_PARAM_GET;
	u8TestTrParamFeature = UNI_ATE_FREQ_OFFSET_FEATURE;
#endif /*WIFI_UNIFIED_COMMAND*/
	ATE_param.Data.rFreqOffset.ucBandIdx = BandIdx;
#ifdef CFG_BIG_ENDIAN
...
error:
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "(ret = %d)\n", ret);
#else
	ret = UniCmdATETest(pAd, &ATE_param, u8TestTrParamFeature, ucParamNum, pFreqOffsetResult);
#endif /*WIFI_UNIFIED_COMMAND*/
	return ret;
}
...
static VOID MtCmdGetCfgStatRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	case EXT_ATE_GET_TSSI:
		...
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "TSSI Enable:%d Band:%d\n", TSSI_Status->ucEnable,
				 TSSI_Status->ucBand);
		break;

	case EXT_ATE_GET_DPD:
		os_move_mem(msg->attr.rsp.wb_buf_in_calbk, &DPD_Status->ucEnable, sizeof(DPD_Status->ucEnable));
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "DPD Enable:%d Band:%d\n", DPD_Status->ucEnable,
				 DPD_Status->ucBand);
		break;
	...
	case EXT_ATE_GET_THERNAL_COMPENSATION:
		...
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "THER Enable:%d\n", THER_Status->ucEnable);
		break;
...
INT32 MtCmdGetCfgOnOff(RTMP_ADAPTER *pAd, UINT32 Type, UINT8 Band, UINT32 *Status)
{
...
#else
	case 0:
		u8TestTrParamFeature = UNI_ATE_TSSI_STATUS_FEATURE;
		break;

	case 1:
		u8TestTrParamFeature = UNI_ATE_DPD_STATUS_FEATURE;
		break;

	case 2:
		u8TestTrParamFeature = UNI_ATE_RATE_POWER_OFFSET_FEATURE;
		break;

	case 3:
		u8TestTrParamFeature = UNI_ATE_THERMO_COMP_STATUS_FEATURE;
		break;
#endif
	default:
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_WARN,
			"Not support : type:%d n", Type);
		goto error;
	}
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"type:%d Para:%lld\n",
		Type, u8TestTrParamFeature);

	ATE_param.ucAction = ENUM_TR_PARAM_GET;
	ret = UniCmdATETest(pAd, &ATE_param, u8TestTrParamFeature, ucParamNum, Status);
#endif /*WIFI_UNIFIED_COMMAND*/
...
#else
	ATE_param.ucAction = ENUM_TR_PARAM_SET;
	u8TestTrParamFeature = UNI_ATE_PHY_STATUS_COUNT_FEATURE;
#endif /*WIFI_UNIFIED_COMMAND*/
...
INT32 MtCmdGetAllStaStats(struct _RTMP_ADAPTER *pAd, UINT8 subevent_type)
{
	...
	if (subevent_type == EVENT_PHY_ALL_TX_RX_RATE)
		uni_tag = UNI_CMD_ALL_STA_TX_RATE;
	else if (subevent_type == EVENT_PHY_TX_STAT_PER_WCID)
		uni_tag = UNI_CMD_ALL_STA_TX_STAT;
	else if (subevent_type == EVENT_PHY_TXRX_AIR_TIME)
		uni_tag = UNI_CMD_ALL_STA_TXRX_AIR_TIME;
	else if (subevent_type == EVENT_PHY_RX_STAT)
		uni_tag = UNI_CMD_STATE_RX_STAT;
	else if (subevent_type == EVENT_PHY_TXRX_ADM_STAT)
		uni_tag = UNI_CMD_ALL_STA_TXRX_ADM_STAT;
	else if (subevent_type == EVENT_PHY_TRX_MSDU_COUNT)
		uni_tag = UNI_CMD_ALL_STA_TRX_MSDU_COUNT;
#if defined(TXRX_STAT_SUPPORT) || defined(TR181_SUPPORT)
	else if (subevent_type == EVENT_PHY_TX_DATA_RETRY_COUNT)
		uni_tag = UNI_CMD_ALL_STA_DATA_TX_RETRY_COUNT;
#endif /* TXRX_STAT_SUPPORT */
	else
		return NDIS_STATUS_INVALID_DATA;

	if (subevent_type == EVENT_PHY_RX_STAT)
		UniCmdStateGetRxStat(pAd);
	else
		UniCmdAllStaInfoGetInfo(pAd, uni_tag);
#else
	struct cmd_msg *msg;
...
#ifdef WIFI_UNIFIED_COMMAND
		if (cap->uni_cmd_support)
			return UniCmdRestartDLReqNoRsp(ad);
		else
#endif /* WIFI_UNIFIED_COMMAND */
...
INT32 MtCmdGetWtblTxStat(RTMP_ADAPTER *pAd, UINT32 u4Field, UINT16 ucWcid)
{

	struct cmd_msg *msg;
	INT32 Ret = 0;
	EXT_CMD_GET_WTBL_TX_COUNT_T WtblTxCntCmd = {0};
	struct _CMD_ATTRIBUTE attr = {0};
...
INT32 MtCmdSetDfsTxStart(RTMP_ADAPTER *pAd, UCHAR BandIdx)
{
	...
	if (pChipCap->uni_cmd_support)
			ret = UniCmdSetMacTxRx(pAd, BandIdx, ENUM_MAC_DFS_TXSTART);
	else
#endif /* WIFI_UNIFIED_COMMAND */
	{
		struct cmd_msg *msg;
...
INT32 MtCmdSetGroupPreCal_7915(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_GROUP_PRE_CAL);
#else
	struct cmd_msg *msg;
	...
	)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_TX_DPD_FLATNESS_PRE_CAL);
#else
	struct cmd_msg *msg;
	...
	)
{
	...
	Len = DpdFlatnessCalInfoAlloc_7986(pAd, &pCmdDpdFlatnessCalInfo, idx, length, bSecBw80);
	...
	)
{
	...
	Len = DpdFlatnessCalInfoAlloc_7916(pAd, &pCmdDpdFlatnessCalInfo, idx, length, bSecBw80);
	...
	)
{
	...
	Len = DpdFlatnessCalInfoAlloc_7981(pAd, &pCmdDpdFlatnessCalInfo, idx, length, bSecBw80);
...
INT32 MtCmdSetGroupPreCal_bellwether(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_GROUP_PRE_CAL);
	...
	Len = GroupPreCalInfoAlloc_bellwether(pAd, &pCmdGroupPreCalInfo, idx, length);
...
INT32 MtCmdSetDpdFlatnessCal_bellwether(
	RTMP_ADAPTER *pAd,
	UINT16       idx,
	UINT32       length,
	BOOLEAN      bSecBw80
	)
{
	...
	Len = DpdFlatnessCalInfoAlloc_bellwether(pAd, &pCmdDpdFlatnessCalInfo, idx, length, bSecBw80);
...
INT32 MtCmdSetGroupPreCal_7990(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_GROUP_PRE_CAL, 0);
#else
	struct cmd_msg *msg;
	VOID *pCmdGroupPreCalInfo = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	Len = GroupPreCalInfoAlloc_7990(pAd, &pCmdGroupPreCalInfo, idx, length);
...
INT32 MtCmdSetDpdFlatnessCal_7990(
	RTMP_ADAPTER *pAd,
	UINT16       idx,
	UINT32       length,
	UINT32       eeprom_ofst,
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, u4CalId, eeprom_ofst);
#else
	struct cmd_msg *msg;
	VOID *pCmdDpdFlatnessCalInfo = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	Len = DpdFlatnessCalInfoAlloc_7990(pAd, &pCmdDpdFlatnessCalInfo, idx, length, eeprom_ofst);
...
INT32 MtCmdSetGroupPreCal_7992(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_GROUP_PRE_CAL, 0);
	...
	Len = GroupPreCalInfoAlloc_7992(pAd, &pCmdGroupPreCalInfo, idx, length);
...
INT32 MtCmdSetDpdFlatnessCal_7992(
	RTMP_ADAPTER *pAd,
	UINT16       idx,
	UINT32       length,
	UINT32       eeprom_ofst,
...
)
{
	...
	Len = DpdFlatnessCalInfoAlloc_7992(pAd, &pCmdDpdFlatnessCalInfo, idx, length, eeprom_ofst);
...
INT32 MtCmdSetGroupPreCal_7993(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdSetPreCalSetResult(pAd, idx, length, CAL_ID_GROUP_PRE_CAL, 0);
	...
	Len = GroupPreCalInfoAlloc_7993(pAd, &pCmdGroupPreCalInfo, idx, length);
...
INT32 MtCmdSetDpdFlatnessCal_7993(
	RTMP_ADAPTER *pAd,
	UINT16       idx,
	UINT32       length,
	UINT32       eeprom_ofst,
...
)
{
	...
	Len = DpdFlatnessCalInfoAlloc_7993(pAd, &pCmdDpdFlatnessCalInfo, idx, length, eeprom_ofst);
...
INT32 MtCmdThermalMode(RTMP_ADAPTER *pAd, UINT8 Mode, UINT8 Action)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetCalMode(pAd, Mode, Action);
#else
	struct cmd_msg *msg;
...
INT32 mt_cmd_set_fcc5_min_lpn(RTMP_ADAPTER *pAd, UINT16 min_lpn_update)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetFcc5MinIpn(pAd, min_lpn_update);
#else
	struct cmd_msg *msg;
...
INT32 mt_cmd_set_radar_thres_param(RTMP_ADAPTER *pAd, P_CMD_RDM_RADAR_THRESHOLD_UPDATE_T p_radar_threshold)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetRadarThresParam(pAd, p_radar_threshold);
#else
	struct cmd_msg *msg;
...
INT32 mt_cmd_set_pls_thres_param(RTMP_ADAPTER *pAd, P_CMD_RDM_PULSE_THRESHOLD_UPDATE_T p_pls_threshold)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetPlsThresParam(pAd, p_pls_threshold);
#else
	struct cmd_msg *msg;
	...
	IN UINT8 sw_rdd_log_cond)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetRddLogConfig(pAd, hw_rdd_log_en, sw_rdd_log_en, sw_rdd_log_cond);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdBgndScanNotify(RTMP_ADAPTER *pAd, MT_BGND_SCAN_NOTIFY BgScNotify)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"ucNotifyFunc = %d, ucBgndScanStatus = %d\n",
...
INT32 MtCmdCr4Query(RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 arg1, UINT32 arg2)
{
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_SDO, DBG_LVL_DEBUG,
		 "arg0(%d) arg1(%d) arg2(%d)\n",
		  arg0, arg1, arg2);

#ifdef WIFI_UNIFIED_COMMAND
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_SDO_ONLY))
		return UniCmdGetSdo(pAd, arg0, arg1, arg2, NULL);
#endif
	...
	Ret = chip_cmd_tx(pAd, msg);
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_SDO, DBG_LVL_DEBUG,
			 "(ret = %d)\n", Ret);
	return Ret;
}

INT32 MtCmdCr4MultiQuery(RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 arg1, UINT32 arg2, struct _CR4_QUERY_STRUC *cr4_query_list)
{
	struct cmd_msg *msg;
	INT32 Ret = 0;
...
#ifdef WIFI_UNIFIED_COMMAND
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_SDO_ONLY))
		return UniCmdGetSdo(pAd, arg0, arg1, arg2, cr4_query_list);
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_SDO_ONLY))
		return UniCmdGetSdo(pAd, CR4_QUERY_OPTION_GET_BSS_ACQ_PKT_NUM, u4bssbitmap, 0, NULL);
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_SDO_ONLY))
		return UniCmdSetSdo(pAd, arg0, arg1, arg2, 0, NULL);
#endif /* WIFI_UNIFIED_COMMAND */
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdHeraSupportRateTable(pAd, tx_mode, tx_nss, tx_bw, *mcs_cap, set);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdHeraDbgCtrl(pAd, param_num, param);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPowerSKUCtrl(pAd, tx_pwr_sku_en, BandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCManCtrl(pAd, fgTpcManual);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCEnableCfg(pAd, fgTpcEnable);
#else
	struct cmd_msg *msg;
	CMD_TPC_MAN_CTRL_T TpcCtrl = {0};
	struct _CMD_ATTRIBUTE attr = {0};
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCWlanIdCtrl(pAd, fgUplink, u1EntryIdx, u1EntryIdx, u1DlTxType);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCUlAlgoCtrl(pAd, u1TpcCmd, u1ApTxPwr, u1EntryIdx, u1TargetRssi, u1UPH, fgMinPwrFlag);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCDlAlgoCtrl(pAd, u1TpcCmd, fgCmdCtrl, u1DlTxType, (int8_t)DlTxPwr, u1EntryIdx, DlTxpwrAlpha);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCManTblInfo(pAd, fgUplink);
#else
	struct cmd_msg *msg;
	...
	INT16	i2Value)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCUlUtVarCfg(pAd, u1EntryIdx, u1VarType, i2Value);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTPCUlUtGo(pAd, fgTpcUtGo);
#else
	struct cmd_msg *msg;
	CMD_TPC_UL_UT_CTRL_T TpcCtrl = {0};
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "fgTpcUtGo: %d\n", fgTpcUtGo);
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPowerPercentCtrl(pAd, fgTxPowerPercentEn, BandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPowerDropCtrl(pAd, cPowerDropLevel, BandIdx);
#else
	struct cmd_msg *msg;
	...
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			 "%s: cPowerDropLevel: %d, BandIdx: %d\n", __FUNCTION__, cPowerDropLevel, BandIdx);
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxBfBackoffCtrl(pAd, fgTxBFBackoffEn, BandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPwrShowInfo(pAd, ucTxPowerInfoCatg, ucBandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEapInitIPICtrl(pAd, BandIdx);
#else
...
static VOID ShowEapRaTblInfoCallback(char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	if (pTblentry->u1RW) {
		...
		if (!fname) {
			...
			os_file_open(fname, (RTMP_OS_FD_EXT *)&srcf, O_WRONLY | O_CREAT | O_APPEND, 0);
...
VOID eapEventDispatcher(struct cmd_msg *msg, char *rsp_payload,
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEapGetIPIValue(pAd, BandIdx);
#else
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEapSetDataTxPwrOffset(pAd, WlanIdx, TxPwr_Offset, BandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	CMD_SET_RA_TABLE rSetRaTblParam;
	UINT16 u2Len = (TblLength - sizeof(rSetRaTblParam.ucBuf));

	ret = UniCmdEapSetRaTable(pAd, BandIdx, TblType, TblIndex, TblLength, Buffer, u2Len);
#else
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdEapGetRaTblInfo(pAd, BandIdx, TblType, TblIndex, ReadnWrite);
#else
...
)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_INFO:DBG_LVL_ERROR,
			 "(ret = %d)\n",  ret);
	return ret;
}
	...
	UINT8        u1BandIdx)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdMuPwrCtrl(pAd, fgMuTxPwrManEn, cMuTxPwr, u1BandIdx);
#else
	struct cmd_msg *msg;
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPwrDataPktCtrl(pAd, pEntry->wcid, i1MaxBasePwr, pEntry->PowerOffset, u1BandIdx);
#endif
	return ret;
}
...
)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTxPwrMinDataPktCtrl(pAd, i1MinBasePwr, u1BandIdx);
#endif
	...
	IN UINT8 u1ThermalAdc)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalManCtrl(pAd, u1BandIdx, fgManualMode, u1ThermalAdc);
#else
	struct cmd_msg *msg;
	...
	IN UINT32 u4FuncPtr)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalTaskCtrl(pAd, u1BandIdx, fgTrigEn, fgTrigEn, u4FuncPtr);
#else
	struct cmd_msg *msg;
	...
	)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdThermalBasicInfo(pAd, u1BandIdx);
#else
	struct cmd_msg *msg;
...
INT32 mt_cmd_ext_twt_agrt_update(
	...
	struct MT_TWT_AGRT_PARA_T *agrt_para)
{
	struct cmd_msg *msg;
	struct ext_cmd_twt_agrt_update _ext_cmd_twt_agrt_update;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
#ifdef CFG_BIG_ENDIAN
	int i = 0;
	...
	os_zero_mem(&_ext_cmd_twt_agrt_update,
		sizeof(struct ext_cmd_twt_agrt_update));
	os_move_mem((UINT8 *)&_ext_cmd_twt_agrt_update,
		(UINT8 *)agrt_para,
		sizeof(struct MT_TWT_AGRT_PARA_T));
#ifdef CFG_BIG_ENDIAN
	_ext_cmd_twt_agrt_update.peer_id_grp_id
		= cpu2le16(_ext_cmd_twt_agrt_update.peer_id_grp_id);
	_ext_cmd_twt_agrt_update.agrt_sp_start_tsf_low
		= cpu2le32(_ext_cmd_twt_agrt_update.agrt_sp_start_tsf_low);
	_ext_cmd_twt_agrt_update.agrt_sp_start_tsf_high
		= cpu2le32(_ext_cmd_twt_agrt_update.agrt_sp_start_tsf_high);
	_ext_cmd_twt_agrt_update.agrt_sp_wake_intvl_mantissa
		= cpu2le16(_ext_cmd_twt_agrt_update.agrt_sp_wake_intvl_mantissa);
	for (i = 0; i < TWT_HW_BTWT_MAX_MEMBER_CNT; i++)
		_ext_cmd_twt_agrt_update.sta_list[i]
			= cpu2le16(_ext_cmd_twt_agrt_update.sta_list[i]);
#endif
	...
	UINT8 CentralChannel)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdTpcFeatureCtrl(pAd, TpcPowerValue, ucBandIdx, CentralChannel);
#else
	struct cmd_msg *msg;
	...
	UCHAR        ATEMode)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdATEModeCtrl(pAd, ATEMode);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdPktBudgetCtrl(struct _RTMP_ADAPTER *pAd, UINT8 bss_idx, UINT16 wcid, UCHAR type)
{
	...
	pbc.band_idx = hc_get_hw_band_idx(pAd);

	switch (type) {
	case PBC_TYPE_NORMAL: {
		for (i = 0; i < PBC_NUM_OF_PKT_BUDGET_CTRL_QUE; i++) {
			entry = &pbc.aacQue[i];
			if (pChipCap->asic_caps & fASIC_CAP_HW_ADMISSION_CTRL) {
				entry->lower_bound = 0;
	...
	case PBC_TYPE_WMM: {
		for (i = 0; i < PBC_NUM_OF_PKT_BUDGET_CTRL_QUE; i++) {
			...
			if (pChipCap->asic_caps & fASIC_CAP_HW_ADMISSION_CTRL) {
				...
				entry->lower_bound = cpu2le16(pAd->pbc_bound[i]);
...
INT32 MtCmdSetMaxPhyRate(RTMP_ADAPTER *pAd, UINT16 u2MaxPhyRate)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdRAParameterSet(pAd, (PUCHAR)(&u2MaxPhyRate), UNI_CMD_RA_SET_MAX_PHY_RATE);
#else
	struct cmd_msg *msg;
...
INT32 MtCmdSetUseVhtRateFor2G(RTMP_ADAPTER *pAd)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdRAParameterSet(pAd, NULL, UNI_CMD_RA_SET_VHT_RATE_FOR_2G);
#else
	struct cmd_msg *msg;
		...
		PUINT8	pucTxPwrUpValue)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "ucTxPwrUpCat: %d\n", ucTxPwrUpCat);

	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "ucTxPwrUpRate: ");
	for (ucRateIdx = 0; ucRateIdx < CMW_POWER_UP_RATE_NUM; ucRateIdx++)
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, " %d", *(pucTxPwrUpValue + ucRateIdx));
...
INT32 MtCmdPhyShapingFilterDisable(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_DEBUG,
			 "phyShapingFilterDisable = %d\n", PHY_SHAPING_FILTER_DISABLE);
...
INT ShowHeraRuRaInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret;
#ifdef WIFI_UNIFIED_COMMAND
	INT32 result = NDIS_STATUS_SUCCESS;

	result = UniCmdGetRuRaInfo(pAd, arg);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
...
INT SetHeraIara_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (!arg) {
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_ERROR, "Null Parameters\n");
		return FALSE;
	}

	u4Value = os_str_tol(arg, 0, 10);
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "Enable=%d\n", u4Value);
...
INT SetRaOption(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret;
#ifdef WIFI_UNIFIED_COMMAND
	INT32 result = NDIS_STATUS_SUCCESS;

	result = UniCmdRAParameterSet(pAd, arg, UNI_CMD_RA_OPTION_CTRL);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	if (!arg) {
		MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "Null Parameters\n");
		return FALSE;
	}
...
INT ShowHeraMuRaInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	result = UniCmdGetMuRaInfo(pAd, arg);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PUINT8 pucData)
{
	...
	result = UniCmdRAParameterSet(pAd, (PUCHAR)pucData, UNI_CMD_RA_HERA_STBC_PRIORITY);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	PRTMP_ADAPTER pAd)
{
	...
	result = UniCmdRAParameterSet(pAd, NULL, UNI_CMD_RA_SET_VHT_1024_QAM);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	UINT32 u4HeraCmdType = HERA_VHT_1024QAM_CMD;
	CMD_SET_VHT_1024_QAM_T rHeraSetVht1024Qam = {0};
	Ret = TRUE;
	...
	IN	RTMP_STRING *arg)
{
	...
	result = UniCmdRAParameterSet(pAd, arg, UNI_CMD_RA_CFG_PTEC_PER_PPDU);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	PCHAR pch = NULL;
	...
	CMD_CFG_PTEC_PER_PPDU_T param = {0};
	Ret = TRUE;
	...
	if (pch != NULL) {
		...
		if (param.u1BandIdx >= CFG_WIFI_RAM_BAND_NUM) {
			MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_ERROR,
					"Invalid Band Index !!\n");
			Ret = 0;
			goto error;
...
error:
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, Ret?DBG_LVL_INFO:DBG_LVL_ERROR,
			"(Ret = %d\n", Ret);
#endif
	return Ret;
}
	...
	IN	RTMP_STRING *arg)
{
	...
	result = UniCmdRAParameterSet(pAd, arg, UNI_CMD_RA_CFG_MU_INIT_RATE_INTV);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	PCHAR pch = NULL;
	/* prepare command message */
	...
	CMD_CFG_MU_INIT_RATE_INTV_T param = {0};
	Ret = TRUE;
	...
	IN	RTMP_STRING *arg)
{
	...
	result = UniCmdRAParameterSet(pAd, arg, UNI_CMD_RA_CFG_MU_DIS_SWITCH_SU);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	PCHAR pch = NULL;
	/* prepare command message */
	...
	CMD_CFG_MU_DIS_SWITCH_SU_T param = {0};
	Ret = TRUE;
...
#endif
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "ipi_hist_idx: 0x%x\n",
...
#ifdef WIFI_UNIFIED_COMMAND
VOID TransUniStructToNormalRxStat(struct _UNI_EVENT_TESTMODE_STAT_WB_T *prUniTestmodeStat,
	struct _TESTMODE_STATISTIC_INFO *prTestmodeStat)
{
	int8_t i = 0;

	prTestmodeStat->mac_rx_fcs_err_cnt = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2MacRxFcsErrCnt;
	prTestmodeStat->mac_rx_len_mismatch = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2MacRxLenMisMatch;
	prTestmodeStat->mac_rx_fcs_ok_cnt = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2MacRxFcsOkCnt;
	prTestmodeStat->mac_rx_fifo_full = prUniTestmodeStat->rEvtTestmodeStat.rInfoComm.u2MacRxFifoFull;
	prTestmodeStat->mac_rx_mdrdy_cnt = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u4MacRxMdrdyCnt;

	/* phy part */
	prTestmodeStat->phy_rx_fcs_err_cnt_cck = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxFcsErrCntCck;
	prTestmodeStat->phy_rx_fcs_err_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxFcsErrCntOfdm;
	prTestmodeStat->phy_rx_pd_cck = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxPdCck;
	prTestmodeStat->phy_rx_pd_ofdm = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxPdOfdm;
	prTestmodeStat->phy_rx_sig_err_cck = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxSigErrCck;
	prTestmodeStat->phy_rx_sfd_err_cck = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxSfdErrCck;
	prTestmodeStat->phy_rx_sig_err_ofdm = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxSigErrOfdm;
	prTestmodeStat->phy_rx_tag_err_ofdm = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxTagErrOfdm;
	prTestmodeStat->phy_rx_mdrdy_cnt_cck = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxMdrdyCntCck;
	prTestmodeStat->phy_rx_mdrdy_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStat.rInfoBand.u2PhyRxMdrdyCntOfdm;

	prTestmodeStat->aci_hit_low = prUniTestmodeStat->rEvtTestmodeStat.rInfoComm.u4AciHitLow;
	prTestmodeStat->aci_hit_high = prUniTestmodeStat->rEvtTestmodeStat.rInfoComm.u4AciHitHigh;


	for (i = 0; i < 4; i++) {
	/* rxv part */
		prTestmodeStat->rcpi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoRXV[i].u2Rcpi;
		prTestmodeStat->rssi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoRXV[i].i2Rssi;
		prTestmodeStat->snr[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoRXV[i].i2Snr;

	/* RSSI */
		prTestmodeStat->fagc_ib_rssi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoFagc[i].i1RssiIb;
		prTestmodeStat->fagc_wb_rssi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoFagc[i].i1RssiWb;
		prTestmodeStat->inst_ib_rssi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoInst[i].i1RssiIb;
		prTestmodeStat->inst_wb_rssi[i] = prUniTestmodeStat->rEvtTestmodeStat.rInfoInst[i].i1RssiWb;
	}
}

VOID TransUniStructToNormalRxStatBand(struct _UNI_EVENT_TESTMODE_STAT_WB_T *prUniTestmodeStat,
	struct _TESTMODE_STATISTIC_INFO_BAND *prTestmodeStat)
{

	prTestmodeStat->mac_rx_fcs_err_cnt = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2MacRxFcsErrCnt;
	prTestmodeStat->mac_rx_len_mismatch = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2MacRxLenMisMatch;
	prTestmodeStat->mac_rx_fcs_ok_cnt = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2MacRxFcsOkCnt;
	prTestmodeStat->mac_rx_mdrdy_cnt = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u4MacRxMdrdyCnt;

	/* phy part */
	prTestmodeStat->phy_rx_fcs_err_cnt_cck = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxFcsErrCntCck;
	prTestmodeStat->phy_rx_fcs_err_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxFcsErrCntOfdm;
	prTestmodeStat->phy_rx_pd_cck = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxPdCck;
	prTestmodeStat->phy_rx_pd_ofdm = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxPdOfdm;
	prTestmodeStat->phy_rx_sig_err_cck = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxSigErrCck;
	prTestmodeStat->phy_rx_sfd_err_cck = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxSfdErrCck;
	prTestmodeStat->phy_rx_sig_err_ofdm = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxSigErrOfdm;
	prTestmodeStat->phy_rx_tag_err_ofdm = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxTagErrOfdm;
	prTestmodeStat->phy_rx_mdrdy_cnt_cck = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxMdrdyCntCck;
	prTestmodeStat->phy_rx_mdrdy_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStatBand.rInfoBand.u2PhyRxMdrdyCntOfdm;
}


VOID TransUniStructToNormalRxStatAll(struct _UNI_EVENT_TESTMODE_STAT_WB_T *prUniTestmodeStat,
	struct _TEST_RX_STAT_ALL_INFO *prTestmodeStat)
{
	int i = 0;

	/* band */
	prTestmodeStat->rx_band_info.mac_rx_fcs_err_cnt = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2MacRxFcsErrCnt;
	prTestmodeStat->rx_band_info.mac_rx_len_mismatch = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2MacRxLenMisMatch;
	prTestmodeStat->rx_band_info.mac_rx_fcs_ok_cnt = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2MacRxFcsOkCnt;
	prTestmodeStat->rx_band_info.mac_rx_mdrdy_cnt = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u4MacRxMdrdyCnt;
	prTestmodeStat->rx_band_info_ext.mac_rx_u2m_mpdu_cnt = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBandExt1.u4RxMpduU2MCnt;

	/* phy part */
	prTestmodeStat->rx_band_info.phy_rx_fcs_err_cnt_cck = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxFcsErrCntCck;
	prTestmodeStat->rx_band_info.phy_rx_fcs_err_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxFcsErrCntOfdm;
	prTestmodeStat->rx_band_info.phy_rx_pd_cck = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxPdCck;
	prTestmodeStat->rx_band_info.phy_rx_pd_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxPdOfdm;
	prTestmodeStat->rx_band_info.phy_rx_sig_err_cck = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxSigErrCck;
	prTestmodeStat->rx_band_info.phy_rx_sfd_err_cck = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxSfdErrCck;
	prTestmodeStat->rx_band_info.phy_rx_fcs_err_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxFcsErrCntOfdm;
	prTestmodeStat->rx_band_info.phy_rx_sig_err_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxSigErrOfdm;
	prTestmodeStat->rx_band_info.phy_rx_tag_err_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxTagErrOfdm;
	prTestmodeStat->rx_band_info.phy_rx_mdrdy_cnt_cck = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxMdrdyCntCck;
	prTestmodeStat->rx_band_info.phy_rx_mdrdy_cnt_ofdm = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoBand.u2PhyRxMdrdyCntOfdm;

	{
		struct _TEST_RX_STAT_BAND_INFO *r = &prTestmodeStat->rx_band_info;

		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			"\n mac_rx_fcs_err_cnt:%u\n mac_rx_fcs_ok_cnt0:%u\n",
			r->mac_rx_fcs_err_cnt,
			r->mac_rx_fcs_ok_cnt);
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			" mac_rx_mdrdy_cnt:%u\n",
			r->mac_rx_mdrdy_cnt);
		...
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			...
			r->phy_rx_fcs_err_cnt_ofdm,
			r->phy_rx_pd_ofdm);
		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			" phy_rx_mdrdy_cnt_ofdm:%u\n",
			r->phy_rx_mdrdy_cnt_ofdm);
	}

	/* path */
	for (i = 0; i < 5; i++) {
		prTestmodeStat->rx_path_info[i].rcpi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoRXV[i].u2Rcpi;
		prTestmodeStat->rx_path_info[i].rssi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoRXV[i].i2Rssi;
		prTestmodeStat->rx_path_info[i].fagc_rssi_adc = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoRXV[i].i2AdcRssi;


		{
			struct _UNI_TESTMODE_STATISTIC_INFO_RXV_T *r = &prUniTestmodeStat->rEvtTestmodeStatAll.rInfoRXV[i];
	...
	for (i = 0; i < 5; i++) {
		prTestmodeStat->rx_path_info[i].fagc_ib_rssi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoFagc[i].i1RssiIb;
		prTestmodeStat->rx_path_info[i].fagc_wb_rssi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoFagc[i].i1RssiWb;
		prTestmodeStat->rx_path_info[i].inst_ib_rssi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoInst[i].i1RssiIb;
		prTestmodeStat->rx_path_info[i].inst_wb_rssi = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoInst[i].i1RssiWb;

		{
			struct _UNI_TESTMODE_STATISTIC_INFO_RSSI_T *fagc = &prUniTestmodeStat->rEvtTestmodeStatAll.rInfoFagc[i];
			struct _UNI_TESTMODE_STATISTIC_INFO_RSSI_T *inst = &prUniTestmodeStat->rEvtTestmodeStatAll.rInfoInst[i];
	...
	/* user */
	for (i = 0; i < 16; i++) {
		prTestmodeStat->rx_user_info[i].freq_offset_from_rx = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoUser[i].i4FreqOffsetFromRx;
		prTestmodeStat->rx_user_info[i].snr = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoUser[i].i4Snr;
		prTestmodeStat->rx_user_info[i].fcs_error_cnt = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoUser[i].u4FcsErrorCnt;
	}

	{
		struct _UNI_TESTMODE_STATISTIC_INFO_USER_T *r = &prUniTestmodeStat->rEvtTestmodeStatAll.rInfoUser[0];

		MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
			" i4FreqOffsetFromRx:%d\n i4Snr:%d\n u4FcsErrorCnt:%d\n",
			r->i4FreqOffsetFromRx,
			r->i4Snr,
			r->u4FcsErrorCnt);
	}

	/* comm */
	prTestmodeStat->rx_comm_info.rx_fifo_full = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoComm.u2MacRxFifoFull;
	prTestmodeStat->rx_comm_info.aci_hit_low = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoComm.u4AciHitLow;
	prTestmodeStat->rx_comm_info.aci_hit_high =  prUniTestmodeStat->rEvtTestmodeStatAll.rInfoComm.u4AciHitHigh;
	prTestmodeStat->rx_comm_info.mu_pkt_count = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoCommExt1.u4MuRxCnt;
	prTestmodeStat->rx_comm_info.sinr = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoCommExt1.u4Sinr;
	prTestmodeStat->rx_comm_info.sig_mcs = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoCommExt1.u1EhtSigMcs;
	prTestmodeStat->rx_comm_info.ne_var_db_all_user = prUniTestmodeStat->rEvtTestmodeStatAll.rInfoUserExt1[0].u1NeVarDbAllUser;

	{
		struct _TEST_RX_STAT_COMM_INFO *r = &prTestmodeStat->rx_comm_info;
...
VOID TransUniStructToNormalRxStatPath(struct _UNI_EVENT_TESTMODE_STAT_WB_T *prUniTestmodeStat,
	struct _TESTMODE_STATISTIC_INFO_PATH *prTestmodeStat)
{

	/* RSSI */
	prTestmodeStat->inst_ib_rssi = prUniTestmodeStat->rEvtTestmodeStatPath.rInfoInst.i1RssiIb;
	prTestmodeStat->inst_wb_rssi = prUniTestmodeStat->rEvtTestmodeStatPath.rInfoInst.i1RssiWb;
}

VOID TransUniStructToNormalRxStatComm(struct _UNI_EVENT_TESTMODE_STAT_WB_T *prUniTestmodeStat,
	struct _TESTMODE_STATISTIC_INFO_COMM *prTestmodeStat)
{

	/* RSSI */
	prTestmodeStat->mac_rx_fifo_full = prUniTestmodeStat->rEvtTestmodeStatComm.rInfoComm.u2MacRxFifoFull;
	prTestmodeStat->aci_hit_low = prUniTestmodeStat->rEvtTestmodeStatComm.rInfoComm.u4AciHitLow;
	prTestmodeStat->aci_hit_high = prUniTestmodeStat->rEvtTestmodeStatComm.rInfoComm.u4AciHitHigh;
}
#endif /* WIFI_UNIFIED_COMMAND */
	...
	P_TESTMODE_STATISTIC_INFO p_rx_stat_rlt)
{
...
#ifndef WIFI_UNIFIED_COMMAND
	struct cmd_msg *msg = NULL;
	EXT_CMD_GET_RX_STATISTIC_INFO rx_stat_cmd = {0};
	...
	ret = chip_cmd_tx(pAd, msg);
#else
	struct _UNI_CMD_TESTMODE_RX_T rx_stat_cmd = {0};
	uint64_t u8TestmodeRxFeature = 0;
	struct _UNI_EVENT_TESTMODE_STAT_WB_T rTestmodeRxWb = {0};

	rx_stat_cmd.rRxStatCmd.u1BandIdx = band_idx;
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_FEATURE;
	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, band_idx);
	TransUniStructToNormalRxStat(&rTestmodeRxWb, p_rx_stat_rlt);
...
INT32 mt_cmd_get_rx_stat_all(
	PRTMP_ADAPTER pAd,
	UCHAR band_idx,
	struct _TEST_RX_STAT_ALL_INFO *prx_stat_all)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	struct _UNI_CMD_TESTMODE_RX_T rx_stat_cmd = {0};
	...
	rx_stat_cmd.rRxStatAllCmd.u1BandIdx = band_idx;
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_ALL_FEATURE;
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO,
		"RxFeature:%llu\n", u8TestmodeRxFeature);

	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, band_idx);
	TransUniStructToNormalRxStatAll(&rTestmodeRxWb, prx_stat_all);
	...
	TESTMODE_STATISTIC_INFO_BAND *rx_stat_band)
{
...
#ifndef WIFI_UNIFIED_COMMAND
	struct cmd_msg *msg = NULL;
	EXT_CMD_GET_RX_STATISTIC_INFO_BAND rx_stat_band_cmd = {0};
	struct _CMD_ATTRIBUTE attr = {0};
	...
	rx_stat_cmd.rRxStatBandCmd.u1BandIdx = band_idx;
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_BAND_FEATURE;

	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, band_idx);
	TransUniStructToNormalRxStatBand(&rTestmodeRxWb, rx_stat_band);
...
#else
INT32 mt_uni_cmd_get_rx_stat_path(
	PRTMP_ADAPTER pAd,
	UCHAR path_idx,
	UCHAR band_idx,
	TESTMODE_STATISTIC_INFO_PATH *rx_stat_path)
{
	...
	rx_stat_cmd.rRxStatPathCmd.u1PathIdx = path_idx;
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_PATH_FEATURE;

	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, band_idx);
	TransUniStructToNormalRxStatPath(&rTestmodeRxWb, rx_stat_path);
	...
	TESTMODE_STATISTIC_INFO_USER *rx_stat_user)
{
	...
	ret = chip_cmd_tx(pAd, msg);
#else
	struct _UNI_CMD_TESTMODE_RX_T rx_stat_cmd;
	uint64_t u8TestmodeRxFeature = 0;
	struct _UNI_EVENT_TESTMODE_STAT_WB_T rTestmodeRxWb;

	rx_stat_cmd.rRxStatUserCmd.u1UserIdx = user_idx;
#ifdef CFG_BIG_ENDIAN
	rx_stat_cmd.rRxStatUserCmd.u1UserIdx = cpu2le16(rx_stat_cmd.rRxStatUserCmd.u1UserIdx);
#endif

	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_USER_FEATURE;

	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, 0);
	if (ret != NDIS_STATUS_SUCCESS)
		goto error;
	...
	TESTMODE_STATISTIC_INFO_COMM *rx_stat_comm)
{
...
#ifndef WIFI_UNIFIED_COMMAND
	struct cmd_msg *msg;
	EXT_CMD_GET_RX_STATISTIC_INFO_COMM rx_stat_comm_cmd = {0};
	struct _CMD_ATTRIBUTE attr = {0};
	...
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_GET_RX_STAT_COMM_FEATURE;

	ret = UniCmdTestmodeRxStat(pAd, &rx_stat_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, 0);
	TransUniStructToNormalRxStatComm(&rTestmodeRxWb, rx_stat_comm);
	...
	UINT16 user_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_FW, CATFW_DBGINFO, DBG_LVL_INFO, "band_idx: %d, user_idx: 0x%x\n", band_idx, user_idx);
	...
	u8TestmodeRxFeature = UNI_CMD_TESTMODE_SET_RX_STAT_USER_CTRL_FEATURE;
	rx_stat_cmd.rRxStatUserCtrlCmd.u1BandIdx = band_idx;
	rx_stat_cmd.rRxStatUserCtrlCmd.u2UserIdx = user_idx;
#ifdef CFG_BIG_ENDIAN
	rx_stat_cmd.rRxStatUserCtrlCmd.u2UserIdx = cpu2le16(rx_stat_cmd.rRxStatUserCtrlCmd.u2UserIdx);
#endif
...
)
{
	...
	ret = chip_cmd_tx(pAd, msg);
#else
	struct _UNI_CMD_TESTMODE_RX_T rxv_ctrl_cmd;
	uint64_t u8TestmodeRxFeature = 0;
	struct _UNI_EVENT_TESTMODE_STAT_WB_T rTestmodeRxWb;

	u8TestmodeRxFeature = UNI_CMD_TESTMODE_SET_RXV_CTRL_FEATURE;
	rxv_ctrl_cmd.rRxvCtrlCmd.fgRxvEnable = fgRxvEnable;

	ret = UniCmdTestmodeRxStat(pAd, &rxv_ctrl_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, 0);
...
)
{
	...
	ret = chip_cmd_tx(pAd, msg);
#else
	struct _UNI_CMD_TESTMODE_RX_T rxv_ru_ctrl_cmd;
	uint64_t u8TestmodeRxFeature = 0;
	struct _UNI_EVENT_TESTMODE_STAT_WB_T rTestmodeRxWb;

	u8TestmodeRxFeature = UNI_CMD_TESTMODE_SET_RXV_RU_CTRL_FEATURE;
	rxv_ru_ctrl_cmd.rRxvRuCtrlCmd.u1RxvRuIdx = rxv_ru_idx;

	ret = UniCmdTestmodeRxStat(pAd, &rxv_ru_ctrl_cmd, u8TestmodeRxFeature, (uint8_t *)&rTestmodeRxWb, 0);
...
INT32 ShowHwcfgInfo(
	...
	uint16_t dump_size,
	uint8_t *epprom_content
)
{
#ifdef WIFI_UNIFIED_COMMAND
	UniCmdHwcfgGet(pAd, dump_offset, dump_size, epprom_content);
#endif
	return TRUE;
}
...
INT ShowHeraRelatedInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	result = UniCmdRAParameterSet(pAd, arg, UNI_CMD_RA_GET_HERA_RELATED_INFO);
	Ret = (result != NDIS_STATUS_SUCCESS)?FALSE:TRUE;
#else
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	UINT16 Len)
{
	...
	EventLteSafeChnHandler(pAd, Data, Len);
}
...
VOID QueryLteSafeChannel(struct _RTMP_ADAPTER *pAd)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdQuerySafeChannel(pAd);
#else
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
...
VOID MtCmdIdcInfoQuery(struct _RTMP_ADAPTER *pAd)
{
	...
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_QUERY_AND_WAIT_RSP);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, sizeof(struct COEX_IDC_INFO));
...
INT32 MtCmdIdcStateUpdate(struct _RTMP_ADAPTER *pAd)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdUpdateIdcState(pAd);
#else
	UINT32 Val;
	struct cmd_msg *msg;
	struct _CMD_ATTRIBUTE attr = {0};
...
INT32 MtCmdIdcSetTxPwrLimit(struct _RTMP_ADAPTER *pAd, UINT16 maxTxPwrLimit)
{
	INT32 ret = 0;
#ifdef WIFI_UNIFIED_COMMAND
	ret = UniCmdSetIdcPwrBackoff(pAd, maxTxPwrLimit);
#else
	POS_COOKIE pObj = (POS_COOKIE)pAd->OS_Cookie;
	INT32 if_idx = pObj->ioctl_if;
	struct wifi_dev *pwdev = get_wdev_by_ioctl_idx_and_iftype(pAd, if_idx, pObj->ioctl_if_type);

	ret = CmdExtCmdCfgUpdate(pAd, pwdev, CFGINFO_POWER_BACKOFF_FEATURE, &maxTxPwrLimit);
#endif
	MTWF_DBG(pAd, DBG_CAT_COEX, CATCOEX_MD, DBG_LVL_INFO,
			 "(ret = %d)\n", ret);
	return ret;
