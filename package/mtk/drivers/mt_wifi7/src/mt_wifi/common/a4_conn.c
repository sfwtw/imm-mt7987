...
)
{
	...
	if (add_inf) {
		if (adapter->physical_dev->a4_interface_count == 0)
			/*this cmd set CR is a per-card global register,
			first interface up set true*/
			MtCmdSetA4Enable(adapter, HOST2CR4, TRUE);
		adapter->physical_dev->a4_interface_count++;
	}

	MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_INFO,
		"a4_interface_count: %d\n",
		adapter->physical_dev->a4_interface_count);
	return TRUE;
}
	...
	if (is_ap) {
		...
		if (mbss->a4_init) {
			...
			DlListForEachSafe(a4_entry, pConnEntryTmp, a4_entry_list, A4_CONNECT_ENTRY, List) {
				if (a4_entry) {
					entry = entry_get(adapter, a4_entry->wcid);
...
)
{
	...
	OS_SEM_LOCK(&mbss->a4_entry_lock);
	count = DlListLen(&mbss->a4_entry_list);
	OS_SEM_UNLOCK(&mbss->a4_entry_lock);
	return count;
}
...
)
{
	...
	DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List) {
		if (a4_entry->valid && IS_WCID_VALID(adapter, a4_entry->wcid)) {
			entry = entry_get(adapter, a4_entry->wcid);
			if (entry && MAC_ADDR_EQUAL(mac_addr, entry->Addr)) {
				found = TRUE;
				break;
			}
		}
...
)
{
	...
	if (a4_entry) {
		...
		MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_ERROR,
...
)
{
	...
	UINT SrcBufLen = 0;
	UCHAR frag_nums;
	struct tr_counter *tr_cnt = &adapter->tr_ctl.tr_cnt;
	...
	frag_nums = get_frag_num(adapter, wdev, pkt);
	...
	if (IS_ASIC_CAP(adapter, fASIC_CAP_MCU_OFFLOAD)) {
		...
		tr_entry = tr_entry_get(adapter, wcid);
		if (!RTMPCheckEtherType(adapter, pkt, tr_entry, wdev)) {
			MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_WARN,
...
)
{
	...
	if ((a4_get_entry_count(adapter, if_index) > 0)) {
		...
		if (ap_wdev->IgmpSnoopEnable) { /* If snooping enabled*/
			...
			pDestAddr = GET_OS_PKT_DATAPTR(pkt);
			if (IS_MULTICAST_MAC_ADDR(pDestAddr)) {
				...
				/* If detected, set internal query Hold duration to avoid */
				/* flooding in network */
				if (protoType == ETH_P_IP) {
					...
					if (isMLDquery(adapter, pDestAddr, pData, GET_OS_PKT_DATATAIL(pkt)))
		...
		OS_SEM_LOCK(&adapter->ApCfg.MBSSID[if_index].a4_entry_lock);
		DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List) {
			if (a4_entry->valid && IS_WCID_VALID(adapter, a4_entry->wcid)) {
				...
				if (entry->mlo.mlo_en && mld_entry) {
					PMAC_TABLE_ENTRY setup_entry = NULL;

					setup_entry = mld_entry_link_select(mld_entry);
					if (!IS_VALID_ENTRY(setup_entry)) {
						MTWF_DBG(NULL, DBG_CAT_MLO,
							CATMLO_WDS, DBG_LVL_ERROR,
							"Invalid entry\n");
						mt_rcu_read_unlock();
						continue;
					}
					if  (entry->wcid != setup_entry->wcid) {
						mt_rcu_read_unlock();
						continue;
					}
				...
				if (pkt_clone == NULL) {
					OS_SEM_UNLOCK(&adapter->ApCfg.MBSSID[if_index].a4_entry_lock);
					MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_ERROR,
				...
				if (adapter->bTSEnable) {
					if (!map_ts_tx_process(adapter, wdev, pkt_clone, entry)) {
...
)
{
	...
	mbss = &adapter->ApCfg.MBSSID[if_index];
	if (mbss && !(mbss->a4_init & (1 << type))) {
		MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_ERROR,
				"A4Entry not init, entry mac="MACSTR"\n",
					MAC2STR(entry->Addr));
		return FALSE;
	}
	...
	if (IS_ENTRY_A4(entry) == FALSE) {
		...
		if (wifi_sys_header_trans_update(adapter, entry) != NDIS_STATUS_SUCCESS)
			return FALSE;
#else
...
)
{
	...
	if (type > GET_ENTRY_A4(entry)) {
		...
		SET_ENTRY_A4(entry, type);
		MTWF_PRINT("SET_A4_ENTRY APCLI OK!\n");
	}
	return TRUE;
}
	...
	IN  RTMP_STRING * arg)
{
	...
	DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List) {
		if (a4_entry->valid && IS_WCID_VALID(adapter, a4_entry->wcid)) {
			count = 0;
			proxy_mac_addr = entry_addr_get(adapter, a4_entry->wcid);
			...
			if (GetRoutingEntryAll(adapter, if_index, a4_entry->wcid, ROUTING_ENTRY_A4,
				...
				for (i = 0; i < count; i++) {
					...
					if (routing_entry->IPAddr != 0) {
						...
						ret = snprintf(proxy_ip, sizeof(proxy_ip),
								"%d.%d.%d.%d",
								(ip_addr & 0xff),
								((ip_addr & (0xff << 8)) >> 8),
								((ip_addr & (0xff << 16)) >> 16),
						...
						ret = snprintf(proxy_ip,
							sizeof(proxy_ip), "0.0.0.0");

					if (os_snprintf_error(sizeof(proxy_ip), ret)) {
						MTWF_PRINT("%s: snprintf error!\n", __func__);
						break;
					}

					MTWF_PRINT(
						"MAC:"MACSTR", IP:%s, AgeOut:%lus, Retry:(%d,%d)\n",
	...
	IN IE_LISTS *ie_list)
{
	...
	HcReleaseUcastWcid(pAd, pEntry->wdev, oldwcid);
...
)
{
	...
	DlListForEachSafe(eth_entry, eth_entryTmp, eth_entry_list, Eth_CONNECT_ENTRY, List) {
		if (MAC_ADDR_EQUAL(mac_addr, eth_entry->mac)) {
			MTWF_DBG(adapter, DBG_CAT_AP, CATAP_WDS, DBG_LVL_NOTICE,
