...
const CHAR *ACT_FSM_STATE_STR[MAX_ACT_STATE] = {
	"ACT_IDLE",
};
	...
	OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, ACT_IDLE, MT2_MLME_WAIT_ACK_STS, (STATE_MACHINE_FUNC)mlme_twt_wait_ack_action);
#ifdef APCLI_SUPPORT
...
#endif
#ifdef DOT11_EHT_BE
	StateMachineSetAction(S, ACT_IDLE, MT2_PEER_PROTECTED_EHT_CATE, (STATE_MACHINE_FUNC)PeerProtectEHTAction);
#endif /* DOT11_EHT_BE */
}

BOOLEAN act_fsm_state_transition(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev, ULONG NextState)
{
	ULONG OldState = 0;

	OldState = ad->Mlme.ActMachine.CurrState;
	ad->Mlme.ActMachine.CurrState = NextState;

	MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_ACTION, DBG_LVL_INFO,
		"ACT: [%s] \t==============================================> [%s]\n",
		ACT_FSM_STATE_STR[OldState],
		ACT_FSM_STATE_STR[NextState]);
...
VOID MlmeADDBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	pEntry = entry_get(pAd, pInfo->Wcid);
	if (!pEntry) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_ERROR,
				"Abnormal: pEntry is NULL!\n");
		return;
	}
	...
	Frame.BaParm.BufSize = (pInfo->BaBufSize & 0x03FF);
	Frame.Token = pInfo->Token;
	Frame.TimeOutValue = pInfo->TimeOutValue;
	Frame.BaStartSeq.field.FragNum = 0;

	{
		UCHAR use_sw_query = 0;

		chip_do_extra_action(pAd, NULL, NULL, CHIP_EXTRA_ACTION_ADDBA_SSN, &use_sw_query, NULL);
		Frame.BaStartSeq.field.StartSeq = (use_sw_query ? chip_get_tid_sn(pAd, pInfo->Wcid, pInfo->TID) : 0);
	}
	...
	*(USHORT *)(&(Frame.BaParm)) = cpu2le16((*(USHORT *)(&(Frame.BaParm))));
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  ...
					  END_OF_ARGS);
#ifdef DOT11_EHT_BE
	if (pInfo->BaBufSize & 0x1C00) {
		UINT8 addba_ext_ie = IE_ADDBA_EXT;
		UINT8 addba_ext_ie_len = 1;
		UINT8 addba_ext_value = 0;
		ULONG TmpLen = 0;

		SET_ADDBA_EXT_BUF_SIZE(
			addba_ext_value,
			(pInfo->BaBufSize & 0x1C00) >> ADDBA_BUF_SIZE_BITS);
...
#endif /* DOT11_EHT_BE */
	MiniportMMRequest(pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[pInfo->TID]), pOutBuffer, FrameLen, NULL);
	MlmeFreeMemory(pOutBuffer);
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO,
...
VOID MlmeDELBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	*(USHORT *)(&Frame.DelbaParm) = cpu2le16(*(USHORT *)(&Frame.DelbaParm));
	...
	MiniportMMRequest(pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[pInfo->TID]),
			  pOutBuffer, FrameLen, NULL);
	MlmeFreeMemory(pOutBuffer);
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO,
		"Initiator = %d DELBA sent\n", pInfo->Initiator);
}
...
VOID mlme_send_addba_resp(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeAllocateMemory(pAd, &pOutBuffer) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO, "ACTION - PeerBAAction() allocate memory failed\n");
		return;
	}
	...
	ADDframe.BaParm.BufSize = (pInfo->buf_size & 0x03FF);
	...
	*(USHORT *)(&ADDframe.BaParm) = cpu2le16(*(USHORT *)(&ADDframe.BaParm));
	...
	if (pInfo->buf_size & 0x1C00) {
		...
		SET_ADDBA_EXT_BUF_SIZE(
			addba_ext_value,
			(pInfo->buf_size & 0x1C00) >> ADDBA_BUF_SIZE_BITS);
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
				1, &addba_ext_ie,
				1, &addba_ext_ie_len,
				1, &addba_ext_value,
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO, "(%d): TID(%d), BufSize(%d) <==\n", Elem->Wcid, ADDframe.BaParm.TID,
	...
	IN UCHAR InfoReq)
{
	...
	if (wdev) {
		if (WDEV_BSS_STATE(wdev) < BSS_READY) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO,
					"wdev(%d) bss(%d) not ready (state:%d)!!\n",
					wdev->wdev_idx, apidx, WDEV_BSS_STATE(wdev));
			return;
		}
	...
	pAddr1 = entry_addr_get(pAd, Wcid);
	...
	IN struct wifi_dev *wdev)
{
	...
	FRAME_ACTION_HDR frame;
	ULONG frameLen;
	OPERATING_MODE op_mode_ntf;
	UCHAR op_vht_bw = wlan_operate_get_vht_bw(wdev);
	INT8 rx_streams = wlan_operate_get_rx_stream(wdev) - 1;

	if (MlmeAllocateMemory(pAd, &pOutBuffer) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO,
			"VHT OMN - allocate memory failed\n");
		return;
	}

	if (rx_streams < 0)
		rx_streams = 0;
	else if (rx_streams > 7)
		rx_streams = 7;

	NdisZeroMemory((PUCHAR)&op_mode_ntf, sizeof(OPERATING_MODE));
	op_mode_ntf.rx_nss_type = 0;
	op_mode_ntf.rx_nss = rx_streams;

	if (op_vht_bw == VHT_BW_2040) {
		if (wlan_operate_get_ht_bw(wdev) == HT_BW_40)
			op_mode_ntf.ch_width = 1;
		else
			op_mode_ntf.ch_width = 0;
	} else if (op_vht_bw == VHT_BW_80)
		op_mode_ntf.ch_width = 2;
	else if ((op_vht_bw == VHT_BW_160) ||
			 (op_vht_bw == VHT_BW_8080)) {
		op_mode_ntf.ch_width = 2;
		op_mode_ntf.bw160_bw8080 = 1;
	}

	ActHeaderInit(pAd, &frame.Hdr, wdev->bssid, wdev->if_addr, wdev->bssid);
	frame.Category = CATEGORY_VHT;
	frame.Action = ACT_VHT_OPMODE_NOTIFY;
	MakeOutgoingFrame(pOutBuffer, &frameLen,
					  sizeof(FRAME_ACTION_HDR), &frame,
					  sizeof(OPERATING_MODE), &op_mode_ntf,
					  END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, frameLen, NULL);
	MlmeFreeMemory(pOutBuffer);
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_INFO,
		"ACT - VHT OMN, RxNssType=%d, RxNss=%d, ChBW=%d, BW160/8080=%d\n",
		op_mode_ntf.rx_nss_type, op_mode_ntf.rx_nss, op_mode_ntf.ch_width, op_mode_ntf.bw160_bw8080);
}
#endif /* DOT11_VHT_AC */
#endif /* CONFIG_STA_SUPPORT */
	...
	IN BOOLEAN	bAddIntolerantCha)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO,
		"ACT -BSSCoexist2040 = %x. EventANo = %d.\n",
	...
	if (pAd->CommonCfg.bForty_Mhz_Intolerant == TRUE)
		pAd->CommonCfg.BSSCoexist2040.field.Intolerant40 = 1;
	...
	IN BOOLEAN bAddIntolerantCha)
{
...
#ifdef CONFIG_STA_SUPPORT
	wdev = entry_wdev_get(pAd, Wcid);
	pStaCfg = GetStaCfgByWdev(pAd, wdev);
	ActHeaderInit(pAd, &Frame.Hdr, entry->Addr, wdev->if_addr, pStaCfg->Bssid);
#ifdef APCLI_SUPPORT
	if (IS_ENTRY_PEER_AP(entry)) {
		apidx = entry_func_tb_idx_get(pAd, Wcid);
		ActHeaderInit(
			pAd, &Frame.Hdr, entry->Addr, pAd->StaCfg[apidx].wdev.if_addr, entry->Addr);
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_DEBUG,
			"\x1b[31m\n"MACSTR" \x1b[m\n",
			MAC2STR(entry->Addr));
	}
	...
	if (!((IntolerantChaRepLen == 0) && (pAd->CommonCfg.BSSCoexist2040.word == 0)))
		MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen + IntolerantChaRepLen, NULL);
...
static VOID PeerBss2040CoexistAction(
	RTMP_ADAPTER * pAd,
	...
	PSTA_ADMIN_CONFIG pStaCfg)
{
	...
	wdev = entry_wdev_get(pAd, Elem->Wcid);
	...
	if (wlan_config_get_ch_band(wdev) != CMD_CH_BAND_24G) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_NOTICE,
			"not 2G band, ignore 20/40 BSS Coexistence action frame!!\n");
		return;
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	if (Elem->MsgLen <= (LENGTH_802_11 + sizeof(BSS_2040_COEXIST_ELEMENT))) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR,
			"20/40 BSS Coexistence Management Frame length too short!len=%ld!\n"
			, Elem->MsgLen);
		return;
	}
#ifdef APCLI_SUPPORT
	...
	if ((pBssCoexistIe->field.BSS20WidthReq == 1) || (pBssCoexistIe->field.Intolerant40 == 1)) {
		...
		for (i = 0; i < (pIntolerantReport->Len - 1); i++, ptr++)
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO, "%d,", *ptr);
		...
		if (retVal == TRUE && coexChRange.effectChStart < MAX_NUM_OF_CHANNELS && coexChRange.effectChEnd < MAX_NUM_OF_CHANNELS) {
			ptr = pIntolerantReport->ChList;
			bNeedFallBack = FALSE;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT,
			DBG_LVL_INFO,
			"Check Channel List in effectedChList(%d~%d)\n",
			...
			for (i = 0; i < (pIntolerantReport->Len - 1); i++, ptr++) {
				...
				if (chEntry >= pChCtrl->ChList[coexChRange.effectChStart].Channel &&
					...
					MTWF_DBG(NULL, DBG_CAT_PROTO,
						CATPROTO_HT, DBG_LVL_INFO,
						"Found in effect range=%d!\n",
						*ptr);
					bNeedFallBack = TRUE;
					return;
				}
	...
	if (bNeedFallBack) {
		...
		if (!(pAd->CommonCfg.Bss2040CoexistFlag & BSS_2040_COEXIST_TIMER_FIRED)) {
			...
			MTWF_DBG(NULL, DBG_CAT_PROTO,
				CATPROTO_HT, DBG_LVL_INFO,
				"Already fallback to 20MHz");
			MTWF_DBG(NULL, DBG_CAT_PROTO,
				CATPROTO_HT, DBG_LVL_INFO,
				", Extend Bss2040CoexistTimer!\n");
			/* More 5 sec for the scan report of STAs.*/
		...
		/*FIXME: Carter, I don't think call update routine
			here is a good choice,the thing need to do for
			20/40 update in legacy chip is a historical burden
		*/
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO,
			CATPROTO_HT, DBG_LVL_INFO,
			"2040_COEXIST Update Beacon for idx:%d\n",
			entry->apidx);
...
VOID PeerPublicAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#endif /* RACTRL_FW_OFFLOAD_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
#if defined(WAPP_SUPPORT) && defined(CONFIG_AP_SUPPORT) && defined(CONFIG_DOT11U_INTERWORKING)
#ifdef DPP_SUPPORT
	if (!pAd->bDppEnable)
#endif /* DPP_SUPPORT */
	...
	case ACTION_BSS_2040_COEXIST:
		PeerBss2040CoexistAction(pAd, Elem, cap, pStaCfg);
	break;
#endif /* DOT11N_DRAFT3 */
#endif /* DOT11_N_SUPPORT */
	...
	{
		if (Elem->wdev->FtmCtrl.FTMResp == true)
			ReceiveFTMReq(pAd, Elem);
		else
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_ERROR,
				"haven't enable FTM responder: mwctl dev <devname> set ftm enable=1\n");
		break;
	}
#endif /* FTM_SUPPORT */
...
static VOID ReservedAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_ACTION, DBG_LVL_INFO,
...
VOID PeerVHTAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case ACT_VHT_OPMODE_NOTIFY: {
		...
		if (WMODE_CAP_AX_6G(pEntry->wdev->PhyMode)) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_WARN,
				"ACTION - Operating Mode Notification at invalid band\n");
			return;
		}
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_INFO,
			"\t RxNssType=%d, RxNss=%d, ChBW=%d, BW160/8080=%d\n",
				 op_mode->rx_nss_type, op_mode->rx_nss, op_mode->ch_width, op_mode->bw160_bw8080);
#ifdef ZERO_PKT_LOSS_SUPPORT
	...
	case ACT_VHT_COMPRESS_BF: {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_INFO,
			"ACTION - VHT Compressed Beamforming action---->\n");
...
VOID SendRefreshBAR(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, UCHAR TID)
{
	FRAME_BAR FrameBar;
	ULONG FrameLen;
	NDIS_STATUS NStatus;
	...
	/* make sure sequence not clear in DEL function.*/
	FrameBar.StartingSeq.field.StartSeq = Sequence;
	FrameBar.BarControl.TID = TID;
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_BAR), &FrameBar, END_OF_ARGS);
	MiniportMMRequest(pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[TID]), pOutBuffer, FrameLen, NULL);
	MlmeFreeMemory(pOutBuffer);
...
VOID SendBeaconRequest(RTMP_ADAPTER *pAd, UINT16 Wcid)
{
	...
	entry = entry_get(pAd, Wcid);
	if (!entry || IS_ENTRY_PEER_AP(entry) || IS_ENTRY_REPEATER(entry))
		return;
	...
	apidx = entry->apidx;
	ActHeaderInit(pAd, &Frame.Hdr, entry->Addr, pAd->ApCfg.MBSSID[apidx].wdev.if_addr, pAd->ApCfg.MBSSID[apidx].wdev.bssid);
	...
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen, NULL);
	MlmeFreeMemory(pOutBuffer);
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_INFO, "Radio - SendBeaconRequest\n");
}
#endif /* CONFIG_AP_SUPPORT */
...
void sta_send_event_report(struct _RTMP_ADAPTER *pAd, struct _MLME_QUEUE_ELEM *Elem)
{
	...
	MakeOutgoingFrame(buffer, &frame_len,
			...
			END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, buffer, frame_len, NULL);
	MlmeFreeMemory(buffer);

	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "WNM event report\n");
#endif
}
#endif

#ifdef DOT11_EHT_BE
VOID PeerProtectEHTAction(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	struct eht_prot_action_frame *eht_prot_act = (struct eht_prot_action_frame *)Elem->Msg;
	UINT8 Action = eht_prot_act->prot_eht_action, *pos;
	struct wifi_dev *wdev = Elem->wdev;
	MAC_TABLE_ENTRY *pEntry = NULL;
	struct mld_entry_t *mld_entry = NULL;
	uint16_t mld_sta_idx, mld_link_num;

	if (!VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid) || !wdev)
		return;
	...
	case EHT_PROT_ACT_T2LM_TEARDOWN:
		nt2lm_peer_t2lm_req_action(pAd, Elem);
		break;
#ifdef EPCS_SUPPORT
	case EHT_PROT_ACT_EPCS_REQUEST:
	case EHT_PROT_ACT_EPCS_RESPONSE:
	case EHT_PROT_ACT_EPCS_TEARDOWN:
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_EHT, DBG_LVL_NOTICE,
			"received EPCS Action(%d) Frame.\n", Action);
		...
		break;
#endif
	case EHT_PROT_ACT_EML_OP_NOTIF:
		{
			UINT32 eml_mode_bmap = 0;
			struct mld_eml_op eml_op = {0};
			...
			if (!mld_entry) {
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_EHT, DBG_LVL_ERROR,
					"[EML Op Notif] mld_entry=NULL\n");
				mt_rcu_read_unlock();
				break;
			}

			mld_link_num = mld_entry->link_num;
			mld_sta_idx = mld_entry->mld_sta_idx;
			mt_rcu_read_unlock();

			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_EHT, DBG_LVL_INFO,
				"[EML Op Notif] MsgLen=%ld, wcid=%d, wdev=%s, assoc_link_num=%d\n",
				Elem->MsgLen, Elem->Wcid, wdev->if_dev->name,
				mld_link_num);

			pos = eht_prot_act->data;

			NdisMoveMemory(&eml_op.dialog_token, pos, sizeof(eml_op.dialog_token));
			pos += sizeof(eml_op.dialog_token);

			NdisMoveMemory(&eml_mode_bmap, pos, sizeof(eml_mode_bmap));

			eml_op.mld_sta_idx = mld_sta_idx;
			eml_op.emlsr_en = GET_DOT11BE_EML_CTRL_EMLSR_MODE(eml_mode_bmap);
			eml_op.emlmr_en = GET_DOT11BE_EML_CTRL_EMLMR_MODE(eml_mode_bmap);
			eml_op.emlsr_param_upd_ctrl = GET_DOT11BE_EML_CTRL_EMLSR_PARAM_UPD_CTRL(eml_mode_bmap);
			if (eml_op.emlsr_en || eml_op.emlmr_en) { /* D3.0 */
				eml_op.eml_link_bmap = GET_DOT11BE_EML_CTRL_EMLSR_LINK_BMAP(eml_mode_bmap);
				eml_op.eml_link_bmap = cpu_to_le16(eml_op.eml_link_bmap);
			}
			eml_op.priv_ptr = (void *)pEntry;

			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_EHT, DBG_LVL_ERROR,
				"[EML Op Notif] mld_sta_idx=%d, EMLSR_en=%d, EMLSR_bmap=0x%x, token=%d\n",
				eml_op.mld_sta_idx, eml_op.emlsr_en, eml_op.eml_link_bmap,
				eml_op.dialog_token);

			if (wdev->wdev_type == WDEV_TYPE_AP)
				bss_mngr_mld_eml_op_update(wdev, &eml_op);
		}
		break;
