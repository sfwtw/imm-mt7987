...
VOID BuildBgndScanChList(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (is_aband) {
		...
		if (vht_bw == VHT_BW_80)
			...
			pAd->BgndScanCtrl.ScanBW = BW_320;
#endif
		else
#endif /* DOT11_VHT_AC */
		...
		for (channel_idx = 0; channel_idx < pChCtrl->ChListNum; channel_idx++) {
			...
			&& (eht_bw != EHT_BW_320)
				...
				if (eht320_channel_group(pAd, ch, wdev)) {
					UCHAR ch_band = wlan_config_get_ch_band(wdev);

					pAd->BgndScanCtrl.BgndScanChList[ChListNum].Channel = pChCtrl->ChList[channel_idx].Channel;
					pAd->BgndScanCtrl.BgndScanChList[ChListNum].CenChannel = eht_cent_ch_freq(wdev, ch, EHT_BW_320, ch_band);
					pAd->BgndScanCtrl.BgndScanChList[ChListNum].DfsReq = pChCtrl->ChList[channel_idx].DfsReq;
					pAd->BgndScanCtrl.BgndScanChList[ChListNum].SkipChannel = BackgroundScanSkipChannelCheck(pAd, ch);
					ChListNum++;
				}
	...
	for (channel_idx = 0; channel_idx < pAd->BgndScanCtrl.ChannelListNum; channel_idx++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
				"Support channel: PrimCh=%d, CentCh=%d, DFS=%d\n",
	...
	OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(Sm, BGND_SCAN_LISTEN, BGND_MAINRADIO_RX_SCAN, (STATE_MACHINE_FUNC)mainRadio_rx_hist_scan_timeout_action);
#endif
	...
	IN struct wifi_dev *wdev)
{
	...
	*/
	...
	if (pAd->BgndScanCtrl.BgndScanSupport) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"Background scan doesn't support in current settings....\n");
}
...
#ifdef WIFI_UNIFIED_COMMAND
	INT32 ret = 0;
	struct _UNI_CMD_RDD_IPI_HIST_T uni_cmd_rdd_ipi_hist;
#else
	EXT_CMD_RDD_IPI_HIST_T cmd_rdd_ipi_hist;
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
	os_zero_mem(&uni_cmd_rdd_ipi_hist, sizeof(struct _UNI_CMD_RDD_IPI_HIST_T));
#else
	os_zero_mem(&cmd_rdd_ipi_hist, sizeof(EXT_CMD_RDD_IPI_HIST_T));
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
		/* Read clear below CR */
		uni_cmd_rdd_ipi_hist.ipi_hist_idx = RDD_SET_IPI_HIST_RESET;
		uni_cmd_rdd_ipi_hist.set_val = 0;
		uni_cmd_rdd_ipi_hist.band_idx = band_idx;
		ret = UniCmdSetRddIpiHist(pAd, &uni_cmd_rdd_ipi_hist);
#else
		/* Read clear below CR */
		cmd_rdd_ipi_hist.ipi_hist_idx = RDD_SET_IPI_HIST_RESET;
		cmd_rdd_ipi_hist.set_val = 0;
...
NDIS_STATUS set_ipi_scan_hist_proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING * arg)
{
	...
	struct _UNI_CMD_RDD_IPI_SCAN_T cmd_rdd_ipi_scan;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	struct wifi_dev *wdev =
		get_wdev_by_ioctl_idx_and_iftype(pAd, pObj->ioctl_if, pObj->ioctl_if_type);

	os_zero_mem(&cmd_rdd_ipi_scan, sizeof(cmd_rdd_ipi_scan));

	RTMPInitTimer(pAd, &pAd->BgndScanCtrl.hist_scan_timer,
		GET_TIMER_FUNCTION(mainRadio_rx_hist_scan_timeout), pAd, FALSE);
	...
	if (arg) {
		...
		if (kstrtol(threshold, 10, &value) == 0)
			pAd->BgndScanCtrl.ipi_th = (UCHAR)value;
		if (kstrtol(period, 10, &value) == 0)
			pAd->BgndScanCtrl.dfs_ipi_period = (UINT32)value;

		pAd->BgndScanCtrl.band_idx = band_idx;
		pAd->BgndScanCtrl.antena_idx = (INT8)ant_index;
		...
		status = UniCmdSetRddIpiScan(pAd, &cmd_rdd_ipi_scan);
...
VOID mainRadio_rx_hist_scan_timeout(
	IN PVOID SystemSpecific1,
	IN PVOID FunctionContext,
	IN PVOID SystemSpecific2,
	IN PVOID SystemSpecific3)
{
	...
	MlmeEnqueue(pAd, BGND_SCAN_STATE_MACHINE, BGND_MAINRADIO_RX_SCAN, 0, NULL, 0);
	RTMP_MLME_HANDLER(pAd);
...
VOID mainRadio_rx_hist_scan_timeout_action(
	RTMP_ADAPTER * pAd,
	MLME_QUEUE_ELEM *Elem)
{
	struct _UNI_EVENT_RDD_IPI_SCAN_T event_rdd_ipi_hist;
	struct _UNI_CMD_RDD_IPI_SCAN_T cmd_rdd_ipi_scan;
	UINT8 ipi_idx = 0;
	UCHAR ipi_th = pAd->BgndScanCtrl.ipi_th;
	UINT32 ipi_hist_val = 0;
	UINT32 free_cnt = 0;
	UINT64 ipi_percent = 0, self_idle_ratio = 0, ipi_idle_ratio = 0, ch_load = 0;
	UINT32 threshold = 0;
	UINT32 period = 0;
#define INC_PRECISION 1000
	...
	os_zero_mem(&cmd_rdd_ipi_scan, sizeof(cmd_rdd_ipi_scan));
	os_zero_mem(&event_rdd_ipi_hist, sizeof(event_rdd_ipi_hist));
	cmd_rdd_ipi_scan.u1Band = pAd->BgndScanCtrl.band_idx;
	cmd_rdd_ipi_scan.u1pdSetting = 1;

	UniCmdGetRddIpiScan(pAd, &cmd_rdd_ipi_scan, &event_rdd_ipi_hist);
	...
	for (i = 0; i < antena_count; i++) {
		free_cnt = 0;
		ipi_hist_val = 0;
		for (ipi_idx = 0; ipi_idx < POWER_INDICATE_HIST_NUMBER_MAX; ipi_idx++) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
					"ipi[%d],\t ipi_hist_cnt=\t%d\n",
					ipi_idx,
				event_rdd_ipi_hist.au4IpiHistVal[start_idx][ipi_idx]);

			/* Get IPI2-IPI10 total cnt */
			if (ipi_th <= ipi_idx && ipi_idx <= RDD_IPI_HIST_10)
				ipi_hist_val += event_rdd_ipi_hist.au4IpiHistVal[start_idx][ipi_idx];

			/* Get IPI free run cnt */
			free_cnt += event_rdd_ipi_hist.au4IpiHistVal[start_idx][ipi_idx];
		}
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
					"tx_assert_time %d (ms)\n", event_rdd_ipi_hist.u4TxAssertTime / 1000);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
					"Period %d (ms)\n", pAd->BgndScanCtrl.dfs_ipi_period);
		/* channel_load */
		ipi_percent = ((UINT64)INC_PRECISION * 100 * ipi_hist_val) / free_cnt;
		...
		self_idle_ratio = (INC_PRECISION * 100 * (UINT64)(period - (event_rdd_ipi_hist.u4TxAssertTime / 1000))) / period;
		...
		if (self_idle_ratio <= threshold * INC_PRECISION) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
					"band[%d] - self_idle_ratio %llu%% < threshold %u%%\n",
				event_rdd_ipi_hist.ucBandIdx, self_idle_ratio/INC_PRECISION, threshold/INC_PRECISION);
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
			"ch_load %llu%%, band[%d] antena idx[%d] - self_idle_ratio %llu%%, idle_ratio %llu%%\n",
...
{
#ifdef WIFI_UNIFIED_COMMAND
	struct UNI_EVENT_RDD_IPI_HIST uni_rdd_ipi_hist_rlt;
	INT32 ret = 0;
#else
	EXT_EVENT_RDD_IPI_HIST rdd_ipi_hist_rlt;
#endif
...
#define INC_PRECISION 1000
#ifdef WIFI_UNIFIED_COMMAND
	os_zero_mem(&uni_rdd_ipi_hist_rlt, sizeof(struct UNI_EVENT_RDD_IPI_HIST));

	uni_rdd_ipi_hist_rlt.u1BandIdx = pAd->BgndScanCtrl.band_idx;
	ret = UniCmdGetRddIpiHist(pAd, RDD_IPI_HIST_ALL_CNT, &uni_rdd_ipi_hist_rlt);
	for (ipi_idx = 0; ipi_idx < (IPI_HIST_TYPE_NUM - 1); ipi_idx++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_WARN,
				"ipi[%d],\t ipi_hist_cnt=\t%d\n",
				ipi_idx,
				uni_rdd_ipi_hist_rlt.u4IpiHistVal[ipi_idx]);

		/* Get IPI free run cnt */
		free_cnt += uni_rdd_ipi_hist_rlt.u4IpiHistVal[ipi_idx];

		/* Get IPI2-IPI10 total cnt */
		if (ipi_th <= ipi_idx && ipi_idx > RDD_IPI_HIST_6 && ipi_idx <= RDD_IPI_HIST_10)
			ipi_hist_val += uni_rdd_ipi_hist_rlt.u4IpiHistVal[ipi_idx];
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			...
			ipi_th, ipi_hist_val, free_cnt);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"tx_assert_time %d (ms)\n", uni_rdd_ipi_hist_rlt.u4TxAssertTime / 1000);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"Period %d (ms)\n", pAd->BgndScanCtrl.dfs_ipi_period);
	...
	period = pAd->BgndScanCtrl.dfs_ipi_period; /* ms */
	self_idle_ratio = (INC_PRECISION * 100 * (period - (uni_rdd_ipi_hist_rlt.u4TxAssertTime / 1000))) / period;
	/* Channel_Load = (Self_Idle_Ratio - Idle_Ratio)/Self_Idle_Ratio */
	if (self_idle_ratio < ipi_idle_ratio)
		ch_load = 0;
	else
	...
	if (self_idle_ratio <= threshold * INC_PRECISION) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_ERROR,
				...
				uni_rdd_ipi_hist_rlt.u1BandIdx, self_idle_ratio/INC_PRECISION, threshold/INC_PRECISION);
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			...
			ch_load, uni_rdd_ipi_hist_rlt.u1BandIdx, self_idle_ratio/INC_PRECISION, ipi_idle_ratio/INC_PRECISION);
...
#define DIFF_TIME(record_time, cur_time) \
	((record_time <= cur_time) ? (cur_time - record_time) : ((0xFFFFFFFF - record_time) + cur_time))

static UCHAR dfs_pre_cac_get_next_detect_channel(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev)
{
	UINT_8 i;
	PCHANNEL_CTRL pChCtrl = NULL;
	ULONG max_diff_time = 0, cur_time, diff_time;
	UCHAR oldest_cac_ch = 0, inband_ch = 0;
	UCHAR band_idx = 0;
	DFS_PARAM *pDfsParam = &pAd->CommonCfg.DfsParameter;
	...
	if (wdev) {
		inband_ch = wdev->channel;
		band_idx = HcGetBandByWdev(wdev);

		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
			"band_idx=%d, cur_time=%lu, inband_ch=%d\n", band_idx, cur_time, inband_ch);
	} else {
		if (pAd->CommonCfg.DfsParameter.OutBandCh == 0) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_ERROR, "OutBandCh=0!\n");
			return 0;
		}
		band_idx = dfs_get_band_by_ch(pAd, pAd->CommonCfg.DfsParameter.OutBandCh);
	}
	...
	for (i = 0; i < pChCtrl->ChListNum; i++) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
			"ch=%d, dfsReq=%d, nonOccup=%d, flags=0x%x, cac_time=%lu\n",
			pChCtrl->ChList[i].Channel, pChCtrl->ChList[i].DfsReq,
			pChCtrl->ChList[i].NonOccupancy, pChCtrl->ChList[i].Flags,
			pChCtrl->ChList[i].cac_done_timestamp);
		if (!pChCtrl->ChList[i].DfsReq)
			continue;

		if (pChCtrl->ChList[i].NonOccupancy)
			continue;

		if (wdev && pChCtrl->ChList[i].Channel == inband_ch)
			continue;

		if (DfsPrimToCent(pChCtrl->ChList[i].Channel, pDfsParam->band_bw)
			== DfsPrimToCent(inband_ch, pDfsParam->band_bw))
			continue;

		if (!(pChCtrl->ChList[i].Flags
			& CHANNEL_CAC_DONE)) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
				"band_idx=%d, cac not done ch=%d\n", band_idx, pChCtrl->ChList[i].Channel);
			/* if cac not do, return as next cac channel */
			return pChCtrl->ChList[i].Channel;
		}

		diff_time = DIFF_TIME(pChCtrl->ChList[i].cac_done_timestamp, cur_time);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
			"band_idx=%d, diff_time=%lu\n", band_idx, diff_time);
		if (diff_time > max_diff_time) {
			max_diff_time = diff_time;
			oldest_cac_ch = pChCtrl->ChList[i].Channel;
		}
	}

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
		"band_idx=%d, oldest_cac_ch=%d, max_diff_time=%lu\n", band_idx, oldest_cac_ch, max_diff_time);

	return oldest_cac_ch;
}

VOID dfs_pre_cac_detect_next_channel(IN RTMP_ADAPTER * pAd, IN struct wifi_dev *wdev)
{
	DFS_PARAM *pDfsParam = &pAd->CommonCfg.DfsParameter;
	ENUM_DFS_INB_CH_SWITCH_STAT_T *ch_stat = &pDfsParam->inband_ch_stat;
	UCHAR band_idx = 0;
	...
	if (pDfsParam->bPreCacEn
		...
		next_ch = dfs_pre_cac_get_next_detect_channel(pAd, wdev);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO, "next_ch=%d, out-bw=%d\n", next_ch, pDfsParam->OutBandBw);
		...
		if (next_ch != 0) {
			...
			case DFS_INB_DFS_RADAR_OUTB_CAC_DONE_QUICK:
				if (pDfsParam->OutBandCh != 0)
					break;

				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
					"Fall into OUTB_CH_CAC.\n");
				*ch_stat = DFS_INB_DFS_OUTB_CH_CAC;
				fallthrough;
			case DFS_INB_DFS_OUTB_CH_CAC_DONE:
				DfsDedicatedOutBandSetChannel(pAd, next_ch, pDfsParam->pre_cac_BW, RDD_DEDICATED_RX);
				return;
...
VOID dfs_pre_cac_start_detect(IN RTMP_ADAPTER * pAd, IN struct wifi_dev *wdev)
{
	...
	if (pAd->CommonCfg.DfsParameter.bPreCacEn
		...
		ENUM_DFS_INB_CH_SWITCH_STAT_T *ch_stat = &pAd->CommonCfg.DfsParameter.inband_ch_stat;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	// if (pAd->CommonCfg.DfsParameter.bOcacEnable == 0)
	// *ch_stat = DFS_OUTB_CH_CAC;
#endif
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE, "*ch_stat=%d\n", *ch_stat);
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!pDfsParam->bDfsDedicatedRxPreselectCh && (*ch_stat == DFS_INB_CH_SWITCH_CH)) {
		/* Do not pre-select DFS channel for dedicated RX */
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
				 "Disable pre-select DFS channel, stop dedicated RX\n");
	...
	if (pAd->CommonCfg.DfsParameter.SetChByCmd == TRUE) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_NOTICE,
		"Do not get new outband DFS channel for set channel flow\n");
		pAd->CommonCfg.DfsParameter.SetChByCmd = FALSE; // init set channel flag
	} else {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
	...
	if (pAd->CommonCfg.DfsParameter.bOcacEnable == 1)
		return;
	...
	case DFS_INB_DFS_OUTB_CH_CAC_DONE:
		...
		*ch_stat = DFS_INB_DFS_OUTB_CH_CAC;
		if (pAd->CommonCfg.DfsParameter.SetOutBandChStat == OUTB_SET_CH_CAC_DONE)
	...
	IN PVOID SystemSpecific3)
{
	...
	switch (*ch_stat) {
	case DFS_OUTB_CH_CAC:
		DfsDedicatedOutBandSetChannel(pAd, *ch_outband, *phy_bw_outband, RDD_DEDICATED_IDX);
		break;
	...
	case DFS_INB_DFS_RADAR_OUTB_CAC_DONE:
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_INFO,
			"New inband channel %d bandidx %d\n",
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	BACKGROUND_SCAN_CTRL *BgndScanCtrl = &pAd->BgndScanCtrl;
	ChBusyTime = pAd->OneSecMibBucket.ChannelBusyTime;
	MyTxAirTime = pAd->OneSecMibBucket.MyTxAirtime;
	MyRxAirTime = pAd->OneSecMibBucket.MyRxAirtime;
	...
	IN UCHAR bgnd_scan_type)
{
	...
	if (pChipCap->uni_cmd_support)
		UniCmdOffChScan(pAd, &offch_cmd_cfg);
	else
#endif /* WIFI_UNIFIED_COMMAND */
	...
	IN UCHAR bgnd_scan_type)
{
	...
	UCHAR control_ch_outband = 0, central_ch_outband = 0, bw_outband = 0;
    BOOLEAN is_aband = FALSE;
	// UCHAR bgnd_band_scan_info = pAd->BgndScanCtrl.ScanType; /*unused*/
	UINT8 band_idx = 0;
	USHORT PhyMode = 0;
#ifdef MT_DFS_SUPPORT
	CHAR out_band_ch = GET_BGND_PARAM(pAd, OUTBAND_CH);
	...
	if ((BandIdx == BAND1)
		...
		|| ((pAd->CommonCfg.DfsParameter.bDfsSdbEnable == TRUE) &&
			BandIdx == BAND0)
#endif /* DFS_SDB_SUPPORT */
	) {
		in_band_ch = GET_BGND_PARAM(pAd, INBAND_CH_BAND1);
		in_band_bw = GET_BGND_PARAM(pAd, INBAND_BW_BAND1);
	} else if (BandIdx == BAND2) {
		in_band_ch = GET_BGND_PARAM(pAd, INBAND_CH_BAND2);
		in_band_bw = GET_BGND_PARAM(pAd, INBAND_BW_BAND2);
	}
#endif
	...
	IN UCHAR enabled)
{
	...
	start_idx = 1;
	end_idx = hc_get_chip_wtbl_max_num(pAd) - 1;
