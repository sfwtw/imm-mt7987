...
#define HTC_ENABLE_HE_OM_IE BIT(0)
#define HTC_ENABLE_EHT_OM_IE BIT(1)
#define HTC_ENABLE_UPH_OM_IE BIT(2)

#define IS_HTC_ENABLE_HE_OM_IE(_bmp) (_bmp & HTC_ENABLE_HE_OM_IE)
#define IS_HTC_ENABLE_EHT_OM_IE(_bmp) (_bmp & HTC_ENABLE_EHT_OM_IE)
#define IS_HTC_ENABLE_UPH_OM_IE(_bmp) (_bmp & HTC_ENABLE_UPH_OM_IE)

#define HTC_OM_VHT_VALID BIT(0)
#define HTC_OM_HE_VALID BIT(1)

#define ACTRL_CLASS_OM_LEN  16
#define ACTRL_CLASS_UPH_LEN  12
#define ACTRL_CLASS_EHT_OM_LEN  10

#define ACTRL_CLASS_CTRL_FIELD_LEN  4
#define SET_HTC_OM_IE(_htc, _para) (_htc = (_htc | _para))

#define FLAG_QOS_DATA		(1 << 0)
#define FLAG_HTC_CTRL		(1 << 1)
#define CHECK_PKT_CTRL_FLAG(_map, _idx)	(_map & _idx)
...
struct SEND_QOS_INFO {
	UINT32 u4wcid;
	UINT32 u4CtrlFlag;
	UINT32 u4HtcFlag;
	struct HE_ACTRL_OMI *prHeActrl;
	struct EHT_ACTRL_OMI *prEhtActrl;
	struct HE_ACTRL_UPH *prHeActrlUph;
};
...
struct capi_set_cmd ap_set_rfeature_capi[] = {
	...
	{"coding_type", ap_set_rfeature_coding_type},
	{"bsrp_per_txop", ap_set_rfeature_bsrp_per_txop},
	{"trig_variant", ap_set_rfeature_trig_variant},
	...
	{"disable_he_muedca", ap_set_rfeature_dis_he_muedca},
	{"bf_in_acq", ap_set_rfeature_bf_in_acq},
#ifdef DOT11_EHT_BE
	{"eht_ap_omctrl", ap_set_rfeature_eht_om_ctrl},
	{"eht_ap_epcs", ap_set_rfeature_eht_nsep_priority_access},
	{"eht_ap_max_mpdu", ap_set_rfeature_eht_max_mpdu_len},
	{"eht_cmm_nominal_pad", ap_set_rfeature_eht_cmm_nominal_pad},
#endif
	/* STA MU EDCA */
	...
	/* eht phy cap */
#ifdef DOT11_EHT_BE
	{"mcs14_disable", ap_set_rfeature_mcs14_disable},
	{"mcs15_in_mru_disable", ap_set_rfeature_ap_mcs15_in_mru_disable},
	{"extra_eht_ltf_disable", ap_set_rfeature_ap_extra_eht_ltf_disable},
#endif
...
struct capi_set_cmd dev_send_frame_capi[] = {
	...
	{"om_qos_null", dev_send_frame_om_qos_null},
	...
	if ((capi_s != NULL) && (strlen(capi_s) > 0)) {
		for (i = 0; i < (sizeof(ap_set_rfeature_capi)/sizeof(struct capi_set_cmd)); i++) {
			if (strcmp((ap_set_rfeature_capi + i)->capi, capi_s) == 0) {
				if (strcmp("trig_variant", capi_s) == 0) {
					MTWF_PRINT("func:%s, [capi match], arg: %s\n", __func__, arg);
					(ap_set_rfeature_capi + i)->set_func(ad, arg);
				} else if (strcmp("trig_type", capi_s) == 0) {
					MTWF_PRINT("func:%s, [capi match], arg: %s\n", __func__, arg);
					(ap_set_rfeature_capi + i)->set_func(ad, arg);
				} else {
...
UINT32 construct_htc_ie(struct _RTMP_ADAPTER *ad,
				  struct wifi_dev *wdev,
				  struct SEND_QOS_INFO *prQosInfo)
{
	UINT32 u4EnableBmp, u4HtcField = 0;
	UINT8 ucShift = 0;
	struct HE_ACTRL_OMI *prHe_actrl;
	struct EHT_ACTRL_OMI *prEht_actrl;
	struct HE_ACTRL_UPH *prHe_actrl_uph;

	u4EnableBmp = prQosInfo->u4HtcFlag;
	prHe_actrl = prQosInfo->prHeActrl;
	prEht_actrl = prQosInfo->prEhtActrl;
	prHe_actrl_uph = prQosInfo->prHeActrlUph;

	if (WMODE_CAP_AC(wdev->PhyMode))
		SET_HTC_OM_IE(u4HtcField, HTC_OM_VHT_VALID);

	if (WMODE_CAP_AX(wdev->PhyMode))
		SET_HTC_OM_IE(u4HtcField, HTC_OM_HE_VALID);

	/* VHT valid and HE Valid */
	ucShift += 2;

	if (IS_HTC_ENABLE_HE_OM_IE(u4EnableBmp)) {
		u4HtcField |= WLAN_ACTRL_OM << (ucShift);
		u4HtcField |= *(PUINT16)prHe_actrl << (ucShift + ACTRL_CLASS_CTRL_FIELD_LEN);
		ucShift += ACTRL_CLASS_OM_LEN;
	}

	if (IS_HTC_ENABLE_EHT_OM_IE(u4EnableBmp)) {
		u4HtcField |= WLAN_ACTRL_EHT_OM << (ucShift);
		u4HtcField |= *(PUINT8)prEht_actrl <<  (ucShift + ACTRL_CLASS_CTRL_FIELD_LEN);
		ucShift += ACTRL_CLASS_EHT_OM_LEN;
	}

	if (IS_HTC_ENABLE_UPH_OM_IE(u4EnableBmp)) {
		u4HtcField |= WLAN_ACTRL_UPH << (ucShift);
		if ((ucShift + ACTRL_CLASS_CTRL_FIELD_LEN) > 31)
			u4HtcField |= 0;
		else
			u4HtcField |= *(PUINT8)prHe_actrl_uph << (ucShift + ACTRL_CLASS_CTRL_FIELD_LEN);
		ucShift += ACTRL_CLASS_UPH_LEN;
	}

	if (ucShift > 32) {
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"func:%s, HT Field out of len %d\n", __func__, ucShift);
	}

	return u4HtcField;
}

VOID dev_send_frame_qos_null(
	...
	struct SEND_QOS_INFO *prQosInfo)
{
	...
	pEntry = entry_get(pAd, prQosInfo->u4wcid);
	u2FramLen = sizeof(HEADER_802_11);

	if (NState != NDIS_STATUS_SUCCESS)
		return;

	MgtMacHeaderInit(pAd, pNullFr, SUBTYPE_DATA_NULL, 0, pEntry->Addr,
	...
	if (CHECK_PKT_CTRL_FLAG(prQosInfo->u4CtrlFlag, FLAG_QOS_DATA)) {
		pNullFr->FC.SubType = SUBTYPE_QOS_NULL;

		pucQos = ((UCHAR *)pNullFr) + u2FramLen;
		pucQos[0] = (1 << 4);	/* EOSP bit4=1 */
		pucQos[1] = 0;
		u2FramLen += 2;
	}

	if (CHECK_PKT_CTRL_FLAG(prQosInfo->u4CtrlFlag, FLAG_HTC_CTRL)) {
		UINT32 u4HtcField = 0;

		pNullFr->FC.Order |= 1;/*set HTC bit in frame control.*/

		pucHtc = ((UCHAR *)pNullFr) + u2FramLen;

		u4HtcField = construct_htc_ie(pAd, wdev, prQosInfo);
		NdisCopyMemory(pucHtc, &u4HtcField, sizeof(UINT32));
		u2FramLen += sizeof(UINT32);
	}

	pNullFr->Duration = RTMPCalcDuration(pAd, pEntry->CurrTxRate, u2FramLen);

	MiniportMMRequest(pAd, 0, (PUCHAR)pNullFr, u2FramLen, NULL);
...
VOID dev_send_frame_smpdu(VOID *ad, VOID *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_DEBUG,
		"wcid = %d txop %d interval %d\n", wcid, txopDur, interval);
	MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_DEBUG, "\n");
#endif /* DOT11_HE_AX */
}
...
VOID dev_send_frame_uncast_deauth(VOID *ad, VOID *arg)
{
	...
	INT protect = os_str_tol(strsep((char **) &str, "-"), 0, 10);
...
VOID dev_send_frame_om_qos_null(VOID *ad, VOID *arg)
{
	struct _RTMP_ADAPTER *rad = (struct _RTMP_ADAPTER *) ad;
	RTMP_STRING *assign_he_str = NULL;
	RTMP_STRING *assign_eht_str = NULL;
	RTMP_STRING *assign_uph_str = NULL;
	RTMP_STRING *assign_wcid_str = NULL;
	RTMP_STRING *assign_rate_str = NULL;
	struct HE_ACTRL_OMI he_actrl;
	struct EHT_ACTRL_OMI eht_actrl;
	struct HE_ACTRL_UPH he_actrl_uph;
	UINT32 u4EnableBmp = 0;
	CHAR *pch = NULL;
	UINT32 setting_value = 0, u4Wcid;
	struct SEND_QOS_INFO rQosInfo;
	POS_COOKIE pObj = (POS_COOKIE)rad->OS_Cookie;
	struct wifi_dev *wdev = get_wdev_by_ioctl_idx_and_iftype(ad, pObj->ioctl_if, pObj->ioctl_if_type);

	NdisZeroMemory(&rQosInfo, sizeof(struct SEND_QOS_INFO));

	if (arg) {
		assign_wcid_str = strstr(arg, "wcid:");
		if (assign_wcid_str) {
			pch = strchr(assign_wcid_str, ':');
			if (!pch) {
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"null pointer\n");
				return;
			}
			u4Wcid = (UINT32)os_str_tol(pch + 1, 0, 16);
		} else {
			MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"null pointer\n");
			return;
		...
		assign_he_str = strstr(arg, "he:");
		if (assign_he_str) {
			pch = strchr(assign_he_str, ':');
			if (!pch) {
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"null pointer\n");
				return;
			...
			NdisCopyMemory(&he_actrl,
				&setting_value,
				sizeof(struct HE_ACTRL_OMI));

			u4EnableBmp |= HTC_ENABLE_HE_OM_IE;
		} else {
			MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"null pointer\n");
			return;
		...
		assign_eht_str = strstr(arg, "eht:");
		if (assign_eht_str) {
			pch = strchr(assign_eht_str, ':');
			if (!pch) {
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"null pointer\n");
				return;
			...
			setting_value = (UINT32)os_str_tol(pch + 1, 0, 16);
			NdisCopyMemory(&eht_actrl,
				&setting_value,
				sizeof(struct EHT_ACTRL_OMI));

			u4EnableBmp |= HTC_ENABLE_EHT_OM_IE;
		} else {
			MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"null pointer\n");
			return;
		...
		assign_uph_str = strstr(arg, "uph:");
		if (assign_uph_str) {
			pch = strchr(assign_uph_str, ':');
			if (!pch) {
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"null pointer\n");
				return;
		...
		assign_rate_str = strstr(arg, "rate:");
		if (assign_rate_str) {
			pch = strchr(assign_rate_str, ':');
			if (!pch) {
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
				"null pointer\n");
				return;
			...
			if (setting_value == 0)
				wdev->bMgmtQAutoRate = FALSE;
			else
				wdev->bMgmtQAutoRate = TRUE;
		} else {
			MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"null pointer\n");
			return;
		...
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"iwpriv ra0 set dev_send_frame=om_qos_null-para:value:para:value....\n");
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"para = wcid/he/eht/uph\n");
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"he [0:2]RxNss, [3:4]Bw, [5]ULMuDisable, [6:8]TxNss [11]ULMuDataDisable\n");
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"eht [0]RxNssExt, [1]BwExt, [2]TxNssExt\n");
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"uph [0:4]UPH, [5]MinPwrFlag\n");
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR,
			"rate 0:Fixed rate 1:Auto Rate\n");

		return;
	}

	rQosInfo.u4wcid = u4Wcid;
	rQosInfo.u4HtcFlag = u4EnableBmp;
	rQosInfo.prHeActrl = &he_actrl;
	rQosInfo.prEhtActrl = &eht_actrl;
	rQosInfo.prHeActrlUph = &he_actrl_uph;

	rQosInfo.u4CtrlFlag |= (FLAG_QOS_DATA | FLAG_HTC_CTRL);

	dev_send_frame_qos_null(rad, wdev, &rQosInfo);
}
...
VOID ap_set_wireless_fixed_mcs(VOID *ad, VOID *param)
{
	...
	MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
			"func:%s, arg:%u\n", __func__, mcs);
...
VOID ap_set_wireless_mumimo(VOID *ad, VOID *param)
{
	...
	if ((wlan_config_get_ht_bw(wdev) == BW_40)
			...
			/* <2Usr>-<RU67>-<3.2GI>-<HEFB>-<DL>-<WCID>-<MCS2> */
			ret = snprintf(mimo_param, sizeof(mimo_param), "%s",
								"2-134-0-1-0-1-2-2-2");
...
VOID ap_set_wireless_discovery_apply(VOID *ad, VOID *param)
{
	...
	struct wifi_dev *wdev = (struct wifi_dev *)param;
	UINT8 unsolicit_type;
	UINT8 tu;
	UINT8 unsolicit_txmode;
	if (!wdev)
		return;

	if (WMODE_CAP_6G(wdev->PhyMode)) {
		tu = wlan_operate_get_unsolicit_tx_tu(wdev);
		unsolicit_type = wlan_operate_get_unsolicit_tx_type(wdev);
		unsolicit_txmode = wlan_operate_get_unsolicit_tx_mode(wdev);

		in_band_discovery_update_oper(wdev, unsolicit_type, tu, unsolicit_txmode);
	}
...
#if defined(CONFIG_AP_SUPPORT) && defined(DOT11_HE_AX) && defined(CONFIG_6G_SUPPORT)
	struct wifi_dev *wdev = (struct wifi_dev *)param;
	UINT8 rnr_6g_in_probe = wlan_config_get_rnr_in_probe_rsp(wdev);
...
VOID ap_set_rfeature_trig_type(VOID *ad, VOID *param)
{
#ifdef DOT11_HE_AX
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	RTMP_STRING  *str = (RTMP_STRING *) param;
	UINT8 trig_type = 0;
	...
	pch = strsep((char **) &str, "-");
	trig_type = os_str_tol(pch, 0, 10);

	/* Set trigger band if assign */
	if (str != NULL) {
		pch = strsep((char **) &str, "-");
	...
	if ((trig_type == CAPI_BASIC)) {
		char mu_ru_bsrp[15] = "1-0-5-67-0-";

		mu_ru_bsrp[11] = u1Band + '0';
		mu_ru_bsrp[12] = '\0';
		/*iwpriv ra0 set set_muru_bsrp_ctrl=1-0-5-67-0-band;*/
		SetMuruBsrpCtrl(adapt, mu_ru_bsrp); /* set timer 5 ms */
	} else if (trig_type == CAPI_BSRP) {
		char mu_ru_bsrp[15] = "1-0-5-67-4-";

		mu_ru_bsrp[11] = u1Band + '0';
		mu_ru_bsrp[12] = '\0';
		/*iwpriv ra0 set set_muru_bsrp_ctrl=1-0-5-67-4-band;*/
		SetMuruBsrpCtrl(adapt, mu_ru_bsrp); /* set timer 5 ms */
...
VOID ap_set_rfeature_trig_variant(VOID *ad, VOID *param)
{
#ifdef DOT11_EHT_BE

	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	RTMP_STRING  *str = (RTMP_STRING *) param;
	UINT8 trig_variant = os_str_tol(str, 0, 10);

	MTWF_PRINT("%s(%d): trig_variant = %d\n",  __func__, __LINE__, trig_variant);
	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
			"arg:%d\n", trig_variant);

	/* EXT_CID, EXT_CMD_ID_MURU_CTRL, MURU_SET_TRIG_TYPE */
	set_muru_trig_variant(adapt, trig_variant);
#endif /* DOT11_EHT_BE */
...
#ifdef DOT11_HE_AX
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	UINT32 wcid, startWcid = 0;
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP * pChipCap = hc_get_chip_cap(adapt->hdev_ctrl);
#endif /* WIFI_UNIFIED_COMMAND */
	UINT8 he_ltf_old[CAPI_HE_LTF_NUM] = {
...
VOID ap_set_rfeature_he_gi(VOID *ad, VOID *param)
{
	...
	RTMP_CHIP_CAP * pChipCap = hc_get_chip_cap(adapt->hdev_ctrl);
#endif /* WIFI_UNIFIED_COMMAND */
	UINT8 he_gi_old[CAPI_HE_GI_NUM] = {
...
#endif /* WIFI_UNIFIED_COMMAND */
	UINT8 cfg_he_gi = 0;
	UINT8 input_he_gi = *((UINT8 *)param);
	MAC_TABLE_ENTRY *pEntry = NULL;
#ifdef SW_CONNECT_SUPPORT
	STA_TR_ENTRY *tr_entry = NULL;
#endif /* SW_CONNECT_SUPPORT */

	if (input_he_gi >= CAPI_HE_GI_NUM)
		return;

#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		cfg_he_gi = he_gi[input_he_gi];
	else
#endif /* WIFI_UNIFIED_COMMAND */
		cfg_he_gi = he_gi_old[input_he_gi];
...

	{
		...
		for (wcid = startWcid; VALID_UCAST_ENTRY_WCID(adapt, wcid); wcid++) {
			...
			if (pEntry && (IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_PEER_AP(pEntry) || IS_ENTRY_REPEATER(pEntry)))
				snd_ra_fw_cmd(RA_PARAM_GI_UPDATE, adapt, wcid, &cfg_he_gi);
		}
	}
#endif /*DOT11_HE_AX*/
	return;
...
VOID ap_set_rfeature_coding_type(VOID *ad, VOID *param)
{
#ifdef DOT11_HE_AX
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	UINT32 wcid = CAPI_ALL_STA, startWcid = 0;
	UINT8 cfg_codingtype = *((UINT8 *)param);
	MAC_TABLE_ENTRY *pEntry = NULL;
	char linebuf[128] = "";
	int ret = 0;
#ifdef SW_CONNECT_SUPPORT
...
#ifdef SW_CONNECT_SUPPORT
		tr_entry = tr_entry_get(adapt, pEntry->tr_tb_idx);
		if (IS_SW_STA(tr_entry))
			continue;
#endif /* SW_CONNECT_SUPPORT */

		if (pEntry && (IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_PEER_AP(pEntry) || IS_ENTRY_REPEATER(pEntry))) {
			ret = snprintf(linebuf, sizeof(linebuf), "%u-%u", wcid, cfg_codingtype);
			if (ret < 0)
				break;
#ifdef DBG
			Set_Fixed_Ecc_Update(adapt, linebuf);
#endif /* DBG */
		}
        }
#endif /*DOT11_HE_AX*/
...
VOID ap_set_he_fixed_gi_ltf_by_wcid_or_bss(RTMP_ADAPTER *pAd, UINT8 gi, UINT8 mode, UINT32 wcid, struct wifi_dev *wdev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO, "%s:: Enter.\n", __func__);
	if (!pAd)
		return;
	...
	if (ltf >= CAPI_HE_LTF_NUM) {
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR, "Invalid ltf value(%d)!!!.\n", ltf);
		return;
	}
	...
	if (pChipCap->uni_cmd_support) {
		cfg_he_gi = he_gi[gi];
		cfg_he_ltf = he_ltf[ltf];
	} else
#endif /* WIFI_UNIFIED_COMMAND */
	{
		cfg_he_gi = he_gi_old[gi];
		cfg_he_ltf = he_ltf_old[ltf];
	}
	...
	switch (mode) {
		case GI_BY_WCID:
			if (VALID_UCAST_ENTRY_WCID(pAd, wcid)) {
				...
				if (IS_SW_STA(tr_entry)) {
					MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR, "%s:: wcid (%d) is SW STA SKIP!.\n", __func__, wcid);
					break;
				}
#endif /* SW_CONNECT_SUPPORT */
				snd_ra_fw_cmd(RA_PARAM_GI_UPDATE, pAd, wcid, &cfg_he_gi);
				snd_ra_fw_cmd(RA_PARAM_HELTF_UPDATE, pAd, wcid, &cfg_he_ltf);
				MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
					"%s:: Set %s by wcid success.\n", __func__, gi_ltf_info[gi]);
			}
			else
				MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO, "%s:: Invalid wcid value(%d)!!!.\n", __func__, wcid);
			break;
		case GI_BY_BSS:
			if (!wdev) {
			...
			for (u_wcid = startWcid; VALID_UCAST_ENTRY_WCID(pAd, u_wcid); u_wcid++) {
				pEntry = entry_get(pAd, u_wcid);
				if (!(pEntry && (IS_ENTRY_CLIENT(pEntry))))
					continue;

#ifdef SW_CONNECT_SUPPORT
				tr_entry = tr_entry_get(pAd, u_wcid);
				if (IS_SW_STA(tr_entry))
					continue;
#endif /* SW_CONNECT_SUPPORT */
				...
				if (!(entry_wdev && (entry_wdev == wdev)))
					continue;
				snd_ra_fw_cmd(RA_PARAM_GI_UPDATE, pAd, u_wcid, &cfg_he_gi);
				snd_ra_fw_cmd(RA_PARAM_HELTF_UPDATE, pAd, wcid, &cfg_he_ltf);
			}
			MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
					"%s:: Set %s by BSS success.\n", __func__, gi_ltf_info[gi]);
			break;
		default:
...
VOID ap_set_rfeature_unsolicited_probe_rsp(VOID *ad, VOID *param)
{
	...
	for (IdBss = 0; IdBss < adapt->ApCfg.BssidNum; IdBss++) {
		...
		if (WMODE_CAP(wdev->PhyMode, WMODE_AX_6G) &&
			...
			if (en)
				in_band_discovery_update_conf(wdev, UNSOLICIT_TX_PROBE_RSP, 8, tx_mode, TRUE);
			else
				in_band_discovery_update_conf(wdev, UNSOLICIT_TX_FILS_DISC, 8, tx_mode, TRUE);
		}
	}
#endif /* defined(CONFIG_AP_SUPPORT) && defined(CONFIG_6G_SUPPORT) */
}

VOID ap_set_rfeature_bsrp_per_txop(VOID *ad, VOID *param)
{
#ifdef DOT11_HE_AX
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	UINT8 flag = *((UINT8 *)param);
	...
	set_muru_cert_bsrp_per_txop(adapt, flag);
#endif /*CFG_SUPPORT_FALCON_MURU*/
#endif /*DOT11_HE_AX*/
}
...
VOID ap_set_rfeature_bss_max_idle_ie(VOID *ad, VOID *param)
{
	...
	if (pMbss) {
		...
		UpdateBeaconHandler(ad, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
	}
#endif
}
...
VOID ap_set_rfeature_dis_he_muedca(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	BOOLEAN need_update_bcn = FALSE;

	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
		"func:%s, arg:%u\n", __func__, value);
	chip_cap = PD_GET_CHIP_CAP_PTR(adapt->physical_dev);

	if (value) {
		if ((chip_cap->phy_caps & fPHY_CAP_HE_UL_MUOFDMA) != 0)
			need_update_bcn = TRUE;
	} else {
		if ((chip_cap->phy_caps & fPHY_CAP_HE_UL_MUOFDMA) == 0)
			need_update_bcn = TRUE;
	}

	if (need_update_bcn == TRUE) {
		/*acquire BPCC lock*/
		if (bcn_bpcc_op_lock(ad, wdev, TRUE, BCN_BPCC_MUEDCA) == FALSE) {
			if (in_interrupt()) {
				/* defer it to workqueue */
				struct _BCN_MUEDCA_UPDATE_PARAM update_param = {0};

				update_param.MUEDCAUpdateReason = RSN_RFEATURE_DIS_MUEDCA;
				update_param.value = value;
				bcn_bpcc_ct_switch(ad, wdev, BCN_BPCC_MUEDCA, BCN_REASON(BCN_UPDATE_IE_CHG), update_param);
				return;
			} else
...
#ifdef DOT11_EHT_BE
VOID ap_set_rfeature_eht_om_ctrl(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	wlan_config_set_eht_om_ctrl(wdev, (u8)value);
	UpdateBeaconHandler(ad, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
#endif /* CONFIG_AP_SUPPORT */
}

VOID ap_set_rfeature_eht_nsep_priority_access(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	wlan_config_set_nsep_priority_access(wdev, (u8)value);
	UpdateBeaconHandler(ad, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
#endif /* CONFIG_AP_SUPPORT */
}
#endif /* DOT11_EHT_BE */

VOID ap_set_rfeature_eht_max_mpdu_len(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	struct wifi_dev *wdev = get_wdev_by_ioctl_idx_and_iftype(adapt, pObj->ioctl_if, pObj->ioctl_if_type);
	struct _RTMP_CHIP_CAP *chip_cap = PD_GET_CHIP_CAP_PTR(adapt->physical_dev);
	UINT8 value = *((UINT8 *)param);

	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
		"func:%s, arg:%u\n", __func__, value);

	if (value <= MPDU_11454_OCTETS) {
		if (value == 0)
			chip_cap->ppdu.max_mpdu_len = MPDU_3895_OCTETS;
		else if (value == 1)
...
VOID ap_set_rfeature_bf_in_acq(VOID *ad, VOID *param)
{
#ifdef DOT11_HE_AX
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	UINT8 bf_in_ac = *((UINT8 *)param);
	BOOL bf_in_ac_en = FALSE;
	...
	if (bf_in_ac == 1)
		bf_in_ac_en = TRUE;
	set_muru_bf_in_acq(adapt, bf_in_ac_en);
	...
	VOID *ad, struct wifi_dev *wdev, UINT32 aci, UINT16 value)
{
	...
	EDCA_PARM *wmm_edca_param = hwifi_get_edca(adapt, wdev);
	UCHAR ac_queue[] = {
...
#ifdef WIFI_UNIFIED_COMMAND
	if (AsicSetWmmParam(adapt, wdev, wmm_idx, ac_queue[q_idx], WMM_PARAM_TXOP, value) == NDIS_STATUS_FAILURE)
		MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_ERROR, "fails to set APTxop[%d]\n", q_idx);
#else
...
VOID ap_set_rfeature_mcs14_disable(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	if ((value & 0xFE) != 0) {
		MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_WARN,
			"Not Support !! input 0 or 1\n");
		return;
	}
	wlan_config_set_mcs14_disable(wdev, value);
#endif
}

VOID ap_set_rfeature_ap_mcs15_in_mru_disable(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	wlan_config_set_mcs15_in_mru_disable(wdev, value);
#endif
}
VOID ap_set_rfeature_ap_extra_eht_ltf_disable(VOID *ad, VOID *param)
{
#ifdef CONFIG_AP_SUPPORT
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	...
	wlan_config_set_extra_eht_ltf_disable(wdev, value);
#endif
}

VOID ap_set_rfeature_eht_cmm_nominal_pad(VOID *ad, VOID *param)
{
	struct _RTMP_ADAPTER *adapt = (struct _RTMP_ADAPTER *)ad;
	POS_COOKIE pObj = (POS_COOKIE) adapt->OS_Cookie;
	struct wifi_dev *wdev = get_wdev_by_ioctl_idx_and_iftype(adapt, pObj->ioctl_if, pObj->ioctl_if_type);
	UINT8 eht_cmm_nominal_pad = *((UINT8 *)param);

	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
		"arg:%d\n", eht_cmm_nominal_pad);

	wlan_config_set_eht_cmm_nominal_pad(wdev, eht_cmm_nominal_pad);
}
...
VOID sta_set_wireless_addbareq_bufsize(VOID *ad, VOID *param)
{
	...
	UINT16 max_ba_wsize = ba_get_default_max_ba_wsize(wdev, ad);

	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_INFO,
			"set addbareq_bufsize tx/rx win_size = %d\n", addbareq_bufsize);
	wlan_config_set_ba_txrx_wsize(wdev, addbareq_bufsize, addbareq_bufsize, max_ba_wsize);
	return;
}
...
VOID sta_set_wireless_addbaresp_bufsize(VOID *ad, VOID *param)
{
	...
	wlan_config_set_ba_txrx_wsize(wdev, addbaresp_bufsize, addbaresp_bufsize, max_ba_wsize);
	return;
