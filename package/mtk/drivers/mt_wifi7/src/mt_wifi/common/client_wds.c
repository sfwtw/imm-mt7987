	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->ApCfg.pCliWdsEntryPool) {
		...
		MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
			"Fail to alloc memory for pAd->CommonCfg.pCliWdsEntryPool");
	...
	while (pCliWdsEntry) {
		if (MAC_ADDR_EQUAL(pMac, pCliWdsEntry->Addr)) {
			...
			if ((pCliWdsEntry->Aid == Aid) && (VALID_UCAST_ENTRY_WCID(pAd, pCliWdsEntry->Aid)))
				return entry_addr_get(pAd, pCliWdsEntry->Aid);
			else {
