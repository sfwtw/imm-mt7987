...
PUCHAR MATDBEntryAlloc(IN MAT_STRUCT * pMatStruct, IN UINT32 size)
{
	...
	if (pPtr == NULL) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_ERROR,
			"allocate memory for pPtr failed!\n");
		return NULL;
	}
	return (PUCHAR)pPtr;
}
	...
	IN UINT32	entry_type)
{
	...
	if (PD_GET_MAT_CFG_STATUS(pAd->physical_dev) != MAT_ENGINE_STAT_INITED)
		return NULL;
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		if (protoType == MATProtoTb[i].protocol) {
			...
			default:
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_ERROR,
					"\033[1;31m Unexpected Entry Type \033[0m\n");
				/* assign ZERO MAC ADDR*/
				pMacAddr = ZeroMac;
				break;
			}
#ifdef DOT11_EHT_BE
			if (protoType == ETH_P_ARP || protoType == ETH_P_IPV6) {
				struct wifi_dev *wdev = &pAd->StaCfg[ifIdx].wdev;
				MAC_TABLE_ENTRY *peer_entry = GetAssociatedAPByWdev(pAd, wdev);
...
#endif
			if (pHandle->tx != NULL)
				retSkb = pHandle->tx((PVOID)PD_GET_MAT_CFG_PTR(pAd->physical_dev),
					RTPKT_TO_OSPKT(pPkt), pLayerHdr, pMacAddr);
			return retSkb;
		}
	}
	...
	IN UINT				infIdx)
{
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		if (protoType == MATProtoTb[i].protocol) {
			...
			/*			OS_SEM_LOCK(&MATDBLock); */
			if (pHandle->rx != NULL)
				pMacAddr = pHandle->rx(
					(PVOID)PD_GET_MAT_CFG_PTR(pAd->physical_dev),
					RTPKT_TO_OSPKT(pPkt), pLayerHdr, NULL);
			/*			OS_SEM_UNLOCK(&MATDBLock); */
			break;
		}
	}
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		while (i < MAX_APCLI_NUM) {
			...
			if ((pAd->StaCfg[i].ApcliInfStat.Valid == TRUE)
				...
				&& (pAd->StaCfg[i].wdev.wds_enable == 0)
...
NDIS_STATUS MATEngineExit(
	void *device)
{
	struct physical_device *ph_dev = (struct physical_device *)device;
	struct _MATProtoOps *pHandle = NULL;
	int i;

	if (ph_dev->mat_cfg.status == MAT_ENGINE_STAT_EXITED)
		return TRUE;
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		...
		if (pHandle->exit != NULL)
			pHandle->exit(&ph_dev->mat_cfg);
	}

	ph_dev->mat_cfg.status = MAT_ENGINE_STAT_EXITED;
	ph_dev->mat_cfg.nodeCount = 0;
	return TRUE;
}
	...
	void *device)
{
	...
	if (ph_dev->mat_cfg.status == MAT_ENGINE_STAT_INITED)
		return TRUE;
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		...
		if ((pHandle != NULL) && (pHandle->init != NULL)) {
			status = pHandle->init(&ph_dev->mat_cfg);

			if (status == FALSE) {
				MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_ERROR,
					"MATEngine Init Protocol (0x%x) failed!\n",
	...
	OS_NdisAllocateSpinLock(&ph_dev->mat_cfg.MATDBLock);
#ifdef MAC_REPEATER_SUPPORT
	ph_dev->mat_cfg.bMACRepeaterEn = FALSE;
#endif /* MAC_REPEATER_SUPPORT */
	ph_dev->mat_cfg.pPriv = (VOID *)ph_dev;
	ph_dev->mat_cfg.status = MAT_ENGINE_STAT_INITED;
	return TRUE;
init_failed:
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		...
		if (pHandle != NULL) {
			if (pHandle->exit != NULL) {
				status = pHandle->exit(&ph_dev->mat_cfg);
