...
#ifdef DOT11_EHT_BE
/*
 * Convert Probe response frames of reported link into STA Profile
 * in Per-STA Profile sub-element in Basic multi-link IE
 */
NDIS_STATUS eht_ap_mlo_postprocess_reported_link_probe_rsp(
	IN OUT struct buffer_wrapper *repted_rsp_buf,
	IN struct buffer_wrapper *repting_rsp_buf)
{
// #define POST_REMOVE_IE
#undef POST_REMOVE_IE

	UINT8 *repting_source = repting_rsp_buf->buf;
	ULONG repting_source_len = repting_rsp_buf->buf_len;

	UINT8 *repted_source = repted_rsp_buf->buf;
	ULONG repted_source_len = repted_rsp_buf->buf_len;
	...
	struct buffer_wrapper repting_rsp, repted_rsp, target;
	...
	/* remove 802.11 header & Timestamp: 8, Beacon Interval: 2, CapInfo: 2 */
	shift = sizeof(HEADER_802_11) + 12;
	repting_source += shift;
	repting_source_len -= shift;

	/* Step 2. then process non-setup link */
	/* remove 802.11 header & Timestamp: 8, Beacon Interval: 2 */
	shift = sizeof(HEADER_802_11) + 10;
	repted_source += shift;
	repted_source_len -= shift;

	/* store the cap info (will be put in STA Profile) */
	NdisMoveMemory(&cap_info, repted_source, sizeof(cap_info));
	repted_source += sizeof(cap_info);
	repted_source_len -= sizeof(cap_info);

	/* Step 3. inheritance rule */
	WRAP_BUFFER(repting_rsp, repting_source, repting_source_len);
	WRAP_BUFFER(repted_rsp, repted_source, repted_source_len);
	WRAP_BUFFER(target, target_buf, target_len);
	status = eht_mlo_sta_profile_inheritance_rule(&repting_rsp, &repted_rsp, &target);
	if (status != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_CONN, DBG_LVL_ERROR,
			"inheritance rule fail, stop post-processing\n");
		goto non_setup_probe_post_fail;
	}

	target_len = target.buf_len;

	/* Step 4. start to form the output (STA Profile for reported link) */
	repted_rsp_buf->buf_len = 0;
	NdisMoveMemory(repted_rsp_buf->buf, &cap_info, sizeof(cap_info));
	repted_rsp_buf->buf_len += sizeof(cap_info);
	...
	WRAP_BUFFER(repted_rsp, target_buf, target_len);
	WRAP_BUFFER(target, repted_rsp_buf->buf, repted_rsp_buf->buf_len);
	WRAP_BUFFER(id_buf, ie_list, ie_list_len);
	WRAP_BUFFER(id_ext_buf, ie_ext_list, ie_ext_list_len);
	eht_remove_ie_by_id_list(&repted_rsp, &target, &id_buf, &id_ext_buf);

	repted_rsp_buf->buf_len = target.buf_len;
	MTWF_DBG(NULL, DBG_CAT_MLO, CATMLO_CONN, DBG_LVL_ERROR,
		"length after removal: %d\n", repted_rsp_buf->buf_len);
#else
	NdisMoveMemory(repted_rsp_buf->buf + repted_rsp_buf->buf_len, target_buf, target_len);
	repted_rsp_buf->buf_len += target_len;
#endif

non_setup_probe_post_fail:
	if (target_buf)
...
static void ap_add_peer2scan_table(
	struct _RTMP_ADAPTER *mac_ad,
	struct wifi_dev *wdev,
	MLME_QUEUE_ELEM *elem,
	BCN_IE_LIST *ie_list,
	CHAR RealRssi,
	NDIS_802_11_VARIABLE_IEs *vie,
	USHORT vie_len)
{
	BSS_TABLE *ScanTab = NULL;
	ULONG Bssidx = 0;
	BOOLEAN add2table = FALSE;

	ScanTab = get_scan_tab_by_wdev(mac_ad, wdev);
	Bssidx = BssTableSearch(ScanTab, ie_list->Bssid, ie_list->Channel);
#ifdef FTM_SUPPORT
	if ((ie_list->ExtCapInfo.ftm_init == 1) || (ie_list->ExtCapInfo.ftm_resp == 1))
		add2table = TRUE;
#endif /* FTM_SUPPORT */

#ifdef OCE_SUPPORT
	if (IS_OCE_ENABLE(wdev) && (add2table == FALSE))
		add2table = TRUE;
#endif /* OCE_SUPPORT */
	...
	if (Bssidx == BSS_NOT_FOUND) {
		...
		if (Bssidx < MAX_LEN_OF_BSS_TABLE) {
			...
			MTWF_DBG(mac_ad, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG,
			"UPDATE Info[%lu],RSSI=%d,MAC=%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (add2table == TRUE) {
		Bssidx = BssTableSetEntry(mac_ad, wdev, ScanTab, ie_list, RealRssi, vie_len, vie);
		if ((Bssidx == BSS_NOT_FOUND) || (Bssidx >= MAX_LEN_OF_BSS_TABLE))
			MTWF_DBG(mac_ad, DBG_CAT_MLME, CATMLME_SYNC, DBG_LVL_DEBUG,
				"ERROR: bss not found or invalid idx:0x%lx\n", Bssidx);
		else {
			NdisMoveMemory(ScanTab->BssEntry[Bssidx].PTSF, &elem->Msg[24], 4);
			NdisMoveMemory(&ScanTab->BssEntry[Bssidx].TTSF[0],
				&elem->TimeStamp.u.LowPart, 4);
			NdisMoveMemory(&ScanTab->BssEntry[Bssidx].TTSF[4],
				&elem->TimeStamp.u.LowPart, 4);
			ScanTab->BssEntry[Bssidx].MinSNR = elem->Signal % 10;
			if (ScanTab->BssEntry[Bssidx].MinSNR == 0)
				ScanTab->BssEntry[Bssidx].MinSNR = -5;
			NdisMoveMemory(ScanTab->BssEntry[Bssidx].MacAddr,
		...
		struct wifi_dev *wdev, BCN_IE_LIST *ie_list, MLME_QUEUE_ELEM *Elem)
{
	...
	if (hc_radio_query_by_wdev(wdev, &oper) == HC_STATUS_FAIL) {
		if ((wlan_config_get_ch_band(wdev) == CMD_CH_BAND_24G) && (pAd->CommonCfg.bOverlapScanning == TRUE)) {
		...
		NDIS_802_11_VARIABLE_IEs *pVIE, USHORT LenVIE)
{
	...
	ULONG Bssidx = 0;
#endif
	struct _BSS_TABLE *ScanTab = get_scan_tab_by_wdev(pAd, wdev);
#ifdef OCE_SUPPORT
	...
	if ((IS_APCLI_BW_SYNC_FEATURE_ENBL(pAd) == FALSE) &&
...
#endif /* OCE_SUPPORT */
	ap_add_peer2scan_table(pAd, wdev, Elem, ie_list, RealRssi, pVIE, LenVIE);
	...
	if (pAd->WdsTab.Mode != WDS_DISABLE_MODE) {
		...
		if (pEntry && WDS_IF_UP_CHECK(pAd, HcGetBandByWdev(wdev), pEntry->func_tb_idx))
			...
			if (!pEntry)
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG,
...
#endif /* WDS_SUPPORT */
	if ((wlan_config_get_ch_band(wdev) == CMD_CH_BAND_24G) && (WorkChannel != wdev->channel)) {
	...
	if (ie_list->Channel == pAd->ApCfg.AutoChannel_Channel) {
		...
		if (pAutoChCtrl->pChannelInfo) {
			...
			if (AutoChBssSearchWithSSID(pAd, ie_list->Bssid, (PUCHAR)ie_list->Ssid,
					ie_list->SsidLen, ie_list->Channel, wdev) == BSS_NOT_FOUND)
		...
		struct wifi_dev *wdev, struct _PEER_PROBE_REQ_PARAM *ProbeReqParam, MLME_QUEUE_ELEM *Elem)
{
	...
	if (ProbeReqParam->SsidLen != 0) {
		...
		if (!isSsidMatched && IS_MBSSID_IE_NEEDED(&mbss->mbss_11v)) {
			UCHAR apidx = 0;
			BSS_STRUCT *nonTxMbss;
			u32 request_bssidx_bitmap = 0;

			Elem->multip_bss_info.request_bssidx_bitmap = 0;

			for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
				nonTxMbss = &pAd->ApCfg.MBSSID[apidx];

				if (IS_BSSID_11V_NON_TRANS(&nonTxMbss->mbss_11v)
					&& nonTxMbss->mbss_11v.mbss_11v_t_bss
					&& (nonTxMbss->mbss_11v.mbss_11v_t_bss == mbss)) {
					if ((ProbeReqParam->SsidLen == nonTxMbss->SsidLen) &&
						NdisEqualMemory(ProbeReqParam->Ssid, nonTxMbss->Ssid, (ULONG)ProbeReqParam->SsidLen)) {
						isSsidMatched = TRUE;
						request_bssidx_bitmap = (nonTxMbss->mbss_idx >= nonTxMbss->mbss_11v.mbss_11v_t_bss_idx)
							? (nonTxMbss->mbss_idx - nonTxMbss->mbss_11v.mbss_11v_t_bss_idx) : 0;
						Elem->multip_bss_info.request_bssidx_bitmap |=
							BIT(request_bssidx_bitmap);
						Elem->multip_bss_info.request_num++;
						MTWF_DBG(NULL,
							DBG_CAT_AP, CATAP_11V_MBSS, DBG_LVL_WARN,
							"NonTx SSID found, mbss_idx=%d, mbss_11v_t_bss_idx=%u\n", nonTxMbss->mbss_idx, nonTxMbss->mbss_11v.mbss_11v_t_bss_idx);
						break;
					}
				}
			}
...
#ifdef DOT11V_MBSSID_SUPPORT
	else {	/* peer request wild card ssid, we will take all mbss ie out. */
		if (IS_MBSSID_IE_NEEDED(&mbss->mbss_11v)) {
			Elem->multip_bss_info.request_bssidx_bitmap |= 0xFFFF;
			Elem->multip_bss_info.request_num = MAX_BEACON_NUM;
		}
	}
#endif
	...
	if (!mbss->bHideSsid && !isSsidMatched) {
		if (ProbeReqParam->ShortSSID == mbss->ShortSSID)
			isSsidMatched = TRUE;
		else
			isSsidMatched = bss_mngr_query_is_repted_ssid(wdev,
				ProbeReqParam->ShortSSID, ProbeReqParam->Ssid, ProbeReqParam->SsidLen);
	}
#endif
	...
	if (BS_CheckConnectionReq(pAd, wdev, ProbeReqParam->Addr2, Elem, ProbeReqParam) == FALSE)
		return FALSE;
#endif
#ifdef BAND_STEERING
...
BOOLEAN ap_probe_response_xmit(struct _RTMP_ADAPTER *pAd,
									  struct wifi_dev *wdev, struct _PEER_PROBE_REQ_PARAM *ProbeReqParam, MLME_QUEUE_ELEM *Elem)
{
...
#endif
#ifdef DOT11_EHT_BE
	BOOLEAN is_mlo_repting_link = (Elem && Elem->Others) ? FALSE : TRUE;
#endif
#ifdef DOT11V_MBSSID_SUPPORT
	...
	if (IS_MAP_CERT_ENABLE(pAd) || IS_MAP_API_ENABLE(pAd) ||
			((IS_MAP_BS_ENABLE(pAd)) &&
			 pAd->ApCfg.Disallow_ProbeEvent != TRUE)) {
		/* Send event if MAP CERT is enabled or if band steering enabled */
		/* add is_mlo_repting_link condition for skip MLO reported link processing */
		if (Elem
#ifdef DOT11_EHT_BE
			&& is_mlo_repting_link
#endif
	...
	if (WMODE_CAP_6G(wdev->PhyMode)
		...
		|| (IS_OCE_ENABLE(wdev)
			...
			&& ProbeReqParam->IsOceCapability) /* is OCE STA */
#endif /* OCE_SUPPORT */
		) {
	...
	/* mbssid ie */
	if (IS_MBSSID_IE_NEEDED(&mbss->mbss_11v) &&
		(Elem->multip_bss_info.request_num || pAd->CommonCfg.wifi_cert)) {

		mbss_info.is_probe_rsp = TRUE;	/* probe rsp */
		mbss_info.ml_ie = HAS_EHT_MLD_EXIST(ProbeReqParam->cmm_ies.ie_exists) ?
			ProbeReqParam->cmm_ies.ml_ie : NULL;
		if (pAd->CommonCfg.wifi_cert)	/*for tgax cert, we take all mbss ie*/
			mbss_info.query_bitmap = 0xffff;
		else
			mbss_info.query_bitmap = Elem->multip_bss_info.request_bssidx_bitmap;
		mbss_info.f_buf = (u8 *)(pOutBuffer + FrameLen);
...
#ifdef QOS_R2
	if (IS_QOS_ENABLE(pAd) && (wdev->func_idx >= 0)) {
	...
	if (mbss->GASCtrl.b11U_enable) {
		ULONG TmpLen;
		OS_SEM_LOCK(&mbss->GASCtrl.IeLock);
		...
		FrameLen += TmpLen;
		if (mbss->GASCtrl.RoamingConsortiumIELen && mbss->GASCtrl.RoamingConsortiumIE) {
			/* Roaming Consortium element */
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  mbss->GASCtrl.RoamingConsortiumIELen,
					  mbss->GASCtrl.RoamingConsortiumIE, END_OF_ARGS);
			FrameLen += TmpLen;
		}
		OS_SEM_UNLOCK(&mbss->GASCtrl.IeLock);
	}
	...
	if ((pAd->CommonCfg.bIEEE80211H == 1)
		...
		build_channel_switch_relatd_ie(pAd, wdev, (UCHAR *)(pOutBuffer + FrameLen), &ie_size);
		FrameLen += ie_size;
	}
...
#ifdef DOT11_EHT_BE
	&& !(!IS_BROADCAST_MAC_ADDR(ProbeReqParam->Addr2)
	&& HAS_EHT_MLD_EXIST(ProbeReqParam->cmm_ies.ie_exists))
#endif /* DOT11_EHT_BE */
	) {
		ULONG DPPIeTmpLen = 0;
...
#endif /* CUSTOMER_VENDOR_IE_SUPPORT */
	/* rnr ie */
	FrameLen += bss_mngr_query_rnr_ie(wdev, (u8 *)(pOutBuffer + FrameLen));

#ifdef DOT11_EHT_BE
	if (WMODE_CAP_BE(wdev->PhyMode) && wdev->DesiredHtPhyInfo.bHtEnable) {
		if (PD_CEHCK_MLO_V1_DISABLE(pAd->physical_dev) ||
			(PD_CEHCK_MLO_V1_1_ENABLE(pAd->physical_dev) &&
				ProbeReqParam->cmm_ies.vendor_ie.mtk_mlo_enable != (MEDIATEK_TLV1_TYPE4_MLO_V1 | MEDIATEK_TLV1_TYPE4_MLO_FORCE)) ||
			(PD_CEHCK_MLO_V1_ENABLE(pAd->physical_dev) &&
				(!is_mlo_repting_link ||
				(chip_determine_mlo_allow(pAd, &ProbeReqParam->cmm_ies.vendor_ie) &&
					HAS_EHT_MLD_EXIST(ProbeReqParam->cmm_ies.ie_exists))))) {
			/* only reporting link need to build multi-link ie (and reported not) */
			if (is_mlo_repting_link) {
				UINT8 *local_fbuf = pOutBuffer + FrameLen;
				ULONG ml_ie_len = 0;
				...
				local_fbuf = eht_build_multi_link_ie(
						...
						HAS_EHT_MLD_EXIST(ProbeReqParam->cmm_ies.ie_exists) ?
							ProbeReqParam->cmm_ies.ml_ie : NULL,
						NULL, BMGR_QUERY_ML_IE_PROBE_RSP, BMGR_MAX_MLD_STA_CNT);

				ml_ie_len = local_fbuf - pOutBuffer - FrameLen;
				FrameLen += ml_ie_len;

				local_fbuf = eht_build_reconfig_multi_link_ie(
					wdev, local_fbuf, FrameLen, BMGR_QUERY_ML_IE_PROBE_RSP);

				ml_ie_len = local_fbuf - pOutBuffer - FrameLen;
				FrameLen += ml_ie_len;
			}

			FrameLen += eht_add_probe_rsp_ies(
					wdev, pOutBuffer, FrameLen, &ProbeReqParam->cmm_ies);
		}
	}
	...
	if (mbss->TRANSIE_Len) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
			mbss->TRANSIE_Len, mbss->TRANS_IE, END_OF_ARGS);
		FrameLen += TmpLen;
	}
#endif
#ifdef DOT11_EHT_BE
	if (WMODE_CAP_BE(wdev->PhyMode) && wdev->DesiredHtPhyInfo.bHtEnable) {
		if (PD_CEHCK_MLO_V1_ENABLE(pAd->physical_dev) &&
			ProbeReqParam->cmm_ies.vendor_ie.mtk_mlo_enable
						& MEDIATEK_TLV1_TYPE4_MLO_V1) {
			UCHAR mtk_vendor_ie_tlv1_type = 0;

			if (HAS_EHT_MLD_EXIST(ProbeReqParam->cmm_ies.ie_exists))
				mtk_vendor_ie_tlv1_type = BIT(MEDIATEK_TLV1_TYPE4);
			else
				mtk_vendor_ie_tlv1_type = BIT(MEDIATEK_TLV1_TYPE4)
								| BIT(MEDIATEK_TLV1_TYPE2);

			FrameLen += build_mtk_tlv1_vendor_ie(pAd, wdev, (pOutBuffer + FrameLen),
				VIE_PROBE_RESP, mtk_vendor_ie_tlv1_type);
		} else
			FrameLen += build_mtk_tlv1_vendor_ie(pAd, wdev, (pOutBuffer + FrameLen),
				VIE_PROBE_RESP, BIT(MEDIATEK_TLV1_TYPE4));
	}
#endif /* DOT11_EHT_BE */
	...
	FrameLen += build_rsn_override_ie(
		pAd, wdev, NULL, SUBTYPE_PROBE_RSP, (UCHAR *)(pOutBuffer + FrameLen));
	...
	*/
	...
	if (pAd->CommonCfg.bMLREnable)
		FrameLen += build_mtk_tlv1_vendor_ie(pAd, wdev, (pOutBuffer + FrameLen),
			VIE_PROBE_RESP, BIT(MEDIATEK_TLV1_TYPE1));
#endif



	if (!is_mlo_repting_link) {
		/* reported link queries probe response frame */
		struct buffer_wrapper *rsp_buf = (struct buffer_wrapper *)Elem->Others;

		rsp_buf->buf = pOutBuffer;
		rsp_buf->buf_len = FrameLen;
		/* In this case, pOutBuffer will be freed */
		/* in bss_mngr_con_sync_ml_probe_rsp_per_sta_profile() */
		MTWF_DBG(pAd, DBG_CAT_MLO, CATMLO_CONN, DBG_LVL_INFO,
			"non-setup link probe rsp: %p, len: %ld\n",
			rsp_buf->buf, rsp_buf->buf_len);
	} else
#endif /* DOT11_EHT_BE */
	{
