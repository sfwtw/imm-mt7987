	...
	UCHAR *data)
{
	...
	if (!pAd) {
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_CNTL, DBG_LVL_ERROR, "pAd is NULL!\n");
		return FALSE;
	}
	...
	USHORT reason)
{
	...
	MTWF_DBG(NULL, DBG_CAT_MLME, CATMLME_CNTL, DBG_LVL_WARN,
...
#ifdef APCLI_RANDOM_MAC_SUPPORT
	if (ifIndex < MAX_MULTI_STA && pAd->StaCfg[ifIndex].apcli_random_mac_support) {
		if (apcli_set_random_mac_addr(wdev, FALSE) == FALSE) {
			MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_CNTL, DBG_LVL_ERROR,
			"PE: encountered error, please chk!!!\n");
		}
	}
#endif

	if (MlmeEnqueueWithWdev(pAd, MLME_CNTL_STATE_MACHINE, CNTL_MLME_SCAN,
			sizeof(MLME_SCAN_REQ_STRUCT), mlme_scan_request, 0, wdev, FALSE, NULL))
		RTMP_MLME_HANDLER(pAd);
	else {
		/* enq scan req fail, we should release channelOp here.*/
		owner = GetCurrentChannelOpOwner(pAd, wdev);
...
full_reset:
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_NOTICE,
		"wdev(%d) reset scan fsm, current status is 0x%x\n", wdev->BssIdx, status);
	...
	for (BssIdx = 0; BssIdx < MaxNumBss; BssIdx++) {
		...
		if (!apWdev || apWdev->channel != wdev->channel)
			continue;
	...
	if (owner == CH_OP_OWNER_SCAN)
		...
		if (ScanCtrl->PartialScan.TimerInterval == 0) {
			/* Don't reset pwdev if periodic PartialScan is set */
