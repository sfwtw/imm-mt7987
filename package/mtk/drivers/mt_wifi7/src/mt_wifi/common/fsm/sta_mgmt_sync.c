		...
		NDIS_802_11_VARIABLE_IEs *pVIE, USHORT LenVIE)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid)) {
...
#endif
#ifdef CONFIG_MAP_SUPPORT
			if (!IS_MAP_CERT_ENABLE(pAd) && !pStaCfg->bAutoRoaming)
		...
		if (pEntry && bcn_ie_list->NewChannel != 0) {
			...
			if (pAd->CommonCfg.DfsParameter.bRCSAEn) {
				...
				ChannelSwitchAction(pAd, &bcn_ie_list->CsaInfo);
			} else
#endif
		...
		if (!IS_APCLI_DISABLE_MLO(wdev)
			...
			if (parse_multi_link_ie(bcn_ie_list->cmm_ies.ml_ie,
				...
				sta_sync_mld_per_link_mlo_csa(pEntry, &bcn_ie_list->mlo_ie_info);
		}

		if (SSID_EQUAL(pStaCfg->CfgSsid, pStaCfg->CfgSsidLen, bcn_ie_list->Ssid, bcn_ie_list->SsidLen))
			sta_mld_link_change_handle(pAd, wdev, pEntry->wcid, bcn_ie_list);
#endif
	}
	...
	if (Bssidx == BSS_NOT_FOUND) {
		...
		/*if mlo 11v not-tx bss take two ml ie upload to supplicant,
		  ...
		  split it here, or just do not send probe resp to supplicant.
		  TBD:maybe we need remove the unused ml ie when driver get probe resp.*/
		if (Elem->MsgType == SYNC_FSM_PEER_PROBE_RSP
			&& strlen(bcn_ie_list->cmm_ies.ml_ie_2nd))
		...
		for (i = 0; i < MAC_ADDR_LEN; i++) {
			if (bcn_ie_list->Bssid[i] > pStaCfg->Bssid[i]) {
				MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"SYNC - merge to the IBSS with bigger BSSID="MACSTR"\n",
	...
	if (is_my_bssid) {
...
#ifdef DOT11_EHT_BE
		if (HAS_EHT_OP_EXIST(bcn_ie_list->cmm_ies.ie_exists))
			eht_update_peer_dscb_info(wdev, pEntry, &bcn_ie_list->cmm_ies);
#endif
		...
		if ((pAd->CommonCfg.bIEEE80211H == 1) &&
...
#endif /* WSC_STA_SUPPORT */
			{
				if (((((IS_SECURITY_Entry(wdev)) << 4) ^ bcn_ie_list->CapabilityInfo) & 0x0010)
					...
					/*
					To prevent STA connect to OPEN/WEP AP when STA is
					OPEN/NONE or STA connect to OPEN/NONE AP
					when STA is OPEN/WEP AP.
					*/
					&& (bcn_ie_list->selReg == 0)
				)
				   ) {
					...
					*/
					MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
						"AP privacy:%x is differenct from STA privacy:%x\n",
						(bcn_ie_list->CapabilityInfo & 0x0010) >> 4,
		...
		if (INFRA_ON(pStaCfg)) {
			...
			*/
			...
			if ((wlan_config_get_ch_band(wdev) == CMD_CH_BAND_24G) && (bUseBGProtection != OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_BG_PROTECTION_INUSED))) {
				if (bUseBGProtection) {
			...
			if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED)	  &&
				...
				hwifi_update_edca(pAd, wdev, &bcn_ie_list->EdcaParm, FALSE);
				hwifi_set_edca(wdev);
...
#else
			if (INFRA_ON(pStaCfg) && !pStaCfg->bAutoRoaming) {
				BOOLEAN bChangeBW = FALSE;
			...
			if (bcn_ie_list->MessageToMe) {
...
#endif /* UAPSD_SUPPORT */
				{
					if (pStaCfg->WindowsBatteryPowerMode == Ndis802_11PowerModeFast_PSP) {
						...
						MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
							"Send PS-POLL to retrieve buffered data\n");
...
static BOOLEAN sta_join_ap_security_check(
	...
	BOOLEAN matchFlag)
{
	ULONG Bssidx;
	BSS_ENTRY *pInBss;
	BSS_TABLE *ScanTab = get_scan_tab_by_wdev(mac_ad, wdev);
...
#if defined(CONFIG_OWE_SUPPORT)
			if (matchFlag) {
				if (sta_handle_owe_trans(mac_ad, wdev, pInBss))
					return FALSE;
				MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"OWE TRANS IE HANDLING FAILED OR NOT REQUIRED\n");
			}
#endif
			if ((!((IS_AKM_OWE(wdev->SecConfig.AKMMap))
				&& (IS_AKM_OPEN_ONLY(pInBss->AKMMap))))
				&& (!((IS_AKM_WPA3PSK(wdev->SecConfig.AKMMap)) &&
					(IS_AKM_WPA2PSK_ONLY(pInBss->AKMMap)) &&
					((wdev->SecConfig.PairwiseCipher
						& pInBss->PairwiseCipher) != 0)))
				&& (((wdev->SecConfig.AKMMap & pInBss->AKMMap) == 0)
					|| ((wdev->SecConfig.PairwiseCipher
						& pInBss->PairwiseCipher) == 0))
				&& (!ie_list->selReg)
			) {
				MTWF_DBG(mac_ad, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
						"RSN IE VALIDATE FAIL\n");
				return FALSE;
			}

			if (WMODE_CAP_6G(wdev->PhyMode)) {
				/* 6G band only support WPA3, WPA3-192, WPA3PSK, DPP or OWE. */
				if (!IS_AKM_OWE(pInBss->AKMMap) &&
					!IS_AKM_WPA3(pInBss->AKMMap) &&
					!IS_AKM_WPA3_192BIT(pInBss->AKMMap) &&
					...
					MTWF_DBG(mac_ad, DBG_CAT_CLIENT,
						CATCLIENT_APCLI, DBG_LVL_ERROR,
						"Only support DPP, WPA3 SAE, WPA3 OWE");
					MTWF_DBG(mac_ad, DBG_CAT_CLIENT,
						CATCLIENT_APCLI, DBG_LVL_ERROR,
						", WPA3 Enterprise");
					MTWF_DBG(mac_ad, DBG_CAT_CLIENT,
						CATCLIENT_APCLI, DBG_LVL_ERROR,
						", WPA3-192bit Enterprise for 6G band\n");
					return FALSE;
				}

				/*
				 * For WiFi 6E cert 5.2.4_6G, cannot connect to AP with WPA3PSKSAE
				 * with Huntingand Pecking.
				 */
				if (IS_AKM_WPA3PSK(pInBss->AKMMap)
					&& !IS_AKM_WPA2PSK(pInBss->AKMMap)
					&& (pInBss->sae_conn_type != SAE_CONNECTION_TYPE_H2E)) {
					MTWF_DBG(mac_ad, DBG_CAT_CLIENT,
						CATCLIENT_APCLI, DBG_LVL_ERROR,
						"WPA3 SAE connection type is not hash to element");
					MTWF_DBG(mac_ad, DBG_CAT_CLIENT,
						CATCLIENT_APCLI, DBG_LVL_ERROR,
						"(sae_conn_type: %x)\n",
		...
		struct wifi_dev *wdev, BCN_IE_LIST *ie_list, MLME_QUEUE_ELEM *Elem)
{
	...
	if ((ssidEmptyFlag == FALSE) || (bssidEmptyFlag == FALSE)) {
		...
		if ((matchFlag == TRUE) && (bssidEqualFlag == FALSE)) {
			...
			MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "SYNC - previous Rssi = %ld current Rssi=%ld\n", pStaCfg->MlmeAux.Rssi, (LONG)RealRssi);
		...
		MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "SYNC - bssidEqualFlag=%d, ssidEqualFlag=%d, matchFlag=%d\n",
	...
	if (ie_list->Channel != pStaCfg->MlmeAux.Channel) {
		MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, "SYNC - current ie channel=%d, apcli channel=%d!\n",
				 ie_list->Channel, pStaCfg->MlmeAux.Channel);
		return FALSE;
	}

#ifndef RT_CFG80211_SUPPORT
	matchFlag = sta_join_ap_security_check(pAd, wdev, ie_list, matchFlag);
#endif /* !RT_CFG80211_SUPPORT */
		...
		NDIS_802_11_VARIABLE_IEs *pVIE, USHORT LenVIE)
{
		...
		*/
		MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
			"receive desired BEACON,Channel=%d, (%d, %d)\n",
		...
		*/
		if (pStaCfg->MlmeAux.SsidLen == 0) {
			...
			if (Idx == BSS_NOT_FOUND) {
				...
				rssi_sample.AvgRssi[2] = Elem->rssi_info.raw_rssi[2];
				rssi_sample.AvgRssi[3] = Elem->rssi_info.raw_rssi[3];
				Rssi = RTMPAvgRssi(pAd, &rssi_sample);
...
#endif /* WSC_STA_SUPPORT */
					{
						...
						*/
						if (((((IS_SECURITY_Entry(wdev)) << 4) ^ ie_list->CapabilityInfo) & 0x0010)
							...
							/* When using -Dwext and trigger WPS,
							...
							*/
							&& (!ie_list->selReg)
								...
								MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
									"AP privacy %d is differenct from STA privacy%d\n",
									(ie_list->CapabilityInfo & 0x0010) >> 4, IS_SECURITY_Entry(wdev));
		...
		*/
		...
		if (pStaCfg->Dot11RCommInfo.bFtSupport &&
			...
			MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
				"PeerBeaconAtJoinAction! (MdId=%x%x, FtOverDs=%d, RsrReqCap=%d)\n",
				pStaCfg->MlmeAux.MdIeInfo.MdId[0], pStaCfg->MlmeAux.MdIeInfo.MdId[1],
		...
		if (!pEntry && (
			...
			pEntry = MacTableInsertEntry(pAd, ie_list->Bssid, &pStaCfg->wdev, ENTRY_INFRA, OPMODE_STA, TRUE, MLD_STA_NONE, NULL);
			ASSERT(pEntry);
		}
		...
		if (pEntry) {
			...
			PaserSecurityIE(wdev, pEntry, ie_list,
				&LenVIE,
			...
			if (((pEntry_SecConfig->AKMMap == 0) || (pEntry_SecConfig->PairwiseCipher == 0))
			   ...
			   && (!ie_list->selReg)) {
				MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"%u pEntry_Sec AKM %x Ciper %x, return due to desired BSS sec not mach\n",
					__LINE__, pEntry_SecConfig->AKMMap, pEntry_SecConfig->PairwiseCipher);
				return FALSE; /* None matched*/
			}
			...
			|| (IS_AKM_DPP(pEntry_SecConfig->AKMMap))
				...
				if ((IS_MAP_ENABLE(pAd) && (IS_AKM_WPA3PSK(pEntry_SecConfig->AKMMap) ||
					...
					if (((pProfile_SecConfig->PmfCfg.MFPR) && (RsnCap.field.MFPC == FALSE))
						...
						MTWF_DBG(pAd,
							DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR,
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_INFO,
				"Final Security AKM = 0x%x, PairwiseCipher = 0x%x, GroupCipher = 0x%x, bAllowNrate=%d\n",
				pEntry_SecConfig->AKMMap, pEntry_SecConfig->PairwiseCipher,
				pEntry_SecConfig->GroupCipher, bAllowNrate);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_INFO,
				"pEntry_SecConfig->PairwiseKeyId=%d, pEntry_SecConfig->GroupKeyId=%d\n",
			...
			if (pEntry_SecConfig->key_deri_alg == SEC_KEY_DERI_SHA384)
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_INFO,
					"[PMF] Use SHA384\n");
		...
		if (((HAS_HT_CAPS_EXIST(ie_list->cmm_ies.ie_exists)) || (HAS_PREN_CAPS_EXIST(ie_list->cmm_ies.ie_exists))) &&
			...
			if (cfg_ht_bw) {
				...
				pStaCfg->StaActive.SupportedPhyInfo.bw = BW_40;
			} else {
				wlan_operate_set_ht_bw(wdev, BW_20, EXTCHA_NONE);
				pStaCfg->StaActive.SupportedPhyInfo.bw = BW_20;
			}
			...
			MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
				"(MpduDensity=%d, MaxRAmpduFactor=%d, BW=%d)\n",
				pStaCfg->StaActive.SupportedHtPhy.MpduDensity,
			...
			if (WMODE_CAP_AC(wdev->PhyMode) &&
				...
				if (pStaCfg->StaActive.SupportedPhyInfo.bw == BW_40) {
					if (op_info.bw > VHT_BW_80)
						pStaCfg->StaActive.SupportedPhyInfo.bw = BW_160;
					else if (op_info.bw == VHT_BW_80)
						pStaCfg->StaActive.SupportedPhyInfo.bw = BW_80;
				}
				MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
					"CentralChannel=>%d\n", CentralChannel);
			}
...
#endif /* DOT11_N_SUPPORT */
		{
			...
			if (WMODE_CAP_6G(wdev->PhyMode) && HAS_HE_CAPS_EXIST(ie_list->cmm_ies.ie_exists)) {
				...
				UINT8 ch_width_from_6g_opinfo = HE_BW_20;

				if (cmm_ies->he_ops.he_op_param.param2 & DOT11AX_OP_6G_OPINFO_PRESENT) {
					ch_width_from_6g_opinfo = (cmm_ies->he6g_opinfo.ctrl & HE_6G_OP_CONTROL_CH_WIDTH_MASK);

					MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
							"ch_width = %d, ch_width_from_6g_opinfo = %d\n", ch_width, ch_width_from_6g_opinfo);
					if (ch_width != ch_width_from_6g_opinfo)
						ch_width = ch_width_from_6g_opinfo;
				}
				pStaCfg->StaActive.SupportedPhyInfo.bw = he_bw_2_rf_bw(ch_width);
#ifdef DOT11_EHT_BE
				if (HAS_EHT_CAPS_EXIST(ie_list->cmm_ies.ie_exists)) {
					if ((cmm_ies->eht_caps.phy_cap.phy_capinfo_1 & DOT11BE_PHY_CAP_320M_6G) &&
						(pStaCfg->StaActive.SupportedPhyInfo.bw == BW_160))
						pStaCfg->StaActive.SupportedPhyInfo.bw = BW_320;
				}
#ifdef CONFIG_MAP_SUPPORT
				else if (HAS_VENDOR_EHT_OP_EXIST(pStaCfg->MlmeAux.vendor_ie.ie_exists)) {
					UCHAR opbw = BW_20;

					opbw = eht_bw_2_rf_bw(ie_list->vendor_ie.eht_op.op_info.control&0x7);
					if ((pStaCfg->StaActive.SupportedPhyInfo.bw == BW_160) && (opbw == BW_320))
						pStaCfg->StaActive.SupportedPhyInfo.bw = BW_320;
				}
#endif
#endif
...
#ifdef DOT11_VHT_AC
			pStaCfg->StaActive.SupportedPhyInfo.bVhtEnable = FALSE;
			/* mapping BW to VHT_BW for 6G APCLI BW*/
			pStaCfg->StaActive.SupportedPhyInfo.vht_bw = rf_bw_2_vht_bw(pStaCfg->StaActive.SupportedPhyInfo.bw);
#endif /* DOT11_VHT_AC */
		...
		MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
				"Set CentralChannel=%d, get peer bw = %d\n",
				pStaCfg->MlmeAux.CentralChannel, pStaCfg->StaActive.SupportedPhyInfo.bw);
		...
		MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
			...
			pStaCfg->MlmeAux.rate.sup_rate_len, pStaCfg->MlmeAux.rate.ext_rate_len);
		...
		if (pStaCfg->BssType == BSS_INFRA) {
			...
			AdjustChannelRelatedValue(pAd, &BwFallBack, BSS0, pStaCfg->MlmeAux.Channel,
...
#endif
			{
				...
				if (Bssidx != BSS_NOT_FOUND && Bssidx < MAX_LEN_OF_BSS_TABLE) {
...
#endif /* APCLI_AUTO_BW_TMP */
					{
						...
						MTWF_DBG(pAd, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_WARN,
							"APCLI_AUTO_BW_TMP Switch to channel :%d\n", ScanTab->BssEntry[Bssidx].Channel);
						AdjustChannelRelatedValue(pAd, &BwFallBack, BSS0, ScanTab->BssEntry[Bssidx].Channel,
									pStaCfg->MlmeAux.CentralChannel, &pStaCfg->wdev);
						isGoingToConnect = TRUE;
		...
		MTWF_DBG(NULL, DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
			"%u isGoingToConnect %d\n", __LINE__, isGoingToConnect);
