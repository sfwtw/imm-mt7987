...
struct wifi_dev *get_wdev_by_ioctl_idx_and_iftype(RTMP_ADAPTER *pAd, INT idx, INT iftype)
{
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_TX, CATMLME_WTBL, DBG_LVL_ERROR,
			"can not find ioctl_if_type(%d), if_idx(%d)\n", iftype, idx);
		break;
	}
...
struct wifi_dev *get_wdev_by_idx(RTMP_ADAPTER *pAd, INT idx)
{
	...
	if (wdev == NULL)
		MTWF_DBG(pAd, DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, "invalid idx(%d)\n", idx);
