	...
	IN PCIPHER_KEY	pWpaKey)
{
	...
	if (pWpaKey->KeyLen == 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_AES, DBG_LVL_INFO,
			"failed!(the Length can not be 0)\n");
		return FALSE;
	}
	...
	*(pData + 1) = *(pData + 1) | 0x40;
	...
	if (!NdisEqualMemory(MIC, TrailMIC, 8)) {
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_AES, DBG_LVL_ERROR,
			"MIC Error !\n");	 /* MIC error. */
		return FALSE;
	}
...
int ccmp_init_iv_and_aad(u8 *hdr_ptr, u8 *pn, u8 *iv, u8 *aad)
{
	u8 i;
	u8 *pos, qc = 0;
	size_t aad_len;
...
int ccmp_encrypt(struct crypto_aead *tfm, TX_BLK *pTxBlk, u8 *hdr_ptr, u8 *wifi_hdr)
{
	u8 aad[30];
	u8 iv[AES_BLOCK_SIZES];
	int aad_len;
	u8 iv_offset = 0;
	size_t mic_len = mt_crypto_aead_authsize(tfm);
	...
	if (mt_aead_encrypt(tfm, iv, aad, aad_len, pTxBlk->pSrcBufData, pTxBlk->SrcBufLen, (pTxBlk->pSrcBufData + pTxBlk->SrcBufLen))) {
		/* encryp error , drop */
...
int ccmp_decrypt(struct crypto_aead *tfm, RX_BLK *pRxBlk, u8 *pn, u8 *wifi_hdr)
{
	...
	if (mt_aead_decrypt((struct crypto_aead *)tfm, iv, aad, aad_len, hdr_ptr + iv_offset, data_len - iv_offset - mic_len, hdr_ptr + data_len - mic_len))
		return NDIS_STATUS_FAILURE;
	...
	IN INT input)
{
	...
	MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_AES, DBG_LVL_INFO,
		"== CCMP test vector ==\n");
	/* Check AAD */
