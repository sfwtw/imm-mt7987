...
#ifdef __KERNEL__
#define DEBUGPRINT(fmt, args...) MTWF_PRINT(fmt, ## args)
#else
...
static UCHAR bi_pool_lock_status = FALSE;

void big_integer_lock_init(void)
{
	OS_NdisAllocateSpinLock(&bi_pool_lock);
	NdisAcquireSpinLock(&bi_pool_lock);
	bi_pool_lock_status = FALSE;
	NdisReleaseSpinLock(&bi_pool_lock);
}

void big_integer_lock_deinit(void)
{
	NdisAcquireSpinLock(&bi_pool_lock);
	bi_pool_lock_status = FALSE;
	NdisReleaseSpinLock(&bi_pool_lock);
	NdisFreeSpinLock(&bi_pool_lock);
}
	...
	VOID)
{
	...
	if (pool_is_small == FALSE)
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_WARN,
...
VOID BigInteger_Free_AllocSize(
	IN PBIG_INTEGER *pBI)
{
	UCHAR need_self_lock = FALSE;

	if (!bi_pool_lock_status) {
		NdisAcquireSpinLock(&bi_pool_lock);
		bi_pool_lock_status = TRUE;
		need_self_lock = TRUE;
	}
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	if (*pBI_Result == NULL) {
		if (BigInteger_Init(pBI_Result) == FALSE)
			return;
	}
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	if ((pFirstOperand->Signed * pSecondOperand->Signed) > 0) {
		...
		GET_BI_INS_FROM_POOL(pTempBI);
		if (BigInteger_Init(&pTempBI) == FALSE)
			return;
	...
	OUT PBIG_INTEGER *pBI_Remainder)
{
	...
	if (*pBI_Remainder == NULL) {
		if (BigInteger_Init(pBI_Remainder) == FALSE)
			return;
	}
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	if (mont == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "mont == NULL\n");
		return;
	}
	if (*pBI_Result == NULL)
		BigInteger_Init(pBI_Result);
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	BIG_INTEGER *tmp = NULL;
	BIG_INTEGER *b = NULL;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_INFO, "\n");
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	if (ptest) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "ptest is NULL!\n");
		return;
	}

	if (res == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "res allocate failed!\n");
		return;
	}
	...
	OUT PBIG_INTEGER *pBI_Result)
{
	...
	if (e) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ALG, DBG_LVL_ERROR, "e is NULL.\n");
