	...
	OUT BIG_INTEGER_EC_POINT **ec_point_res)
{
	...
	if (n6 == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_INFO, "allocate fail\n");
		ecc_point_free(ec_point_res);
		goto freeBN;
	}
	...
	INOUT BIG_INTEGER_EC_POINT *ec_point_res)
{
	...
	if (ec_point_res->z == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_INFO, "z is null\n");
		return;
	}
	Z = ec_point_res->z;
...
VOID ecc_point_dump_time(
	VOID)
{
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_SAE, ECC_COST_TIME_DBG_LVL, "ecc_point time record:\n");

	if (ec_op_ti_rec.add_op.exe_times) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ECC, ECC_COST_TIME_DBG_LVL,
			"add_op time record:\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ECC, ECC_COST_TIME_DBG_LVL,
	...
	if (ec_op_ti_rec.on_curve_check_op.exe_times) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ECC, ECC_COST_TIME_DBG_LVL,
			"\tavg_time=%lu jiffies", ec_op_ti_rec.on_curve_check_op.avg_time_interval);
#ifdef LINUX
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_ECC, ECC_COST_TIME_DBG_LVL,
			"%u msec", jiffies_to_msecs(ec_op_ti_rec.on_curve_check_op.avg_time_interval));
#endif
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_SAE, ECC_COST_TIME_DBG_LVL,
