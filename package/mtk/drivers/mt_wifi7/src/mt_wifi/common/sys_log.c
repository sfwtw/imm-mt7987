	...
	IN	CHAR					Rssi)
{
	...
	if (!wdev) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR, "Invalid wdev\n");
		return;
	}
	...
	if (event_table_len == 0) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"The type(%0x02x) is not valid.\n", type);
		return;
	}

	if (event >= event_table_len) {
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"The event(%0x02x) is not valid.\n", event);
		return;
	}
	...
	if (pBuf != NULL) {
		...
		left_buf_size = IW_CUSTOM_MAX_LEN;
		if (pAddr)
			ret_val = snprintf(pBufPtr, left_buf_size, "STA(%02x:%02x:%02x:%02x:%02x:%02x) ", PRINT_MAC(pAddr));
		else if ((wdev->wdev_type == WDEV_TYPE_AP || wdev->wdev_type == WDEV_TYPE_GO) && (wdev->func_idx < MAX_MBSSID_NUM(pAd)))
			ret_val = snprintf(pBufPtr, left_buf_size, "BSS(ra%d) ", wdev->func_idx);
		else
			ret_val = snprintf(pBufPtr, left_buf_size, " ");
		...
		if (type == IW_SYS_EVENT_FLAG_START) {
			left_buf_size = IW_CUSTOM_MAX_LEN - strlen(pBuf);
			ret_val = snprintf(pBufPtr, left_buf_size, "%s", pWirelessSysEventText[event]);
			if (os_snprintf_error(left_buf_size, ret_val)) {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
					"snprintf check error!\n");
				os_free_mem(pBuf);
			...
			if (Event_flag == IW_CHANNEL_CHANGE_EVENT_FLAG) {
				left_buf_size = IW_CUSTOM_MAX_LEN - strlen(pBuf);
				ret_val = snprintf(pBufPtr, left_buf_size, "%3d", Rssi);
				if (os_snprintf_error(left_buf_size, ret_val)) {
					MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
						"snprintf check error!\n");
					os_free_mem(pBuf);
			...
			left_buf_size = IW_CUSTOM_MAX_LEN - strlen(pBuf);
			ret_val = snprintf(pBufPtr, left_buf_size, "%s (RSSI=%d)", pWirelessSpoofEventText[event], Rssi);
			...
			left_buf_size = IW_CUSTOM_MAX_LEN - strlen(pBuf);
			ret_val = snprintf(pBufPtr, left_buf_size, "%s", pWirelessFloodEventText[event]);
			...
			left_buf_size = IW_CUSTOM_MAX_LEN - strlen(pBuf);
			ret_val = snprintf(pBufPtr, left_buf_size, "%s", pWirelessWscEventText[event]);
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_DBGLOG, DBG_LVL_ERROR,
			"Can't allocate memory for wireless event.\n");
}
