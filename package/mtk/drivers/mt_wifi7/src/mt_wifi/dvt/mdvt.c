...
const MDVT_MODULE_UPDATE_T arMdvtModuleUpdateTable[] = {
	...
	{MDVT_MODULE_SER,					"ser"},
	{MDVT_MODULE_LIT_WTBL,					"litwtbl"},
	{MDVT_MODULE_LITMIB,					"litmib"},
	{MDVT_MODULE_LIT_WFRMAC,				"litwfrmac"},
	{MDVT_MODULE_PTA_IDC_COEX,				"ptaidccoex"},
	{MDVT_MODULE_MUMIMO,					"mumimo"},
	{MDVT_MODULE_PRMBPUNC,					"prmbpunc"},
	{MDVT_MODULE_CERT,					"cert"},
	{MDVT_MODULE_SR,					"sr"},
	{MDVT_MODULE_ARB_COEX,					"arbcoex"},
	{MDVT_MODULE_BF,					"beamform"},
	{MDVT_MODULE_RLM_CMM,                                   "rlmcmm"},
	{MDVT_MODULE_UMAC_CFG,					"umaccfg"},
	{MDVT_MODULE_MAX,						"all"}
};
	...
	for (i = 0 ; i < u4MdvtModuleUpdateTableSize; i++) {
		...
		if (strcmp(set_str, module_field) == 0) {
			...
			if (cap->uni_cmd_support) {
				...
				cap_str = strsep(&arg, "-");
				if (set_val && cap_str) {
					UniCmdMDVT(pAd, arMdvtModuleUpdateTable[i].eModuleId, os_str_tol(set_val, 0, 10), os_str_tol(cap_str, 0, 10));
					MTWF_PRINT("%s:Module %s Module ID = %d Test Case Idx = %d CapId = %d\n",
						 __func__, module_field, arMdvtModuleUpdateTable[i].eModuleId,
						 (int)os_str_tol(set_val, 0, 10), (int)os_str_tol(cap_str, 0, 10));
				} else {
					MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_ERROR,
						"set_val or cap_str is NULL!\n");
					goto show;
				}
			} else
#endif /* WIFI_UNIFIED_COMMAND */
...
show:
	MTWF_PRINT("iwpriv ra0 set mdvt=(module)-(value)-(capId)\n");
