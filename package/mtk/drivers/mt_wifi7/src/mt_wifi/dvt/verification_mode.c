				  ...
				  UCHAR *buf)
{
	...
	default:
		...
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_ERROR,
			" try to construct wrong type:%d\n", veri_ctrl->veri_pkt_type);
		ret = FALSE;
	}
...
static BOOLEAN veri_pkt_ctnt_len_sanity_check(struct veri_ctrl *veri_ctrl, UCHAR tx_hw_hdr_len)
{
	...
	if ((veri_ctrl->veri_pkt_length + tx_hw_hdr_len) > MAX_LEN_OF_VERI_BUF) {
		MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_ERROR,
...
INT send_veri_pkt(struct _RTMP_ADAPTER *ad, char *arg)
{
	...
	if (!buf) {
		MTWF_DBG(ad, DBG_CAT_FW, CATTEST_MDVT, DBG_LVL_ERROR,
	...
	RTMP_QueryPacketInfo(pkt_skb, pSrcBufVA, SrcBufLen);
	MTWF_DBG(NULL, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_INFO,
		"allocate pkt, wdev_idx=%d, pkt_va:%p, VA:%p, Len:%u\n",
		wdev->wdev_idx, pkt_skb, pSrcBufVA, SrcBufLen);
...
INT prepare_veri_pkt_ctnt(struct _RTMP_ADAPTER *ad, UCHAR *padded_ctnt, UINT32 ctnt_length)
{
	...
	NdisZeroMemory(veri_ctrl->veri_pkt_ctnt, MAX_LEN_OF_VERI_BUF);
	if (ctnt_length > MAX_LEN_OF_VERI_BUF)
		ctnt_length = MAX_LEN_OF_VERI_BUF;
...
#define MAC_INPUT_FORMAT_LEN 18
	...
	if (arg) {
		...
		MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_DEBUG,
		...
		for (i = 0, value = rstrtok(addr1_str, ":"); value && i < MAC_ADDR_LEN; value = rstrtok(NULL, ":")) {
		...
		for (i = 0, value = rstrtok(addr2_str, ":"); value && i < MAC_ADDR_LEN; value = rstrtok(NULL, ":")) {
			if ((strlen(value) != 2) || (!isxdigit(*value)) || (!isxdigit(*(value + 1)))) {
				ret = FALSE;
				goto print_format;
			}
		...
		for (i = 0, value = rstrtok(addr3_str, ":"); value && i < MAC_ADDR_LEN; value = rstrtok(NULL, ":")) {
			if ((strlen(value) != 2) || (!isxdigit(*value)) || (!isxdigit(*(value + 1)))) {
				ret = FALSE;
				goto print_format;
			}
...
static BOOLEAN veri_fill_offload_tx_blk(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	PNDIS_PACKET pPacket;

	pPacket = pTxBlk->pPacket;
	pTxBlk->Wcid = RTMP_GET_PACKET_WCID(pPacket);
	RTMP_QueryPacketInfo(pPacket, pTxBlk->pSrcBufHeader, pTxBlk->SrcBufLen);
...
INT32 verify_pkt_tx(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev, struct _TX_BLK *tx_blk)
{
	...
	union _HTTRANSMIT_SETTING *transmit, transmit_setting;
	...
	if (arg) {
		...
		if (assign_htc_str) {
			if (CHECK_VERI_PKT_CTRL_IDX(veri_ctrl->veri_pkt_ctrl_map, VERI_HTC_CTRL)) {
				...
				MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_MDVT, DBG_LVL_INFO,
