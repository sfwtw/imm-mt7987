	...
	IN UCHAR RsnIeLen)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "AUTH_RSP - Send FT-AUTH response (%d)...\n", StatusCode);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (apidx = 0; apidx < MAX_MBSSID_NUM(pAd); apidx++) {
		...
		(void)snprintf(R0khIdBuf, sizeof(R0khIdBuf), "Ralink:%02x:%02x:%02x:%02x:%02x:%02x",
	...
	OUT PFT_INFO pFtInfoBuf)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "done. StatusCode(%d)\n", result);
	return result;
}
	...
	OUT PFT_INFO	pFtInfoBuf)
{
	...
	if ((pFtCfg->FtCapFlag.Dot11rFtEnable)
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "it's FT client\n");
		...
		** ("Invalid MDIE"). */
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "it isn't FT client\n");
	...
	IN PFT_R1HK_ENTRY pEntry)
{
	...
	HashId = FT_R1KH_HASH_INDEX(pEntry->PmkR1Name);
	OS_SEM_LOCK(&(pFtTab->FT_R1khEntryTabLock));
	delEntryList(&pFtTab->FT_R1khEntryTab[HashId],
				 (RT_LIST_ENTRY *)pEntry);
	os_free_mem(pEntry);
	pFtTab->FT_R1khEntryTabSize--;
	OS_SEM_UNLOCK(&(pFtTab->FT_R1khEntryTabLock));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	case FT_ACTION_BT_REQ:
		...
		if (IsTerminate) {
			...
			result = FT_ReqActionParse(pAd, (FtActLen - sizeof(FT_ACTION)),
	...
	case FT_ACTION_BT_ACK:
		...
		MakeOutgoingFrame(pFtActFrame,  &FtActFrameLen,
						  Elem->MsgLen,				pHdr,
						  END_OF_ARGS);
		MiniportMMRequest(pAd, 0, pFtActFrame, FtActFrameLen, NULL);
		break;
	...
	if ((ft_act_len + sizeof(HEADER_802_11)) < MAX_MGMT_PKT_LEN) {
		...
		if (pEntry) {
			REPORT_MGMT_FRAME_TO_MLME(pAd, Wcid, pOutBuffer, FrameLen,
					0, 0, 0, 0, 0, 0, OPMODE_AP, wdev, pEntry->HTPhyMode.field.MODE, 0, 0);
		} else {
			/* Report basic phymode if pEntry = NULL  */
			REPORT_MGMT_FRAME_TO_MLME(pAd, Wcid, pOutBuffer, FrameLen,
					0, 0, 0, 0, 0, 0, OPMODE_AP, wdev, (WMODE_CAP_6G(wdev->PhyMode) || WMODE_CAP_5G(wdev->PhyMode)) ? MODE_OFDM : MODE_CCK, 0, 0);
		}
	} else
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, "len is overflow\n");
	if (pOutBuffer)
	...
	OUT	PFT_FTIE_INFO		pFtInfo)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "key idx(%d)\n", key_idx);
	...
	OUT FT_KDP_EVT_KEY_ELM * pEvtKeyReq)
{
	...
	CacheIdx = RTMPSearchPMKIDCache(
		...
		pMbss_wdev->bssid, pEvtKeyReq->MacAddr, TRUE, FALSE);
	...
	pkeyInfo = &PD_GET_PMKID_PTR(pAd->physical_dev)->BSSIDInfo[CacheIdx];
	...
	OUT PFT_INFO			pFtInfo_out)
{
	...
	result = WPAValidateRSNIE(wdev, sec_config, pFtInfo_in->RSN_IE, pFtInfo_in->RSNIE_Len);
	if (result != MLME_SUCCESS)
		return result;
	...
	*R0KH_IdLen = 0;
	...
	while (RemainLen > 0) {
		...
		if ((subEidPtr->SubElementId == FT_R1KH_ID) &&
			(subEidPtr->Len <= FT_R1KH_ID_LEN)) {
			RTMPMoveMemory(pR1KH_Id, subEidPtr->Oct, subEidPtr->Len);
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "R1KHID length(%d)\n", subEidPtr->Len);
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "done\n");
}
	...
	IN	PRTMP_ADAPTER	pAd, IN struct wifi_dev *wdev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "==> FT_CheckForRoaming\n");
	...
	for (i = 0; i < ScanTab->BssNr; i++) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "max_rssi = %d, pBss->Rssi = %d\n", max_rssi, pBss->Rssi);
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "<== FT_CheckForRoaming (BssNr=%d)\n", pRoamTab->BssNr);

	if (pRoamTab->BssNr > 0) {
		/* check CntlMachine.CurrState to avoid collision with NDIS SetOID request */
		if (cntl_idle(wdev)) {
			pAd->RalinkCounters.PoorCQIRoamingCount++;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "MMCHK - Roaming attempt #%ld\n", pAd->RalinkCounters.PoorCQIRoamingCount);
			return TRUE;
		}
	}
...
#else
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_DEBUG, "c_len = %d\n", c_len);
#endif /* RT_CFG80211_SUPPORT */
...
INT	Show_FTConfig_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		PAP_BSSID_INFO pBssInfo = &PD_GET_PMKID_PTR(pAd->physical_dev)->BSSIDInfo[i];

		if ((pBssInfo->Valid) && (MAC_ADDR_EQUAL(&pBssInfo->own_mac, pAd->ApCfg.MBSSID[apidx].wdev.bssid))) {
			MTWF_PRINT("IDX: %d, "MACSTR", %ld\n",
				i, MAC2STR(pBssInfo->peer_mac), pBssInfo->RefreshTime);
			MTWF_PRINT("PMKID:");
			for (j = 0; j < 16 ; j++)
