	...
	UCHAR *pFrame)
{
	...
	if (pMbss && pMbss->GASCtrl.b11U_enable) {
		ULONG TmpLen;

		OS_SEM_LOCK(&pMbss->GASCtrl.IeLock);
		...
		FrameLen += TmpLen;
		if (pMbss->GASCtrl.RoamingConsortiumIELen && pMbss->GASCtrl.RoamingConsortiumIE) {
			/* Roaming Consortium element */
			MakeOutgoingFrame(pFrame + FrameLen, &TmpLen,
					  pMbss->GASCtrl.RoamingConsortiumIELen,
					  pMbss->GASCtrl.RoamingConsortiumIE, END_OF_ARGS);
			FrameLen += TmpLen;
		}
		OS_SEM_UNLOCK(&pMbss->GASCtrl.IeLock);
	}
...
VOID GASCtrlRemoveAllIE(PGAS_CTRL pGasCtrl)
{
	...
	if (pGasCtrl->RoamingConsortiumIELen && pGasCtrl->RoamingConsortiumIE) {
		os_free_mem(pGasCtrl->RoamingConsortiumIE);
		pGasCtrl->RoamingConsortiumIE = NULL;
		pGasCtrl->RoamingConsortiumIELen = 0;
	}
	...
	GAS_EVENT_DATA *Event)
{
	...
	if (Event->EventType == GAS_RSP) {
		...
		*Pos++ = Event->u.GAS_RSP_DATA.AdvertisementProID; /* Advertisement Protocol ID field */
		...
		if (VarLen - (Pos - GASFrame->u.GAS_INIT_RSP.Variable) >= 2)
		...
		if (Event->u.GAS_RSP_DATA.QueryRspLen > 0 &&
				...
				VarLen - (Pos - GASFrame->u.GAS_INIT_RSP.Variable) >= Event->u.GAS_RSP_DATA.QueryRspLen)
	...
	GAS_EVENT_DATA *Event)
{
	...
	UCHAR *Buf, *Pos;
	GAS_FRAME *GASFrame;
	ULONG FrameLen = 0, tmpLen = 0, VarLen;
	...
	VarLen = (GAS_AD_PROTO_ID_LEN + GAS_QUERY_RESPONSE_LEN) + pGASCtrl->MMPDUSize;
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "%s\n", __func__);
	OS_SEM_LOCK(&pGASCtrl->GASPeerListLock);
	DlListForEachSafe(GASPeerEntry, GASPeerEntryTmp,
				 &pGASCtrl->GASPeerList, GAS_PEER_ENTRY, List) {
		if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr, Event->PeerMACAddr)) {
	...
	if (Event->EventType == GAS_CB_REQ) {
		...
		*Pos++ = Event->u.GAS_CB_REQ_DATA.AdvertisementProID; /* Advertisement Protocol ID field */
		...
		if ((Event->u.GAS_CB_REQ_DATA.StatusCode == 0) && bGASQueryRspFragFound && GASQueryRspFrag) {
			...
			if (VarLen - (Pos - GASFrame->u.GAS_CB_RSP.Variable) >= 2)
			...
			if (GASQueryRspFrag->FragQueryRspLen > 0 &&
					VarLen - (Pos - GASFrame->u.GAS_CB_RSP.Variable) >= GASQueryRspFrag->FragQueryRspLen)
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	 ...
	 * when no GAS Init request received */
	if (pAd->bDppEnable && pAd->bDppGasInitReqRecived) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				"not used in case of DPP GAS CB Req\n");
		return;
	}
#endif /* DPP_SUPPORT */
...
BOOLEAN GasEnable(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!pGASCtrl) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_DEBUG,
			"Can not find Peer Control DA="MACSTR"\n", MAC2STR(GASFrame->Hdr.Addr1));
		return FALSE;
	}
