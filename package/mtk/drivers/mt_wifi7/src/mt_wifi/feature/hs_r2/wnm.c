							   ...
							   const char *btm_query, UINT16 btm_query_len)
{
	...
	if (buf == NULL) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
						...
						IN INT is_atomic)
{
	...
	if (ProtoType == ETH_P_ARP) {
		...
		if ((NdisCmpMemory(SenderIP, TargetIP, 4) == 0) && (NdisCmpMemory(DAMacAddr, BroadcastMac, 6) == 0)) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "The Packet is GratuitousARP\n");
			return TRUE;
		}
		...
		if (ARPOperation == 0x0002) {
			OS_SEM_LOCK(&pWNMCtrl->ProxyARPListLock);
			...
			OS_SEM_UNLOCK(&pWNMCtrl->ProxyARPListLock);
	...
	if (ProtoType == ETH_P_ARP) {
		...
		if (ARPOperation == 0x0001) {
			...
			if (NdisCmpMemory(SenderIP, TargetIP, 4) != 0) {
				MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "IPv4ProxyARPCandidate\n");
				return TRUE;
			}
		}
	}
								 ...
								 IN PUCHAR pDataEnd)
{
	...
	/* Check ifZZ DHCPv6 solicitation */
	type = (unsigned char *)(Pos + 8);
	if (type >= pDataEnd)
		return FALSE;
	...
	if ((*type == 1) || (*type == 4)) {
		...
		if ((SrcPort == 546) && (DstPort == 547)) {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
				"The Packet is DHCPv6 Solicitation,msg type=%d\n", *type);
			return TRUE;
		}
	}
								 ...
								 IN PUCHAR pDataEnd)
{
	...
	if (*Pos == 0x85) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
			"The Packet is IPv6 Router Solicitation\n");
		return TRUE;
	}
								  ...
								  IN PUCHAR pOffset)
{
	...
	if (*Pos == 0x86) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
			"The Packet is IPv6 Router Advertisement\n");
					 ...
					 IN PUCHAR pData)
{
	...
	if (ProtoType == 0x890d) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "THe Packet is TDLS\n");
		return TRUE;
	}
					 ...
					 IN INT is_atomic)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
		"wdev_idx %d TargetIP %d:%d:%d:%d\n", pMbss->wdev.wdev_idx,
		TargetIPAddr[0], TargetIPAddr[1], TargetIPAddr[2], TargetIPAddr[3]);
	if (!is_atomic)
		OS_SEM_LOCK(&pWNMCtrl->ProxyARPListLock);
	DlListForEach(ProxyARPEntry, &pWNMCtrl->IPv4ProxyARPList, PROXY_ARP_IPV4_ENTRY, List) {
	...
	if (IsFound) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
			"TargetIP %d:%d:%d:%d indicate to daemon\n",
			TargetIPAddr[0], TargetIPAddr[1], TargetIPAddr[2], TargetIPAddr[3]);
...
#ifdef CONFIG_DOT11V_WNM
static void receive_event_report(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	PUCHAR ptr = &Elem->Msg[sizeof(HEADER_802_11) + 3];
	int rest_len = (int)Elem->MsgLen;
#ifdef RT_CFG80211_SUPPORT
#ifndef CFG80211_BYPASS
	BOOL os_need = FALSE;
#endif
#endif
	rest_len -= (sizeof(HEADER_802_11) + 3);
	while (rest_len > 0) {
		struct WNM_EVT_REPORT_ELEMENT *pRptElement = (struct WNM_EVT_REPORT_ELEMENT *)ptr;

		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
			"Element_ID = %d, EvtToken = %d, EvtStatus = %d, EvtType = %d\n",
			pRptElement->Element_ID, pRptElement->EvtToken, pRptElement->EvtStatus,
			pRptElement->EvtType);
		switch (pRptElement->Element_ID) {
		case IE_EVENT_REPORT:
			if (pRptElement->EvtToken == 0) { /* sent autonomously */
#ifdef DOT11_HE_AX
				if (pRptElement->EvtStatus == STATUS_SUCCESSFUL) {
					if (pRptElement->EvtType == BSS_COLOR_COLLISION)
						bss_color_parse_collision_report(Elem->wdev, pRptElement->EvtReport);
					else if (pRptElement->EvtType == BSS_COLOR_INUSE)
						bss_color_parse_inuse_report(Elem->wdev, pRptElement->EvtReport);
				}
#endif
			}
#ifdef RT_CFG80211_SUPPORT
#ifndef CFG80211_BYPASS
			if (pRptElement->EvtType <= WNM_LOG)
				os_need = TRUE;
#endif
#endif
			break;
		...
		ptr += (pRptElement->Length + 2);
		rest_len -= (pRptElement->Length + 2);
	}
#ifdef RT_CFG80211_SUPPORT
#ifndef CFG80211_BYPASS
	if (os_need == TRUE) {
		...
		freq /= 1000;
		CFG80211OS_RxMgmt(Elem->wdev->if_dev, freq, Elem->Msg, Elem->MsgLen);
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO,
			"Pass wnm action frame to os\n");
	}
#endif
#endif
}
							...
							IN MLME_QUEUE_ELEM * Elem)
{
	...
	MAP_CHANNEL_ID_TO_KHZ(pAd->LatchRfRegs.Channel, freq);
	freq /= 1000;
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_DEBUG, "CFG80211OS_RxMgmt!\n");
	...
	IN UINT32 btm_req_data_len)
{
	...
	if (p_btm_req_data->num_candidates == 0) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_WARN,
			"candidate count equals to 0; btm request is meaningless\n");
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	pAd->ApCfg.btm_stat.btm_request_sent++;
	if (pAd->ApCfg.btm_stat.btm_request_sent == 9999)
		pAd->ApCfg.btm_stat.btm_request_sent = 0;
#endif
		...
		if (pEntry != NULL) {
			...
			if ((*(BTMData) & 0x1C) != 0) {
				pEntry->BTMDisassocCount = (((*(BTMData + 2) << 8) | (*(BTMData + 1))) *
					pAd->CommonCfg.BeaconPeriod) / 1000;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (sizeof(ActHdr) <= MAX_MGMT_PKT_LEN) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_ERROR,
			"Not have enough buffer size for pOutBuffer!\n");
	...
	if ((FrameLen + btm_reqinfo_len) > MAX_MGMT_PKT_LEN) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_ERROR,
			"!!! ERROR Total Len %lu > MAX_MGMT_PKT_LEN(%u) !!!\n", (ULONG)(FrameLen + btm_reqinfo_len), MAX_MGMT_PKT_LEN);
...
static VOID SendBTMConfirm(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "\n");
#ifdef CCN67_BS_SUPPORT
	if (Event->u.PEER_BTM_RSP_DATA.BTMRspLen > 0) {
		UINT8 status = Event->u.PEER_BTM_RSP_DATA.BTMRsp[0];

		if (status <= 8)
			pAd->ApCfg.btm_stat.btm_response[status]++;
		else {
			status = 9;
			pAd->ApCfg.btm_stat.btm_response[status]++;
		}
		if (pAd->ApCfg.btm_stat.btm_response[status] >= 9999)
			pAd->ApCfg.btm_stat.btm_response[status] = 0;
	}
#endif
	/* Send BTM confirm to daemon */
	...
	IN UINT32 btm_req_data_len)
{
	...
	if (p_btm_req_data->btm_req_len <= btm_req_data_len)
		NdisMoveMemory(Event->u.BTM_REQ_DATA.BTMReq,
			p_btm_req_data->btm_req, p_btm_req_data->btm_req_len);
	else
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
			"Not have enough buffer size for Event->u.BTM_REQ_DATA.BTMReq!\n");
...
NDIS_STATUS wnm_handle_command(IN PRTMP_ADAPTER pAd, IN struct wnm_command *pCmd_data)
{
	...
	case OID_802_11_WNM_CMD_ENABLE:
	{
		if (pCmd_data->command_body[0] == 0) {
			...
			UpdateBeaconHandler(pAd, &pAd->ApCfg.MBSSID[APIndex].wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
		} else {
		}
		...
		IN UINT32 BTMRspLen)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_NOTICE, "--->\n");
	...
	if (!IsFound) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				"BTMPeerEntry not found\n");
		goto error0;
	}

	os_alloc_mem(NULL, (UCHAR **)&Buf, sizeof(*WNMFrame) + BTMRspLen);

	if (!Buf) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR, "Not available memory\n");
		goto error0;
	}
	NdisZeroMemory(Buf, sizeof(*WNMFrame) + BTMRspLen);

	WNMFrame = (WNM_FRAME *)Buf;
	ActHeaderInit(pAd, &WNMFrame->Hdr, PeerMACAddr,
			pAd->StaCfg[APcliIndex].wdev.if_addr,
			PeerMACAddr);
	Len += sizeof(HEADER_802_11);
	WNMFrame->Category = CATEGORY_WNM;
	Len += 1;
	WNMFrame->u.BTM_RSP.Action = BSS_TRANSITION_RSP;
	Len += 1;
	WNMFrame->u.BTM_RSP.DialogToken = BTMPeerEntry->DialogToken;
	Len += 1;
	NdisMoveMemory(WNMFrame->u.BTM_RSP.Variable, BTMRsp, BTMRspLen);
	Len += BTMRspLen;
	MiniportMMRequest(pAd, 0, Buf, Len, NULL);
...
BOOLEAN nr_peer_btm_req_action(IN PRTMP_ADAPTER pAd, struct wifi_dev *wdev, const char *btm_req,
		UINT16 btm_req_len, enum bss_trans_mgmt_status_code *status)
{
	UINT8 *pos = (UINT8 *)btm_req;
	UINT8 *end = (UINT8 *)btm_req + btm_req_len;
	UINT8 req_mode = 0, ret = 0;
	UINT8 disassoc_timer = 0;
	UINT8 val_interval = 0;
	INT32 parsed_len = 0;
	...
	if (req_mode & (1 << CAND_LIST_INCLUDED_BIT_MAP)) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
			"WNM: Preferred List Available, Neighbor report tag %d\n", *pos);
#ifdef WAPP_SUPPORT
		NdisZeroMemory(&wdev->daemon_nr_list, sizeof(DAEMON_NR_LIST));
#endif /* WAPP_SUPPORT */
		while ((parsed_len + 2) < btm_req_len) {
			if (*pos == IE_NEIGHBOR_REPORT) {
				if ((btm_req_len - parsed_len)
					< sizeof(struct neighbor_report_element)) {
					ret = MBO_INVALID_ARG;
					break;
				}
				if (parsed_len + 2 + pos[1] > btm_req_len) {
					ret = MBO_INVALID_ARG;
					break;
				}
				ret = parse_nr_elem(pAd, wdev, (void *)pos);

			} else {
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
					"WNM: unrecognized sub-element ID %d\n", *pos);
			}
			parsed_len +=  2 + pos[1];
			pos += 2 + pos[1];
		}
		if (ret > 0) {
			*status = WNM_BSS_TM_REJECT_NO_SUITABLE_CANDIDATES;
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				"WNM: Candidate list included bit is set, but no candidates found %d\n",
				*pos);
			return FALSE;
		}
	}
...
void build_btm_rsp(IN PRTMP_ADAPTER pAd,
		struct wifi_dev *wdev,
		enum bss_trans_mgmt_status_code status_code,
		UINT8 *peer_mac_addr,
		UINT8 *target_bssid, UINT8 ChNum)
{
	char btm_rsp_buf[1000] = {0};
	...
	frame->u.btm_rsp.bss_termination_delay = 0;
	pos += 1;
	btm_rsp_len += 1;
	...
	ret = Send_BTM_Rsp(pAd, peer_mac_addr, btm_rsp_buf, btm_rsp_len);
	/* Roam to target_bssid */
	if (ret == TRUE) {
		pObj = (POS_COOKIE) pAd->OS_Cookie;
		ifIndex = pObj->ioctl_if;

		pAd->StaCfg[ifIndex].isReassoc = TRUE;
		pAd->StaCfg[ifIndex].bAutoRoaming = TRUE;
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_NOTICE,
				"WNM set bAutoRoaming=%d\n", pAd->StaCfg[ifIndex].bAutoRoaming);
		NdisZeroMemory(pAd->StaCfg[ifIndex].CfgApCliBssid, MAC_ADDR_LEN);
		if (target_bssid)
			COPY_MAC_ADDR(pAd->StaCfg[ifIndex].CfgApCliBssid, target_bssid);
		MTWF_DBG(pAd, DBG_CAT_CLIENT, CATPROTO_WNM, DBG_LVL_NOTICE,
		...
		if (status_code == WNM_BSS_TM_ACCEPT) {
			...
			if (!pEntry) {
				MTWF_DBG(pAd, DBG_CAT_CLIENT, CATPROTO_WNM, DBG_LVL_NOTICE,
						"Could not find the Peer MAC addr, pEntry is NULL");
				return;
			}
			cntl_disconnect_request(pEntry->wdev, CNTL_DEAUTH, pEntry->Addr, REASON_DEAUTH_STA_LEAVING);
		}
	}
}

BOOLEAN check_btm_req_sanity(
		IN PRTMP_ADAPTER pAd,
		struct wifi_dev *wdev,
		const UCHAR *peer_mac_addr,
		const char *btm_req,
		UINT16 btm_req_len,
		enum bss_trans_mgmt_status_code *status)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_NOTICE, "\n");
	ret = nr_peer_btm_req_action(pAd, wdev, btm_req, btm_req_len, status);
	return ret;
}
						  ...
						  IN MLME_QUEUE_ELEM * Elem)
{
	...
	UINT8 target_bssid[MAC_ADDR_LEN] = {0};
	UINT8 ChNum = 0;
	enum bss_trans_mgmt_status_code status;
	...
	if (check_btm_req_sanity(pAd, wdev, BTMPeerEntry->PeerMACAddr,
				(PUCHAR)&(WNMFrame->u.BTM_REQ.Variable), VarLen, &status) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				"BTM Sanity Failed\n");
		return;
	}
...
#ifdef MBO_SUPPORT
	if (nr_list_get_target_bss(pAd, wdev, target_bssid, &ChNum) != MBO_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
	...
	build_btm_rsp(pAd, wdev, status, BTMPeerEntry->PeerMACAddr, target_bssid, ChNum);
	return;
}
				   ...
				   IN MLME_QUEUE_ELEM * Elem)
{
	...
	default:
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "Invalid action field = %d\n", Action);
		break;
	}
}
#endif
			...
			IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "sta mac "MACSTR"\n",
		MAC2STR(WNMFrame->Hdr.Addr2));
#endif /* MBO_SUPPORT */

	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "MsgLen %ld MBSS "MACSTR"\n",
	...
	NetDev = pAd->ApCfg.MBSSID[APIndex].wdev.if_dev;
	VarLen = Elem->MsgLen -	sizeof(HEADER_802_11);
	ptr = Elem->Msg + sizeof(HEADER_802_11);
	...
	while ((pos+1) < OptionalElementLen) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO, "pos %d OptionalElementLen %d ElementID %d ElementLen %d\n", pos, OptionalElementLen, ElementID, ElementLen);
			...
			if (NdisEqualMemory(wfa_oui, &WNMFrame->u.WNM_NOTIFY_REQ.Variable[pos], 3)) {
				...
				if (pWdev && IS_MBO_ENABLE(pWdev)) {
					...
					case MBO_OUI_NON_PREFERRED_CHANNEL_REPORT:
						...
						bIndicateNPC = TRUE;
						MTWF_DBG(pAd,
						DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_INFO
						, "npc_num %d mac_addr "MACSTR" bIndicateNPC %d\n"
						, MboStaInfoNPC.npc_num
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (Event->EventType == 0) { /* remediation */
		...
		if ((VarLen+1) >= 3)
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				"Not have enough buffer size for WNMFrame->u.WNM_NOTIFY_REQ.Variable!\n");
