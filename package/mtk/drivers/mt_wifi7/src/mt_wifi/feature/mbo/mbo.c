...
UINT8 nr_list_get_target_bss(PRTMP_ADAPTER pAd, struct wifi_dev *wdev,
		UINT8 *target_bssid, UINT8 *ChNum)
{
	...
	if (wdev->daemon_nr_list.CurrListNum >= DAEMON_NEIGHBOR_REPORT_MAX_NUM - 1) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
				"candidate list is Empty %d\n",
				wdev->daemon_nr_list.CurrListNum);
		return MBO_INVALID_ARG;
	}

	for (i = 0; i < wdev->daemon_nr_list.CurrListNum; i++) {
		wapp_nr_info *nr_entry = &wdev->daemon_nr_list.NRInfo[i];
...
int nr_list_lookup_by_mac_addr(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, PUINT8 mac_addr)
{
	UINT16 i = 0;
	UINT8 ret = 0;

#ifdef WAPP_SUPPORT
	ret = NR_ENTRY_NOT_FOUND;

	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO, "--->\n");

	for (i = 0; i < wdev->daemon_nr_list.CurrListNum; i++) {
		if (MAC_ADDR_EQUAL(mac_addr, &wdev->daemon_nr_list.NRInfo[i].Bssid[0])) {
			ret = i;
			break;
		}
	}
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO, "ret index %d\n", ret);
#endif /* WAPP_SUPPORT */
	return ret;
}

int nr_entry_add(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, wapp_nr_info *nr_info)
{
#ifdef WAPP_SUPPORT
	int nr_index = 0;
	...
	nr_index = nr_list_lookup_by_mac_addr(pAd, wdev, nr_info->Bssid);

	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
			"nr_index %d CurrListNum %d\n",
			nr_index, wdev->daemon_nr_list.CurrListNum);

	if (nr_index < NR_ENTRY_NOT_FOUND)  /* found exist entry */
		nr_entry = &wdev->daemon_nr_list.NRInfo[nr_index];
		...
		if (wdev->daemon_nr_list.CurrListNum < (DAEMON_NEIGHBOR_REPORT_MAX_NUM - 1)) {
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
					"list is full\n");
			return MBO_RESOURCE_ALLOC_FAIL;
		}
	}
...
UINT8 parse_nr_elem(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, void *nr_elem)
{
	...
	wapp_nr_info nrInfo;
	UINT16 parseLen = 0;
	UINT8 ret = MBO_SUCCESS;

	if (nrElem == NULL) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR, "nrElem is NULL\n");
		return MBO_RESOURCE_ALLOC_FAIL;
	}

	/* Subtract 2 from sizeof(struct neighbor_report_element) to skip EID & Length */
	if (sizeof(wapp_nr_info) >= (sizeof(struct neighbor_report_element) - SKIP_EID_LEN))
		NdisCopyMemory(&nrInfo, (UINT8 *)nrElem + SKIP_EID_LEN, sizeof(struct neighbor_report_element) - SKIP_EID_LEN);
	else
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
		"length error! t_len=%u, s_len=%u\n", (UINT32)sizeof(wapp_nr_info),
		(UINT32)(sizeof(struct neighbor_report_element) - SKIP_EID_LEN));
	...
	while (parseLen + 2 < nrElem->length) {
		switch (nrSubElem->subelement_id) {
		case MBO_RRM_SUBID_BSS_TRANSITION_CANDIDATE_PREFERENCE:
			...
			ret = nr_entry_add(pAd, wdev, &nrInfo);
			if (ret == MBO_RESOURCE_ALLOC_FAIL)
				return ret;
			break;
		default:
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
					"Unhandled SubElement %d\n",
					nrSubElem->subelement_id);
			break;
		}
		parseLen += 2 + nrSubElem->length;
		nrSubElem = (struct neighbor_report_subelement *)
			((UINT8 *)nrSubElem + 2 + nrSubElem->length);
	}
	...
	)
{
	...
	if (wdev) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
			"Mbss is NULL !!!!!\n");
		return MBO_INVALID_ARG;
	}
	...
	)
{
	...
	IS_AKM_WPA_CAPABILITY(wdev->SecConfig.AKMMap)) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_DEBUG,
				"Security is WPA2/WPA3, please enable PMF MFPC\n");
		return;
	}
#endif
...
BOOLEAN MboParseApMboIE(PRTMP_ADAPTER pAd, UCHAR *pAddr, UCHAR *buf, UCHAR len)
{
	...
	*/
	while ((ParsedLen + 2) <= len) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_DEBUG,
			"eid_ptr->Len %d ParsedLen %d IE len %d Break %d\n",
			eid_ptr->Len, ParsedLen, len, ((ParsedLen+2) < len));
	...
	UINT16 report_buf_len)
{
...
#else
	buflen = sizeof(struct wapp_event);
#endif
	...
	if (buf == NULL) {
		...
		event->event_id = WAPP_CONFIG_NR_EVENT;
		event->ifindex = RtmpOsGetNetIfIndex(net_dev);
		NdisCopyMemory(&event->data, NeighborRepList, report_buf_len);
...
#endif /* DOT11_VHT_AC */
#ifdef DOT11_HE_AX
		BssidInfo.field.HE = WMODE_CAP_AX(pWdev->PhyMode) ? 1:0;
#endif
#ifdef DOT11_EHT_BE
		BssidInfo.field.EHT = WMODE_CAP_BE(pWdev->PhyMode) ? 1:0;
#endif
		if (WMODE_CAP_BE(pWdev->PhyMode))
			CondensedPhyType = 16;/*dot11PHYType-<<Draft P802.11be_D2.0.pdf>>*/
		else if (WMODE_CAP_AX(pWdev->PhyMode))
			CondensedPhyType = 14;
		else if (WMODE_CAP_AC(pWdev->PhyMode))
			CondensedPhyType = 9;
		else if (WMODE_CAP_N(pWdev->PhyMode))
			CondensedPhyType = 7;
...
#endif
#ifdef DOT11_EHT_BE
		BssidInfo.field.EHT = HAS_EHT_CAPS_EXIST(pBssEntry->ie_exists) ? 1:0;
#endif
	...
	if (ScanTab->BssNr > 0) {
		...
		for (loop = 0; loop < pAd->ApCfg.BssidNum; loop++) {
			...
			if ((NeighborRepList.CurrNum % PER_EVENT_LIST_MAX_NUM == 0) ||
				...
				WextMboSendNeighborReportToDaemonEvent(pAd, pWdev->if_dev,
						&NeighborRepList,
		...
		for (loop = 0; loop < ScanTab->BssNr; loop++) { /* minus our own NR entry */
			...
			if (pBssEntry) {
				...
				loop, (UINT32)sizeof(wapp_nr_info),
				MAC2STR(pNeighborEntry->Bssid),
				...
				hex_dump("neighbor_entry", (UCHAR *)pNeighborEntry,
					sizeof(wapp_nr_info));
			}
	...
	UINT32 Value)
{
	...
	case PARAM_MBO_AP_ASSOC_DISALLOW:
		if (Value > (UINT32)0xff) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
					"invalid mbo ap assoc disallow value %u\n",
					Value);
			return FALSE;
		}
		pMboCtrl->AssocDisallowReason = (UINT8)Value;
		break;
	case PARAM_MBO_AP_CAP:
		if (Value > (UINT32)0xff) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
					"invalid mbo ap cap value %u\n",
					Value);
			return FALSE;
		}
		pMboCtrl->MboCapIndication = (UINT8)Value;
		break;
	case PARAM_MBO_AP_CDCP:
		if (Value > (UINT32)0xff) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
					"invalid mbo cellular preference value %u\n",
					Value);
			return FALSE;
		}
		pMboCtrl->CellularPreference = (UINT8)Value;
		break;
	...
	UINT16 ReportBufLen)
{
	...
	if (buf == NULL) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
					...
					(UINT32)sizeof(MBO_STA_CH_PREF_CDC_INFO), ReportBufLen, buflen, MboMsgTypeToString(MsgType));
	...
	MBO_MSG_TYPE MsgType)
{
	...
	if (pEntry != NULL) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_INFO,
		"\033[1;33m pEntry->wcid %d pStaInfo->akm 0x%x pStaInfo->cipher 0x%x\033[0m\n"
		, pEntry->wcid, pStaInfo->akm, pStaInfo->cipher);
	} else {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
	...
	*/
	while ((ParsedLen+2) <= len) {
		...
		case MBO_ATTR_STA_NOT_PREFER_CH_REP:
				...
				, pMboStaInfoNPC->npc_num
				, MAC2STR(pMboStaInfoNPC->mac_addr), pEntry->bIndicateNPC);
				break;
		default:
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR,
					"ignored MBO_ATTR [%d]\n", eid_ptr->Eid);
	...
	UINT16 ReportBufLen)
{
	...
	if (buf == NULL) {
		...
		if (pStaDisassocInfo && MAC_ADDR_EQUAL(pStaDisassocInfo->mac_addr, ZERO_MAC_ADDR)) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_WARN,
				"["MACSTR"] sizeof %u report_buf_len %d buflen %d msg_type %s\n",
		...
		RTMP_STRING *arg)
{
	...
	if (req_type != CLEAR_PREF && i < 5) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MBO, DBG_LVL_ERROR, "Incorrect parameters\n");
		return -1;
	}
