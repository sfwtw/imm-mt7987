	...
	struct wifi_dev *wdev)
{
	...
	if (IS_FD_FRAME_FW_MODE(cap)) {
		HW_SET_FRAME_OFFLOAD(pAd, wdev->wdev_idx, 0,
			FALSE, 0, 1 /* BSSINFO_UNSOLICIT_TX_FILS_DISC */, 0, 200, NULL);
	} else {
		BOOLEAN Cancelled;
	...
	)
{
	...
	case OCE_ATTR_AP_RNR_COMPLETE:
		...
		if (pMbss && pMbss->ReducedNRListExist && (pMbss->ReducedNRListInfo.ValueLen)) {
			...
			for (i = 0; i < OceAttr.AttrLen && ((i + 1) * SHORT_SSID_LEN <= OCE_ATTR_MAX_LEN); i++) {
	...
	)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_DEBUG,
		"collect oce attr for FrameType %d\n", FrameType);
	...
	)
{
	...
	while (ParsedLen + 2 <= len) {
		/* avoid eid_ptr trace over than len. */
		...
		case OCE_ATTR_STA_PRB_SUP_BSSID:
			...
			for (Bssid_offset = 0; Bssid_offset < eid_ptr->Len; Bssid_offset += 6) {
				/* insure the basic address len would not more than buffer Len*/
				if ((eid_ptr->Len - Bssid_offset) < 6)
					break;
	...
	OCE_MSG_TYPE MsgType)
{
	...
	if (pEntry != NULL) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_INFO,
				"\033[1;33m %u pEntry->wcid %d \033[0m\n", __LINE__, pEntry->wcid);
	} else {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_OCE, DBG_LVL_ERROR,
