...
#ifdef UAPSD_DEBUG
	MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> allocate a spinlock!\n");
#endif /* UAPSD_DEBUG */
#ifdef UAPSD_DEBUG
...
#ifdef UAPSD_DEBUG
	MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> release a spinlock!\n");
#endif /* UAPSD_DEBUG */
...
========================================================================
*/
static inline VOID __UAPSD_SP_Close(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	if ((pEntry != NULL) && (pEntry->PsMode == PWR_SAVE)) {
		...
		if (pEntry->bAPSDFlagSPStart != 0) {
...
#ifdef UAPSD_DEBUG
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> [3] close SP!\n");
#endif /* UAPSD_DEBUG */
			...
			if (pEntry->pUAPSDEOSPFrame != NULL) {
				...
				*/
#ifdef UAPSD_DEBUG
				MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> [3] Free EOSP (UP = %d)!\n",
						 RTMP_GET_PACKET_UP(
...
{
	struct _QUEUE_HEADER *pQueApsd;
	struct _QUEUE_ENTRY *pQueEntry;
	...
	IN BOOLEAN FlgApsdCapable)
{
	...
	if ((pEntry) && (FlgApsdCapable == TRUE)) {
		...
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "apsd> UAPSD %d %d %d %d!\n",
				 pQosInfo->UAPSD_AC_BE, pQosInfo->UAPSD_AC_BK,
				 pQosInfo->UAPSD_AC_VI, pQosInfo->UAPSD_AC_VO);
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "apsd> MaxSPLength = %d\n",
				 pEntry->MaxSPLength);
		...
		if ((pEntry->bAPSDCapablePerAC[QID_AC_BE] == 1) &&
			...
			/* all AC are U-APSD */
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "apsd> all AC are UAPSD\n");
			pEntry->bAPSDAllAC = 1;
		} else {
			/* at least one AC is not U-APSD */
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "apsd> at least one AC is not UAPSD %d %d %d %d\n",
	...
	IN BOOLEAN bFromHead)
{
	...
	*/
	...
	struct _QUEUE_HEADER *pQueUapsd;
	ULONG flags = 0;

	/* check if current queued UAPSD packet number is too much */
	if (pEntry == NULL) {
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> pEntry == NULL!\n");
		return;
	}
	...
	if (pQueUapsd->Number >= MAX_PACKETS_IN_UAPSD_QUEUE) {
		...
		if (RTMP_GET_PACKET_MGMT_PKT(pPacket) == 1)
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "ps> mgmt to uapsd queue...\n");
		else {
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO,
...
#ifdef UAPSD_DEBUG
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> aux: Station actives! Close SP!\n");
#endif /* UAPSD_DEBUG */
		/* pEntry->bAPSDFlagSPStart = 0; */
		pEntry->bAPSDFlagEOSPOK = 0;
		UAPSD_SP_END(pAd, pEntry);
	...
	if (pEntry->PsMode == PWR_SAVE) {
...
#ifdef UAPSD_DEBUG
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> aux: Tx Num = %d\n", pEntry->UAPSDTxNum);
#endif /* UAPSD_DEBUG */
		FlgEosp = FALSE;
		...
		if (pEntry->bAPSDFlagSPStart == 0) {
			...
			*/
			pEntry->bAPSDFlagLegacySent = 0;
#ifdef UAPSD_DEBUG
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> legacy PS packet is sent!\n");
#endif /* UAPSD_DEBUG */
		} else {
#ifdef UAPSD_TIMING_RECORD_FUNC
		...
		if (pEntry->UAPSDTxNum > 0) {
			...
			if (pEntry->UAPSDTxNum == 1) {
				...
				if (pEntry->pUAPSDEOSPFrame != NULL) {
					...
					PNDIS_PACKET pPkt;
#ifdef UAPSD_DEBUG
					MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> aux: send EOSP frame...\n");
#endif /* UAPSD_DEBUG */
...
#ifdef UAPSD_DEBUG
							MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD,
							DBG_LVL_INFO,
							"SP is suspend, keep SP if exists!\n");
#endif /* UAPSD_DEBUG */
...
VOID UAPSD_TimingRecord(RTMP_ADAPTER *pAd, UINT32 Type)
{
	...
	case UAPSD_TIMING_RECORD_TX2AIR:
		...
		if ((Index % UAPSD_TIMING_RECORD_DISPLAY_TIMES) == 0) {
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> Isr2Tasklet=%d, Trig2Queue=%d, Queue2Air=%d micro seconds\n",
					 gUAPSD_TimingSumIsr2Tasklet /
		...
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "sam> Isr->Tasklet:%d, Trig->TxQueue:%d, TxQueue->TxDone:%d\n",
				 (UINT32)(gUAPSD_TimingTasklet[Index] - gUAPSD_TimingIsr[Index]),
				 (UINT32)(gUAPSD_TimingMov2Tx[Index] - gUAPSD_TimingTrgRcv[Index]),
				 (UINT32)(gUAPSD_TimingTx2Air[Index] - gUAPSD_TimingMov2Tx[Index]));
		break;
...
{
	struct _QUEUE_HEADER	*pAcPsQue;
	struct _QUEUE_ENTRY	*pQuedEntry;
	...
	*/
	...
	if (pEntry->bAPSDFlagSPStart != 0) {
		...
		*/
#ifdef UAPSD_DEBUG
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> sorry! SP is not yet closed!\n");
#endif /* UAPSD_DEBUG */
#ifdef UAPSD_SP_ACCURATE
		/*
...
#ifdef UAPSD_DEBUG
			MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> SP period is too large so SP is closed first! (%lu %lu %lu)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n",
	...
	if (pEntry->pUAPSDEOSPFrame != NULL) {
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> EOSP is not NULL!\n");
		UAPSD_SEM_UNLOCK(&pAd->UAPSDEOSPLock, flags);
		return;
	}
	...
	*/
...
#ifdef UAPSD_DEBUG
	{
		...
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> start a SP (Tx Num = %d) (Rough SP = %d) (Has Any Mgmt = %d) (Abnormal = %d) (Time = %lu)\n",
	...
	if (TxPktNum <= 1) {
		...
		*/
		RTMP_SET_PACKET_EOSP(pQuedPkt, TRUE);
		RTMP_SET_PACKET_TXTYPE(pQuedPkt, TX_LEGACY_FRAME);
		pEntry->pUAPSDEOSPFrame = (struct _QUEUE_ENTRY *)pQuedPkt;
	...
	if ((pEntry->pUAPSDEOSPFrame != NULL) &&
		...
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> The EOSP frame is a management frame.\n");
	...
	*/
	if ((pEntry->bAPSDFlagSpRoughUse == 0) &&
		...
		pEntry->UAPSDTxNum++;
#ifdef UAPSD_DEBUG
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> A legacy PS is sent! UAPSDTxNum = %d\n",
				 pEntry->UAPSDTxNum);
#endif /* UAPSD_DEBUG */
	}
...
#ifdef UAPSD_DEBUG
		MTWF_DBG(NULL, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_INFO, "uapsd> end a SP by a QoS Null frame!\n");
#endif /* UAPSD_DEBUG */
	}
