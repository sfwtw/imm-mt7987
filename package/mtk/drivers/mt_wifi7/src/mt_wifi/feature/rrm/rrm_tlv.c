	...
	IN INT StaIdx)
{
	...
	if (StaIdx < pAd->MSTANum && StaIdx >= 0)
	...
	IN PRRM_MLME_BCN_REQ_INFO pMlmeBcnReq)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO, "%s() allocate memory failed\n", __func__);
		return;
	}

	/* build action frame header. */
	if (wcid) {
		MgtMacHeaderInit(pAd, &ActHdr, SUBTYPE_ACTION, 0, entry_addr_get(pAd, wcid),
	...
	if (sizeof(HEADER_802_11) <= MAX_MGMT_PKT_LEN)
		NdisMoveMemory(pOutBuffer, (PCHAR)&ActHdr, sizeof(HEADER_802_11));
	else {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_ERROR, "%s() move memory failed\n", __func__);
		return;
	}
	...
	IN p_bcn_req_data_t p_bcn_req_data)
{
	...
	if (sizeof(HEADER_802_11) <= MAX_MGMT_PKT_LEN)
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO, "%s() move memory failed\n", __func__);
		return NDIS_STATUS_FAILURE;
	}
	...
	NStatus = MiniportMMRequest(pAd, MGMT_USE_QUEUE_FLAG | QID_AC_BE, pOutBuffer, FrameLen, NULL);
	...
	IN UINT32 BcnReqLen)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO, "%s\n", __func__);
...
#define MIN(_x, _y) ((_x) > (_y) ? (_x) : (_y))
		...
		if (BssMatch) {
			...
			*/
			...
			if (pBssEntry->Channel > 14) { /* 5G case */
				...
				if (HAS_HE_CAPS_EXIST(pBssEntry->ie_exists))
					pBssEntry->CondensedPhyType = 14;
				else if (HAS_HT_CAPS_EXIST(pBssEntry->ie_exists)) /* HT case */
					pBssEntry->CondensedPhyType = 7;
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_INFO, "send Neighbor RSP\n");
	MiniportMMRequest(pAd, (MGMT_USE_QUEUE_FLAG | QID_AC_BE), pOutBuffer, FrameLen, NULL);
	...
	IN PRRM_MLME_TRANSMIT_REQ_INFO pMlmeTxMeasureReq)
{
	...
	MeasureReqMode.word = 0;
	/* build action frame header. */
	MgtMacHeaderInit(pAd, &ActHdr, SUBTYPE_ACTION, 0, entry_addr_get(pAd, Aid),
	...
	*/
	...
	COPY_MAC_ADDR(TxMeasureReq.PeerStaMac, entry_addr_get(pAd, Aid));
		...
		UCHAR *bssid)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_NOTICE, "%s\n", __func__);
	...
	if (Token != DialogToken || !bcn_rep_cnt) {
