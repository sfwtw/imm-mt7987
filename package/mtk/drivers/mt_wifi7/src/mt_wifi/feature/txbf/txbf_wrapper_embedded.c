			...
			if (pStaCfg) {
				if (pStaCfg->MlmeAux.vht_cap.vht_cap.bfer_cap_su) {
					...
					MTWF_DBG(NULL, DBG_CAT_HW, CATHW_PHY, DBG_LVL_INFO,
						"%s num_snd_dimension=%d, bfee_sts_cap=%d, bfer=%d, bfee=%d\n",
						wdev->if_dev->name, pStaCfg->MlmeAux.vht_cap.vht_cap.num_snd_dimension,
						pStaCfg->MlmeAux.vht_cap.vht_cap.bfee_sts_cap,
						pStaCfg->MlmeAux.vht_cap.vht_cap.bfer_cap_su,
						pStaCfg->MlmeAux.vht_cap.vht_cap.bfee_cap_su);
				}
			}
		}
	}
	...
	struct he_bf_info *he_bf_struct)
{
	...
	/* Tx Path Num from eeprom: 1 for 1T, 2 for 2T, etc. */
	UINT8 u1TxPath = mcs_nss->max_path[MAX_PATH_TX];
	...
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_PHY, DBG_LVL_DEBUG,
		"mcs_nss->bw160_max_nss=%u, u1TxPath=%u\n", mcs_nss->bw160_max_nss, u1TxPath);
	...
	struct eht_bf_info *eht_bf_struct)
{
	...
	/* tx_stream: 1 for 1T, 2 for 2T, etc. */
	UINT8 u1TxPath = mcs_nss->max_path[MAX_PATH_TX];
	VOID *hdev_ctrl = hc_get_hdev_ctrl(wdev);
	struct _RTMP_CHIP_OP *ops = hc_get_chip_ops(hdev_ctrl);
	RTMP_CHIP_CAP *pChipCap = hc_get_chip_cap(hdev_ctrl);
	...
	if (!IS_PHY_CAPS(pChipCap->phy_caps, fPHY_CAP_DUALPHY)) {
		if (eht_bw > EHT_BW_80) {
			/* bw160_max_nss: 1 for 2T, 2 for 3T, etc. */
			if (u1TxPath > (mcs_nss->bw160_max_nss))
				u1TxPath = mcs_nss->bw160_max_nss;
		}
		if (eht_bw > EHT_BW_160) {
			EHT_PORTING_TODO(Need nss of BW 320 for EHT);
		}
	}
	txbf_status.eht_bf_info = eht_bf_struct;
	txbf_status.ucTxPathNum = u1TxPath;
	txbf_status.cmmCfgETxBfEnCond = wlan_config_get_etxbf(wdev);
	if (ops->get_eht_etxbf_cap)
		ops->get_eht_etxbf_cap(wdev, &txbf_status);
}
#endif /*DOT11_EHT_BE*/
