...
INT32 vow_set_sta(PRTMP_ADAPTER pad, UINT16 sta_id, UINT32 subcmd)
{
	...
	case ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP:
		sta_ctrl.rAirTimeCtrlValue.u4ComValue = pad->vow_sta_cfg[sta_id].group + pad->vow_gen.VOW_BWC_GROUP_MIN;
#ifdef CFG_BIG_ENDIAN
		sta_ctrl.rAirTimeCtrlValue.u4ComValue = cpu2le32(sta_ctrl.rAirTimeCtrlValue.u4ComValue);
#endif
		break;
	...
	case ENUM_VOW_DRR_CTRL_FIELD_AIRTIME_QUANTUM_L7:
		idx = subcmd - ENUM_VOW_DRR_CTRL_FIELD_AIRTIME_QUANTUM_L0;
		if (idx < VOW_MAX_STA_DWRR_NUM) {
			sta_ctrl.rAirTimeCtrlValue.u4ComValue =
				pad->vow_cfg.vow_sta_dwrr_quantum[idx];
			MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
				"(SubCmd %x, Value = 0x%x)\n",
				subcmd,	pad->vow_cfg.vow_sta_dwrr_quantum[idx]);
		}
#ifdef CFG_BIG_ENDIAN
		sta_ctrl.rAirTimeCtrlValue.u4ComValue = cpu2le32(sta_ctrl.rAirTimeCtrlValue.u4ComValue);
#endif
...
VOID vow_fill_group_all(PRTMP_ADAPTER pad, UINT8 group_id, EXT_CMD_BSS_CTRL_T *group_ctrl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(min rate token = 0x%x)\n",
		pad->vow_bss_cfg[group_id].min_rate_token);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max rate token = 0x%x)\n",
		pad->vow_bss_cfg[group_id].max_rate_token);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(min airtime token = 0x%x)\n",
		pad->vow_bss_cfg[group_id].min_airtime_token);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max airtime token = 0x%x)\n",
		pad->vow_bss_cfg[group_id].max_airtime_token);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(min rate bucket = 0x%x)\n",
		pad->vow_bss_cfg[group_id].min_ratebucket_size);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max rate bucket = 0x%x)\n",
		pad->vow_bss_cfg[group_id].max_ratebucket_size);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(min airtime bucket = 0x%x)\n",
		pad->vow_bss_cfg[group_id].min_airtimebucket_size);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max airtime bucket = 0x%x)\n",
		pad->vow_bss_cfg[group_id].max_airtimebucket_size);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max baclog size = 0x%x)\n",
		pad->vow_bss_cfg[group_id].max_backlog_size);
	MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "(max wait time = 0x%x)\n",
...
INT vow_set_feature_all(PRTMP_ADAPTER pad)
{
	...
	/* DW1 - flags */
	feature_ctrl.u2IfApplyBssCheckTimeToken_0_to_16_CtrlFlag = 0;
	/* DW2 - flags */
	feature_ctrl.u2IfApplyBssCheckLengthToken_0_to_16_CtrlFlag = 0;
	/* DW5 - ctrl values */
	...
	if (ph_dev && ph_dev->bw_en_mask)
		feature_ctrl.u2EnbwCtrlValue = 1; /* 1'b */
	else
	...
	if (ph_dev && (band_idx < CFG_WIFI_RAM_BAND_NUM)) {
	...
	if (ph_dev) {
		for (i = 0; i < CFG_WIFI_RAM_BAND_NUM; i++)
			feature_ctrl.u2BssCheckTimeToken_0_to_16_CtrlValue |= ph_dev->bw_group_at_on_mask[i];

		for (i = 0; i < CFG_WIFI_RAM_BAND_NUM; i++)
			feature_ctrl.u2BssCheckLengthToken_0_to_16_CtrlValue |= ph_dev->bw_group_bw_on_mask[i];
	}
	/* DW6 - ctrl values */
	for (i = 0; i < VOW_MAX_GROUP_NUM; i++)
	...
	case ENUM_RX_AT_BITWISE_CTRL:
		...
		case ENUM_RX_AT_BITWISE_SUB_TYPE_AIRTIME_CLR: /* clear all RX airtime counters */
			...
			MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
				"(cmd = 0x%x, subcmd = 0x%x, value = 0x%x)\n",
				cmd, subcmd,
				rx_at_ctrl.rRxAtGeneralCtrl.rRxAtBitWiseSubCtrl.fgRxAirTimeClrEn);
			break;
...
INT vow_set_backoff_time(PRTMP_ADAPTER pad, UINT8 target)
{
	...
	case ENUM_RX_AT_WMM_GROUP_3:
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
			"(cmd = 0x%x, subcmd = 0x%x, group = 0x%x",
			rx_at_ctrl.u4CtrlFieldID, rx_at_ctrl.u4CtrlSubFieldID, target);
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO,
			", BK = 0x%x, BE = 0x%x, VI = 0x%x, VO = 0x%x)\n",
			pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_BK],
			pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_BE],
			pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_VI],
			pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_VO]);
...
void vow_dump_umac_CRs(PRTMP_ADAPTER pad)
{
	...
	for (i = 0x8340; i < 0x83c0; i += 4) {
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "0x%0x -> 0x%0x\n", i, val);
	}
}
/* ---------------------- end -------------------------------*/
...
static void vow_init_feature_by_chip(PRTMP_ADAPTER pAd)
{
	...
	pAd->vow_gen.VOW_BWC_GROUP_MIN = UMAC_BWC_GROUP_MIN;
	pAd->vow_gen.VOW_BWC_GROUP_MAX = UMAC_BWC_GROUP_MAX;

	if (pAd->vow_gen.VOW_GEN == VOW_GEN_FALCON) {
		pAd->vow_gen.VOW_BWC_GROUP_MIN = UMAC_FALCON_BWC_GROUP_MIN;
		pAd->vow_gen.VOW_BWC_GROUP_MAX = UMAC_FALCON_BWC_GROUP_MAX;
	}
}
...
void vow_variable_reset(PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < VOW_MAX_GROUP_NUM; i++) {
		pAd->vow_bss_cfg[i].min_rate = 0; /* Mbps */
		pAd->vow_bss_cfg[i].max_rate = 20000; /* Mbps */
		pAd->vow_bss_cfg[i].min_airtime_ratio = 0; /* % */
		pAd->vow_bss_cfg[i].max_airtime_ratio = 100; /* % */
...
VOID vow_init(PRTMP_ADAPTER pad)
{
	...
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_feature_all(pad);
	else
#endif /* WIFI_UNIFIED_COMMAND */
	...
	if (idx < CFG_WIFI_RAM_BAND_NUM) {
		if (pad->vow_cfg.en_bw_ctrl)
			ph_dev->bw_en_mask |= (1 << idx);
		else
			ph_dev->bw_en_mask &= ~(1 << idx);
	}
...
{
#ifdef WIFI_UNIFIED_COMMAND
	unsigned long dbg_mode = 0;
	RTMP_CHIP_CAP *pChipCap = hc_get_chip_cap(pad->hdev_ctrl);
	UINT32 ret;

	if (arg) {
		if (kstrtoul(arg, 16, &dbg_mode))
			return FALSE;

		if (pChipCap->uni_cmd_support) {
			ret = uni_cmd_vow_set_drr_dbg(pad, dbg_mode);
			MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"%s: set drr dbg_mode:0x%lx\n", __func__, dbg_mode);
			if (ret) {
				MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
...
VOID vow_init_sta(PRTMP_ADAPTER pad)
{
...
#ifdef WIFI_UNIFIED_COMMAND
		if (pChipCap->uni_cmd_support)
			ret = uni_cmd_vow_set_sta_DWRR_max_time(pad);
		else
#endif /* WIFI_UNIFIED_COMMAND */
			ret = vow_set_sta_DWRR_max_time(pad);

	/* WM's DRR Table is same as WTBL , need to skip for test mode WTBL only 64 */
	if (ph_dev && ph_dev->TestModeEn)
		return;
	...
	if (pad->vow_gen.VOW_GEN < VOW_GEN_BESRA) {
...
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		uni_cmd_vow_set_group(pad, VOW_ALL_GROUP, ENUM_BSSGROUP_CTRL_BW_GROUP_QUANTUM_ALL);
	else
#endif /* WIFI_UNIFIED_COMMAND */
...
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		uni_cmd_vow_set_group(pad, VOW_ALL_GROUP, ENUM_BSSGROUP_CTRL_ALL_ITEM_FOR_ALL_GROUP);
	else
#endif /* WIFI_UNIFIED_COMMAND */
...
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		uni_cmd_vow_set_group_DWRR_max_time(pad);
	else
#endif
...
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support)
				ret = uni_cmd_vow_set_at_estimator_group(pad, ENUM_AT_PROC_EST_GROUP_RATIO_CTRL, i);
			else
#endif /* WIFI_UNIFIED_COMMAND */
				ret = vow_set_at_estimator_group(pad, ENUM_AT_PROC_EST_GROUP_RATIO_CTRL, i);
	...
	if (pBssEdca) {
		...
		for (ac_idx = 0; ac_idx < WMM_NUM_OF_AC; ac_idx++) {
			...
			pad->vow_rx_time_cfg.wmm_backoff[wmm_idx][ac_idx] =
				...
				|| wlan_config_get_ch_band(wdev) == CMD_CH_BAND_6G) ? SIFS_TIME_5G : SIFS_TIME_24G) +
...
VOID vow_mbss_init(PRTMP_ADAPTER pad, struct wifi_dev *wdev)
{
	...
	if (wdev && (wdev->wdev_type != WDEV_TYPE_REPEATER)) {
		...
		if (wdev->func_idx < 0 || wdev->func_idx >= VOW_GROUP_TABLE_MAX) {
			MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR, "%s:Failed.\n", __func__);
			return;
		}

		/* configure BCMC entry */
		if ((pad->vow_gen.VOW_FEATURE & VOW_FEATURE_BWCG) && (pad->vow_cfg.en_bw_ctrl) &&
			(pad->bss_group.bw_group_idx[wdev->func_idx] < VOW_GROUP_TABLE_MAX))
...
VOID vow_mbss_wmm_map(PRTMP_ADAPTER pad, struct wifi_dev *wdev)
{
	...
	if (wdev) {
		...
		if (wdev->func_idx < 0 || wdev->func_idx >= VOW_MAX_GROUP_NUM) {
			MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR, "%s:Failed.\n", __func__);
			return;
		}
...
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support)
				ret = uni_cmd_vow_set_sta(pad, sta, ENUM_VOW_DRR_CTRL_FIELD_STA_BSS_GROUP);
			else
#endif /* WIFI_UNIFIED_COMMAND */
...
INT mtk_cfg80211_set_vow_bw_en(IN PRTMP_ADAPTER pad, IN UCHAR vow_bw_en)
{
	INT ret;
	UINT32 idx;
#ifdef WIFI_UNIFIED_COMMAND
	...
	pad->vow_cfg.en_bw_ctrl = vow_bw_en;
	...
	pad->vow_at_est.at_estimator_en = (vow_bw_en > 0) ? TRUE : FALSE;
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		uni_cmd_vow_set_at_estimator(pad, ENUM_AT_PROC_EST_FEATURE_CTRL);
	else
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_atf_en(
	IN PRTMP_ADAPTER pad,
	IN UCHAR vow_atf_en)
{
	INT ret;
	...
	RTMP_CHIP_CAP * pChipCap = hc_get_chip_cap(pad->hdev_ctrl);
#endif /* WIFI_UNIFIED_COMMAND */
	pad->vow_cfg.en_airtime_fairness = vow_atf_en;
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_feature_all(pad);
	else
	...
	if (arg) {
		...
		if ((rv > 0)) {
			...
			if (pad->vow_cfg.mcli_schedule_en)
				MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"mcli schedule code enable.\n");
			else
				MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"mcli schedule code disable.\n");
		} else
			return FALSE;
	} else
		return FALSE;
	...
	IN	RTMP_STRING * arg)
{
	...
	if (if_type == INT_MAIN || if_type == INT_MBSSID) {
		...
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_VOW, DBG_LVL_INFO, "Unexpected if_type\n");
		return FALSE;
	}
	...
	if (arg) {
		...
		case MCLI_TCP_CNT_TH:
			...
			MTWF_PRINT("tcp_cnt_th=%u\n", pad->vow_cfg.mcli_sch_cfg.tcp_cnt_th);
			break;
		case MCLI_DL_WRR_ENABLE:
			pad->vow_cfg.mcli_sch_cfg.dl_wrr_en = (op1 > 0);
			MTWF_PRINT("band%u dl_wrr_en=%u\n", bandidx,
				pad->vow_cfg.mcli_sch_cfg.dl_wrr_en);
			break;
		case MCLI_CWMIN_CWMAX_PARAM:
			if (op1 == VOW_MCLI_DL_MODE) {
				pad->vow_cfg.mcli_sch_cfg.cwmin[VOW_MCLI_DL_MODE] = op2;
				pad->vow_cfg.mcli_sch_cfg.cwmax[VOW_MCLI_DL_MODE] = op3;
				MTWF_PRINT("band%u, DL mode, cwmin:cwmax=%u:%u\n", bandidx,
					pad->vow_cfg.mcli_sch_cfg.cwmin[VOW_MCLI_DL_MODE],
					pad->vow_cfg.mcli_sch_cfg.cwmax[VOW_MCLI_DL_MODE]);
			} else if (op1 == VOW_MCLI_UL_MODE) {
				pad->vow_cfg.mcli_sch_cfg.cwmin[VOW_MCLI_UL_MODE] = op2;
				pad->vow_cfg.mcli_sch_cfg.cwmax[VOW_MCLI_UL_MODE] = op3;
				MTWF_PRINT("band%u, UL mode, cwmin:cwmax=%u:%u\n", bandidx,
					pad->vow_cfg.mcli_sch_cfg.cwmin[VOW_MCLI_UL_MODE],
					pad->vow_cfg.mcli_sch_cfg.cwmax[VOW_MCLI_UL_MODE]);
			} else
				MTWF_PRINT("mode%u not support now\n", op1);
			break;
		default:
			MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"Invalid parameter combination, cmd:%d, op:%d\n", cmd, op1);
			break;
		}
	} else
		return FALSE;
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 1) && (group < VOW_MAX_GROUP_NUM)) {
			...
			pad->vow_bss_cfg[group].group_table_idx = group;
			pad->bss_group.bw_group_idx[group] = group;
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support)
				ret = uni_cmd_vow_set_feature_all(pad);
			else
			...
			if (idx < CFG_WIFI_RAM_BAND_NUM) {
				if (val)
					ph_dev->bw_group_at_on_mask[idx] |= (1 << group);
				else
					ph_dev->bw_group_at_on_mask[idx] &= ~(1 << group);
			}
		} else
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_atc_en(
	IN  PRTMP_ADAPTER pad,
	IN  UINT32 group, IN UINT32 en)
{
	INT ret, idx;
#ifdef WIFI_UNIFIED_COMMAND
	RTMP_CHIP_CAP * pChipCap = hc_get_chip_cap(pad->hdev_ctrl);
	...
	pad->vow_bss_cfg[group].at_on = en;
	pad->vow_bss_cfg[group].group_table_idx = group;
	pad->bss_group.bw_group_idx[group] = group;
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
	...
	if (idx >= 0 && idx < CFG_WIFI_RAM_BAND_NUM) {
		if (en)
			ph_dev->bw_group_at_on_mask[idx] |= (1 << group);
		else
			ph_dev->bw_group_at_on_mask[idx] &= ~(1 << group);
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 1) && (group < VOW_MAX_GROUP_NUM)) {
			...
			if (idx < CFG_WIFI_RAM_BAND_NUM) {
				if (val)
					ph_dev->bw_group_bw_on_mask[idx] |= (1 << group);
				else
					ph_dev->bw_group_bw_on_mask[idx] &= ~(1 << group);
			}
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_bw_ctrl_en(
	IN PRTMP_ADAPTER pad,
	IN UINT32 group, IN UINT32 en)
{
	INT ret, idx;
	...
	pad->vow_bss_cfg[group].bw_on = en;
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_feature_all(pad);
	else
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 1) && (IS_WCID_VALID(pad, sta)) && (val < 4)) {
			...
			MTWF_DBG(pad, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"sta %d W AC change rule %d.\n",
				sta, pad->vow_sta_cfg[sta].ac_change_rule);
		} else
			return FALSE;
	} else
		return FALSE;
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_min_rate(
	IN PRTMP_ADAPTER pad,
	IN UINT32 group,
	IN UINT32 rate)
{
	...
	pad->vow_bss_cfg[group].min_rate = rate;
	pad->vow_bss_cfg[group].min_rate_token = vow_convert_rate_token(pad, VOW_MIN, group);
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MIN_RATE_TOKEN_CFG_ITEM);
	else
#endif /* WIFI_UNIFIED_COMMAND */
		ret = vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MIN_RATE_TOKEN_CFG_ITEM);
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_max_rate(
	IN PRTMP_ADAPTER pad,
	IN UINT32 group,
	IN UINT32 rate)
{
	...
	pad->vow_bss_cfg[group].max_rate = rate;
	pad->vow_bss_cfg[group].max_rate_token = vow_convert_rate_token(pad, VOW_MAX, group);
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MAX_RATE_TOKEN_CFG_ITEM);
	else
#endif /* WIFI_UNIFIED_COMMAND */
		ret = vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MAX_RATE_TOKEN_CFG_ITEM);
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_min_ratio(
	IN PRTMP_ADAPTER pad,
	IN UINT32 group,
	IN UINT32 ratio)
{
	...
	pad->vow_bss_cfg[group].min_airtime_ratio = ratio;
	pad->vow_bss_cfg[group].min_airtime_token = vow_convert_airtime_token(pad, VOW_MIN, group);
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MIN_AIRTIME_TOKEN_CFG_ITEM);
	else
#endif /* WIFI_UNIFIED_COMMAND */
		ret = vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MIN_AIRTIME_TOKEN_CFG_ITEM);
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vow_max_ratio(
	IN PRTMP_ADAPTER pad,
	IN UINT32 group,
	IN UINT32 ratio)
{
	...
	pad->vow_bss_cfg[group].max_airtime_ratio = ratio;
	pad->vow_bss_cfg[group].max_airtime_token = vow_convert_airtime_token(pad, VOW_MAX, group);
#ifdef WIFI_UNIFIED_COMMAND
	if (pChipCap->uni_cmd_support)
		ret = uni_cmd_vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MAX_AIRTIME_TOKEN_CFG_ITEM);
	else
#endif /* WIFI_UNIFIED_COMMAND */
		ret = vow_set_group(pad, group, ENUM_BSSGROUP_CTRL_MAX_AIRTIME_TOKEN_CFG_ITEM);
...
#endif /* WIFI_UNIFIED_COMMAND */
				ret = vow_set_rx_airtime(pAd, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_AIRTIME_EN);
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR, "set %u.\n", val);
...
)
{
	...
	if ((u2StaID > hc_get_chip_wtbl_max_num(pvow_pad)) ||
		(ucAcId >= 4) ||
		((fgChargeMode != TRUE) && (fgChargeMode != FALSE)))
		return FALSE;
...
#ifdef WIFI_UNIFIED_COMMAND
			if (pChipCap->uni_cmd_support)
				uni_cmd_vow_set_feature_all(pAd);
			else
#endif /* WIFI_UNIFIED_COMMAND */
	...
	IN  RTMP_STRING * arg)
{
	...
	if ((opmode == 0x22) || (opmode == 0x23)) {
		...
		start = 0;
		end = hc_get_chip_wtbl_max_num(pAd);
	}
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (IS_WCID_VALID(pAd, sta))) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"Sta%d deficit token: ac0 %d(0x%08X), ac1 %d(0x%08X)",
				sta, val1_sign_ext, val1, val2_sign_ext, val2);
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				", ac2 %d(0x%08X), ac3 %d(0x%08X)\n",
				val3_sign_ext, val3, val4_sign_ext, val4);
		} else
			return FALSE;
	} else
		return FALSE;
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (bss < VOW_MAX_GROUP_NUM)) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"Airtime ratio --> %u/%u %%\n",
				pAd->vow_bss_cfg[bss].max_airtime_ratio,
				pAd->vow_bss_cfg[bss].min_airtime_ratio);
			/* Rate token */
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
	...
	IN  RTMP_STRING * arg)
{
	...
	UINT32 band_idx = hc_get_hw_band_idx(pAd);
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
		"band(%d) near_far_txop_running = %d\n",
		 band_idx, pAd->txop_ctl.near_far_txop_running);
	...
	IN  RTMP_STRING * arg)
{
	...
	MTWF_PRINT("vow_bss_conf = <group>\nvow_all_bss_conf\nvow_dump_sta (raw)\n");
	MTWF_PRINT("vow_dump_bss_bitmap (raw)\nvow_dump_bss (raw)\nvow_dump_vow (raw)\n");
	MTWF_PRINT("vow_show_sta_dtoken = <wlanidx> DWRR\nvow_show_bss_dtoken = <group> DWRR\n");
	MTWF_PRINT("vow_show_bss_atoken = <group> airtime\n");
	MTWF_PRINT("vow_show_bss_ltoken = <group> length\n");
	MTWF_PRINT("vow_sch_ctrl(type->0:algo,1:hw, policy->0:SRR,1:WRR)\n");
	return TRUE;
}
/* airtime estimator */
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg && vow_watf_is_enabled(pAd)) {
		...
		if (rv > 1) {
			...
			os_alloc_mem(NULL, (UCHAR **)&pwatf_string, size);
			...
			case 0: {
				...
				ret = snprintf(pwatf_string, size, "%d-%d", 0, pAd->vow_watf_q_lv0);
				set_vow_sta_dwrr_quantum(pAd, pwatf_string);
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"vow_watf_q_lv0 is set to %d\n", pAd->vow_watf_q_lv0);
				break;
			}

			case 1: {
				pAd->vow_watf_q_lv1 =  val;
				ret = snprintf(pwatf_string, size, "%d-%d", 1, pAd->vow_watf_q_lv1);
				set_vow_sta_dwrr_quantum(pAd, pwatf_string);
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"vow_watf_q_lv1 is set to %d\n", pAd->vow_watf_q_lv1);
				break;
			}

			case 2: {
				pAd->vow_watf_q_lv2 =  val;
				ret = snprintf(pwatf_string, size, "%d-%d", 2, pAd->vow_watf_q_lv2);
				set_vow_sta_dwrr_quantum(pAd, pwatf_string);
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"vow_watf_q_lv2 is set to %d\n", pAd->vow_watf_q_lv2);
				break;
			}

			case 3: {
				pAd->vow_watf_q_lv3 =  val;
				ret = snprintf(pwatf_string, size, "%d-%d", 3, pAd->vow_watf_q_lv3);
				set_vow_sta_dwrr_quantum(pAd, pwatf_string);
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"vow_watf_q_lv3 is set to %d\n", pAd->vow_watf_q_lv3);
				break;
			}
	...
	if (arg) {
		...
		if ((rv == 7) && (level < VOW_WATF_LEVEL_NUM)) {
			...
			for (i = 0; i < VOW_WATF_LEVEL_NUM; i++) {
				for (j = 0; j < ptmpwatf->Num; j++)
					if (memcmp(ptmpwatf->Entry[j].Addr, &macAddr, MAC_ADDR_LEN) == 0) {
						isDuplicate = TRUE;
						MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
							"This MAC Address "MACSTR" is duplicate.\n",
							MAC2STR(macAddr));
						break;
					}
			...
			if (!isDuplicate) {
				...
				NdisMoveMemory(pwatf->Entry[pwatf->Num++].Addr,
							   &macAddr, MAC_ADDR_LEN);
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
					"The entry Level %d - " MACSTR " is set complete!\n",
				...
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_NOTICE,
						 "Add WATF entry " MACSTR "\n", MAC2STR(macAddr));
				set_vow_watf_sta_dwrr(pAd, &pEntry->Addr[0], pEntry->wcid);

#ifdef WIFI_UNIFIED_COMMAND
				if (pChipCap->uni_cmd_support)
					ret = uni_cmd_vow_set_sta(
							pAd,
							pEntry->wcid,
			...
			MTWF_PRINT("Wrong format, vow_watf_add_entry=");
			MTWF_PRINT("[Level]-[Addr]:[Addr]:[Addr]:[Addr]:[Addr]:[Addr]\n");
			MTWF_PRINT("[Level] should be among 0 to 3 !\n");
			return FALSE;
		}
	} else
		return FALSE;
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv == 7) && (level < VOW_WATF_LEVEL_NUM)) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"Wrong format, vow_watf_add_entry=");
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"[Level]-[Addr]:[Addr]:[Addr]:[Addr]:[Addr]:[Addr]\n");
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_ERROR,
				"[Level] should be among 0 to 3 !\n");
			return FALSE;
		}
	...
	IN  RTMP_STRING * arg)
{
	...
	MTWF_PRINT("bw_ctrl enable mask : 0x%x\n", ph_dev->bw_en_mask);

	for (i = 0; i < CFG_WIFI_RAM_BAND_NUM; i++)
		MTWF_PRINT("bw_ctrl band%u group at mask : 0x%x\n", i, ph_dev->bw_group_at_on_mask[i]);

	for (i = 0; i < CFG_WIFI_RAM_BAND_NUM; i++)
		MTWF_PRINT("bw_ctrl band%u group bw mask : 0x%x\n", i, ph_dev->bw_group_bw_on_mask[i]);
	...
	IN  PRTMP_ADAPTER pAd)
{
	...
	if (pAd->vow_show_en == 1) {
		...
		vow_ampdu_cnt += cnt;
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_VOW, DBG_LVL_NOTICE, "AMPDU[Band0] count %d\n", cnt);
		...
		if (pAd->vow_gen.VOW_GEN >= VOW_GEN_FALCON) {
			...
			chip_dbg->show_txcmdq_info(pAd, ple_txcmd_stat, 0);
		} else {
