...
static NTSTATUS WHCStaProbeReq(IN PRTMP_ADAPTER pAd, ...)
{
	...
	pProbeReq->cap = 0;
	if (HAS_HT_CAPS_EXIST(pProbeReqParam->ie_list.cmm_ies.ie_exists)) {
...
#ifdef DOT11_VHT_AC
	if (HAS_VHT_CAPS_EXIST(pProbeReqParam->ie_list.cmm_ies.ie_exists)) {
...
static NTSTATUS WHCExtUpLinkStatus(IN PRTMP_ADAPTER pAd, ...)
{
	...
	pExtUplink->channel = wdev->channel;
...
static NTSTATUS WHCStaTimeout(IN PRTMP_ADAPTER pAd, ...)
{
	...
	pStaTimeout->channel = wdev->channel;
...
EventHdlr GetEventNotiferHook(IN UINT32 EventID)
{
	...
	if (EventID > CMDTHREAD_END_CMD_ID) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WHC, DBG_LVL_ERROR,
			"Event ID(%d) is out of boundary.\n", EventID);
		return NULL;
	}
	...
	if (Handler == NULL)
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_WHC, DBG_LVL_ERROR,
			"No corresponding EventHdlr for this Event ID(%d).\n",  EventID);
...
INT SetCustomVIEProc(PRTMP_ADAPTER pAd, PSTRING arg)
{
...
#endif /* APCLI_SUPPORT */
	default:
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WHC, DBG_LVL_ERROR,
	...
	if (!wdev) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_WHC, DBG_LVL_ERROR, "wdev is NULL!\n");
		return FALSE;
	}
...
{
	INT i = 0, j = 0;
	struct wifi_dev *p2GWdev = NULL, *p5GWdev = NULL;
	PNET_DEV if_dev = NULL;
	UINT32 ChLoadDetectPeriod = 10;
	UCHAR concurrent_bands = 0;
	BOOLEAN IsRun2G = FALSE, IsRun5G = FALSE;
	struct EventNotifierCfg *pEventNotifyCfg = &pAd->ApCfg.EventNotifyCfg;
	struct drvevnt_channel_load_report *pChload = &pEventNotifyCfg->Chload;

	if (!pEventNotifyCfg->bChLoadDetect)
		return;

	IsRun2G = HcIsRfRun(pAd, RFIC_24GHZ);
	IsRun5G = HcIsRfRun(pAd, RFIC_5GHZ);
	concurrent_bands = PD_GET_BAND_NUM(pAd->physical_dev);
	if (pEventNotifyCfg->ChLoadDetectPeriod > 0)
		ChLoadDetectPeriod = pEventNotifyCfg->ChLoadDetectPeriod;

	if (pEventNotifyCfg->ChLoadRound == 0)
		NdisZeroMemory(pChload, sizeof(*pChload));

	pEventNotifyCfg->ChLoadRound++;
}
