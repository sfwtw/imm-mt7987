	...
	IN PVOID SystemSpecific3)
{
	...
	if (actionBandIdx < MAX_NUM_OF_INF)
		...
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
			"Invalid bandIdx for multi-interface!\n");
		return;
	}
#else
	pActionAd = pAd;
	...
	BOOLEAN from_apcli)
{
	...
	(void)snprintf(uuidTmpStr, sizeof(uuidTmpStr),
	...
	strlcpy((RTMP_STRING *)uuidAscStr, uuidTmpStr, UUID_LEN_STR);
...
#endif /* CON_WPS_AP_SAME_UUID */
#endif /* MULTI_INF_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "The UUID Hex string is:");

	for (i = 0; i < 16; i++)
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "%02x", (uuidHexStr[i] & 0xff));

	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "\n");
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "The UUID ASCII string is:%s!\n", uuidAscStr);
	return 0;
}
	...
	IN PVOID SystemSpecific3)
{
	...
	*/
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "UPnP StateMachine TimeOut(State=%d!)\n", pWscControl->WscState);
	...
	if (
		...
		(0)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"pEntry maybe gone or already received M2 Packet!\n");
		goto done;
	}

	if (pWscControl->M2DACKBalance != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"waiting for M2DACK balance, extend the time!\n");
		...
		if (CurOpMode == AP_MODE) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"send EAP-Fail to wireless Station!\n");
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscUPnPMsgTimeOutAction\n");
...
#define WSC_UPNP_TIMER_PENDIND_WAIT	2000
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "UPnPMsgTimer Pending......\n");
	} else {
		int dataLen;
		UCHAR *pWscData;
	...
	IN  MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscEAPOLStartAction\n");
	wdev = Elem->wdev;
	if (wdev == NULL) {
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "wdev is NULL.\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscEAPOLStartAction\n");
		return;
	}
	...
	if (pEntry == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "pEntry is NULL.\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscEAPOLStartAction\n");
		return;
	}
...
#endif /* CON_WPS */
#endif /* CONFIG_AP_SUPPORT */
	OS_SEM_LOCK(&pWpsCtrl->WscPeerListSemLock);
	WscInsertPeerEntryByMAC(&pWpsCtrl->WscPeerList, pEntry->Addr);
	OS_SEM_UNLOCK(&pWpsCtrl->WscPeerListSemLock);
	...
	*/
	if (pWpsCtrl->WscPeerList.size != 0) {
		...
		if (NdisEqualMemory(pEntry->Addr, pWscPeer->mac_addr, MAC_ADDR_LEN) == FALSE) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "This is not first WSC peer, ignore this EAPOL_Start!\n");
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscState = %d\n", pWpsCtrl->WscState);
	...
	if ((pEntry->Receive_EapolStart_EapRspId == 0) ||
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEAPOLStartAction - receive EAPOL-Start from "MACSTR"\n",
				 MAC2STR(pEntry->Addr));
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEAPOLStartAction(ra%d) - send EAP-Req(Id) to "MACSTR"\n",
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Ignore EAPOL-Start.\n");
	...
	IN  MLME_QUEUE_ELEM * Elem)
{
	...
	if (MsgType == WSC_MSG_EAP_REG_RSP_ID) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEAPAction: Rx Identity\n");
		...
		if (bUPnPMsg) {
				...
				if ((pAdapter->CommonCfg.WscStaPbcProbeInfo.WscPBCStaProbeCount[current_band] == 1) &&
					...
					for (bss_index = 0; bss_index < pAdapter->ApCfg.BssidNum; bss_index++) {
						if ((current_band == HcGetBandByWdev(&pAdapter->ApCfg.MBSSID[bss_index].wdev)) &&
							...
							MTWF_DBG(NULL,
							DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
							"found pAd->ApCfg.MBSSID[%d] WPS on\n",
							bss_index);
							break;
						}
					}
					...
					if (bss_index < pAdapter->ApCfg.BssidNum) {
						MTWF_DBG(pAdapter,
							DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
							"BSS[%d] WPS on, PBC Overlap detected\n",
							bss_index);
						pAdapter->CommonCfg.WscPBCOverlap = TRUE;
					} else {
						MTWF_DBG(pAdapter,
							DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
							"BSS[%d] WPS off, PBC Overlap is invalid\n",
							bss_index);
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE) &&
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEAPAction: PBC Session Overlap!\n");
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Receive EAP_Req/Identity from WPS AP or WCN\n");
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Receive EAP_Req(Wsc_Start) from WPS AP\n");
		...
		if (pWscControl->WscConfMode == WSC_ENROLLEE) {
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Ignore EAP_Req(Wsc_Start) from WPS AP\n");
	} else if (MsgType == WSC_MSG_EAP_FAIL) {
		/* Receive EAP_Fail from WPS AP */
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Receive EAP_Fail from WPS AP\n");
		...
		*/
		...
		if (((pWscControl->WscConfMode & WSC_PROXY) != 0) && (!bUPnPMsg) && (CurOpMode == AP_MODE)) {
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscTrigger is FALSE, ignore EAP M1.\n");
			goto out;
		}
			...
			if ((rv == 0) && (CurOpMode == STA_MODE)) {
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "(Line:%d)Ignore EAP M1.\n", __LINE__);
				goto out;
			}
		}
		...
		if (((pWscControl->WscConfMode & WSC_REGISTRAR) != 0) &&
			...
			if (!bUPnPMsg && pWscControl->WscUPnPNodeInfo.bUPnPInProgress) {
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "UPnP Registrar is working now, ignore EAP Messages.\n");
				goto out;
			} else {
				pWscControl->WscActionMode = WSC_REGISTRAR;
		...
		if (((pWscControl->WscConfMode & WSC_ENROLLEE) != 0) &&
			...
			if (MsgType == WSC_MSG_M2) {
				...
				if (bReadOwnPIN) {
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "(%d) WscEnrolleePinCode: %08u\n", bReadOwnPIN, pWscControl->WscEnrolleePinCode);
				} else
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscPinCode: %08u\n", pWscControl->WscPinCode);
			}
		...
		if (!bUPnPMsg &&
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Receive NACK from WPS client.\n");
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Unsupported Msg Type (%02X)\n", MsgType);
		pWscControl->WscStatus = STATUS_WSC_FAIL;
		pWscControl->RegData.SelfInfo.ConfigError = WSC_ERROR_NO_ERROR;
...
out:
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscEAPAction\n");
}
#ifdef CONFIG_MAP_SUPPORT
		...
		IN MAC_TABLE_ENTRY *pEntry)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
		"SEND Event to WAPP for WSC profile currAPIndex %d\n", CurApIdx);
	...
	wapp_send_wapp_qry_rsp(pAdapter, event);
	...
	if (OpCode > WSC_OPCODE_UPNP_MASK)
		...
		if (pWscControl->WscState != WSC_STATE_CONFIGURED) {
			...
			if (pWscControl->bWscFragment && (DataLen > pWscControl->WscFragSize)) {
				...
				if (pWscControl->WscFragSize <= MaxWscDataLen) {
					...
					MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
						"Not have enough buffer size for WscData!\n");
			}
...
Fail:
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapEnrolleeAction : rv = %d\n", rv);
	...
	if (rv) {
		...
		if (CurOpMode == STA_MODE) {
			...
			if (pWscControl->WscSsid.SsidLength) {
				...
				if (pWscControl->WscSsid.SsidLength <= ARRAY_SIZE(pStaCfg->MlmeAux.AutoReconnectSsid))
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
						"Not have enough buffer size for pStaCfg->MlmeAux.AutoReconnectSsid[0]!\n");
			} else
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction Enter!\n");
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction():pEntry=%p, ElemMsgType=%ld, MsgType=%d!\n", pEntry, Elem->MsgType, MsgType);

	if ((pWscControl->WscActionMode != WSC_PROXY) ||
		((Elem->MsgType == WSC_EAPOL_PACKET_MSG) && (pEntry == NULL))) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "EarlyCheckFailed: gWscActionMode=%d, pEntry=%p!\n", pWscControl->WscActionMode, pEntry);
		goto Fail;
	}
	...
	if (WscData == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscData Allocate failed!\n");
		goto Fail;
	}
	...
	if (Elem->MsgType == WSC_EAPOL_UPNP_MSG) {
		...
		case WSC_MSG_M6:
		case WSC_MSG_M8:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: Rx WscMsg(%d) from UPnP, eventID=0x%x!\n", MsgType, reqID);
		...
		case WSC_MSG_M2D:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: Rx WscMsg M2D(%d) from UPnP, eventID=0x%x!\n", MsgType, reqID);
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Recv WscMsg(%d) from UPnP, request EventID=%d! drop it!\n", MsgType, reqID);
			break;
		}
	} else {
		...
		case WSC_MSG_M5:
		case WSC_MSG_M7:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: Rx WscMsg(%d) from EAP\n", MsgType);
		...
		case WSC_MSG_WSC_ACK:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: Rx WSC_ACK from EAP\n");
		...
		case WSC_MSG_WSC_DONE:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: Rx WSC_DONE from EAP\n");
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapApProxyAction: send WSC_DONE to UPnP Registrar!\n");
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Recv WSC Msg(%d) from EAP , it's impossible, drop it!\n", MsgType);
			break;
		}
	}
...
Fail:
	...
	if (sendToUPnP && (bUPnPStatus == FALSE)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Need to send UPnP but bUPnPStatus is false!MsgType=%d, regID=0x%x!\n", MsgType, reqID);
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	PSTA_ADMIN_CONFIG pStaCfg = NULL;
#endif
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapRegistrarAction Enter!\n");
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
	CurOpMode = AP_MODE;
#endif /* CONFIG_AP_SUPPORT */
...
Fail:
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscEapRegistrarAction : rv = %d\n", rv);
...
/* WPS_BandSteering Support */
		...
		if (pAdapter->ApCfg.BandSteering) {
			...
			if (wdev) {
				PBND_STRG_CLI_TABLE table = &pAdapter->ApCfg.BndStrgTable;
				...
				if (table->bEnabled) {
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
						"channel %u wps whitelist cleared, size : %d\n",
...
#endif /* CONFIG_STA_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscTimeOutProcess\n");
}
	...
	IN PVOID SystemSpecific3)
{
	...
	PSTA_ADMIN_CONFIG sta_cfg = NULL;
#endif /* CONFIG_STA_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscEAPOLTimeOutAction\n");
	...
	if ((CurOpMode == AP_MODE) || ADHOC_ON(pAd)) {
		if (pEntry == NULL) {
			...
			*/
			...
			pWscControl->EapolTimerRunning = FALSE;
			NdisZeroMemory(pWscControl->EntryAddr, MAC_ADDR_LEN);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "sta is left.\n");
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscEAPOLTimeOutAction\n");
			return;
		}
	}
	...
	if (pWscControl->EapolTimerPending) {
		RTMPModTimer(&pWscControl->EapolTimer, WSC_EAP_MSG_TIME_OUT);
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "EapolTimer Pending......\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscEAPOLTimeOutAction\n");
		return;
	}
	...
	case WSC_STATE_WAIT_PIN:
		if (pWscControl->WscRetryCount >= 2)
			...
			pWscControl->WscRetryCount++;
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "No PIN CODE, cannot send M2 out!\n");
			RTMPModTimer(&pWscControl->EapolTimer, WSC_EAP_MSG_TIME_OUT);
		}
	...
	IN PVOID SystemSpecific3)
{
...
#endif /* CONFIG_STA_SUPPORT */
	struct wifi_dev *wdev;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> Wsc2MinsTimeOutAction\n");
	...
	if (pWscControl != NULL) {
		...
		if (pAd == NULL) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "pAd is NULL!\n");
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- Wsc2MinsTimeOutAction\n");
			return;
		}
...
#endif /* CONFIG_STA_SUPPORT */
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Wsc2MinsTimerRunning is %s\n",
...
/* WPS_BandSteering Support */
		...
		if ((pStaCfg && STA_STATUS_TEST_FLAG(pStaCfg, fSTA_STATUS_MEDIA_STATE_CONNECTED)) &&
			(pWscControl->WscState == WSC_STATE_OFF) &&
			(pWscControl->WscStatus == STATUS_WSC_CONFIGURED))
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscConnectTimeout --> Connection OK\n");
		else {
			UCHAR WPSLEDStatus;
			...
			if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
				...
				if (pWscControl->WscMode == WSC_PIN_MODE) { /* PIN method. */
					...
					case LED_WPS_SESSION_OVERLAP_DETECTED:
						...
						RTMPSetTimer(&pWscControl->WscSkipTurnOffLEDTimer, WSC_WPS_SKIP_TURN_OFF_LED_TIMEOUT);
						MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
							"Last WPS LED status is LED_WPS_ERROR.\n");
						break;
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscConnectTimeout --> Fail to connect\n");
		}
...
VOID WscInitRegistrarPair(RTMP_ADAPTER *pAd, WSC_CTRL *pWscControl, UCHAR apidx)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscInitRegistrarPair\n");
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	CurOpMode = AP_MODE;
#endif /* CONFIG_AP_SUPPORT */
	...
	if (CurOpMode == AP_MODE) {
		if (!RTMP_TEST_FLAG(pWscControl, 0x04)) {
			...
			(void)snprintf(&pWscControl->RegData.SelfInfo.DeviceName[0],
	...
	if (CurOpMode == AP_MODE) {
		if (apidx < MAX_BEACON_NUM
				&& (pAd->ApCfg.MBSSID[apidx].SsidLen <= ARRAY_SIZE(pWscControl->RegData.SelfInfo.Ssid)))
	...
	if ((CurOpMode == STA_MODE)
			&& (pAd->StaCfg[0].SsidLen <= ARRAY_SIZE(pWscControl->RegData.SelfInfo.Ssid)))
	...
	IN  UCHAR CurOpMode)
{
	...
	/* 1. Send EAP-Rsp Id */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscSendEapReqId\n");
#ifdef CONFIG_AP_SUPPORT
	...
	IN  VOID *wdev_obj)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscSendEapolStart\n");
}
	...
	IN PWSC_CTRL pWscControl)
{
	...
	NdisZeroMemory(Header802_3, sizeof(UCHAR) * 14);
	/* 1. Send EAP-Rsp Id */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscSendEapRspId\n");
	...
	IN  UINT eventID)
{
	...
	wdev = (struct wifi_dev *)pWscControl->wdev;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Into WscUPnPErrHandle, send WSC_OPCODE_UPNP_CTRL with eventID=0x%x!\n", eventID);
...
#define WSC_JOIN_MSG_LEN (MAX_LEN_OF_SSID + MAC_ADDR_LEN + 1)
	...
	strlcpy((RTMP_STRING *) pData, (RTMP_STRING *)ssidStr, MAX_LEN_OF_SSID);
	...
	IN UCHAR CurOpMode)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscSendUPnPMessage\n");
	...
	IN  UCHAR EapType)
{
	...
	/* Send message */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscSendMessage\n");
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
	CurOpMode = AP_MODE;
#endif /* CONFIG_AP_SUPPORT */
	...
	if (pEntry)
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN, "pEntry is NULL\n");
	...
	IN  UCHAR CurOpMode)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscBuildBeaconIE\n");
	/* WSC IE HEader */
	ieHdr.elemId = 221;
	ieHdr.length = 4;
	ieHdr.oui[0] = 0x00;
	...
	IN  UCHAR CurOpMode)
{
	...
	pWpsCtrl = &wdev->WscControl;
	pReg = &pWpsCtrl->RegData;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "apidx = %x\n", apidx);
	/* WSC IE Header */
	ieHdr.elemId = 221;
	ieHdr.length = 4;
	ieHdr.oui[0] = 0x00;
	...
	if (Len > 251)
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Len is overflow!\n");
...
#endif /* CONFIG_AP_SUPPORT */
	MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscSend EapFail\n");
}
	...
	OUT PUCHAR pIeLen)
{
	...
	PWSC_REG_DATA pReg;
	struct wifi_dev *wdev;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscBuildAssocRespIE\n");
	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **)&Data, 512);
...
#endif /* WSC_V2_SUPPORT */
	ieHdr.length = ieHdr.length + Len;
	if (sizeof(WSC_IE_HEADER) <= bufLen)
		NdisMoveMemory(pOutBuf, &ieHdr, sizeof(WSC_IE_HEADER));
	else
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
			"Not have enough buffer size for pOutBuf!\n");
	if (Len <= (bufLen - sizeof(WSC_IE_HEADER))) {
	...
	IN  UCHAR apidx)
{
	...
	if (Length < 4) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscSelectedRegistrar --> Unknown IE\n");
		return;
	}
	...
	while (Length > 4) {
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscSelectedRegistrar --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (wsc_sel_reg == 0x01) {
		...
		if ((AuthorizedMACsLen == 6) &&
			...
			OS_SEM_LOCK(&pWscCtrl->WscPeerListSemLock);
			...
			OS_SEM_UNLOCK(&pWscCtrl->WscPeerListSemLock);
		}
	...
	IN PVOID SystemSpecific3)
{
	...
	if (pAdapter != NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscProfileRetryTimeout:: WSC profile retry timeout index: %d\n", pWscControl->WscProfile.ApplyProfileIdx);
		...
		if ((CurOpMode == AP_MODE)
			...
			for (i = 0; VALID_UCAST_ENTRY_WCID(pAdapter, i); i++) {
				PMAC_TABLE_ENTRY pEntry = entry_get(pAdapter, i);
				STA_TR_ENTRY *tr_entry = tr_entry_get(pAdapter, i);
		...
		if ((CurOpMode == STA_MODE)
			&& INFRA_ON(&pAdapter->StaCfg[0])
			&& (pAdapter->StaCfg[0].wdev.bLinkUpDone)) {
		...
		if (bReConnect) {
			...
			/*#endif */
			RtmpOsTaskWakeUp(&(pAdapter->wscTask));
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscProfileRetryTimeout:: WSC profile retry index: %d\n", pWscControl->WscProfile.ApplyProfileIdx);
		}
	...
	IN  PMAC_TABLE_ENTRY pEntry)
{
	...
	wdev = pEntry->wdev;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscInformFromWPA\n");
	...
	if (MAC_ADDR_EQUAL(pEntry->Addr, wdev->WscControl.EntryAddr)) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Reset EntryIfIdx to %d\n", WSC_INIT_ENTRY_APIDX);
	}
...
/* WPS_BandSteering Support */
	...
	if (CurOpMode == STA_MODE) {
		UINT8 ssidlen = MAX_LEN_OF_SSID;

		if (((UINT8)pStaCfg->MlmeAux.SsidLen) < MAX_LEN_OF_SSID)
			ssidlen = ((UINT8)pStaCfg->MlmeAux.SsidLen);

		pStaCfg->MlmeAux.AutoReconnectSsidLen = pStaCfg->MlmeAux.SsidLen;
		NdisZeroMemory(&pStaCfg->MlmeAux.AutoReconnectSsid[0], MAX_LEN_OF_SSID);
		NdisMoveMemory(&pStaCfg->MlmeAux.AutoReconnectSsid[0], &pStaCfg->MlmeAux.Ssid[0], ssidlen);
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (bFromApCli) {
		...
		if (pOpposAd != NULL) {
			for (loop = 0; loop < MAX_BEACON_NUM; loop++) {
				...
				if ((wdev) &&
					...
					WscBuildProbeRespIE(pOpposAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, FALSE, 0, 0,
										loop, NULL, 0, AP_MODE);
					UpdateBeaconHandler(pOpposAd, wdev, BCN_REASON(BCN_UPDATE_IE_CHG));
...
#endif
#ifdef MAP_R6
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_WPA2PSK | WSC_AUTHTYPE_DPP | WSC_AUTHTYPE_SAE):
		return "DPPWPA3PSKWPA2PSKSAE-EXT";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_WPA2PSK | WSC_AUTHTYPE_DPP):
		return "DPPWPA2PSKSAE-EXT";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_SAE | WSC_AUTHTYPE_DPP):
		return "DPPWPA3PSKSAE-EXT";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_SAE | WSC_AUTHTYPE_WPA2PSK):
		return "SAE-EXTWPA2PSKWPA3PSK";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_WPA2PSK):
		return "SAE-EXTWPA2PSK";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_SAE):
		return "SAE-EXTWPA3PSK";
	case (WSC_AUTHTYPE_SAE_AKM24_D | WSC_AUTHTYPE_DPP):
		return "SAE-EXTDPP";
	case WSC_AUTHTYPE_SAE_AKM24_D:
		return "SAE-EXT";
#endif
	...
	IN  BOOLEAN bEnrollee)
{
	...
	PSTA_ADMIN_CONFIG pStaCfg = &pAd->StaCfg[0];
#endif
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscWriteConfToPortCfg\n");
#ifdef CONFIG_STA_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
	...
	if (bEnrollee || (CurOpMode == AP_MODE)) {
		if (CurOpMode == AP_MODE) {
			UINT len = MAX_LEN_OF_SSID;

			if (pCredential->SSID.SsidLength < len)
				len = pCredential->SSID.SsidLength;
			if (ARRAY_SIZE(pCredential->SSID.Ssid) < len)
				len = ARRAY_SIZE(pCredential->SSID.Ssid);

			NdisZeroMemory(pAd->ApCfg.MBSSID[CurApIdx].Ssid, MAX_LEN_OF_SSID);
			NdisMoveMemory(pAd->ApCfg.MBSSID[CurApIdx].Ssid, pCredential->SSID.Ssid, len);
			pAd->ApCfg.MBSSID[CurApIdx].SsidLen = len;
		}
		...
		if (CurOpMode == STA_MODE) {
			UINT AutoConnSSIDlen = NDIS_802_11_LENGTH_SSID;
			UINT MLMESSIDLen = NDIS_802_11_LENGTH_SSID;

			if (pCredential->SSID.SsidLength < AutoConnSSIDlen)
				AutoConnSSIDlen = pCredential->SSID.SsidLength;
			if (ARRAY_SIZE(pStaCfg->MlmeAux.AutoReconnectSsid) < AutoConnSSIDlen)
				AutoConnSSIDlen = ARRAY_SIZE(pStaCfg->MlmeAux.AutoReconnectSsid);

			pStaCfg->MlmeAux.AutoReconnectSsidLen = AutoConnSSIDlen;
			NdisZeroMemory(pStaCfg->MlmeAux.AutoReconnectSsid, NDIS_802_11_LENGTH_SSID);
			NdisMoveMemory(pStaCfg->MlmeAux.AutoReconnectSsid, pCredential->SSID.Ssid, AutoConnSSIDlen);
			pStaCfg->MlmeAux.SsidLen = AutoConnSSIDlen;

			if (pStaCfg->MlmeAux.SsidLen < MLMESSIDLen)
				MLMESSIDLen = pStaCfg->MlmeAux.SsidLen;
			if (ARRAY_SIZE(pStaCfg->MlmeAux.Ssid) < MLMESSIDLen)
				MLMESSIDLen = ARRAY_SIZE(pStaCfg->MlmeAux.Ssid);

			NdisZeroMemory(pStaCfg->MlmeAux.Ssid, NDIS_802_11_LENGTH_SSID);
			NdisMoveMemory(pStaCfg->MlmeAux.Ssid, pCredential->SSID.Ssid, MLMESSIDLen);
...
#endif /* CONFIG_STA_SUPPORT */
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ra%d - AuthType: %u, EncrType: %u\n", CurApIdx, pCredential->AuthType, pCredential->EncrType);
		...
		if (pCredential->AuthType & (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK | WSC_AUTHTYPE_WPANONE)) {
			if (!(pCredential->EncrType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))) {
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "AuthType is WPAPSK or WPA2PAK.\n"
		...
		if (pCredential->EncrType != WSC_ENCRTYPE_NONE) {
			if (pCredential->EncrType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES)) {
				...
				if (pCredential->KeyLength >= 8 && pCredential->KeyLength <= 64) {
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WpaPskLen = %d\n", pWscControl->WpaPskLen);
				} else {
					pWscControl->WpaPskLen = 0;
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WPAPSK: Invalid Key Length (%d)\n", pCredential->KeyLength);
				}
	...
	IN  UCHAR CurOpMode)
{
	...
	if (bNewFormat == FALSE) {
		...
		if (CurOpMode == AP_MODE) {
			for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && apidx < MAX_BEACON_NUM && (strlen(pTempStr) < TempStrLen); apidx++) {
				UINT ssidlen = (pAd->ApCfg.MBSSID[apidx].SsidLen <= ARRAY_SIZE(pAd->ApCfg.MBSSID[apidx].Ssid)) ?
					pAd->ApCfg.MBSSID[apidx].SsidLen : ARRAY_SIZE(pAd->ApCfg.MBSSID[apidx].Ssid);
				UINT extralen = (apidx == 0) ? strlen("SSID=") : 1;

				if (TempStrLen >= (strlen(pTempStr) + ssidlen + extralen)) {
					if (apidx == 0) {
		...
		if (CurOpMode == STA_MODE) {
			...
			UINT len = (pCredential->SSID.SsidLength <= ARRAY_SIZE(pCredential->SSID.Ssid)) ?
				pCredential->SSID.SsidLength : ARRAY_SIZE(pCredential->SSID.Ssid);

			if ((offset + len + strlen("SSID=") + strlen(pTempStr)) < TempStrLen) {
				NdisMoveMemory(pTempStr, "SSID=", strlen("SSID="));
				offset = strlen(pTempStr);
				NdisMoveMemory(pTempStr + offset, pCredential->SSID.Ssid, len);
			} else
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN,
					"Failed to store STA SSID\n");
		}
		...
		for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && apidx < MAX_BEACON_NUM && (strlen(pTempStr) < TempStrLen); apidx++) {
			...
			(void)snprintf(item_str, sizeof(item_str), "SSID%d", (apidx + 1));
			len = strlen(item_str);
			if (rtstrstr(pTempStr, item_str)) {
				UINT ssidlen = (pAd->ApCfg.MBSSID[apidx].SsidLen <= ARRAY_SIZE(pAd->ApCfg.MBSSID[apidx].Ssid)) ?
					pAd->ApCfg.MBSSID[apidx].SsidLen : ARRAY_SIZE(pAd->ApCfg.MBSSID[apidx].Ssid);

				/* Compare total str size and current str size and
				 * incoming string size, +1 for "=" */
				if (TempStrLen >= (strlen(pTempStr) + len + ssidlen + 1)) {
					NdisZeroMemory(pTempStr, (TempStrLen - strlen(pTempStr)));
					NdisMoveMemory(pTempStr, item_str, len);
					offset = len;
					NdisMoveMemory(pTempStr + offset, "=", 1);
					offset += 1;
					NdisMoveMemory(pTempStr + offset, pAd->ApCfg.MBSSID[apidx].Ssid, ssidlen);
				} else {
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN,
						"Failed to store AP SSID\n");
					break;
	...
	IN  BOOLEAN bNewFormat)
{
	...
	if (bNewFormat == FALSE) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && apidx < MAX_BEACON_NUM && (strlen(pTempStr) < TempStrLen); apidx++) {
				UINT psklen = 0;
				UINT extralen = (apidx == 0) ? strlen("WPAPSK=") : 1;

				pWscControl = &pAd->ApCfg.MBSSID[apidx].wdev.WscControl;
				psklen = (pWscControl->WpaPskLen <= ARRAY_SIZE(pWscControl->WpaPsk)) ? pWscControl->WpaPskLen : ARRAY_SIZE(pWscControl->WpaPsk);

				if (TempStrLen >= (strlen(pTempStr) + extralen + psklen)) {
					if (apidx == 0) {
					...
					NdisMoveMemory(pTempStr + offset, pWscControl->WpaPsk, psklen);
				} else {
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN,
						"Failed to WPAPSK\n");
					break;
		...
		for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && apidx < MAX_BEACON_NUM && (strlen(pTempStr) < TempStrLen); apidx++) {
			...
			(void)snprintf(item_str, sizeof(item_str), "WPAPSK%d", (apidx + 1));
			len = strlen(item_str);
			if (rtstrstr(pTempStr, item_str)) {
				UINT psklen = 0;
				...
				pWscControl = &pAd->ApCfg.MBSSID[apidx].wdev.WscControl;
				psklen = (pWscControl->WpaPskLen <= ARRAY_SIZE(pWscControl->WpaPsk)) ? pWscControl->WpaPskLen : ARRAY_SIZE(pWscControl->WpaPsk);
				if (TempStrLen >= (strlen(pTempStr) + len + psklen + 1)) {
					NdisZeroMemory(pTempStr, (TempStrLen - strlen(pTempStr)));
					NdisMoveMemory(pTempStr, item_str, len);
					offset = strlen(pTempStr);
					NdisMoveMemory(pTempStr + offset, "=", 1);
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscCheckNonce\n");

	if (bFlag) {
		/* check Registrar Nonce */
		WscId = WSC_ID_REGISTRAR_NONCE;
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "check Registrar Nonce\n");
	} else {
	...
	while (Length > 4) {
		...
		if (WscType == WscId) {
			if (RTMPCompareMemory(pWscControl->RegData.SelfNonce, pData, 16) == 0) {
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Nonce match!!\n");
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscCheckNonce\n");
				return TRUE;
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Zero Enrollee Nonce!!\n");
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscCheckNonce\n");
				return TRUE;
			}
		}
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (pWscControl->WscPinCode == 0) {
		(void)snprintf((RTMP_STRING *) tempPIN, sizeof(tempPIN), "00000000");
		...
		if (pWscControl->WscPinCodeLen == 4) {
			...
			(void)snprintf((RTMP_STRING *) temp4PIN, sizeof(temp4PIN), "%04u", pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, temp4PIN, 4);
			pWscControl->RegData.PinCodeLen = 4;
		} else {
			(void)snprintf((RTMP_STRING *) tempPIN, sizeof(tempPIN), "%08u", pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, tempPIN, 8);
			pWscControl->RegData.PinCodeLen = 8;
		}
	}
	...
	OUT USHORT *pConfigError)
{
	...
	while (Length > 4) {
		...
		if (WscType == WSC_ID_CONFIG_ERROR) {
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WSC_ID_CONFIG_ERROR: %d\n", ntohs(ConfigError));
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WSC_ID_CONFIG_ERROR is missing\n");
}
	...
	IN  RTMP_STRING *arg)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
			...
			if (IS_ENTRY_CLIENT(entry))
				...
				if (IS_MAP_ENABLE(pAd))
					if (!((entry->DevPeerRole) & BIT(MAP_ROLE_BACKHAUL_STA)))
#endif
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"IF(ra%d) (AuthMode=0x%x)\n", apidx, wdev->SecConfig.AKMMap);
	}
	...
	IN  RTMP_STRING *arg)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"IF(ra%d) (EncrypType=0x%x)\n", apidx, wdev->SecConfig.PairwiseCipher);
	}
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscPushPBCAction\n");
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscPINAction\n");
	...
	/* For PBC, the PIN is all '0' */
	/* WscGetRegDataPIN(pAd, pWscControl->WscPinCode, pWscControl); */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Enrollee_pin_code-----> %u\n", pWscControl->WscEnrolleePinCode);
	/* 2. Set 2 min timout routine */
	RTMPSetTimer(&pWscControl->Wsc2MinsTimer, WSC_TWO_MINS_TIME_OUT);
	pWscControl->Wsc2MinsTimerRunning = TRUE;
	pWscControl->bWscTrigger = TRUE;	/* start work */
	...
	IN  PWSC_CTRL pWscControl)
{
		...
		if (pWscControl->WscPBCBssCount == 0) {
			...
			*/
			...
			if (LED_MODE(pAd) == WPS_LED_MODE_9) { /* WPS LED mode 9. */
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
					"Skip the WPS session overlap detected LED indication.\n");
			} else { /* Other LED mode. */
	...
	if (CurOpMode == STA_MODE) {
		...
		if (apcli_idx >= ARRAY_SIZE(pAd->StaCfg)) {
			MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
				"Invalid apcli index!!!\n");
			return FALSE;
		}
...
#endif /* WSC_LED_SUPPORT */
			MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"--> AP list is %d, wait for next time\n", pWscControl->WscPINBssCount);
#ifdef CONFIG_STA_SUPPORT
	...
	if (CurOpMode == STA_MODE) {
		if (pStaCfg->BssType == BSS_INFRA) {
			MlmeEnqueueWithWdev(pAd,
								...
								(VOID *)&pWscControl->WscBssid[0], 0, &pStaCfg->wdev, FALSE, NULL);
		} else {
			MlmeEnqueueWithWdev(pAd,
								MLME_CNTL_STATE_MACHINE,
								OID_802_11_SSID,
								sizeof(NDIS_802_11_SSID),
								(VOID *)&pWscControl->WscSsid, 0, &pStaCfg->wdev, FALSE, NULL);
		}
	}
	...
	while ((Len > 0) && (bFound == FALSE)) {
		...
		while (Len > 0) {
			...
			if (be2cpu16(pWscIE->Type) == WSC_ID_SEL_REGISTRAR) {
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "bSelReg = %d\n", bSelReg);
			}
			...
			if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
...
#endif /* WINBOND */
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscPBCBssTableSort : DevicePasswordID = 0x%04x\n", DevicePasswordID);
				...
				if (((pWscControl->WscMode == WSC_PBC_MODE) && (DevicePasswordID == DEV_PASS_ID_PBC)) ||
					...
					bFound = TRUE;
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "DPID=PBC Found -->\n");
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "#  Bssid "MACSTR"\n",
	...
	while ((Len > 0) && (bFound == FALSE)) {
		...
		while (Len > 0) {
			...
			if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
				...
				/*DevicePasswordID = be2cpu16(*((USHORT *) &pWscIE->Data[0])); */
#endif /* WINBOND */
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscPINBssTableSort : DevicePasswordID = 0x%04x\n", DevicePasswordID);
				...
				if ((pWscControl->WscMode == WSC_PIN_MODE) && (DevicePasswordID == DEV_PASS_ID_PIN)) {
					...
					bFound = TRUE;
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "DPID=PIN Found -->\n");
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "#  Bssid "MACSTR"\n",
							 MAC2STR(pInBss->Bssid));
...
#ifdef CONFIG_STA_SUPPORT
		if (wdevBand != hc_get_hw_band_idx(pAd))
			continue;
	...
	IN WSC_CTRL *pWscControl)
{
	...
	for (i = 0; i < ScanTab->BssNr; i++) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "#  wsc Bssid "MACSTR"\n",
				 MAC2STR(pWscControl->WscBssid));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "# pIn Bssid "MACSTR"\n",
				 MAC2STR(pInBss->Bssid));
	...
	*/
	if (pWscControl->WscKeyASCII == 0) {
		...
		for (idx = 0; idx < 32; idx++) {
			...
			(void)snprintf((RTMP_STRING *) &tmpStrB[0], 3, "%02x", tempRandomByte);
	...
	OUT PWSC_PROFILE pWscProfile)
{
...
#endif /* CONFIG_STA_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscGetDefaultProfileForM8\n");
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
		"pWscControl->WscConfStatus  = %d, OpMode = %d\n",
		pWscControl->WscConfStatus, OpMode);
	...
	if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED) &&
		...
		case WSC_ENCRTYPE_WEP:
			...
			if (pSecConfig && pSecConfig->WepKey[WepKeyId].KeyLen) {
				...
				for (i = 0; i < pSecConfig->WepKey[WepKeyId].KeyLen; i++)
					(void)snprintf((RTMP_STRING *) pCredential->Key + strlen((RTMP_STRING *)pCredential->Key),
						64 - strlen((RTMP_STRING *)pCredential->Key), "%02x", pSecConfig->WepKey[WepKeyId].Key[i]);
		...
		case (WSC_ENCRTYPE_AES | WSC_ENCRTYPE_TKIP):
			...
			memcpy(pCredential->Key,
				   pWscControl->WpaPsk,
				   ((pWscControl->WpaPskLen > ARRAY_SIZE(pCredential->Key)) ? ARRAY_SIZE(pCredential->Key) : (pWscControl->WpaPskLen)));
			break;
		}
	}
	...
	if (CurOpMode == AP_MODE) {
		if ((OpMode & 0x0F) == AP_MODE) {
			...
			if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED) &&
				...
				copylen = pWscControl->WscDefaultSsid.SsidLength;
				if (copylen > ARRAY_SIZE(pCredential->SSID.Ssid))
					copylen = ARRAY_SIZE(pCredential->SSID.Ssid);
				NdisMoveMemory(pCredential->SSID.Ssid, pWscControl->WscDefaultSsid.Ssid, copylen);
				pCredential->SSID.SsidLength = copylen;
			} else {
				copylen = pAd->ApCfg.MBSSID[apidx].SsidLen;
				if (copylen > ARRAY_SIZE(pCredential->SSID.Ssid))
					copylen = ARRAY_SIZE(pCredential->SSID.Ssid);
				NdisMoveMemory(pCredential->SSID.Ssid, pAd->ApCfg.MBSSID[apidx].Ssid, copylen);
				pCredential->SSID.SsidLength = copylen;
			}
		}
			...
			if (apidx < MAX_APCLI_NUM) {
				UINT ssidlen = pAd->StaCfg[apidx].SsidLen;

				if (ssidlen > ARRAY_SIZE(pCredential->SSID.Ssid))
					ssidlen = ARRAY_SIZE(pCredential->SSID.Ssid);

				NdisMoveMemory(pCredential->MacAddr, APCLI_ROOT_BSSID_GET(pAd, pAd->StaCfg[apidx].MacTabWCID), 6);
				NdisMoveMemory(pCredential->SSID.Ssid, pAd->StaCfg[apidx].Ssid, ssidlen);
				pCredential->SSID.SsidLength = ssidlen;
			}
		}
	...
	if (CurOpMode == STA_MODE) {
		UINT copylen = pAd->StaCfg[0].SsidLen;
		...
		NdisMoveMemory(pCredential->SSID.Ssid, pAd->StaCfg[0].Ssid, copylen);
		pCredential->SSID.SsidLength = copylen;
	}
...
#endif /* WSC_V2_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscCreateProfileFromCfg\n");
}
	...
	IN  BOOLEAN bEnrollee)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscWriteConfToApCliCfg (apcli%d)\n", CurApIdx);
	{
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "AuthType: %d, EncrType: %d\n", pCredential->AuthType, pCredential->EncrType);
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscWriteConfToApCliCfg\n");
}
	...
	IN  UCHAR apidx)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscApCliLinkDownById, apidx=%u, apcliEn=%d\n", apidx, apcliEn);
	pWscControl = &pAd->StaCfg[apidx].wdev.WscControl;
	NdisMoveMemory(pWscControl->RegData.SelfInfo.MacAddr,
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscApCliLinkDown, apidx=%u\n", apidx);
	...
	if ((Elem->MsgType == WSC_EAPOL_UPNP_MSG) && (Elem->MsgLen > HeaderLen)) {
		...
		if (pEapFrame->Code == EAP_CODE_FAIL) {
			...
			if (Elem->MsgLen <= HeaderLen) {
				MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Elem->MsgLen(%ld) <= HeaderLen(%d) !!\n", Elem->MsgLen, HeaderLen);
				return;
			}
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ERROR: WscCheckWSCHeader() return FALSE!\n");
			return;
		}
		...
		if (IS_ENTRY_CLIENT(pEntry) &&
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "RTMPCancelTimer EapolTimer!!\n");
	...
	IN  BOOLEAN bFromUPnP)
{
	...
	if (pWscControl->WscConfMode == WSC_DISABLE) {
		pWscControl->bWscTrigger = FALSE;
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscGetConfForUpnp:: WPS is disabled.\n");
		return FALSE;
	}
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "trigger WSC state machine\n");
	return TRUE;
}
#endif /* CONFIG_AP_SUPPORT */
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (pWscData == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscSendNACK:: WscData Allocate failed!\n");
		return;
	}
	...
	IN  PVOID wdev_obj)
{
	...
	if (INFRA_ON(sta_cfg) && STA_STATUS_TEST_FLAG(sta_cfg, fSTA_STATUS_MEDIA_STATE_CONNECTED)) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscLinkDown(): Disassociate with current WPS AP...\n");
	...
	IN	UCHAR current_band)
{
	...
	if (current_band >= MAX_NUM_BAND) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
			"invalid current_band:%d\n", current_band);
		return;
	}
	NdisGetSystemUpTime(&now);
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
		"STA_MAC = "MACSTR"\n", MAC2STR(pMacAddr));
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscPBCStaProbeCount[%d] = %d\n",
		current_band, pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band]);
}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			if (pAd->ApCfg.MBSSID[index].SsidLen) {
				if (index == 0)
					(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%s", GetAuthModeStr(pAd->ApCfg.MBSSID[index].wdev.SecConfig.AKMMap));
				else
					(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%s", GetAuthModeStr(pAd->ApCfg.MBSSID[index].wdev.SecConfig.AKMMap));
			}
		}
	...
	if (CurOpMode == STA_MODE) {
		...
		(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%s", WscGetAuthTypeStr(auth_flag));
	}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr),
				"%s",
	...
	if (CurOpMode == STA_MODE) {
		...
		(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%s", WscGetEncryTypeStr(encrypt_flag));
	}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			...
			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", wsc_ctrl->WscConfMode);
			else
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", wsc_ctrl->WscConfMode);
		}
	}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			...
			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", wsc_ctrl->WscConfStatus);
			else
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", wsc_ctrl->WscConfStatus);
		}
	}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", pAd->ApCfg.MBSSID[index].wdev.SecConfig.PairwiseKeyId + 1);
			else
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", pAd->ApCfg.MBSSID[index].wdev.SecConfig.PairwiseKeyId + 1);
		}
	}
	...
	if (CurOpMode == STA_MODE)
		(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
...
#define WSC_PRINT_WEP_KEY(__idx, __key_len, __key, __out_buf) \
	{ \
		for (__idx = 0; __idx < __key_len; (__idx)++) \
			(void)snprintf(__out_buf + strlen(__out_buf), 512 - strlen(__out_buf), "%02x", __key[__idx]); \
	}
	...
	IN  UINT TempLen)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		tempStrLen = (TempLen < strlen(WepKeyFormatName)) ? TempLen : strlen(WepKeyFormatName);
		if ((strncmp(pTempStr, WepKeyFormatName, tempStrLen) == 0)) {
			if (IS_CIPHER_WEP_Entry(&pAd->ApCfg.MBSSID[apidx].wdev)) {
				...
				pTempStr = temp_ptr;
				NdisZeroMemory(pTempStr, TempLen);
				NdisMoveMemory(pTempStr, WepKeyFormatName, tempStrLen);

				for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
					if (idx == apidx)
						(void)snprintf(pTempStr + strlen(pTempStr), TempLen - strlen(pTempStr), "%d", 0);
					else
						(void)snprintf(pTempStr + strlen(pTempStr), TempLen - strlen(pTempStr), "%d", KeyType[idx]);

					if (apidx < (pAd->ApCfg.BssidNum - 1))
						(void)snprintf(pTempStr + strlen(pTempStr), TempLen - strlen(pTempStr), "%s", ";");
				}
			}
			...
			tempStrLen = (TempLen < strlen(WepKeyName)) ? TempLen : strlen(WepKeyName);

			if (IS_CIPHER_WEP_Entry(&pAd->ApCfg.MBSSID[apidx].wdev)) {
				NdisZeroMemory(pTempStr, TempLen);
				NdisMoveMemory(pTempStr, WepKeyName, tempStrLen);

				if (pCredentail->KeyLength && ((pCredentail->KeyLength + tempStrLen) <= TempLen)) {
	...
	if (CurOpMode == STA_MODE) {
		if (rtstrstr(pTempStr, (RTMP_STRING *) WepKeyFormatName)) {
			...
			if (IS_CIPHER_WEP_Entry(&pAd->StaCfg[0].wdev)) {
				NdisZeroMemory(pTempStr, TempLen);
				(void)snprintf(pTempStr, tempStrLen, "%s0",
					WepKeyFormatName); /* Hex */
			}
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		(void)snprintf((RTMP_STRING *) WepKeyName, sizeof(WepKeyName), "Key%dStr%d=", pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId + 1, apidx + 1);
		(void)snprintf((RTMP_STRING *) WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId + 1);
	}
	...
	if (CurOpMode == STA_MODE) {
		...
		(void)snprintf(WepKeyName, sizeof(WepKeyName), "Key%dStr=", pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
		(void)snprintf(WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
	}

#endif /* CONFIG_STA_SUPPORT */
	if (fileName == NULL) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
			"-->1) Error opening file, file name NULL\n");
		return;
	}
		...
		for (;;) {
			...
			if ((size_t)(offset - cfgData) < fileLen) {
				...
				if ((strncmp(pTempStr, "SSID=", strlen("SSID=")) == 0) ||
					...
					WscWriteSsidToDatFile(pAd, pTempStr, StrLen, bNewFormat, CurOpMode);
				}
					...
					if (pAd->StaCfg[0].BssType == BSS_ADHOC)
						(void)snprintf(pTempStr, 512, "NetworkType=Adhoc");
					else
						(void)snprintf(pTempStr, 512, "NetworkType=Infra");
				}

#endif /* CONFIG_STA_SUPPORT */
				else if (strncmp(pTempStr, "AuthMode=", strlen("AuthMode=")) == 0) {
					NdisZeroMemory(pTempStr, StrLen);
					(void)snprintf(pTempStr, StrLen, "AuthMode=");
					WscWriteAuthToDAT(pAd, CurOpMode, pTempStr);
				} else if (strncmp(pTempStr, "EncrypType=", strlen("EncrypType=")) == 0) {
					NdisZeroMemory(pTempStr, StrLen);
					(void)snprintf(pTempStr, StrLen, "EncrypType=");
...
out:
	RtmpOSFSInfoChange(&osFSInfo, FALSE);
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "<----- WscWriteConfToDatFile\n");
	return;
	}
	...
	OUT INT *datoffset)
{
	...
	for (index = 1; index <= 4; index++) {
		(void)snprintf(WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", index);
		{
			len = (TempLen < sizeof(WepKeyFormatName)) ? TempLen : sizeof(WepKeyFormatName);
			NdisZeroMemory(pTempStr, TempLen);
			offset = 0;
			NdisMoveMemory(pTempStr, WepKeyFormatName, len);

			for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && apidx < MAX_BEACON_NUM && (TempLen >= (strlen(pTempStr) + 2)); apidx++) {
				if (IS_CIPHER_WEP(pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.SecConfig.PairwiseCipher)) {
					pCredentail = &pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscProfile.Profile[0];

					if ((pCredentail->KeyLength == 5) ||
						(pCredentail->KeyLength == 13))
						(void)snprintf(pTempStr + strlen(pTempStr), TempLen - strlen(pTempStr), "%d", 1); /* ASCII */
					else
						(void)snprintf(pTempStr + strlen(pTempStr), TempLen - strlen(pTempStr), "%d", 0); /* Hex */
				}
			...
			(void)snprintf(pTempStr + strlen(pTempStr),
					TempLen - strlen(pTempStr), "%s", "\n");
			offset = strlen(pTempStr);
			if (((*datoffset) + offset) <= DatLen) {
		...
		(void)snprintf(WepKeyName, sizeof(WepKeyName), "Key%dStr=", index);
		/*if (rtstrstr(pTempStr, WepKeyName)) */
		{
			NdisZeroMemory(pTempStr, TempLen);
			offset = 0;
			len = (TempLen < sizeof(WepKeyName)) ? TempLen : sizeof(WepKeyName);
			NdisMoveMemory(pTempStr, WepKeyName, len);
			tempStrLen = len;

			for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); apidx++) {
				pCredentail = &pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscProfile.Profile[0];

				if ((pCredentail->KeyLength) && ((tempStrLen + pCredentail->KeyLength) < TempLen)) {
					NdisMoveMemory(pTempStr + tempStrLen, pCredentail->Key, pCredentail->KeyLength);
					tempStrLen = strlen(pTempStr);
				} else {
					MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
		...
		for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); apidx++) {
			(void)snprintf(WepKeyName, sizeof(WepKeyName), "Key%dStr%d=", index, (apidx + 1));
			...
			if (IS_CIPHER_WEP(pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseCipher)) {
				...
				tempStrLen = len;
				pCredentail = &pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscProfile.Profile[0];
				if ((tempStrLen + pCredentail->KeyLength + 2) < TempLen) {
					NdisMoveMemory(pTempStr + tempStrLen, pCredentail->Key, pCredentail->KeyLength);
	...
	IN  UCHAR CurOpMode)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "-----> WscWriteConfToAR9File\n");
	...
	if (IS_FILE_OPEN_ERR(file_w))
		...
		if (!pTempStr) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "pTempStr mem alloc fail. (512)\n");
			RtmpOSFileClose(file_w);
			goto WriteErr;
		}
		...
		if (!pDatStr) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "pDatStr mem alloc fail. (4096)\n");
			RtmpOSFileClose(file_w);
			goto WriteErr;
		}
		...
		for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); apidx++) {
			(void)snprintf(item_str, sizeof(item_str), "SSID%d", (apidx + 1));
			{
				...
				if ((strlen(pTempStr) + strlen(item_str) + 2) < 512) {
					NdisMoveMemory(pTempStr, item_str, strlen(item_str));
					offset = strlen(pTempStr);
					NdisMoveMemory(pTempStr + offset, "=", 1);
					offset += 1;
					len = ((strlen(pTempStr) + pAd->ApCfg.MBSSID[apidx].SsidLen + 1) > 512) ?
						(512 - (strlen(pTempStr) + 1)) : pAd->ApCfg.MBSSID[apidx].SsidLen;
					NdisMoveMemory(pTempStr + offset, pAd->ApCfg.MBSSID[apidx].Ssid, len);
					offset += len;
					NdisMoveMemory(pTempStr + offset, "\n", 1);
					offset += 1;
				} else {
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN,
						"input data exceeds 512\n");
					break;
				}
			}
			NdisZeroMemory(item_str, 10);
		...
		offset = 0;
		NdisZeroMemory(pTempStr, 512);
		(void)snprintf(pTempStr, 512, "AuthMode=");
		WscWriteAuthToDAT(pAd, CurOpMode, pTempStr);
		offset = strlen(pTempStr);
		NdisMoveMemory(pDatStr + datoffset, pTempStr, offset);
		datoffset += offset;
		offset = 0;
		NdisZeroMemory(pTempStr, 512);
		(void)snprintf(pTempStr, 512, "EncrypType=");
		WscWriteEncrToDAT(pAd, CurOpMode, pTempStr);
		offset = strlen(pTempStr);
		NdisMoveMemory(pDatStr + datoffset, pTempStr, offset);
		datoffset += offset;
		...
		for (apidx = 0; (apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); apidx++) {
			...
			/*if (rtstrstr(pTempStr, item_str)) */
			{
				...
				if ((strlen(pTempStr) + strlen(item_str) + 2) < 512) {
					...
					NdisMoveMemory(pTempStr + offset, "=", 1);
					offset += 1;
					len = ((strlen(pTempStr) + pWscControl->WpaPskLen + 1) > 512) ?
						(512 - (strlen(pTempStr) + 1)) : pWscControl->WpaPskLen;
					NdisMoveMemory(pTempStr + offset, pWscControl->WpaPsk, len);
					offset += len;
					NdisMoveMemory(pTempStr + offset, "\n", 1);
					offset += 1;
				} else {
		...
		offset = 0;
		NdisZeroMemory(pTempStr, 512);
		(void)snprintf(pTempStr, 512, "WscConfMode=");
#ifdef CONFIG_AP_SUPPORT

		for (index = 0; (index < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); index++) {
			pWscControl = &pAd->ApCfg.MBSSID[index].wdev.WscControl;

			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", pWscControl->WscConfMode);
			else
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", pWscControl->WscConfMode);
		}

		(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%s", "\n");
		offset = strlen(pTempStr);
		NdisMoveMemory(pDatStr + datoffset, pTempStr, offset);
		datoffset += offset;
#endif /* CONFIG_AP_SUPPORT */
		offset = 0;
		NdisZeroMemory(pTempStr, 512);
		(void)snprintf(pTempStr, 512, "WscConfStatus=");
#ifdef CONFIG_AP_SUPPORT
		...
		for (index = 0; (index < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); index++) {
			...
			if (index == 0)
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", pWscControl->WscConfStatus);
			else
				(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", pWscControl->WscConfStatus);
		}
		...
		offset = 0;
		NdisZeroMemory(pTempStr, 512);
		(void)snprintf(pTempStr, 512, "DefaultKeyID=");
#ifdef CONFIG_AP_SUPPORT

		if (CurOpMode == AP_MODE) {
			for (index = 0; (index < pAd->ApCfg.BssidNum) && (apidx < MAX_BEACON_NUM); index++) {
				if (index == 0)
					(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), "%d", pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId + 1);
				else
					(void)snprintf(pTempStr + strlen(pTempStr), 512 - strlen(pTempStr), ";%d", pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId + 1);
			}

			(void)snprintf(pTempStr + strlen(pTempStr),
					512 - strlen(pTempStr), "%s", "\n");
			offset = strlen(pTempStr);
			NdisMoveMemory(pDatStr + datoffset, pTempStr, offset);
			datoffset += offset;
		}
		...
		if (CurOpMode == AP_MODE)
			WscWriteWepKeyToAR9File(pAd, pTempStr, 512, pDatStr, 4096, &datoffset);
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd)
{
	...
	ret = snprintf(task_name, TASK_NAME_LEN,
		"RtmpWscTask_%d%d", PD_GET_DEVICE_IDX(pAd->physical_dev), band_idx);
	...
	if (status == NDIS_STATUS_SUCCESS) {
		...
		if (pAd->pWscElme == NULL) {
			MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
				"Wsc pWscElme memory alloc failed!\n");
			status = NDIS_STATUS_FAILURE;
		}
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"Support WPS LED mode (The WPS LED mode = %d).\n", LED_MODE(pAd));
		return TRUE; /* Support WPS LED mode. */
	}

	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
		"Not support WPS LED mode (The WPS LED mode = %d).\n", LED_MODE(pAd));
	...
	IN PWSC_CTRL pWscControl)
{
	...
	default: {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
			"Incorrect encryption types (%d)\n",
			pWscControl->WscProfile.Profile[currentIdx].EncrType);
		ASSERT(FALSE);
		break;
	}
	}
	...
	IN PVOID SystemSpecific3)
{
	...
	if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
		...
		case LED_WPS_SUCCESS:
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"LED_WPS_SUCCESS => LED_WPS_TURN_LED_OFF\n");
			break;
		...
		case LED_WPS_TURN_LED_OFF:
			if ((pWscControl->WscState == WSC_STATE_OFF) &&
				...
				if (WscAPHasSecuritySetting(pAd, pWscControl) == TRUE) { /* The NIC connects with an AP using an encryption algorithm. */
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
						"LED_WPS_TURN_LED_OFF => LED_WPS_TURN_ON_BLUE_LED\n");
				} else { /* The NIC connects with an AP using OPEN-NONE. */
					/* Turn off the WPS LED. */
					WPSLEDStatus = LED_WPS_TURN_LED_OFF;
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
						"LED_WPS_TURN_LED_OFF => LED_WPS_TURN_LED_OFF\n");
				}
			}
		...
		case LED_WPS_ERROR:
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"LED_WPS_ERROR/LED_WPS_SESSION_OVERLAP_DETECTED => LED_WPS_TURN_LED_OFF\n");
			break;
		...
		case LED_WPS_SESSION_OVERLAP_DETECTED:
			WPSLEDStatus = LED_WPS_TURN_LED_OFF; /* Turn off the WPS LED. */
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"LED_WPS_SESSION_OVERLAP_DETECTED => LED_WPS_TURN_LED_OFF\n");
			break;
	...
	IN PVOID SystemSpecific3)
{
	...
	pWscControl->WscProfile.ApplyProfileIdx &= 0x7FFF;
	{
		...
		*/
		...
		/* Avoid AP restart when MAP enable since*/
		/* this will hit only when WPS unconfigured enable*/
		if (!IS_MAP_ENABLE(pAd)) {
#endif
			enq_rv = RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RESTART, (VOID *)&apidx, sizeof(UCHAR));
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
				"en-queu CMDTHREAD_AP_RESTART - enq_rv = 0x%x\n", enq_rv);
#if defined(CONFIG_MAP_SUPPORT) && defined(WPS_UNCONFIG_FEATURE_SUPPORT)
		}
#endif
	}
	...
	IN	UCHAR current_band)
{
	...
	PUCHAR pData = NULL;
	INT Len = 0;
	USHORT DevicePasswordID = DEV_PASS_ID_NOSPEC;
		...
		if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
			if (Len >= (4 + sizeof(DevicePasswordID))) {
				...
				if (DevicePasswordID == DEV_PASS_ID_PBC) {
					 ...
					 * Do NOT add this STA to WscPeerList after AP is triggered
					 * to do PBC. */
					if (pWscCtrl &&
					...
					MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
						"DevicePasswordID = 0x%04x\n", DevicePasswordID);
				}
			} else
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN,
					"strange size of DevicePasswordID = 0x%04x\n", DevicePasswordID);
...
INT WscStaShowPeerList(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pWscEnList->size != 0) {
		...
		while (pPeerEntry != NULL) {
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MAC:"MACSTR"\tReveive Time:%lu\n",
					 MAC2STR(pPeerEntry->mac_addr),
					 pPeerEntry->receive_time);
			pListEntry = pListEntry->pNext;
			pPeerEntry = (PWSC_PEER_ENTRY)pListEntry;
	...
	OUT PUSHORT pWscBufLen)
{
	...
	Len = IeDataLen;
	if (Len < sizeof(WSC_IE) || Len > 512)
		return FALSE;

	while (Len >= sizeof(WSC_IE)) {
		WSC_IE	WscIE;
		...
		if (be2cpu16(pWscIE->Type) == WscTag) {
			...
			if ((DataLen <=  (1 << (sizeof(USHORT) * 8)) - 1) && ((Len - 4) >= DataLen)) {
				if (pWscBufLen)
					*pWscBufLen = DataLen;
...
VOID ApWscDefaultInit(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev)
{
	...
	NdisZeroMemory(&ad->CommonCfg.WscStaPbcProbeInfo, sizeof(WSC_STA_PBC_PROBE_INFO));
	pWscControl->WscMode = 1;
	...
	pWscControl->WscRejectSamePinFromEnrollee = FALSE;
	ad->CommonCfg.WscPBCOverlap = FALSE;
	...
	*/
	for (idx = 0; idx < 192; idx++)
		pWscControl->RegData.EnrolleeRandom[idx] = RandomByte(ad);

	/* Enrollee Nonce, first generate and save to Wsc Control Block*/
	for (idx = 0; idx < 16; idx++)
		pWscControl->RegData.SelfNonce[idx] = RandomByte(ad);
	...
	NdisAllocateSpinLock(ad, &pWscControl->WscPeerListSemLock);
...
VOID StaWscDefaultInit(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev)
{
#ifdef WSC_STA_SUPPORT
	INT		    idx;
	PWSC_CTRL	    pWscControl;
	...
	*/
	pWscControl = &wdev->WscControl;
#ifdef APCLI_SUPPORT
	pWscControl->WscApCliScanMode = TRIGGER_FULL_SCAN;
#endif /* APCLI_SUPPORT */
	pWscControl->WscConfMode = WSC_DISABLE;
	...
	os_alloc_mem(ad, &pWscControl->pWscRxBuf, MAX_MGMT_PKT_LEN);
