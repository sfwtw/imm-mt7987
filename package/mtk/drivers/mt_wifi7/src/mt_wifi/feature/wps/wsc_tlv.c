	...
	while (PlainLength > 4) {
		...
		case WSC_ID_MAC_ADDR:
			if (WscLen && (PlainLength >= MAC_ADDR_LEN)) {
				...
				RTMPMoveMemory(pProfile->Profile[0].MacAddr, pData, MAC_ADDR_LEN);
			}
			break;
		...
		case WSC_ID_DPP_URI_INFO:
			if (WscLen <= sizeof(pWscControl->rcvd_dpp_uri)) {
				...
				MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
					"%s --> Received URI %s with len %d from peer device\n",
					__func__, pWscControl->rcvd_dpp_uri,
					pWscControl->rcvd_uri_len);
			}
			break;
#endif /* MAP_R3 */

		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "WscParseEncrSettings --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	UINT pskbufsize)
{
	...
	UINT copylen = SHA256_DIGEST_SIZE;
	...
	NdisZeroMemory(pTempPsk, SHA256_DIGEST_SIZE);
	{
#ifndef RT_CFG80211_SUPPORT
		/* Generate PSK1 */
	...
	copylen = (copylen <= pskbufsize) ? copylen : pskbufsize;
	NdisMoveMemory(pPSK1, pTempPsk, copylen);
	...
	UCHAR *pPSK2)
{
	...
	os_zero_mem(pTempPsk, SHA256_DIGEST_SIZE);
	...
	OUT	VOID *pbuf)
{
	...
	if ((pWscControl->WscConfStatus == 1) &&
		(pAdapter->ApCfg.MBSSID[apidx].wdev.WscControl.wps_unconfig_trigger == TRUE)) {
	...
	*/
...
#endif /* WSC_V2_SUPPORT */
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "BuildMessageM1 - bUPnPMsgTimerRunning = %d, pWscControl->WscUseUPnP = %d, pWscControl->EapMsgRunning = %d\n",
	...
	if (pWscControl->WscUPnPNodeInfo.bUPnPMsgTimerRunning ||
		...
		INT copylen = (Len <= ARRAY_SIZE(pReg->LastTx.Data)) ? Len : ARRAY_SIZE(pReg->LastTx.Data);

		/* Copy the content to Regdata for lasttx information */
		pReg->LastTx.Length = copylen;
		NdisMoveMemory(pReg->LastTx.Data, pbuf, copylen);
	}
	...
	OUT	VOID *pbuf)
{
	...
	PWSC_REG_DATA		pReg;
	UCHAR				KDK[32] = {0}, KdkInput[38], KdfKey[80] = {0};
#ifndef RT_CFG80211_SUPPORT
	UCHAR				DHKey[32];
#endif /* RT_CFG80211_SUPPORT */
	INT					DH_Len;
	...
	if ((DH_Len != sizeof(pReg->SecretKey)) &&
		...
		DH_Len += DiffCnt;
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Do zero padding!\n");
	}
#ifndef RT_CFG80211_SUPPORT
	...
	if ((pAdapter->pHmacData) && (HmacLen <= 2048)) {
	...
	Len   += templen;
	/* Copy the content to Regdata for lasttx information */
	pReg->LastTx.Length = (Len <= ARRAY_SIZE(pReg->LastTx.Data)) ? Len : ARRAY_SIZE(pReg->LastTx.Data);
	NdisMoveMemory(pReg->LastTx.Data, pbuf, pReg->LastTx.Length);
	pWscControl->WscRetryCount = 0;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "BuildMessageM2.\n");
	return Len;
}
...
#endif /* WSC_V2_SUPPORT */
	pWscControl->WscRetryCount = 0;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "BuildMessageM2D.\n");
	return Len;
}
	...
	OUT	VOID *pbuf)
{
	...
	/* */
	/* Generate PSK1 */
	WscGenPSK1(pAdapter, pWscControl, &TB[0], sizeof(TB));
...
#endif /* WSC_V2_SUPPORT */
	pWscControl->WscRetryCount = 0;
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "BuildMessageACK :\n");
	return Len;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	if (Length > ARRAY_SIZE(pReg->LastRx.Data)) {
		ret = WSC_ERROR_OOB_INT_READ_ERR;
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, "Not enough buffer\n");
		return ret;
	}
	...
	if (Length == 0 || Length > sizeof(WSC_REG_DATA)) {
		ret = WSC_ERROR_OOB_INT_READ_ERR;
		return ret;
	}
	...
	while (Length > 4) {
		...
		if (WscLen == 0 || WscLen > MAX_VIE_LEN) {
			ret = WSC_ERROR_OOB_INT_READ_ERR;
			return ret;
		}
		...
		case WSC_ID_DEVICE_PWD_ID:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "   WPS Registrar DPID %04x\n", pReg->SelfInfo.DevPwdId);

			if (get_unaligned((PUSHORT) pData) == WSC_DEVICEPWDID_DEFAULT) { /**(PUSHORT) pData) */
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Rx WPS           DPID PIN\n");
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Rx WPS           DPID PBC\n");
				pWscControl->RegData.SelfInfo.DevPwdId = cpu2be16(DEV_PASS_ID_PBC);
			} else
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "Rx WPS           DPID unsupport\n");
		...
		case WSC_ID_VENDOR_EXT:
			...
			if (pWscControl->WscV2Info.bEnableWpsV2) {
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM1 --> Version2 = %x\n", pReg->PeerInfo.Version2);
			}
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM1 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	OUT PWSC_REG_DATA pReg)
{
	...
	if (Length == 0 || Length > MAX_VIE_LEN) {
		ret = WSC_ERROR_OOB_INT_READ_ERR;
		return ret;
	}
	...
	while (Length > 4) {
		...
		case WSC_ID_VENDOR_EXT:
			...
			if (pWscControl->WscV2Info.bEnableWpsV2) {
				...
				WscParseV2SubItem(WFA_EXT_ID_VERSION2, pData, WscLen, &pReg->PeerInfo.Version2, &tmp_data_len);
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM2 --> Version2 = %x\n", pReg->PeerInfo.Version2);
			}
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM2 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		...
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_PUBLIC_KEY:
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM2D --> Receive WSC_ID_PUBLIC_KEY!! werid!\n");
			break;
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM2D --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM3 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	OUT PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			if (WscLen > 512) {
				...
				MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ApEncrSettings array size is not enough, require %d\n", WscLen);
				break;
			}
			...
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "M4 ApEncrSettings len = %d\n", EncrLen);
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM4 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (RTMPCompareMemory(pReg->RHash1, RHash, 32) != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM4 --> RHash1 not matched\n");
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM4 --> HMAC not match\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
		ret = WSC_ERROR_HMAC_FAIL;
	}
	...
	OUT PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			EncrLen = sizeof(pReg->ApEncrSettings);
			AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16), pReg->KeyWrapKey, sizeof(pReg->KeyWrapKey), IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "M5 ApEncrSettings len = %d\n", EncrLen);
			/* Parse encryption settings */
			WscParseEncrSettings(pAdapter, pReg->ApEncrSettings, EncrLen, pWscControl);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_SETTINGS))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_ENCR_SETTINGS));
			break;
	...
	if (RTMPCompareMemory(pReg->EHash1, EHash, 32) != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM5 --> EHash1 not matched\n");
...
#endif /*#ifndef RT_CFG80211_SUPPORT*/
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM5 --> HMAC not match\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
		ret = WSC_ERROR_HMAC_FAIL;
	}
	...
	OUT PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			EncrLen = sizeof(pReg->ApEncrSettings);
			AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16), pReg->KeyWrapKey, sizeof(pReg->KeyWrapKey), IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "M6 ApEncrSettings len = %d\n", EncrLen);
			/* Parse encryption settings */
			WscParseEncrSettings(pAdapter, pReg->ApEncrSettings, EncrLen, pWscControl);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_SETTINGS))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_ENCR_SETTINGS));
			break;
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM6 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (RTMPCompareMemory(pReg->RHash2, RHash, 32) != 0) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM6 --> RHash2 not matched\n");
		ret = WSC_ERROR_DEV_PWD_AUTH_FAIL;
		goto out;
	}
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM6 --> HMAC not match\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
		ret = WSC_ERROR_HMAC_FAIL;
	}
	...
	OUT PWSC_REG_DATA pReg)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			EncrLen = sizeof(pReg->ApEncrSettings);
			AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16), pReg->KeyWrapKey, sizeof(pReg->KeyWrapKey), IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "M7 ApEncrSettings len = %d\n", EncrLen);
#ifdef CONFIG_STA_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter) {
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM7 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM7 --> HMAC not match\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
		ret = WSC_ERROR_HMAC_FAIL;
	}
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			EncrLen = sizeof(pReg->ApEncrSettings);
			AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16), pReg->KeyWrapKey, sizeof(pReg->KeyWrapKey), IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "M8 ApEncrSettings len = %d\n", EncrLen);
		...
		default:
			MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM8 --> Unknown IE 0x%04x\n", WscType);
			break;
		}
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		MTWF_DBG(pAdapter, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "ProcessMessageM8 --> HMAC not match\n");
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT) KDK)), (UINT)cpu2be32(*((PUINT)(KDK + 4))));
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "calculated --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)&Hmac[0])), (UINT)cpu2be32(*((PUINT)&Hmac[4])));
		ret = WSC_ERROR_HMAC_FAIL;
	}
