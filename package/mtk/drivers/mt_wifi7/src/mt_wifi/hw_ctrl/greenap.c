	...
	BOOLEAN greenap_enter)
{
	...
	greenap_on_off.band_idx = hc_get_hw_band_idx(ad);
	...
	MAC_TABLE_ENTRY *entry)
{
	...
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_INFO,
 ...
 */
#ifdef DOT11_EHT_BE
static BOOLEAN greenap_rule_5(
	RTMP_ADAPTER * ad,
	MAC_TABLE_ENTRY *entry)
{
	BOOLEAN enetr_greenap = FALSE;
	enum eht_nss_mcs_bw u1PeerBw = EHT_NSS_MCS_80;

	if (entry->wdev->channel < 14 && WMODE_CAP_2G(entry->wdev->PhyMode)) {
		if ((entry->cap.ch_bw.he_ch_width & SUPP_40M_CW_IN_24G_BAND) == 0)
			u1PeerBw = EHT_NSS_MCS_20;
	}

	if (entry->MaxHTPhyMode.field.MODE == MODE_EHT) {
#ifdef TXBF_SUPPORT
		enetr_greenap = greenap_bf_check(ad, entry) ? TRUE : FALSE;
#else
		enetr_greenap = TRUE;
#endif /* TXBF_SUPPORT */
		if (u1PeerBw == EHT_NSS_MCS_20) {
			enetr_greenap &= ((entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_20].mcs0_7 <= 1)
					&& (entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_20].mcs8_9 <= 1));
		} else {
			enetr_greenap &= ((entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs0_9 <= 1)
					&& (entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs10_11 <= 1)
					&& (entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs12_13 <= 1));
		}
	}

	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_INFO,
			"enetr_greenap(%d),AP_e_i_BF(%d,%d),STA_eBF(%d)EHT(%d)BW(%d)MCS-NSS(0x%x,0x%x;0x%x,0x%x,0x%x)\n",
			enetr_greenap,
#ifdef TXBF_SUPPORT
			greenap_ap_ebf_cap(ad),
			greenap_ap_ibf_cap(ad),
			...
			0,
#endif /* TXBF_SUPPORT */
			(entry->MaxHTPhyMode.field.MODE == MODE_EHT) ? TRUE : FALSE,
			u1PeerBw,
			entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_20].mcs0_7,
			entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_20].mcs8_9,
			entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs0_9,
			entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs10_11,
			entry->cap.rate.tx_nss_mcs[EHT_NSS_MCS_80].mcs12_13
		);
...
#ifdef DOT11_EHT_BE
	enetr_greenap |= greenap_rule_5(ad, entry);
#endif
	return enetr_greenap;
}
	...
	BOOLEAN greenap_allow)
{
	...
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_INFO,
			 "band(%d), previous_greenap_active(%d),  greenap_allow(%d), greenap_enter(%d)\n",
			  ...
			  greenap_allow,
			  greenap_enter);
	greenap_update(ad, previous_greenap_active, (greenap_allow & greenap_enter));
}
...
VOID greenap_suspend(RTMP_ADAPTER *ad, UINT32 reason)
{
	...
	if (greenap_cap) {
		...
		greenap_update(ad, IsHcGreenAPActive(ad), FALSE);
...
VOID greenap_resume(RTMP_ADAPTER *ad, UINT32 reason)
{
	...
	if (!greenap || (band_idx == INVALID_WIFI_RAM_BAND_NUM))
		return;
...
{
	UCHAR   band_idx;
	BOOLEAN previous_greenap_active = IsHcGreenAPActive(ad);
	BOOLEAN greenap_cap = greenap_get_capability(ad);
	struct greenap_ctrl *greenap = &ad->ApCfg.greenap;
...
VOID greenap_show(RTMP_ADAPTER *ad)
{
	...
	RTMP_SPIN_LOCK(&greenap->lock);
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_INFO,
			 "\tGREENAP::greenap_cap(%d), greenap_allow(%d), greeap_suspend(%x), dbdc_mode(%d)\n",
...
VOID EnableAPMIMOPSv2(RTMP_ADAPTER *ad, struct greenap_on_off_ctrl *greenap_on_off)
{
	...
	HcSetGreenAPActive(ad, TRUE);
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_DEBUG,
...
VOID DisableAPMIMOPSv2(RTMP_ADAPTER *ad, struct greenap_on_off_ctrl *greenap_on_off)
{
	...
	HcSetGreenAPActive(ad, FALSE);
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_DEBUG,
		"DisableAPMIMOPSNew, 30xx reserve only one antenna\n");
}
	...
	IN struct greenap_on_off_ctrl *greenap_on_off)
{
	...
	if (wdev == NULL) {
		MTWF_DBG(ad, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_ERROR,
	...
	TxPinCfg &= 0xFFFFF3FF;
	HcSetGreenAPActive(ad, TRUE);
	MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_DEBUG, "Run with BW_20\n");
	...
	IN struct greenap_on_off_ctrl *greenap_on_off)
{
	...
	if ((ht_bw == BW_40) && (wdev->channel != 14)) {
		MTWF_DBG(NULL, DBG_CAT_HW, CATHW_GREENAP, DBG_LVL_DEBUG, "Run with BW_40\n");
