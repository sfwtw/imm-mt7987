...
static NTSTATUS hw_ctrl_flow_v2_open(struct WIFI_SYS_CTRL *wsys)
{
	...
	MTWF_DBG(ad, DBG_CAT_HW, CATHW_HDEV_CTRL, DBG_LVL_INFO,
		"wdev_idx=%d\n", wsys->wdev->wdev_idx);
...
static NTSTATUS hw_ctrl_flow_v2_link_up(struct WIFI_SYS_CTRL *wsys)
{
	...
	if (sta_rec->EnableFeature) {
		if (sta_rec->EnableFeature & STA_REC_BASIC_STA_RECORD_BMC_FEATURE) {
			oriEnableFeature = sta_rec->EnableFeature;
			oriWlanIdx = sta_rec->WlanIdx;

			sta_rec->EnableFeature &= ~STA_REC_BASIC_STA_RECORD_BMC_FEATURE;
			AsicStaRecUpdate(ad, sta_rec);

			sta_rec->EnableFeature = oriEnableFeature & ~STA_REC_BASIC_STA_RECORD_FEATURE;
			sta_rec->WlanIdx = sta_rec->WlanIdx2;
			AsicStaRecUpdate(ad, sta_rec);

			sta_rec->EnableFeature = oriEnableFeature;
			sta_rec->WlanIdx = oriWlanIdx;
		} else
			AsicStaRecUpdate(ad, sta_rec);

#ifdef CONFIG_6G_SUPPORT
		if (wdev && WMODE_CAP_6G(wdev->PhyMode)) {
			struct _MAC_TABLE_ENTRY *entry = entry_get(ad, wdev->tr_tb_idx);

			iob_mode = wlan_operate_get_unsolicit_tx_mode(wdev);
			if ((iob_mode == UNSOLICIT_TXMODE_NON_HT_DUP)
				&& IS_WCID_VALID(ad, wdev->tr_tb_idx))
...
static NTSTATUS hw_ctrl_flow_v2_connt_act(struct WIFI_SYS_CTRL *wsys)
{
	...
	* 1)if certification enable, directly do AsicStaRecUpdate for TGac/ax test;
