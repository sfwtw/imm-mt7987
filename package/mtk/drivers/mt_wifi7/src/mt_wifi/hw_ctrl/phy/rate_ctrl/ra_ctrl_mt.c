...
)
{
	...
	if (RATE_TABLE_AGBS(pTable))
		...
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_ERROR, "Not AGBS table!\n");
		return;
	}
...
)
{
	...
	if (RATE_TABLE_AGBS(pTable))
		...
		MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_ERROR, "Not AGBS table!\n");
		return;
	}
	...
	if (pRaCfg->ucRateAlg == RATE_ALG_AGBS) {
		if (RATE_TABLE_AGBS(*ppTable) == FALSE) {
			*ppTable = RateSwitchTableAGBS11B;
			MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_ERROR,
