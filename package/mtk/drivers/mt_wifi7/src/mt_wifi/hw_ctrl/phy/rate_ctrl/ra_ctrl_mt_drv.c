...
)
{
	...
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
		"sizeof CMD_BSSINFO_AUTO_RATE_CFG_T = %d\n",
		(UINT_32)sizeof(CMD_BSSINFO_AUTO_RATE_CFG_T));
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
...
)
{
	...
	pCmdStaRecAutoRate->ucBBPCurrentBW = GetCfgBw2RawBw(pRaEntry->ucBBPCurrentBW);
...
#ifdef CFG_RATE_ADJUST_PARAM_DEBUG
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
		"%s: sizeof CMD_STAREC_AUTO_RATE_T = %d\n",
		__func__, (UINT_32)sizeof(struct _STAREC_AUTO_RATE_T));
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
		"fgRaValid=%d\n", pCmdStaRecAutoRate->fgRaValid);
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
		"fgAutoTxRateSwitch=%d\n", pCmdStaRecAutoRate->fgAutoTxRateSwitch);
	MTWF_DBG(NULL, DBG_CAT_RA, CATRA_CFG, DBG_LVL_INFO,
	...
	MTWF_DBG(NULL, DBG_CAT_FW, CATFW_STAREC, DBG_LVL_NOTICE,
		"wcid(%d), PhyMode(%d), RateMode(%d), 256QAM(%d), AmpduF(%d), MaxPhyCfg: MODE(%u) Flag(%x) STBC(%u) GI(%u) BW(%u) ldpc(%u) MCS(%u) Nss(%u)\n",
		pRaEntry->u2Wcid,
		pCmdStaRecAutoRate->ucPhyMode, pCmdStaRecAutoRate->ucSupportRateMode,
		pCmdStaRecAutoRate->ucGband256QAMSupport, pCmdStaRecAutoRate->ucMaxAmpduFactor,
		pCmdStaRecAutoRate->MaxPhyCfg.MODE,
		pCmdStaRecAutoRate->MaxPhyCfg.Flags,
		pCmdStaRecAutoRate->MaxPhyCfg.STBC,
		pCmdStaRecAutoRate->MaxPhyCfg.ShortGI,
	...
	UINT8 u1ShortPreamble)
{
	...
	ret = UniCmdRAFixRateUpdateWoSta(pAd, u2Wcid, pFixedRateCfg, u1SpeEn, u1ShortPreamble);
#else
