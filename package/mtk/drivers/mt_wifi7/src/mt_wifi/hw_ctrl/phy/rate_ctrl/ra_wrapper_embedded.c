...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_DEBUG,
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	UINT8 u1BandIdx, u1StbcPriority;

	pPhyCtrl =  &pHwResource->PhyCtrl;
	pRadioCtrl =  &pPhyCtrl->RadioCtrl;

	u1BandIdx = hc_get_hw_band_idx(pAd);
	u1StbcPriority = pAd->CommonCfg.HeraStbcPriority;
...
INT Set_Hera_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0, s1Value = rstrtok(arg, "-"); s1Value && i < 4; s1Value = rstrtok(NULL, "-"), i++) {
...
static void set_eht_mcs_by_eap(u8 *eht_mcs, u8 eap_rx_mcs, u8 eap_tx_mcs)
{
	u8 rx_mcs, tx_mcs;

	rx_mcs = (*eht_mcs) & DOT11BE_MCS_NSS_MASK;
	if (((rx_mcs) > (eap_rx_mcs))) {
		(*eht_mcs) &= ~DOT11BE_MCS_NSS_MASK;
		(*eht_mcs) |= (eap_rx_mcs & DOT11BE_MCS_NSS_MASK);
	}
	tx_mcs = (((*eht_mcs) >> DOT11BE_MCS_NSS_SHIFT) & DOT11BE_MCS_NSS_MASK);
	if (tx_mcs > eap_tx_mcs) {
		(*eht_mcs) &= DOT11BE_MCS_NSS_MASK;
		(*eht_mcs) |= ((eap_tx_mcs & DOT11BE_MCS_NSS_MASK) << DOT11BE_MCS_NSS_SHIFT);
	}
}
/*----------------------------------------------------------------------------*/
/*!
...
#endif /* DOT11_N_SUPPORT */
#ifdef DOT11_HE_AX
	if (entry->wdev->eap.eap_hesuprate_en && IS_HE_STA(entry->cap.modes)) {
		struct _STA_TR_ENTRY *tr_entry = tr_entry_get(pAd, entry->wcid);
		struct he_mcs_info *mcs = &tr_entry->StaRec.he_sta.max_nss_mcs;
...
#ifdef DOT11_EHT_BE
	if (entry->wdev->eap.eap_ehtsuprate_en && IS_EHT_STA(entry->cap.modes)) {
		struct eht_support_mcs_nss *mcs = &entry->eht_support_mcs_nss;
		struct rate_caps *rate = &entry->wdev->eap.rate;
		INT i;

		if (entry->MaxHTPhyMode.field.BW == BW_20) {
			set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs0_7_nss, rate->tx_nss_mcs[EHT_NSS_MCS_80].mcs0_9, rate->rx_nss_mcs[EHT_NSS_MCS_80].mcs0_9);
			set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs8_9_nss, rate->tx_nss_mcs[EHT_NSS_MCS_80].mcs0_9, rate->rx_nss_mcs[EHT_NSS_MCS_80].mcs0_9);
			set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs10_11_nss, rate->tx_nss_mcs[EHT_NSS_MCS_80].mcs10_11, rate->rx_nss_mcs[EHT_NSS_MCS_80].mcs10_11);
			set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs12_13_nss, rate->tx_nss_mcs[EHT_NSS_MCS_80].mcs12_13, rate->rx_nss_mcs[EHT_NSS_MCS_80].mcs12_13);

			MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_INFO,
					 "%s, wcid %d: BW %d, mcs0_7_nss %x, mcs8_9_nss %x, mcs10_11_nss %x, mcs12_13_nss %x\n",
					  entry->wdev->if_dev->name, entry->wcid,
					  0, mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs0_7_nss, mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs8_9_nss,
					  mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs10_11_nss, mcs->eht_txrx_mcs_nss_20_only.max_tx_rx_mcs12_13_nss);
		} else {
			u8 maxbw_num = EHT_NSS_MCS_80;

			if (entry->MaxHTPhyMode.field.BW <= BW_80)
				maxbw_num = EHT_NSS_MCS_160;
			else if (entry->MaxHTPhyMode.field.BW <= BW_160)
				maxbw_num = EHT_NSS_MCS_320;
			else if (entry->MaxHTPhyMode.field.BW == BW_320)
				maxbw_num = EHT_MCS_NSS_BW_NUM;
			else
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_WARN,
					"%s, wcid %d: invalid BW %d!\n",
					entry->wdev->if_dev->name, entry->wcid, entry->MaxHTPhyMode.field.BW);

			for (i = EHT_NSS_MCS_80; i < maxbw_num; i++) {
				MTWF_DBG(pAd, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_INFO,
					 "Before update:\n %s, wcid %d: BW %d, mcs0_9_nss %x, mcs10_11_nss %x, mcs12_13_nss %x\n",
					  entry->wdev->if_dev->name, entry->wcid,
					  i, mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs0_9_nss, mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs10_11_nss,
					  mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs12_13_nss);

				set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs0_9_nss, rate->tx_nss_mcs[i].mcs0_9, rate->rx_nss_mcs[i].mcs0_9);
				set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs10_11_nss, rate->tx_nss_mcs[i].mcs10_11, rate->rx_nss_mcs[i].mcs10_11);
				set_eht_mcs_by_eap(&mcs->eht_txrx_mcs_nss[i-1].max_tx_rx_mcs12_13_nss, rate->tx_nss_mcs[i].mcs12_13, rate->rx_nss_mcs[i].mcs12_13);
...
#endif
#ifdef DOT11_EHT_BE
		|| pEntry->MaxHTPhyMode.field.MODE == MODE_EHT
#endif
		)
	...
	IN UINT_8 idx)
{
	...
	pEntry = entry_get(pAd, idx); /* point to information of the individual station */
...
#ifdef SW_CONNECT_SUPPORT
		/* NOTE: RA not support S/W STA, actually now is RA offload too */
	...
	IN PRTMP_ADAPTER pAd)
{
	ULONG i, TxTotalCnt;
	MAC_TABLE_ENTRY *pEntry;
	TX_STA_CNT1_STRUC StaTx1 = {0};
	TX_STA_CNT0_STRUC TxStaCnt0 = {0};
	...
	for (i = 0; i < pAd->MSTANum; i++)
		if ((pAd->StaCfg[i].wdev.DevInfo.WdevActive) && (pAd->StaCfg[i].PwrMgmt.bDoze)) {
			MTWF_DBG(pAd, DBG_CAT_PS, CATPS_LP, DBG_LVL_DEBUG,
				"H/W in PM4, return\n");
			return;
		}
...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_DEBUG,
		"sup_rate_len: %d,  ext_rate_len: %d\n", sup_rate_len, ext_rate_len);
	if (sup_rate_len > 0) {
		if (sup_rate_len <= MAX_LEN_OF_SUPPORTED_RATES) {
			...
			MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_WARN,
				"wrong SUPP RATES., Len=%d\n", sup_rate_len);
			sup_rate_len = 8;
		}
	}
	...
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_DEBUG,
		"sum_rate_len: %d,  sum_rate: ", sum_rate_len);
	for (idx = 0; idx < sum_rate_len; idx++)
		MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_DEBUG, "0x%x ", sum_rate[idx]);
	MTWF_DBG(pAd, DBG_CAT_RA, CATRA_CFG, DBG_LVL_DEBUG, "\n");
		...
		if (has_vht_cap && (vht_cap != NULL) && pDesired_ht_phy->bVhtEnable) {
			...
			for (j = nTxStream; j > 0; j--) {
				...
				case 2:
					...
					if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss2 < VHT_MCS_CAP_NA)
					 && (vht_cap->mcs_set.rx_mcs_map.mcs_ss2 == VHT_MCS_CAP_9)
					 && (pAd->CommonCfg.wifi_cert)
					 && (cfg_ht_bw == 0)) {
						pEntry->SupportVHTMCS2SS = 0;
						pEntry->SupportVHTMCS2SS |= 1 << 8;
					}
