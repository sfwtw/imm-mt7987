...
#define IS_BSS_COLOR_MANUAL_ACTIVE(pAd, band_idx) \
  (pAd->ApCfg.bss_color_cfg.bss_color_enable >= BSS_COLOR_VALUE_MIN \
				&& pAd->ApCfg.bss_color_cfg.bss_color_enable <= BSS_COLOR_VALUE_MAX)
#endif
...
enum dbg_action {
	...
	BSS_COLOR_DBG_DISABLE,
	BSS_COLOR_DBG_IGNORE_COLLISION,
	BSS_COLOR_DBG_MAX = BSS_COLOR_DBG_IGNORE_COLLISION
};
...
void deliver_bss_color(struct _RTMP_ADAPTER *ad, struct _BSS_INFO_ARGUMENT_T *bss_info, struct bss_color_ctrl *bss_color);
