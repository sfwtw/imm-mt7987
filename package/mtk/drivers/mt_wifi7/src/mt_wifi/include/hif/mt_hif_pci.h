...
#define FE_WIFI_RESET_DONE	BIT(11)
#define FE_WIFI_TARFFIC_DONE	BIT(12)
#define FE_WIFI_TARFFIC_DONE_FAIL BIT(13)
#define FE_WIFI_WDMA_FAIL_RESET	BIT(14)
#endif /* ERR_RECOVERY .*/
...
#define ERROR_DETECT_STOP_PDMA_PRE BIT(11)
...
typedef struct _PCI_HIF_T {
	...
	UINT8 swq_to_tx_ring[PACKET_TYPE_NUM][WMM_QUE_NUM];
...
struct bus_info {
	...
	const u32 l1_remap_addr;
	const u32 l1_remap_mask;
	const u32 l1_remap_shift;
	const u32 l1_remap_base_mask;
	const u32 l1_remap_base_addr;
	const u32 l1_remap_offset;
	const u32 l2_remap_addr;
	const u32 l2_remap_mask;
	const u32 l2_remap_shift;
	const u32 l2_remap_base_mask;
	const u32 l2_remap_base_addr;
	const u32 l2_remap_offset;
	u8 (*io_remap_is_l1_remap)(u32 *reg);
};
...
#ifdef MEMORY_SHRINK
VOID pci_free_fw_ring_mem(struct _RTMP_ADAPTER *pAd);
#endif
