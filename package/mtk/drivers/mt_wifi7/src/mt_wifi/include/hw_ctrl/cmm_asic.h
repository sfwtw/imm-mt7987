...
#define RRO_STEP_ONE_TIMEOUT_V1 512
#define RRO_FLUSH_ALL_TIMEOUT_V1 1024
#define RRO_STEP_ONE_TIMEOUT_V2 128
#define RRO_FLUSH_ALL_TIMEOUT_V2 256

#define RRO_RSS_MAP_1RING 0x0	/* CR default value */
#define RRO_RSS_MAP_4RING 0x32103210
...
BOOLEAN AsicMcastEntryDenyList(struct _RTMP_ADAPTER *pAd, UINT8 BssIdx, UINT8 entry_cnt, UINT8 add_to_list, UINT8 *pAddr, UINT8 *Prefix_list);
#endif
...
typedef struct _RTMP_ARCH_OP {
...
#ifdef DOT11_EHT_BE
	INT32 (*archUpdateDscbInfo)(struct _RTMP_ADAPTER *pAd, struct CMD_STATIC_PP_DSCB_T *dscb);
	INT32 (*archUpdateEMLOp)(struct _RTMP_ADAPTER *pAd, struct _STA_REC_CTRL_T *sta_rec_ctrl);
	INT32 (*archReqAt2lmRes)(struct _RTMP_ADAPTER *pAd, struct AT2LM_RES_REQ_CTRL_T *req, struct AT2LM_RES_RSP_CTRL_T *rsp);
	INT32 (*archReqNt2lm)(struct _RTMP_ADAPTER *pAd, struct NT2LM_REQ_CTRL_T *req);
	INT32 (*archReqReconfigRmLink)(struct _RTMP_ADAPTER *pAd, struct RECONFIG_RM_LINK_REQ_CTRL_T *req);
	INT32 (*archSetReconfigTmr)(struct _RTMP_ADAPTER *pAd, struct RECONFIG_SET_TMR_CTRL_T *reconfig_ctrl);
	INT (*archUpdateEmlsrMgmt)(struct _RTMP_ADAPTER *pAd, MAC_TX_INFO *info, struct _TX_BLK *tx_blk);
#endif
	...
	INT (*mlme_hw_tx)(struct _RTMP_ADAPTER *pAd, UCHAR *tmac_info, MAC_TX_INFO *info, union _HTTRANSMIT_SETTING *pTransmit, struct _TX_BLK *tx_blk);
#ifdef CONFIG_ATE
	INT32 (*ate_hw_tx)(struct _RTMP_ADAPTER *pAd, struct _TMAC_INFO *info, struct _TX_BLK *tx_blk);
#endif
	VOID (*write_tmac_info_fixed_rate)(struct _RTMP_ADAPTER *pAd,
...
#ifdef IGMP_SNOOPING_DENY_LIST
	BOOLEAN (*archMcastEntryDenyList)(RTMP_ADAPTER *pAd, UINT8 BssIdx, UINT8 entry_cnt, UINT8 add_to_list, UINT8 *pAddr, UINT8 *Prefix_list);
#endif
#ifdef IGMP_TVM_SUPPORT
	...
	VOID (*arch_txpower_boost_ctrl)(struct _RTMP_ADAPTER *pAd, CHAR cPwrUpCat, PUCHAR pcPwrUpValue);
	...
	BOOLEAN (*arch_txpower_boost_info_V2)(struct _RTMP_ADAPTER *pAd, enum POWER_BOOST_TABLE_CATEGORY_V2 ePowerBoostRateType);
#endif
	...
	VOID (*txd_post_process)(struct _RTMP_ADAPTER *pAd, UCHAR *tmac_info, MAC_TX_INFO *info, union _HTTRANSMIT_SETTING *pTransmit);
#ifdef ERR_RECOVERY
	INT (*ser_handler)(struct _RTMP_ADAPTER *pAd, u32 action, u32 status);
#endif /* ERR_RECOVERY */
#ifdef WF_RESET_SUPPORT
	VOID (*wf_subsys_reset)(struct _RTMP_ADAPTER *pAd);
	VOID (*heart_beat_check)(struct _RTMP_ADAPTER *ad);
#endif
#ifdef TXRX_STAT_SUPPORT
	UINT32 (*arch_get_mib_txrx_cnts)(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx);
	UINT32 (*arch_get_all_rate_cnts)(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx, UINT8 ucDirection);
	UINT32 (*arch_get_stbc_cnts)(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx);
	UINT32 (*arch_get_gi_cnts)(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx);
...
VOID asic_write_tmac_info_mgmt(struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev, UCHAR sub_type, UCHAR *tmac_buf, union _HTTRANSMIT_SETTING *beaconTransmit, ULONG frmLen);
