...
typedef VOID(*MSG_TO_HANDLER)(struct cmd_msg *msg);
...
typedef enum _ENUM_EXT_CMD_CR4_SET_ID_T {
	...
	WA_SET_OPTION_HW_PATH_HIF_VER = 0x2f,
...
enum CONTROL_FLAG_INIT_COMBINATION {
	...
	INIT_LEN_VAR_CMD_SET_AND_WAIT_RETRY_RSP = 0x2F,
...
#define SET_CMD_ATTR_RSP_HANDLER(attr, callback) ({	\
	...
	(attr).rsp.to_handler = UniCmdResultToRsp;	\
	})
#else
#define SET_CMD_ATTR_RSP_HANDLER(attr, callback) ({	\
	(attr).rsp.handler = (callback);	\
...
enum EXT_CMD_TYPE {
	...
	EXT_CMD_ID_IGMP_PER_BAND_SIZE_CMD = 0xE0,
	EXT_CMD_ID_IGMP_MWDS_CLONE_CMD = 0xE1,
#endif
...
struct GNU_PACKED _EXT_CMD_CR4_MULTI_QUERY_T {
	UINT32 u4Cr4QueryOptionArg0;
	UINT32 u4Cr4QueryOptionArg1;
	UINT32 u4Cr4QueryOptionArg2;
...
struct GNU_PACKED _EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_5X5_ALL_T {
	UINT8      ucPfmuProfileFormatId;
	UINT8      ucPfmuIdx;
	UINT8      ucBandIdx;
	UINT8      aucReserved[1];
...
typedef enum _TXBF_SND_CMD_ACTION {
	...
	BF_SND_CFG_INF,
	BF_SND_CFG_TXOP_SND,
	BF_SND_CFG_IBF_ABNRML_CTRL,
	BF_SND_CFG_IBF_SW_SOL
} TXBF_SND_CMD_ACTION;
...
typedef enum _HERA_METRIC_CMD_ACTION {
	...
	HERA_METRIC_SET_ENABLE,
	HERA_METRIC_SET_MU_INIT_PLY
...
struct GNU_PACKED MEC_SET_AMSDU_MAX_SIZE_STA_T {
	UINT16 u2WlanIdx;
	UINT8 u1Reserved[2];
};
...
typedef struct GNU_PACKED _CMD_MEC_CTRL_CMD_T {
	...
	union MEC_CMD_PARA {
		...
		struct MEC_SET_AMSDU_MAX_SIZE_STA_T mec_set_amsdu_max_size_t;
	} mecCmdPara;
...
#define ACTION_SET_ENGINE       3
#define ACTION_GET_ENGINE       4
/* OPMODE */
...
/* ATTRIBUTE */
#define ATTRIBUTE_ICAP_SET 0
#define ATTRIBUTE_ICAP_GET 1
...
typedef enum {
	...
	TX_DPD_FLATNESS_CAL_A5 = 0x30000000, /* ubit 28 and bit 29 represent PerCH 5G Cal*/
	TX_DPD_FLATNESS_CAL_A5_MEM = 0x50000000, /* bit 28 and bit 30 represent PerCH 5G Mem Cal*/
	TX_DPD_FLATNESS_CAL_A6 = 0x70000000, /* bit 28,bit 29 and bit 30 represent PerCH 6G Cal */
	TX_DPD_FLATNESS_CAL_A6_MEM = 0x90000000, /*  bit 28 and bit 31 represent PerCH 6G Mem Cal*/
...
typedef enum _TWT_AGRT_CTRL_CODE_T {
	...
	TWT_AGRT_CTRL_AGRT_ALLOC = 7,
	TWT_AGRT_CTRL_AGRT_FREE = 8
...
#define SET_RTWT_ID(_ctrl, btwt_id) \
	((_ctrl->peer_id_grp_id) = ((TWT_BTWT_ID_BIT | TWT_RTWT_ID_BIT) | btwt_id))

#define IS_RTWT_ID(_ctrl) \
	(((_ctrl->peer_id_grp_id) & TWT_RTWT_ID_BIT) ? TRUE : FALSE)
...
enum {
	...
	STA_REC_HE_BASIC = 25,	/* 0x19 */
	STA_REC_HDR_TRANS = 26,
	STA_REC_RCA1 = 27,
	STA_REC_BASIC_STA_RECORD_BMC = 28,	/* 0x1c */
	STA_REC_MLD_SETUP = 32,	/* 0x20 */
	STA_REC_EHT_MLD = 33,		/* 0x21 */
	STA_REC_EHT_BASIC = 34,	/* 0x22 */
	STA_REC_MLD_TEARDOWN = 35,	/* 0x23 */
	STA_REC_WTBL_PSM = 36,	/* 0x24 */
	STA_REC_BMC_WTBL = 37,	/* 0x23 */
	STA_REC_MLR_INFO = 45,	/* 0x2d*/
	STA_REC_A4_INFO = 0x31,
	STA_REC_MAX_NUM = 0x36
};
...
enum {
	...
	STA_REC_HE_BASIC_FEATURE = (1 << STA_REC_HE_BASIC),
	STA_REC_HDR_TRANS_FEATURE = (1 << STA_REC_HDR_TRANS),
	STA_REC_RCA1_FEATURE = (1 << STA_REC_RCA1),
	STA_REC_BASIC_STA_RECORD_BMC_FEATURE = (1 << STA_REC_BASIC_STA_RECORD_BMC),
	STA_REC_MLD_SETUP_FEATURE = ((UINT64)1 << STA_REC_MLD_SETUP),
	STA_REC_EHT_MLD_FEATURE = ((UINT64)1 << STA_REC_EHT_MLD),
	STA_REC_EHT_BASIC_FEATURE = ((UINT64)1 << STA_REC_EHT_BASIC),
	STA_REC_MLD_TEARDOWN_FEATURE = ((UINT64)1 << STA_REC_MLD_TEARDOWN),
	STA_REC_BMC_WTBL_FEATURE = ((UINT64)1 << STA_REC_BMC_WTBL),
	STA_REC_MLR_INFO_FEATURE = ((UINT64)1 << STA_REC_MLR_INFO),
	STA_REC_A4_INFO_FEATURE = ((UINT64)1 << STA_REC_A4_INFO),
	STA_REC_MAX_NUM_FEATURE = ((UINT64)1 << STA_REC_MAX_NUM),
};
...
#define STAREC_COMMON_EXTRAINFO_MEDIATEK	BIT(3)
...
struct GNU_PACKED _STAREC_PSM_T {
	UINT16 u2Tag;       /* Tag = 0x2a(unicmd) / 0x24 */
	UINT16 u2Length;
	UINT8 ucPsmMode;
	UINT8 acuReserve[1];
};
...
enum {
	...
	BSS_INFO_MLD_FEATURE = (1 << BSS_INFO_MLD),
...
enum _ENUM_MURU_SET_MDVT_INFO_T {
	MDVT_RLM_SET_USR_MCS = 1,
	MDVT_RLM_SET_USR_NSTS = 2,
	MDVT_RLM_SET_USR_RU_ALLOC = 3,
	MDVT_RLM_SET_UL_LEN = 4,
	MDVT_RLM_SET_START_SS = 5,
	MDVT_RLM_SET_MU_MIMO_SPATIAL = 6,
	MDVT_RLM_SET_FIX_BAND = 7,
	MDVT_RLM_SET_MLO_EN_CTRL = 8,
	MDVT_RLM_SET_ICS_TO_HOST = 9,
	MDVT_RLM_SET_ADD_DEV_FRM_ATE = 10,
	MDVT_RLM_SET_REAL_STA_CNT = 11,
	MDVT_RLM_SET_SUBCASE_RANGE = 12,
	MDVT_RLM_SET_STA_CNT = 13,
	MDVT_RLM_CLR_FIX_CMM = 14,
	MDVT_RLM_SET_STEP_RUN = 15,
	MDVT_RLM_SET_RECAL_BW = 16,
	MDVT_RLM_SET_BF_TXOP_SND = 17,
	MDVT_RLM_SET_BF_PROTECT_EN = 18,
	MDVT_RLM_SET_BF_DIRECT_FBK_EN = 19,
	MDVT_RLM_SET_BF_SEG_EN = 20,
	MDVT_RLM_SET_BF_DATA_TYPE = 21,
	MURU_CMD_SET_MDVT_UPPER_IDX
};
...
typedef struct GNU_PACKED _CMD_OFFLOAD_BCN_BTWT_T {
	UINT16	u2SubTag;		/* SubTag(SUB_TAG_BCN_BTWT) = 0x4 */
	UINT16	u2Length;
...
/**
 * struct peer_mld - mld information of peer
 *
 * @mld_primary_idx: h/w primary index from hwifi
 * @mld_secondary_idx: h/w secondary index from hwifi
 */
struct peer_mld {
	u32 mld_primary_idx;
...
enum ENUM_SEC_CIPHER_ID_T {
	...
	SEC_CIPHER_ID_BCN_PROT_CMAC_128 = 12,
	SEC_CIPHER_ID_BCN_PROT_CMAC_256 = 13,
	SEC_CIPHER_ID_BCN_PROT_GMAC_128 = 14,
	SEC_CIPHER_ID_BCN_PROT_GMAC_256 = 15,
	SEC_CIPHER_ID_BIP_GMAC_128 = 16,
	SEC_CIPHER_ID_BIP_GMAC_256 = 17
};
...
/*  WTBL Update Opeartion */
enum _ENUM_WTBL_OPERATION_T {
	WTBL_OPERATION_RESET_AND_SET = 0x01,
	WTBL_OPERATION_SET = 0x02,
	WTBL_OPERATION_QUERY = 0x03,
	WTBL_OPERATION_ALL_RESET = 0x04,
	WTBL_OPERATION_MAX_NUM
};
#endif
...
typedef struct _EVENT_LTE_SAFE_CHN_T {
	...
	UINT_32	u4TdmChannelBitmask[4];
	UINT_32	u4PwrChannelBitmask[4];
} EVENT_LTE_SAFE_CHN_T, *P_EVENT_LTE_SAFE_CHN_T;
#endif /* WIFI_MD_COEX_SUPPORT */

INT32 MtCmdGetWtblTxStat(struct _RTMP_ADAPTER *pAd, UINT32 u4Field, UINT16 ucWcid);
...
typedef enum _STA_STAT_EVENT_TYPE {
	...
	EVENT_PHY_TXRX_ADM_STAT = 0x04,
	EVENT_PHY_TXRX_AIR_TIME = 0x05,
	EVENT_PHY_TRX_MSDU_COUNT = 0x06,
#if defined(TXRX_STAT_SUPPORT) || defined(TR181_SUPPORT)
	EVENT_PHY_TX_DATA_RETRY_COUNT = 0x07,
...
#if defined(EAP_STATS_SUPPORT) || defined(CONFIG_MAP_SUPPORT)
...
typedef struct _EXT_EVENT_TX_STAT_RESULT_T {
	...
	UINT_16 u2StaNum;
	EXT_EVENT_ONE_TX_STAT_T rTxStatResult[EXT_CMD_GET_ALL_STA_STATS_NUM_PER_EVENT];
...
typedef struct _EXT_EVENT_RX_STAT_RESULT_T {
	UINT_8 u1PhyEventId;
	UINT_8 u1Reserved[3];
	EXT_EVENT_RX_STAT_T rRxStatResult[CFG_WIFI_RAM_BAND_NUM];
...
typedef struct _EXT_EVENT_TX_RATE_RESULT_T {
	...
	UINT_16 ucStaNum;
	ONE_TX_RATE_RESULT_T rAllTxRateResult[EXT_CMD_GET_ALL_STA_STATS_NUM_PER_EVENT];
...
typedef struct _EXT_EVENT_TXRX_AIRTIME_T {
	UINT_16 u2WlanId;
	UINT_32 u4WtblTxTime[MAX_TX_RX_ADM_NUM];
	UINT_32 u4WtblRxTime[MAX_TX_RX_ADM_NUM];
...
struct _EXT_EVENT_ID_GET_TEMP_ADC_T {
	UINT_8 u1ThermalCategory;
	UINT_8 u1Reserved[3];
	UINT32 u1temp;
	UINT32 u1adc;
};
...
struct _EXT_EVENT_EFUSE_FREE_BLOCK_V2_T {
	UINT8  ucFreeBlockNum;
	UINT8  ucVersion; /* 0: original format ; 1: modified format */
	UINT8  ucTotalBlockNum; /* Total Block */
	UINT8  uceFuseNum; /* number of A/D-die eFuse, 0: bank not exist */
};
...
enum _ENUM_UMAC_VOW_DRR_CFG_ITEM_T {
	ENUM_VOW_DRR_ALL_CFG_ITEM = 0x0,
	ENUM_VOW_DRR_BSSGROUP_CFG_ITEM = 0x1,
	ENUM_VOW_DRR_PRIORITY_CFG_ITEM = 0x2,
	ENUM_VOW_DRR_AC0_CFG_ITEM = 0x3,
	ENUM_VOW_DRR_AC1_CFG_ITEM = 0x4,
	ENUM_VOW_DRR_AC2_CFG_ITEM = 0x5,
	ENUM_VOW_DRR_AC3_CFG_ITEM = 0x6,
	ENUM_VOW_DRR_WMM_CFG_ITEM = 0x7,    /* For MT7615 E1 only */
	ENUM_VOW_DRR_BWCGROUP_CFG_ITEM = 0x8,
	ENUM_VOW_DRR_EXCLUDE_GROUP_CFG_ITEM = 0x9
};

typedef enum _ENUM_EXT_CMD_VOW_DRR_CTRL_ID_T {
	/* Type 1 */
	ENUM_VOW_DRR_CTRL_FIELD_STA_ALL           = ENUM_VOW_DRR_ALL_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_BSS_GROUP     = ENUM_VOW_DRR_BSSGROUP_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_PRIORITY      = ENUM_VOW_DRR_PRIORITY_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_AC0_QUA_ID    = ENUM_VOW_DRR_AC0_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_AC1_QUA_ID    = ENUM_VOW_DRR_AC1_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_AC2_QUA_ID    = ENUM_VOW_DRR_AC2_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_AC3_QUA_ID    = ENUM_VOW_DRR_AC3_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_WMM_ID        = ENUM_VOW_DRR_WMM_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_BWC_GROUP     = ENUM_VOW_DRR_BWCGROUP_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_STA_EXCLUDE_GROUP =  ENUM_VOW_DRR_EXCLUDE_GROUP_CFG_ITEM,
	ENUM_VOW_DRR_CTRL_FIELD_DBG               = 0xe,
...
} ENUM_RX_AT_BITWISE_SUB_FIELD_T, *P_ENUM_RX_AT_BITWISE_SUB_FIELD_T;
...
typedef struct GNU_PACKED _UNI_EVENT_TEST_TR_PARAM_T {
	/*fix field*/
	...
	*   TAG                           | ID  | structure
	*   ------------------------------|-----|--------------
	*   UNI_EVENT_TEST_TR_PARAM_CTRL  | 0x0 | UNI_EVENT_TEST_TR_PARAM_CTRL_T
	*/
} _UNI_EVENT_TEST_TR_PARAM_T, *P_UNI_EVENT_TEST_TR_PARAM_T;

enum _UNI_EVENT_TEST_TR_PARAM_TAG_T {
	UNI_EVENT_TEST_TR_PARAM_CTRL = 0,
};
...
typedef struct GNU_PACKED _UNI_EVENT_ATE_TEST_MODE_T {
	UINT16 u2Tag;
	UINT16 u2Length;
	UINT16 u2ParamIdx;
	UINT8 aucReserved[2];
	UINT_8 aucAteResult[0];
} UNI_EVENT_ATE_TEST_MODE_T, *P_UNI_EVENT_ATE_TEST_MODE_T;
...
typedef struct _TXD_FR_TABLE_INFO_T {
	/* DW0 */
	uint16_t                u2FixedRate;

	/* DW1 */
	uint8_t                 ucFrAntIdx;
	uint8_t                 ucFrSpeIdxSel;
	uint8_t                 ucFrSpeIdx;
	uint8_t                fgFrGI;
	uint8_t                 ucFrHeLtf;
	uint8_t                fgFrLDPC;
	uint8_t                fgFrTBF;
	uint8_t                fgFrDynBw;
} TXD_FR_TABLE_INFO_T, *P_TXD_FR_TABLE_INFO_T;

typedef struct _UNI_CMD_TEST_FRTBL_CFG_T {
	int8_t ucTblIdx;
	int8_t ucReserved;
	struct _TXD_FR_TABLE_INFO_T rFrTblInfo;
	uint8_t aucReserved[4];
} UNI_CMD_TEST_FRTBL_CFG_T, *P_UNI_CMD_TEST_FRTBL_CFG_T;

typedef struct _UIN_CMD_TEST_PREAMBLE_PUNC_CFG_T {
	uint8_t ucDbdcIdx;
	uint8_t fgEnable;
	uint16_t u2PreamblePuncBitmap;
} UIN_CMD_TEST_PREAMBLE_PUNC_CFG_T, *P_UIN_CMD_TEST_PREAMBLE_PUNC_CFG_T;
...
typedef struct GNU_PACKED _EXT_CMD_FRAME_OFFLOAD_T {
    UINT8 ucOwnMacIdx;
    UINT8 ucEnable;
	UINT8 ucWlanIdx;	/* #256STA for non-uni cmd */
    UINT8 ucBandIdx;
	UINT8 ucTxType;
	UINT8 ucTxMode;			/* SU/MU/SU+MU */
	UINT8 ucTxInterval;		/* in unit of TU */
...
#ifdef WIFI_UNIFIED_COMMAND
	UINT16 u2Wcid;		/* adpated for uni-cmd */
#endif

	UINT8 acPktContent[2048]; /* Eagle project max in-band cmd size */
} CMD_FRAME_OFFLOAD_T, *P_CMD_FRAME_OFFLOAD_T;
...
struct MT_MGMT_FRAME_OFFLOAD_T {
	UINT16 wcid;
	UINT8 bssinfo_idx;
	UINT8 token;
	/* mlo */
...
struct CMD_SET_RED_TX_REPORT {
	UCHAR cmd;
	UCHAR *buffer;
	UINT16 Length;
};
struct CMD_FwLog2Host {
	UCHAR McuDest;
	UINT32 FWLog2HostCtrl;
};
...
struct CMD_STAT_EVENT_TYPE {
	UINT32 eventTypeBitmap;
};
...
#define SER_ENABLE_L4_RRO_LEAK_RECOVER	BIT(9)
...
#define SER_SET_ETH_HANDLER_RECOVER	8
#define SER_SET_L4_RRO_LEAK_RECOVER 9

#ifdef MTK_FE_RESET_RECOVER
#define SER_FE_STOP 0xffff0000
#define SER_FE_START 0xffff0001
#endif
...
/*MAC INFO ID:  Get TSF (0x06)*/
struct EXTRA_ARG_TSF_DIFF_T {
	UINT8  ucBssIdx0;
	UINT8  ucBssIdx1;
	UINT8  aucReserved[2];
};
...
typedef union {
	...
	struct EXTRA_ARG_TSF_DIFF_T TsfDiffArg;
...
struct _TEST_RX_STAT_BAND_INFO_EXT1 {
	/* mac part */
	UINT_32 mac_rx_u2m_mpdu_cnt;
	/* phy part */
	UINT_8 u1Reserved[4];
};
...
typedef struct _TEST_RX_STAT_COMM_INFO {
	...
	UINT32 ne_var_db_all_user;
} TEST_RX_STAT_COMM_INFO, *P_TEST_RX_STAT_COMM_INFO;

struct _TEST_RX_STAT_ALL_INFO {
	struct _TEST_RX_STAT_BAND_INFO rx_band_info;
	struct _TEST_RX_STAT_BAND_INFO_EXT1 rx_band_info_ext;
	struct _TEST_RX_STAT_PATH_INFO rx_path_info[5];
	struct _TEST_RX_STAT_USER_INFO rx_user_info[16];
	struct _TEST_RX_STAT_COMM_INFO rx_comm_info;
};
...
struct _TESTMODE_STATISTIC_INFO_ALL {
	struct _TESTMODE_STATISTIC_INFO_BAND rx_info_band;
	struct _TESTMODE_STATISTIC_INFO_PATH rx_info_path;
	struct _TESTMODE_STATISTIC_INFO_USER rx_info_user;
	struct _TESTMODE_STATISTIC_INFO_COMM rx_info_comm;
};
...
enum ENUM_MIB_COUNTER_T {
	MIB_CNT_CCA_NAV_TX_TIME = 0,
	MIB_CNT_P_ED_TIME = 2,
	MIB_CNT_MAC2PHY_TX_TIME = 3,
	MIB_CNT_TX_DUR_CNT = 6,
	MIB_CNT_RX_DUR_CNT = 8,
	MIB_CNT_BA_CNT = 19,
	MIB_CNT_RX_FCS_ERR = 47,
	MIB_CNT_MDRDY = 152,
	MIB_CNT_AMDPU_RX_COUNT = 153,
	MIB_CNT_RX_TOTAL_BYTE = 154,
	MIB_CNT_RX_MPDU = 157,
	MIB_CNT_RX_FIFO_OVERFLOW = 159,
	MIB_CNT_RX_OUT_OF_RANGE_COUNT = 160,
	MIB_CNT_DELIMITER_FAIL = 161,
	MIB_CNT_LEN_MISMATCH = 162,
	MIB_CNT_BCN_TX = 194,
	MIB_CNT_P_CCA_TIME = 215,
	MIB_CNT_S_CCA_TIME = 216,
	MIB_CNT_CHANNEL_IDLE = 239,
	RMAC_CNT_OBSS_AIRTIME = 499,
	RMAC_CNT_NONWIFI_AIRTIME = 500

};
/*MacInfo ID: 0x04 EDCA*/
typedef struct _TX_AC_PARAM_T {
	UINT8 ucAcNum; /* AC Queue Index */
	UINT8 ucVaildBit;
	UINT8 ucAifs;
	UINT8 ucWinMin;
	UINT16 u2WinMax;
...
struct AT2LM_RES_REQ_CTRL_T {
	UINT8 aucMldaddr[MAC_ADDR_LEN];
	UINT8 ucFwMldIdx;		/*WM viewpoint*/
	UINT8 ucFlag;
	UINT8 ucAt2lmId;
	UINT8 ucBssInfoIdx;
	UINT8 ucEnMstTimer;
...
struct BPCC_BEACON_CT_SWITCH {
	struct wifi_dev *wdev;
	UINT32 switch_type;
	union {
		UINT32 *UpdateReason;
		struct _EDCA_PARM *APEdcaParm;
		struct _BCN_MUEDCA_UPDATE_PARAM *APMUEdcaParam;
		struct _BCN_BSSCCA_UPDATE_PARAM *APBSSCCAParam;
	};
	UINT8 content[128];
};

struct NT2LM_REQ_CTRL_T {
	UINT8 aucSelfMldaddr[MAC_ADDR_LEN];	/* self mld addr */
	UINT8 aucPeerMldaddr[MAC_ADDR_LEN];	/* peer mld addr */
	UINT8 ucMldIdx;
	UINT16 u2LinkIdBitmap;		/* which link_id tidmap is valid */
	UINT16 u2LinkIdToWcid[MLD_LINK_MAX];
	UINT8 aucTidmapDl[MLD_LINK_MAX];
	UINT8 aucTidmapUl[MLD_LINK_MAX];
};

struct RECONFIG_RM_LINK_REQ_CTRL_T {
	UINT8 aucMldaddr[MAC_ADDR_LEN];	/* AP MLD addr */
	UINT8 ucFwMldIdx;		/*WM viewpoint*/
	UINT8 ucFlag;
	UINT16 u2RmLinkIdBitmap;		/* which link_id tidmap is valid */
	UINT16 u2LinkIdBssInfoIdx[MLD_LINK_MAX];
};

struct RECONFIG_SET_TMR_CTRL_T {
	UINT8 aucMldAddr[MAC_ADDR_LEN];
	UINT8 ucFwMldIdx;
	UINT8 ucFlag;
...
struct MT_BSS_RECONF_SM {
	struct wifi_dev *wdev;
	UINT32 reconfig_state;
};
...
#ifdef ZERO_PKT_LOSS_SUPPORT
struct GNU_PACKED _UNI_CMD_SET_ZERO_PKT_LOSS_VARIABLE_T {
	UINT_16 u2Tag;
	UINT_16 u2Length;
...
struct GNU_PACKED _UNI_CMD_SET_MAC_TX_ENABLE_T {
	UINT_16 u2Tag;
	UINT_16 u2Length;
	BOOLEAN fgMacTXEnable;
	UINT_8 ucBandIdx;
	UINT_8 aucReserve[3];
};

struct GNU_PACKED _UNI_CMD_SET_WTBL_SKIP_TX_ENABLE_T {
	UINT_16 u2Tag;
	UINT_16 u2Length;
	UINT_16 u2WlanIdx;
	UINT_8  ucOperation;
...
struct GNU_PACKED _EXT_CMD_CFG_SET_RTS_SIGTA_EN_T {
	UINT16  u2Tag;
	UINT16  u2Length;
...
struct GNU_PACKED _EXT_CMD_CFG_SET_SCH_DET_DIS_T {
	UINT16  u2Tag;
	UINT16  u2Length;
...
struct GNU_PACKED _EXT_CMD_CFG_SET_RTS0_PKT_THRESHOLD_CFG_T {
	UINT16  u2Tag;
	UINT16  u2Length;
	BOOLEAN Enable;   /* 0: Disable, 1: Enable */
	/* useless : old cmd path fix coverity */
...
typedef enum _EXT_CMD_CFGINFO_TAG_T {
	...
	EXT_CMD_CFGINFO_11V_BCN_DUP_CFG,
	EXT_CMD_CFG_MAX_NUM,
...
typedef enum _ENUM_CFG_FEATURE {
	...
	CFGINFO_11V_BCN_DUP_CFG_FEATURE = (1 << EXT_CMD_CFGINFO_11V_BCN_DUP_CFG),
...
struct _EXT_EVENT_GET_CR4_MULTI_TX_STATISTICS_T {
	UINT16 number;
	UINT8 reserved[2];
...
typedef struct GNU_PACKED _EXT_CMD_ID_MCAST_MWDS_CLONE_POLICY {
	UINT8 uIgmpType;
	UINT8 uMcastMwdsClonePolicy;
	UINT16 u2Reserve;
} EXT_CMD_ID_MCAST_MWDS_CLONE_POLICY_T, *P_EXT_CMD_ID_MCAST_MWDS_CLONE_POLICY_T;
...
typedef struct GNU_PACKED _EXT_CMD_ID_MULTICAST_SN_DENY_LIST {
	...
	UINT8 ucAddToList;
	UINT8 ucReserve;
	UINT8 deny_list[IGMP_DENY_TABLE_SIZE_MAX][IPV6_ADDR_LEN];
	UINT8 Prefix_list[IGMP_DENY_TABLE_SIZE_MAX];
...
typedef struct GNU_PACKED _EXT_CMD_ID_IGMP_PER_BAND_SIZE_CMD {
	UINT8 enable;
	UINT8 Rsvd[3];
	UINT32 uCfgNum[3];
} EXT_CMD_ID_IGMP_PER_BAND_SIZE_CMD_T, *P_EXT_CMD_ID_IGMP_PER_BAND_SIZE_CMD_T;
...
struct CMD_SET_PER_PKT_TX_POWER_T {
	UINT8 u1PowerCtrlFormatId;
	INT8 i1PowerOffset;
	UINT8 u1Reserved[2];
	UINT8 u1BandIdx;
...
#define BACKOFF_PARAM_NUM    494 /*support 5T format*/

struct _BACKOFF_TABLE_TYPE {
	INT_8 i1PwrLimit[BACKOFF_PARAM_NUM];
};

/* TxPower BACKOFF Table Info */
struct _EXT_EVENT_TXPOWER_BACKOFF_T {
	UINT_8 u1TxPowerCategory;
	UINT_8 u1BandIdx;
	UINT_8 u1ChBand;
	BOOLEAN fgTxPwrBackOffEn;

	/* BackOff Tbl info */
	struct _BACKOFF_TABLE_TYPE rBackOffTblInfo;
	UINT_8 u1Reserved3[3];
};
...
struct _UNI_EVENT_ID_FAST_PATH_T {
    /* fixed field */
	UINT_8 aucPadding[4];
...
enum _UNI_EVENT_ID_FAST_PATH_TAG_T {
	UNI_EVENT_FAST_PATH_PROCESS  = 0,
	UNI_EVENT_ID_FAST_PATH_MAX_NUM
};

struct _UNI_EVENT_FAST_PATH_PROCESS_T {
	UINT_16 u2Tag;
	UINT_16 u2Length;
	UINT_16 u2Mic;
	UINT_8 ucKeynum;
	UINT_8 u4KeybitmapMatchStatus;
	CAP_FAST_PATH_T cap;
};
...
typedef struct GNU_PACKED _EXT_EVENT_FASTPATH_RPT_T {
	...
	UINT8	u1Seq;
	UINT8	u1Reserved0[1];
	UINT16	u2FreeTokenUnderflowCnt[CFG_WIFI_RAM_BAND_NUM];
...
INT32 MtCmdFdFrameOffloadSet(struct _RTMP_ADAPTER *pAd, P_CMD_FRAME_OFFLOAD_T fdFrame_offload);
...
#ifdef DOT11_HE_AX
#ifdef WIFI_TWT_SUPPORT
INT32 mt_cmd_ext_twt_agrt_update(struct _RTMP_ADAPTER *ad, struct MT_TWT_AGRT_PARA_T *agrt_para);
#endif /* WIFI_TWT_SUPPORT */
#endif /* DOT11_HE_AX */
...
INT32 MtCmdSetTestEngine(struct _RTMP_ADAPTER *pAd, UINT32 id, UINT32 data);
INT32 MtCmdGetTestEngine(struct _RTMP_ADAPTER *pAd, UINT32 id, UINT32 data, UINT32 *ret_data);
...
INT32 MtCmdEepromAccessRead(struct _RTMP_ADAPTER *pAd, UINT32 offset, PUCHAR pData, PUINT isVaild, UINT32 DataLen);

VOID MtCmdEepromAccessWrite(struct _RTMP_ADAPTER *pAd, UINT32 offset, PUCHAR pData, UINT32 DataLen);
...
INT32 MtCmdCr4MultiQuery(struct _RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 arg1, UINT32 arg2, struct _CR4_QUERY_STRUC *cr4_query_list);
...
INT32 CmdMcastAllowMwdsCloneEnable(struct _RTMP_ADAPTER *pAd, UINT8 Msg_type, BOOLEAN Enable);
...
INT32 CmdExtRtsThenCtsRetryCnt(struct _RTMP_ADAPTER *pAd, UINT16 u2WlanIdx, UINT8 u1Ac, UINT8 u1RtsFailThenCtsRetryCnt);
...
INT SetRaOption(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
INT32 ShowHwcfgInfo(struct _RTMP_ADAPTER *pAd, uint16_t dump_offset, uint16_t dump_size, uint8_t *epprom_content);
...
INT32 MtCmdSetDpdFlatnessCal_7916(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length,  BOOLEAN bSecBw80);
...
INT32 MtCmdSetDpdFlatnessCal_7986(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length,  BOOLEAN bSecBw80);
#endif /* PRE_CAL_MT7986_SUPPORT */
#ifdef PRE_CAL_MT7981_SUPPORT
INT32 MtCmdSetGroupPreCal_7981(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length);
INT32 MtCmdSetDpdFlatnessCal_7981(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length,  BOOLEAN bSecBw80);
#endif /* PRE_CAL_MT7981_SUPPORT */
#ifdef PRE_CAL_BELLWETHER_SUPPORT
INT32 MtCmdSetGroupPreCal_bellwether(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length);
INT32 MtCmdSetDpdFlatnessCal_bellwether(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length, BOOLEAN bSecBw80);
#endif /* PRE_CAL_BELLWETHER_SUPPORT */
#ifdef PRE_CAL_MT7990_SUPPORT
INT32 MtCmdSetGroupPreCal_7990(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length);
INT32 MtCmdSetDpdFlatnessCal_7990(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length, UINT32 eeprom_ofst, UINT32 u4CalId);
#endif /* PRE_CAL_MT7990_SUPPORT */
#ifdef PRE_CAL_MT7992_SUPPORT
INT32 MtCmdSetGroupPreCal_7992(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length);
INT32 MtCmdSetDpdFlatnessCal_7992(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length, UINT32 eeprom_ofst, UINT32 u4CalId);
#endif /* PRE_CAL_MT7992_SUPPORT */
#ifdef PRE_CAL_MT7993_SUPPORT
INT32 MtCmdSetGroupPreCal_7993(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length);
INT32 MtCmdSetDpdFlatnessCal_7993(struct _RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length, UINT32 eeprom_ofst, UINT32 u4CalId);
...
INT32 mt_cmd_get_rx_stat_all(struct _RTMP_ADAPTER *pAd, UCHAR band_idx, struct _TEST_RX_STAT_ALL_INFO *prx_stat_all);
#ifndef WIFI_UNIFIED_COMMAND
...
#else
INT32 mt_uni_cmd_get_rx_stat_path(struct _RTMP_ADAPTER *pAd, UCHAR path_idx, UCHAR band_idx, TESTMODE_STATISTIC_INFO_PATH *rx_stat_path);
#endif
...
#ifdef DOT11_EHT_BE
/* static PP DSCB bitmap */
struct GNU_PACKED UNI_CMD_DSCB_CTRL_T  {
	UINT16 u2Tag;  /* Tag = 0x1E */
	UINT16 u2Length;
	UINT8  ucBandIdx;
	UINT8  ucOmacIdx;
	UINT8  fgIsEhtOpPresent;
	UINT8  fgIsEhtDscpPresent;
	UINT16 u2EhtDisSubChanBitmap;
	UINT8  ucEhtCtrl;
	UINT8  ucEhtCcfs0;
	UINT8  ucEhtCcfs1;
	UINT8  aucPadding[3];
...
struct GNU_PACKED CMD_STATIC_PP_DSCB_T {
	UINT8  ucBandIdx;
	UINT8  ucOmacIdx;
	UINT8  fgIsEhtOpPresent;
	UINT8  fgIsEhtDscpPresent;
...
struct GNU_PACKED UNI_CMD_PP_MU_CTRL  {
	UINT16 u2Tag;
	UINT16 u2Length;
	UINT8  ucBssIdx;
	UINT8  PpMuBitmap;
...
struct GNU_PACKED CMD_PP_MU_CTRL_T {
	UINT8  ucBssIdx;
	UINT8  PpMuBitmap;
	UINT8  value;
};

struct GNU_PACKED UNI_CMD_CHANGE_PP_CAP_CTRL  {
	UINT16 u2Tag;
	UINT16 u2Length;
	UINT32  value1;
	UINT32  value2;
...
struct GNU_PACKED CMD_CHANGE_PP_CAP_CTRL_T {
	UINT32  value1;
	UINT32  value2;
};
#endif
