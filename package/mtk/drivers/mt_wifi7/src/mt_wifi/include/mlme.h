...
#define KR      6
#define BAND4	7
#define MAX_RD_REGION 8
#define BEACON_LOST_TIME            (4 * OS_HZ)    /* 2048 msec = 2 sec */

#define DLS_TIMEOUT                 1200      /* unit: msec */
#define AUTH_TIMEOUT                300       /* unit: msec */
#define ASSOC_TIMEOUT               1500       /* unit: msec */
...
//TGX_TEST
struct GNU_PACKED MAX_CHANNEL_SWITCH_TIME_IE {
	UCHAR		ID;
	UCHAR		Length;
	UCHAR		ID_EXTENTION;
	UINT16		SwitchTime;
...
#define WIDE_BW_CH_SWITCH_ELEMENT_LEN 3
struct GNU_PACKED WIDE_BW_CH_SWITCH_IE {
	UCHAR id;
	UCHAR len;
	UINT8 bw;
	UINT8 ccfs0;
...
struct GNU_PACKED MLO_CSA_INFO {
	BOOLEAN Ch_Switching;
	UCHAR reporting_band;
	UCHAR new_channel;
	UCHAR apidx;
	EXT_CH_SW_ANN_INFO ExtChSwAnnIE;
	CH_SW_ANN_INFO ChSwAnnIE;
	struct MAX_CHANNEL_SWITCH_TIME_IE MCST_info;
	WIDE_BW_CH_SWITCH_ELEMENT wb_info;
};
...
#define SEQ_SUB(_SEQ1, _SEQ2, _Limit)		(((_SEQ1)-(_SEQ2)) & (_Limit))
#define SEQ_INC(_SEQ1, _SEQ2, _Limit)		(((_SEQ1)+(_SEQ2)) & (_Limit))
...
enum IE_FILTER_RULE {
	IE_FILTER_RULE_DISAPPEAR	= 0,
	IE_FILTER_RULE_MODIFY		= 1,
	IE_FILTER_RULE_APPEND		= 2,
	IE_FILTER_RULE_INVALID		= 3
};

struct ie_filter_node {
	struct ie_filter_node *pNext;
	UINT16 eid;
	UCHAR frm;
	UCHAR action;
	UCHAR ie_len;
...
enum IE_FILTER_MODE {
	IE_FILTER_DISABLE	= 0,
	IE_FILTER_ENABLE_ALL	= 1,
	IE_FILTER_ENABLE_RULE	= 2,
	IE_FILTER_INVALID	= 3
};

struct ie_filter {
	NDIS_SPIN_LOCK ie_filter_lock;
	struct _LIST_HEADER ie_filter_list;
	UCHAR filter_mode;
};
...
struct mbssie_parse_info {
	u8	nontx_bss_num;
	u8	nontx_bssid[MAX_BEACON_NUM][MAC_ADDR_LEN];
	u8	nontx_ssid[MAX_BEACON_NUM][MAX_LEN_OF_SSID];
	u16	nontx_ssid_Len[MAX_BEACON_NUM];
};
#endif
...
#define HAS_EHT_CAPS_EXIST(exist)       ((exist) & EHT_CAPS_EXIST)
#define HAS_EHT_OP_EXIST(exist)         ((exist) & EHT_OP_EXIST)
#define HAS_EHT_MLD_EXIST(exist)        ((exist) & EHT_MLD_EXIST)
#define HAS_RNR_EXIST(exist)            ((exist) & RNR_EXIST)
#define HAS_EHT_ML_RECONFIG_EXIST(exist) ((exist) & EHT_ML_RECONFIG_EXIST)
...
#define SET_EHT_CAPS_EXIST(exist)       ((exist) |= EHT_CAPS_EXIST)
#define SET_EHT_OP_EXIST(exist)         ((exist) |= EHT_OP_EXIST)
#define SET_EHT_MLD_EXIST(exist)        ((exist) |= EHT_MLD_EXIST)
#define SET_RNR_EXIST(exist)            ((exist) |= RNR_EXIST)
#define SET_EHT_ML_RECONFIG_EXIST(exist) ((exist) |= EHT_ML_RECONFIG_EXIST)
...
#define CLR_HE_6G_CAP_EXIST(exist)      ((exist) &= (~HE_6G_CAP_EXIST))
...
#define CLR_EHT_CAPS_EXIST(exist)       ((exist) &= (~EHT_CAPS_EXIST))
#define CLR_EHT_OP_EXIST(exist)         ((exist) &= (~EHT_OP_EXIST))
#define CLR_EHT_MLD_EXIST(exist)        ((exist) &= (~EHT_MLD_EXIST))
#define CLR_RNR_EXIST(exist)            ((exist) &= (~RNR_EXIST))
#define CLR_EHT_ML_RECONFIG_EXIST(exist) ((exist) &= (~EHT_ML_RECONFIG_EXIST))

struct ie_multi_link_sta_profile_info {
	uint8_t ctrl;
	uint8_t link_addr[MAC_ADDR_LEN];
};

struct GNU_PACKED ie_multi_link_info {
	u16 ml_ctrl;
#ifdef DOT11_BE_D12
	uint8_t cmm_info_len;
#endif
...
struct multi_link_info {
	...
	uint8_t mld_addr[MAC_ADDR_LEN];
	uint8_t link_num;
	struct sta_profile sta_profiles[BSS_MNGR_MAX_BAND_NUM];
};

struct rnr_ml_param {
	uint8_t is_valid;
	uint8_t mld_id;/*0: reported in same mld; 1-254: mbss no tx bss mld id; 255: no mld dev*/
	uint8_t link_id;/*valid link id: 0-15*/
...
typedef struct _BSS_ENTRY {
	...
	UINT8	mbssid_index;
	CHAR	trans_ssid[MAX_LEN_OF_SSID];
	UCHAR	trans_bssid[MAC_ADDR_LEN];
#endif
#ifdef DOT11_HE_AX
	struct he_cap_ie he_caps;
	struct he_txrx_mcs_nss he_mcs_nss_160;
	...
	struct eht_cap_ie eht_caps;
	struct eht_op_ie eht_op;
	UCHAR basic_multi_link_ie[MAX_LEN_OF_MLIE];
	/*store all rnr and nerighbir ap informs ml_param_info*/
	struct rnr_ml_param rnr_ml_parms[MAX_MLD_GROUP_NUM];
#endif /* DOT11_EHT_BE */
...
struct raw_rssi_info {
	UCHAR raw_rssi[MAX_RSSI_LEN];
	UCHAR raw_snr;
	UCHAR Channel;
};

#ifdef DOT11V_MBSSID_SUPPORT
struct nonTx_bss_query {
	u32 request_bssidx_bitmap; /*for extension of SSID_LIST ie */
	u8 request_num;
};
#endif
typedef struct _MLME_QUEUE_ELEM {
	...
	struct wifi_dev *wdev;
#ifdef DOT11V_MBSSID_SUPPORT
	struct nonTx_bss_query multip_bss_info;
#endif
	void *Others;
	UINT16 RawChannel;
	BOOLEAN need_wait;
	RTMP_OS_COMPLETION mlme_ack_done;
	struct list_head list;
...
#endif
	void *EntryMem;
	struct list_head EntryFreeHead;
	struct list_head EntryInuseHead;
...
#ifdef DOT11_EHT_BE
	struct eht_nss_mcs tx_nss_mcs[EHT_MCS_NSS_BW_NUM];
	struct eht_nss_mcs rx_nss_mcs[EHT_MCS_NSS_BW_NUM];
#endif/*DOT11_EHT_BE*/
...
typedef struct _MLME_AUX {
	...
	RALINK_TIMER_STRUCT AuthTimerExt;
...
#ifdef APCLI_CFG80211_SUPPORT
	TIMER_FUNC_CONTEXT  AuthTimerFuncContextExt;
#endif /* APCLI_CFG80211_SUPPORT */
	...
	UINT8	mbssid_index;
	UCHAR	trans_bssid[MAC_ADDR_LEN];
	UCHAR	trans_ssid[MAX_LEN_OF_SSID];
#ifdef DOT11_EHT_BE
	u8 mld_id;
#endif
#endif
...
/* For bss color related event reports. 9.4.2.67.1, 802.11ax-2021 */
struct GNU_PACKED WNM_EVT_REPORT_ELEMENT {
	UCHAR   Element_ID;
	UCHAR   Length;
	UCHAR   EvtToken;
	UCHAR   EvtType;
...
struct common_ies {
...
#endif /*DOT11_HE_AX*/
#ifdef DOT11_EHT_BE
	uint8_t rnr_ie[MAX_LEN_OF_RNRIE];
	struct eht_cap_ie eht_caps;
	struct eht_op_ie eht_op;
	...
	struct eht_support_mcs_nss eht_support_mcs_nss;
	UINT8 *no_inherit_ie;
	UINT8 ml_ie[MAX_LEN_OF_MLIE];
	struct frag_ie_info ml_frag_info;
	UINT8 ml_ie_2nd[MAX_LEN_OF_MLIE];
	struct frag_ie_info ml_frag_info_2nd;
	UINT8 t2l_ie[MAX_LEN_OF_T2LIE];
#endif /* DOT11_EHT_BE */
};
...
typedef struct _IE_lists {
	...
	UCHAR rsneo_ie[MAX_LEN_OF_RSNIE];
	UCHAR rsneo_ie_len;
	UCHAR rsnxeo_ie[MAX_LEN_OF_RSNIE];
	UCHAR rsnxeo_ie_len;
...
#endif
#ifdef DOT11V_MBSSID_SUPPORT
	struct mbssie_parse_info mbss_info;
#endif
#if DOT11_EHT_BE
	struct ml_ie_info mlo_ie_info;
	struct ml_ie_info mlo_ie_reconfig;
#endif
} BCN_IE_LIST;
...
#endif
#ifdef RT_CFG80211_SUPPORT
VOID cfg80211_StateMachineInit(struct _RTMP_ADAPTER *pAd, STATE_MACHINE * S, STATE_MACHINE_FUNC Trans[]);
#endif
...
UINT8 *fragment_information_element(
	IN OUT UINT8 *ie_buf,
	IN OUT UINT16 *buf_len);

NDIS_STATUS defragment_information_element(
	IN UINT8 *ie_buf,
	IN UINT16 buf_len,
	OUT UINT8 *defrag_buf,
	OUT UINT16 *defrag_len);
...
VOID peak_adjust_txop(struct _RTMP_ADAPTER *pAd);
...
INT update_andlink_statistics(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *pEntry);
#endif /*(defined (ANDLINK_FEATURE_SUPPORT) && defined (ANDLINK_V4_0))*/
#endif	/* MLME_H__ */
