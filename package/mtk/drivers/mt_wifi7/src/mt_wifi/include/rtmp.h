...
#include "feature/txbf/mt_txbf_cal.h"
#endif /* MT_MAC */
#endif /* TXBF_SUPPORT */
...
/*API func*/
INT andlink_send_wifi_change_event(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, struct andlink_wifi_ch_info *wifi_ch_info);
...
#ifdef FORCE_NULL_TX
#define NULL_USE_ALTX_FLAG	0x20
#endif
...
#define	MAXSEQ          (0xFFF)
#define MAXPN           (0x0000FFFFFFFFFFFF)
...
typedef struct _MULTICAST_DENY_LIST_FILTER_TABLE {
	...
	NDIS_SPIN_LOCK MulticastDenyTabLock;
...
#define CONTD_PER_ERR_CNT		200				/* 200 * 300ms = 60sec*/
#define CONTD_TX_FAIL_CNT		(100 * 256)
struct tx_stats_monitor_threshold {
	UINT32 per_err_total;
...
#define IS_ASIC_CAP(_pAd, _caps)		((PD_GET_ASIC_CAP(_pAd->physical_dev) & (_caps)) != 0)
...
typedef struct _MT_MIB_BUCKET_ONE_SEC {
	UCHAR	Enabled;
	UINT32 ChannelBusyTime; /* Every second update once. primary channel only */
	UINT32 ChannelBusyTimeCcaNavTx; /* Every second update once. including CCA NAV TX time*/
	UINT32 OBSSAirtime;
	UINT32 MyTxAirtime;
	UINT32 MyRxAirtime;
	UINT32 EDCCAtime;
	...
	UINT32 RxCckMdrdyTime;
	UINT32 RxOfdmLgMixedMdrdyTime;
	UINT32 RxOfdmGreenMdrdyTime;
	UINT32 MyMac2phyTxTime;

	UINT32 MibBss0CtrlFrameCntTime;
	UINT32 MibCntAmpduTime;

	UINT32 MyAjustTxAirTime;
	UINT32 MyAjustRxAirTime;
	UINT32 AjustObssAirTime;
} MT_MIB_BUCKET_ONE_SEC, *PMT_MIB_BUCKET_ONE_SEC;

typedef struct _MT_MIB_COUNTER_STAT {
	UINT64 ObssAirtimeAcc[CFG_WIFI_RAM_BAND_NUM];/* RMAC.AIRTIME14 */
	UINT64 MyTxAirtimeAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR36 */
	UINT64 MyRxAirtimeAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR37 */
	UINT64 EdccaAirtimeAcc[CFG_WIFI_RAM_BAND_NUM];/* RMAC.AIRTIME13 */
	UINT64 CcaNavTxTimeAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR9 */
	UINT64 PCcaTimeAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR16 */
	UINT64 RxCckMdrdyTimeAcc[CFG_WIFI_RAM_BAND_NUM];
	UINT64 RxOfdmLgMixedMdrdyTimeAcc[CFG_WIFI_RAM_BAND_NUM];
	UINT64 RxOfdmGreenMdrdyTimeAcc[CFG_WIFI_RAM_BAND_NUM];
	UINT64 MyMac2phyTxTimeAcc[CFG_WIFI_RAM_BAND_NUM];

	UINT64 MibBss0CtrlFrameCntAcc[CFG_WIFI_RAM_BAND_NUM];
	UINT64 MibCntAmpduAcc[CFG_WIFI_RAM_BAND_NUM];

	ULONG UpdateSystemTime[CFG_WIFI_RAM_BAND_NUM];
#ifdef ZERO_PKT_LOSS_SUPPORT
	UINT32 BACountAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR31 */
	UINT32 MyMac2PhyTxTimeAcc[CFG_WIFI_RAM_BAND_NUM];/* MIB.M0SDR35 */
#endif
...
typedef struct _MT_MIB_BUCKET_MS {
	UCHAR	CurIdx;
	UCHAR	Enabled;
	UINT32 ChannelBusyTime[2];
	UINT32 ChannelBusyTimeCcaNavTx[2];
	UINT32 OBSSAirtime[2];
	UINT32 MyTxAirtime[2];
	UINT32 MyRxAirtime[2];
	UINT32 EDCCAtime[2];
	UINT32 PdCount[2];
	UINT32 MdrdyCount[2];

	UINT32 RxCckMdrdyTime[2];
	UINT32 RxOfdmLgMixedMdrdyTime[2];
	UINT32 RxOfdmGreenMdrdyTime[2];
	UINT32 MyMac2phyTxTime[2];

	UINT32 MibBss0CtrlFrameCnt[2];
	UINT32 MibCntAmpdu[2];

	ULONG UpdateSystemTime[2];
...
/*
	Some utility macros
*/
#define MCS_NSS_CAP(_pAd)			(&_pAd->mcs_nss)
...
#define REPORT_MGMT_FRAME_TO_MLME(_pAd, Wcid, _pFrame, _FrameSize, _Rssi0, _Rssi1, _Rssi2, _Rssi3, \
	_MinSNR, _channel, _OpMode, _wdev, _RxPhyMode, _Others, _RawChannel)\
	do {																					   \
		...
		MlmeEnqueueForRecv(_pAd, Wcid, &_rssi_info, _FrameSize, _pFrame, _OpMode, _wdev, _RxPhyMode, _Others, _RawChannel);   \
	} while (0)
...
typedef struct _COUNTER_802_11 {
	...
	LARGE_INTEGER APAckCount;
	LARGE_INTEGER APCliAckCount;
#ifdef MT_MAC
	...
	/* FW RX counter is accumulated, we need to record last value for counter reset. */
	LARGE_INTEGER LastRxFcsErrorCount;
	LARGE_INTEGER LastRxFifoFullCount;
	LARGE_INTEGER LastRxMpduCount;
#ifdef TXRX_STAT_SUPPORT
	LARGE_INTEGER LastRxTotByteCount;
	LARGE_INTEGER LastTxMgmtRetryCnt;
	LARGE_INTEGER LastRxDupDropCount;
	LARGE_INTEGER LastRxBaCnt;
	LARGE_INTEGER LastRTSTotalCnt;
	LARGE_INTEGER LastRTSFailureCount;
	LARGE_INTEGER LastMultipleRetryCount;
	LARGE_INTEGER LastACKFailureCount;
	LARGE_INTEGER LastTxAmpduCnt;
	LARGE_INTEGER LastTxAbortCnt;
	LARGE_INTEGER LastTxMpduRetryDropCnt;
	LARGE_INTEGER LastTxRtsDropCnt;
	LARGE_INTEGER LastTxCtrlCnt;
	LARGE_INTEGER LastRxBeaconCnt;
	LARGE_INTEGER LastRxPhy2MacLenMismatchCount;
	LARGE_INTEGER LastRxFcsOkCount;
	LARGE_INTEGER LastTxBaCnt;
	LARGE_INTEGER LastRxMpduInAmpduCount;
	LARGE_INTEGER LastRxDelimiterFailCount;
	LARGE_INTEGER LastRxOutOfRangeCount;
	LARGE_INTEGER LastRxFilterDropMpduCount;
...
#ifdef TXRX_STAT_SUPPORT
	/* TX section*/
	LARGE_INTEGER TxFragmentationCount; /* the fragmented frame txed cnt */
	LARGE_INTEGER TxManagementCnt;
	LARGE_INTEGER TxManagementBytes;
	LARGE_INTEGER TxBarCnt;
	LARGE_INTEGER TxBaCnt;
	LARGE_INTEGER TxPsPollCnt;
	LARGE_INTEGER TxDataNullCnt;
	LARGE_INTEGER TxQosNullCnt;
	LARGE_INTEGER TxAssocReqCnt;
	LARGE_INTEGER TxAssocRspCnt;
	LARGE_INTEGER TxReassocReqCnt;
	LARGE_INTEGER TxReassocRspCnt;
	LARGE_INTEGER TxDisassocCnt;
	LARGE_INTEGER TxAuthCnt;
	LARGE_INTEGER TxDeauthCnt;
	LARGE_INTEGER TxProbeReqCnt;
	LARGE_INTEGER TxProbeRspCnt;
	LARGE_INTEGER TxActionCnt;
	LARGE_INTEGER TxDataByte;
	LARGE_INTEGER TxDataRetryCnt;
	LARGE_INTEGER TxMgmtRetryCnt;
	LARGE_INTEGER TxTotalCnt; /* txed to air cnt */
	LARGE_INTEGER TxCtrlCnt;
	LARGE_INTEGER TxAbortCnt;
	LARGE_INTEGER TxRtsDropCnt;
	LARGE_INTEGER TxMpduRetryDropCnt;
	LARGE_INTEGER RTSTotalCnt;
	LARGE_INTEGER TxAmpduCnt;
	/* RX section*/
	LARGE_INTEGER RxFragmentationCount;    /* the fragmented frame recv cnt */
	LARGE_INTEGER RxManagementCnt; /* for all received mgmt frame */
	LARGE_INTEGER RxManagementBytes;
	LARGE_INTEGER RxCtrlCnt; /* for all received ctrl frame */
	LARGE_INTEGER RxNot2MDataCnt;
	LARGE_INTEGER RxNot2MMgmtCnt;
	LARGE_INTEGER RxNot2MCtrlCnt;
	LARGE_INTEGER RxBarCnt;
	LARGE_INTEGER RxBaCnt;
	LARGE_INTEGER RxPsPollCnt;
	LARGE_INTEGER RxDataNullCnt;
	LARGE_INTEGER RxQosNullCnt;
	LARGE_INTEGER RxAssocReqCnt;
	LARGE_INTEGER RxAssocRspCnt;
	LARGE_INTEGER RxReassocReqCnt;
	LARGE_INTEGER RxReassocRspCnt;
	LARGE_INTEGER RxDisassocCnt;
	LARGE_INTEGER RxAuthCnt;
	LARGE_INTEGER RxDeauthCnt;
	LARGE_INTEGER RxProbeReqCnt;
	LARGE_INTEGER RxProbeRspCnt;
	LARGE_INTEGER RxActionCnt;
	LARGE_INTEGER RxFcsOkCount;
	LARGE_INTEGER RxMpduInAmpduCount;
	LARGE_INTEGER RxDelimiterFailCount;
	LARGE_INTEGER RxOutOfRangeCount;
	LARGE_INTEGER RxFilterDropMpduCount;
	LARGE_INTEGER RxPhy2MacLenMismatchCount;
	LARGE_INTEGER RxDupDropCount; /* BSSID0-3 use the same value */
	LARGE_INTEGER RxDataByte;
	LARGE_INTEGER RxBeaconCnt;

	/* Crypto */
	LARGE_INTEGER RxDecryptionSuccessCount;
	LARGE_INTEGER RxWepDecErrCount;
	LARGE_INTEGER RxTkipDecErrCount;
	LARGE_INTEGER RxCcmpDecErrCount;
	LARGE_INTEGER RxCcmpReplayCount;
	LARGE_INTEGER RxTkipReplayCount;
	LARGE_INTEGER RxTkipMicErrCount;
	LARGE_INTEGER RxCcmpMicErrCount;
	LARGE_INTEGER RxWepExcludeCount;
	LARGE_INTEGER TKIPCounterMeasuresInvokedCnt;
	LARGE_INTEGER FourWayFailCount;
#endif /* TXRX_STAT_SUPPORT*/
#ifdef CCN67_BS_SUPPORT
	UINT32		ProbeWithHeldCount;
	UINT32		AuthWithHeldCount;
#endif
...
typedef struct _CHANNEL_TX_POWER {
	...
	USHORT IsOutBandCacDone;
	USHORT NOPSaveForRestore; /* NOP restore time, unit: sec */
...
#define CHANNEL_320M_CAP			0x200
#define CHANNEL_RDD_HIT				0x2000
#define CHANNEL_CAC_DONE			0x1000

	UINT Flags;

	ULONG cac_done_timestamp;

} CHANNEL_TX_POWER, *PCHANNEL_TX_POWER;

struct ml_info_event {
	UCHAR event_id; /* enum CFG80211_ML_EVENT */
	UCHAR mld_grp_idx;
	UCHAR link_id;
	UCHAR addr[MAC_ADDR_LEN];
};
...
struct Chan_Config {
	BOOLEAN chan_prio_valid;
	struct Chan_Prio ch_prio[MAX_NUM_OF_CHANNELS];
	BOOLEAN bw40_forbid;
	UCHAR boot_chan;
};
...
typedef struct _CHANNEL_CTRL {
	...
	/*channel priority*/
	UCHAR MaxChPrio;/*the max channel priority of ChList*/
	BOOLEAN NeedResetToCfgChPrio;/*flag to indicate if channel list need update from cfg file*/
	UCHAR CurChPrioOpOwner;/*the op type currently assigned to ch prio*/
	NDIS_SPIN_LOCK ChPrioOpLock;/*lock to protect channel priority*/
...
#ifdef CONFIG_6G_SUPPORT
	CHANNEL_TX_POWER ChList_6G_psc_scan[MAX_NUM_OF_CHANNELS];
	UCHAR ChListNum_6G_psc_scan;
#endif
	USHORT FreqList[MAX_NUM_OF_CHANNELS];
	UCHAR FreqListNum;

	/*store channel cfguration*/
	struct Chan_Config ch_cfg;
...
typedef struct _MLME_STRUCT {
	...
	/* TxS */
	STATE_MACHINE_FUNC TxsFunc[TXS_FUNC_SIZE];
	STATE_MACHINE TxsMachine;
...
#endif
#ifdef RT_CFG80211_SUPPORT
	STATE_MACHINE CfgSyncMachine;
	STATE_MACHINE_FUNC CfgSyncFunc[CFG80211_FUNC_SIZE];
#endif
...
/*	NDIS_SPIN_LOCK TaskLock; */
	...
	MLME_QUEUE HPQueue; /*Mlme High priority queue */
	MLME_QUEUE LPQueue; /*Mlme Low priority queue */
...
typedef struct GNU_PACKED _OID_BA_REC_ENTRY {
	...
	UCHAR rsv;
	UCHAR BufSize[8];
	enum REC_BLOCKACK_STATUS REC_BA_Status[8];
...
typedef struct GNU_PACKED _OID_BA_ORI_ENTRY {
	...
	UCHAR rsv;
	UCHAR BufSize[8];
	enum ORI_BLOCKACK_STATUS ORI_BA_Status[8];
...
enum STOP_BCN_TX_FLAG {
	STOP_BCN_TX_DISABLE = 0,
	STOP_BCN_TX_T2LM = 1,
	STOP_BCN_TX_NO_BCN = 2,
	STOP_BCN_TX_MAX_NUM = 4,
};
...
struct crit_upd_info {
	BOOLEAN crit_upd_flag;
	UINT32 crit_upd_bitmap;
	UINT32 bypass_bcn_seq_bmap;
};

struct csa_ie_info {
	UINT16 ml_ie_offset;
	UINT16 ml_csa_ie_offset;
	UINT16 csa_ie_offset_in_ml;
	UINT8 csa_bss_idx;
};

struct at2lm_ie_info_t {
	u8 vld_id; /*bitmap*/
	u16 ie_offset[BSS_MNGR_MAX_AT2LM_SET_NUM];
};

struct reconf_sub_info_t {
	UINT8  fw_bss_idx;	/* reference BSS to set Timer TBTT */
	UINT16 tmr_offset;	/* AP Removal Timer in Per-STA Profile */
	UINT8  rsvd[1];
};
struct reconf_ie_info_t {
	UINT8  num_offset;
	struct reconf_sub_info_t sub_info[48]; /* TODO: define size*/
};
...
#endif
#ifdef DOT11_EHT_BE
	UINT16 mlt_ie_offset;
	struct crit_upd_info cu_info;
	struct csa_ie_info csa;
	struct at2lm_ie_info_t at2lm_info;
	struct reconf_ie_info_t reconf_info;
#endif /* DOT11_EHT_BE */
	UCHAR stop_tx; /* stop beacon tx when wdev is active */
...
struct wifi_dev_ops {
	...
	BOOLEAN (*ieee_802_11_rx_bcn_dup)(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,  struct _RX_BLK *rx_blk);
...
enum fixed_swpath_mode {
	FORCE_SW_PATH_NONE = 0x0,
	FORCE_SW_PATH_TX = 0x1,
	FORCE_SW_PATH_RX = 0x2,
	FORCE_SW_PATH_ALL = 0x3
};
#endif
...
struct dpp_gas_frame_list {
	DL_LIST List;
	UCHAR sta_mac[MAC_ADDR_LEN];
	BOOLEAN dpp_gas_ongoing;
};
...
typedef struct _ap_6g_cfg {
	discov_iob		dsc_iob;
	/* move formation of RNR IE into BSS Manager
	discov_oob		dsc_oob;
	*/
...
#ifdef DOT11_EHT_BE
#define IS_APCLI_DISABLE_MLO(_wdev) ((_wdev)->apcli_mlo_diable == 1)
#endif

enum VHTOP_INFO_UPDATE_ST {
	VHTOP_INFO_UPDATE_SUC,
	VHTOP_INFO_UPDATE_FAIL
};

struct vht_opinfo_update_info {
	enum VHTOP_INFO_UPDATE_ST status;
	struct vht_opinfo vht_opinfo_orig;
	NDIS_SPIN_LOCK vht_opinfo_bpcc_lock;
};
/**
...
struct wifi_dev {
	...
	UINT16 tr_tb_idx2; /* index refer to BSS which this device belong (2nd) */
	UINT32 wdev_type;
	UINT32 client_num;
	USHORT PhyMode;
	UCHAR IsAXOnly;
	...
	struct _SECURITY_CONFIG SecConfig_ext; /* For rsn override */
	...
	UINT8 hostapd_peer_ml_ie[MAX_LEN_OF_MLIE];
	UINT8 do_not_send_sta_del_event_flag;
#endif
	...
	union _HTTRANSMIT_SETTING HTPhyMode, MaxHTPhyMode, MinHTPhyMode;
	...
	BOOLEAN bLinkUpDone;
	atomic_t bGotEapolPkt;
	struct _RX_BLK *pEapolPktFromAP;

	UCHAR mgmt_txd_txpwr_offset;
#ifdef MGMT_TXPWR_CTRL
	UCHAR mgmt_txd_txpwr_offset_cck[MODULATION_SYSTEM_CCK_NUM];
	UCHAR mgmt_txd_txpwr_offset_ofdm[MODULATION_SYSTEM_OFDM_NUM];
#endif
	/* 7636 psm */
	...
	/* BPCC for critical update*/
	/* lock to modify bpcc protect related parameter */
	NDIS_SPIN_LOCK BcnBPCCLock;

	/* task to run cn update instead of running it in interrupt mode */
	/* lock while running bcn update to prevent race condition */
	RTMP_OS_SEM BcnBPCC_op_lock;
	BOOLEAN BcnBPCC_op_lock_flag;
	UINT32 BcnBPCC_op_lock_by_bssmngr_fail_cnt;
	UINT32 BcnBPCC_op_lock_by_legacy_fail_cnt;
	UINT32 BcnBPCC_TO_cnt;
	UINT32 BcnBPCC_hwcmd_max;
	UINT32 BcnBPCC_op_lock_by_bssmngr_fail_cnt_per_IE_INT[32];
	UINT32 BcnBPCC_op_lock_by_bssmngr_fail_cnt_per_IE_CT[32];
	UINT32 BcnBPCC_TO_cnt_per_IE[32];
	UINT32 BcnBPCC_op_lock_refine_NG_per_IE_bitmap;
	UCHAR dbg_BcnBPCC_op_lock_caller[80];
	/* timer to unlock BcnBPCC_op_lock to prevent deadlock */
	RALINK_TIMER_STRUCT BcnBPCC_OP_Timer;
	ULONG BcnBPCC_OP_Timer_TT_start;
	ULONG BcnBPCC_OP_Timer_TT_cancel;
	ULONG BcnBPCC_op_lock_TT_start;
	ULONG BcnBPCC_op_lock_TT_end;
	ULONG BcnBPCC_op_lock_max;
	ULONG BcnBPCC_op_lock_avg;
	ULONG BcnBPCC_op_lock_mld_max;
	ULONG BcnBPCC_op_lock_mld_avg;
	ULONG BcnBPCC_op_lock_get_mld_max;
	ULONG BcnBPCC_op_lock_get_mld_avg;
	ULONG BcnBPCC_op_lock_get_legacy_max;
	ULONG BcnBPCC_op_lock_get_legacy_avg;
	ULONG BcnBPCC_op_lock_get_fail_mld_max;
	ULONG BcnBPCC_op_lock_get_fail_mld_avg;
	ULONG BcnBPCC_op_lock_get_fail_legacy_max;
	ULONG BcnBPCC_op_lock_get_fail_legacy_avg;

	/* timer to run bpcc update, eg EDCA */
	RALINK_TIMER_STRUCT BcnBPCC_Trigger_Timer;
	BOOLEAN BcnBPCC_trigger_flag;
	struct wifi_dev *BcnBPCC_trigger_wdev;
	BOOLEAN BcnBPCC_Init;
	BOOLEAN BcnBPCC_protect_dynctrl;
	UINT32 BcnBPCC_protect_dynctrl_cnt;
	BOOLEAN BcnBPCC_protect_disable;
	BOOLEAN BcnBPCC_protect_manual;
	ULONG BcnBPCC_Init_TT;
	UINT32 BcnBPCC_UpdScope;
	/* carry defer bcn update reason for timer usage */
	UINT8 BcnBPCC_UpdOriReason;
	/* bpcc ie type history for debug information */
	UINT32 BcnBPCCIeType_bitmap;
	UINT32 BcnBPCCIeType_bitmap_hist[20];
	ULONG BcnBPCCIeType_bitmap_hist_TT[20];
	UINT32 BcnBPCCIeType_cnt_hist[32];
	UINT32 BcnBPCCIeType_bitmap_hist_idx;
	BOOLEAN BcnBPCC_log_on;
	UINT8 BcnBPCC_mld_grp_id;
	BOOLEAN BcnBPCC_MLD_log_flag[64+1];

	struct vht_opinfo_update_info bpcc_vhtop_info;
	...
	*/
	...
	BOOLEAN wps_ie_flag;	/* Determine Peer AP's wps status for mwds */
#endif /* MWDS */
#ifdef BAND_STEERING
#ifdef CONFIG_AP_SUPPORT
	BOOLEAN bInfReady;
...
#ifdef RT_CFG80211_SUPPORT
	BOOLEAN cfg80211_bcn_rate;
	union _HTTRANSMIT_SETTING bcn_rate;
#endif
#ifdef CONFIG_MAP_SUPPORT
...
#endif
	BOOLEAN bMgmtQAutoRate;
#ifdef DOT11_EHT_BE
	UCHAR apcli_mlo_diable;
	BOOLEAN ap_mlo_disable;
#endif
/* station mld device */
	struct mld_dev *mld_dev;
	struct ml_info_event ml_event;
	UINT32 rx_drop_long_len;
#ifdef CONFIG_MAP_SUPPORT
	BOOLEAN bh_ready_sent;
#endif
	UINT64 multicast_cnt;

#ifdef CONFIG_VLAN_GTK_SUPPORT
	struct list_head vlan_gtk_list;
	UINT8 vlan_cnt;
#endif
	struct ie_filter ap_ie_filter;
#ifdef ANDLINK_V4_0
...
typedef struct _TXRX_STAT_BSS {
	...
	union _HTTRANSMIT_SETTING LastMulticastTxRate;
...
typedef struct _BSS_STRUCT {
...
#ifdef CCN67_BS_SUPPORT
	UINT8				iv_bb_enabled;
	UINT16				iv_bb_rssi_thresh;
	UINT32				is_auth_witheld;
	UINT32				is_probe_withheld;
#endif
#ifdef OCE_SUPPORT
	...
	*/
	...
	ULONG RxErrorCount;
	ULONG RxDropCount;
	UINT32 TxBytesAdm[MAX_ACCESS_CAT_NUM];
	UINT32 RxBytesAdm[MAX_ACCESS_CAT_NUM];
...
#endif
#ifdef DOT11_EHT_BE
	UCHAR mld_grp_idx;		/* MLD group index */
	BOOLEAN mld_grp_by_cfg;	/* Indicate MldGroup from profile or user cmd */
	UCHAR pf_mld_addr[MAC_ADDR_LEN]; /* MLD Address of mld_grp_idx, read from profile */
	BOOLEAN mld_addr_by_cfg; /* Indicate MLD addr read from profile */
	UCHAR tid_mapping;		/* TID [7:0] mapping */
	BOOLEAN unique_mld_addr_enable; /* Indicate unique MLD addr for multi link group */
	UCHAR multilink_mld_addr[MAC_ADDR_LEN]; /* unique MLD Address of mld_grp_idx */
#endif /* DOT11_EHT_BE */
...
#ifdef PRE_CFG_SUPPORT
struct PRECFG_CTRL PreCfgCtrl;
#endif /* PRE_CFG_SUPPORT */
#ifdef CONFIG_MAP_SUPPORT
	struct bh_assoc_disallow_info bh_disallow_info;
#endif
#ifdef DOT11V_MBSSID_SUPPORT
	struct mbss_11v_ctrl mbss_11v;
#endif /* DOT11V_MBSSID_SUPPORT */

#ifdef DOT11_EHT_BE
	enum bss_reconfig_state reconfig_state;
	RALINK_TIMER_STRUCT btm_timer;
	NDIS_SPIN_LOCK reconfig_state_lock;
	BOOLEAN reconfig_state_init;
	UINT32 reconfig_state_bitmap_bypass;
#endif
} BSS_STRUCT;
...
typedef struct _COMMON_CONFIG {
	...
	/* If bTpePSDsingleBW20Flag=TRUE,
	set TPE_PSD power count=0(only one power value for indicating all BW power value) */
	BOOLEAN bTpePSDsingleBW20Flag;
	...
	UINT8 ucTxPowerPercentage;	/* 0~100 % */

#ifdef TX_POWER_CONTROL_SUPPORT
	POWER_BOOST_PARA_V0 PowerBoostParamV0;
	POWER_BOOST_PARA_V1 PowerBoostParamV1;
#endif /* TX_POWER_CONTROL_SUPPORT */
#ifdef TX_POWER_CONTROL_SUPPORT_V2
	struct POWER_BOOST_PARA_V2 PowerBoostParamV2[POWER_UP_CATE_V2_NUM];
#endif /* TX_POWER_CONTROL_SUPPORT_V2 */

	UCHAR SKUenable;
	UCHAR SKU_DUP_Patch_enable;
	UINT8 SKUBandIdx;
	UINT8 SKUChannelBand;
	UINT8 SKUControlChannel;
	UINT8 SKUCentralChannel;
	UCHAR SKUTableIdx;
	CHAR  cTxPowerCompBackup[SKU_TABLE_SIZE][SKU_TX_SPATIAL_STREAM_NUM];
	UCHAR PERCENTAGEenable;
	UCHAR BFBACKOFFenable;
	UINT8 CCKTxStream;

#ifdef LINK_TEST_SUPPORT
	UCHAR LinkTestSupport;
	UCHAR LinkTestSupportTemp;
#endif /* LINK_TEST_SUPPORT */

	UINT8 u1EDCCACtrl;
	UINT8 u1EDCCAMode;
	UINT8 u1EDCCAThreshold[4];
	UINT8 u1EDCCACfgMode;
	...
	UINT8 ucTxPowerDefault;	/* keep for TxPowerPercentage */
	UINT8 PwrConstraint;
	...
	UINT8 SlotTime;	/* Slot time */
	BOOLEAN bEnableTxBurst;	/* 1: enble TX PACKET BURST (when BA is established or AP is not a legacy WMM AP), 0: disable TX PACKET BURST */
	BOOLEAN bIEEE80211H;	/* 1: enable IEEE802.11h spec. */
#ifdef PEAK_ENHANCE
	struct peak_enhance_ctrl peek_enhance;
#endif /* PEAK_ENHANCE */
#ifdef MAP_R3
	...
	UCHAR BssCoexApCnt;
	UCHAR BssCoexApMac[AP_MAC_CNT][MAC_ADDR_LEN];
	...
	BSS_COEX_SCAN_LAST_RESULT BssCoexScanLastResult;
	...
	*/
	...
	union _HTTRANSMIT_SETTING MCastPhyMode;
	union _HTTRANSMIT_SETTING MCastPhyMode_5G;
#else
	union _HTTRANSMIT_SETTING mcastphymode;
	...
	BOOLEAN HeraStbcPriority; /* STBC priority against eBF and iBF*/
	UINT8	MaxBfeeSS; /* Max bfee ss, for debuging*/
	UINT8	BfNDPARate; /* Set NDPA Rate */
#endif /* TXBF_SUPPORT */
#endif /* DOT11_N_SUPPORT */
	...
	UCHAR SRSDEnable;
	UCHAR SRDPDEnable;
	UCHAR DisSrBfrConnected;
	UCHAR fgSrApCliMode;
#ifdef CONFIG_MAP_SUPPORT
	UCHAR SRMeshUlMode;
#endif /* CONFIG_MAP_SUPPORT */
#endif /* CFG_SUPPORT_FALCON_SR */
	UINT32 SRmode_req_ifindex;/** SR_CMD_GET_SR_CFG_SUPPORT_MODE **/
#ifdef CFG_SUPPORT_FALCON_PP
	UCHAR pp_enable;
	USHORT punctured_bitmap;
#endif /* CFG_SUPPORT_FALCON_PP */
#ifdef DFS_VENDOR10_CUSTOM_FEATURE
	BOOLEAN bCh144Enabled;
#endif
#if defined(RT_CFG80211_SUPPORT) && defined(DOT11_EHT_BE)
	UINT8 mlo_preset_link_id;/*for supplicant preset the mlo link of gtk/igtk/bigtk*/
#endif
#ifdef ANDLINK_FEATURE_SUPPORT
	UINT8 andlink_enable;
	UINT8 andlink_ip_hostname_en;
	UINT32 andlink_simple_val;
	struct mtk_andlink_rate_conf andlink_uplink_rate_cfg[CFG_WIFI_RAM_BAND_NUM];
	struct mtk_andlink_rate_conf andlink_sta_rate_cfg[CFG_WIFI_RAM_BAND_NUM];
#endif/*ANDLINK_FEATURE_SUPPORT*/
#ifdef ACK_CTS_TIMEOUT_SUPPORT
	UCHAR ack_cts_enable;
	UINT32 distance;/*unit: m*/
	UINT32 cck_timeout;/*unit: us*/
	UINT32 ofdm_timeout;/*unit: us*/
	UINT32 ofdma_timeout;/*unit: us*/
#endif/*ACK_CTS_TIMEOUT_SUPPORT*/
#ifdef CFG_SUPPORT_FALCON_MURU
	UCHAR Dis160RuMu;
	UCHAR MaxRuOfdma;
...
#ifdef WIFI_MD_COEX_SUPPORT
	UINT16 powerBackoff; /* Power backoff in an amplifier */
#endif
#ifdef DOT11_EHT_BE
	...
	UINT8 AfcDeviceType;
	UINT8 AfcSpectrumType;
	BOOLEAN AcsAfterAfc;
	UINT8 AfcSpBwDup;
	INT8 AfcDGgainOffset;
#endif /*CONFIG_6G_AFC_SUPPORT*/
...
struct EXCLUDED_IES {
	...
	UCHAR extended_capa_len;
	UCHAR operating_class_len;
};
...
#endif /* WIFI_TWT_SUPPORT */
#endif /* DOT11_HE_AX */
	CHAR BcnRssiLast[MAX_RSSI_LEN];
	CHAR BcnRssiAvg[MAX_RSSI_LEN];
	struct _BSS_ENTRY *bss_entry;
#ifdef APCLI_SUPPORT
...
struct mld_entry_ctrl_t {
	NDIS_SPIN_LOCK mld_entry_lock;
	struct hlist_head mld_hash[HASH_TABLE_SIZE];
};

struct sta_msdu_pkt_cnt {
	ULONG txMsduDropCnt;
...
typedef struct _BSS_COLOR_CFG {
	UINT8 bss_color_enable;	/* 0:Bss color disable,255:random init bss color value, 1-63:Manual config bss color value */
	UINT8 bss_color_next;
	UINT8 rem_ap_bss_color_change_cnt;
	UINT8 bss_color_collision_ignore;
} BSS_COLOR_CFG;
#endif

struct ACS_CONFIG {
	UINT32 ACSCheckTime; /* Periodic timer to trigger Auto Channel Selection (unit: second) */
	UCHAR ChUtilThr; /* Periodic ACS trigger threshold: the channel util ratio by others */
	USHORT StaNumThr; /* Periodic ACS trigger threshold: the connected station number */
};
...
typedef struct _AP_ADMIN_CONFIG {
	...
	UINT32 dot11v_mbssid_bitmap;
	UCHAR dot11v_max_indicator[MAX_TX_BSS_CNT];
	UCHAR dot11v_BssidNum[MAX_TX_BSS_CNT];
#endif /* DOT11V_MBSSID_SUPPORT */
	...
	BOOLEAN mac_repeater_en; /* MAC Reoeater band related enable */
...
#endif /* BAND_STEERING */
#ifdef CCN67_BS_SUPPORT
	struct bs_cli_table BSTable;
	struct ieee80211req_bba_stats  btm_stat;
#endif
...
#endif /* OCE_SUPPORT */
#ifdef MGMT_TXPWR_CTRL
	UINT8 MgmtTxPwr; /* for 5G : 6M (OFDM) pwr  2.4G: 1M (CCK) pwr */
	UINT8 MgmtTxPwrCck[MODULATION_SYSTEM_CCK_NUM]; /*for CCK pwr*/
	UINT8 MgmtTxPwrOfdm[MODULATION_SYSTEM_OFDM_NUM]; /*for OFDM pwr*/
	UINT8 MaxMgmtTxpwr;
	UINT8 EpaFeGain;
#endif
#ifdef DATA_TXPWR_CTRL
	...
	UINT8 MinBaseTxPwr;
#endif
	UINT16 BandMaxStaNum;  /* Per-Band limit STA number*/
	UINT16 perBandStaCount; /* Per-Band count STA number*/
	...
	/* trigger by bcn offload and used by bcn/probe rsp/assoc rsp*/
	struct BTWT_BUF_STRUCT btwt;
#endif /* WIFI_TWT_SUPPORT */
#endif /* DOT11_HE_AX */
#ifdef DOT11_EHT_BE
	struct eht_period_link_recomm_t link_recom;
#endif /* DOT11_EHT_BE */
#ifdef CONFIG_MAP_SUPPORT
...
struct TABLE_SIZE_PER_BAND_SETTING {
	UCHAR enable;
	UINT cfg_num[MAX_BAND_NUM];
	UINT used_num[MAX_BAND_NUM];
};
...
typedef struct _MULTICAST_FILTER_TABLE {
	...
	USHORT Size;
#endif
	struct TABLE_SIZE_PER_BAND_SETTING size_cfg;
...
typedef enum _GREEN_AP_SUSPEND_REASON {
	...
	GREENAP_REASON_STA_ACTIVE_SCAN = (1 << 3),
...
typedef struct _SCAN_CTRL_ {
	...
	BOOLEAN opAbortScan; /* abort scan process*/
#ifdef CONFIG_6G_SUPPORT
	BOOLEAN psc_scan_en; /* enable 6G psc scan*/
#endif
	INT32 AvgNF;
	UINT64 busytime[MAX_NUM_OF_CHANNELS + 1];  /*scan channel busy time*/
	UINT64 tx_time[MAX_NUM_OF_CHANNELS + 1]; /*scan tx air time*/
	UINT64 rx_time[MAX_NUM_OF_CHANNELS + 1]; /*scan rx air time*/
	UINT64 bss_rx_time[MAX_NUM_OF_CHANNELS + 1]; /*other bss tx air time*/

	UINT64 pre_bss_rx_time;
	UINT64 pre_tx_time;
	UINT64 pre_rx_time;
	UINT64 pre_busytime;
	UINT64 active_time[MAX_NUM_OF_CHANNELS + 1];
	ktime_t  survey_time;
	INT32   ch_AvgNF[MAX_NUM_OF_CHANNELS + 1];
} SCAN_CTRL;
...
typedef struct _CFG80211_CONTROL {
...
#ifdef RT_CFG80211_SUPPORT
	/*mlme related*/
	UCHAR cfg80211_mlme_status; /*for some ops to do sync with other driver module*/
#endif
	...
	UINT16 mgmt_bit_mask;
#if defined(APCLI_CFG80211_SUPPORT)
	UINT16 mgmt_bit_mask_sta;
#endif /* APCLI_CFG80211_SUPPORT */
...
/* Rx Statistic */
#define MAX_ANT_NUM MAX_RSSI_LEN
...
typedef struct _RX_STAT_CNT {
	...
	UINT32 all_mac_rx_u2m_mpdu_cnt;
...
struct txop_ctl {
	...
	struct wifi_dev *spot_ap_wdev;
	struct wifi_dev *spot_sta_wdev;
	ULONG all_tx_bytes_ap;
	ULONG all_rx_bytes_ap;
	ULONG all_tx_pkts_ap;
	ULONG all_rx_pkts_ap;
	ULONG all_tx_bytes_sta;
	ULONG all_rx_bytes_sta;
	ULONG all_tx_pkts_sta;
	ULONG all_rx_pkts_sta;
};
...
#if defined(OFFCHANNEL_SCAN_FEATURE) || defined(TXRX_STAT_SUPPORT) || defined(ZERO_PKT_LOSS_SUPPORT)
...
#endif
#ifdef ZERO_PKT_LOSS_SUPPORT
	MT_MIB_COUNTER_STAT		PrevPeriodicStatStore;
	/*store stat value of previous onchannel periodic read*/
	MT_MIB_COUNTER_STAT     CurrentPeriodicChStat;
	/*channel stat for current on channel periodic read*/
...
#define CSI_RING_SIZE 1024
#define CSI_MAX_DATA_COUNT 1024		/*for bw320, at most 1024*/
#define CSI_MAX_RSVD1_COUNT 10
#define CSI_MAX_RSVD2_COUNT 10
#define Max_Stream_Bytes 4300
#define Max_Sta_Mac_Num 10	/*max filter station number*/
...
#define MAX_CSI_ACTIVE_STA_NUM 31

/*frame type bit offset
 *QOS data -> 34
 *bit[1 0 0 0 | 1 0]
 *1 0 0 0 -> subtype 3
 *1 0 -> frame type 1*/
#define GET_CSI_FRAME_TYPE(value)	(value & 0x3)
#define GET_CSI_SUBFRAME_TYPE(value)	((value & 0x3c)>>2)

/*active mode add or remove sta*/
#define CSI_ACTIVE_MODE_SHOW 2
...
#define CSI_IDLE 0
#define CSI_PROC 1
#define CSI_NETLINK 2

/*11ax will do 1/4 sample, so the tone number of 11ax is same with 11ac*/
#define CSI_BW20_TONE_NUM 64
#define CSI_BW40_TONE_NUM 128
#define CSI_BW80_TONE_NUM 256
#define CSI_BW160_TONE_NUM 512
#define CSI_BW320_TONE_NUM 1024
...
/*get rx mode or rx mcs rate*/
#define GET_CSI_RX_MODE(DATA) (DATA & BITS(0, 15))
#define GET_CSI_RATE(DATA) ((DATA & BITS(16, 31)) >> 16)
...
struct CSI_DATA_T {
	...
	INT_32 i4Rsvd3;
	UINT_8 ucRsvd4;
	UINT_32 chain_info;	/*show chain idx, total chain fot this pkt, pkt counter*/
	UINT_32 Tx_Rx_Idx;		/*show tx stream and rx stream idx of the packet*/
	UINT_32 u4SegmentNum;	/*idx for BW, for BW >= 160, it need be divided into N * BW80*/
	UINT_8 ucRemainLast;	/*set to 1: there is more BW80 transmiting */
	UINT_16 pkt_sn;		/*for debug, sniffer check*/
	UINT_8 tr_stream;	/*for debug, tx rx stream nuber, [0:3]-tx [4:7]-rx */
	INT_16 *ac2IData;	/*pointer to CSI I DATA*/
	INT_16 *ac2QData;	/*pointer to CSI Q DATA*/
};

struct CSI_MAX_DATA_T {
	UINT_8 ucBw;
	UINT_8 FWVer;
	UINT_16 u2DataCount;
	UINT_8 ucDbdcIdx;
...
struct CSI_INFO_T {
	...
	struct proc_dir_entry *data_proc;
	struct proc_dir_entry *debug_proc;
	/* CSI function config or status*/
	struct wifi_dev *csi_wdev;	/*store the wdev starting csi*/
	UINT_8 ucValue1[CSI_CONFIG_ITEM_NUM];
	UINT_8 ucValue2[CSI_CONFIG_ITEM_NUM];
	UINT_8 CSI_report_mode;
	UCHAR protocol_filter;	/*filter protocal mode set by user */
	/* Variable for manipulating the CSI ring buffer */
	UINT_8 FWVer;
	struct CSI_DATA_T arCSIBuffer[CSI_RING_SIZE];
	struct CSI_MAX_DATA_T rCSISegmentTemp;	/* for bw >=160, we need combine N * BW80*/
	...
	/*used by netlink mode*/
	struct genl_family *csi_genl_family;
	struct genl_ops *csi_genl_ops;
	struct nla_policy *csi_genl_policy;
	/*used by netlink auto report*/
	RALINK_TIMER_STRUCT csi_report_timer;
	UINT_32 report_interval;
	struct genl_info peer_info; /*store peer info and reply later*/
	/*used by filter mode -- timestamp filter*/
	UINT_32 usr_offset;	/*to filter data with specific TS => unit:us*/
	UINT_32 cur_TS;	/*store current ts in the pkt*/
	UINT_32 ExpTs_offset;
	/*active tx mode*/
	UINT_32 wcid_bitmap; /*choose which sta need do active tx*/
	UINT_32 active_interval; /*unit: ms*/
};
...
struct _DUMMY_WCID_OBJ {
	UCHAR State;
	UINT16 HwWcid;
	BOOLEAN bFixedRateSet;
	/* pre config fix rate table idx */
	UCHAR fr_tbl_idx;
	UCHAR fr_bw;
	/* reference count for the smae one hw resource */
	atomic_t ref_cnt;
	atomic_t connect_cnt;
	struct mtk_mac_sta *mac_sta;
};
#endif /* SW_CONNECT_SUPPORT */

#ifdef DFS_SLAVE_SUPPORT
struct DFS_SLAVE_CTRL {
	UINT8 enable;
	UINT8 disable_beacon;
	UINT8 disconnect_sta;
};
...
struct ch_chg_info {
	UCHAR old_channel;
	UCHAR old_bw;
	UINT_32 ch_chg_cnt;	/* Used for stats. */
};

struct pause_bitmap_ {
	/* per bands for legacy pause bitmap */
	UINT32  **pause;
	/* per bands for twt pause bitmap */
	UINT32  **twt_pause;
};
...
struct _RTMP_ADAPTER {
	...
	PNET_DEV net_dev; /* TODO: need to move to one card structure */
	RTMP_OS_FD_EXT srcf; /* declare to write fwlog file */
	...
	CHANNEL_BUSY_TIME ScanChnlStats;
	UINT8 OnChannelScanOngoing;
#endif
	...
	UINT8 ucRxStreamStatePrev;
	UINT8 ucRxFilterstate;
	UINT8 ucTxCsdState;
	UINT8 ucTxPwrBoostState;
	UINT8 ucLinkRcpiState;
	UINT8 ucLinkSpeState;
	UINT8 ucLinkSpeStatePrev;
	UINT8 ucLinkBwStatePrev;
	UINT8 ucTxCsdStatePrev;

	/* BW Control Paramter */
	UINT8   ucPrimChannel;
	UINT8	ucCentralChannel;
	UINT8	ucCentralChannel2;
	UINT8	ucExtendChannel;
	UINT8	ucHtBw;
	UINT8	ucVhtBw;
	...
	UINT8	u1SpeRssiIdxPrev;
	CHAR	cWBRssiTh;
	CHAR	cIBRssiTh;
	UINT8	u1RxStreamSwitchReason;
	UINT8	ucRxSenCount;
	...
	UINT8	ucCmwChannelBand;

	/* channel band Control Paramter */
	BOOLEAN fgChannelBandInfoUpdate;
	...
	/*======Cmd Thread for BPCC CT */
	struct _CmdQ BPCC_CmdQ;
	NDIS_SPIN_LOCK BPCC_CmdQLock;	/* BPCC_CmdQLock spinlock */
	RTMP_OS_TASK BPCC_cmdQTask;
	...
	struct _QUEUE_HEADER TxSwQueue[WMM_NUM_OF_AC];
#ifdef IP_ASSEMBLY
...
*/
	...
	/* Store B/G RSSI #0/1/2 Offset value on EEPROM 0x46h */
	signed char BGRssiOffset[MAX_RSSI_LEN];
	signed char ARssiOffset[MAX_RSSI_LEN]; /* Store A RSSI 0/1/2 Offset value on EEPROM 0x4Ah */
	...
	struct wifi_dev act_mon_wdev;
	BOOLEAN	bActionMonInit;
	BOOLEAN bActionMonOn;
#endif
#ifdef RT_CFG80211_SUPPORT
	...
	*/
...
#ifdef TXRX_STAT_SUPPORT
	struct COUNTER_TXRX_RATE_REPORT TxRxRateReport;
#endif /* TXRX_STAT_SUPPORT */
	...
	UINT8 mcsnoop_policy;
#endif
#ifdef IGMP_SNOOPING_DENY_LIST
	P_MULTICAST_DENY_LIST_FILTER_TABLE pMcastDLTable;
#endif
	UINT8 mcsnoop_mwds_clone_policy;
#endif /* IGMP_SNOOP_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
...
#define ONE_SECOND_NON_BE_PACKETS_CERT_THRESHOLD	(50)
#define ONE_SECOND_NON_BE_PACKETS_THRESHOLD		(200)
	UCHAR is_on;
...
#define FT_RIC_CB		((FT_RIC_CTRL_BLOCK *)(pAd->pFT_RIC_Ctrl_BK))
	...
	struct __CFG80211_CB *pCfg80211_CB;
#if defined(IWCOMMAND_CFG80211_SUPPORT) || defined(HOSTAPD_MBSS_SUPPORT)
	UINT CfgAPIfUseCnt;
	UINT CfgSTAIfUseCnt;
...
#define MAX_FW_DUMP_SIZE 1024000
	...
	UINT8  rts_retrylimit;
	UINT8  retrylimit[TxQ_IDX_AC33 + 1];
	UINT16 retrylimit_wcid;
	UINT8  cp_support;
	BOOLEAN  cp_support_off_by_qos_map;
	UINT8  dabs_qos_op;
...
#endif /* SMART_CARRIER_SENSE_SUPPORT */
#ifdef TXBF_SUPPORT
	struct bf_ctrl Bfctrl;
#endif /* TXBF_SUPPORT */
	RvR_Debug_CTRL RVRDBGCtrl;

	UINT16						   CCI_ACI_TxOP_Value;
...
#ifdef ANTENNA_DIVERSITY_SUPPORT
	struct ant_diversity_ctl diversity_ctl;
	UINT32 phy_rate_average;
	UINT32 rate_rec_sum;
	UINT32 rate_rec_num;
	UINT32 cn_rec_sum;
	UINT32 cn_rec_num;
	...
	BOOLEAN rec_start_cn_flag;
	BOOLEAN rec_start_rx_rate_flag;
	INT32 RSSI[MAX_ANT_NUM];
	INT32 RCPI[MAX_ANT_NUM];
#endif
...
#endif /* DPP_SUPPORT */
#if defined(MTK_HOSTAPD_SUPPORT) && defined(CONFIG_MAP_SUPPORT) && defined(MAP_R3)
	DL_LIST dpp_gas_event_list;
	BOOLEAN is_dpp_gas_list_init;
	NDIS_SPIN_LOCK gas_frame_list_lock;
#endif /* MTK_HOSTAPD_SUPPORT && CONFIG_MAP_SUPPORT && MAP_R3 */
#ifdef WIFI_DIAG
	void *pDiagCtrl;
#endif
...
/* For wifi and md coex in colgin project*/
	...
	struct COEX_IDC_INFO idcInfo;
#endif /* WIFI_MD_COEX_SUPPORT */

#ifdef ZERO_PKT_LOSS_SUPPORT
	struct _ZERO_LOSS_STA ZeroLossSta[3];
...
#endif
#endif/*DABS_QOS*/
	CHANNEL_OP_CTRL ChOpCtrl;

#ifdef WIFI_UNIFIED_COMMAND
	BOOLEAN RxvOfRxEnable;
	BOOLEAN RxvOfTxEnable;
...
#endif /* DELAY_TCP_ACK_V2 */
#ifdef DOT11_HE_AX
	struct bss_color_table color_tbl;
#ifdef WIFI_TWT_SUPPORT
	struct TWT_CTRL_T twt_ctrl;
#endif /* DOT11_HE_AX */
#endif /* WIFI_TWT_SUPPORT */
#ifdef CFG_SUPPORT_FALCON_SR
	struct sr_mesh_topology_update_params SrMeshTopologyUpdateParams;
	struct SR_MESH_SRG_BITMAP_T SrSelfSrgBM;
	struct SR_MESH_SRG_BITMAP_T SrSelfSrgBMMan;
	UINT_8 SrSelfSrgBMMode;
	UINT_8 SrMeshTopoLock;
	UINT_8 SrMeshUlMode;
	UINT_8 StaModeRptUnLock;
#endif
#ifdef MAP_R3
	BOOLEAN ReconfigTrigger;
#endif
...
#endif
#ifdef CONFIG_6G_AFC_SUPPORT
			struct AFC_CTRL afc_ctrl;
			struct AFC_RESPONSE_DATA afc_response_data;
#endif /*CONFIG_6G_AFC_SUPPOR*/

	INT32 noise_floor;
	/* count the per second probe resp pkt cnt */
	UINT32 probe_rsp_cnt_per_s;
	UINT32 bn_probe_rsp_drop_cnt;
#ifdef APCLI_SUPPORT
	CHAR AckCntEnable;
#endif
	/* BMC WlanIdx */
	...
	UINT32 rps_per_round;
	UINT32 rps_special_hash_en;
#endif /* CONFIG_SPECIAL_RPS_SUPPORT */

	struct mcs_nss_caps mcs_nss;

#ifdef SW_CONNECT_SUPPORT
	struct _DUMMY_WCID_OBJ dummy_obj;
#endif /* SW_CONNECT_SUPPORT */
#ifdef DFS_SLAVE_SUPPORT
	struct DFS_SLAVE_CTRL slave_ctrl;
#endif /* DFS_SLAVE_SUPPORT */
	INT radio_measure_rate;
	INT radio_measure_interval;
	BOOLEAN radio_measure_enable;
#ifdef CONFIG_MAP_SUPPORT
	UCHAR disconnect_all_sta;
#ifdef MTK_HOSTAPD_SUPPORT
	INT map_apcli_mlo_disable;
#endif
#endif
	UINT BarRetryLimitCnt;
#if (defined(CONFIG_MAP_SUPPORT) && defined(EM_PLUS_SUPPORT))
	UCHAR MapStatsDisable;
#endif
	struct ch_chg_info ch_chg_info;

	BOOLEAN assoc_record_enable;
	UCHAR last_assoc_req[MAX_MGMT_PKT_LEN];
	UCHAR last_assoc_resp[MAX_MGMT_PKT_LEN];
	UINT32 last_assoc_req_len;
	UINT32 last_assoc_resp_len;
	CHAR max_power_2g;
	CHAR max_power_5g;
};
...
#ifdef DOT11_EHT_BE
	struct common_ies cmm_ies;	/* TBD: duplicated in ie_list */
#endif /* DOT11_EHT_BE */
...
struct wifi_radio_stats_measure {
	INT radio_stats_measuring_rate;
	INT radio_stats_measuring_interval;
};
...
struct wifi_bss_stats {
	...
	UINT32 WmmBytesSent[MAX_ACCESS_CAT_NUM];
	UINT32 WmmBytesReceived[MAX_ACCESS_CAT_NUM];
};
#ifdef TR181_SUPPORT
enum AUTH_MODE {
	AUTH_None = 0,
...
struct wifi_sta_rates {
	UCHAR MacAddr[MAC_ADDR_LEN];  /* Search key, original Address for repeater */
	struct wifi_sta_txrx_rates rates;
	BOOLEAN valid;  /* if find mac, set to true, else set to false */
};
...
struct wifi_accounting_server {
	...
	UCHAR Secret[64][2]; /* 0: Primary Server Secret, 1: Secondary Server Secret */
};
#endif /* TR181_SUPPORT */
	...
	IN	BOOLEAN		FromRxDoneInt)
{
	...
	*(USHORT *)(pMacHdr + 2) = SWAP16(*(USHORT *)(pMacHdr + 2));
	...
	if (pFrame->FC.Type == FC_TYPE_MGMT) {
		...
		MTWF_DBG(pAd, DBG_CAT_RX, CATRX_CTRL, DBG_LVL_ERROR, "Invalid Frame Type!!!\n");
	...
	IN UINT size)
{
	...
	if (size % 4)
		MTWF_DBG(NULL, DBG_CAT_TX, CATTX_DATA, DBG_LVL_ERROR, "Invalid DATA Type!!!\n");
	else {
		for (i = 0; i < size / 4; i++)
			*(((UINT32 *)pData) + i) = SWAP32(*(((UINT32 *)pData) + i));
	}
...
static inline VOID ConvertMulticastMAC2IP(IN PUCHAR pMacAddr,
														IN UINT16 ProtoType,
...
struct dev_type_name_map_t {
	INT type;
	RTMP_STRING prefix[CFG_WIFI_RAM_BAND_NUM][IFNAMSIZ];
};
...
INT SetCPSupportEnable(
	IN PRTMP_ADAPTER pAd,
	IN UINT8  cp_support);


#define cp_support_is_enabled(_pAd) SetCPSupportEnable(_pAd, _pAd->cp_support)
...
VOID rtmp_read_afc_freq_range_param(RTMP_ADAPTER *pAd, RTMP_STRING *buffer);
...
#endif /* TX_AGG_ADJUST_WKR */
VOID tx_bytes_calculate(RTMP_ADAPTER *pAd, struct _TX_BLK *tx_blk);
...
VOID write_tmac_info_offload_pkt(
	...
	UCHAR sub_type,
	UCHAR *tmac_buf,
	union _HTTRANSMIT_SETTING *BeaconTransmit,
	ULONG frmLen);

#ifdef RANDOM_PKT_GEN
VOID random_write_qidx(RTMP_ADAPTER *pAd, UCHAR *buf, struct _TX_BLK *pTxBlk);
#endif
...
UINT32 mt_rx_pkt_process(RTMP_ADAPTER *, UINT8 resource_idx, struct _RX_BLK *pRxBlk, PNDIS_PACKET pRxPacket);
#endif /* !HWIFI_SUPPORT */
#endif /* MT_MAC */
...
ULONG BssSsidTableSearchByBSSID(
	IN BSS_TABLE * Tab,
	IN PUCHAR pBssid);

ULONG BssSsidTableSearchByBSSIDHiddenSSID(
	IN BSS_TABLE * Tab,
	IN PUCHAR pBssid);
...
NDIS_STATUS  MlmeQueueInitEntryHead(
	IN RTMP_ADAPTER *pAd);

VOID  MlmeQueueDestroy(
	IN struct _MLME_STRUCT *mlme);

VOID  MlmeQueueDestroyEntryHead(
	IN struct _MLME_STRUCT *mlme);
...
VOID ParseChPriority(
	IN RTMP_ADAPTER *pAd,
	IN RTMP_STRING *arg);

VOID ParseForbidBW40(
	IN RTMP_ADAPTER *pAd,
	IN RTMP_STRING *arg);
...
UCHAR is_rsne_pmkid_cache_match(
	...
	OUT INT *cacheidx);
#else
#define RTMPMaintainPMKIDCache(_ph_dev)
#endif /* DOT1X_SUPPORT */
...
UINT16 tx_pkt_classification(RTMP_ADAPTER *pAd, PNDIS_PACKET  pPacket, struct _TX_BLK *pTxBlk);
...
void ba_flush_reordering_timeout_mpdus(
	IN RTMP_ADAPTER *pAd,
	IN struct BA_REC_ENTRY *pBAEntry);
...
UINT16 cal_ori_ba_wsize(
	struct _MAC_TABLE_ENTRY *peer,
	UINT16 cfg_ori_wsize,
	UINT16 peer_rec_wsize);

UINT16 cal_rec_ba_wsize(
	struct _MAC_TABLE_ENTRY *peer,
	UINT16 cfg_rec_wsize,
	UINT16 peer_ori_wsize);
...
#endif /*CONFIG_SWITCH_CHANNEL_NO_CSA_SUPPORT*/
...
INT set_muru_trig_variant(RTMP_ADAPTER *ad, UINT8 variant);
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_set_vht_bw_signaling(RTMP_ADAPTER *pAd, u32 inf_idx, INT inf_type, UCHAR value);
#endif
...
VOID ap_ieee802_3_data_rx(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);
VOID sta_ieee802_3_data_rx(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk);

VOID indicate_rx_pkt(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx);
VOID indicate_ampdu_pkt(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx);
VOID indicate_amsdu_pkt(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx);
VOID ba_reorder(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx);
...
VOID indicate_eapol_pkt(RTMP_ADAPTER *pAd, struct _RX_BLK *pRxBlk, UCHAR wdev_idx);
...
NDIS_STATUS MATEngineInit(void *device);
NDIS_STATUS MATEngineExit(void *device);
...
BOOLEAN RTMPExpandPacketForSwEncrypt(RTMP_ADAPTER *pAd, struct _TX_BLK *pTxBlk);
INT tx_sw_encrypt(RTMP_ADAPTER *pAd, struct _TX_BLK *pTxBlk, UCHAR *buf_ptr, HEADER_802_11 *wifi_hdr);
#endif /* SOFT_ENCRYPT || SW_CONNECT_SUPPORT */

#ifdef SOFT_ENCRYPT
VOID RTMPUpdateSwCacheCipherInfo(RTMP_ADAPTER *pAd, struct _TX_BLK *pTxBlk, UCHAR *pHdr);
#endif /* SOFT_ENCRYPT */
...
INT Set_RandomMacEnable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_SequenceNum_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
#endif /* CONFIG_STA_SUPPORT */
...
VOID RtmpCleanupPsQueue(
	IN  RTMP_ADAPTER *pAd,
	IN  struct _QUEUE_HEADER   *pQueue);
...
#ifdef CONFIG_AP_SUPPORT
#ifdef CONFIG_HOTSPOT_R2
VOID hotspot_send_dls_resp(IN PRTMP_ADAPTER pAd, IN struct _RX_BLK *pRxBlk);
#endif
#endif
#ifdef CONFIG_STA_SUPPORT
...
VOID MgmtTableSetMcastEntry(RTMP_ADAPTER *pAd, UINT16 wcid, struct wifi_dev *wdev);
...
uint8_t MacTableDeleteEntry(struct _RTMP_ADAPTER *pAd, uint16_t wcid, UCHAR *pAddr);
uint8_t _MacTableDeleteEntry(struct _RTMP_ADAPTER *pAd, uint16_t wcid, UCHAR *pAddr);
...
INT SetEeprom(RTMP_ADAPTER *pAd, RTMP_STRING *Arg);
...
VOID mac_entry_delete(struct _RTMP_ADAPTER *ad, struct _MAC_TABLE_ENTRY *entry, u8 rm_now);
...
#endif /* WIFI_EAP_FEATURE */
INT SetEDCCAEnable(PRTMP_ADAPTER pAd, UINT8 u1EDCCACtrl, UINT8 u1BandIdx);
...
UINT32 Get_OBSS_AirTime_MAP(PRTMP_ADAPTER pAd, UCHAR BandIdx);
UINT32 Get_My_Tx_AirTime_MAP(PRTMP_ADAPTER pAd, UCHAR BandIdx);
UINT32 Get_My_Rx_AirTime_MAP(PRTMP_ADAPTER pAd, UCHAR BandIdx);
#endif
...
enum EDCCA_REGION_T {
	EDCCA_REGION_DEFAULT   = 0,
	EDCCA_REGION_FCC       = 1,
	EDCCA_REGION_ETSI_2017 = 2,
	EDCCA_REGION_JAPAN     = 3,
	EDCCA_REGION_ETSI_2023 = 4,
	EDCCA_REGION_NONE      = 255
};
...
void ie_filter_init(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev);
INT ie_filter_oper_proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT ie_filter_rule_proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
BOOLEAN makeup_ie(UINT16 TagNumber, struct wifi_dev *wdev, ULONG *pFrameLen, UCHAR *pFrame);
...
VOID parse_RXV_packet_v1(RTMP_ADAPTER *pAd, UINT32 Type, struct _RX_BLK *RxBlk, UCHAR *Data);
...
#ifdef RT_CFG80211_SUPPORT
INT mtk_cfg80211_Set_APAifsn_Proc(RTMP_ADAPTER *pAd, UCHAR *value);
INT mtk_cfg80211_Set_APCwmin_Proc(RTMP_ADAPTER *pAd, UCHAR *value);
INT mtk_cfg80211_Set_APCwmax_Proc(RTMP_ADAPTER *pAd, UCHAR *value);
INT mtk_cfg80211_Set_APTxop_Proc(RTMP_ADAPTER *pAd, UCHAR *value);
...
#define MLD_ENTRY_FIELD_FLAGS_NSEP	(1 << 0)
#define MLD_ENTRY_FIELD_FLAGS_EMLSR	(1 << 1)
#define MLD_ENTRY_FIELD_FLAGS_SETUP_LINK_WCID	(1 << 2)
#define MLD_ENTRY_FIELD_FLAGS_SETUP_LINK_ID	(1 << 3)
...
#if defined(RT_CFG80211_SUPPORT) && defined(DOT11_EHT_BE)
INT mtk_cfg80211_mlo_preset_link_id(
	struct _RTMP_ADAPTER *pAd, UCHAR mlo_preset_link);
#endif
...
#endif
#ifdef EAP_STATS_SUPPORT
VOID CalStaTxLatency(
	IN PMAC_TABLE_ENTRY pEntry,
	IN UINT32 transmitDelay,
	IN UINT32 airDelay,
	IN UINT32 txCnt);
#endif /* EAP_STATS_SUPPORT */
