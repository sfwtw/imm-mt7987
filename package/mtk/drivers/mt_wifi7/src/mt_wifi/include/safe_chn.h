...
#define	SAFE_CHN_START_IDX_6G	1
#define	SAFE_CHN_END_IDX_6G	233
#define MAX_80M_CHECK_BIT	14

typedef enum _ENUM_SAFE_CHN_TYPE_T {
	SAFE_CHN_TYPE_NONE = 0x0,
	SAFE_CHN_TYPE_2G4 = 0x1,
	SAFE_CHN_TYPE_5G = 0x2,
	SAFE_CHN_TYPE_6G = 0x4
...
typedef enum _ENUM_SAFE_CHN_MASK_IDX_T {
	...
	SAFE_CHN_MASK_BAND_6G = 3, /* 6G, ch1~233 */
	SAFE_CHN_MASK_IDX_NUM = 4
...
typedef struct _LTE_SAFE_CH_CTRL {
	...
	UINT_32 PwrChnChgCnt;	/* Used for stats. */
	UINT_32 ChnSwitchCnt;	/* Used for stats. */
	UINT_32 BandBanCnt;	/* Used for stats. */
	UINT_32	FailCnt;	/* Used for stats. */
	UINT_32 BandUpCnt;	/* Used for stats. */
	UINT_32 TriggerEventIntvl;	/* Used for debug */
	UINT_32	WaitForSafeChOpCnt;	/* Count for LTE event waiting for operting */
	UINT_32	WaitForPwrChOpCnt;	/* Count for LTE event waiting for operting */
	BOOLEAN bAllUnsafe;	/* TRUE/FALSE: all available channels are unsafe on the band /or not */
	/* 6g Safe channel (80M) bitmask (not filter unavailable channels) 1: safe, 0: unsafe */
	UINT_32 SafeChn6GBitmask;
	/* 6g power backoff channel (80M) bitmask (not filter unavailable channels) */
	UINT_32 PwrChn6GBitmask;
	/* Safe channel bitmask (not filter unavailable channels) 1: safe, 0: unsafe */
	UINT_64 SafeChnBitmask[SAFE_CHN_MASK_IDX_NUM];
	/* power back off Safe channel bitmask (not filter unavailable channels) */
	UINT_64 PwrChnBitmask[SAFE_CHN_MASK_IDX_NUM];
	/* All available channel bitmask (not filter unsafe channels) 1: safe, 0: unsafe */
	UINT_64 AvaChnBitmask[SAFE_CHN_MASK_IDX_NUM];
...
* @pwr_channel_bit_mask: the pwr backoff channel bit mask
*
**/
