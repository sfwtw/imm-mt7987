...
client_get_wf_reset_in_progress(void *priv_data)
{
	...
	return PD_GET_WF_RESET_IN_PROGRESS(ad->physical_dev);
}
...
client_whnat_en_get(void *priv_data)
{
	...
	return PD_GET_WHNAT_ENABLE(ad->physical_dev);
}
...
client_whnat_en_set(void *priv_data, u32 en)
{
	...
	PD_SET_WHNAT_ENABLE(ad->physical_dev, en);
}
...
dump_raw(char *str, u8 *va, u32 size)
{
	...
	for (x = 0; x < size; x++) {
		...
		snprintf(buf + len, 512 - len, "%02x ", ((u8)pt[x]));
		len = strlen(buf);
	}
...
*
*/
void client_warp_ver_notify(void *priv_data, u8 ver, u8 sub_ver, int hw_cap)
{
	struct _RTMP_ADAPTER *ad = (struct _RTMP_ADAPTER *)priv_data;
...
void client_do_wifi_reset(void *priv_data)
{
	...
	PD_SET_WF_RESET_WO_COUNT(ad->physical_dev, PD_GET_WF_RESET_WO_COUNT(ad->physical_dev) + 1);
...
void client_update_wo_rxcnt(void *priv_data, void *wo_rxcnt)
{
	...
	if (IS_WCID_VALID(ad, r_cnt->wlan_idx) && (r_cnt->tid < NUM_OF_TID)) {
		...
		if (r_cnt->wlan_idx >= WTBL_MAX_NUM(ad)) {
			proxy_print("%s(): invalid wcid=%d, ignored!\n", __func__, r_cnt->wlan_idx);
			return;
		}
		if (r_cnt->wlan_idx >= NUM_OF_TID) {
...
#ifdef TR181_SUPPORT
#ifdef STAT_ENHANCE_SUPPORT
	pEntry = entry_get(ad, r_cnt->wlan_idx);
	...
	if (pEntry->pMbss) {
		...
		mbss->RxCount += r_cnt->rx_pkt_cnt;
		mbss->ReceivedByteCount += r_cnt->rx_byte_cnt;
		mbss->RxDropCount += r_cnt->rx_drop_cnt;
		mbss->RxErrorCount += r_cnt->rx_err_cnt;
		mbss->RxCountPerAC[tid_to_ac_mapping[r_cnt->tid]] += r_cnt->rx_pkt_cnt;
		mbss->ReceivedByteCountPerAC[tid_to_ac_mapping[r_cnt->tid]] += r_cnt->rx_byte_cnt;
		mbss->RxErrorCountPerAC[tid_to_ac_mapping[r_cnt->tid]] += r_cnt->rx_err_cnt;
		mbss->RxDropCountPerAC[tid_to_ac_mapping[r_cnt->tid]] += r_cnt->rx_drop_cnt;
	}
#endif
#endif
}
...
client_cap_get(struct wifi_hw *hw, void *vad)
{
	...
	hw->sw_rx_token_num = chip_cap->tkn_info.token_rx_cnt - data_ring_num * ring_size;
...
client_driver_probe(void *ad)
{
	...
	hw = warp_alloc_client(chip_id, bus_type, slot_id, client_get_hif_dev(ad));
	if (!hw) {
