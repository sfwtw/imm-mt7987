...
{
	strlcpy(name, "Ralink STA", IFNAMSIZ);
	return 0;
}
...
{
	VOID *pAd = NULL;
	ULONG Mode = RTMP_CMD_STA_MODE_AUTO;
					  ...
					  struct iw_point *data, char *extra)
{
	...
	UINT16 val;
	int i;
	ULONG Mode = RTMP_CMD_STA_MODE_AUTO, ChannelListNum = 0;
	UCHAR *pChannel;
	UINT32 *pFreq;
	...
	struct iw_point *dwrq,	char *extra)
{
	...
	ext[len - 1] = 0x00;
	ext_len = min(dwrq->length, (UINT16)(strlen(ext) + 1));
	GET_PAD_FROM_NET_DEV(pAd, dev);
	...
	if (rtstrcasecmp(ext, "START") == TRUE) {
		...
		ret = snprintf(ext, ext_len, "rssi %d", AvgRssi0);
		if (os_snpri2dntf_error(ext_len, ret) != 0)
			MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_ERROR,
				"snprintf error\n");
	} else if (rtstrcasecmp(ext, "LINKSPEED") == TRUE)
		ret = snprintf(ext, ext_len, "LINKSPEED %d", 150);
				   ...
				   struct sockaddr *ap_addr, char *extra)
{
	...
	MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_INFO, "IOCTL::SIOCSIWAP "MACSTR"\n",
			 MAC2STR(Bssid));
	return 0;
}
					  ...
					  struct iw_point *data, char *extra)
{
	...
	os_zero_mem(pBssList, sizeof(*pBssList));
					 ...
					 struct iw_point *data, char *extra)
{
...
#endif /* ANDROID_SUPPORT */
	os_zero_mem(pIoctlScan, sizeof(*pIoctlScan));
	...
	for (i = 0; i < pIoctlScan->BssNr; i++) {
		RT_CMD_STA_IOCTL_BSS_TABLE *pBssEntry;
		...
		pBssEntry = &pIoctlScan->pBssTable[i];
			...
			if (pBssEntry->has_ht_cap) {
				...
				if (isGonly == TRUE)
					...
					if (pBssEntry->SupRateLen == 4 && pBssEntry->ExtRateLen == 0)
						ret = snprintf(iwe.u.name, IFNAMSIZ, "%s",
								"802.11b");
					else
						ret = snprintf(iwe.u.name, IFNAMSIZ, "%s",
								"802.11b/g");
		...
		if (pIoctlScan->pBssTable[i].WpaIeLen > 0) {
			...
			if (pIoctlScan->pBssTable[i].WpaIeLen > sizeof(custom))
				pIoctlScan->pBssTable[i].WpaIeLen = sizeof(custom);
		...
		if (pIoctlScan->pBssTable[i].RsnIeLen > 0) {
			memset(&iwe, 0, sizeof(iwe));
			memset(&custom[0], 0, MAX_CUSTOM_LEN);
			if (pIoctlScan->pBssTable[i].RsnIeLen >  sizeof(custom))
				pIoctlScan->pBssTable[i].RsnIeLen =  sizeof(custom);
		...
		if (pIoctlScan->pBssTable[i].WpsIeLen > 0) {
			memset(&iwe, 0, sizeof(iwe));
			memset(&custom[0], 0, MAX_CUSTOM_LEN);
			if (pIoctlScan->pBssTable[i].WpsIeLen > sizeof(custom))
				pIoctlScan->pBssTable[i].WpsIeLen = sizeof(custom);
		...
		if (pIoctlScan->pBssTable[i].WpaIeLen > 0) {
			...
			for (idx = 0; idx < pIoctlScan->pBssTable[i].WpaIeLen; idx++)
				snprintf(custom, MAX_CUSTOM_LEN, "%s%02x", custom, pIoctlScan->pBssTable[i].pWpaIe[idx]);
		...
		if (pIoctlScan->pBssTable[i].RsnIeLen > 0) {
			...
			for (idx = 0; idx < pIoctlScan->pBssTable[i].RsnIeLen; idx++)
				snprintf(custom, MAX_CUSTOM_LEN, "%s%02x", custom, pIoctlScan->pBssTable[i].pRsnIe[idx]);
		...
		if (pIoctlScan->pBssTable[i].WpsIeLen > 0) {
			...
			for (idx = 0; idx < pIoctlScan->pBssTable[i].WpsIeLen; idx++)
				snprintf(custom, MAX_CUSTOM_LEN, "%s%02x", custom, pIoctlScan->pBssTable[i].pWpsIe[idx]);
					  ...
					  struct iw_point *data, char *essid)
{
	...
	os_zero_mem(pIoctlEssid, sizeof(*pIoctlEssid));
	pIoctlEssid->pSsid = (CHAR *)essid;
	pIoctlEssid->Status = 0;
				   ...
				   struct iw_point *erq, char *key)
{
	...
	os_zero_mem(pIoctlSec, sizeof(*pIoctlSec));
							...
							UINT32 *uwrq, char *extra)
{
	...
	/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, NULL) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO, "INFO::Network is down!\n");
		return -ENETDOWN;
	}
...
static int mt_private_meta_qurey(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	...
	MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_INFO,
		"<== wrq->length = %d, wrq->pointer:%p\n", wrq->length, wrq->pointer);
	GET_PAD_FROM_NET_DEV(pAd, dev);
					 ...
					 char *extra)
{
	...
	default:
		MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_INFO, "====> Unknown Command\n");
		break;
	}
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	pIoctlWpa->flags = 0;
	pIoctlWpa->value = (UINT32)param->value; /* default */
						  ...
						  char *extra)
{
	...
	if (alg == IW_ENCODE_ALG_NONE)
		...
		MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_WARN,
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, NULL) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_INFO, "Network is down!\n");
		return -ENETDOWN;
	}
...
INT rt28xx_sta_ioctl(void *net_dev_obj, void *rq, INT cmd) /* snowpin for ap/sta */
{
	...
	if (wrqin == NULL) {
		MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_DEBUG, "INFO::wrqin is null!\n");
		return Status;
	}
	...
	case SIOCGIFHWADDR:
		MTWF_DBG(NULL, DBG_CAT_INTF, CATINTF_CTRL, DBG_LVL_INFO, "IOCTL::SIOCGIFHWADDR\n");
