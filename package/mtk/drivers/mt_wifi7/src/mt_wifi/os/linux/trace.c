...
static VOID TraceCrPseFrameInfo(RTMP_ADAPTER *pAd, UINT8 PID, UINT8 QID)
{
	...
	RTMP_IO_READ32(pAd->hdev_ctrl, C_GFF, &Value);
	Value &= ~GET_QID_MASK;
	Value |= SET_GET_QID(QID);
	Value &= ~GET_PID_MASK;
	Value |= SET_GET_PID(PID);
	RTMP_IO_WRITE32(pAd->hdev_ctrl, C_GFF, Value);
	RTMP_IO_READ32(pAd->hdev_ctrl, C_GFF, &Value);
	...
	for (DwIndex = 0; DwIndex < 8; DwIndex++) {
		RTMP_IO_READ32(pAd->hdev_ctrl, (MT_PCI_REMAP_ADDR_1 + (((CurFID) * 128)) + (DwIndex * 4)),
					   &PseData[DwIndex * 4]);
	}
	...
	while (1) {
		RTMP_IO_READ32(pAd->hdev_ctrl, C_GF, &Value);
		Value &= ~CURR_FID_MASK;
		Value |= SET_CURR_FID(CurFID);
		RTMP_IO_WRITE32(pAd->hdev_ctrl, C_GF, Value);
		RTMP_IO_READ32(pAd->hdev_ctrl, C_GF, &Value);
...
VOID TraceWtblInfo(RTMP_ADAPTER *pAd, UINT32 wtbl_idx)
{
	...
	for (idx = start_idx; idx <= end_idx; idx++) {
		...
		RTMP_IO_READ32(pAd->hdev_ctrl, addr, &wtbl_1.wtbl_1_d0.word);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 4, &wtbl_1.wtbl_1_d1.word);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 8, &wtbl_1.wtbl_1_d2.word);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 12, &wtbl_1.wtbl_1_d3.word);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 16, &wtbl_1.wtbl_1_d4.word);
		...
		for (tok = 0; tok < sizeof(struct wtbl_2_struc) / 4; tok++)
			RTMP_IO_READ32(pAd->hdev_ctrl, addr + tok * 4, &val[tok]);
		...
		RTMP_IO_READ32(pAd->hdev_ctrl, addr, &wtbl_4.ac0.word[0]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 4, &wtbl_4.ac0.word[1]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 8, &wtbl_4.ac1.word[0]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 12, &wtbl_4.ac1.word[1]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 16, &wtbl_4.ac2.word[0]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 20, &wtbl_4.ac2.word[1]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 24, &wtbl_4.ac3.word[0]);
		RTMP_IO_READ32(pAd->hdev_ctrl, addr + 28, &wtbl_4.ac3.word[1]);
...
INT32 TracePSTable(RTMP_ADAPTER *pAd, UINT32 ent_type, BOOLEAN bReptCli)
{
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (asic_get_wtbl_entry234(pAd, pEntry->wcid, &tb_entry) == FALSE)
			return FALSE;

		RTMP_IO_READ32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0] + 12, &dw3->word);
		/* get PSE register */
		...
		RTMP_IO_WRITE32(pAd->hdev_ctrl, PSE_RTA, rPseRdTabAccessReg);

		do {
			RTMP_IO_READ32(pAd->hdev_ctrl, PSE_RTA, &rPseRdTabAccessReg);
