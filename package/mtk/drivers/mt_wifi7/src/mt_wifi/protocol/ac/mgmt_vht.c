...
#define CH_40M_GROUP_MAX_NUM	ARRAY_SIZE(vht_ch_40M_6G)
#define CH_80M_GROUP_MAX_NUM	ARRAY_SIZE(vht_ch_80M_6G)
#define CH_160M_GROUP_MAX_NUM	ARRAY_SIZE(vht_ch_160M_6G)
...
struct vht_ch_layout *get_ch_array(UINT8 bw, UCHAR ch_band, INT *layout_size)
{
	switch (ch_band) {
	case CMD_CH_BAND_24G:
	case CMD_CH_BAND_5G:
		if (bw == BW_40) {
			*layout_size = ARRAY_SIZE(vht_ch_40M);
			return vht_ch_40M;
		} else if (bw == BW_80) {
			*layout_size = ARRAY_SIZE(vht_ch_80M);
			return vht_ch_80M;
		} else {
			*layout_size = ARRAY_SIZE(vht_ch_160M);
			return vht_ch_160M;
		}
	case CMD_CH_BAND_6G:
		if (bw == BW_40) {
			*layout_size = ARRAY_SIZE(vht_ch_40M_6G);
			return vht_ch_40M_6G;
		} else if (bw == BW_80) {
			*layout_size = ARRAY_SIZE(vht_ch_80M_6G);
			return vht_ch_80M_6G;
		} else {
			*layout_size = ARRAY_SIZE(vht_ch_160M_6G);
...
VOID trigger_vht_ndpa(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *entry)
{
	...
	MiniportMMRequest(pAd, 0, buf, frm_len, NULL);
	MlmeFreeMemory(buf);
#ifdef SOFT_SOUNDING
	...
	if (1) {
		...
		MiniportMMRequest(pAd, 0, NullFrame, frm_len, NULL);
	}
...
UCHAR vht_prim_ch_idx(UCHAR vht_cent_ch, UCHAR prim_ch, UINT8 rf_bw)
{
	...
	if (rf_bw == RF_BW_80) {
		while (idx < ARRAY_SIZE(vht_ch_80M) &&
				vht_ch_80M[idx].ch_up_bnd != 0) {
		...
		while (idx < ARRAY_SIZE(vht_ch_160M) &&
				vht_ch_160M[idx].ch_up_bnd != 0) {
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_DEBUG,
			"(VhtCentCh=%d, PrimCh=%d) =>BbpChIdx=%d\n",
			vht_cent_ch, prim_ch, bbp_idx);
	return bbp_idx;
}
...
{
	INT idx = 0, ch_layout_size[2] = {0};
	struct vht_ch_layout *ch_80M = get_ch_array(BW_80, ch_band, &(ch_layout_size[0]));
	struct vht_ch_layout *ch_160M = get_ch_array(BW_160, ch_band, &(ch_layout_size[1]));

	if (vht_bw == VHT_BW_2040 || ((prim_ch < 36) && ch_band == CMD_CH_BAND_5G))
		return prim_ch;
	else if ((vht_bw == VHT_BW_80) || (vht_bw == VHT_BW_8080)) {
		while ((idx >= 0 && idx < ch_layout_size[0]) &&
				ch_80M && ch_80M[idx].ch_up_bnd != 0) {
		...
		while ((idx >= 0 && idx < ch_layout_size[1]) &&
				ch_160M && ch_160M[idx].ch_up_bnd != 0) {
...
{
	INT idx = 0, ch_layout_size = 0;
	struct vht_ch_layout *ch_40M = get_ch_array(BW_40, ch_band, &ch_layout_size);

	if ((vht_bw == VHT_BW_2040) &&
		((ch_band == CMD_CH_BAND_5G) || (ch_band == CMD_CH_BAND_6G)))
		while ((idx >= 0 && idx < ch_layout_size) &&
				ch_40M && ch_40M[idx].ch_up_bnd != 0) {
...
VOID update_peer_vht_caps(RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies)
{
	...
	if ((pAd != NULL) && (wdev != NULL) && (cmm_ies != NULL)) {

		set_vht_cap(pAd, peer, &cmm_ies->vht_cap);
		NdisMoveMemory(&peer->vht_cap_ie, &cmm_ies->vht_cap, sizeof(VHT_CAP_IE));
		dot11_vht_mcs_to_internal_mcs(pAd, wdev, &cmm_ies->vht_cap, &peer->MaxHTPhyMode);
	} else {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR,
			"!!ERROR!! NULL Pointer\n");
	}
}
VOID sta_vht_mode_adjust(RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies)
{
	...
	vht_mode_adjust(pAd, peer, &cmm_ies->vht_cap, &cmm_ies->vht_op, NULL, NULL);
}
	...
	VHT_CAP_IE *cap, VHT_OP_IE *op, OPERATING_MODE *op_mode, UCHAR *bw_from_opclass)
{
	...
	/*update peer vht bw cap*/
	if (cap->vht_cap.ch_width == 0) {
		pEntry->cap.ch_bw.vht_support_ch_width_set = VHT_BW_80;
	} else
		pEntry->cap.ch_bw.vht_support_ch_width_set = VHT_BW_160;
	...
	if ((pEntry->MaxHTPhyMode.field.BW == BW_40) && (pEntry->wdev)) {
		...
		if (ht_phyinfo) {
			...
			if ((ht_phyinfo->vht_bw == VHT_BW_2040)) {
				...
				if (op != NULL) {
					if (op->vht_op_info.ch_width == 0)  /* peer VHT2040 */
	...
	if (pEntry->cap.ch_bw.ht_support_ch_width_set == BW_40) {
		if (cap->vht_cap.ch_width == 0) {
			if (op != NULL) {
				if (op->vht_op_info.ch_width == 0) {/* peer support VHT20,40 */
					pEntry->sta_vht_ch_bw = BW_40;
				} else
					pEntry->sta_vht_ch_bw = BW_80;
				} else if (op_mode != NULL) {
					if (op_mode->ch_width == 0) { /* peer support VHT20 */
						pEntry->sta_vht_ch_bw = BW_20;
					} else if (op_mode->ch_width == 1) {
						pEntry->sta_vht_ch_bw = BW_40;
					} else
						pEntry->sta_vht_ch_bw = BW_80;
				...
				/* bw160 or bw80+80 */
				pEntry->sta_vht_ch_bw = BW_160;
			}
	} else {
		/* STA doesn't support HT40 in this case*/
		pEntry->sta_vht_ch_bw = BW_20;
	}
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_INFO,
		"%s(), Max VHT peer BW = %d\n", __func__,
		pEntry->sta_vht_ch_bw);

	return TRUE;
}

INT vht_mode_adjust_sta_max_capab(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry,
	VHT_CAP_IE *cap, VHT_OP_IE *op, OPERATING_MODE *op_mode, UCHAR *bw_from_opclass)
{
	...
	if (pEntry->cap.ch_bw.ht_support_ch_width_set == BW_40) {
		...
		pEntry->sta_vht_ch_bw = BW_20; /* since STA doesn't support HT40 in this case */
...
INT dot11_vht_mcs_to_internal_mcs(
	...
	VHT_CAP_IE *vht_cap,
	union _HTTRANSMIT_SETTING *tx)
{
	union _HTTRANSMIT_SETTING *tx_mode = tx;
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss1 != VHT_MCS_CAP_NA) && (wlan_operate_get_tx_stream(wdev) >= 1)) {
		...
		if (vht_cap->mcs_set.rx_mcs_map.mcs_ss1 < ARRAY_SIZE(spec_cap_to_mcs))
			peer_cap = spec_cap_to_mcs[vht_cap->mcs_set.rx_mcs_map.mcs_ss1];
		else {
			peer_cap = spec_cap + 1;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR,
					"mcs_ss1 cap exceed array range of spec_cap_to_mcs\n");
		}
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA) && (wlan_operate_get_tx_stream(wdev) >= 2)) {
		...
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
		if (vht_cap->mcs_set.rx_mcs_map.mcs_ss2 < ARRAY_SIZE(spec_cap_to_mcs))
			peer_cap = spec_cap_to_mcs[vht_cap->mcs_set.rx_mcs_map.mcs_ss2];
		else {
			peer_cap = spec_cap + 1;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR,
				"mcs_ss2 cap exceed array range of spec_cap_to_mcs\n");
		}
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA) && (wlan_operate_get_tx_stream(wdev) >= 3)) {
		...
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
		if (vht_cap->mcs_set.rx_mcs_map.mcs_ss3 < ARRAY_SIZE(spec_cap_to_mcs))
			peer_cap = spec_cap_to_mcs[vht_cap->mcs_set.rx_mcs_map.mcs_ss3];
		else {
			peer_cap = spec_cap + 1;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR,
				"mcs_ss3 cap exceed array range of spec_cap_to_mcs\n");
		}
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA) && (wlan_operate_get_tx_stream(wdev) >= 4)) {
		...
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
		if (vht_cap->mcs_set.rx_mcs_map.mcs_ss4 < ARRAY_SIZE(spec_cap_to_mcs))
			peer_cap = spec_cap_to_mcs[vht_cap->mcs_set.rx_mcs_map.mcs_ss4];
		else {
			peer_cap = spec_cap + 1;
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR,
				"mcs_ss4 cap exceed array range of spec_cap_to_mcs\n");
		}
...
INT build_vht_oper_ie(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR Channel, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	if (wdev->wdev_type == WDEV_TYPE_AP) {
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_INFO, "bpcc_vhtop_info.status = %u\n", wdev->bpcc_vhtop_info.status);
		if (wdev->bpcc_vhtop_info.status == VHTOP_INFO_UPDATE_SUC) {
			NdisMoveMemory((UCHAR *)&wdev->bpcc_vhtop_info.vht_opinfo_orig,
				(UCHAR *)&vht_op.vht_op_info, sizeof(struct vht_opinfo));
		} else {
			NdisMoveMemory((UCHAR *)&vht_op.vht_op_info,
				(UCHAR *)&wdev->bpcc_vhtop_info.vht_opinfo_orig, sizeof(struct vht_opinfo));
		}
...
INT build_vht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	mcs_cap = MCS_NSS_CAP(pAd)->max_vht_mcs;
...
INT build_vht_ies(RTMP_ADAPTER *pAd, struct _build_ie_info *info)
{
	...
	if (
		...
		if ((info->frame_subtype == SUBTYPE_BEACON) ||
			...
			/* optional IE, FOR VHT40 STA/ApClient with op_mode ie to notify AP
			   and avoid the BW info not sync.Used in Beacon Protection test case
			 */
#ifdef BCN_PROTECTION_SUPPORT
			if (pStaCfg->MlmeAux.ExtCapInfo.operating_mode_notification &&
				(vht_bw < VHT_BW_160))
...
VOID get_vht_ch_array_from_chCtrl(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR ch_band, struct vht_ch_layout *vht_ch)
{
	UINT idx = 0, idx2 = 0, chidx = 0, ch_num = 0;
	UINT ch_layout_size = 0;
	struct vht_ch_layout *ch_layout = NULL;
	CHANNEL_CTRL *pChCtrl = hc_get_channel_ctrl(pAd->hdev_ctrl);

	if (bw == BW_40) {
		ch_num = 1;
		ch_layout = get_ch_array(BW_40, ch_band, &ch_layout_size);
	} else if (bw == BW_80) {
		ch_num = 3;
		ch_layout = get_ch_array(BW_80, ch_band, &ch_layout_size);
	} else if (bw == BW_160) {
		ch_num = 7;
		ch_layout = get_ch_array(BW_160, ch_band, &ch_layout_size);
	} else {
		return;
	}

	while (idx < ch_layout_size && ch_layout[idx].ch_up_bnd != 0
		&& idx2 < pChCtrl->ChListNum) {
		if (idx2 + ch_num >= pChCtrl->ChListNum)
			break;

		if (pChCtrl->ChList[idx2].Channel < ch_layout[idx].ch_low_bnd) {
			idx2++;
		} else if (pChCtrl->ChList[idx2].Channel > ch_layout[idx].ch_up_bnd) {
			idx++;
		} else if (pChCtrl->ChList[idx2 + ch_num].Channel <= ch_layout[idx].ch_up_bnd) {
			vht_ch[chidx].ch_low_bnd = ch_layout[idx].ch_low_bnd;
			vht_ch[chidx].ch_up_bnd = ch_layout[idx].ch_up_bnd;
			vht_ch[chidx++].cent_freq_idx = ch_layout[idx++].cent_freq_idx;
			idx2 += ch_num + 1;
		} else {
			idx2++;
		}
...
{
	INT idx = 0;
	UCHAR region = 0, groupch = 0;
	UCHAR ch_band = wlan_config_get_ch_band(wdev);
	struct vht_ch_layout ch_layout[CH_40M_GROUP_MAX_NUM] = {0}, *vht_ch = ch_layout;

	get_vht_ch_array_from_chCtrl(pAd, BW_40, ch_band, vht_ch);

	switch (ch_band) {
	case CMD_CH_BAND_5G:
		region = GetCountryRegionFromCountryCode(pAd->CommonCfg.CountryCode);
		while ((idx >= 0 && idx < CH_40M_GROUP_MAX_NUM) &&
				vht_ch[idx].ch_up_bnd != 0) {
			if (channel >= vht_ch[idx].ch_low_bnd &&
				channel <= vht_ch[idx].ch_up_bnd) {
				if ((strncmp((RTMP_STRING *) pAd->CommonCfg.CountryCode, "GB", 2) != 0)
					/*Allow United Kingdom by latest regulations:vns-203083*/
					&& (strncmp((RTMP_STRING *) pAd->CommonCfg.CountryCode, "AU", 2) != 0)
					/*Allow Australia*/
					&& region == CE && vht_ch[idx].cent_freq_idx == 142) {
					idx++;
					continue;
				}
				...
				/*check is 40M group channel valid*/
				for (groupch = vht_ch[idx].ch_low_bnd;
					groupch <= vht_ch[idx].ch_up_bnd; groupch += 4) {
...
BOOLEAN vht80_channel_group(RTMP_ADAPTER *pAd, UCHAR channel, struct wifi_dev *wdev)
{
	...
	struct vht_ch_layout ch_layout[CH_80M_GROUP_MAX_NUM] = {0}, *vht_ch = ch_layout;
	UCHAR groupch = 0;

	get_vht_ch_array_from_chCtrl(pAd, BW_80, ch_band, vht_ch);

	if (ch_band == CMD_CH_BAND_6G) {
		while (idx >= 0 && idx < CH_80M_GROUP_MAX_NUM &&
			vht_ch && vht_ch[idx].ch_up_bnd != 0) {
			if (channel >= vht_ch[idx].ch_low_bnd &&
				channel <= vht_ch[idx].ch_up_bnd) {
				/*check is 80M group channel valid*/
				for (groupch = vht_ch[idx].ch_low_bnd;
					groupch <= vht_ch[idx].ch_up_bnd; groupch += 4) {
					if (!IsValidChannel(pAd, groupch, wdev))
						return FALSE;
	...
	} else if (ch_band == CMD_CH_BAND_5G) {
		while ((idx >= 0 && idx < CH_80M_GROUP_MAX_NUM) &&
			vht_ch && vht_ch[idx].ch_up_bnd != 0) {
			if (channel >= vht_ch[idx].ch_low_bnd &&
				channel <= vht_ch[idx].ch_up_bnd) {
				...
				if ((strncmp((RTMP_STRING *) pAd->CommonCfg.CountryCode, "GB", 2) != 0)
					...
					&& (strncmp((RTMP_STRING *) pAd->CommonCfg.CountryCode, "AU", 2) != 0)
					/*Allow Australia*/
					&& vht_ch[idx].cent_freq_idx == 138 && region == CE) {
					/*Allow Japan by latest regulations*/
#endif
					idx++;
					continue;
...
{
	INT idx = 0;
	struct vht_ch_layout ch_layout[CH_160M_GROUP_MAX_NUM] = {0}, *vht_ch = ch_layout;
	UCHAR ch_band = wlan_config_get_ch_band(wdev), groupch;

	get_vht_ch_array_from_chCtrl(pAd, BW_160, ch_band, vht_ch);

	/*Weather channel not supported*/
	if (!CheckWeatherChannel(pAd, wdev, channel) && (ch_band == CMD_CH_BAND_5G))
		vht_ch = vht_ch_160M_non_weather;

	if (ch_band == CMD_CH_BAND_6G) {
		while ((idx >= 0 && idx < CH_160M_GROUP_MAX_NUM) &&
			vht_ch && vht_ch[idx].ch_up_bnd != 0) {
			if (channel >= vht_ch[idx].ch_low_bnd &&
				channel <= vht_ch[idx].ch_up_bnd) {
				/*check is 160M group channel valid*/
				for (groupch = vht_ch[idx].ch_low_bnd;
					groupch <= vht_ch[idx].ch_up_bnd; groupch += 4) {
					if (!IsValidChannel(pAd, groupch, wdev))
						return FALSE;
...
void print_vht_op_info(struct vht_opinfo *vht_op)
{
	...
	if (vht_op->ch_width < ARRAY_SIZE(ch_with))
...
UINT32 starec_vht_feature_decision(
...
UCHAR vht_bw_2_rf_bw(UCHAR vht_bw)
{
	UCHAR rf_bw = BW_20;

	if (vht_bw == VHT_BW_160 || vht_bw == VHT_BW_8080)
		rf_bw = BW_160;
	else if (vht_bw == VHT_BW_80)
		rf_bw = BW_80;
	else if (vht_bw == VHT_BW_2040)
		rf_bw = BW_40;
	else
		rf_bw = BW_20;
...
UCHAR rf_bw_2_vht_bw(UCHAR rf_bw)
{
	...
	if (rf_bw == BW_80)
		vht_bw = VHT_BW_80;
	else if ((rf_bw == BW_160) || (rf_bw > BW_8080))
		vht_bw = VHT_BW_160;
...
VOID update_peer_support_mcs(PRTMP_ADAPTER pAd, UCHAR op_mode, MAC_TABLE_ENTRY *peer, struct common_ies *cmm_ies)
{
	struct legacy_rate *rate = NULL;

	if ((pAd == NULL) || (peer == NULL) || (cmm_ies == NULL)) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_VHT, DBG_LVL_ERROR, "Pointer is NULL\n");
		return;
	}

	rate = &cmm_ies->rate;

	RTMPSetSupportMCS(pAd, op_mode, peer, rate,
#ifdef DOT11_VHT_AC
