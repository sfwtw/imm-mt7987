...
VOID wlan_config_set_ht_edca(struct wifi_dev *wdev, UINT8 param, UINT8 aci, UINT16 value)
{
	...
	if (bcn_bpcc_op_lock(wdev->sys_handle, wdev, TRUE, BCN_BPCC_EDCA) == FALSE)
		MTWF_DBG(NULL, DBG_CAT_AP, CATAP_BPCC, DBG_LVL_INFO, "bcn_bpcc_op_lock fail\n\r");
	...
	if (cfg && (aci < WMM_NUM_OF_AC)) {
		...
		case WMM_PE_ECW_MIN:
			...
			MTWF_PRINT("configure Cwmin[%d]=%d\n", aci, value);
		...
		case WMM_PE_ECW_MAX:
			...
			MTWF_PRINT("configure Cwmax[%d]=%d\n", aci, value);
	...
	if (updated_cnt) {
		...
		UpdateBeaconHandler_BPCC(wdev->sys_handle, wdev, BCN_REASON(BCN_UPDATE_IE_CHG), BCN_BPCC_EDCA, updated_cnt);
	} else
		bcn_bpcc_op_unlock(wdev->sys_handle, wdev, TRUE);
	return;

err:
	bcn_bpcc_op_unlock(wdev->sys_handle, wdev, TRUE);
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "invalid input!!\n");
}
...
VOID wlan_config_set_ba_txrx_wsize(struct wifi_dev *wdev,
		UINT16 tx_wsize, UINT16 rx_wsize, UINT16 max_ba_wsize)
{
	struct wlan_config *cfg = NULL;
	/* TX */
	if ((tx_wsize == 0) || (tx_wsize > max_ba_wsize))
		tx_wsize = max_ba_wsize;
	/* RX */
	if ((rx_wsize == 0) || (rx_wsize > max_ba_wsize))
		rx_wsize = max_ba_wsize;
...
UINT8 wlan_config_get_ba_enable(struct wifi_dev *wdev)
{
	...
	if (wdev) {
		...
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "NULL wdev\n");
