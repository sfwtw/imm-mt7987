	...
	OUT USHORT * pStatus)
{
	...
	*pStatus = pFtRspFrame->StatusCode;
	...
	while ((Length > 0) && (Length >= (2 + pEid->Len))) {
		...
		case IE_FT_FTIE:
			if (parse_ft_ie(pEid)) {
				NdisMoveMemory(pFtIe, pEid->Octet, sizeof(FT_FTIE));
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_NOTICE,
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!MlmeFtReqSanity
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
			"FT_OTD_ACTION - sanity check failed\n");
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
				 "FT_OTD_ACTION :allocate memory failed\n");
		return;
	}
	...
	/* MDIE */
	FT_InsertMdIE(pOutBuffer + FrameLen, &FrameLen, MdIe.MdId,
				  MdIe.FtCapPlc);

	/* Process with RSN */
	if (pAd->StaCfg[0].AuthMode >= Ndis802_11AuthModeWPA)
	...
	if (PeerFtRspSanity
		...
		if (MAC_ADDR_EQUAL(pAd->CommonCfg.Bssid, pFrame->Hdr.Addr2)) {
			...
			if (Status == MLME_SUCCESS) {
				UINT8 ptk_len;
				PMAC_TABLE_ENTRY pEntry = entry_get(pAd, MCAST_WCID);
				...
				FT_DerivePMKR1(pAd->StaCfg[0].Dot11RCommInfo.PMKR0, pAd->StaCfg[0].Dot11RCommInfo.PMKR0Name, TargetAddr,	/*pAd->MlmeAux.Bssid, */
							   pAd->CurrentAddress,
							   pEntry->FT_PMK_R1,
				...
				if ((pAd->StaCfg[0].Dot11RCommInfo.bSupportResource)
					...
					pAd->StaCfg[0].Dot11RCommInfo.FtRspSuccess = FT_OTD_RESPONSE;
					 ...
					 */
					BssIdx = BssTableSearch(ScanTab, TargetAddr, 1);

					/* search 5G band, if AP does not exist in 2.4G band */
					if (BssIdx == BSS_NOT_FOUND)
						BssIdx = BssTableSearch(ScanTab, TargetAddr, 36);

					if (BssIdx == BSS_NOT_FOUND) {
						MTWF_DBG(NULL,
							DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO,
					...
					wlan_operate_set_prim_ch(pEntry->wdev, pAd->MlmeAux.Channel);
					RTMPUpdateMlmeRate(pAd pEntry->wdev);
					AssocParmFill(pAd, &AssocReq,
								  pAd->MlmeAux.Bssid,
								  pAd->MlmeAux.CapabilityInfo,
								  ASSOC_TIMEOUT,
								  pAd->StaCfg[0].DefaultListenCount);
					MlmeEnqueue(pAd, ASSOC_FSM,
				...
				pAd->StaCfg[0].AuthFailReason = Status;
				COPY_MAC_ADDR(pAd->StaCfg[0].AuthFailSta, pFrame->Hdr.Addr2);
				pAd->Mlme.FtOtdActMachine.CurrState = FT_OTD_IDLE;
