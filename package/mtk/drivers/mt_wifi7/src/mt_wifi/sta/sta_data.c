...
{
	INT allowToSend = NDIS_STATUS_FAILURE;
	...
	if ((!pSrcBufVA) || (SrcBufLen <= 14)) {
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_DATA, DBG_LVL_ERROR, "pkt error(%p, %d)\n",
				 pSrcBufVA, SrcBufLen);
	...
	if ((pStaCfg->BssType == BSS_ADHOC) && ADHOC_ON(pAd)) {
		if (MAC_ADDR_IS_GROUP(pSrcBufVA)) {
			...
			wcid = wdev->tr_tb_idx;
			allowToSend = NDIS_STATUS_SUCCESS;
		} else {
	...
	if (!RTMPCheckEtherType(pAd, pPacket, tr_entry, wdev))
		return NDIS_STATUS_FAILURE;
...
VOID sta_find_cipher_algorithm(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bClearEAPFrame)) {
		...
		pTxBlk->pKey = NULL;
		if ((pTxBlk->KeyIdx < SEC_KEY_NUM) && IS_CIPHER_WEP(wdev->SecConfig.GroupCipher))
		...
		if ((pTxBlk->KeyIdx < SEC_KEY_NUM) && IS_CIPHER_WEP(pMacEntry->SecConfig.PairwiseCipher))
			pTxBlk->pKey =  (PCIPHER_KEY)&(pMacEntry->SecConfig.WepKey[pTxBlk->KeyIdx]);
	}
}

#ifdef DOT11_N_SUPPORT
VOID sta_build_cache_802_11_header(RTMP_ADAPTER *pAd, struct _TX_BLK *pTxBlk, UCHAR *buf)
{
...
VOID sta_build_802_11_header(RTMP_ADAPTER *pAd, struct _TX_BLK *pTxBlk)
{
	HEADER_802_11 *wifi_hdr;
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	UINT8 tx_hw_hdr_len = cap->tx_hw_hdr_len;
...
INT sta_ieee_802_3_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_DEBUG,
		"wcid=%d, wdev_idx=%d, pRxBlk->Flags=0x%x, fRX_AP/STA/ADHOC=0x%x/0x%x/0x%x",
	...
	MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_DEBUG,
		", Type/SubType=%d/%d, FrmDS/ToDS=%d/%d\n",
	...
	if (rx_chk_duplicate_frame(pAd, pRxBlk, wdev) == NDIS_STATUS_FAILURE) {
		MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_3D, DBG_LVL_INFO,
			"%s(): duplicate frame drop it!\n", __func__);
		return FALSE;
	}
...
INT sta_ieee_802_11_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	wdev_idx = wdev->wdev_idx;
	MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_11D, DBG_LVL_DEBUG,
		"wcid=%d, wdev_idx=%d, pRxBlk->Flags=0x%x, fRX_AP/STA/ADHOC=0x%x/0x%x/0x%x",
		pEntry->wcid, wdev->wdev_idx,
		pRxBlk->Flags,
		RX_BLK_TEST_FLAG(pRxBlk, fRX_AP),
	...
	/*check if duplicate frame, ignore it and then drop*/
	if (rx_chk_duplicate_frame(pAd, pRxBlk, wdev) == NDIS_STATUS_FAILURE) {
		MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_11D, DBG_LVL_INFO,
	...
	if (pFmeCtrl->SubType & 0x04) { /* bit 2 : no DATA */
		MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_11D, DBG_LVL_DEBUG, "Null/QosNull frame!\n");
		drop_err = FALSE;
		return FALSE;
	}
		...
		if ((pFmeCtrl->Wep == 1) && (pRxInfo->Decrypted == 0)) {
...
#endif /* HDR_TRANS_SUPPORT */
			{
				...
				if (IS_ENTRY_PEER_AP(pEntry)) {
					...
					if (!pSwKey) {
						MTWF_DBG(NULL, DBG_CAT_RX,
						CATRX_802_11D, DBG_LVL_DEBUG,
						"No valid cipher key for SW decryption!!!\n");
	...
	MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_11D, DBG_LVL_DEBUG,
	"pRxInfo->Bcast=%d,pRxInfo->Mcast=%d,pRxBlk->Flags=0x%x,fRX_AP/STA/ADHOC=0x%x/0x%x/0x%x\n",
	pRxInfo->Bcast, pRxInfo->Mcast, pRxBlk->Flags,
	RX_BLK_TEST_FLAG(pRxBlk, fRX_AP),
	RX_BLK_TEST_FLAG(pRxBlk, fRX_STA),
	RX_BLK_TEST_FLAG(pRxBlk, fRX_ADHOC));
	...
	if (pRxInfo->Bcast || pRxInfo->Mcast) {
		...
		if (pFmeCtrl->MoreFrag) {
			MTWF_DBG(NULL, DBG_CAT_RX, CATRX_802_11D, DBG_LVL_DEBUG, "MoreFrag!\n");
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return TRUE;
		}
	...
	if (pRxInfo->U2M) {
		...
		if (pAd->DiagStruct.inited) {
			...
			if ((pAd->DiagStruct.ArrayCurIdx >= 0) && (pAd->DiagStruct.ArrayCurIdx < DIAGNOSE_TIME)) {
...
INT sta_amsdu_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *tx_blk)
{
	struct wifi_dev_ops *wdev_ops = wdev->wdev_ops;
...
err:
	do {
		RELEASE_NDIS_PACKET(pAd, ((void *)q_entry), NDIS_STATUS_SUCCESS);
...
INT sta_legacy_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	...
	pAd->WlanCounters.TxTotByteCount.QuadPart += pTxBlk->SrcBufLen;
#endif
...
INT sta_frag_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	...
	if (IS_CIPHER_TKIP(pTxBlk->CipherAlg)) {
		...
		RTMP_QueryPacketInfo(pTxBlk->pPacket, pTxBlk->pSrcBufHeader, pTxBlk->SrcBufLen);
	}
...
BOOLEAN sta_fill_non_offload_tx_blk(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	PNDIS_PACKET pPacket;
...
#endif /* XLINK_SUPPORT */
		{
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
			{
				...
				if ((pAd->EthConvert.ECMode & ETH_CONVERT_MODE_DONGLE) &&
					...
					if (!NdisEqualMemory(pAd->CurrentAddress, (GET_OS_PKT_DATAPTR(pPacket) + 6), MAC_ADDR_LEN)) {
						TX_BLK_SET_FLAG(pTxBlk, fTX_bDonglePkt);
						MTWF_DBG(NULL, DBG_CAT_TX,
							CATTX_DATA, DBG_LVL_DEBUG,
							"%s(): Dongle Packet)\n", __func__);
					}
...
BOOLEAN sta_fill_offload_tx_blk(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *pTxBlk)
{
	PNDIS_PACKET pPacket;
#ifdef ETH_CONVERT_SUPPORT
	PUCHAR pSrcBufVA = NULL;
	...
	if ((pAd->EthConvert.ECMode & ETH_CONVERT_MODE_DONGLE) &&
		...
		if (!NdisEqualMemory(pAd->CurrentAddress, (GET_OS_PKT_DATAPTR(pPacket) + 6), MAC_ADDR_LEN)) {
			TX_BLK_SET_FLAG(pTxBlk, fTX_bDonglePkt);
			MTWF_DBG(NULL, DBG_CAT_TX, CATTX_DATA, DBG_LVL_DEBUG, "Dongle Packet\n");
		}
...
INT sta_mlme_mgmtq_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, struct _TX_BLK *tx_blk)
{
	...
	if (pHeader_802_11->FC.Type == FC_TYPE_MGMT) {
		...
		default:
			MTWF_DBG(pAd, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_ERROR,
			"Unexpected frame(Type=%d, SubType=%d) send to MgmtRing\n",
		...
		if (pMacEntry)
			tr_entry = tr_entry_get(pAd, wcid);
		if (pMacEntry && tr_entry && tr_entry->PsDeQWaitCnt)
			PID = PID_PS_DATA;
		...
		MTWF_DBG(pAd, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_ERROR,
			"Unexpected frame send to MgmtRing, need to assign the length!\n");
	}
	...
	MTWF_DBG(NULL, DBG_CAT_TX, CATTX_MGMT, DBG_LVL_DEBUG,
			"WMMSET=%d,QId=%d\n",
			mac_info.wmm_set, mac_info.q_idx);
	...
	*/
	...
	if (pHeader_802_11->FC.Type == FC_TYPE_MGMT) {
		...
		if (pMacEntry)
			tr_entry = tr_entry_get(pAd, wcid);
		if (IS_WCID_VALID(pAd, wcid) &&
				pMacEntry && tr_entry && tr_entry->PsDeQWaitCnt)
