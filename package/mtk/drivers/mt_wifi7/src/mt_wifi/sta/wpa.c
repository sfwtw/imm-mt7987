	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	os_alloc_mem(NULL, (PUCHAR *)&pPacket, TX_EAPOL_BUFFER);

	if (pPacket == NULL) {
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_ERROR, "!!!no memory!!!\n");
		return;
	}
	...
	*((USHORT *)&pPacket->KeyDesc.KeyInfo) = cpu2le16(*((USHORT *)&pPacket->KeyDesc.KeyInfo));
	 ...
	 */
	len = CONV_ARRARY_TO_UINT16(pPacket->Body_Len) + 4;
	if (len != MIN_LEN_OF_EAPOL_KEY_MSG) {
		os_free_mem(pPacket);
		MlmeFreeMemory((PUCHAR)pOutBuffer);
		return;
	}
	MakeOutgoingFrame(pOutBuffer, &FrameLen, len, pPacket, END_OF_ARGS);
...
VOID WpaStaPairwiseKeySetting(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_INFO, "AID(%d) port secured\n", pEntry->Aid);
}
...
VOID WpaStaGroupKeySetting(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (wdev->SecConfig.GroupKeyId > 3) {
		MTWF_DBG(pAd, DBG_CAT_SEC, CATSEC_KEY, DBG_LVL_ERROR, "!!! DefaultKeyId > 3 !!!\n");
		return;
	}
