...
enum {
	PAO_PRT_PLD_HIF_TXD_V0_0 = 0,
	PAO_PRT_PLD_HIF_TXD_V0_1 = 1,
	PAO_PRT_PLD_HIF_TXD_V1_0 = 2,
};

enum {
	PAO_NON_PRT_PLD_HIF_TXD_V2 = 0,
	PAO_NON_PRT_PLD_HIF_TXD_V3 = 1,
};
...
static void
warp_pao_set_prt_pld_hif_version(struct wed_entry *wed,
				struct wifi_entry *wifi)
{
	struct wifi_hw *hw = &wifi->hw;
	u8 hif_txd_ver = PAO_PRT_PLD_HIF_TXD_V1_0;
	u32 value;

	warp_io_read32(wed, WED_PAO_HIFTXD_ADDR, &value);

	if (hw->hif_txd_ver & BIT(WIFI_HIF_TXD_V1_0))
		hif_txd_ver = PAO_PRT_PLD_HIF_TXD_V1_0;
	else if (hw->hif_txd_ver & BIT(WIFI_HIF_TXD_V0_1))
		hif_txd_ver = PAO_PRT_PLD_HIF_TXD_V0_1;
	else if (hw->hif_txd_ver & BIT(WIFI_HIF_TXD_V0_0))
		hif_txd_ver = PAO_PRT_PLD_HIF_TXD_V0_0;

	value &= ~WED_PAO_HIFTXD_w_prt_pld_hif_version_MASK;
	value |= SET_FIELD(WED_PAO_HIFTXD_w_prt_pld_hif_version, hif_txd_ver);

	warp_io_write32(wed, WED_PAO_HIFTXD_ADDR, value);
}

static void
warp_pao_set_non_prt_pld_hif_version(struct wed_entry *wed,
				struct wifi_entry *wifi)
{
	struct wifi_hw *hw = &wifi->hw;
	u8 hif_txd_ver = PAO_NON_PRT_PLD_HIF_TXD_V2;
	u32 value;

	warp_io_read32(wed, WED_PAO_HIFTXD_ADDR, &value);

	if (hw->hif_txd_ver & BIT(WIFI_HIF_TXD_V3_0))
		hif_txd_ver = PAO_NON_PRT_PLD_HIF_TXD_V3;

	value &= ~WED_PAO_HIFTXD_wo_prt_pld_hif_version_MASK;
	value |= SET_FIELD(WED_PAO_HIFTXD_wo_prt_pld_hif_version, hif_txd_ver);
...
warp_pao_init_hw(struct wed_entry *wed, struct wifi_entry *wifi)
{
	...
	/* Set HIF_TXD version for both types */
	warp_pao_set_prt_pld_hif_version(wed, wifi);
	warp_pao_set_non_prt_pld_hif_version(wed, wifi);
