...
static int set_pwm_freq(struct pwm_fan_ctx *ctx, unsigned int freq)
{
	struct pwm_state *state = &ctx->pwm_state;
	int ret;
	...
	/* Calculate new period from frequency */
	state->period = NSEC_PER_SEC / freq;
	
	/* Check if the new period is valid */
	if (state->period > ULONG_MAX / MAX_PWM + 1) {
		mutex_unlock(&ctx->lock);
		return -EINVAL;
	}

	/* Recalculate duty cycle to maintain the same PWM ratio */
	if (ctx->pwm_value > 0) {
		state->duty_cycle = DIV_ROUND_UP(ctx->pwm_value * (state->period - 1), MAX_PWM);
	}

	/* Apply the new PWM configuration */
	ret = pwm_apply_might_sleep(ctx->pwm, state);
	if (!ret) {
		ctx->pwm_freq = freq;
		/* If PWM was enabled, ensure it stays enabled with new frequency */
		if (ctx->enabled && ctx->pwm_value > 0) {
			ret = pwm_fan_power_on(ctx);
		}
	}
...
static int pwm_fan_probe(struct platform_device *pdev)
{
	...
	/* Store initial PWM frequency */
	ctx->pwm_freq = NSEC_PER_SEC / ctx->pwm_state.period;
	...
	/*
	 * Set initial duty cycle to 0 (fan stopped)
	 */
	ret = set_pwm(ctx, 0);
	if (ret) {
	...
	channels[0] = HWMON_CHANNEL_INFO(pwm, HWMON_PWM_INPUT | HWMON_PWM_ENABLE | HWMON_PWM_FREQ);
